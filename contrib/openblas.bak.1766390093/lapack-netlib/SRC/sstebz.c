...
/* Computing MAX */
	r__3 = abs(d__[1]) + abs(e[1]), r__4 = (r__1 = d__[*n], abs(r__1)) + (
		r__2 = e[*n - 1], abs(r__2));
	tnorm = f2cmax(r__3,r__4);

	i__1 = *n - 1;
	for (j = 2; j <= i__1; ++j) {
/* Computing MAX */
	    r__4 = tnorm, r__5 = (r__1 = d__[j], abs(r__1)) + (r__2 = e[j - 1]
		    , abs(r__2)) + (r__3 = e[j], abs(r__3));
	    tnorm = f2cmax(r__4,r__5);
/* L30: */
	}
