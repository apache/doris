...
/*              .    unnecessary. ==== */
		...
		if (k >= *ktop) {
			...
			tst1 = (r__1 = h__[k + k * h_dim1], abs(r__1)) + (
				r__2 = h__[k + 1 + (k + 1) * h_dim1], abs(
				r__2));
			if (tst1 == 0.f) {
			    if (k >= *ktop + 1) {
				tst1 += (r__1 = h__[k + (k - 1) * h_dim1], 
					abs(r__1));
			    }
			    if (k >= *ktop + 2) {
				tst1 += (r__1 = h__[k + (k - 2) * h_dim1], 
					abs(r__1));
			    }
			    if (k >= *ktop + 3) {
				tst1 += (r__1 = h__[k + (k - 3) * h_dim1], 
					abs(r__1));
			    }
			    if (k <= *kbot - 2) {
...
/*                    .    Otherwise, use the new one. ==== */
			...
			if ((r__1 = h__[k + 2 + k * h_dim1] - refsum * vt[1], 
				abs(r__1)) + (r__2 = refsum * vt[2], abs(r__2)
				) > ulp * ((r__3 = h__[k + k * h_dim1], abs(
				r__3)) + (r__4 = h__[k + 1 + (k + 1) * h_dim1]
				, abs(r__4)) + (r__5 = h__[k + 2 + (k + 2) * 
				h_dim1], abs(r__5)))) {
...
/* Computing MAX */
		    r__2 = smlnum, r__3 = ulp * tst1;
		    if ((r__1 = h__[k + 1 + k * h_dim1], abs(r__1)) <= f2cmax(
			    r__2,r__3)) {
/* Computing MAX */
			r__3 = (r__1 = h__[k + 1 + k * h_dim1], abs(r__1)), 
				r__4 = (r__2 = h__[k + (k + 1) * h_dim1], abs(
				r__2));
			h12 = f2cmax(r__3,r__4);
/* Computing MIN */
			r__3 = (r__1 = h__[k + 1 + k * h_dim1], abs(r__1)), 
...
/* Computing MIN */
		...
		sgemm_("C", "N", &nu, &jlen, &nu, &c_b8, &u[k1 + k1 * u_dim1],
			 ldu, &h__[incol + k1 + jcol * h_dim1], ldh, &c_b7, &
			wh[wh_offset], ldwh);
		slacpy_("ALL", &nu, &jlen, &wh[wh_offset], ldwh, &h__[incol + 
			k1 + jcol * h_dim1], ldh);
/* L150: */
	    }
...
/* Computing MIN */
		...
		jlen = f2cmin(i__7,i__4);
		sgemm_("N", "N", &jlen, &nu, &nu, &c_b8, &h__[jrow + (incol + 
			k1) * h_dim1], ldh, &u[k1 + k1 * u_dim1], ldu, &c_b7, 
			&wv[wv_offset], ldwv);
		slacpy_("ALL", &jlen, &nu, &wv[wv_offset], ldwv, &h__[jrow + (
			incol + k1) * h_dim1], ldh);
/* L160: */
	    }
