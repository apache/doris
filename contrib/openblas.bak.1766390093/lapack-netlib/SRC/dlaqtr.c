	...
	*scale, doublereal *x, doublereal *work, integer *info)
{
    ...
    doublereal bignum;
    logical notran;
    doublereal smlnum, rec, eps, tjj, tmp;
...
/*     Set constants to control overflow */
    ...
    if (! (*lreal)) {
/* Computing MAX */
	d__1 = xnorm, d__2 = abs(*w), d__1 = f2cmax(d__1,d__2), d__2 = dlange_(
		"M", n, &c__1, &b[1], n, d__);
...
/* Computing MAX */
			...
			if (f2cmax(d__1,d__2) > (bignum - xmax) * rec) {
			    dscal_(n, &rec, &x[1], &c__1);
			    *scale *= rec;
			}
		    }
...
/* Computing MAX */
			...
			if (f2cmax(d__1,d__2) > (bignum - xj) * rec) {
			    dscal_(n, &rec, &x[1], &c__1);
			    *scale *= rec;
			    xmax *= rec;
			}
...
/* Computing MAX */
		    d__3 = (d__1 = x[j1], abs(d__1)), d__4 = (d__2 = x[j2], 
			    abs(d__2)), d__3 = f2cmax(d__3,d__4);
		    xmax = f2cmax(d__3,xmax);
...
/* Computing MAX */
			d__1 = work[j1], d__2 = work[j2];
			if (f2cmax(d__1,d__2) > (bignum - xj) / xmax) {
			    dscal_(&n2, &rec, &x[1], &c__1);
			    *scale *= rec;
			    xmax *= rec;
			}
...
/* Computing MAX */
		    d__5 = (d__1 = x[j1], abs(d__1)) + (d__2 = x[*n + j1], 
			    abs(d__2)), d__6 = (d__3 = x[j2], abs(d__3)) + (
			    d__4 = x[*n + j2], abs(d__4)), d__5 = f2cmax(d__5,
			    d__6);
		    xmax = f2cmax(d__5,xmax);
