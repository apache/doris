...
/*        Panel factorization */
	...
	i__1 = 2 - k1;
	i__2 = *n - j;
	clahef_aa_(uplo, &i__1, &i__2, &jb, &a[f2cmax(1,j) + (j + 1) * a_dim1], 
...
/*                 Update (J2, J2) diagonal block with CGEMV */
			...
			cgemm_("Conjugate transpose", "Transpose", &c__1, &mj,
				 &i__3, &q__1, &a[j1 - k2 + j3 * a_dim1], lda,
...
/*                 Update off-diagonal block of J2-th block row with CGEMM */
		    ...
		    cgemm_("Conjugate transpose", "Transpose", &nj, &i__3, &
			    i__4, &q__1, &a[j1 - k2 + j2 * a_dim1], lda, &
...
/*        Panel factorization */
	...
	i__2 = 2 - k1;
	i__1 = *n - j;
	clahef_aa_(uplo, &i__2, &i__1, &jb, &a[j + 1 + f2cmax(1,j) * a_dim1], 
		lda, &ipiv[j + 1], &work[1], n, &work[*n * nb + 1])
		;
...
/*                 Update (J2, J2) diagonal block with CGEMV */
			...
			cgemm_("No transpose", "Conjugate transpose", &mj, &
				c__1, &i__3, &q__1, &work[j3 - j1 + 1 + k1 * *
...
/*                 Update off-diagonal block of J2-th block column with CGEMM */
		    ...
		    cgemm_("No transpose", "Conjugate transpose", &i__3, &nj, 
			    &i__4, &q__1, &work[j3 - j1 + 1 + k1 * *n], n, &a[
