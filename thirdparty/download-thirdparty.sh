...
if [[ " ${TP_ARCHIVES[*]} " =~ " PAIMON_CPP " ]]; then
    PAIMON_CPP_PATCHED_MARK="patched_mark_arrow_24_compute"
    if ! [[ -f "${TP_SOURCE_DIR}/${PAIMON_CPP_SOURCE}/${PATCHED_MARK}" &&
        -f "${TP_SOURCE_DIR}/${PAIMON_CPP_SOURCE}/${PAIMON_CPP_PATCHED_MARK}" ]]; then
        rm -rf "${TP_SOURCE_DIR:?}/${PAIMON_CPP_SOURCE}"
        "${TAR_CMD}" xzf "${TP_SOURCE_DIR}/${PAIMON_CPP_NAME}" -C "${TP_SOURCE_DIR}/"
