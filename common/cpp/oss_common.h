...
enum class OSSCredProviderType {
    ...
    DEFAULT = 2,          // Default credential provider chain
    ENV = 3,              // OSS_ACCESS_KEY_ID / OSS_ACCESS_KEY_SECRET env vars
    ANONYMOUS = 4,        // No credentials — public bucket access only
};

// Normalize OSS endpoint by ensuring it has a scheme (http:// or https://).
// Defaults to HTTPS if no scheme is specified.
