...
namespace doris {
    ...
    // full URI, which is a complete URL, and an authorization token file that the kubelet rotates
    // in place. A runtime never sets both URIs, so code that reads only one of them silently gets
    ...
    // Builds the credentials provider that talks to a container runtime's credentials endpoint,
    // from the four AWS_CONTAINER_* environment variables above. Every caller that asks for
    // container credentials - the default provider chain, and the explicit CONTAINER/ECS provider
    // modes of the BE S3 client factory, the cloud recycler and the Kafka MSK IAM signer - has to
    // support both ECS and EKS Pod Identity, so none of them can afford to read a subset of the
    // variables.
    std::shared_ptr<Aws::Auth::AWSCredentialsProvider> create_container_credentials_provider();

    // True when the runtime advertises a container credentials endpoint through either URI
    // variable.
