...
// Generic commit message payload used for external table sinks like Paimon.
// The binary payload is usually generated by the corresponding external writer
// (for example, paimon-cpp CommitMessage::SerializeList output).
struct TCommitMessage {
    1: optional binary payload
}

enum TPaimonWriteShuffleMode {
    PAIMON_SHUFFLE_RANDOM = 0,
    PAIMON_SHUFFLE_PARTITION = 1,
    PAIMON_SHUFFLE_BUCKET = 2,
}

// Paimon append-table sink description. It carries the minimal context
// required by BE to construct its writer and route rows under fixed-bucket
// semantics.
struct TPaimonTableSink {
    // External catalog & table identifiers
    1: optional string catalog_name
    2: optional string db_name
    3: optional string tb_name

    // Root location (usually the table path under Paimon warehouse)
    4: optional string table_location

    // Paimon/FS options copied from catalog properties, passed down so that
    // BE paimon-cpp can reconstruct catalog/table context.
    5: optional map<string, string> options

    // Partition & bucket routing info used by BE sink to build WriteKey.
    // Names must match the output column names seen by BE.
    6: optional list<string> partition_keys
    7: optional list<string> bucket_keys
    8: optional i32 bucket_num
    9: optional TPaimonWriteShuffleMode shuffle_mode
    // Explicitly pass column names because PipelineX may strip them from Block
    10: optional list<string> column_names
    11: optional string serialized_table
}
