...
在两阶段提交方案中，publish阶段每个tablet独立完成以下流水线：
...
- 在两阶段提交方案中，虽然查询通过visible version过滤（不会看到version > visible_version的rowset），但为了防御性设计和简化推理，仍然保持 "先delete bitmap，后formal rowset" 的顺序
...
2. **recycle_txn_key尚未创建**：在两阶段提交方案中，`recycle_txn_key` 是在轻量级publish阶段才创建的。在那之前，recycler的事务回收流程不会触及这些rowset
