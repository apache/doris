...
4. **导入参数持久化到TxnInfoPB**：整个导入只需持久化一次，不需要per-tablet持久化
5. **Delete bitmap更新用现有RPC**，rowset转正用新RPC
6. **启用两阶段提交的表不再需要lazy commit和大事务机制**
7. **TXN_STATUS_COMMITTED语义变更**：在启用两阶段提交的表上，COMMITTED表示两阶段commit完成（而非lazy commit的含义）
8. **仅限新表**，通过表属性控制是否启用
...
FE Publish Daemon（一个调度线程 + 线程池执行）从 committed txns set 取出事务
  │
  ├── tablet1: FE下发CalcDeleteBitmapTask到BE
