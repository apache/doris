...
本模块设计BE侧`CalcDeleteBitmapTask`的改造方案。在异步发布模式下，FE按BE分组下发任务（每个BE一个请求，包含该BE上所有涉及的tablet），BE收到后对每个tablet**独立完成完整流水线**：
...
- **所有老代码保留**，通过`enable_mow_async_publish`标志分叉
...
FE在`TCalcDeleteBitmapRequest`中需要新增一个标志位，标识当前请求是异步发布模式：

```thrift
struct TCalcDeleteBitmapRequest {
    // ... 现有字段 ...
    // 新增：是否为异步发布模式
    optional bool enable_mow_async_publish;
...
注意：`max_version_unlocked()`方法（`cloud_tablet.h`第170行）直接返回`_max_version`，不需要持锁。但在异步发布的流程中，所有对`_max_version`的读取都在`rowset_update_lock`保护范围内，因此不会有并发问题。

### C.3 和查询的交互

**查询读取路径**：查询通过FE分配的visible version读取数据。在异步发布方案中：
...
在异步发布方案中，cache miss的处理得到了增强：

**关键改进：导入参数已持久化到MS的TxnInfoPB中**

在Commit阶段（Module 4），coordinator（FE或BE）将导入参数（`TOlapTableSchemaParam`）Thrift序列化后写入`TxnInfoPB.load_schema_param`，持久化到MS。因此：

```
当前方案:  导入参数只存在于BE的txn_delete_bitmap_cache内存中
新方案:    导入参数持久化在MS的TxnInfoPB.load_schema_param中（TOlapTableSchemaParam Thrift bytes）
```
...
if (status.is<ErrorCode::NOT_FOUND>()) {
    ...
    // 2. 从TxnInfoPB.load_schema_param反序列化TOlapTableSchemaParam，构造partial_update_info
    TOlapTableSchemaParam schema_param;
    ThriftDeserializer deser;
    deser.deserialize(&schema_param,
        (const uint8_t*)txn_info.load_schema_param().data(),
        txn_info.load_schema_param().size());
    partial_update_info = build_partial_update_info(schema_param);
...
| 文件路径 | 修改内容 |
|---------|---------|
| `be/src/cloud/cloud_engine_calc_delete_bitmap_task.h` | `CloudTabletCalcDeleteBitmapTask`新增成员：`_enable_mow_async_publish`标志、tablet元信息（db_id, table_id, partition_id）。新增方法：`_convert_tmp_rowset()`、`_apply_rowset_locally()` |
| `be/src/cloud/cloud_engine_calc_delete_bitmap_task.cpp` | `handle()`方法增加异步发布逻辑分支：在delete bitmap计算完成后调用MS convert_tmp_rowset + 本地apply。增加已转正rowset的检测逻辑（处理部分成功重试场景） |
...
| 文件 | 修改内容 |
|------|---------|
| `gensrc/thrift/AgentService.thrift` 或相关文件 | `TCalcDeleteBitmapRequest`新增`enable_mow_async_publish`字段和`tablet_meta_infos`字段 |
...
| `TxnInfoPB.load_schema_param` | cache miss时从MS获取导入参数（反序列化为 `TOlapTableSchemaParam`） |
...
public:
    ...
    // 新增：设置异步发布模式
    void set_mow_async_publish(bool enable) { _enable_mow_async_publish = enable; }
...
1. **正常异步发布流程**：
   - Mock一个完整的CalcDeleteBitmapTask请求（`enable_mow_async_publish=true`）
   - 验证delete bitmap计算正确
...
2. **非异步发布不执行新步骤**：
   - 请求中`enable_mow_async_publish=false`或未设置
...
4. **空rowset跳过**：
   - 验证空rowset（`is_empty_rowset`返回true）在异步发布模式下也被正确跳过
...
18. **性能基准**：
    - 测量异步发布模式下单tablet处理时间（包含MS RPC和本地apply）
    - 对比当前方案的处理时间
    - 新增的开销应主要是convert_tmp_rowset RPC（一次网络往返）

19. **非异步发布表无回归**：
    - 对未启用异步发布的表执行完整的CalcDeleteBitmapTask
    - 验证行为和性能与改动前完全一致
