...
修改 `fe-core` 任意一个 `.java` 文件后，`./build.sh --fe` 不应重新清空并全量重建 `fe-core/target/`。最终目标是让 Doris FE 构建回到 Maven 的标准模型：

- `clean` 只由显式 `mvn clean` 或 `./build.sh --clean` 触发
- `compile/package/install` 允许 Maven 复用 `target/` 做增量编译
- FE 运行时依赖目录由可重复、可缓存、无 stale 文件的打包插件生成
...
[fe/fe-core/pom.xml](../../fe/fe-core/pom.xml) 当前通过 `maven-clean-plugin` 在默认生命周期的 `initialize` 阶段执行 `auto-clean`：
...
[fe/fe-core/pom.xml](../../fe/fe-core/pom.xml) 当前用 `maven-dependency-plugin:copy-dependencies` 将运行时依赖复制到 `target/lib/`：
...
[fe/.mvn/maven-build-cache-config.xml](../../fe/.mvn/maven-build-cache-config.xml) 当前将 `copy-dependencies` 配置为 `runAlways`。原因是 build cache 命中后 Maven 会跳过模块构建，`target/lib/` 这类普通目录不会自动恢复；如果刚执行过 clean，`build.sh` 会找不到 `target/lib/*`。
...
[fe/pom.xml](../../fe/pom.xml) 和部分子模块默认绑定了若干不属于本地日常编译闭环的插件：
...
最终状态下，FE Maven 构建遵循标准生命周期：

- `validate`：执行 enforcer、checkstyle 等质量门禁
- `generate-sources`：只执行真正需要的代码生成
- `compile`：增量 javac
- `test-compile`：仅在需要测试类或 test-jar 时执行
- `package`：生成模块 jar 和可重复的 FE runtime lib 目录
- `install`：安装当前 reactor 产物，供下游模块解析
- `clean`：只在显式 clean 时删除输出

`fe-core` 不应在 `initialize` 或其它默认阶段执行 `maven-clean-plugin`。如果需要彻底清理，使用：
...
### 3.2 FE runtime lib 使用 assembly 生成

`fe-core` 的运行时依赖目录改由 `maven-assembly-plugin` 生成，替代 `maven-dependency-plugin:copy-dependencies`。assembly 的职责是根据当前 Maven 依赖图重新生成一个完整目录，避免 stale jar。
...
`fe-core/pom.xml` 中明确配置输出路径，不能依赖 assembly 默认命名：
...
使用 `finalName=lib` 的原因是让输出继续稳定在 `fe/fe-core/target/lib/`，从而保持 [build.sh](../../build.sh) 当前拷贝契约不变：
...
如果后续希望改名为 `target/fe-lib/`，必须同时修改 `build.sh` 和 build-cache `attachedOutputs`，不能只改 descriptor。

### 3.3 build cache 显式缓存 runtime lib 目录

目录型输出不是 Maven 主 artifact。移除 `copy-dependencies` 的 `runAlways` 后，必须让 build cache 显式保存和恢复 `target/lib/`：
...
最终的 [fe/.mvn/maven-build-cache-config.xml](../../fe/.mvn/maven-build-cache-config.xml) 应满足：

- 删除 `maven-dependency-plugin:copy-dependencies` 的 `runAlways`
- 保留 `maven-checkstyle-plugin` 的 `runAlways`
- 增加 `attachedOutputs`，确保 cache hit 后 `fe-core/target/lib/` 可被恢复
- 保持 generated sources 恢复能力，避免 cache hit 后缺少生成源码

这样 cache hit 时，`doris-fe.jar` 和 `target/lib/` 都来自同一个缓存结果；cache miss 时，assembly 根据当前依赖图重新生成目录。

### 3.4 `build.sh --fe` 的最终行为

`build.sh --fe` 继续作为统一入口，但 Maven 参数应符合日常开发构建：
...
该命令应先执行 `clean_fe()`，再进行正常 FE 构建。无需新增 `--full-clean`；现有 `--clean` 已经表达了“先清理再构建”。

### 3.5 非日常构建动作进入 profile
...
### 3.6 test-jar 只在真正需要时构建

[fe/fe-common/pom.xml](../../fe/fe-common/pom.xml) 当前在 `test-compile` 阶段默认生成 test-jar。最终状态应先梳理哪些模块真实依赖 `fe-common` 的 test-jar：

- 如果只有测试代码依赖，把 test-jar 绑定放入 `tests` 或 `test-artifacts` profile
- 如果主构建确实依赖 test-jar，保留绑定，但在文档中说明原因
- 不使用 `maven.test.skip=true` 作为默认优化手段，因为它会跳过测试编译并破坏 test-jar 产物

这个调整应以依赖分析为前提，不能只为了缩短构建时间盲目删除。
...
最终一步到位的 Maven 编译优化不要求改变该机制。只有在完成 runtime lib、auto-clean、profile、cache 修正后，仍确认该处理器是主要瓶颈，才单独设计：
...
4. 在 `fe/fe-core/pom.xml` 中新增 `maven-assembly-plugin:single`，明确输出到 `target/lib/`。
5. 更新 `fe/.mvn/maven-build-cache-config.xml`：
   - 删除 `copy-dependencies` 的 `runAlways`
   - 保留 checkstyle `runAlways`
   - 增加 `attachedOutputs` 的 `lib` 目录
6. 将 `license`、`source jar`、`jacoco`、发布所需 `flatten` 动作移入显式 profile。
7. 保持 `maven-checkstyle-plugin:check` 默认运行。
8. 梳理 `fe-common` test-jar 依赖，决定是否移入显式 test artifacts profile。
9. 保持 `build.sh` 输出契约不变：从 `fe/fe-core/target/lib/` 和 `target/doris-fe.jar` 组装 `output/fe/lib`。
...
- `fe/fe-core/target/lib/` 与 `output/fe/lib/` 的第三方 jar 清单一致
...
4. 确认 `fe/fe-core/target/doris-fe.jar` 和 `fe/fe-core/target/lib/` 都被恢复或重新生成。
