...
> 注1：副本修复的主要思想是先通过创建或补齐使得分片的副本数达到期望值。然后在删除多余的副本。
> 
> 注2：一个 clone 任务就是完成从一个指定远端拷贝指定数据到指定目的端的过程。
...
* use\_new\_tablet\_scheduler
    ...
    * 说明：是否启用新的副本调度方式。新的副本调度方式即本文档介绍的副本调度方式。如果开启，则 `disable_colocate_join` 必须为 `true`。因为新的调度策略暂不支持 co-locotion 表的数据分片调度。
    * 默认值：true
    * 重要性：高
