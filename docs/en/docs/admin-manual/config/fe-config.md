...
### `be_report_query_statistics_timeout_ms`
...
### `enable_urgent_balance_no_low_backend`
...
### `hive_default_input_format`

Default hive input format for creating table.

Type: `String`

Default: `org.apache.hadoop.hive.ql.io.orc.OrcInputFormat`

Mutable: `true`

Master only: `true`

### `hive_default_output_format`

Default hive output format for creating table.

Type: `String`

Default: `org.apache.hadoop.hive.ql.io.orc.OrcOutputFormat`
...
Default: `org.apache.hadoop.hive.ql.io.orc.OrcSerde`
...
The safe path of the JDBC driver. When creating a JDBC Catalog,you can configure multiple files or network paths that are allowed to be used,separated by semicolonsThe default is * to allow all, if set to empty, all are not allowed
...
The threshold of load labels' number. After this number is exceeded, the labels of the completed import jobs or tasks will be deleted, and the deleted labels can be reused.
...
### `manual_drop_replica_valid_second`
...
### `experimental_max_binlog_messsage_size`
...
### `split_assigner_max_split_num_variance`
...
### `split_assigner_min_consistent_hash_candidate_num`
...
### `split_assigner_min_random_candidate_num`
...
### `split_assigner_optimized_local_scheduling`
...
### `split_assigner_virtual_node_number`
...
### `urgent_balance_disk_usage_extra_threshold`
...
### `urgent_balance_pick_large_disk_usage_percentage`
...
### `urgent_balance_pick_large_tablet_num_threshold`
...
### `workload_runtime_status_thread_interval_ms`
