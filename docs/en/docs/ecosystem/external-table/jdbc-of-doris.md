...
6. Query SQL report "OutOfMemoryError" of MYSQL

In order to reduce memory usage, when getting the result set, only the size of batchSize is obtained each time, so as to obtain the results by batch. By default, MYSQL loads all the results into memory at one time,
The set batch acquisition cannot take effect. You need to specify the active display in the URL: "jdbc_url"="jdbc: mysql://IP:PORT/doris_test?useCursorFetch=true "

7. If an error similar to "CAUSED BY: SQLException OutOfMemoryError" occurs during the query using JDBC

If MYSQL has actively set useCursorFetch, you can modify the jvm in be.conf_max_heap_size. Try to increase the memory of the JVM. The current default value is 1024M.
