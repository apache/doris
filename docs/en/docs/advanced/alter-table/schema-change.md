...
* `alter_index_worker_count`: Number of threads used to perform historical data build index on the BE size. The default is 3. If you want to speed up the Index Change job, you can increase this parameter appropriately and restart the BE. But too many threads can cause increased IO pressure and affect other operations.
