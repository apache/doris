...
| doris.request.tablet.size        | Integer.MAX_VALUE | The number of Doris Tablets corresponding to an inputSplit. The smaller this value is set, the more inputSplit will be generated. This will increase the parallelism on the Flink side, but at the same time will cause greater pressure on Doris. |
