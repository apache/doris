...
2. storage_medium & storage\_cooldown\_time
    ...
    * Note that when specifying storage_medium, if no SSD storage medium is set in the cluster, an error 'Failed to find enough host in all backends with storage medium is SSD' is reported.
