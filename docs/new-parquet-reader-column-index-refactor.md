...
`ConstantIndex`。`TableColumnMapper::localize_filters()` 会在 `FileScanRequest` 确定
file-local block position 后生成 `GlobalIndex -> FilterEntry`，并用它驱动 row-level conjunct
和 column predicate pruning 的下推选择。`CONSTANT` 和 `UNSET` target 不进入 file reader
row-level filter。
...
DuckDB 对应概念引入。DuckDB 对应的 column map 类型是最终 `global_to_local` map 的
