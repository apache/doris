...
共享定义集中在 `be/src/format/reader/column_data.h`。file reader 通用请求定义在
`be/src/format/reader/file_reader.h`。new Parquet reader 自己的 Parquet 内部 schema tree 定义在
...
`std::vector<reader::ColumnDefinition>`：
...
`ColumnMapping::field_id` 是 file-reader local id：

- root mapping 中可转成 `LocalColumnId`。
- nested mapping 中表示 parent 下的 child id。
- constant/missing/virtual mapping 没有 `field_id`。

这里的 `field_id` 名称保留了旧代码习惯，但语义已经不是 schema identity field id；它是
file-local id。schema identity 只在 `ColumnDefinition::identifier` 中。
...
定义位置：`be/src/format/new_parquet/parquet_column_schema.h`
