...
每个导入方式的说明请参阅单个导入方式可以通过帮助文档获取更多信息，帮助文档在 mysql 命令行下执行`help`获取更多帮助信息，例如`help stream load` 。
...
* Stream load
    ...
    用户通过 HTTP 协议提交请求并携带原始数据创建导入。主要用于快速将本地文件或数据流中的数据导入到 Doris。导入命令同步返回导入结果。支持 JSON 和 CSV 两种数据格式。
...
* Routine load
    ...
    用户通过 MySQL 协议提交例行导入作业，生成一个常驻线程，不间断的从数据源（如 Kafka）中读取数据并导入到 Doris 中。支持 JSON 和 CSV 两种数据格式。
