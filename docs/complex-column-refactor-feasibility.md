...
MAP 相关类型集中在 `MapColumnReader` 和 `nested_column_reader.h`，调用边界比较清楚，但不适合简单删除。Doris 的输出列是 `ColumnMap`，直接写 key/value/offsets 是正确的物理布局；DuckDB 的 MAP 可以复用 LIST reader，是因为 DuckDB MAP 的内部 vector 本身就是 list-like。
...
Phase 2: Change 1 — 收敛 MAP entry shape/value stream
         影响: 中，保留 MapColumnReader 和 ColumnMap 输出，抽公共 MAP read context、
               finish output、entry value stream helper；不引入 ColumnArray(ColumnStruct) 主路径
...
Phase 3+4 (内联+简化) ──→ 依赖 Phase 2
                         MAP entry helper 收敛后，内联 Dremel 时分支更清晰
```
