...
在启动以后可以通过 api 接口的方式查看当前 be 进程正在执行的配置项，查看方式可查看 UPDATE/VIEW CONFIG 文档中，对接口的描述。
...
在启动之后可以通过接口的方式对 be 的配置进行动态修改，修改的前提条件需要该配置支持动态修改功能。动态修改的方式可查看 UPDATE/VIEW CONFIG 文档中，对接口的描述。
