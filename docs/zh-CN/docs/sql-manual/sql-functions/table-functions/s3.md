...
S3表函数（table-valued-function,tvf），可以读取S3上的文件，将文件中的内容转换为doris中的内存表,生成一张doris临时表。目前支持`csv/csv_with_names/csv_with_names_and_types/json/parquet/orc`文件格式。
...
- `access_key`： (必填)
- `secret_key`： (必填)
- `use_path_style`：(选填) 默认为`false` 。S3 SDK 默认使用 virtual-hosted style 方式。但某些对象存储系统可能没开启或没支持vvirtual-hosted style 方式的访问，此时我们可以添加 use_path_style 参数来强制使用 path style 方式。
