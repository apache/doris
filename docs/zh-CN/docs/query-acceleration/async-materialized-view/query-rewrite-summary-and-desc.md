...
JOIN衍生（TODO）
当查询和物化的 JOIN 的类型不一致时，但物化可以提供查询所需的所有数据时，通过在 JOIN 的上层补偿谓词，也可以进行透明改写，
举例如下，待支持。
...
如下用例可以进行透明改写，查询和物化使用聚合的维度一致，可以使用维度中的字段进行过滤结果，查询会尝试使用物化视图 SELECT 后的表达式。
...
- 目前 WHERE 条件补偿，支持物化视图没有 WHERE，查询有 WHERE；或者物化视图有 WHERE 且查询的 WHERE 条件是物化视图的超集。目前暂时还不支持
