...
- 写时合并(merge-on-write)。在1.2版本中，我们引入了写实合并实现，该实现会在数据写入阶段完成所有数据去重的工作，因此能够提供非常好的查询性能。

自2.1版本起，写实合并已经非常成熟稳定，也成为了Unique模型的默认实现。关于两种实现方式的详细区别，用户可以本章节后续内容的介绍。关于两种实现方式的性能差异，参考后续章节[聚合模型的局限性](#聚合模型的局限性)的描述。
...
- 部分列更新。如果用户希望更新部分字段，需要使用写实合并实现，并通过特定的参数来开启部分列更新的支持。请查阅文档[部分列更新](../data-operate/update-delete/partial-update.md)获取相关使用建议
...
> 1. 对于新用户，强烈推荐使用2.0以上版本。在2.0版本中，写实合并的性能和稳定性都有大幅的提升和优化
