...
自增列可以看做有默认值的列，其默认值为生成出的值。生成出的值从0开始，以1为步长依次递增。在导入数据时，如果导入的目标列中不包含自增列，则自增列的值将全部由生成的值填充。如果导入的目标列中包含自增列，则导入数据中该列的null值将会被生成的值替换，非null值则保持不变。

Doris保证了自增列上生成的值具有表内唯一性。但需要注意的是，**自增列的唯一性仅保证表内生成的值具有唯一性，而不关心用户显式插入的值**。例如在一个Duplicate模型上
