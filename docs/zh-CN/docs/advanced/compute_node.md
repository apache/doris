...
## 解决方案
实现一种专门用于联邦计算的BE节点角色: 计算节点, 计算节点专门处理这类数据湖这类远程的联邦查询. 原来的BE节点类型称为混合节点, 这类节点既能直接SQL任务, 又有Tablet数据存储.

有了计算节点后, 集群部署拓扑也会发生变: 混合节点用于OLAP类型表的数据计算, 这个节点根据存储的需求而扩容, 而计算节点用于外表等计算, 该节点类型随着计算负载而扩容.

计算节点由于没有存储, 因此计算节点可以使用HDD磁盘机器, 另外计算节点可以实现快速扩缩容.
...
### 使用
当查询时使用MultiCatalog功能时, 查询会优先调度到计算节点, 为了均衡任务调度, FE有一个`backend_num_for_federation`配置项, 默认是3.
...
### 一些限制
- 计算节点目前只支持MultiCatalog对应的查询语法, 普通外表的计算依然在混合节点上.
...
- 优雅下线: 当计算下线的时候, 任务新任务自动调度到其他节点; 老任务全部完成后再下线; 老任务无法按时结束时, 能够任务能够自己结束.
