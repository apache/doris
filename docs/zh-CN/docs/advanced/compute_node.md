...
### 使用
当查询时使用[MultiCatalog](../lakehouse/multi-catalog/multi-catalog.md)功能时, 查询会优先调度到计算节点, 为了均衡任务调度, FE有一个`backend_num_for_federation`配置项, 默认是3.
...
### 一些限制
- 计算节点目前只支持[MultiCatalog](../lakehouse/multi-catalog/multi-catalog.md)对应的Hive MetaStore表类型查询语法, 普通外表的计算依然在混合节点上.
