...
物化索引是将预先计算（根据定义好的 SELECT 语句）好的数据集，存储在 Doris 中的一个特殊的表。

物化索引的出现主要是为了满足用户，既能对原始明细数据的任意维度分析，也能快速的对固定维度进行分析查询。
...
- Doris自动维护物化索引的数据，无论是新的导入，还是删除操作都能保证base 表和物化索引表的数据一致性。无需任何额外的人工维护成本。
- 查询时，会自动匹配到最优物化索引，并直接从物化索引中读取数据。

*自动维护物化索引的数据会造成一些维护开销，会在后面的物化索引的局限性中展开说明。*

## 物化索引 VS Rollup

在没有物化索引功能之前，用户一般都是使用 Rollup 功能通过预聚合方式提升查询效率的。但是 Rollup 具有一定的局限性，他不能基于明细模型做预聚合。

物化索引则在覆盖了 Rollup 的功能的同时，还能支持更丰富的聚合函数。所以物化索引其实是 Rollup 的一个超集。

也就是说，之前 [ALTER TABLE ADD ROLLUP](../sql-manual/sql-reference/Data-Definition-Statements/Alter/ALTER-TABLE-ROLLUP.md) 语法支持的功能现在均可以通过 [CREATE MATERIALIZED INDEX](../sql-manual/sql-reference/Data-Definition-Statements/Create/CREATE-MATERIALIZED-INDEX.md) 实现。

## 使用物化索引

Doris 系统提供了一整套对物化索引的 DDL 语法，包括创建，查看，删除。DDL 的语法和 PostgreSQL, Oracle都是一致的。

### 创建物化索引

这里首先你要根据你的查询语句的特点来决定创建一个什么样的物化索引。这里并不是说你的物化索引定义和你的某个查询语句一模一样就最好。这里有两个原则：

1. 从查询语句中**抽象**出，多个查询共有的分组和聚合方式作为物化索引的定义。
2. 不需要给所有维度组合都创建物化索引。

首先第一个点，一个物化索引如果抽象出来，并且多个查询都可以匹配到这张物化索引。这种物化索引效果最好。因为物化索引的维护本身也需要消耗资源。

如果物化索引只和某个特殊的查询很贴合，而其他查询均用不到这个物化索引。则会导致这张物化索引的性价比不高，既占用了集群的存储资源，还不能为更多的查询服务。

所以用户需要结合自己的查询语句，以及数据维度信息去抽象出一些物化索引的定义。

第二点就是，在实际的分析查询中，并不会覆盖到所有的维度分析。所以给常用的维度组合创建物化索引即可，从而到达一个空间和时间上的平衡。

创建物化索引是一个异步的操作，也就是说用户成功提交创建任务后，Doris 会在后台对存量的数据进行计算，直到创建成功。

具体的语法可查看[CREATE MATERIALIZED INDEX](../sql-manual/sql-reference/Data-Definition-Statements/Create/CREATE-MATERIALIZED-INDEX.md) 。


<version since="2.0.0"></version>

在`Doris 2.0`版本中我们对物化索引的做了一些增强(在本文的`最佳实践4`中有具体描述)。我们建议用户在正式的生产环境中使用物化索引前，先在测试环境中确认是预期中的查询能否命中想要创建的物化索引。

如果不清楚如何验证一个查询是否命中物化索引，可以阅读本文的`最佳实践1`。

与此同时，我们不建议用户在同一张表上建多个形态类似的物化索引，这可能会导致多个物化索引之间的冲突使得查询命中失败。（当然，这些可能出现的问题都可以在测试环境中验证）
...
为保证物化索引表和 Base 表的数据一致性, Doris 会将导入，删除等对 base 表的操作都同步到物化索引表中。并且通过增量更新的方式来提升更新效率。通过事务方式来保证原子性。

比如如果用户通过 INSERT 命令插入数据到 base 表中，则这条数据会同步插入到物化索引中。当 base 表和物化索引表均写入成功后，INSERT 命令才会成功返回。

### 查询自动匹配

物化索引创建成功后，用户的查询不需要发生任何改变，也就是还是查询的 base 表。Doris 会根据当前查询的语句去自动选择一个最优的物化索引，从物化索引中读取数据并计算。

用户可以通过 EXPLAIN 命令来检查当前查询是否使用了物化索引。
...
其中 bitmap 和 hll 的聚合函数在查询匹配到物化索引后，查询的聚合算子会根据物化索引的表结构进行改写。详细见实例2。

### 查询物化索引

查看当前表都有哪些物化索引，以及他们的表结构都是什么样的。通过下面命令：
...
可以看到当前 `mv_test` 表一共有三张物化索引：mv_1, mv_2 和 mv_3，以及他们的表结构。

### 删除物化索引

如果用户不再需要物化索引，则可以通过命令删除物化索引。

具体的语法可查看[DROP materialized index](../sql-manual/sql-reference/Data-Definition-Statements/Drop/DROP-MATERIALIZED-INDEX.md) 

### 查看已创建的物化索引

用户可以通过命令查看已创建的物化索引的

具体的语法可查看[SHOW CREATE materialized index](../sql-manual/sql-reference/Show-Statements/SHOW-CREATE-MATERIALIZED-INDEX.md)
...
由于创建物化索引是一个异步的操作，用户在提交完创建物化索引任务后，需要异步的通过命令检查物化索引是否构建完成。命令如下：
...
这个命令中 `db_name` 是一个参数, 你需要替换成自己真实的 db 名称。命令的结果是显示这个 db 的所有创建物化索引的任务。结果如下：
...
其中 TableName 指的是物化索引的数据来自于哪个表，RollupIndexName 指的是物化索引的名称叫什么。其中比较重要的指标是 State。

当创建物化索引任务的 State 已经变成 FINISHED 后，就说明这个物化索引已经创建成功了。这就意味着，查询的时候有可能自动匹配到这张物化索引了。

**第三步：查询**

当创建完成物化索引后，用户再查询不同门店的销售量时，就会直接从刚才创建的物化索引 `store_amt` 中读取聚合好的数据。达到提升查询效率的效果。
...
上面查询就能自动匹配到 `store_amt`。用户可以通过下面命令，检验当前查询是否匹配到了合适的物化索引。
...
+----------------------------------------------------------------------------------------------+
```
从最底部的`test.sales_records(store_amt)`可以表明这个查询命中了`store_amt`这个物化索引。值得注意的是，如果表中没有数据，那么可能不会命中物化索引。

## 最佳实践2 PV,UV

业务场景: 计算广告的 UV，PV。

假设用户的原始广告点击数据存储在 Doris，那么针对广告 PV, UV 查询就可以通过创建 `bitmap_union` 的物化索引来提升查询速度。
...
1. 创建物化索引
   ...
   针对这种求 UV 的场景，我们就可以创建一个带 `bitmap_union` 的物化索引从而达到一个预先精确去重的效果。

   在 Doris 中，`count(distinct)` 聚合的结果和 `bitmap_union_count`聚合的结果是完全一致的。而`bitmap_union_count` 等于 `bitmap_union` 的结果求 count， 所以如果查询中**涉及到 `count(distinct)` 则通过创建带 `bitmap_union` 聚合的物化索引方可加快查询**。

   针对这个 case，则可以创建一个根据广告和渠道分组，对 `user_id` 进行精确去重的物化索引。
...
2. 查询自动匹配
   ...
   当物化索引表创建完成后，查询广告 UV 时，Doris就会自动从刚才创建好的物化索引 `advertiser_uv` 中查询数据。比如原始的查询语句如下：
   ...
   通过 EXPLAIN 命令可以检验到 Doris 是否匹配到了物化索引：
   ...
   在 EXPLAIN 的结果中，首先可以看到`VOlapScanNode`命中了`advertiser_uv`。也就是说，查询会直接扫描物化索引的数据。说明匹配成功。
...
1. 创建物化索引
   ...
   通过上面语法创建完成后，物化索引中既保留了完整的明细数据，且物化索引的前缀索引为 k3 列。表结构如下：
...
2. 查询匹配
   ...
   这时候查询就会直接从刚才创建的 mv_1 物化索引中读取数据。物化索引对 k3 是存在前缀索引的，查询效率也会提升。

## 最佳实践4

<version since="2.0.0"></version>

在`Doris 2.0`中，我们对物化索引所支持的表达式做了一些增强，本示例将主要体现新版本物化索引对各种表达式的支持。
...
1. 物化索引的聚合函数的参数不支持表达式仅支持单列，比如： sum(a+b)不支持。(2.0后支持)
2. 如果删除语句的条件列，在物化索引中不存在，则不能进行删除操作。如果一定要删除数据，则需要先将物化索引删除，然后方可删除数据。
3. 单表上过多的物化索引会影响导入的效率：导入数据时，物化索引和 base 表数据是同步更新的，如果一张表的物化索引表超过10张，则有可能导致导入速度很慢。这就像单次导入需要同时导入10张表数据是一样的。
4. 相同列，不同聚合函数，不能同时出现在一张物化索引中，比如：select sum(a), min(a) from table 不支持。(2.0后支持)
5. 物化索引针对 Unique Key数据模型，只能改变列顺序，不能起到聚合的作用，所以在Unique Key模型上不能通过创建物化索引的方式对数据进行粗粒度聚合操作

## 异常错误

1. DATA_QUALITY_ERROR: "The data quality does not satisfy, please check your data" 由于数据质量问题或者Schema Change内存使用超出限制导致物化索引创建失败。如果是内存问题，调大`memory_limitation_per_thread_for_schema_change_bytes`参数即可。 注意：bitmap类型仅支持正整型, 如果原始数据中存在负数，会导致物化索引创建失败



## 更多帮助

关于物化索引使用的更多详细语法及最佳实践，请参阅 [CREATE materialized index](../sql-manual/sql-reference/Data-Definition-Statements/Create/CREATE-MATERIALIZED-INDEX.md) 和 [DROP materialized index](../sql-manual/sql-reference/Data-Definition-Statements/Drop/DROP-MATERIALIZED-INDEX.md) 命令手册，你也可以在 MySql 客户端命令行下输入 `HELP CREATE materialized index` 和`HELP DROP materialized index`  获取更多帮助信息。
