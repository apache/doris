...
**可行改动**：保留 Doris 的 `MapColumnReader` 和 `ColumnMap` 输出语义，抽出 MAP entry 的 repeated shape、overflow 和 value stream 对齐 helper。DuckDB 的启发点是 MAP 的 entry shape 与 LIST 类似，不是要求 Doris 把 MAP 主路径改成 `ColumnArray(ColumnStruct)` 或删除 MAP reader。尤其是 `Map<K, List<V>>`，value 侧仍有内部 repeated shape，不能简单只用 key leaf 作为唯一 driver。
