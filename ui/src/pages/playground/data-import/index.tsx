...
import { AdHocAPI, deleteUploadData, doUp, getUploadData } from 'Src/api/api';
import { Button, Empty, Form, Input, notification, Select, Steps, Table, Typography, Upload } from 'antd';
...
export default function DataImport(props: any) {
  ...
  function getTableInfoRequest() {
    ...
    AdHocAPI.getDatabaseList(param)
      .then((res) => {
  ...
  return (
    <div style={{ padding: "50px" }}>
        ...
        {current === 1 ? (
          <>
            <Form {...layout}>
              ...
              <Form.Item label={t("selectTheFile")}>
                <Upload {...uploadData}>
                  <Button>
                    <UploadOutlined />
                    {t("upload")}
                      ...
                      <tbody className="ant-table-tbody">
                        {prevData?.map((item, index) => (
                          <tr className="ant-table-row" key={index}>
                            {item.map((tdData, i) => (
                              <td className="ant-table-cell" key={i + tdData}>
                                {tdData == "\\N" ? "-" : tdData}
                              </td>
                            ))}
                          </tr>
      ...
      <div className="steps-action" style={{ textAlign: "end" }}>
        ...
        &nbsp;<Button onClick={() => doBack()}>{t("cancelButton")}</Button>
      </div>
    </div>
  );
}
