...
import { AdhocContentRouteKeyEnum, CODEMIRROR_OPTIONS } from '../adhoc.data';
...
import { getDbName, getTimeNow, isSuccess } from 'Utils/utils';
import { openInEditorSubject, runSQLSuccessSubject } from '../adhoc.subject';
      ...
      }),
    {
      ...
      onSuccess: (res) => {
        ...
        if (isSuccess(res)) {
          ...
          props.history.push({
            pathname: `/Playground/result/${db_name}-${tbl_name}`,
            state: res,
          });
    ...
    <div>
      ...
      <Switch>
        <Route
          ...
          render={(props) => <AdhocContentResult {...props} />}
        />
        <Route
          ...
          render={(props) => (
            <div style={{ display: "flex", height: "53vh" }}>
              <div style={{ flex: 3 }}>
                <ContentStructure
                  queryTable={(tableName) => handleQueryTable(tableName)}
