...
import { Card, Col, Pagination, Popover, Progress, Row, Tabs } from 'antd';
...
export function AdhocContentResult(props) {
  ...
  useEffect(() => {
    ...
    if (runningQueryInfo.data?.type === "exec_status") {
      ...
      const tableData =
        runningQueryInfo.data && typeof runningQueryInfo.data === "object"
          ? (runningQueryInfo.data?.data).slice(0, 20)
          : [];
      setTableDate(tableData);
      setTotal(
        runningQueryInfo.data && typeof runningQueryInfo.data === "object"
          ? runningQueryInfo.data?.data.length
          : 0
      );
    }
  ...
  function renderRunStatus() {
    return (
      <>
        <Progress style={{ width: 340 }} percent={100} />
        ...
        <LoadingWrapper loading={false}>
          {runningQueryInfo.status !== "failed" ? (
            ...
            <TextWithIcon
              ...
              text={
                `${t("executionFailed")}: ` +
                runningQueryInfo.msg +
                " " +
                runningQueryInfo.data
              }
              color="red"
              style={{
                marginBottom: 10,
