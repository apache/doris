...
# ---- missing python3: loud exit 2 with the OFF-switch hint ----
OUT="$(PYTHON=/nonexistent/python3 bash "${ENTRY}" 2>&1)"; EC=$?
...
if [ "${FAILED}" -eq 0 ]; then
    echo "PASS: entry aggregates all checks, fails loud without python3, and is green when clean."
    exit 0
fi
exit 1
