...
TEST_P(CharColumnReadTest, EmbeddedNulRoundTrip) {
    ...
    const auto key = GetParam().cid == 0 ? char_value() : Field::create_field<TYPE_INT>(1);
    const auto value = GetParam().cid == 1 ? char_value() : Field::create_field<TYPE_INT>(10);
    ...
    auto row_store = schema->create_storage_block({0, 1});
    ASSERT_TRUE(
            BaseTablet::fetch_value_through_row_column(rowset, *schema, 0, {0}, {0, 1}, row_store)
                    .ok());
    ASSERT_EQ(row_store.rows(), 1);
    for (uint32_t cid : {0, 1}) {
        auto expected = row_store.get_by_position(cid).column->clone_empty();
        expected->insert(cid == 0 ? key : value);
        // Field equality does not compare ARRAY/MAP/STRUCT payloads; compare actual columns.
        EXPECT_EQ(row_store.get_by_position(cid).column->compare_at(0, 0, *expected, 1), 0);
        EXPECT_EQ(physical.get_by_position(cid).column->compare_at(0, 0, *expected, 1), 0);
        auto gathered = expected->clone_empty();
        ASSERT_TRUE(BaseTablet::fetch_value_by_rowids(rowset, 0, {0}, schema->column(cid), gathered)
                            .ok());
    ...
    ASSERT_TRUE(
            convertor.set_source_content_with_specifid_column(physical.get_by_position(0), 0, 1, 0)
                    .ok());
