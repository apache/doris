...
{% macro doris__drop_relation(relation) -%}
    ...
    drop
{{ relation_type }} if exists {{ relation }}
    {% endcall %}
{%- endmacro %}
...
{% macro doris__rename_relation(from_relation, to_relation) -%}
  {% call statement(''drop_relation'') %}
    drop
{{ to_relation.type }} if exists {{ to_relation }}
  {% endcall %}
  {% call statement(''rename_relation'') %}
    {% if to_relation.is_view %}
    {% set results = run_query(''show create view '' + from_relation.render() ) %}
create view {{ to_relation
}
} as {{ results[0
]['' Create View ''
].replace(from_relation.table, to_relation.table
).split('' AS '',1
)[1
]
}
...
alter
view {{ relation1 }} as {{  to_results[0][''
Create View ''
].split('' AS '',1
)[1
]
...
alter
view {{ relation2 }} as {{  from_results[0][''
Create View ''
].split('' AS '',1
)[1
]
