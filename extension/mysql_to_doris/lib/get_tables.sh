...
echo "select concat('doris.e_', TABLE_SCHEMA , '_', TABLE_NAME) from information_schema.TABLES where TABLE_SCHEMA in ('$(echo ${dbs} | sed "s/,/','/g")') and TABLE_TYPE = 'BASE TABLE' order by TABLE_SCHEMA, TABLE_NAME;" | mysql -N -h"${mysql_host}" -P"${mysql_port}" -u"${mysql_username}" -p"${mysql_password}" > "${home_dir}"/conf/doris_external_tables
# get mysql tables and generate olap tables and write to ${home_dir}/conf/doris_tables
echo "select concat('doris.', TABLE_SCHEMA , '_', TABLE_NAME) from information_schema.TABLES where TABLE_SCHEMA in ('$(echo ${dbs} | sed "s/,/','/g")') and TABLE_TYPE = 'BASE TABLE' order by TABLE_SCHEMA, TABLE_NAME;" | mysql -N -h"${mysql_host}" -P"${mysql_port}" -u"${mysql_username}" -p"${mysql_password}" > "${home_dir}"/conf/doris_tables