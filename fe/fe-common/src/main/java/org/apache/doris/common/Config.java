...
public class Config extends ConfigBase {
    ...
    @ConfField(mutable = true, masterOnly = true, description = {
        "有些场景下集群出现了不可恢复的元数据问题，且数据的visibleversion已经和be不匹配，"
            + "这种情况下仍然需要恢复剩余的数据（可能会导致数据的正确性有问题），"
            + "这个配置同`recover_with_empty_tablet`一样只能在紧急情况下使用"
            + "这个配置有三个值："
            + "   disable：出现异常会正常报错"
        ...
        "In some scenarios, there is an unrecoverable metadata problem in the cluster,"
            + "and the visibleVersion of the data does not match be."
            + "In this case, it is still necessary to restore the remaining data (which may cause"
            + "problems with the correctness of the data). This configuration is"
