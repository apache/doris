...
public class CreateTableTest {
    ...
    public void testNotEnoughBackend() throws Exception {
        ...
        expectedException.expect(DdlException.class);
        expectedException.expectMessage("errCode = 2, detailMessage = Failed to find enough host in all backends (storage medium is HDD). need: 3");
        createTable(createOlapTblStmt);
    }
    ...
    public void testTableNotFoundStorageMedium() throws Exception {
        ...
        expectedException.expect(DdlException.class);
        expectedException.expectMessage("errCode = 2, detailMessage = Failed to find enough host in all backends (storage medium is SSD). need: 1");
        createTable(createOlapTblStmt);
    }
}
