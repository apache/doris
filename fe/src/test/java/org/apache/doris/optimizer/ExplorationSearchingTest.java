...
public class ExplorationSearchingTest {
    ...
    public void testSevenJoin() {
        ...
        Assert.assertTrue(MemoStatus.checkStatusForExplore(optimizer.getMemo()));
        Assert.assertEquals(255, optimizer.getMemo().getGroups().size());
        Assert.assertEquals(6058, optimizer.getMemo().getMExprs().size());
        final long planCount = PlansStatus.getPlanCount(optimizer.getRoot());
        LOG.info("Plans:" + planCount);
        Assert.assertEquals(17297280, planCount);
    }
