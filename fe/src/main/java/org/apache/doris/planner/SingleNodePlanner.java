...
public class SingleNodePlanner {
   ...
   /**
    ...
    private void pushDownPredicatesPastSort(Analyzer analyzer, SelectStmt stmt) throws AnalysisException {
        ...
        if (putPredicatesOnWindows(stmt, analyzer, predicates, pushDownPredicates)) {
            return;
        }

        if (putPredicatesOnAggregations(stmt, analyzer, predicates, pushDownPredicates)) {
            return;
        }
    ...
    private void pushDownPredicatesPastAggregation(Analyzer analyzer, SelectStmt stmt) throws AnalysisException {
        ...
        final List<Expr> predicatesCanPushDown = getPredicatesBoundedByGroupbysAndReplaceSlotWithSourceExpr(predicates, analyzer);
        putPredicatesOnFrom(stmt, analyzer, predicatesCanPushDown);
    }
    ...
    private List<Expr> getPredicatesReplacedSlotWithSourceExpr(List<Expr> predicates, Analyzer analyzer) {
        ...
        for (Expr predicate : predicates) {
            ...
            analyzer.markConjunctAssigned((org.apache.doris.analysis.Predicate) predicate);
    ...
    private void replacePredicateSlotRefWithSlotRefSource(Expr parent, Expr predicate, int childIndex, Analyzer analyzer) {
        if (predicate instanceof SlotRef) {
            ...
            final SlotDescriptor slotDesc = analyzer.getDescTbl().getSlotDesc(slotRef.getSlotId());
            if (parent != null && childIndex >= 0) {
                final Expr newReplacedExpr = slotDesc.getSourceExprs().get(0).clone();
    ...
    private boolean putPredicatesOnAggregations(SelectStmt stmt, Analyzer analyzer,
                                      List<Expr> oldPredicates, List<Expr> newPredicates) throws AnalysisException {
        final AggregateInfo aggregateInfo = stmt.getAggInfo();
        if (aggregateInfo != null) {
            analyzer.markConjunctsAssigned(oldPredicates);
            analyzer.registerConjuncts(newPredicates, aggregateInfo.getOutputTupleId());
            return true;
        }
        return false;
    }
    ...
    private boolean putPredicatesOnWindows(SelectStmt stmt, Analyzer analyzer,
        ...
        if (analyticInfo != null) {
            analyzer.markConjunctsAssigned(oldPredicates);
            analyzer.registerConjuncts(newPredicates, analyticInfo.getOutputTupleId());
            return true;
        }
        return false;
    }
    ...
    private boolean putPredicatesOnFrom(SelectStmt stmt, Analyzer analyzer, List<Expr> predicates) throws AnalysisException {
        ...
        for (Expr predicate : predicates) {
            ...
            if (predicate.isBoundByTupleIds(tableTupleIds)) {
                    analyzer.registerConjunct(predicate, predicateTupleIds);
            } else {
                final StringBuilder logBuilder = new StringBuilder();
                logBuilder.append("Predicate:")
                        .append(predicate.toSql())
                        .append(" can't be assigned to some PlanNode.");
                Preconditions.checkArgument(false, logBuilder.toString());
            }
        }
        return true;
    }
