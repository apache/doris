...
public class S3URI {
     ...
     */
    public static S3URI create(String location) throws StoragePropertiesException {
        return create(location, false, false);
    }

    public static S3URI create(String location, boolean isPathStyle) throws StoragePropertiesException {
    ...
    private S3URI(String location, boolean isPathStyle, boolean forceParsingByStandardUri)
            throws StoragePropertiesException {
        if (Strings.isNullOrEmpty(location)) {
            throw new StoragePropertiesException("s3 location can not be null");
        }
        this.isPathStyle = isPathStyle;
        parseUri(location, forceParsingByStandardUri);
    }

    private void parseUri(String location, boolean forceParsingStandardUri) throws StoragePropertiesException {
     ...
     * @throws StoragePropertiesException
     */
    private void parseURILocation(String location) throws StoragePropertiesException {
        Matcher matcher = URI_PATTERN.matcher(location);
        if (!matcher.matches()) {
            throw new StoragePropertiesException("Failed to parse uri: " + location);
        }
    ...
    private void validateUri() throws StoragePropertiesException {
        if (uri.getScheme() == null || !VALID_SCHEMES.contains(uri.getScheme().toLowerCase())) {
            throw new StoragePropertiesException("Invalid scheme: " + this.uri);
        }
    }

    private void parseAwsCliStyleUri() throws StoragePropertiesException {
        bucket = uri.getAuthority();
        if (bucket == null) {
            throw new StoragePropertiesException("missing bucket: " + uri);
        }
        String path = uri.getPath();
        if (path.length() > 1) {
            key = path.substring(1);
        } else {
            throw new StoragePropertiesException("missing key: " + uri);
        }
    ...
    private void parseStandardUri() throws StoragePropertiesException {
        if (uri.getHost() == null) {
            throw new StoragePropertiesException("Invalid S3 URI: no hostname: " + uri);
        }
    ...
    private void parsePathStyleUri() throws StoragePropertiesException {
        String path = uri.getPath();
        ...
        if (!StringUtils.isEmpty(path) && !"/".equals(path)) {
            ...
            throw new StoragePropertiesException("missing bucket: " + this.uri);
        }
    }

    private void parseVirtualHostedStyleUri() throws StoragePropertiesException {
        ...
        if (!StringUtils.isEmpty(path) && !"/".equals(path)) {
            ...
            throw new StoragePropertiesException("missing key from uri: " + this.uri);
        }
    }
