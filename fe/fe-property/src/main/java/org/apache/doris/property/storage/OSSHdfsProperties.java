...
public class OSSHdfsProperties extends HdfsCompatibleProperties {
    ...
    private void initConfigurationParams() {
        ...
        this.hadoopConfigMap = new LinkedHashMap<>();
        this.backendConfigProperties.forEach(hadoopConfigMap::put);
    }
    ...
    private String validateUri(String uri) throws StoragePropertiesException {
        if (StringUtils.isBlank(uri)) {
            throw new StoragePropertiesException("The uri is empty.");
        }
        URI uriObj = URI.create(uri);
        if (uriObj.getScheme() == null) {
            throw new StoragePropertiesException("The uri scheme is empty.");
        }
        if (!uriObj.getScheme().equalsIgnoreCase("oss")) {
            throw new StoragePropertiesException("The uri scheme is not oss.");
        }
