...
import org.apache.doris.property.storage.exception.AzureAuthType;
...
public class AzureProperties extends StorageProperties {
    ...
    public Map<String, String> getBackendConfigProperties() {
        ...
        Map<String, String> s3Props = new HashMap<>();
        hadoopConfigMap.forEach(s3Props::put);
        return s3Props;
    }

    /**
     * Azure blob/dfs host suffixes used to derive per-endpoint {@code fs.azure.account.key.*} keys, inlined from the
     * legacy fe-core {@code Config.azure_blob_host_suffixes} default. fe-property has no fe-core {@code Config}; the
     * engine may overwrite this at startup to mirror a user-customized value.
     */
    public static volatile String[] azureBlobHostSuffixes = {
    ...
    public void initializeHadoopStorageConfig() {
        ...
        for (String scheme : new String[]{"abfs", "abfss", "wasb", "wasbs"}) {
            hadoopConfigMap.put("fs." + scheme + ".impl.disable.cache", "true");
        }
        ...
        if (azureAuthType != null && azureAuthType.equalsIgnoreCase("OAuth2")) {
            setHDFSAzureOauth2Config(hadoopConfigMap);
        } else {
            setHDFSAzureAccountKeys(hadoopConfigMap, accountName, accountKey);
        }
    }
    ...
    private static void setHDFSAzureAccountKeys(Map<String, String> conf, String accountName, String accountKey) {
        Set<String> endpoints = new LinkedHashSet<>();
        if (azureBlobHostSuffixes != null) {
            for (String endpointSuffix : azureBlobHostSuffixes) {
        ...
        for (String endpoint : endpoints) {
            ...
            conf.put(accountKeyConfig, accountKey);
        }
        conf.put("fs.azure.account.key", accountKey);
    }

    private void setHDFSAzureOauth2Config(Map<String, String> conf) {
        conf.put(String.format("fs.azure.account.auth.type.%s", accountHost), "OAuth");
        conf.put(String.format("fs.azure.account.oauth.provider.type.%s", accountHost),
                "org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider");
        conf.put(String.format("fs.azure.account.oauth2.client.id.%s", accountHost), clientId);
        conf.put(String.format("fs.azure.account.oauth2.client.secret.%s", accountHost), clientSecret);
        conf.put(String.format("fs.azure.account.oauth2.client.endpoint.%s", accountHost), oauthServerUri);
    }
