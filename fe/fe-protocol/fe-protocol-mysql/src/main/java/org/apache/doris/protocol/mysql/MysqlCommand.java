...
public enum MysqlCommand {
    ...
    private static final Map<Integer, MysqlCommand> CODE_MAP = Maps.newHashMap();

    static {
        EnumSet<MysqlCommand> enumSet = EnumSet.allOf(MysqlCommand.class);
        for (MysqlCommand command : enumSet) {
            CODE_MAP.put(command.commandCode, command);
        }
    }
    ...
    /**
     * Gets a command by its code.
     * 
     * @param code command code
     * @return command enum, or null if not found
     */
