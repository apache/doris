...
import org.apache.doris.jni.spi.vec.NativeColumnValue.NativeValue;
...
public class VectorColumn {
    ...
    public ArrayList<Object> getArray(int rowId) {
        ...
        ArrayList<Object> result = new ArrayList<>(endOffset - startOffset);
    ...
    private void checkStructChildArity() {
        int declared = this.getColumnType().getChildNames().size();
        if (declared != childColumns.length) {
            throw new IllegalArgumentException("Struct column '" + this.getColumnType().getName()
                    + "' declares " + declared + " fields but holds " + childColumns.length + " child columns");
        }
    }
}
