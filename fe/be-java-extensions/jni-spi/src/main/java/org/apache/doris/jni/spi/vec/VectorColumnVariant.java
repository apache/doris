...
final class VectorColumnVariant {
    ...
    void reserveRows(int requiredCapacity) {
        ...
        metadataOffsets = OffHeap.reallocateMemory(
                metadataOffsets, oldOffsetsSize, (newCapacity + 1L) * Integer.BYTES);
        ...
        valueOffsets = OffHeap.reallocateMemory(
                valueOffsets, oldOffsetsSize, (newCapacity + 1L) * Integer.BYTES);
        if (rowCapacity == 0) {
    ...
    void append(byte[] metadata, byte[] value) {
        ...
        OffHeap.copyMemory(
                value, OffHeap.BYTE_ARRAY_OFFSET, null, valueBytes + valueBytesSize, value.length);
    ...
    private int appendMetadata(byte[] metadata) {
        ...
        OffHeap.copyMemory(metadata, OffHeap.BYTE_ARRAY_OFFSET,
                null, metadataBytes + metadataBytesSize, metadata.length);
        metadataBytesSize = requiredMetadataBytes;
        metadataCount++;
        OffHeap.putInt(null, metadataOffsets + (long) metadataCount * Integer.BYTES,
                metadataBytesSize);
    ...
    private static int checkedSize(String component, int currentSize, int appendedSize) {
        ...
        if (requiredSize > MAX_CAPACITY) {
            throw new RuntimeException(
                    "Variant " + component + " buffer exceeds the Java JNI size limit");
        }
    ...
    private static int growCapacity(int currentCapacity, int requiredCapacity) {
        ...
        int newCapacity = (int) Math.min(
                MAX_CAPACITY, Math.max(doubledCapacity, requiredCapacity));
