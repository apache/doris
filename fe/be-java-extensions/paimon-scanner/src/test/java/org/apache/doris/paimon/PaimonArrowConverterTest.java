...
public class PaimonArrowConverterTest {
    ...
    @Test
    public void testBlobBinaryTransportCreatesPaimonBlob() {
        Field blobField = new Field(
                "payload", FieldType.nullable(new ArrowType.Binary()), null);
        byte[] expected = new byte[] {0, 1, 2, (byte) 0xff};
        try (RootAllocator allocator = new RootAllocator();
                VectorSchemaRoot root = VectorSchemaRoot.create(
                        new Schema(Collections.singletonList(blobField)), allocator)) {
            VarBinaryVector vector = (VarBinaryVector) root.getVector("payload");
            root.allocateNew();
            vector.setSafe(0, expected);
            root.setRowCount(1);

            PaimonArrowConverter.RowReader rows =
                    new PaimonArrowConverter(ZoneId.of("UTC")).rows(
                            root, new org.apache.paimon.types.DataType[] {new BlobType()});
            Blob actual = (Blob) rows.values(0)[0];
            Assertions.assertArrayEquals(expected, actual.toData());
        }
    }

    @Test
    public void testVectorListTransportCreatesDenseFixedVector() {
        Field vectorField = vectorField();
        try (RootAllocator allocator = new RootAllocator();
                VectorSchemaRoot root = VectorSchemaRoot.create(
                        new Schema(Collections.singletonList(vectorField)), allocator)) {
            ListVector vector = (ListVector) root.getVector("embedding");
            Float4Vector elements = (Float4Vector) vector.getDataVector();
            root.allocateNew();
            vector.startNewValue(0);
            elements.setSafe(0, 1.5f);
            elements.setSafe(1, -2.0f);
            elements.setSafe(2, 3.25f);
            vector.endValue(0, 3);
            root.setRowCount(1);

            PaimonArrowConverter.RowReader rows =
                    new PaimonArrowConverter(ZoneId.of("UTC")).rows(
                            root, new org.apache.paimon.types.DataType[] {
                                    new VectorType(3, new FloatType(false))});
            InternalVector actual = (InternalVector) rows.values(0)[0];
            Assertions.assertArrayEquals(
                    new float[] {1.5f, -2.0f, 3.25f}, actual.toFloatArray());
        }
    }

    @Test
    public void testVectorTransportRejectsWrongDimension() {
        Field vectorField = vectorField();
        try (RootAllocator allocator = new RootAllocator();
                VectorSchemaRoot root = VectorSchemaRoot.create(
                        new Schema(Collections.singletonList(vectorField)), allocator)) {
            ...
            IllegalArgumentException exception = Assertions.assertThrows(
                    IllegalArgumentException.class, () -> rows.values(0));
            Assertions.assertTrue(exception.getCause().getMessage().contains("requires 3 elements"));
        }
    }
    ...
    private static Field vectorField() {
        return new Field(
                "embedding",
                FieldType.nullable(new ArrowType.List()),
                Collections.singletonList(new Field(
                        "element",
                        FieldType.notNullable(new ArrowType.FloatingPoint(
