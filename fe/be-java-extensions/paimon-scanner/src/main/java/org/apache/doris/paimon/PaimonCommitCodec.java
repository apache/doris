...
final class PaimonCommitCodec {
    ...
    /** Decode DPCM-framed chunks back into Paimon commit messages. */
    List<CommitMessage> decode(byte[][] payloads) throws IOException {
        List<CommitMessage> messages = new ArrayList<>();
        if (payloads == null) {
            throw new IOException("Paimon commit payloads are null");
        }
        for (byte[] payload : payloads) {
            if (payload == null || payload.length < HEADER_BYTES
                    || payload[0] != 'D' || payload[1] != 'P'
                    || payload[2] != 'C' || payload[3] != 'M') {
            ...
            if (length < 0 || payload.length != HEADER_BYTES + length) {
                throw new IOException("Invalid Paimon commit message payload length");
            }
            List<CommitMessage> chunk = serializer.deserializeList(
                    version,
                    new DataInputDeserializer(Arrays.copyOfRange(
                            payload, HEADER_BYTES, payload.length)));
            if (chunk == null) {
