...
final class PaimonArrowConverter {
    ...
    private Object convertVectorValue(
        ...
        if (targetType instanceof BlobType) {
            if (!(vector instanceof VarBinaryVector)) {
                throw new IllegalArgumentException(
                        "Paimon BLOB write requires an Arrow binary vector, but got "
                                + vector.getField());
            }
            return vector.isNull(index)
                    ? null : Blob.fromData(((VarBinaryVector) vector).get(index));
        }
        ...
        if (targetType instanceof VectorType) {
            if (!(vector instanceof ListVector)) {
                throw new IllegalArgumentException(
                        "Paimon VECTOR write requires an Arrow list vector, but got "
                                + vector.getField());
            }
            return vector.isNull(index)
                    ? null : convertVectorVector((ListVector) vector, index, (VectorType) targetType);
        }
    ...
    private Object convertToPaimonType(Object value, Field arrowField, DataType targetType) {
        ...
        if (targetType instanceof BlobType) {
            return Blob.fromData(toBytes(value));
        }
    ...
    private BinaryVector convertVectorVector(
            ListVector vector, int index, VectorType vectorType) {
        FieldVector elementVector = vector.getDataVector();
        int start = vector.getElementStartIndex(index);
        int end = vector.getElementEndIndex(index);
        int actualLength = end - start;
        if (actualLength != vectorType.getLength()) {
            throw new IllegalArgumentException(
                    "Paimon VECTOR requires " + vectorType.getLength()
                            + " elements, but Arrow row has " + actualLength);
        }
        Object[] converted = new Object[actualLength];
        for (int elementIndex = start; elementIndex < end; elementIndex++) {
            if (elementVector.isNull(elementIndex)) {
                throw new IllegalArgumentException("Paimon VECTOR elements must not be null");
            }
            converted[elementIndex - start] = convertVectorValue(
                    elementVector, elementIndex, elementVector.getField(),
        ...
        return BinaryVector.fromInternalArray(
                new GenericArray(converted), vectorType.getElementType());
    }
