...
public class DorisAdbcExecutor implements JdbcExecutor {
        ...
        try {
            ...
            while (curBlockRows < batchSize && streamNext()) {
                ...
                for (int i = 0; i < columnCount; ++i) {
                    ...
                    block.get(i)[curBlockRows] = getColumnValue(i, type, replaceStringList, streamRoot);
                }
                curBlockRows++;
            }
    ...
    private Object getColumnValue(int columnIndex, ColumnType type,
                                  String[] replaceStringList, VectorSchemaRoot streamRoot) {
        ...
        switch (type.getType()) {
            ...
            case DATETIMEV2:
                return  LocalDateTime.ofInstant(
                    Instant.ofEpochMilli(Long.parseLong(col.toString())), ZoneId.systemDefault()
                );
            case BOOLEAN:
            case TINYINT:
            case SMALLINT:
