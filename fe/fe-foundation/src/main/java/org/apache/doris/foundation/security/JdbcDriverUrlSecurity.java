 ...
 *
 * <p><b>Where callers invoke it: statement-time validation only.</b> The call sites are the
 * catalogs' {@code checkProperties()} hooks ({@code JdbcExternalCatalog},
 * {@code IcebergExternalCatalog}, {@code PaimonExternalCatalog}), which the engine reaches from the
 ...
 * after an FE restart, so the rule must never run from a metastore-properties build.
 *
