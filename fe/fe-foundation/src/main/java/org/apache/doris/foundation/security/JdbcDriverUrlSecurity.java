 ...
 * it lives in fe-foundation because that is the one module every properties holder already depends on.
 *
 ...
 * {@code jdbc_driver_secure_path} / {@code jdbc_driver_url_white_list} configs, which the engine applies
 * separately; this rule only forbids traversal and enforces the bare-name charset.
 *
 * <p><b>Where callers invoke it: statement-time validation only.</b> The call sites are the property
 * holders' create-time-only hooks ({@code JdbcCatalogProperties.checkCreateTimeOnlyRules} and the
 * iceberg/paimon JDBC metastore holders' {@code validate()}), which the engine reaches from the
 * user-facing CREATE and ALTER CATALOG paths and never from edit-log replay or a catalog rebuild.
 * That placement is load-bearing: a catalog created before this rule existed must keep coming back
 * after an FE restart, so the rule must never run from a holder's {@code of()}.
 *
