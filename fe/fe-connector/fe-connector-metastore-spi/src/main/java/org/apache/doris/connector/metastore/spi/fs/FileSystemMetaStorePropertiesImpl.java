...
/**
 * Filesystem (warehouse-only) metastore backend facts. The storage config is overlaid by the
 * connector at catalog-build time, so this impl carries only the warehouse and declares
 * {@link #needsStorage()} true.
 */
public final class FileSystemMetaStorePropertiesImpl extends AbstractMetaStoreProperties
        implements FileSystemMetaStoreProperties {

    private FileSystemMetaStorePropertiesImpl(Map<String, String> raw) {
        super(raw);
    }

    public static FileSystemMetaStorePropertiesImpl of(Map<String, String> raw) {
        FileSystemMetaStorePropertiesImpl props = new FileSystemMetaStorePropertiesImpl(raw);
