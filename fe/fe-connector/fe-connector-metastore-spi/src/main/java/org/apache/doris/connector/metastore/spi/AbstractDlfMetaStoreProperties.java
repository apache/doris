...
/**
 * Engine-neutral Aliyun DLF metastore backend base: the 8 {@code dlf.catalog.*} keys with
 * endpoint-from-region derivation and {@code catalogId}←{@code uid} fallback (legacy
 * {@code AliyunDLFBaseProperties}/{@code PaimonAliyunDLFMetaStoreProperties}), the OSS-storage overlay, and
 * the {@link #validateConnection()} AK/SK/endpoint rules shared by every engine. Concrete per-engine
 * subclasses supply the {@code of(...)} factory and {@code validate()} (paimon adds
 * {@code requireWarehouse()} + a paimon-specific OSS-storage requirement; iceberg uses the connection
 * check alone — see §4 of the P6-T10 design). {@link #needsStorage()} is true.
 */
public abstract class AbstractDlfMetaStoreProperties extends AbstractMetaStoreProperties
    ...
    /**
     * Engine-neutral DLF connection rules: AK, SK, then the endpoint-or-region derivability check (the
     * fail-fast mirror of {@link #toDlfCatalogConf()}'s endpoint derivation). Paimon prepends
     * {@code requireWarehouse()} and appends a paimon-specific OSS-storage requirement; iceberg uses this
     * check alone (it enforces neither warehouse nor OSS at validate) — see §4 of the P6-T10 design.
     */
