...
/**
 * Engine-neutral Hive Metastore (HMS) backend base: the field set, the {@link #toHiveConfOverrides(String)}
 * conf assembly, and the {@link #validateConnection()} connection rules shared by every engine (paimon,
 * iceberg, ...). Concrete per-engine subclasses live in their own module and supply only the {@code of(...)}
 * factory and {@code validate()} (paimon adds {@code requireWarehouse()}, iceberg uses the connection check
 * alone). This realizes the "MetaStoreProperties capability lives in the shared metastore layer, engines as
 * peers" split — mirroring fe-filesystem's interface-only spi + per-backend impls.
 *
 * <p>{@link #toHiveConfOverrides(String)} produces the neutral key map the connector layers onto its own
 * {@code HiveConf} (the connector seeds {@code new HiveConf()} + {@code hive.conf.resources} first, then
 * applies these overrides). Ported faithfully from the paimon connector's {@code buildHmsHiveConf}, whose
 * ordering is load-bearing: the storage overlay runs BEFORE the kerberos block so a raw
 * {@code hadoop.security.authentication=simple} passthrough cannot clobber the forced {@code kerberos}.
 *
 * <p>The real {@code UGI.doAs} is performed FE-side via {@code ConnectorContext.executeAuthenticated};
 * this base only carries facts ({@link #getAuthType()}, {@link #kerberos()}, neutral string keys), so
 * no hadoop authenticator code is needed here (DV-006).
 */
public abstract class AbstractHmsMetaStoreProperties extends AbstractMetaStoreProperties
    ...
    /**
     * Engine-neutral HMS connection rules (legacy {@code HMSBaseProperties.buildRules}). Fire order:
     * uri-required, then the CASE-SENSITIVE simple/kerberos auth-credential rules. Paimon prepends
     * {@code requireWarehouse()}; iceberg uses this check alone (it omits warehouse) — see §4 of the
     * P6-T10 design. Subclasses call this from their {@code validate()}.
     */
