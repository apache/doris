...
public class PaimonJdbcDriverUrlSecurityTest {
    ...
    @Test
    public void alterOverrideRejectsRepointedDriverUrl() {
        // PaimonConnectorProvider overrides validatePropertiesForUpdate WITHOUT funneling back to
        // validateProperties, so the rule must be wired into the override itself.
        // MUTATION: drop the driverUrlsToValidate(...).forEach(...) line from that override -> the
        // update is otherwise valid, so it returns -> red.
