...
/**
 * A per-scan {@link MetricRegistry} handed to the paimon SDK ({@code InnerTableScan.withMetricRegistry})
 * so {@code scan.plan()} records its {@code ScanMetrics} group here for {@link PaimonScanMetrics} to harvest
 * into the query profile. Self-contained port of the legacy {@code datasource.paimon.profile.PaimonMetricRegistry}
 * (the connector cannot depend on fe-core); paimon-SDK-only, no fe-core imports.
 */
