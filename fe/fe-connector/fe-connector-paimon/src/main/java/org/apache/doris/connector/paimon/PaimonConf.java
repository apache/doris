...
/**
 * This connector's deployment-level settings — the "B class" of the connector property convention,
 * as opposed to the per-catalog properties in {@link PaimonCatalogProperties}. These are read from
 * the plugin's own {@code paimon.conf} (named after {@code ConnectorProvider.name()}), each falling
 * back to the {@code ENV_} name below it, which is the fe.conf key it used to live under and still
 * works.
 *
 * <p>Both conf keys are shared with other connectors at the fe.conf end — one
 * {@code jdbc_drivers_dir} and one {@code hive_metastore_client_timeout_second} serve jdbc, iceberg
 * and paimon. A plugin conf cannot express that, so a deployment that moves to these files sets the
 * value in each plugin's own conf. That is the accepted cost of a per-plugin file; the fe.conf keys
 * stay as the shared fallback.
 */
public final class PaimonConf {
     ...
     * apply it when the catalog does not set {@code hive.metastore.client.socket.timeout} itself.
     * Falls back to the legacy literal so a deployment with neither file behaves as it did.
     */
