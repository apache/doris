...
/**
 * Manages Trino catalog lifecycle within the Doris connector plugin.
 *
 * <p>Adapted from {@code org.apache.doris.trinoconnector.TrinoConnectorServicesProvider}
 * in fe-common. This copy lives in the plugin module to avoid depending on fe-common
 * (which has heavy transitive dependencies like hadoop-common).</p>
 */
public class TrinoServicesProvider implements ConnectorServicesProvider {

    private static final Logger LOG = LogManager.getLogger(TrinoServicesProvider.class);
    ...
    public TrinoServicesProvider(String catalogName, String connectorName,
        ...
        ImmutableList.Builder<CatalogProperties> catalogPropsBuilder = ImmutableList.builder();

        Preconditions.checkState(connectorName != null,
                "TrinoConnector properties does not contain connector.name");
    ...
    public void ensureCatalogsLoaded(Session session, List<CatalogProperties> catalogs) {
        ...
        if (!missingCatalogs.isEmpty()) {
            throw new TrinoException(StandardErrorCode.CATALOG_NOT_AVAILABLE,
    ...
    public ConnectorServices getConnectorServices(CatalogHandle catalogHandle) {
        ...
        Preconditions.checkArgument(catalogConnector != null,
                "No catalog '%s'", catalogHandle.getCatalogName());
        return catalogConnector.getMaterializedConnector(catalogHandle.getType());
    }

    @Override
    public void pruneCatalogs(Set<CatalogHandle> catalogsInUse) {
        // static catalogs do not need pruning
    }
