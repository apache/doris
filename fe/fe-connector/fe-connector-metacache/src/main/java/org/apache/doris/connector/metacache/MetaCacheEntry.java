...
public class MetaCacheEntry<K, V> {
    private static final int DEFAULT_OBJECT_STRIPES = 256;
    private static final long DEFAULT_REFRESH_AFTER_WRITE_SECONDS = 600L;
    ...
    private final AtomicLong invalidationGeneration = new AtomicLong(0);
    ...
    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec, ExecutorService refreshExecutor,
            boolean autoRefresh) {
        this(name, loader, cacheSpec, refreshExecutor, autoRefresh, false, defaultObjectStripeCount(),
    ...
    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec,
        ...
        this(name, loader, cacheSpec, refreshExecutor, autoRefresh, contextualOnly,
                defaultObjectStripeCount(), DEFAULT_REFRESH_AFTER_WRITE_SECONDS, true, null, false);
    }

    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec, ExecutorService refreshExecutor,
            boolean autoRefresh, int stripeCount) {
        this(name, loader, cacheSpec, refreshExecutor, autoRefresh, false, stripeCount,
                DEFAULT_REFRESH_AFTER_WRITE_SECONDS, true, null, false);
    }

    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec,
            ExecutorService refreshExecutor, boolean autoRefresh, boolean contextualOnly, int stripeCount) {
        this(name, loader, cacheSpec, refreshExecutor, autoRefresh, contextualOnly, stripeCount,
                DEFAULT_REFRESH_AFTER_WRITE_SECONDS, true, null, false);
    }

    /**
     * Constructor used by connector-owned caches that resolve refresh and miss-load policy themselves.
     */
    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec,
            ExecutorService refreshExecutor, boolean autoRefresh, boolean contextualOnly,
            long refreshAfterWriteSeconds, boolean manualMissLoadEnabled) {
        this(name, loader, cacheSpec, refreshExecutor, autoRefresh, contextualOnly, defaultObjectStripeCount(),
                refreshAfterWriteSeconds, manualMissLoadEnabled, null, false);
    }

    /**
     * Constructor used by fe-core adapters to inject FE configuration without coupling this runtime to Config.
     */
    public MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec,
            ExecutorService refreshExecutor, boolean autoRefresh, boolean contextualOnly, int stripeCount,
            long refreshAfterWriteSeconds, boolean manualMissLoadEnabled) {
    ...
    public static <K, V> MetaCacheEntry<K, V> withSyncRemovalListener(String name, Function<K, V> loader,
            CacheSpec cacheSpec, ExecutorService refreshExecutor, BiConsumer<K, V> removalListener) {
    ...
    public static <K, V> MetaCacheEntry<K, V> withSyncRemovalListener(String name, Function<K, V> loader,
        ...
        BiConsumer<K, V> requiredRemovalListener =
        ...
        return new MetaCacheEntry<>(
                ...
                false,
                stripeCount,
                DEFAULT_REFRESH_AFTER_WRITE_SECONDS,
                true,
                requiredRemovalListener,
                true);
    }

    private MetaCacheEntry(String name, Function<K, V> loader, CacheSpec cacheSpec,
            ExecutorService refreshExecutor, boolean autoRefresh, boolean contextualOnly,
            int stripeCount, long refreshAfterWriteSeconds, boolean manualMissLoadEnabled,
            BiConsumer<K, V> removalListener, boolean syncRemovalListener) {
        ...
        if (autoRefresh && refreshAfterWriteSeconds <= 0) {
            throw new IllegalArgumentException(
                    "refreshAfterWriteSeconds must be positive when auto refresh is enabled");
        }
        if (stripeCount < 1) {
        ...
        if (syncRemovalListener) {
            this.loadingData = cacheFactory.buildCacheWithSyncRemovalListener(
                    cacheLoader, (key, value, cause) -> removalListener.accept(key, value));
        } else {
    ...
    /**
     * Return the entry-wide invalidation generation for callers that perform their own bulk load.
     */
    public long invalidationGeneration() {
        return invalidationGeneration.get();
    }
    ...
    private ActionPublicationToken<K> beginActionLoadLocked(StripeState<K> state, K key) {
        if (state.activeActionLoads == null) {
            state.activeActionLoads = new HashMap<>();
        }
    ...
    private void releaseActionLoad(
            StripeState<K> state, ActionPublicationToken<K> actionToken) {
        synchronized (state) {
            checkState(state.activeActionLoads != null);
            ActiveActionState currentState = state.activeActionLoads.get(actionToken.key);
            checkState(currentState == actionToken.state);
            checkState(actionToken.state.referenceCount > 0);
    ...
    private static void checkState(boolean condition) {
        if (!condition) {
            throw new IllegalStateException("MetaCacheEntry publication state is inconsistent");
        }
    }
}
