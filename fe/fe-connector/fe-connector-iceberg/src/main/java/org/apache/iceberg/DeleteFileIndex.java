 ...
 *
 * <p>P6.2-T08 (catalog-spi): VENDORED into the iceberg connector module (mirrors the identical fe-core copy)
 * because iceberg 1.10.1 made {@code org.apache.iceberg.DeleteFileIndex} package-private, and the connector —
 * which cannot import fe-core — needs it to drive {@code IcebergScanPlanProvider}'s manifest-level scan
 * planning (the path that consumes the connector-owned {@code IcebergManifestCache}). Being declared in package
 * {@code org.apache.iceberg} gives it package access to the SDK internals it depends on
 * ({@code ManifestEntry}, {@code PartitionMap}, ...). The connector only invokes the
 * {@link #builderFor(Iterable)} (already-loaded delete files) path; the {@code builderFor(FileIO, Iterable)}
 * manifest-reading path (which uses Caffeine) is retained verbatim for the split-package shadowing of the
 * SDK's own usage.
 */
