 ...
 *
 * <p>Connector-local port of {@code org.apache.doris.common.NamedArguments} (banned by the
 * fe-connector import gate). §4 = 4-A signed (D-062): iceberg {@code ALTER TABLE EXECUTE} argument
 * validation lives in the connector, so the engine keeps no per-procedure argument knowledge. The port
 * is verbatim except that {@link #validate} throws the connector's (unchecked) {@link DorisConnectorException}
 * in place of the (banned) {@code AnalysisException}; the validation <em>messages are kept byte-identical</em>
 * to legacy so the errors users see do not drift (T08 byte-parity). The engine still performs the
 * {@code ALTER} privilege check before dispatch.
 *
 * <p>
 * Usage example:
 *
...
public class NamedArguments {
     ...
     * @throws DorisConnectorException If validation or parsing fails
     */
    public void validate(Map<String, String> properties) throws DorisConnectorException {
        ...
        for (String providedArg : properties.keySet()) {
            if (!isRegisteredArgument(providedArg) && !isAllowedArgument(providedArg)) {
                throw new DorisConnectorException("Unknown argument: " + providedArg);
            }
        }
        ...
        for (ArgumentDefinition<?> arg : argumentDefinitions) {
            ...
            if (arg.isRequired() && stringValue == null) {
                throw new DorisConnectorException("Missing required argument: " + arg.getName());
            }
