 ...
 * default {@code asCatalog(empty)} used by the connector's catalog ops, the {@code asViewCatalog(empty)} view
 * path, and per-user delegated sessions (via {@code IcebergSessionCatalogAdapter}) — is a thin view that calls
 * back into this class (see {@link org.apache.iceberg.catalog.BaseSessionCatalog.AsCatalog}). Wrapping here
 * means no holder of any of those references ever sees a stale client after recovery, and non-REST catalogs
