...
public final class RangerUserStoreGroups {
     ...
     *
     * <p>To be called by a plugin's {@code setPolicies} before handing the policies on, on every call and
     * not only the first: a download of policy deltas comes with its own copy of the service definition,
     * which is why {@code RangerBasePlugin} re-adds the enricher on deltas too. The retriever class and the
     * refresh interval are read under the option names Ranger itself reads them under, so that an operator
     * who has tuned them for Ranger's own {@code use.rangerGroups} has tuned them here.
     */
     ...
     *
     * <p>Empty when this is switched off, when no user store has arrived yet - the plugin has not loaded its
     * policies, or Ranger Admin could not be reached - and when the store does not know the user, which is
     * the case for every account that exists in Doris only. Empty and not null on purpose: a request with an
     * empty group set matches items written against users and roles exactly as it did before.
     */
