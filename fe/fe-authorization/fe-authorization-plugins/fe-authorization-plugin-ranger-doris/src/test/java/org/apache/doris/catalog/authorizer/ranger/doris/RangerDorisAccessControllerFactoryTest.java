...
public class RangerDorisAccessControllerFactoryTest {
    ...
    public void testTheSelectorsThisSourceIsNamedBy() {
        ...
        Assertions.assertEquals("ranger-doris", factory.name());
    ...
    public void testBindingsConfiguredAlikeShareOneController() {
        ...
        try (MockedConstruction<RangerDorisPlugin> plugin =
            ...
            Assertions.assertEquals(1, plugin.constructed().size());
        }
    }
    ...
    public void testABindingConfiguredDifferentlyGetsItsOwnControllerOverTheSamePlugin() {
        ...
        try (MockedConstruction<RangerDorisPlugin> plugin =
            ...
            Assertions.assertNotSame(deferring, strict);
            Assertions.assertEquals(1, plugin.constructed().size(),
                    "one Ranger plugin for the whole FE, whatever the bindings configure");
            Assertions.assertTrue(allowsReading(deferring),
                    "the binding that defers to instance scope lost its configuration");
            Assertions.assertFalse(allowsReading(strict),
                    "the binding that refuses to defer was served the other one's configuration");
        }
    }
    ...
    public void testAReleasedControllerRefusesLaterChecks() {
        ...
        try (MockedConstruction<RangerDorisPlugin> plugin =
            ...
            AuthorizationPlugin controller = new RangerDorisAccessControllerFactory()
                    .create(Collections.emptyMap(), context);
            Assertions.assertTrue(allowsReading(controller));

            controller.close();

            Assertions.assertFalse(allowsReading(controller),
                    "a released controller still answered a privilege check");
