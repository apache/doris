...
/** Logical-partition and writer-assignment contract for external table sink writers. */
public abstract class DistributionSpecExternalTableSinkHashPartitioned extends DistributionSpec {
    ...
    protected DistributionSpecExternalTableSinkHashPartitioned(List<ExprId> outputColumnExprIds) {
        this.outputColumnExprIds = ImmutableList.copyOf(
                Objects.requireNonNull(outputColumnExprIds, "outputColumnExprIds must not be null"));
        if (this.outputColumnExprIds.isEmpty()) {
            throw new IllegalArgumentException("outputColumnExprIds must not be empty");
        }
    }
    ...
    /** Hash algorithm required by an external table sink. */
    ...
    /** Assignment policy for routing rows to sink writers. */
