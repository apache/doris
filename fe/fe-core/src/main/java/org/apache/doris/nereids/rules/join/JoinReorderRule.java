...
public class JoinReorderRule extends OneRewriteRuleFactory {
    ...
    public Rule build() {
        ...
        return subTree(LogicalJoin.class).thenApply(ctx -> {
            Plan root = ctx.root;
            Preconditions.checkState(root instanceof LogicalJoin);
            // HyperGraph graph = HyperGraph.from((LogicalJoin<? extends Plan, ? extends Plan>) root);
            return null;
