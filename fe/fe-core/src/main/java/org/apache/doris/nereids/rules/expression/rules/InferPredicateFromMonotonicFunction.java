...
public final class InferPredicateFromMonotonicFunction {
    ...
    private static Optional<Expression> infer(ComparisonPredicate comparison) {
        ...
        Optional<Expression> inferred = inferPrefixPredicate(normalized);
        if (!inferred.isPresent()) {
            inferred = inferYearPredicate(normalized);
        }
        if (!inferred.isPresent()) {
            inferred = inferDateFormatPredicate(normalized);
        }
        return inferred.isPresent() ? inferred : inferRoundingPredicate(normalized);
    }
    ...
    private static Optional<Literal> foldLiteral(Expression expression) {
        try {
            Expression foldable = expression instanceof BoundFunction
                    ...
                    : expression;
            // Only literal casts and deterministic date-boundary arithmetic are folded here.
            Expression folded = FoldConstantRuleOnFE.evaluate(foldable, null);
