...
public class PublishVersionDaemon extends MasterDaemon {
    ...
    private void publishVersion() {
        ...
        for (TransactionState transactionState : readyTransactionStates) {
            boolean hasUnfinishAndAliveBackend = transactionState.getPublishVersionTasks().values().stream()
                    .anyMatch(task -> !task.isFinished() && sysInfoService.checkBackendAlive(task.getBackendId()));
