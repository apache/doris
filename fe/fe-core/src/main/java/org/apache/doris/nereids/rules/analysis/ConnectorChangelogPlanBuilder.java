...
public final class ConnectorChangelogPlanBuilder {
    ...
    private static LogicalPlan buildUpdate(List<Column> schema, ConnectorChangelogMode mode,
        ...
        projects.add(operation(mode.getOperationColumnName(), mode.getUpdateValue()));
    ...
    private static LogicalPlan buildDelete(List<Column> schema, List<String> primaryKeys,
        ...
        ExpressionAnalyzer analyzer = analyzer(child, context);
        List<NamedExpression> projects = new ArrayList<>();
        projects.add(operation(mode.getOperationColumnName(), mode.getDeleteValue()));
        for (Column column : schema) {
        ...
        if (primaryKeys.isEmpty()) {
            throw new AnalysisException("Connector DELETE USING requires a primary-key table");
        }
    ...
    private static final class MergeBuilder {
        ...
        private LogicalPlan build() {
            ...
            for (int column = 0; column <= schema.size(); column++) {
                ...
                String name = column == 0
                        ? mode.getOperationColumnName() : schema.get(column - 1).getName();
        ...
        private void validateNotMatchedPrimaryKeys(List<List<Expression>> branches) {
            ...
            if (!(child instanceof LogicalJoin)) {
                throw new AnalysisException("Connector MERGE input must be a logical join");
            }
            Expression onClause = ((LogicalJoin<?, ?>) child).getOnClauseCondition()
                    .orElseThrow(() -> new AnalysisException("Connector MERGE requires an ON condition"));
        ...
        private AnalysisException invalidNotMatchedKeyCondition() {
            return new AnalysisException("Connector MERGE with NOT MATCHED INSERT requires ON to contain "
