...
/**
 * Each PlanNode represents a single relational operator and encapsulates the information needed by
 * the planner to make optimization decisions.
 * <p/>
 * finalize(): Computes internal state, such as keys for scan nodes; gets called once on the root of
 * the plan tree before the call to toThrift(). Also finalizes the set of conjuncts, such that each
 * remaining one requires all of its referenced slots to be materialized (ie, can be evaluated by
 * calling GetValue(), rather than being implicitly evaluated as part of a scan key).
 * <p/>
 * conjuncts: Each node has a list of conjuncts that can be executed in the context of this node,
 * ie, they only reference tuples materialized by this node or one of its children (= are bound by
 * tupleIds).
 */
public abstract class PlanNode extends TreeNode<PlanNode> implements PlanStats {
    ...
    // In the load execution plan, the difference between "preFilterConjuncts" and "conjuncts" is
    // that
    ...
    protected Map<SlotRef, IDict> requireEncodeSlotToDictColumn = Maps.newHashMap();
    ...
    /**
     * Sets tblRefIds_, tupleIds_, and nullableTupleIds_. The default implementation is a no-op.
     */
    ...
    /**
     * Set the limit to the given limit only if the limit hasn't been set, or the new limit is
     * lower.
     *
     * @param limit
     */
    ...
    void initCompoundPredicate(Expr expr) {
        if (expr instanceof CompoundPredicate) {
            ...
            args.add(Type.BOOLEAN);
            Function function =
                    new Function(new FunctionName("", compoundPredicate.getOp().toString()), args,
                            Type.BOOLEAN, false);
            ...
            for (int i = 0; i < targetConjuncts.size(); i += 2) {
                Expr expr = i + 1 < targetConjuncts.size()
                        ? new CompoundPredicate(CompoundPredicate.Operator.AND,
                                targetConjuncts.get(i), targetConjuncts.get(i + 1))
     ...
     * <p/>
     * root | |----child 2 | limit:1 | |----child 3 | limit:2 | child 1
     * <p/>
     * The root node header line will be prefixed by rootPrefix and the remaining plan output will
    ...
    protected final String getExplainString(String rootPrefix, String prefix,
        ...
        boolean traverseChildren =
                children != null && children.size() > 0 && !(this instanceof ExchangeNode);
        ...
        if (traverseChildren) {
            ...
            for (int i = 1; i < children.size(); ++i) {
                expBuilder.append(children.get(i).getExplainString(childHeadlinePrefix,
                        childDetailPrefix, detailLevel));
    ...
    /**
     * Return the node-specific details. Subclass should override this function. Each line should be
     * prefix by detailPrefix.
     */
    ...
    /**
     * Computes internal state, including planner-relevant statistics. Call this once on the root of
     * the plan tree before calling toThrift(). Subclasses need to override this.
     */
    ...
    /**
     * Computes planner statistics: avgRowSize. Subclasses need to override this. Assumes that it
     * has already been called on all children. This is broken out of finalize() so that it can be
     * called separately from finalize() (to facilitate inserting additional nodes during plan
     ...
     * This value is used to determine the distributed way(broadcast of shuffle) of join in the
     * distributed planning.
     *
     * If the new join reorder and the old join reorder have the same cardinality calculation
     * method, also the calculation is completed in the init(), there is no need to override this
     * function.
     */
    ...
    protected ExprSubstitutionMap getCombinedChildSmap() {
        ...
        ExprSubstitutionMap result = ExprSubstitutionMap.combine(getChild(0).getOutputSmap(),
    ...
    protected ExprSubstitutionMap getCombinedChildWithoutTupleIsNullSmap() {
        ...
        ExprSubstitutionMap result =
                ExprSubstitutionMap.combine(getChild(0).getWithoutTupleIsNullOutputSmap(),
                        getChild(1).getWithoutTupleIsNullOutputSmap());

        for (int i = 2; i < getChildren().size(); ++i) {
            result = ExprSubstitutionMap.combine(result,
                    getChild(i).getWithoutTupleIsNullOutputSmap());
        }
    ...
    protected void createDefaultSmap(Analyzer analyzer) throws UserException {
        ...
        outputSmap = ExprSubstitutionMap.compose(outputSmap, combinedChildSmap, analyzer);

        conjuncts = Expr.substituteList(conjuncts, outputSmap, analyzer, false);
    }

    /**
     * Appends ids of slots that need to be materialized for this tree of nodes. By default, only
     * slots referenced by conjuncts need to be materialized (the rationale being that only
     * conjuncts need to be evaluated explicitly; exprs that are turned into scan predicates, etc.,
     * are evaluated implicitly).
     */
    ...
    /**
     * Returns the estimated combined selectivity of all conjuncts. Uses heuristics to address the
     * following estimation challenges: 1. The individual selectivities of conjuncts may be unknown.
     * 2. Two selectivities, whether known or unknown, could be correlated. Assuming independence
     * can lead to significant underestimation.
     * <p>
     * The first issue is addressed by using a single default selectivity that is representative of
     * all conjuncts with unknown selectivities. The second issue is addressed by an exponential
     * backoff when multiplying each additional selectivity into the final result.
     */
    ...
    /**
     * Compute the product of the selectivity of all conjuncts. This function is used for old
     * cardinality in finalize()
     */
     ...
     *
     * @param requiredSlotIdSet: The upper plan node's requirement slot set for the current plan
     *        node. The requiredSlotIdSet could be null when the upper plan node cannot calculate
     *        the required slot.
     * @param analyzer
     * @throws NotImplementedException
     *
     *         For example: Query: select a.k1 from a, b where a.k1=b.k1 PlanNodeTree: output exprs:
     *         a.k1 | hash join node (input slots: a.k1, b.k1) | | scan a(k1) scan b(k1)
     *
     *         Function params: requiredSlotIdSet = a.k1 After function: hash join node (output
     *         slots: a.k1) (input slots: a.k1, b.k1)
     */
    public void initOutputSlotIds(Set<SlotId> requiredSlotIdSet, Analyzer analyzer)
            throws NotImplementedException {
        throw new NotImplementedException(
                "The `initOutputSlotIds` hasn't been implemented in " + planNodeName);
    }

    /**
     * If an plan node implements this method, its child plan node has the ability to implement the
     * project. The return value of this method will be used as the input(requiredSlotIdSet) of
     * child plan node method initOutputSlotIds. That is to say, only when the plan node implements
     * this method, its children can realize project optimization.
     *
     * @return The requiredSlotIdSet of this plan node
     * @throws NotImplementedException PlanNodeTree: agg node(group by a.k1) | hash join
     *         node(a.k1=b.k1) | | scan a(k1) scan b(k1) After function: agg node (required slots:
     *         a.k1)
     */
    public Set<SlotId> computeInputSlotIds() throws NotImplementedException {
        throw new NotImplementedException(
                "The `computeInputSlotIds` hasn't been implemented in " + planNodeName);
    }
    ...
    public void filterDictSlot(DecodeContext context) {
        ...
        // TODO: some predicate could also be optimized by global dict, we will support it in the
        // future
        conjuncts.forEach(e -> {
    ...
    public boolean shouldDecode(DecodeContext decodeContext) {
        return false;
    }
