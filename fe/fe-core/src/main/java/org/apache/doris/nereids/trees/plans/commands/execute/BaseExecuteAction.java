...
public abstract class BaseExecuteAction implements ExecuteAction {
    ...
    public final ResultSet execute(TableIf table) throws UserException {
        ...
        for (List<String> row : resultRows) {
            Preconditions.checkState(row != null && resultSetMetaData.getColumnCount() == row.size(),
     ...
     * @return result rows (possibly empty) if the action produces a result set, null otherwise
