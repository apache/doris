...
public class InternalCatalog implements CatalogIf<Database> {
    ...
    public boolean createTable(CreateMTMVCommand command) throws UserException {
        ...
        createOlapTable(db, createMTMVInfo);
    ...
    private Type getChildTypeByName(String name, CreateMTMVInfo createMTMVInfo)
    ...
    private void checkLegalityofPartitionExprs(CreateMTMVInfo createMTMVInfo, PartitionDesc partitionDesc)
            throws AnalysisException {
            ...
            if (expr instanceof FunctionCallExpr) { // test them
                ...
                for (int i = 0; i < children.size(); i++) {
                    if (children.get(i) instanceof LiteralExpr) {
                        ...
                        childTypes[i] = getChildTypeByName(children.get(i).getExprName(), createMTMVInfo);
                    } else {
    ...
    private boolean createOlapTable(Database db, CreateMTMVInfo createMTMVInfo) throws UserException {
        String tableName = createMTMVInfo.getTableName();
        if (LOG.isDebugEnabled()) {
        ...
        createTableBinlogConfig.mergeFromProperties(createMTMVInfo.getProperties());
        ...
        createMTMVInfo.getProperties().putAll(createTableBinlogConfig.toProperties());

        // get keys type
        KeysDesc keysDesc = createMTMVInfo.getKeysDesc();
        ...
        // create columns
        List<Column> baseSchema = createMTMVInfo.getColumns();
        validateColumns(baseSchema, isKeysRequired);
        checkAutoIncColumns(baseSchema, keysType);

        // analyze replica allocation
        ReplicaAllocation replicaAlloc = PropertyAnalyzer.analyzeReplicaAllocation(createMTMVInfo.getProperties(), "");
        if (replicaAlloc.isNotSet()) {
            replicaAlloc = ReplicaAllocation.DEFAULT_ALLOCATION;
        }

        long bufferSize = IdGeneratorUtil.getBufferSizeForCreateTable(createMTMVInfo, replicaAlloc);
        IdGeneratorBuffer idGeneratorBuffer = Env.getCurrentEnv().getIdGeneratorBuffer(bufferSize);
        ...
        if (ctx != null && env != null && partitionDesc != null && partitionDesc.getPartitionExprs() != null) {
            checkLegalityofPartitionExprs(createMTMVInfo, partitionDesc);
        }
        ...
        if (partitionDesc != null) {
            ...
            if (DynamicPartitionUtil.checkDynamicPartitionPropertiesExist(createMTMVInfo.getProperties())) {
        ...
        DistributionDesc distributionDesc = createMTMVInfo.getDistributionDesc();
        ...
        short shortKeyColumnCount = Env.calcShortKeyColumnCount(baseSchema, createMTMVInfo.getProperties(),
                isKeysRequired);
        if (LOG.isDebugEnabled()) {
        ...
        OlapTable olapTable = (OlapTable) new OlapTableFactory()
                ...
                .withExtraParams(createMTMVInfo)
                .build();
        olapTable.setComment(createMTMVInfo.getComment());
        ...
        Index.checkConflict(createMTMVInfo.getIndexes(), bfColumns);
        ...
        if (partitionInfo.getType() == PartitionType.UNPARTITIONED) {
            ...
            try {
                dataProperty = PropertyAnalyzer.analyzeDataProperty(createMTMVInfo.getProperties(),
        ...
        try {
            if (partitionInfo.getType() == PartitionType.UNPARTITIONED) {
                ...
                try {
                    DataProperty dataProperty = PropertyAnalyzer.analyzeDataProperty(createMTMVInfo.getProperties(),
                        new DataProperty(DataProperty.DEFAULT_STORAGE_MEDIUM));
            ...
            Pair<Boolean, Boolean> result = db.createTableWithLock(olapTable, false, createMTMVInfo.isIfNotExists());
            if (!result.first) {
                ErrorReport.reportDdlException(ErrorCode.ERR_TABLE_EXISTS_ERROR, tableShowName);
            }
