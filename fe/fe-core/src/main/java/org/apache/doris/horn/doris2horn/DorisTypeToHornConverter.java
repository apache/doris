...
/**
 * Converts Doris Nereids/catalog types to Horn TColumnType (Thrift). Both Nereids {@link DataType}
 * and legacy catalog {@link Type} dispatch through one switch on {@code PrimitiveType}; unsupported
 * types throw {@link UnsupportedOperationException} to trigger fallback to the Nereids optimizer.
 */
public class DorisTypeToHornConverter {
    ...
    /**
     * Map a Doris {@link Type} to a Horn {@link TScalarType}. Throws
     * {@link UnsupportedOperationException} for unsupported types so the caller can fall back to Nereids.
     */
    private static TScalarType mapPrimitive(Type catalogType) {
        ...
        switch (catalogType.getPrimitiveType()) {
            ...
            case NULL_TYPE:
                ...
                return scalarType;
            case INVALID_TYPE:
                // 无值表达式（WindowFrame / FrameBoundary 等）的 placeholder，对齐 kernel 端
                // Scalar 默认 INVALID_TYPE；BE 按 op_type 分发不读 data_type，不影响语义。
