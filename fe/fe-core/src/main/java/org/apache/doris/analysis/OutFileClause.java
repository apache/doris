...
public class OutFileClause {
    ...
    public void analyze(Analyzer analyzer, SelectStmt stmt) throws AnalysisException {
        ...
        analyze(analyzer);
        if(isParquetFormat() && this.schema.isEmpty()) {
            this.schema = ParquetPropertyAnalyzer.genSchemaByResultExpr(stmt.resultExprs);
        }
    }
    ...
    private void analyzeProperties() throws AnalysisException {
        ...
        if (isParquetFormat()) {
            parseParquetProperties(processedPropKeys);
        }
    ...
    private void parseParquetProperties(Set<String> processedPropKeys) throws AnalysisException {
        // 1. parse parquet file properties
        this.fileProperties = ParquetPropertyAnalyzer.parseFileProperties(properties, processedPropKeys);
        // 2. check schema
        String schema = properties.get(SCHEMA);
        if (schema.isEmpty()) {
            throw new AnalysisException("Parquet schema property should not be empty");
        }
        // schema is not defined, generate schema for it
        if (schema == null) {
            return;
        }
        this.schema = ParquetPropertyAnalyzer.parseSchema(schema);
        processedPropKeys.add(SCHEMA);
        // 3. compare schema and projection field
        ParquetPropertyAnalyzer.compareSchemaAndResultExpr(this.selectStmt.resultExprs, this.schema);
    }
