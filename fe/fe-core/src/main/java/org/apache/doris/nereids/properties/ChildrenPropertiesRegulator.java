...
public class ChildrenPropertiesRegulator extends PlanVisitor<List<List<PhysicalProperties>>, Void> {
    ...
    private void enforceSafeShuffleKeySubsetReuse(
        ...
        List<ExprId> fullShuffleKeys = AggregateUtils.getFullAggregateShuffleKeys(agg);
        List<Set<ExprId>> actualShuffleDimensions =
                ShuffleKeyPruneUtils.getIndependentShuffleDimensions(
                        actualChildHash.getOrderedShuffledColumns(), requiredChildHash, actualChildHash);
        ...
        List<ExprId> requiredShuffleKeys = requiredChildHash.getOrderedShuffledColumns();
        List<Set<ExprId>> requiredShuffleDimensions =
                ShuffleKeyPruneUtils.getIndependentShuffleDimensions(
                        requiredShuffleKeys, requiredChildHash, actualChildHash);
        boolean requiredKeysAreReduced = !fullShuffleKeys.isEmpty()
                && requiredShuffleKeys.size() < fullShuffleKeys.size();
        boolean actualKeysAreFurtherReduced =
                actualShuffleDimensions.size() < requiredShuffleDimensions.size();
        ...
        List<List<Slot>> actualShuffleKeyDimensions =
                ShuffleKeyPruneUtils.resolveShuffleKeyDimensions(
                        actualShuffleDimensions, agg.child().getOutput());
        boolean parentShuffleReuseRequest = requiredKeysAreReduced
                && (!agg.getPartitionExpressions().isPresent()
                        || agg.getPartitionExpressions().get().isEmpty());
        boolean safe = inputStatistics != null
                && (parentShuffleReuseRequest && !actualKeysAreFurtherReduced
        ...
        if (actualKeysAreFurtherReduced) {
            boolean requiredKeysAreSafe = !requiredKeysAreReduced;
            if (requiredKeysAreReduced && inputStatistics != null) {
                List<List<Slot>> requiredShuffleKeyDimensions =
                        ShuffleKeyPruneUtils.resolveShuffleKeyDimensions(
                                requiredShuffleDimensions, agg.child().getOutput());
                requiredKeysAreSafe = parentShuffleReuseRequest
                        ? ShuffleKeyPruneUtils.isSafeForParentShuffleDimensions(
                                requiredShuffleKeyDimensions, inputStatistics, instanceNum)
                        : ShuffleKeyPruneUtils.isSafeForShuffleDimensionsPruning(
                                requiredShuffleKeyDimensions, inputStatistics, instanceNum);
            }
            if (requiredKeysAreSafe) {
                updateChildEnforceAndCost(0, PhysicalProperties.createHash(
                        requiredChildHash.withShuffleType(ShuffleType.EXECUTION_BUCKETED)));
                return;
            }
        }
        updateChildEnforceAndCost(0, fullAggregateShuffleProperty(fullShuffleKeys, requiredChildHash));
    }
