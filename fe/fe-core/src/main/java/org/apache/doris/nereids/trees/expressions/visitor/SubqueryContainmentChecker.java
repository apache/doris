...
public class SubqueryContainmentChecker {
    ...
    public boolean check(SubqueryExpr self, SubqueryExpr other, boolean isForSpj) {
        ...
        if (!(selfTopLogicalPlan instanceof LogicalProject) || !(otherTopLogicalPlan instanceof LogicalProject)) {
            return false;
        } else if (isForSpj) {
    ...
    private boolean excludeSpecialFilter(LogicalFilter filter) {
        ...
        return !(filter.getConjuncts().size() == 1
            ...
            && ((SlotReference) ((EqualTo) filter.getConjuncts().iterator().next()).child(1)).getName()
                .equals("__DORIS_DELETE_SIGN__"));
    }
