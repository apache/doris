...
public class MigrationHandler extends AlterHandler {
    ...
    private void createJob(long dbId, OlapTable olapTable, Collection<Long> partitionIds,
        ...
        for (Map.Entry<Long, List<Column>> entry : olapTable.getIndexIdToSchema(true).entrySet()) {
            ...
            // 1. get new tablet version/tablet version hash, short key column count
            int currentSchemaVersion = currentIndexMeta.getSchemaVersion();
            int newSchemaVersion = currentSchemaVersion + 1;
            // generate tablet hash for new index has to generate a new tablet hash not equal to current tablet hash
            ...
            for (Partition partition : olapTable.getPartitions()) {
                ...
                for (Tablet originTablet : originIndex.getTablets()) {
                    ...
                    migrationJob.addTabletIdMap(partitionId, shadowIndexId, shadowTabletId, originTabletId);
        ...
        // 3. write edit log
        Catalog.getCurrentCatalog().getEditLog().logAlterJob(migrationJob);
        LOG.info("finished to create migration job: {}", migrationJob.getJobId());
    }
