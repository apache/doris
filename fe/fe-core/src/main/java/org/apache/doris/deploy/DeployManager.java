...
public class DeployManager extends MasterDaemon {
    ...
    private void processPolling() {
        for (NodeType nodeType : NodeType.values()) {
            ...
            List<SystemInfoService.HostInfo> remoteHosts = getGroupHostInfos(nodeType);
    ...
    private void process(NodeType nodeType, List<SystemInfoService.HostInfo> remoteHosts) {
        if (remoteHosts == null) {
            return;
        }
    ...
    private boolean needDrop(boolean found, SystemInfoService.HostInfo localHostInfo) {
        if (maxMissingTime <= 0) {
            return true;
        }
        if (found) {
     ...
     */
    private void inspectNodeChange(List<SystemInfoService.HostInfo> remoteHostInfos,
    ...
    private void dealDropLocal(SystemInfoService.HostInfo localHostInfo, NodeType nodeType) {
    ...
    private void dealAddRemote(SystemInfoService.HostInfo remoteHostInfo, NodeType nodeType) {
    ...
    private List<SystemInfoService.HostInfo> convertBrokersToHostInfos(List<FsBroker> brokers) {
        List<SystemInfoService.HostInfo> hostPortPair = Lists.newArrayList();
    ...
    private SystemInfoService.HostInfo convertToHostInfo(FsBroker broker) {
        return new SystemInfoService.HostInfo(broker.ip, null, broker.port);
    }
