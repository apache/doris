    ...
    public RowBatch getNext(Status status) throws TException {
        ...
        try {
            while (!isDone && !isCancel) {
                InternalService.PFetchDataRequest request = InternalService.PFetchDataRequest
                        .newBuilder().setFinstId(finstId).setRespInAttachment(false).build();
                ...
                if (packetIdx != pResult.getPacketSeq()) {
                    LOG.warn("receive packet failed, expect={}, receive={}", packetIdx,
                ...
                // It is very tricky here. For example, if sink closed on BE, and get a presult with
                // eos = true
                // did not deal with this pkg here. RowBatch.eos's default value = true, it is
                // depend on this to
                // terminate the query process.
                if (pResult.hasEmptyBatch() && pResult.getEmptyBatch()) {
                    LOG.info("get first empty rowbatch");
                    // If it is an empty batch, then it is set on purpose, then ignore the eos flag,
                    // try to get next batch.
                    // If it is already closed on BE, then it should not have empty batch and the
                    // eos flat == true.
