...
public class RoleManager implements Writable {
    ...
    public RoleManager() {
        roles.put(
                PaloRole.OPERATOR.getRoleName(), PaloRole.OPERATOR);
        roles.put(
                PaloRole.ADMIN.getRoleName(), PaloRole.ADMIN);
    }
    ...
    public PaloRole createDefaultRole(UserIdentity userIdent) {
        PaloRole role = new PaloRole(DEFAULT_ROLE_PREFIX + userIdent.toString());
        //todo 授默认权限
        //todo log
        roles.put(role.getRoleName(), role);
        return role;
    ...
    public PaloRole removeDefaultRole(UserIdentity userIdent) {
        //todo 授默认权限
        //todo log
        PaloRole role = roles.remove(DEFAULT_ROLE_PREFIX + userIdent.toString());
        return role;
    }
}
