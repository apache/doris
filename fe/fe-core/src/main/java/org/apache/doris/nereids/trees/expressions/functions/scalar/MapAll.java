...
/**
 * Scalar function map_all.
 *
 * <p>The Map lambda first produces one Boolean per entry, then ArrayMatchAll checks the result:
 *
 * <pre>
...
public class MapAll extends ScalarFunction
    ...
    @Override
    public MapAll withChildren(List<Expression> children) {
        Preconditions.checkArgument(children.size() == 1);
        return new MapAll(getFunctionParams(children));
    }
    ...
    public Expression rewriteWhenAnalyze() {
        return new ArrayMatchAll(getArgument(0));
    }

    @Override
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitMapAll(this, context);
    }
}
