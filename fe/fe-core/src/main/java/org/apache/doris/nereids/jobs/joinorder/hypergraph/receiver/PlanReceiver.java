...
//        }
    ...
    // after simplify graph, we may add hyper nodes in left or right extend nodes in edges
    // but these extend nodes is not connected sub-graph(csg), it means these nodes must be presented in sub-graph
    // when connecting it with its connected complement(cmp). we can't find all edges for join operator by enumerate
    // neighbours(it works only for csg or cmp). So we use processMissedEdges to find edges should be in join conjuncts
    // but can't be found by neighbours enumerating
    private boolean processMissedEdges(long left, long right, List<JoinEdge> edges) {
        // find all used edges
        BitSet usedEdgesBitmap = new BitSet();
        usedEdgesBitmap.or(usdEdges.get(left));
        usedEdgesBitmap.or(usdEdges.get(right));
