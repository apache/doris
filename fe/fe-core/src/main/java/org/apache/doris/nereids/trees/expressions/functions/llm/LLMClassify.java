...
/**
 * LLM function 'LLM_Classify'
 */
public class LLMClassify extends LLMFunction {
    ...
    public LLMClassify(Expression arg0, Expression arg1) {
        super("llm_classify", arg0, arg1);
        if (children.size() != 2) {
            throw new AnalysisException("LLM_Classify requires exactly 2 arguments: text and fields array, "
                    + "but got " + children.size() + " arguments");
        }
    }

    @Override
    public LLMClassify withChildren(List<Expression> newChildren) {
        Preconditions.checkArgument(newChildren.size() == 2,
                "LLM_Extract requires exactly 2 arguments");
        return new LLMClassify(newChildren.get(0), newChildren.get(1));
    }
    ...
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitLLMClassify(this, context);
    }
}
