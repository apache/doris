...
public class SecondFloor extends ScalarFunction
    ...
    private SecondFloor(ScalarFunctionParams functionParams) {
    ...
    public SecondFloor withChildren(List<Expression> children) {
        Preconditions.checkArgument(children.size() == 1
                || children.size() == 2
                || children.size() == 3);
        return new SecondFloor(getFunctionParams(children));
    }
    ...
    @Override
    public Optional<Expression> nextBucketBoundary(Literal value) {
        Optional<Expression> period = regularBucketPeriod();
        if (!period.isPresent()) {
            return Optional.empty();
        }
        return Optional.of(new SecondsAdd(value, period.get()));
    }
