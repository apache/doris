...
/**
 * ALTER BASELINE PLAN command (design doc 6.8 / 6.9 / 6.17).
 *
 * Syntax:
 *
 *   ALTER BASELINE PLAN baseline_id [ENABLE | DISABLE]
 *
 ...
 * SESSION-range id is handled by the connection-local SessionBaselineStore (a miss means
 * "does not exist" and is NEVER resolved against the shared store), a GLOBAL-range id by
 * BaselineManager.getInstance(). A disabled baseline no longer participates in query
 * rewrite matching.
 *
 * Execution location: a SESSION-range id can only exist in this connection's FE-local
 * store and is altered locally; a GLOBAL-range id is cluster metadata, so the command
 * runs on the master FE (Redirect/Forward, single writer), like other DDL - even while
 * the local cache does not know the id yet.
 */
public class AlterBaselinePlanCommand extends Command implements Forward {

    /** The target baseline id. */
    private final long baselineId;

    /** The desired target status (ENABLED / DISABLED). */
    private final BaselineStatus status;

    public AlterBaselinePlanCommand(long baselineId, BaselineStatus status) {
        super(PlanType.ALTER_BASELINE_PLAN_COMMAND);
     ...
     * store content: a session-range id can only be altered on this FE, a global-range id
    ...
    public void run(ConnectContext ctx, StmtExecutor executor) throws Exception {
        ...
        boolean updated = BaselineScope.ofId(baselineId) == BaselineScope.SESSION
                ? ctx.getSessionBaselineStore().updateStatus(baselineId, status)
                : BaselineManager.getInstance().updateStatus(baselineId, status);
        if (!updated) {
            throw new AnalysisException("Baseline plan " + baselineId + " does not exist");
        }
        String message = "Baseline plan " + baselineId + " is " + status;
        ctx.getState().setOk(0, 0, message);
    }

    @Override
    public <R, C> R accept(PlanVisitor<R, C> visitor, C context) {
        return visitor.visitAlterBaselinePlanCommand(this, context);
    }
    ...
    public void checkSupportedInCloudMode(ConnectContext ctx) throws DdlException {
        throw new DdlException("ALTER BASELINE PLAN is not supported in cloud mode");
    }
}
