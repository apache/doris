...
public class PartitionRebalancer extends Rebalancer {
    ...
    protected List<TabletSchedCtx> selectAlternativeTabletsForCluster(
        ...
        for (TwoDimensionalGreedyRebalanceAlgo.PartitionMove move : moves) {
            ...
            movesInProgress.get()
                .put(pickedTabletId, new Pair<>(new TabletMove(pickedTabletId, move.fromBe, move.toBe), -1L));
    ...
    private boolean buildClusterInfo(ClusterLoadStatistic clusterStat, TStorageMedium medium,
                                     List<TabletMove> movesInProgress, ClusterBalanceInfo info,
        ...
        List<TabletMove> filteredMoves =
            movesInProgress.stream().filter(m -> !toDeleteKeys.contains(m.tabletId)).collect(Collectors.toList());
        ...
        for (TabletMove move : filteredMoves) {
            ...
            TwoDimensionalGreedyRebalanceAlgo.PartitionMove partitionMove =
                new TwoDimensionalGreedyRebalanceAlgo.PartitionMove(meta.getPartitionId(), meta.getIndexId(),
                    move.fromBe, move.toBe);
            boolean st = TwoDimensionalGreedyRebalanceAlgo.applyMove(partitionMove, info.beByTotalReplicaCount,
        ...
        for (TabletMove move : moves) {
            ...
            if (moveIsComplete) {
                ...
                LOG.debug("Move {} is completed. The cur dist: {}", move,
                    invertedIndex.getReplicasByTabletId(move.tabletId).stream().map(Replica::getBackendId)
    ...
    private boolean checkMoveCompleted(TabletMove move) {
        ...
        List<Long> bes = invertedIndex.getReplicasByTabletId(tabletId).stream().map(Replica::getBackendId)
            .collect(Collectors.toList());
    ...
    protected void completeSchedCtx(TabletSchedCtx tabletCtx, Map<Long, TabletScheduler.PathSlot> backendsWorkingSlots)
        ...
        MovesCacheMap.MovesCache movesInProgress =
            movesCacheMap.getCache(tabletCtx.getCluster(), tabletCtx.getTag(), tabletCtx.getStorageMedium());
        Preconditions.checkNotNull(movesInProgress,
