...
class CostModelV1 extends PlanVisitor<Cost, PlanContext> {
    ...
    public Cost visitPhysicalHashAggregate(
        ...
        if (aggregate.getAggPhase().isLocal()) {
            return CostV1.of(context.getSessionVariable(), inputStatistics.getRowCount() / beNumber + exprCost,
                    inputStatistics.getRowCount() / beNumber, 0);
        } else {
            // global
            return CostV1.of(context.getSessionVariable(), inputStatistics.getRowCount() + exprCost,
