...
public class LineageInfoExtractor {
    ...
    private static void collectCteConsumerExprMappings(Plan plan,
            ExpressionLineageReplacer.ExpressionReplaceContext replaceContext) {
        if (plan == null || replaceContext == null) {
            return;
        }
        plan.accept(CteConsumerExprIdCollector.INSTANCE, replaceContext);
    }
    ...
    private static class CteConsumerExprIdCollector
        ...
        private static final CteConsumerExprIdCollector INSTANCE = new CteConsumerExprIdCollector();

        @Override
        public Void visitLogicalCTEConsumer(LogicalCTEConsumer cteConsumer,
                ExpressionLineageReplacer.ExpressionReplaceContext replaceContext) {
            if (cteConsumer == null || replaceContext == null) {
                return null;
            }
            Map<ExprId, Expression> exprIdExpressionMap = replaceContext.getExprIdExpressionMap();
            Set<ExprId> usedExprIdSet = replaceContext.getUsedExprIdSet();
            for (Map.Entry<Slot, Slot> mapping : cteConsumer.getConsumerToProducerOutputMap().entrySet()) {
                Slot consumerSlot = mapping.getKey();
                Slot producerSlot = mapping.getValue();
                if (!(consumerSlot instanceof SlotReference)) {
                    continue;
                }
                SlotReference consumerSlotRef = (SlotReference) consumerSlot;
                exprIdExpressionMap.putIfAbsent(consumerSlotRef.getExprId(), producerSlot);
                if (producerSlot instanceof NamedExpression) {
                    usedExprIdSet.add(((NamedExpression) producerSlot).getExprId());
                }
            }
            return super.visitLogicalCTEConsumer(cteConsumer, replaceContext);
        }
    }
