...
public class ConstantPropagation extends DefaultPlanRewriter<ExpressionRewriteContext> implements CustomRewriter {
    ...
    public Plan visitLogicalSort(LogicalSort<? extends Plan> sort, ExpressionRewriteContext context) {
        ...
        List<OrderKey> newOrderKeys = sort.getOrderKeys().stream()
                .map(key -> key.withExpression(
                        replaceConstants(key.getExpr(), context, childEqualTrait.first, childEqualTrait.second)))
                .collect(ImmutableList.toImmutableList());
    ...
    public Plan visitLogicalAggregate(LogicalAggregate<? extends Plan> aggregate, ExpressionRewriteContext context) {
        ...
        List<NamedExpression> constantExprs = Lists.newArrayList();
        ...
        for (NamedExpression expr : oldOutputExprs) {
            Expression newExpr = replaceConstants(expr, context, childEqualTrait.first, childEqualTrait.second);
            if (newExpr instanceof NamedExpression) {
                ...
                constantExprs.add(new Alias(expr.getExprId(), newExpr, expr.getName()));
            }
        }
        ...
        // ColumnPruning will add all group by expression into output expressions,
        // if new output expression list is empty, here need add group by,
        // otherwise it may cause loop like:
        // ConstantPropagation clear output expressions
        // => ColumnPruning add group by into output expressions
        // => ConstantPropagation clear output expression
        // => ...
        if (newOutputExprs.isEmpty()) {
            newGroupByExprs.stream().filter(e -> e instanceof NamedExpression)
                    .forEach(e -> newOutputExprs.add((NamedExpression) e));
        }
        if (newGroupByExprs.equals(oldGroupByExprs) && newOutputExprs.equals(oldOutputExprs)) {
            return aggregate;
        }

        return PlanUtils.projectOrSelf(constantExprs, aggregate.withGroupByAndOutput(newGroupByExprs, newOutputExprs));
    }
    ...
    public Plan visitLogicalSink(LogicalSink<? extends Plan> sink, ExpressionRewriteContext context) {
        ...
        Pair<ImmutableEqualSet<Slot>, Map<Slot, Literal>> childEqualTrait = getChildEqualSetAndConstants(sink, context);
    ...
    public Expression replaceConstantsAndRewriteExpr(LogicalPlan plan, Expression expression,
        ...
        for (int i = 0; i < 100; i++) {
            ...
            afterExpression = replaceConstants(beforeExpression, context,
    ...
    private Expression replaceConstants(Expression expression, ExpressionRewriteContext context,
            ImmutableEqualSet<Slot> parentEqualSet, Map<Slot, Literal> parentConstants) {
        if (expression instanceof And) {
            return replaceAndConstants((And) expression, context, parentEqualSet, parentConstants);
        } else if (expression instanceof Or) {
            return replaceOrConstants((Or) expression, context, parentEqualSet, parentConstants);
        } else if (!parentConstants.isEmpty()
                && expression.anyMatch(e -> e instanceof Slot && parentConstants.containsKey(e))) {
            Expression newExpr = ExpressionUtils.replace(expression, parentConstants);
    ...
    private Expression replaceAndConstants(And expression, ExpressionRewriteContext context,
            ImmutableEqualSet<Slot> parentEqualSet, Map<Slot, Literal> parentConstants) {
        ...
        for (Expression child : conjunctions) {
            ...
            if (needReplaceWithConstant(newChild, newConstants, myInferConstantSlots)) {
                newChild = replaceConstants(newChild, context, newEqualSet, newConstants);
            }
    ...
    private Expression replaceOrConstants(Or expression, ExpressionRewriteContext context,
            ImmutableEqualSet<Slot> parentEqualSet, Map<Slot, Literal> parentConstants) {
        ...
        for (Expression child : disjunctions) {
            Expression newChild = replaceConstants(child, context, parentEqualSet, parentConstants);
