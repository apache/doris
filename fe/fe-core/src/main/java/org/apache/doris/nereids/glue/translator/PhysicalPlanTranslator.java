...
public class PhysicalPlanTranslator extends DefaultPlanVisitor<PlanFragment, PlanTranslatorContext> {
    ...
    private void connectChildFragment(PlanNode node, int childIdx,
        ...
        if (!(child instanceof ExchangeNode)) {
            ...
            // if exchange node is generated by visitPhysicalDistribute,
            // we need to move it from child fragment to parent fragment
            childFragment.setPlanRoot(child.getChild(0));
        }
        child.setFragment(parentFragment);
        node.setChild(childIdx, child);
        childFragment.setDestination((ExchangeNode) child);
    }
