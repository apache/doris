...
public class ShowPolicyCommand extends Command implements NoForward {
    ...
    public void run(ConnectContext ctx, StmtExecutor executor) throws Exception {
        if (policyType == PolicyTypeEnum.DATA_MASK) {
            ShowPolicyStmt showPolicyStmt = new ShowPolicyStmt(PolicyTypeEnum.DATA_MASK, user, roleName);
            if (user != null) {
                user.analyze();
            }
            // check auth
            ...
            ShowResultSet showResultSet = Env.getCurrentEnv().getPolicyMgr().showPolicy(showPolicyStmt);
