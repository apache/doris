...
public abstract class LoadScanNode extends ScanNode {
    ...
    protected List<Expr> deleteConjuncts = Lists.newArrayList();
    ...
    private List<Expr> initConjuncts(Expr expr, Analyzer analyzer) throws UserException {
        if (expr == null) {
            return null;
        }
        ...
        expr = analyzer.getExprRewriter().rewrite(expr, analyzer);
        ...
        expr = expr.clone(smap);
        expr.analyze(analyzer);
        if (!expr.getType().equals(Type.BOOLEAN)) {
            throw new UserException("where statement is not a valid statement return bool");
        }
        return expr.getConjuncts();
    }

    protected void initWhereExpr(Expr whereExpr, Analyzer analyzer) throws UserException {
        addConjuncts(initConjuncts(whereExpr, analyzer));
    }

    protected void initDeleteExpr(Expr deleteExpr, Analyzer analyzer) throws UserException {
        List<Expr> conjs = initConjuncts(deleteExpr, analyzer);
        if (conjs == null) {
            return;
        } else {
            deleteConjuncts.addAll(conjs);
        }
    }
    ...
    protected void toThrift(TPlanNode planNode) {
        ...
        brokerScanNode.setDelete_conjuncts(Expr.treesToThrift(deleteConjuncts));
        brokerScanNode.setMerge_type(TMergeType.valueOf(mergeType.name()));
