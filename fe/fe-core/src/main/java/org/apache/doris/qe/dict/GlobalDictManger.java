...
public class GlobalDictManger extends Daemon {
    ...
    public void invalidDict(long dbId, long tableId, String column) {
        ...
        try {
            ...
            LOG.info("dict on db={}, table={}, column={} invalided, due to new data load", dbId,
                    tableId, column);
        } finally {
                    ...
                    for (Column column : allColumns) {
                        ...
                        if (column.isLowCardinality()) {
                            if (column.getDataType() == PrimitiveType.VARCHAR
                                    || column.getDataType() == PrimitiveType.CHAR
                                    || column.getDataType() == PrimitiveType.STRING) {
                                DictKey dictKey = new DictKey(db.getId(), olapTable.getId(),
                                        column.getName());
                                LOG.info("dict key is {}", dictKey);
                                if (!dictsMap.containsKey(dictKey)) {
                                    StringDict stringDict = new StringDict(
                                            this.dictIdGenerator.incrementAndGet(), db.getId(),
                                            olapTable.getId(), column.getName());
                                    // Has to set the empty dict invalid, for example,
                                    // 1. if fe set the column to low cardinality
                                    // 2. start a stream load, then the dictsmap does not contain
                                    // the dict, so that it return null.
                                    // 3. fe will think the column not related with dict or not low
                                    // cardinality, so that it will not
                                    // send dict to be, and be will not check the dict diff.
                                    // 4. be commit txn and not set the column to invalid.
                                    // 5. fe could not find the dict is invalid, and maybe use it
                                    // for future query, query failed.
                                    stringDict.invalidDict(true);
                                    dictsMap.put(dictKey, stringDict);
                                } else {
                                    // Set dict num element info to column meta, it is just user for
                                    // user to check the dict num
