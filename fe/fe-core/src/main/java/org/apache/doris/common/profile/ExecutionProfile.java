...
public class ExecutionProfile {
    ...
    // Map plan fragment id to fragment sequence number
    private Map<PlanFragmentId, Integer> planFragmentIdToIndex;
    ...
    public ExecutionProfile(TUniqueId queryId, List<PlanFragment> fragments) {
        ...
        planFragmentIdToIndex = Maps.newHashMap();
        ...
        for (PlanFragment planFragment : fragments) {
            ...
            planFragmentIdToIndex.put(planFragment.getFragmentId(), i);
            fragmentIdBeNum.put(i, 0);
            ++i;
        }
    ...
    public synchronized void addFragmentBackend(PlanFragmentId fragmentId, Long backendId) {
        int fragmentIndex = planFragmentIdToIndex.get(fragmentId);
        fragmentIdBeNum.put(fragmentIndex, fragmentIdBeNum.get(fragmentIndex) + 1);
    }
    ...
    public boolean isCompleted() {
        for (int i = 0; i < fragmentProfiles.size(); ++i) {
            ...
            if (fragmentProfile.isEmpty() || fragmentProfile.getChildList().size() < fragmentIdBeNum.get(i)) {
                return false;
            }
