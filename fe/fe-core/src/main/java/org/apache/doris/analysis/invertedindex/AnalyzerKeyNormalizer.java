...
public final class AnalyzerKeyNormalizer {
    private AnalyzerKeyNormalizer() {
    }

    public static void normalizeInvertedIndexProperties(Map<String, String> properties, String... keys) {
        if (properties == null || keys == null) {
            return;
        }
        for (String key : keys) {
            normalizePropertyIfPresent(properties, key);
        }
    }
