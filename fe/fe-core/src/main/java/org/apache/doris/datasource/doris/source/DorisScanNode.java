...
public class DorisScanNode extends ExternalScanNode {
    ...
    private List<Pair<String, Integer>> parseArrowNodes(List<String> feArrowNodes) {
        ...
        for (String feArrowNode : feArrowNodes) {
            ...
            hostsAndArrowPort.add(new Pair<>(split[0].trim(), Integer.parseInt(split[1].trim())));
        }
    ...
    private String getAdbcQueryStr() {
        ...
        if (sortInfo != null && sortLimit != -1) {
            String colAndIsAscList = IntStream.range(0, sortInfo.getOrderingExprs().size())
                .mapToObj(i -> {
                    String columnName = sortInfo.getOrderingExprs().get(i).getSrcSlotRef().toSql();
                    Boolean isAsc = sortInfo.getIsAscOrder().get(i);
                    return columnName + (isAsc ? " ASC " : " DESC ");
                })
                .collect(Collectors.joining(","));
            sql.append(" ORDER BY ").append(colAndIsAscList);
            sql.append(" LIMIT ").append(sortLimit);
        } else if (limit != -1) {
    ...
    private void executeAdbcQuery() {
        ...
        for (int i = 0; i < retrySize; i++) {
            ...
            try {
                ...
                LOG.warn("arrow request node [{}] failures {}, try next nodes", nextHostAndPort.getKey(), e);
    ...
    private List<Pair<String, byte[]>> getFlightInfoResult(Pair<String, Integer> hostAndPort, String user, String psw, String sql) throws Exception {
        ...
        try {
            ...
            final Location clientLocationFE = new Location(
                new URI("grpc", null, hostAndPort.getKey(), hostAndPort.getValue(), null, null, null)
            );
            ...
            for (FlightEndpoint endpoint : endpoints) {
                ...
                for (Location location : locations) {
                    Pair<String, byte[]> pair = new Pair<>(location.getUri().toString(), ticket.getBytes());
                    list.add(pair);
                }
            }
        } finally {
    ...
    private List<TScanRangeLocations> getShardLocations() throws UserException {
        ...
        for (Pair<String, byte[]> pair : flightInfoResult) {
            ...
            dorisArrowScanRange.setUriStr(pair.getKey());
            dorisArrowScanRange.setTicket(pair.getValue());
