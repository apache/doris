...
public class PhysicalDistribute<CHILD_TYPE extends Plan> extends PhysicalUnary<CHILD_TYPE>
    ...
    /**
     * Prune shuffle keys, preserving any existing aggregate provenance. {@code fromFullAggregateKeys}
     * records whether the current keys are all group-by keys of the parent aggregate.
     */
    public PhysicalDistribute<CHILD_TYPE> withPrunedShuffleKeys(
            List<ExprId> shuffleKeys, boolean fromFullAggregateKeys) {
        Preconditions.checkState(distributionSpec instanceof DistributionSpecHash);
        DistributionSpecHash hashSpec = (DistributionSpecHash) distributionSpec;
        Preconditions.checkArgument(!shuffleKeys.isEmpty()
                        && shuffleKeys.size() < hashSpec.getOrderedShuffledColumns().size()
                        && hashSpec.getOrderedShuffledColumns().containsAll(shuffleKeys),
                "Pruned shuffle keys must be a nonempty strict subset of the original keys");
        DistributionSpecHash prunedHashSpec = hashSpec.withShuffleExprs(shuffleKeys);
        PhysicalProperties prunedProperties = PhysicalProperties.createHash(prunedHashSpec)
                .withOrderSpec(physicalProperties.getOrderSpec());
        return AbstractPlan.copyWithSameId(this, () -> new PhysicalDistribute<>(prunedHashSpec, groupExpression,
                getLogicalProperties(), prunedProperties, statistics,
                prunedFromFullAggregateKeys || fromFullAggregateKeys, child()));
    }
