...
/**
 * Used to collect CriticalColumn.
 */
public class CriticalColumnCollector extends DefaultPlanRewriter<CollectorContext> implements CustomRewriter {
    ...
    public Plan visitLogicalProject(LogicalProject<? extends Plan> project, CollectorContext context) {
        ...
        if (project.child() instanceof LogicalOlapScan) {
            ...
            LogicalOlapScan scan = (LogicalOlapScan) project.child();
    ...
    public Plan visitLogicalJoin(LogicalJoin<? extends Plan, ? extends Plan> join, CollectorContext context) {
        ...
        context.usedInPredicate.addAll(
                join.getConditionSlot().stream().flatMap(s -> backtrace(s, context).stream())
                        .collect(Collectors.toSet())
        );
        return join;
    }
    ...
    private Set<Slot> backtrace(Slot slot, CollectorContext context) {
        ...
        for (SlotReference slotReference : slotReferences) {
            refCol.addAll(backtrace(slotReference, context));
        }
        return refCol;
    }
}
