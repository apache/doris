...
public class InternalQueryExecutor extends StmtExecutor {
    ...
    /**
     * If already pull as many records as need, but the result queue is still not empty then should
     * call cancel to stop the running thread
     */
    public void cancel() {
        isCancelled = true;
    }
    ...
    public void handleQueryStmt() throws Exception {
        QueryDetail queryDetail = new QueryDetail(context.getStartTime(),
                DebugUtil.printId(context.queryId()), context.getStartTime(), -1, -1,
                QueryDetail.QueryMemState.RUNNING, context.getDatabase(), originStmt.originStmt);
        ...
        while (!isCancelled) {
            ...
            if (batch.isEos()) {
                ...
                resultQueue.offer(new TResultBatch(null, false, -1), QUEUE_WAIT_SECONDS,
                        TimeUnit.SECONDS);
                break;
            }
        }
