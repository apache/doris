...
public class CountLiteralRewrite extends OneRewriteRuleFactory {
    ...
    private Expression rewrite(Count count) {
        ...
        if (!count.child(0).isLiteral()) {
            ...
            /*
             * The original count(expr) evaluates expr only for input rows, so an empty input
             * would not evaluate expr at all. After this rewrite, expr is evaluated above the
             * aggregate to decide whether the row-independent expression is NULL, so it can be
             * evaluated once per output group. We intentionally accept that trade-off: trying to
             * hide expr from BE fragment-local constant evaluation requires adding an aggregate
             * dependency into scalar subtrees, which breaks functions whose BE implementation
             * requires specific arguments to stay constant, such as date_trunc's time unit.
             */
            return new If(new IsNull(count.child(0)), new BigIntLiteral(0), countStar);
        }
        return new Count();
    }
}
