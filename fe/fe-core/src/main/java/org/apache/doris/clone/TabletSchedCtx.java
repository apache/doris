...
public class TabletSchedCtx implements Comparable<TabletSchedCtx> {
    ...
    public void chooseDestReplicaForVersionIncomplete(Map<Long, PathSlot> backendsWorkingSlots)
        ...
        if (chosenReplica.getState() == ReplicaState.DECOMMISSION) {
            ...
            LOG.info("choose replica {} on backend {} of tablet {} as dest replica for version incomplete," +
                    " and change state from DECOMMISSION to NORMAL", chosenReplica.getId(), chosenReplica.getBackendId(),
                tabletId);
        }
    ...
    public void releaseResource(TabletScheduler tabletScheduler, boolean reserveTablet) {
        ...
        if (storageMediaMigrationTask != null) {
            AgentTaskQueue.removeTask(storageMediaMigrationTask.getBackendId(), TTaskType.STORAGE_MEDIUM_MIGRATE,
                storageMediaMigrationTask.getSignature());
        }
        if (cloneTask != null) {
    ...
    public void finishCloneTask(CloneTask cloneTask, TFinishTaskRequest request)
        ...
        Database db = Catalog.getCurrentCatalog()
            .getDbOrException(dbId, s -> new SchedException(Status.UNRECOVERABLE, "db " + dbId + " does not exist"));
        ...
        olapTable.writeLockOrException(
            new SchedException(Status.UNRECOVERABLE, "table " + olapTable.getName() + " does not exist"));
        try {
