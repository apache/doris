...
public class NereidsPlanner extends Planner {
    ...
    /** explain and get the tree string from analyzed plan */
    public static String explainAnalyzedPlan(LogicalPlan parsedPlan, StatementContext statementContext) {
        NereidsPlanner nereidsPlanner = new NereidsPlanner(statementContext);

        LogicalPlan plan = nereidsPlanner.preprocess(parsedPlan);

        nereidsPlanner.initCascadesContext(plan);

        nereidsPlanner.analyze();

        Plan analyzedPlan = nereidsPlanner.getCascadesContext().getMemo().copyOut(false);
        return analyzedPlan.treeString();
    }

    /** explain and get the tree string from rewritten plan */
    public static String explainRewrittenPlan(LogicalPlan parsedPlan, StatementContext statementContext) {
        NereidsPlanner nereidsPlanner = new NereidsPlanner(statementContext);
        ...
        nereidsPlanner.rewrite();

        Plan rewrittenPlan = nereidsPlanner.getCascadesContext().getMemo().copyOut(false);
        return rewrittenPlan.treeString();
    }

    /** explain and get the tree string from optimized plan */
    public static String explainOptimizedPlan(LogicalPlan parsedPlan, StatementContext statementContext)
            throws AnalysisException {
        NereidsPlanner nereidsPlanner = new NereidsPlanner(statementContext);
        PhysicalPlan plan = nereidsPlanner.plan(parsedPlan, PhysicalProperties.ANY);
