...
public class NereidsPlanner extends Planner {
                    ...
                    switch (explainLevel) {
                        ...
                        case ALL_PLAN:
                            cascadesContext.getStatementContext().addPlanProcess(
                                    new PlanProcess(0, "ImplementSqlCache",
    ...
    private Plan planWithoutLock(
        ...
        // minidump of input must be serialized first, this process ensure minidump
        // string not null
        try {
        ...
        // serialize optimized plan to dumpfile, dumpfile do not have this part means
        // optimize failed
    ...
    private void configRuntimeFilterWaitTime() {
        if (ConnectContext.get() != null && ConnectContext.get().getSessionVariable() != null
                && ConnectContext.get().getSessionVariable().getRuntimeFilterWaitTimeMs() == VariableMgr
                        .getDefaultSessionVariable().getRuntimeFilterWaitTimeMs()) {
    ...
    protected void rewrite(boolean showPlanProcess) {
        cascadesContext.getStatementContext().addPlanProcess(
                new PlanProcess(0, "REWRITE BEGIN", "", ""));
        if (LOG.isDebugEnabled()) {
    ...
    protected void preMaterializedViewRewrite() {
        ...
        for (Plan planForRewrite : tmpPlansForMvRewrite) {
            ...
            try {
                ...
                // after rbo, maybe the plan changed a lot, so we need to normalize it with
                // original plan
    ...
    protected void optimize(boolean showPlanProcess) {
        ...
        // 2. ut test. In ut test, FeConstants.enableInternalSchemaDb is false or
        // FeConstants.runningUnitTest is true
        ...
        for (int seq = 0; seq < fragments.size(); seq++) {
            ...
            if (enableQueryCache) {
                try {
                    ...
                    Optional<TQueryCacheParam> queryCacheParam = normalizer
                            .normalize(cascadesContext.getConnectContext());
                    if (queryCacheParam.isPresent()) {
                        fragment.queryCacheParam = queryCacheParam.get();
                        // after commons-codec 1.14 (include), Hex.encodeHexString will change
                        // ByteBuffer.pos,
    ...
    public String getExplainString(ExplainOptions explainOptions) {
        ...
        if (ConnectContext.get() != null && cascadesContext != null
                && cascadesContext.getGroupExpressionCount() > ConnectContext.get()
                        .getSessionVariable().memoMaxGroupExpressionSize) {
    ...
    protected void keepOrShowPlanProcess(boolean showPlanProcess, Runnable task) {
        if (showPlanProcess) {
            cascadesContext.getStatementContext().withPlanProcess(showPlanProcess, task);
        } else {
            task.run();
        }
    }
