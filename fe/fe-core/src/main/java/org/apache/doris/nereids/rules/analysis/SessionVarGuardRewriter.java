...
public class SessionVarGuardRewriter extends ExpressionRewrite {
    ...
    /**
     * Whether a stored column expression (a generated column or a synchronous materialized view column
     * definition) is time-zone sensitive: its materialized value depends on the session time zone because
     * a TIMESTAMPTZ operand (possibly nested in a complex type) is converted into a zone-dependent
     * representation. Such expressions cannot be kept consistent across sessions: BE evaluates them in the
     * write/load session time zone, so a value created in one zone and written in another would silently
     * materialize the wrong value. They are therefore rejected at creation time. The exception is a direct
     * CAST of a TIMESTAMPTZ instant into a STRING: the string always embeds the session offset (see
     * {@link #isTimeZoneSensitiveCast}), so it is self-describing and never silently misrepresents the
     * instant, even when the write/load session differs from the reader's.
     */
    ...
    private static boolean isTimeZoneSensitiveLegacy(Expr expr) {
        if (expr instanceof SlotRef || expr instanceof LiteralExpr) {
            // A bare slot/literal only carries a value; whether that value is zone dependent is decided by
            // the operation that renders or interprets it (a TIMESTAMPTZ slot holds a zone-independent
            // instant, an offset-free string slot holds whatever the storage session wrote).
            return false;
        }
        if (expr instanceof IsNullPredicate) {
            // the boolean result of IS (NOT) NULL does not depend on the time zone
            return false;
        }
        if (expr instanceof CastExpr) {
            return isTimeZoneSensitiveCast((CastExpr) expr);
        }
        if (expr instanceof BinaryPredicate && isZoneInvariantComparison((BinaryPredicate) expr)) {
            // Comparing two TIMESTAMPTZ instants is zone invariant, but the operands may themselves be
            // zone-sensitive expressions (e.g. date_trunc(ts, 'day') = ts2), so still recurse into them.
            return isTimeZoneSensitiveLegacy(expr.getChild(0)) || isTimeZoneSensitiveLegacy(expr.getChild(1));
        }
        if (expr instanceof FunctionCallExpr && ((FunctionCallExpr) expr).isAggregateFunction()) {
            // aggregates preserve the UTC instant; only a time-zone conversion inside their argument
        ...
        for (Expr child : expr.getChildren()) {
            if (containsTimeStampTz(child.getType())) {
                return true;
            }
            if (isTimeZoneSensitiveLegacy(child)) {
                return true;
    ...
    /**
     * Classify a cast by both its source and its result type instead of only the children: casting INTO
     * TIMESTAMPTZ interprets an offset-free source representation (string/datetime) in the session zone, and
     * casting a TIMESTAMPTZ instant into a zone-free representation (DATETIME/INT/...) renders it in the
     * session zone. TIMESTAMPTZ &lt;-&gt; TIMESTAMPTZ is instant preserving and only recurses into the source.
     * The single exception is rendering into a STRING: BE's TimestampTzValue::to_string() always embeds the
     * session offset (e.g. "2024-01-01 08:00:00+08:00"), so the stored string is self-describing and never
     * silently misrepresents the instant, even when the write/load session differs from the reader's.
     */
    private static boolean isTimeZoneSensitiveCast(CastExpr castExpr) {
        Expr source = castExpr.getChild(0);
        boolean sourceTz = containsTimeStampTz(source.getType());
        boolean targetTz = containsTimeStampTz(castExpr.getType());
        if (targetTz && !sourceTz) {
            // interpreting an offset-free representation as an instant uses the session zone
            return true;
        }
        if (sourceTz && !targetTz) {
            // A TIMESTAMPTZ instant rendered into a STRING always carries the session offset, so the stored
            // string is self-describing; only zone-free non-string targets (DATETIME/INT/...) are ambiguous.
            if (source.getType().isTimeStampTz() && castExpr.getType().isStringType()) {
                return false;
            }
            // rendering an instant into a zone-free representation uses the session zone
            return true;
        }
        // no TIMESTAMPTZ involved, or TIMESTAMPTZ <-> TIMESTAMPTZ (instant preserving)
        return isTimeZoneSensitiveLegacy(source);
    }

    /**
     * Whether the comparison compares two TIMESTAMPTZ instants directly. Equality / inequality of instants
     * does not depend on the session zone (unlike e.g. ts = '2024-01-01 00:00:00', where the offset-free
     * literal is interpreted in the session zone).
     */
    private static boolean isZoneInvariantComparison(BinaryPredicate binaryPredicate) {
        BinaryPredicate.Operator op = binaryPredicate.getOp();
        if (op != BinaryPredicate.Operator.EQ && op != BinaryPredicate.Operator.NE
                && op != BinaryPredicate.Operator.EQ_FOR_NULL) {
            return false;
        }
        return containsTimeStampTz(binaryPredicate.getChild(0).getType())
                && containsTimeStampTz(binaryPredicate.getChild(1).getType());
    }

    private static boolean containsTimeStampTz(org.apache.doris.catalog.Type type) {
        if (type.isTimeStampTz()) {
            return true;
        }
        if (type.isArrayType()) {
            return containsTimeStampTz(((org.apache.doris.catalog.ArrayType) type).getItemType());
        }
        if (type.isMapType()) {
        ...
        if (type.isStructType()) {
            for (org.apache.doris.catalog.StructField field
                    : ((org.apache.doris.catalog.StructType) type).getFields()) {
                if (containsTimeStampTz(field.getType())) {
                    return true;
                }
            }
        }
