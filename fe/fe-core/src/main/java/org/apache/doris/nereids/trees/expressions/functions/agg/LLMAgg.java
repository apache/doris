...
/**
 * AggregateFunction 'LLM_AGG'.
 */
public class LLMAgg extends NullableAggregateFunction
     ...
     */
    public LLMAgg(Expression arg0, Expression arg1) {
     ...
     */
    public LLMAgg(Expression arg0, Expression arg1, Expression arg2) {
    ...
    public void checkLegalityAfterRewrite() {
        if (!child(arity() - 1).isLiteral()) {
            throw new AnalysisException("LLM_AGG must accept literal for the task.");
        }

        if (arity() == 3) {
            //The resource must be literal
            if (!child(0).isLiteral() || !child(2).isLiteral()) {
                throw new AnalysisException("LLM_AGG must accept literal for the resource name.");
            }
    ...
    @Override
    public LLMAgg withDistinctAndChildren(boolean distinct, List<Expression> children) {
        Preconditions.checkArgument(children.size() == 3,
                "LLM_AGG only accept two or three parameters");
        return new LLMAgg(getFunctionParams(distinct, children));
    }

    @Override
    public NullableAggregateFunction withAlwaysNullable(boolean alwaysNullable) {
        return new LLMAgg(getAlwaysNullableFunctionParams(alwaysNullable));
    }
    ...
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitLLMAgg(this, context);
    }
