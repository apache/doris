...
public class CascadesContext implements ScheduleContext {
     ...
     * referenced.
     */
    private final Map<CTEId, Map<Slot, AtomicInteger>> producerSlotRefCount = new HashMap<>();

    private final Map<Integer, DistributionSpec> cteConsumerIdToDistribution = new HashMap<>();

    // Map alias of slot to slot from consumer
    private final Map<NamedExpression, Slot> aliasSlotToCTESlot = new HashMap<>();
    private final Map<Slot, CTEId> cteSlotToCTEId = new HashMap<>();

    private final Map<CTEId, Map<Slot, Slot>> cteIdToConsumerToProducerOutputMap = new HashMap<>();
    ...
    public Map<NamedExpression, Slot> getAliasSlotToCTESlot() {
        return aliasSlotToCTESlot;
    }

    public Map<Slot, CTEId> getCteSlotToCTEId() {
        return cteSlotToCTEId;
    }

    public Map<CTEId, Map<Slot, Slot>> getCteIdToConsumerToProducerOutputMap() {
        return cteIdToConsumerToProducerOutputMap;
    }
}
