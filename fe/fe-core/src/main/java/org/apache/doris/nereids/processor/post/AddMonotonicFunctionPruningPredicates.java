    ...
    private static boolean supportsStoragePruning(Plan plan) {
        ...
        if (plan instanceof PhysicalFileScan) {
            ...
            if (table instanceof HMSExternalTable) {
                ...
                // getDlaType initializes the HMS metadata before the file-format check.
                DLAType dlaType = hiveTable.getDlaType();
                return dlaType == DLAType.ICEBERG
                        || (dlaType == DLAType.HIVE && hiveTable.supportedHiveTopNLazyTable());
            }
            // Iceberg system tables use a separate ExternalTable subclass and stay excluded.
