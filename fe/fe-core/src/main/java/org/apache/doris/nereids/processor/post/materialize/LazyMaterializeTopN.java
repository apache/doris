...
public class LazyMaterializeTopN extends PlanPostProcessor {
    ...
    private Plan doComputeTopN(PhysicalTopN<? extends Plan> topN, CascadesContext ctx, List<Slot> effectiveOutput) {
        ...
        // the second phase fetch goes to those backends directly (see MaterializationNode
        // nodes info and the cross cluster multiget rpc). Compute the whole fetch address
        // book here, once, and carry it on the PhysicalLazyMaterialize node, so the conflict
        // guard and the address book can never drift apart. Tables of the same remote
        // catalog share one backend map, so record the catalog only: a self join or two
        ...
        for (Relation relation : relationToLazySlotMap.keySet()) {
            ...
            // A remote doris table reaching here is always a RemoteOlapTable bound in the
            // virtual cluster mode; the arrow flight mode binds a RemoteDorisExternalTable
            // which is rejected by MaterializeProbeVisitor beforehand.
            if (relationTable instanceof RemoteOlapTable) {
                remoteCatalogs.add(((RemoteOlapTable) relationTable).getCatalog());
            } else {
                involvesLocalCluster = true;
            }
        }
        // Pass the raw compute group backends; the availability filtering (and the local
        // blacklist semantics) is applied inside buildFetchBackends. Resolution failure
        // degrades to normal execution here instead of failing the query at translate time.
        ...
        if (involvesLocalCluster) {
            try {
                ...
                LOG.warn("Skip TopN lazy materialization: failed to resolve local fetch backends", e);
                return topN;
            }
        }
        ...
        List<Backend> fetchBackends = buildFetchBackends(localBackends, remoteBackends);
        if (fetchBackends == null) {
            LOG.warn("Skip TopN lazy materialization: backend id collides between the involved"
                    + " clusters, remote catalogs={}", remoteCatalogs);
            return topN;
        }
        for (Relation relation : relationToLazySlotMap.keySet()) {
        ...
        if (materializeInput == null) {
            ...
            result = new PhysicalLazyMaterialize(result, result.getOutput(),
                    ...
                    fetchBackends, null, ((AbstractPlan) result).getStats());
    ...
    /**
     * Builds the phase-2 fetch address book and rejects backend id collisions in one pass.
     * Local backends go through the full selection policy (alive, query available, and the
     * SimpleScheduler blacklist keyed by the numeric backend id); ids are unique within one
     * cluster, so the filtered result directly forms the base of the address book. Remote
     * doris catalog backends are numbered by an independent id space and must skip that
     * blacklist: a dead local backend would otherwise evict a healthy remote backend sharing
     * the same id. They are only filtered by query availability, which on a
     * Backend.fromThrift object degrades to the alive flag, and each of them must claim an
     * id not used by the base or by an earlier remote backend.
     *
     * @return null when any backend id appears in more than one cluster's contribution; the
     *         collision would silently route the fetch to a wrong backend, so the caller
     *         must skip the lazy materialization rewrite.
     */
    @VisibleForTesting
    static List<Backend> buildFetchBackends(List<Backend> localBackends, List<Backend> remoteBackends) {
