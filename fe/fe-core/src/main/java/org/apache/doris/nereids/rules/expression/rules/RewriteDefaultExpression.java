...
/** Rewrite DEFAULT(column) to the column's resolved default value. */
public class RewriteDefaultExpression implements ExpressionPatternRuleFactory {
    ...
    private static Expression rewrite(ExpressionMatchingContext<Default> context) {
        ...
        if (originalTable.filter(IcebergExternalTable.class::isInstance).isPresent()) {
            if (!icebergContext.isPresent()
                    || !icebergContext.get().isTargetTable(originalTable.get().getId())
                    || !icebergContext.get().findField(column).isPresent()) {
                throw new AnalysisException("DEFAULT(" + slotRef.toSql()
                        + ") must reference a column in the pinned Iceberg write target");
            }
