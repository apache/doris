...
public class MigrationJob extends AlterJobV2 {
    ...
    public MigrationJob(long jobId, long dbId, long tableId, String tableName, long timeoutMs) {
        super(jobId, JobType.MIGRATION, dbId, tableId, tableName, timeoutMs);
    }
    ...
    protected void runPendingJob() throws AlterCancelException {
        ...
        try {
            ...
            Preconditions.checkState(tbl.getState() == OlapTableState.MIGRATION);
        ...
        LOG.info("transfer migration job {} state to {}, watershed txn id: {}", jobId, this.jobState, watershedTxnId);
    }
     ...
     * 2. If all previous transactions finished, send migration tasks to BE.
    ...
    protected void runWaitingTxnJob() throws AlterCancelException {
        ...
        try {
            if (!isPreviousLoadFinished()) {
                LOG.info("wait transactions before {} to be finished, migration job: {}", watershedTxnId, jobId);
                return;
            }
        ...
        try {
            ...
            for (long partitionId : partitionIndexMap.rowKeySet()) {
                ...
                // the migration task will transform the data before visible version(included).
        ...
        AgentTaskQueue.addBatchTask(migrationBatchTask);
        AgentTaskExecutor.submit(migrationBatchTask);

        this.jobState = JobState.RUNNING;

        // DO NOT write edit log here, tasks will be send again if FE restart or master changed.
        LOG.info("transfer migration job {} state to {}", jobId, this.jobState);
    }

    /**
     * runRunningJob()
     * 1. Wait all migration tasks to be finished.
    ...
    protected void runRunningJob() throws AlterCancelException {
        ...
        if (!migrationBatchTask.isFinished()) {
            LOG.info("migration tasks not finished. job: {}", jobId);
        ...
        Catalog.getCurrentCatalog().getEditLog().logAlterJob(this);
        LOG.info("migration job finished: {}", jobId);
    }
     ...
     */
    private void replayCreateJob(MigrationJob replayedJob) throws MetaNotFoundException {
        ...
        LOG.info("replay pending migration job: {}, table id: {}", jobId, tableId);
    }
     ...
     */
    private void replayPendingJob(MigrationJob replayedJob) throws MetaNotFoundException {
        Database db = Catalog.getCurrentCatalog().getDbOrMetaException(dbId);
        OlapTable olapTable = db.getTableOrMetaException(tableId, TableType.OLAP);
        olapTable.writeLock();
        try {
        ...
        this.jobState = JobState.WAITING_TXN;
        this.watershedTxnId = replayedJob.watershedTxnId;
        LOG.info("replay waiting txn migration job: {} table id: {}", jobId, tableId);
    }
     ...
     */
    private void replayRunningJob(MigrationJob replayedJob) {
        Database db = Catalog.getCurrentCatalog().getDbNullable(dbId);
        if (db != null) {
            OlapTable tbl = (OlapTable) db.getTableNullable(tableId);
            if (tbl != null) {
        ...
        LOG.info("replay finished migration job: {}", jobId);
    }

    /**
     * Replay job in CANCELLED state.
     */
    private void replayCancelled(MigrationJob replayedJob) {
        cancelInternal();
        this.jobState = JobState.CANCELLED;
        this.finishedTimeMs = replayedJob.finishedTimeMs;
        this.errMsg = replayedJob.errMsg;
        LOG.info("replay cancelled migration job: {}", jobId);
    }

    @Override
    public void replay(AlterJobV2 replayedJob) {
        try {
            MigrationJob replayedSchemaChangeJob = (MigrationJob) replayedJob;
            ...
            LOG.warn("[INCONSISTENT META] replay migration job failed {}", replayedJob.getJobId(), e);
        }
    }
    ...
    protected void getInfo(List<List<Comparable>> infos) {
        ...
        if (jobState == JobState.RUNNING && migrationBatchTask.getTaskNum() > 0) {
            progress = migrationBatchTask.getFinishedTaskNum() + "/" + migrationBatchTask.getTaskNum();
        }
    ...
    public List<List<String>> getUnfinishedTasks(int limit) {
        ...
        if (jobState == JobState.RUNNING) {
            List<AgentTask> tasks = migrationBatchTask.getUnfinishedTasks(limit);
            for (AgentTask agentTask : tasks) {
                StorageMediaMigrationV2Task migrationV2Task = (StorageMediaMigrationV2Task) agentTask;
                List<String> info = Lists.newArrayList();
                info.add(String.valueOf(migrationV2Task.getBackendId()));
                info.add(String.valueOf(migrationV2Task.getBaseTabletId()));
                info.add(String.valueOf(migrationV2Task.getSignature()));
                taskInfos.add(info);
            }
        }
        return taskInfos;
