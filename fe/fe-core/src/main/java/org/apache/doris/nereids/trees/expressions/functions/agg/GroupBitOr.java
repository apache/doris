...
/** min agg function. */
public class GroupBitOr extends AggregateFunction implements UnaryExpression, PropagateNullable, CustomSignature {

    public GroupBitOr(Expression child) {
        super("group_bit_or", child);
    }

    public GroupBitOr(AggregateParam aggregateParam, Expression child) {
        super("group_bit_or", aggregateParam, child);
    }
    ...
    public GroupBitOr withChildren(List<Expression> children) {
        ...
        return new GroupBitOr(getAggregateParam(), children.get(0));
    }

    @Override
    public GroupBitOr withAggregateParam(AggregateParam aggregateParam) {
        return new GroupBitOr(aggregateParam, child());
    }
