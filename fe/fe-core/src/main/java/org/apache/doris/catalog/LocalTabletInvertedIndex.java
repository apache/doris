...
public class LocalTabletInvertedIndex extends TabletInvertedIndex {
    ...
    private void logTabletReportSummary(long backendId, long feTabletNum,
        ...
        long toPublishTransactionsPartitions = transactionsToPublish.values().stream()
                .flatMap(m -> m.values().stream()).mapToLong(info -> info.partitionVersionInfos.size()).sum();
    ...
    private void handleBackendTransactions(long backendId, List<Long> transactionIds, long tabletId,
            TabletMeta tabletMeta, Map<Long, Map<Long, RepublishVersionInfo>> transactionsToPublish,
            SetMultimap<Long, Long> transactionsToClear) {
    ...
    private void publishPartition(TransactionState transactionState, long transactionId, TabletMeta tabletMeta,
            long partitionId, Map<Long, Map<Long, RepublishVersionInfo>> transactionsToPublish) {
        ...
        if (versionInfo != null) {
            synchronized (transactionsToPublish) {
                ...
                RepublishVersionInfo info = transactionsToPublish
                        .computeIfAbsent(transactionState.getDbId(), id -> Maps.newLinkedHashMap())
                        .computeIfAbsent(transactionId,
                                id -> new RepublishVersionInfo(transactionState.getRowBinlogColumnMappings(id)));
                info.partitionVersionInfos.add(versionInfo);
            }
        }
    }
