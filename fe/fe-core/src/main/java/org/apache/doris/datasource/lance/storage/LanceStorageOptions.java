...
public final class LanceStorageOptions {
    ...
    static Map<String, String> normalizeAliases(Map<String, String> options, Map<String, String> aliases) {
        Map<String, String> result = new HashMap<>();
        if (options != null) {
            options.forEach((key, value) -> {
                String canonical = aliases.getOrDefault(key.toLowerCase(java.util.Locale.ROOT), key);
                String previous = result.put(canonical, value);
                if (previous != null && !previous.equals(value)) {
                    throw new IllegalArgumentException(
                            "Lance namespace vended conflicting values for storage option '" + canonical + "'");
                }
            });
        }
        return result;
    ...
    static void putIfNotEmpty(Map<String, String> target, String key, String value) {
