...
public class StringDict extends IDict {
    ...
    public IDict doRefresh() {
        ...
        Database db = catalog.getInternalDataSource().getDbNullable(getDbId());
        if (db == null) {
            LOG.warn("could not find database with id = {} from catalog", getDbId());
            return null;
        }
        connectContext.setDatabase(
                ClusterNamespace.getFullName(SystemInfoService.DEFAULT_CLUSTER, db.getFullName()));
        Table tbl = db.getTableNullable(getTableId());
        if (tbl == null) {
            LOG.warn("could not find table with id = {} from database {}", getTableId(),
                    db.getFullName());
        }
            ...
            while (valid) {
                ...
                for (int i = 0; i < rowBatch.getNumRows(); ++i) {
                    ...
                    if (updatedDictValues.size() > Config.max_string_dict_size) {
                        LOG.info("too many dict received larger than limit {}, skip build dict",
                                Config.max_string_dict_size);
                        invalidDict(false);
                        valid = false;
                        break;
                    }
