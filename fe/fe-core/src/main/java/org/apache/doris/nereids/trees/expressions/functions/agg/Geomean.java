...
public class Geomean extends NullableAggregateFunction
     ...
     */
    public Geomean(boolean distinct, Expression arg) {
        this(distinct, false, arg);
    }

    public Geomean(boolean distinct, boolean alwaysNullable, Expression arg) {
        super("geomean", distinct, alwaysNullable, arg);
    }
     ...
     */
    @Override
    public Geomean withDistinctAndChildren(boolean distinct, List<Expression> children) {
        Preconditions.checkArgument(children.size() == 1);
        return new Geomean(distinct, alwaysNullable, children.get(0));
    }

    @Override
    public Geomean withAlwaysNullable(boolean alwaysNullable) {
        return new Geomean(distinct, alwaysNullable, children.get(0));
    }

    @Override
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitGeomean(this, context);
    }
