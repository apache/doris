    ...
    public void checkLegalityBeforeTypeCoercion() {
        if ((arity() & 1) == 1) {
            throw new AnalysisException(
                    "object_construct requires even number of parameters (key-value pairs): " + this.toSql());
        }
        for (int i = 0; i < arity(); i++) {
            if ((i & 1) == 0 && getArgumentType(i).isNullType()) {
                throw new AnalysisException("object_construct key can't be NULL: " + this.toSql());
            }
        }
    }
