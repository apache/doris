...
final class ProjectJoinReorderHelper {
    ...
    private static Optional<LogicalProject<LogicalJoin<Plan, Plan>>> pushDownInnerOuterProject(
        ...
        JoinType joinType = join.getJoinType();
        boolean rightSideNullable = joinType.isLeftOuterJoin()
                || joinType.isAsofLeftOuterJoin() || joinType.isFullOuterJoin();
        boolean leftSideNullable = joinType.isRightOuterJoin()
                || joinType.isAsofRightOuterJoin() || joinType.isFullOuterJoin();
        if ((rightSideNullable && rightContainsComplexExpression)
                || (leftSideNullable && leftContainsComplexExpression)) {
            return Optional.empty();
        }
