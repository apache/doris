...
public abstract class GreatestLeast extends ScalarFunction
    ...
    public FunctionSignature customSignature() {
        ...
        Optional<DataType> commonType = TypeCoercionUtils.findWiderCommonType(forFindCommon);
        if (!commonType.isPresent()) {
            ...
            for (Expression stringLiteral : partitioned.get(true)) {
                ...
                if (!option.isPresent()) {
                    commonType = TypeCoercionUtils.findWiderTypeForTwo(commonType.get(), stringLiteral.getDataType());
                    if (!commonType.isPresent()) {
                        SearchSignature.throwCanNotFoundFunctionException(this.getName(), getArguments());
                    } else {
                        break;
