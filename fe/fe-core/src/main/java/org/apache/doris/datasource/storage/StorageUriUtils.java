 ...
 *
 * <p>Ports the URI subset of the legacy {@code S3PropertyUtils} and the remaining
 * location classifier from {@code AzurePropertyUtils} so the facade does not depend on the to-be-deleted
 * legacy typed storage package (deleted in Phase D). Behavior is kept identical, with one deliberate
...
public final class StorageUriUtils {
     ...
     *
     * <p>Also carries the legacy {@code OSSProperties.validateAndNormalizeUri} override when
     * {@code applyOssBucketRewrite} is true (the caller's binding is the OSS provider): a
     * virtual-hosted {@code scheme://bucket.endpoint/key} authority is rewritten to
     * {@code scheme://bucket/key} before the base normalization, byte-identical to the legacy
     * {@code rewriteOssBucketIfNecessary}. Non-OSS bindings never rewrite — a dotted bucket
     * name under an explicit S3 binding (e.g. {@code oss://logs.prod/…} with
     * {@code fs.s3.support=true}) passes through unchanged, exactly like legacy.</p>
     */
     ...
     *
     * <p>Applied only when the caller's binding is the OSS provider (legacy semantics: the
     * rewrite lived in the OSS typed class). Body is the legacy implementation verbatim.</p>
     */
    private static String rewriteOssBucketIfNecessary(String uri) {
        ...
        // Handle bucket.endpoint format (legacy body verbatim: any dotted authority is
        // chopped at the first dot — the caller already guaranteed the binding is OSS;
        // OSS-HDFS bindings are HDFS-family and never route through this method)
