...
/**
 * Constructs a non-executable single-node plan from an analyzed parse tree. The single-node plan
 * does not contain data exchanges or data-reduction optimizations such as local aggregations that
 * are important for distributed execution. The single-node plan needs to be wrapped in a plan
 * fragment for it to be executable.
 */
public class SingleNodePlanner {
    ...
    /**
     * Generates and returns the root of the single-node plan for the analyzed parse tree in the
     * planner context. The planning process recursively walks the parse tree and performs the
     * following actions. In the top-down phase over query statements: - Materialize the slots
     * required for evaluating expressions of that statement. - Migrate conjuncts from parent blocks
     * into inline views and union operands. In the bottom-up phase generate the plan tree for every
     * query statement: - Generate the plan for the FROM-clause of a select statement: The plan
     * trees of absolute and uncorrelated table refs are connected via JoinNodes. The relative and
     * correlated table refs are associated with one or more SubplanNodes. - A new SubplanNode is
     * placed on top of an existing plan node whenever the tuples materialized by that plan node
     * enable evaluation of one or more relative or correlated table refs, i.e., SubplanNodes are
     * placed at the lowest possible point in the plan, often right after a ScanNode materializing
     * the (single) parent tuple. - The right-hand side of each SubplanNode is a plan tree generated
     * by joining a SingularRowSrcTableRef with those applicable relative and correlated refs. A
     * SingularRowSrcTableRef represents the current row being processed by the SubplanNode from its
     * input (first child). - Connecting table ref plans via JoinNodes is done in a cost-based
     * fashion (join-order optimization). All materialized slots, including those of tuples
     * materialized inside a SubplanNode, must be known for an accurate estimate of row sizes needed
     * for cost-based join ordering. - The remaining aggregate/analytic/orderby portions of a select
     * statement are added on top of the FROM-clause plan. - Whenever a new node is added to the
     * plan tree, assign conjuncts that can be evaluated at that node and compute the stats of that
     * node (cardinality, etc.). - Apply combined expression substitution map of child plan nodes;
     * if a plan node re-maps its input, set a substitution map to be applied by parents.
     */
    ...
    /**
     * Creates an EmptyNode that 'materializes' the tuples of the given stmt. Marks all
     * collection-typed slots referenced in stmt as non-materialized because they are never
     * unnested, and therefore the corresponding parent scan should not materialize them.
     */
    ...
    private PlanNode createQueryPlan(QueryStmt stmt, Analyzer analyzer, long defaultOrderByLimit)
        ...
        if (stmt.evaluateOrderBy()) {
            for (SlotDescriptor sortSlotDesc : stmt.getSortInfo().getSortTupleDescriptor()
                    .getSlots()) {
        ...
        if (stmt.evaluateOrderBy() && sortHasMaterializedSlots) {
            ...
            root = new SortNode(ctx.getNextNodeId(), root, stmt.getSortInfo(), useTopN, limit == -1,
                    stmt.getOffset());
            if (useTopN) {
     ...
     * equivalences for tupleIds that have not yet been enforced, returns a SelectNode on top of
     * root that evaluates those conjuncts; otherwise returns root unchanged. TODO: change this to
     * assign the unassigned conjuncts to root itself, if that is semantically correct
     */
    ...
    private PlanNode addUnassignedConjuncts(Analyzer analyzer, List<TupleId> tupleIds,
    ...
    private void turnOffPreAgg(AggregateInfo aggInfo, SelectStmt selectStmt, Analyzer analyzer,
            PlanNode root) {
        ...
        do {
            ...
            if (selectStmt.getTableRefs().size() > 1) {
                ...
                for (FunctionCallExpr aggExpr : aggExprs) {
                    ...
                    if (Expr.isBound(Lists.newArrayList(aggExpr),
                            Lists.newArrayList(olapTableRef.getId()))) {
                        // do nothing
                        LOG.debug("All agg exprs is bound to olapTable: {}"
                                + olapTableRef.getTable().getName());
                    } else {
                        ...
                        for (TupleId tupleId : tupleIds) {
                            ...
                            if (analyzer.getTupleDesc(tupleId).getRef() != olapTableRef) {
                                if (analyzer.getTupleDesc(tupleId).getTable() != null
                                        && analyzer.getTupleDesc(tupleId).getTable()
                                                .getType() == Table.TableType.OLAP) {
                                    turnOffReason = "agg expr [" + aggExpr.debugString()
                                            + "] is not bound ["
                                            + selectStmt.getTableRefs().get(0).toSql() + "]";
                                    aggTableValidate = false;
                                } else {
                                    LOG.debug(
                                            "The table which agg expr [{}] is bound to, is not OLAP table [{}]",
                                            aggExpr.debugString(),
                                            analyzer.getTupleDesc(tupleId).getTable() == null
                                                    ? "inline view"
                                                    : analyzer.getTupleDesc(tupleId).getTable()
                                                            .getName());
                                }
            ...
            for (FunctionCallExpr aggExpr : aggExprs) {
                    ...
                    if ((child instanceof CastExpr) && (child.getChild(0) instanceof SlotRef)) {
                        ...
                        for (Expr conditionExpr : conditionExprs) {
                            ...
                            for (SlotId conditionSlotId : conditionSlotIds) {
                                conditionColumns.add(analyzer.getDescTbl()
                                        .getSlotDesc(conditionSlotId).getColumn());
                            }
                        }
                ...
                for (Column col : conditionColumns) {
                    ...
                    if (!col.isKey()) {
                        turnOffReason = "the condition column [" + col.getName()
                                + "] is not key type in aggr expr [" + aggExpr.toSql() + "].";
                ...
                for (Column col : returnColumns) {
                    ...
                    if (col.isKey()) {
                        if ((!aggExpr.getFnName().getFunction().equalsIgnoreCase("MAX"))
                            ...
                            turnOffReason =
                                    "the type of agg on StorageEngine's Key column should only be MAX or MIN."
                    ...
                    if (aggExpr.getFnName().getFunction().equalsIgnoreCase("SUM")) {
                        if (col.getAggregationType() != AggregateType.SUM) {
                            turnOffReason = "Aggregate Operator not match: SUM <--> "
                        ...
                        if ((!col.isKey()) && col.getAggregationType() != AggregateType.MAX) {
                            turnOffReason = "Aggregate Operator not match: MAX <--> "
                                    + col.getAggregationType();
                            returnColumnValidate = false;
                            break;
                        }
                        ...
                        if ((!col.isKey()) && col.getAggregationType() != AggregateType.MIN) {
                            turnOffReason = "Aggregate Operator not match: MIN <--> "
                                    + col.getAggregationType();
                            returnColumnValidate = false;
                            break;
                        }
                    } else if (aggExpr.getFnName().getFunction()
                            .equalsIgnoreCase("HLL_UNION_AGG")) {
                        // do nothing
                        ...
                        if ((!col.isKey())) {
                            ...
                            .equalsIgnoreCase(FunctionSet.BITMAP_UNION_INT)) {
                        if ((!col.isKey())) {
                            turnOffReason = "BITMAP_UNION_INT function with non-key column: "
                                    + col.getName();
                            returnColumnValidate = false;
                            break;
                        }
                    } else if (aggExpr.getFnName().getFunction()
                            .equalsIgnoreCase(FunctionSet.BITMAP_UNION)
                            || aggExpr.getFnName().getFunction()
                                    .equalsIgnoreCase(FunctionSet.BITMAP_UNION_COUNT)) {
                        if (col.getAggregationType() != AggregateType.BITMAP_UNION) {
                            turnOffReason = "Aggregate Operator not match: BITMAP_UNION <--> "
                                    + col.getAggregationType();
                            returnColumnValidate = false;
                            break;
                        }
                    } else if (aggExpr.getFnName().getFunction()
                            .equalsIgnoreCase(FunctionSet.QUANTILE_UNION)) {
                        if (col.getAggregationType() != AggregateType.QUANTILE_UNION) {
                            turnOffReason = "Aggregate Operator not match: QUANTILE_UNION <---> "
                                    + col.getAggregationType();
                            returnColumnValidate = false;
                            break;
                        }
                    } else if (aggExpr.getFnName().getFunction()
                            .equalsIgnoreCase("multi_distinct_count")) {
                        // count(distinct k1), count(distinct k2) / count(distinct k1,k2) can turn
                        // on pre aggregation
                        if ((!col.isKey())) {
                            turnOffReason = "Multi count or sum distinct with non-key column: "
                                    + col.getName();
                            returnColumnValidate = false;
                            break;
                        }
                        ...
                        turnOffReason =
                                "Invalid Aggregate Operator: " + aggExpr.getFnName().getFunction();
                        returnColumnValidate = false;
                        break;
                    }
                }
    ...
    /**
     * Return the cheapest plan that materializes the joins of all TableRefs in refPlans and the
     * subplans of all applicable TableRefs in subplanRefs. Assumes that refPlans are in the order
     * as they originally appeared in the query. For this plan: - the plan is executable, ie, all
     * non-cross joins have equi-join predicates - the leftmost scan is over the largest of the
     * inputs for which we can still construct an executable plan - from bottom to top, all rhs's
     * are in increasing order of selectivity (percentage of surviving rows) - outer/cross/semi
     * joins: rhs serialized size is < lhs serialized size; enforced via join inversion, if
     * necessary - SubplanNodes are placed as low as possible in the plan tree - as soon as the
     * required tuple ids of one or more TableRefs in subplanRefs are materialized Returns null if
     * we can't create an executable plan.
     */
    ...
    boolean candidateCardinalityIsSmaller(PlanNode candidate, long candidateInnerNodeCardinality,
        ...
        if (candidate.getCardinality() < newRoot.getCardinality()) {
            ...
            if (((candidate instanceof HashJoinNode)
                    && ((HashJoinNode) candidate).getJoinOp().isInnerJoin())
                    && ((newRoot instanceof HashJoinNode)
                            && ((HashJoinNode) newRoot).getJoinOp().isInnerJoin())) {
    ...
    /**
     * Returns a plan with leftmostRef's plan as its leftmost input; the joins are in decreasing
     * order of selectiveness (percentage of rows they eliminate). Creates and adds subplan nodes as
     * soon as the tuple ids required by at least one subplan ref are materialized by a join node
     * added during plan generation.
     */
    ...
    private PlanNode createJoinPlan(Analyzer analyzer, TableRef leftmostRef,
        ...
        while (!remainingRefs.isEmpty()) {
            ...
            for (Pair<TableRef, PlanNode> tblRefToPlanNodeOfCandidate : remainingRefs) {
                ...
                if (requiredRefs != null) {
                    ...
                    // The semi and outer join nodes are similar to the stop nodes in each round of
                    // the algorithm.
                    // If the stop node is encountered during the current round of optimal
                    // selection,
                ...
                PlanNode candidate =
                        createJoinNode(analyzer, root, rootPlanNodeOfCandidate, tblRefOfCandidate);
                ...
                if (LOG.isDebugEnabled()) {
                    ...
                    stringBuilder.append("The " + tblRefOfCandidate.getUniqueAlias()
                            + " is right child of join node.");
                    stringBuilder
                            .append("The join cardinality is " + candidate.getCardinality() + ".");
                ...
                // 3. The hash join is better than cross join: [t3 cross t1] pk [t3 hash t2] => t3
                // hash t2
                if (newRoot == null
                        || ((candidate.getClass().equals(newRoot.getClass())
                                && candidateCardinalityIsSmaller(candidate,
            ...
            // This means that the current leftmost node must be wrong, and the correct result
            // cannot be planned.
            //
            // For example:
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("Round " + successfulSelectionTimes + " chose "
                        + minEntry.first.getUniqueAlias() + " #lhs=" + lhsCardinality + " #rhs="
                        + rhsCardinality + " #ops=" + numOps);
            }
        ...
        LOG.debug("The final join sequence is " + joinedRefs.stream().map(TableRef::getUniqueAlias)
                .collect(Collectors.joining(",")));
        return root;
    }

    /**
     * Create tree of PlanNodes that implements the Select/Project/Join/Group by/Having of the
     * selectStmt query block.
     */
    private PlanNode createSelectPlan(SelectStmt selectStmt, Analyzer analyzer,
            long defaultOrderByLimit) throws UserException, AnalysisException {
        ...
        if (analyzer.safeIsEnableJoinReorderBasedCost()) {
            LOG.debug(
                    "Using new join reorder strategy when enable_join_reorder_based_cost is true");
            ...
            for (TableRef ref : selectStmt.getTableRefs()) {
                ...
                if (plan instanceof OlapScanNode) {
                    ...
                    // this olap scan node has been turn off pre-aggregation, should not be turned
                    // on again.
            ...
            if (root instanceof OlapScanNode) {
                OlapScanNode olapNode = (OlapScanNode) root;
                // this olap scan node has been turn off pre-aggregation, should not be turned on
                // again.
                // e .g. select sum(v1) from (select v1 from test_table);
                if (!olapNode.isPreAggregation()) {
                    olapNode.setCanTurnOnPreAggr(false);
     ...
     */
    private PlanNode createRepeatNodePlan(SelectStmt selectStmt, Analyzer analyzer, PlanNode root)
            throws UserException {
        GroupByClause groupByClause = selectStmt.getGroupByClause();
    ...
    public boolean selectMaterializedView(QueryStmt queryStmt, Analyzer analyzer)
        ...
        if (queryStmt instanceof SelectStmt) {
            ...
            MaterializedViewSelector materializedViewSelector =
                    new MaterializedViewSelector(selectStmt, analyzer);
    ...
    /**
     * Returns a new AggregationNode that materializes the aggregation of the given stmt. Assigns
     * conjuncts from the Having clause to the returned node.
     */
    ...
    /**
     * Returns a MergeNode that materializes the exprs of the constant selectStmt. Replaces the
     * resultExprs of the selectStmt with SlotRefs into the materialized tuple.
     */
    ...
    /**
     * Create tuple descriptor that can hold the results of the given SelectStmt, with one slot per
     * result expr.
     */
    ...
    // no need to remove?
    public static PartitionColumnFilter createPartitionFilter(SlotDescriptor desc,
            List<Expr> conjuncts) {
        ...
        for (Expr expr : conjuncts) {
            ...
            if (expr instanceof BinaryPredicate) {
                ...
                if (!(inPredicate.getChild(0).unwrapExpr(false) instanceof SlotRef)) {
                    ...
                    // then other children of in predicate should not be used as a condition for
                    // partition prune.
                    continue;
                }
                if (null == partitionColumnFilter) {
                ...
                // If we meet a IsNull predicate on partition column, then other predicates are
                // useless
                // eg: (xxxx) and (col is null), only the IsNull predicate has an effect on
                // partition pruning.
                partitionColumnFilter = new PartitionColumnFilter();
    ...
    /**
     * Returns plan tree for an inline view ref: - predicates from the enclosing scope that can be
     * evaluated directly within the inline-view plan are pushed down - predicates that cannot be
     * evaluated directly within the inline-view plan but only apply to the inline view are
     * evaluated in a SelectNode placed on top of the inline view plan - all slots that are
     * referenced by predicates from the enclosing scope that cannot be pushed down are marked as
     * materialized (so that when computeMemLayout() is called on the base table descriptors
     * materialized by the inline view it has a complete picture)
     */
    private PlanNode createInlineViewPlan(Analyzer analyzer, InlineViewRef inlineViewRef)
            ...
            if (selectStmt.getTableRefs().isEmpty()) {
                if (inlineViewRef.getAnalyzer().hasEmptyResultSet()) {
                    PlanNode emptySetNode =
                            createEmptyNode(null, viewStmt, inlineViewRef.getAnalyzer());
        ...
        if (analyzer.isOuterJoined(inlineViewRef.getId())) {
            ...
            List<Expr> nullableRhs = TupleIsNullPredicate.wrapExprs(outputSmap.getRhs(),
                    rootNode.getTupleIds(), analyzer);
            outputSmap = new ExprSubstitutionMap(outputSmap.getLhs(), nullableRhs);
            // When we process outer join with inline views, we set slot descriptor of inline view
            // to nullable firstly.
            // When we generate plan, we remove inline view, so the upper node's input is inline
            // view's child.
            // So we need to set slot descriptor of inline view's child to nullable to ensure
            // consistent behavior
            // with BaseTable.
            for (TupleId tupleId : rootNode.getTupleIds()) {
                for (SlotDescriptor slotDescriptor : analyzer.getTupleDesc(tupleId)
        ...
        if (!canMigrateConjuncts(inlineViewRef)) {
            rootNode = addUnassignedConjuncts(analyzer, inlineViewRef.getDesc().getId().asList(),
                    rootNode);
        }
        return rootNode;
    }
    ...
    /**
     * Migrates unassigned conjuncts into an inline view. Conjuncts are not migrated into the inline
     * view if the view has a LIMIT/OFFSET clause or if the view's stmt computes analytic functions
     * (see IMPALA-1243/IMPALA-1900). The reason is that analytic functions compute aggregates over
     * their entire input, and applying filters from the enclosing scope *before* the aggregate
     * computation would alter the results. This is unlike regular aggregate computation, which only
     * makes the *output* of the computation visible to the enclosing scope, so that filters from
     * the enclosing scope can be safely applied (to the grouping cols, say).
     */
     ...
     * @throws AnalysisException
     */
    private void migrateNonconstantConjuncts(InlineViewRef inlineViewRef,
            List<Expr> unassignedConjuncts, Analyzer analyzer) throws AnalysisException {
        ...
        final List<Expr> viewPredicates = getPushDownPredicatesForInlineView(inlineViewRef, preds,
                analyzer, pushDownFailedPredicates);
     ...
     * should be assigned to query block from top to bottom, it will try to push down constant
     * conjuncts.
     *
     * @param inlineViewRef
     * @param conjuncts
     * @throws AnalysisException
     */
    private void migrateConstantConjuncts(InlineViewRef inlineViewRef, List<Expr> conjuncts)
            throws AnalysisException {
        if (conjuncts.isEmpty()) {
            return;
        }
        ...
        if (stmt instanceof SelectStmt) {
            ...
            if (select.getAggInfo() != null) {
                ...
                // Conjuncts will be assigned to the lowest outer join node or non-outer join's leaf
                // children.
     ...
     */
    private List<Expr> getPushDownPredicatesForInlineView(InlineViewRef inlineViewRef,
            List<Expr> viewPredicates, Analyzer analyzer, List<Expr> pushDownFailedPredicates) {
        ...
        if (inlineViewRef.getViewStmt().evaluateOrderBy() || inlineViewRef.getViewStmt().hasLimit()
        ...
        if (inlineViewRef.getViewStmt() instanceof SetOperationStmt) {
            final SetOperationStmt setOperationStmt =
                    (SetOperationStmt) inlineViewRef.getViewStmt();
        ...
        if (selectStmt.hasAnalyticInfo()) {
            pushDownPredicates.addAll(getWindowsPushDownPredicates(candicatePredicates,
                    viewPredicates, selectStmt.getAnalyticInfo(), pushDownFailedPredicates));
        } else {
     ...
     * @return
     */
    private List<Expr> getWindowsPushDownPredicates(List<Expr> predicates,
            List<Expr> viewPredicates, AnalyticInfo analyticInfo,
            List<Expr> pushDownFailedPredicates) {
        final List<Expr> pushDownPredicates = Lists.newArrayList();
    ...
    private PlanNode createScanNode(Analyzer analyzer, TableRef tblRef, SelectStmt selectStmt)
        ...
        switch (tblRef.getTable().getType()) {
            case OLAP:
                if (selectStmt.isMetaQuery()) {
                    OlapMetaScanNode olapMetaScanNode =
                            new OlapMetaScanNode(ctx.getNextNodeId(), tblRef.getDesc());
                    scanNode = olapMetaScanNode;
                    break;
                }
                OlapScanNode olapNode =
                        new OlapScanNode(ctx.getNextNodeId(), tblRef.getDesc(), "OlapScanNode");
                olapNode.setForceOpenPreAgg(tblRef.isForcePreAggOpened());
                scanNode = olapNode;
                break;
            case ODBC:
                scanNode = new OdbcScanNode(ctx.getNextNodeId(), tblRef.getDesc(),
                        (OdbcTable) tblRef.getTable());
                break;
            case MYSQL:
                scanNode = new MysqlScanNode(ctx.getNextNodeId(), tblRef.getDesc(),
                        (MysqlTable) tblRef.getTable());
                break;
            case SCHEMA:
                scanNode = new SchemaScanNode(ctx.getNextNodeId(), tblRef.getDesc());
                break;
            case BROKER:
                scanNode = new BrokerScanNode(ctx.getNextNodeId(), tblRef.getDesc(),
                        "BrokerScanNode", null, -1);
                break;
            case ELASTICSEARCH:
            ...
            case HIVE:
                ...
                break;
            case ICEBERG:
                scanNode = new IcebergScanNode(ctx.getNextNodeId(), tblRef.getDesc(),
                        "IcebergScanNode", null, -1);
                break;
            case HUDI:
            ...
            case TABLE_VALUED_FUNCTION:
                scanNode = new TableValuedFunctionScanNode(ctx.getNextNodeId(), tblRef.getDesc(),
                        "TableValuedFunctionScanNode",
                        ((TableValuedFunctionRef) tblRef).getTableFunction());
                break;
        ...
        List<ScanNode> scanNodeList = selectStmtToScanNodes
                .computeIfAbsent(selectStmt.getAnalyzer(), k -> Lists.newArrayList());
    ...
    /**
     * Return join conjuncts that can be used for hash table lookups. - for inner joins, those are
     * equi-join predicates in which one side is fully bound by lhsIds and the other by rhs' id; -
     * for outer joins: same type of conjuncts as inner joins, but only from the JOIN clause Returns
     * the original form in 'joinPredicates'.
     */
    private void getHashLookupJoinConjuncts(Analyzer analyzer, PlanNode left, PlanNode right,
            List<Expr> joinConjuncts, Reference<String> errMsg, JoinOperator op) {
        joinConjuncts.clear();
        ...
        for (Expr e : candidates) {
            ...
            /**
             * The left and right child of origin predicates need to be swap sometimes. Case A:
             * select * from t1 join t2 on t2.id=t1.id The left plan node is t1 and the right plan
             * node is t2. The left child of origin predicate is t2.id and the right child of origin
             * predicate is t1.id. In this situation, the children of predicate need to be swap =>
             * t1.id=t2.id.
             */
            Expr rhsExpr = null;
    ...
    private PlanNode createJoinNodeBase(Analyzer analyzer, PlanNode outer, PlanNode inner,
            TableRef innerRef) throws UserException {
        ...
        // are materialized)
        getHashLookupJoinConjuncts(analyzer, outer, inner, eqJoinConjuncts, errMsg,
                innerRef.getJoinOp());
        if (eqJoinConjuncts.isEmpty()) {
            ...
            if (innerRef.getJoinOp().isOuterJoin() || innerRef.getJoinOp().isSemiAntiJoin()) {
                throw new AnalysisException(
                        "non-equal " + innerRef.getJoinOp().toString() + " is not supported");
            }
            ...
            CrossJoinNode result = new CrossJoinNode(ctx.getNextNodeId(), outer, inner, innerRef);
        ...
        HashJoinNode result = new HashJoinNode(ctx.getNextNodeId(), outer, inner, innerRef,
     ...
     * for joinreorder
     */
    public PlanNode createJoinNode(Analyzer analyzer, PlanNode outer, PlanNode inner,
            TableRef innerRef) throws UserException {
        return createJoinNodeBase(analyzer, outer, inner, innerRef);
    }

    /**
     * Creates a new node to join outer with inner. Collects and assigns join conjunct as well as
     * regular conjuncts. Calls init() on the new join node. Throws if the JoinNode.init() fails.
     */
     ...
     * <p>
     * 'fastPartitionKeyScans' indicates whether to try to produce the slots with metadata instead
     * of table scans. Only applicable to BaseTableRef which is also an HDFS table.
     * <p>
     * Throws if a PlanNode.init() failed or if planning of the given table ref is not implemented.
     */
    ...
    private PlanNode createTableFunctionNode(Analyzer analyzer, PlanNode inputNode,
            List<LateralViewRef> lateralViewRefs, SelectStmt selectStmt) throws UserException {
        ...
        TableFunctionNode tableFunctionNode =
                new TableFunctionNode(ctx.getNextNodeId(), inputNode, lateralViewRefs);
    ...
    /**
     * Create a plan tree corresponding to 'setOperands' for the given SetOperationStmt. The
     * individual operands' plan trees are attached to a single SetOperationNode. If result is not
     * null, it is expected to contain the plan for the distinct portion of the given
     * SetOperationStmt. The result is then added as a child of the returned SetOperationNode.
     */
    private SetOperationNode createSetOperationPlan(Analyzer analyzer,
            SetOperationStmt setOperationStmt, List<SetOperationStmt.SetOperand> setOperands,
            PlanNode result, long defaultOrderByLimit) throws UserException, AnalysisException {
        ...
        for (SetOperationStmt.SetOperand setOperand : setOperands) {
            if (setOperand.getOperation() != null) {
                ...
                Preconditions.checkState(operation == setOperand.getOperation(),
                        "can not support mixed set " + "operations at here");
            }
        }
        switch (operation) {
            case UNION:
        ...
        // one is the root node, and the other is a distinct node in front, so the
        // setOperationDistinctPlan will
    ...
    /**
     * Returns plan tree for unionStmt: - distinctOperands' plan trees are collected in a single
     * UnionNode and duplicates removed via distinct aggregation - the output of that plus the
     * allOperands' plan trees are collected in another UnionNode which materializes the result of
     * unionStmt - if any of the union operands contains analytic exprs, we avoid pushing predicates
     * directly into the operands and instead evaluate them *after* the final UnionNode (see
     * createInlineViewPlan() for the reasoning) TODO: optimize this by still pushing predicates
     * into the union operands that don't contain analytic exprs and evaluating the conjuncts in
     * Select directly above the AnalyticEvalNodes TODO: Simplify the plan of unions with empty
     * operands using an empty set node. TODO: Simplify the plan of unions with only a single
     * non-empty operand to not use a union node (this is tricky because a union materializes a new
     * tuple).
     */
    private PlanNode createSetOperationPlan(SetOperationStmt setOperationStmt, Analyzer analyzer,
            ...
            if (op.getOperation() == null) {
                ...
                if (partialOperands.size() > 0) {
                    if (operation == SetOperationStmt.Operation.INTERSECT
                        ...
                        result = createSetOperationPlan(analyzer, setOperationStmt, partialOperands,
                                result, defaultOrderByLimit);
                    } else {
                        result = createUnionPartialSetOperationPlan(analyzer, setOperationStmt,
                                partialOperands, result, defaultOrderByLimit);
                    }
        ...
        if (setOperationStmt.hasAnalyticExprs() || hasConstantOp) {
            result = addUnassignedConjuncts(analyzer, setOperationStmt.getTupleId().asList(),
                    result);
        }
        return result;
    }
    ...
    private PlanNode createUnionPartialSetOperationPlan(Analyzer analyzer,
            SetOperationStmt setOperationStmt, List<SetOperationStmt.SetOperand> setOperands,
            PlanNode result, long defaultOrderByLimit) throws UserException {
        ...
        if (hasDistinctOps) {
            result = createSetOperationPlan(analyzer, setOperationStmt, distinctOps, result,
                    defaultOrderByLimit);
    ...
    private PlanNode createAssertRowCountNode(PlanNode input,
            AssertNumRowsElement assertNumRowsElement, Analyzer analyzer) throws UserException {
        AssertNumRowsNode root =
                new AssertNumRowsNode(ctx.getNextNodeId(), input, assertNumRowsElement);
        root.init(analyzer);
        return root;
    }

    /**
     * According to the way to materialize slots from top to bottom, Materialization will prune
     * columns which are not referenced by Statement outside. However, in some cases, in order to
     * ensure The correct execution, it is necessary to materialize the slots that are not needed by
     * Statement outside.
     *
     * @param tblRef
     * @param analyzer
    ...
    /**
     * When materialized table ref is a empty tbl ref, the planner should add a mini column for this
     * tuple. There are situations: 1. The tbl ref is empty, such as select a from (select 'c1' a
     * from test) t; Inner tuple: tuple 0 without slot 2. The materialized slot in tbl ref is empty,
     * such as select a from (select 'c1' a, k1 from test) t; Inner tuple: tuple 0 with a not
     * materialized slot k1 In the above two cases, it is necessary to add a mini column to the
     * inner tuple to ensure that the number of rows in the inner query result is the number of rows
     * in the table. 2. count star: select count(*) from t;
     * <p>
     * After this function, the inner tuple is following: case1. tuple 0: slot<k1> materialized true
     * (new slot) case2. tuple 0: slot<k1> materialized true (changed)
     *
     * @param tblRef
     * @param analyzer
     */
    ...
    /**
     * Materialize InlineViewRef result'exprs for Cross Join or Count Star For example: select
     * count(*) from (select k1+1 ,k2 ,k3 from base) tmp Columns: k1 tinyint, k2 bigint, k3 double
     * Input: tmp, analyzer Output: Materialized slot: k1 true, k2 false, k3 false Materialized
     * tuple: base
     *
     * @param inlineView
     * @param analyzer
     */
    private void materializeInlineViewResultExprForCrossJoinOrCountStar(InlineViewRef inlineView,
            Analyzer analyzer) {
    ...
    /**
     * Entrance for push-down rules, it will execute possible push-down rules from top to down and
     * the planner will be responsible for assigning all predicates to PlanNode.
     */
    ...
    private void pushDownPredicatesPastSort(Analyzer analyzer, SelectStmt stmt)
            throws AnalysisException {
        // TODO chenhao, remove isEvaluateOrderBy when SubQuery's default limit is removed.
        if (stmt.evaluateOrderBy() || stmt.getLimit() >= 0 || stmt.getOffset() > 0
                || stmt.getSortInfo() == null) {
            return;
        }
        final List<Expr> predicates =
                getBoundPredicates(analyzer, stmt.getSortInfo().getSortTupleDescriptor());
    ...
    private void pushDownPredicatesPastWindows(Analyzer analyzer, SelectStmt stmt)
            throws AnalysisException {
        ...
        final List<Expr> predicates =
                getBoundPredicates(analyzer, analyticInfo.getOutputTupleDesc());
        if (predicates.size() <= 0) {
            return;
        }
        ...
        final List<Expr> pushDownPredicates =
                getPredicatesBoundedByGroupbysSourceExpr(predicates, analyzer, stmt);
        if (pushDownPredicates.size() <= 0) {
            return;
        }
        if (putPredicatesOnAggregation(stmt, analyzer, pushDownPredicates)) {
     ...
     * @param targetTupleIds target tuple to register. Table tuple ids when process first phase agg.
    ...
    private void pushDownPredicatesPastAggregationOnePhase(AggregateInfo aggregateInfo,
            Analyzer analyzer, SelectStmt stmt, List<TupleId> targetTupleIds)
            throws AnalysisException {
        if (aggregateInfo == null || aggregateInfo.getGroupingExprs().isEmpty()) {
            return;
        }
        final List<Expr> predicates =
                getBoundPredicates(analyzer, aggregateInfo.getOutputTupleDesc());
     ...
     * @throws AnalysisException throw exception when register predicate to tuple failed
     */
    private void pushDownPredicatesPastAggregation(Analyzer analyzer, SelectStmt stmt)
            throws AnalysisException {
        ...
        pushDownPredicatesPastAggregationOnePhase(secondPhaseAggInfo, analyzer, stmt,
                firstPhaseTupleIds);
        pushDownPredicatesPastAggregationOnePhase(firstPhaseAggInfo, analyzer, stmt,
                stmt.getTableRefIds());
    }

    private List<Expr> getPredicatesBoundedByGroupbysSourceExpr(List<Expr> predicates,
            Analyzer analyzer, SelectStmt stmt) {
                ...
                if (stmt.getGroupByClause().isGroupByExtension()
                    ...
                    if (stmt.getGroupByClause().getGroupingType() == GroupByClause.GroupingType.CUBE
                            || stmt.getGroupByClause()
                                    .getGroupingType() == GroupByClause.GroupingType.ROLLUP) {
                        isAllSlotReferToGroupBys = false;
                    } else {
                        // if grouping type is GROUPING_SETS and the predicate not in all grouping
                        // list,
    ...
    private List<Expr> getPredicatesReplacedSlotWithSourceExpr(List<Expr> predicates,
            Analyzer analyzer) {
        final List<Expr> predicatesCanPushDown = Lists.newArrayList();
    ...
    private void replacePredicateSlotRefWithSource(Expr parent, Expr predicate, int childIndex,
            Analyzer analyzer) {
     ...
     */
    private void putPredicatesOnTargetTupleIds(List<TupleId> tupleIds, Analyzer analyzer,
            List<Expr> predicates) throws AnalysisException {
    ...
    /**
     * Returns a normalized version of a binary equality predicate 'expr' where the lhs child expr
     * is bound by some tuple in 'lhsTids' and the rhs child expr is bound by some tuple in
     * 'rhsTids'. Returns 'expr' if this predicate is already normalized. Returns null in any of the
     * following cases: 1. It is not an equality predicate 2. One of the operands is a constant 3.
     * Both children of this predicate are the same expr The so-called normalization is to ensure
     * that the above conditions are met, and then to ensure that the order of expr is consistent
     * with the order of node
     */
