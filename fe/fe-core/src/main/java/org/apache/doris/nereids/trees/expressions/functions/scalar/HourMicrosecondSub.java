...
/**
 * ScalarFunction 'hour_microsecond_sub'.
 */
public class HourMicrosecondSub extends ScalarFunction
    ...
    private HourMicrosecondSub(ScalarFunctionParams functionParams) {
        super(functionParams);
    }

    @Override
    public HourMicrosecondSub withChildren(List<Expression> children) {
        Preconditions.checkArgument(children.size() == 2);
        return new HourMicrosecondSub(getFunctionParams(children));
    }
    ...
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitHourMicrosecondSub(this, context);
    }

    @Override
    public Expression withConstantArgs(Expression literal) {
        return new HourMicrosecondSub(literal, child(1));
    }
