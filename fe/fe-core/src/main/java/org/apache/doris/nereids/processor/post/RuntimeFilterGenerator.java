...
public class RuntimeFilterGenerator extends PlanPostProcessor {
    ...
    public PhysicalPlan visitPhysicalHashJoin(PhysicalHashJoin<? extends Plan, ? extends Plan> join,
        ...
        if (DENIED_JOIN_TYPES.contains(join.getJoinType()) || join.isMarkJoin()) {
            ...
            for (int i = 0; i < join.getHashJoinConjuncts().size(); i++) {
                ...
                /*
                 * if target slot is the first dup-key or unique-key,
                 * we set a min-max filter to enable scan-range prune.
                 */
                if (ConnectContext.get().getSessionVariable().isEnableMinMaxRuntimeFilterOnFirstKey()
                        && ctx.isFirstKeyOfTable(olapScanSlot)
                        && (TRuntimeFilterType.MIN_MAX.getValue()
                            & ctx.getSessionVariable().getRuntimeFilterType()) == 0) {
                    RuntimeFilter minMaxFilter = new RuntimeFilter(generator.getNextId(),
                            equalTo.right(), olapScanSlot, TRuntimeFilterType.MIN_MAX, i, join);
                    ctx.addJoinToTargetMap(join, olapScanSlot.getExprId());
                    ctx.setTargetExprIdToFilter(olapScanSlot.getExprId(), minMaxFilter);
                    ctx.setTargetsOnScanNode(aliasTransferMap.get(unwrappedSlot).first, olapScanSlot);
                }
            }
        }
    ...
    public PhysicalRelation visitPhysicalScan(PhysicalRelation scan, CascadesContext context) {
        ...
        //collect first duplicate-key(unique-key) for min-max filter
