...
public class RuntimeFilterGenerator extends PlanPostProcessor {
    ...
    private void generateDecoupledRuntimeFilters(
        ...
        for (int i = 0; i < hashJoinConjuncts.size(); i++) {
            ...
            // passes most rows → useless (e.g., customer→customer with ratio=1.0).
            // Without stats: if the builder's build side (dimension) has no filter predicates,
            ...
            for (TRuntimeFilterType type : legalTypes) {
                ...
                if (decoupledRfPushed) {
                    if (preferDecoupled) {
                        ...
                        RuntimeFilter standardRf = ctx.getRuntimeFilterBySrcAndType(
                                equalTo.right(), type, join);
                        if (standardRf != null) {
                            standardRf.setNonBlocking(true);
                        }
                    } else {
