...
public class RuntimeFilterGenerator extends PlanPostProcessor {
    ...
    public PhysicalPlan visitPhysicalHashJoin(PhysicalHashJoin<? extends Plan, ? extends Plan> join,
        ...
        return generateRuntimeFiltersForHashLikeJoin(join, context, this::getBuildSideNdv);
    }

    @Override
    public PhysicalPlan visitPhysicalGroupJoin(PhysicalGroupJoin<? extends Plan, ? extends Plan> groupJoin,
            CascadesContext context) {
        return generateRuntimeFiltersForHashLikeJoin(groupJoin, context,
                (join, equalTo) -> getGroupJoinBuildSideNdv(
                        (PhysicalGroupJoin<? extends Plan, ? extends Plan>) join, equalTo));
    }

    private PhysicalPlan generateRuntimeFiltersForHashLikeJoin(
            AbstractPhysicalJoin<? extends Plan, ? extends Plan> join,
            CascadesContext context,
            java.util.function.BiFunction<AbstractPhysicalJoin<? extends Plan, ? extends Plan>,
                    EqualPredicate, Long> ndvEstimator) {
        ...
        for (int i = 0; i < hashJoinConjuncts.size(); i++) {
            ...
            for (TRuntimeFilterType type : legalTypes) {
                ...
                long buildSideNdv = ndvEstimator.apply(join, equalTo);
     ...
     * const propagation
     */
    private boolean isUniqueValueEqualTo(AbstractPhysicalJoin<? extends Plan, ? extends Plan> join,
    ...
    private long getGroupJoinBuildSideNdv(PhysicalGroupJoin<? extends Plan, ? extends Plan> groupJoin,
                                          ComparisonPredicate compare) {
        AbstractPlan right = (AbstractPlan) groupJoin.right();
        if (right.getStats() == null) {
            return -1L;
        }
        double estimatedNdv = right.getStats().getRowCount();
        for (Expression groupBy : groupJoin.getGroupByExpressions()) {
            if (groupBy instanceof SlotReference) {
                ...
                ColumnStatistic colStats = right.getStats().columnStatistics().get(slot);
                if (colStats != null && colStats.ndv > 0) {
                    estimatedNdv = Math.min(estimatedNdv, colStats.ndv);
                }
            }
        }
        return Math.max(1, (long) estimatedNdv);
    }
