...
public class AnalysisManager implements Writable {
    ...
    private final Map<Long, AnalysisJob> idToJob = new ConcurrentHashMap<>();
     ...
     * Mem only.
     */
    public final Map<CriticalColumn, Long/* query time*/> columnUsedInPredicate = new ConcurrentHashMap<>();

    /**
     * Mem only.
     */
    public final Map<CriticalColumn, Long/* query time*/> queriedColumns = new ConcurrentHashMap<>();
    ...
    public void createTaskForEachColumns(AnalysisInfo jobInfo, Map<Long, BaseAnalysisTask> analysisTasks,
        ...
        for (Entry<String, Set<String>> entry : columnToPartitions.entrySet()) {
            ...
            AnalysisInfo analysisInfo = colTaskInfoBuilder
                    .setColName(colName)
                    .setIndexId(indexId)
                    .setTaskId(taskId)
                    .setLastExecTimeInMs(System.currentTimeMillis())
                    .setAnalyzePriority(jobInfo.analyzePriority)
                    .build();
    ...
    public String getJobProgress(long jobId) {
        ...
        String progressDisplayTemplate = "%d Finished  |  %d Failed  |  %d In Progress  |  %d Total";
    ...
    public void handleKillAnalyzeStmt(KillAnalysisJobStmt killAnalysisJobStmt) throws DdlException {
        AnalysisJob job = idToJob.remove(killAnalysisJobStmt.jobId);
        if (job == null) {
            throw new DdlException("Job not exists or already finished");
        }
        BaseAnalysisTask anyTask = job.tasks().stream().findFirst().orElse(null);
        if (anyTask == null) {
            return;
        }
        checkPriv(anyTask);
        ...
        job.kill();
        for (BaseAnalysisTask taskInfo : job.tasks()) {
            taskInfo.cancel();
    ...
    private BaseAnalysisTask createTask(AnalysisInfo analysisInfo) throws DdlException {
        try {
            ...
            BaseAnalysisTask analysisTask = table.createAnalysisTask(analysisInfo);
            analysisTask.priority = analysisInfo.analyzePriority;
            return analysisTask;
        } catch (Throwable t) {
    ...
    public AnalyzePriority getPriority(TableIf tbl, Column col) {
        CriticalColumn criticalColumn = new CriticalColumn(tbl.getId(), col.getName());
        if (isMostCriticalColumn(tbl, col)) {
            return AnalyzePriority.HIGHEST;
        } else if (queriedColumns.containsKey(criticalColumn)) {
            return AnalyzePriority.HIGH;
        } else if (tbl.getBaseSchema().size() <= StatisticConstants.NORMAL_TABLE_WIDTH_UPPER_BOUND) {
            return AnalyzePriority.NORMAL;
        } else {
            return AnalyzePriority.LOW;
        }
    }
    ...
    private static void readAutoJobs(DataInput in, AnalysisManager analysisManager) throws IOException {
        Type type = new TypeToken<LinkedList<AnalysisInfo>>() {
        }.getType();
    ...
    public void clearStatsMeta(Table tbl) {
        ...
        for (Column c : tbl.getBaseSchema()) {
            CriticalColumn criticalColumn = new CriticalColumn(tbl.getId(), c.getName());
            columnUsedInPredicate.remove(criticalColumn);
            queriedColumns.remove(criticalColumn);
        }
    }
    ...
    public void constructJob(AnalysisInfo jobInfo, Collection<? extends BaseAnalysisTask> tasks) {
        idToJob.put(jobInfo.jobId, new AnalysisJob(jobInfo, tasks));
    }
    ...
    public void updateColumnUsedInPredicate(Set<Slot> slotReferences) {
        updateColumn(slotReferences, columnUsedInPredicate);
    }

    public void updateQueriedColumn(Collection<Slot> slotReferences) {
        updateColumn(slotReferences, queriedColumns);
    }

    protected void updateColumn(Collection<Slot> slotReferences, Map<CriticalColumn, Long/* query time*/> map) {
        clearCriticalColumn();
        syncToMaster();
        for (Slot s : slotReferences) {
            ...
            if (optionalColumn.isPresent() && optionalTable.isPresent()) {
                map.put(new CriticalColumn(optionalTable.get().getId(),
                        optionalColumn.get().getName()), System.currentTimeMillis());
            }
        }
    }

    private void clearCriticalColumn() {
        long now = System.currentTimeMillis();
        if (now - lastClearTime < StatisticConstants.CRITICAL_COLUMN_VALID_TIME_IN_MILLIS) {
            return;
        }
        lastClearTime = now;
        Predicate<Entry<CriticalColumn, Long>> p = e
                -> now - e.getValue() > StatisticConstants.CRITICAL_COLUMN_VALID_TIME_IN_MILLIS;
        queriedColumns.entrySet()
                .removeIf(p);
        columnUsedInPredicate.entrySet().removeIf(p);
    }
    ...
    private void syncToMaster() {
        ...
        long now = System.currentTimeMillis();
        if (!(now - lastSyncTime > StatisticConstants.CRITICAL_COLUMN_SYNC_IN_MILLIS)) {
            return;
        }
        lastSyncTime = now;
        List<TSyncCriticalColumn> tQueried
                = queriedColumns.keySet().stream().map(CriticalColumn::toThrift).collect(Collectors.toList());
        List<TSyncCriticalColumn> tPredicate
                = columnUsedInPredicate.keySet().stream().map(CriticalColumn::toThrift).collect(Collectors.toList());
        queriedColumns.clear();
        columnUsedInPredicate.clear();
        TSyncCriticalColumns tSyncCriticalColumns = new TSyncCriticalColumns();
        tSyncCriticalColumns.usedInPredicate = tPredicate;
        tSyncCriticalColumns.usedInQuery = tQueried;
        for (Frontend frontend : Env.getCurrentEnv().getFrontends(FrontendNodeType.FOLLOWER)) {
            ...
            try {
                ...
                client.syncTCriticalColumn(tSyncCriticalColumns);
            } catch (Throwable t) {
    ...
    public void mergeFollowerCriticalColumn(Collection<TSyncCriticalColumn> usedInPredicate,
            Collection<TSyncCriticalColumn> queried) {
        long now = System.currentTimeMillis();
        this.columnUsedInPredicate.putAll(
                usedInPredicate
                        .stream()
                        .collect(Collectors.toMap(k -> new CriticalColumn(k.tblId, k.colName), v -> now)));
        this.queriedColumns.putAll(
                queried
                        .stream()
                        .collect(Collectors.toMap(k -> new CriticalColumn(k.tblId, k.colName), v -> now)));
    }

    private boolean isMostCriticalColumn(TableIf tbl, Column column) {
        if (column.isKey()) {
            return true;
        }
        if (columnUsedInPredicate.containsKey(new CriticalColumn(tbl.getId(), column.getName()))) {
            return true;
        }
        ...
        if (partitionInfo.getPartitionColumns().contains(column)) {
            return true;
        }
        return olapTable.getDistributionColumnNames().contains(column.getName());
    }
