...
public abstract class LoadJob extends AbstractTxnStateChangeCallback implements LoadTaskCallback, Writable {
    ...
    private void checkAuth(String command) throws DdlException {
        ...
        if (!Env.getCurrentEnv().getAccessCtlMgr().checkPrivByAuthInfo(ConnectContext.get(), authorizationInfo,
                PrivPredicate.LOAD)) {
    ...
    private void checkAuthWithoutAuthInfo(String command) throws DdlException {
        ...
        try {
            ...
            if (tableNames.isEmpty()) {
                ...
                if (!Env.getCurrentEnv().getAccessCtlMgr().checkDbPriv(ConnectContext.get(), db.getFullName(),
                        PrivPredicate.LOAD)) {
                    ErrorReport.reportDdlException(ErrorCode.ERR_SPECIFIC_ACCESS_DENIED_ERROR,
                            Privilege.LOAD_PRIV);
                }
                ...
                for (String tblName : tableNames) {
                    if (!Env.getCurrentEnv().getAccessCtlMgr().checkTblPriv(ConnectContext.get(), db.getFullName(),
