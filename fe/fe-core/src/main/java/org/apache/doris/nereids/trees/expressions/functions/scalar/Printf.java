...
public class Printf extends ScalarFunction
    ...
    public List<FunctionSignature> getSignatures() {
        // printf function can't format decimalV2 and decimalV3, should convert them to double
        Function<DataType, DataType> replaceDecimalV2WithDouble = dataType -> TypeCoercionUtils
                .replaceSpecifiedType(dataType, DecimalV2Type.class, DoubleType.INSTANCE);
        Function<DataType, DataType> replaceDecimalV3WithDouble = dataType -> TypeCoercionUtils
                .replaceSpecifiedType(dataType, DecimalV3Type.class, DoubleType.INSTANCE);
        // support dynamic type arguments
        List<DataType> argTypes = children.stream().map(ExpressionTrait::getDataType)
                .map(replaceDecimalV2WithDouble)
                .map(replaceDecimalV3WithDouble)
