...
public class BindSlotReference implements AnalysisRuleFactory {
    ...
    public List<Rule<Plan>> buildRules() {
        ...
        Rule<Plan> bindJoinSlot = logicalJoin().then(join -> {
            ...
            List<Slot> newOutput = op.computeOutput(join.left(), join.right());
            return new LogicalBinaryPlan(op, null, new LogicalProperties(newOutput), join.left(), join.right());
    ...
    private class SlotBinder extends DefaultExpressionRewriter<Void> {
        ...
        public Expression visitUnboundSlot(UnboundSlot unboundSlot, Void context) {
            ...
            switch (bounded.size()) {
                ...
                default:
                    throw new AnalysisException(unboundSlot + " is ambiguous： " + bounded.stream().map(Slot::toString).collect(Collectors.joining(", ")));
            }
        }
