...
public class RewriteBinaryPredicatesRule implements ExprRewriteRule {
    ...
    /**
     * Convert the binary predicate of the form
     * <CastExpr<SlotRef(type=FLOAT)>> <op><FloatLiteral> to the binary
     * predicate of <SlotRef(type=FLOAT)> <new op> <new FloatLiteral>, thereby allowing the binary predicate
     * The predicate pushes down and completes the bucket clipped.
     *
     * Examples & background
     * For query "select * from T where t1 = 2.0", when the type of column t1 is equal to FLOAT, in the binary
     * predicate analyze, the type will be unified to DOUBLE, so the binary predicate will be converted to
     * <CastExpr<SlotRef>> <op In the form of ><FloatLiteral>,
     * because Cast wraps the t1 column, it cannot be pushed down, resulting in cannot apply by inverted index.
     * We convert it to the equivalent type with column t1 to push down and improve performance.
     *
     */
    private Expr rewriteFloatSlotRefCompareFloatLiteral(Expr expr0, Type expr0ColumnType, FloatLiteral expr1,
            BinaryPredicate.Operator op) {
        try {
            // Convert childExpr to column type and compare the converted values. There are 3 possible situations:
            // case 1. The value of childExpr exceeds the range of the column type,
            // then castTo() will throw an exception.
            // case 2. childExpr is converted to column type, but the value of childExpr loses precision.
            // case 3. childExpr is precisely converted to column type.
            LiteralExpr newExpr = (LiteralExpr) expr1.castTo(expr0ColumnType);
            int compResult = expr1.compareLiteral(newExpr);
            // case 2
            if (compResult != 0) {
    ...
    public Expr apply(Expr expr, Analyzer analyzer, ExprRewriter.ClauseType clauseType) throws AnalysisException {
        ...
        if (expr0 instanceof CastExpr && expr0.getType().isFloatingPointType()
                && expr0.getChild(0) instanceof SlotRef
                && expr0.getChild(0).getType().getResultType() == Type.DOUBLE
                && expr1 instanceof FloatLiteral) {
            return rewriteFloatSlotRefCompareFloatLiteral(expr0,
                    expr0.getChild(0).getType(), (FloatLiteral) expr1, op);
        }
        return expr;
    }
}
