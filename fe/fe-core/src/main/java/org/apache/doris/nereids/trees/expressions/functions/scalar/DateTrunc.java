...
public class DateTrunc extends ScalarFunction
    ...
    public Optional<Expression> nextBucketBoundary(Literal value) {
        ...
        if (!(unit instanceof StringLikeLiteral)) {
            return Optional.empty();
        }
        IntegerLiteral one = new IntegerLiteral(1);
        switch (((StringLikeLiteral) unit).getStringValue().toLowerCase()) {
            case "year":
                return Optional.of(new YearsAdd(value, one));
            case "quarter":
                return Optional.of(new QuartersAdd(value, one));
            case "month":
                return Optional.of(new MonthsAdd(value, one));
            case "week":
                return Optional.of(new WeeksAdd(value, one));
            case "day":
