...
public abstract class BaseAnalysisTask {
    ...
    protected static final String LINEAR_ANALYZE_TEMPLATE = "WITH cte1 AS ("
            ...
            +         "t.`column_key`, \":\", ROUND(t.`count` * ${scaleFactor} * 100 / ${rowCount}, 2)), \";\") "
            +         "as `hot_value` "
            +     "FROM ("
            +         "SELECT ${subStringColName} as `column_key`, COUNT(1) AS `count` "
            +         "FROM cte1 WHERE `${colName}` IS NOT NULL "
            +         "GROUP BY `column_key` ORDER BY `count` DESC LIMIT 3) t) "
    ...
    protected static final String DUJ1_ANALYZE_TEMPLATE = "WITH cte1 AS ("
            ...
            + "cte3 AS ("
            +     "SELECT GROUP_CONCAT(CONCAT("
            +         "t2.`column_key`, \":\", ROUND(t2.`count` * ${scaleFactor} * 100 / ${rowCount}, 2)), \";\") "
            +         "as `hot_value` "
            +     "FROM ("
            +         "SELECT `column_key`, `count` "
            +             "FROM cte1 "
            +             "WHERE `column_key` IS NOT NULL ORDER BY `count` DESC LIMIT 3) t2) "
            + "SELECT * FROM cte2 CROSS JOIN cte3";
