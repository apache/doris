...
public class MTMVPartitionExprDateTrunc implements MTMVPartitionExprService {
    ...
    private Optional<String> getDateFormat(Map<String, String> mvProperties) {
        Optional<String> dateFormat = StringUtils
                .isEmpty(mvProperties.get(PropertyAnalyzer.PROPERTIES_PARTITION_DATE_FORMAT))
                        ? Optional.empty()
    ...
    private DateTimeV2Literal dateTrunc(String value,
        ...
        // for (2020-01-31,2020-02-01),if not -1, lower value and upper value will not
        // same after rollup
        if (isUpper) {
