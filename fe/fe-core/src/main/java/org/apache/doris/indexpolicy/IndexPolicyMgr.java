...
public class IndexPolicyMgr implements Writable, GsonPostProcessable {
    ...
    public void createIndexPolicy(CreateIndexPolicyStmt stmt) throws UserException {
        ...
        IndexPolicy indexPolicy = IndexPolicy.create(policyName, stmt.getType(), stmt.getProperties());

        writeLock();
        try {
            validatePolicyProperties(stmt.getType(), stmt.getProperties());
    ...
    private void validateTokenizerProperties(Map<String, String> properties) throws DdlException {
        ...
        if (!IndexPolicy.BUILTIN_TOKENIZERS.contains(type)) {
            throw new DdlException("Invalid tokenizer type: " + type
                    + ". Valid types are: " + IndexPolicy.BUILTIN_TOKENIZERS);
        }
    }
    ...
    private void validateTokenFilterProperties(Map<String, String> properties) throws DdlException {
        ...
        if (!IndexPolicy.BUILTIN_TOKEN_FILTERS.contains(type)) {
            throw new DdlException("Invalid token filter type: " + type
                    + ". Valid types are: " + IndexPolicy.BUILTIN_TOKEN_FILTERS);
        }
    }
    ...
    public ShowResultSet showIndexPolicy(ShowIndexPolicyStmt showStmt) throws AnalysisException {
        ...
        readLock();
        try {
            IndexPolicyTypeEnum filterType = showStmt.getType();
            for (IndexPolicy policy : idToIndexPolicy.values()) {
                if (policy.isInvalid()) {
                    continue;
                }
                if (filterType != null && policy.getType() != filterType) {
                    continue;
                }
