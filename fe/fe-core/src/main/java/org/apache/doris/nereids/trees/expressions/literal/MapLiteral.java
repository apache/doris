            ...
            return new MapLiteral(
                    map.entrySet().stream()
                            .collect(ImmutableMap.toImmutableMap(
                                    entry -> {
                                        try {
                                            return (Literal) entry.getKey().uncheckedCastTo(((MapType) targetType)
                                                .getKeyType());
                                        } catch (Exception ignored) {
                                            return (Literal) entry.getKey()
                                                .deprecatingUncheckedCastTo(((MapType) targetType).getKeyType());
                                        }
                                    },
                                    entry -> {
                                        try {
                                            return (Literal) entry.getValue().uncheckedCastTo(((MapType) targetType)
                                                .getValueType());
                                        } catch (Exception ignored) {
                                            return (Literal) entry.getValue()
                                                .deprecatingUncheckedCastTo(((MapType) targetType).getValueType());
                                        }
                                    }
                            )),
                    targetType
