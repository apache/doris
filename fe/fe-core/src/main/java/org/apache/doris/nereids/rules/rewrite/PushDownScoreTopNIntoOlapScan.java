 ...
 * It will push down score function as a virtual column, and also push down the topN info.
 *
 * Pattern:
 ...
 * 2. The ordering expression in TopN must be a slot reference that refers to a Score function.
 * 3. The Filter node must contain at least one Match function.
 *
 * Example:
 * Before:
 * SELECT score() as score FROM table WHERE text_col MATCH 'query' ORDER BY score DESC LIMIT 10
 *
 * After:
 * The Score function is pushed down into the OlapScan node as a virtual column,
 * and the TopN information (order by, limit) is also pushed down to be used by the storage engine.
 */
public class PushDownScoreTopNIntoOlapScan implements RewriteRuleFactory {
    ...
    public List<Rule> buildRules() {
        return ImmutableList.of(
                logicalTopN(logicalProject(logicalFilter(logicalOlapScan())))
                        // 1. Pre-condition: ORDER BY only has one expression
                        .when(topN -> topN.getOrderKeys().size() == 1)
                        .then(topN -> {
    ...
    private Plan pushDown(
        ...
        // 3. Requirement: ORDER BY expression must refer to a SELECT score() alias.
        ...
        // Find the 'score()' expression in the project list that matches the order by slot.
        ...
        // This is the core action: push score() as a virtual column and also push the topN info.
        ...
        // We need to replace the original score() function with a reference to the new virtual column slot.
