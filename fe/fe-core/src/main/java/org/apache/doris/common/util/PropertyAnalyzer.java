...
public class PropertyAnalyzer {
    ...
    public static void checkCatalogProperties(Map<String, String> properties, boolean isAlter)
            throws AnalysisException {
        // firstly, check if the property's key contains empty character
        for (Map.Entry<String, String> entry : properties.entrySet()) {
            String key = entry.getKey();
            if (key.contains(BLANK) || key.contains(TABS)) {
                throw new AnalysisException("property key contains ' ' or '\t' character");
            }
        }
