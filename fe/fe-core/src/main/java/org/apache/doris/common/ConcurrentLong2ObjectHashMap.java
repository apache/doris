 ...
 * compute, merge, putIfAbsent, replace, remove) are overridden to ensure atomicity within a
 * segment. Boxed {@link Map}-level compound methods delegate to these primitive overrides via
 * fastutil's bridge methods, so they are also atomic.
 *
