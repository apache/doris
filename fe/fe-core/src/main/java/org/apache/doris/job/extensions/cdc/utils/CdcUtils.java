...
public class CdcUtils {
    ...
    public static JdbcClient getJdbcClient(Map<String, String> properties) throws JobException {
        ...
        config.setJdbcUrl("jdbc:" + properties.get(DB_SOURCE_TYPE) + "://" + properties.get(HOST) + ":" + properties.get(PORT) + "/");
        switch (sourceType) {
            case MYSQL:
    ...
    public static List<CreateTableStmt> generateCreateTableStmts(String dorisDb, Map<String, String> properties) throws JobException {
        ...
        for(String table : tablesNameList){
            if(!table.matches(tablesRegex)) {
                LOG.info("Skip table {} in database {} as it does not match the regex {}", table, database, tablesRegex);
                continue;
            }
            ...
            List<String> primaryKeys = columns.stream().filter(Column::isKey).map(Column::getName).collect(Collectors.toList());
            ...
            DistributionDesc distributionDesc = new HashDistributionDesc(FeConstants.default_bucket_num,true, primaryKeys);
            CreateTableStmt stmt = new CreateTableStmt(true, false, tblName, columns, new ArrayList<>(), "olap", keysDesc, null, distributionDesc, tableCreateProperties, new HashMap<>(), "", new ArrayList<>(), null);
            stmts.add(stmt);
        }
        return stmts;
    }
