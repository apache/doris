...
/**
 * function builder for python udtf
 */
public class PythonUdtfBuilder extends UdfBuilder {
    ...
    public PythonUdtfBuilder(PythonUdtf udtf) {
        this.udtf = udtf;
        this.isVarArgs = udtf.hasVarArguments();
        this.arity = udtf.arity();
    }

    @Override
    public List<DataType> getArgTypes() {
        return Suppliers.memoize(() -> udtf.getSignatures().get(0).argumentsTypes.stream()
    ...
    public Pair<PythonUdtf, PythonUdtf> build(String name, List<?> arguments) {
        ...
        List<DataType> argTypes = udtf.getSignatures().get(0).argumentsTypes;
    ...
    public String parameterDisplayString() {
        ...
        for (int i = 0; i < udtf.getArgumentsTypes().size(); ++i) {
            if (i > 0) {
                string.append(", ");
            }
            string.append(udtf.getArgumentsTypes().get(i));
            if (isVarArgs && i + 1 == udtf.getArgumentsTypes().size()) {
                string.append("...");
            }
        }
