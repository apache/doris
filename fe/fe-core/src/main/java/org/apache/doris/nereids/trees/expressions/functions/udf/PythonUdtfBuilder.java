...
/**
 * {@link PythonUdtfBuilder} is used to build the table generating function {@link PythonUdtf}.
 */
public class PythonUdtfBuilder extends UdfBuilder {

    private final PythonUdtf udf;

    private final int arity;

    private final boolean isVarArgs;

    public PythonUdtfBuilder(PythonUdtf udf) {
        this.udf = udf;
    ...
    @Override
    public Pair<PythonUdtf, PythonUdtf> build(String name, List<?> arguments) {
        List<Expression> expressions = arguments.stream().map(Expression.class::cast).collect(Collectors.toList());
        List<DataType> argTypes = udf.getSignatures().get(0).argumentsTypes;

        List<Expression> processedExpressions = Lists.newArrayList();
        for (int i = 0; i < expressions.size(); ++i) {
            processedExpressions.add(TypeCoercionUtils.castIfNotSameType(expressions.get(i), argTypes.get(i)));
        }
        return Pair.ofSame(udf.withChildren(processedExpressions));
    }
}
