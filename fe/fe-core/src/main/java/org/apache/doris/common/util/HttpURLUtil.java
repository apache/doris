...
public class HttpURLUtil {
    ...
    public static HttpURLConnection getConnectionWithNodeIdent(String request) throws IOException {
        ...
        String normalizedRequest = provider.normalizeInternalUrl(
                request, InternalHttpClientProvider.Target.FE);
        try {
            SecurityChecker.getInstance().startSSRFChecking(normalizedRequest);
            HttpURLConnection conn = provider.openConnection(
                    normalizedRequest, InternalHttpClientProvider.Target.FE);
