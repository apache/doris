...
public class Coalesce extends ScalarFunction implements CustomSignature {
    ...
    public FunctionSignature customSignature() {
        ...
        Optional<DataType> commonType = TypeCoercionUtils.findWiderCommonTypeForCaseWhen(forFindCommon);
        if (!commonType.isPresent()) {
            ...
            for (Expression stringLiteral : partitioned.get(true)) {
                ...
                if (!option.isPresent()) {
                    List<DataType> commonTypes = Lists.newArrayList(commonType.get(), stringLiteral.getDataType());
                    commonType = TypeCoercionUtils.findWiderCommonTypeForCaseWhen(commonTypes);
                    if (!commonType.isPresent()) {
                        SearchSignature.throwCanNotFoundFunctionException(this.getName(), getArguments());
                    } else {
                        break;
