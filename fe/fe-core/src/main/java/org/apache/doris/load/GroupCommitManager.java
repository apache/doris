...
public class GroupCommitManager {
        ...
        if (backendId != null) {
            ...
            if (pressure == null) {
                ...
                if (isBackendAvailable(backend)) {
                    debugBlockAfterCachedBackendAvailable(cacheKey, tableId, backendId);
    ...
    private static void debugBlockAfterCachedBackendAvailable(
        ...
        final String debugPointName = "GroupCommitManager.getCachedBackend.afterAvailable.block";
        DebugPointUtil.DebugPoint debugPoint = DebugPointUtil.getDebugPoint(debugPointName);
        if (debugPoint == null) {
            return;
        }
        ...
        if ((expectedTableId != -1L && expectedTableId != tableId)
                || (expectedBackendId != -1L && expectedBackendId != backendId)) {
            return;
        }
        LOG.info("debug block after cached group commit backend passed availability check, "
                        + "cacheKey={}, tableId={}, backendId={}", cacheKey, tableId, backendId);
        while (DebugPointUtil.isEnable(debugPointName)) {
            try {
                Thread.sleep(50);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                LOG.warn("group commit cached backend debug block interrupted, tableId={}, backendId={}",
                        tableId, backendId);
                return;
            }
        }
