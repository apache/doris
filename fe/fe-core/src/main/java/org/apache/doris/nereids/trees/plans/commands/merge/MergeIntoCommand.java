...
public class MergeIntoCommand extends Command implements ForwardWithSync, Explainable {
    private static final Logger LOG = LogManager.getLogger(MergeIntoCommand.class);
    private static final String MATCHED_MARK = "__DORIS_MERGE_INTO_MATCHED_MARK__";
    ...
    private NamedExpression generateBranchLabel() {
        ...
        return new Alias(new If(onClause, matchedLabel, notMatchedLabel), BRANCH_LABEL);
    }

    /**
     * use on clause generate mark column (1: delete or update candidate, 0: candidate).
     */
    private NamedExpression generateMatchedMark() {
        return new UnboundAlias(new If(onClause, new TinyIntLiteral((byte) 1),
                new TinyIntLiteral((byte) 0)), MATCHED_MARK);
    }
    ...
    private List<Expression> generateUpdateProjection(MergeMatchedClause clause,
        ...
        for (Column column : columns) {
            if (colNameToExpression.containsKey(column.getName())) {
                ...
                builder.add(colNameToExpression.get(column.getName()));
                ...
                builder.add(new NereidsParser().parseExpression(
    ...
    private List<Expression> generateInsertWithoutColListProjection(MergeNotMatchedClause clause,
        ...
        Expression sqlColExpr = new NullLiteral();
    ...
    private List<Expression> generateInsertWithColListProjection(MergeNotMatchedClause clause,
            List<Column> columns, OlapTable targetTable, boolean hasSequenceCol, int seqColumnIndex,
            String seqColumnName, Optional<Column> seqColumnInTable) {
        ImmutableList.Builder<Expression> builder = ImmutableList.builder();
        Map<String, Expression> colNameToExpression = Maps.newTreeMap(String.CASE_INSENSITIVE_ORDER);
        ...
        if (hasSequenceCol) {
            ...
            if (!colNameToExpression.containsKey(seqColumnName)
                    && (!seqColumnInTable.isPresent() || seqColumnInTable.get().getDefaultValue() == null
                    || !seqColumnInTable.get().getDefaultValue()
        ...
        for (Column column : columns) {
            ...
            if (column.isGeneratedColumn()) {
                if (colNameToExpression.containsKey(column.getName())
                        && !(colNameToExpression.get(column.getName()) instanceof DefaultValueSlot)) {
                    throw new AnalysisException("The value specified for generated column '"
                            + column.getName() + "' in table '" + targetTable.getName() + "' is not allowed.");
                }
                continue;
        ...
        if (targetTable.hasSequenceCol()) {
            if (colNameToExpression.containsKey(Column.SEQUENCE_COL)) {
                builder.add(colNameToExpression.get(Column.SEQUENCE_COL));
                ...
                builder.add(colNameToExpression.get(seqColumnName));
                colNameToExpression.remove(seqColumnName);
            } else {
                builder.add(new Now());
            }
     ...
     */
    @VisibleForTesting
    public LogicalPlan completeQueryPlan(ConnectContext ctx) {
        ...
        if (hasSequenceCol) {
            ...
            if (seqColName != null) {
                ...
                seqColInTable = columns.stream()
                        .filter(col -> col.getName().equalsIgnoreCase(finalSeqColName))
                        .findFirst();
            }

        }
        if (seqColumnIndex != -1 && !seqColInTable.isPresent()) {
        ...
        outputProjections.add(generateMatchedMark());
        ...
        for (MergeNotMatchedClause clause : notMatchedClauses) {
            if ((clause.getColNames().isEmpty() && (columnCount != clause.getRow().size()))
                    || (!clause.getColNames().isEmpty() && clause.getColNames().size() != clause.getRow().size())) {
            ...
            if (clause.getColNames().isEmpty()) {
                ...
                finalProjections.add(generateInsertWithColListProjection(clause, columns, targetTable,
                        hasSequenceCol, seqColumnIndex, seqColName, seqColInTable));
            }
        }
