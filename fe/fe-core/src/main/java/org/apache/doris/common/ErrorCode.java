...
public enum ErrorCode {
    ...
    ERR_WRONG_TABLE_NAME(1103, new byte[] {'4', '2', '0', '0', '0'},
            "Incorrect table name '%s'. Table name regex is '%s'"),
    ...
    ERR_TRUNCATED_WRONG_VALUE_FOR_FIELD(1366, new byte[] {'H', 'Y', '0', '0', '0'}, "Incorrect %s value: '%s' for "
            + "column '%s' at row %d"),
    ERR_ILLEGAL_VALUE_FOR_TYPE(1367, new byte[] {'2', '2', '0', '0', '7'},
    ...
    ERR_XAER_INVAL(1398, new byte[] {'X', 'A', 'E', '0', '5'},
    ...
    ERR_PARTITION_MERGE_ERROR(1572, new byte[] {'H', 'Y', '0', '0', '0'},
    ...
    ERR_EVENT_CANNOT_ALTER_IN_THE_PAST(1589, new byte[] {'H', 'Y', '0', '0', '0'}, "Event execution time is in the "
            + "past and ON COMPLETION NOT PRESERVE is set. The event was dropped immediately after creation."),
    ERR_SLAVE_INCIDENT(1590, new byte[] {'H', 'Y', '0', '0', '0'},
    ...
    ERR_DELAYED_NOT_SUPPORTED(1616, new byte[] {'H', 'Y', '0', '0', '0'},
    ...
    ERR_ERROR_IN_TRIGGER_BODY(1710, new byte[] {'H', 'Y', '0', '0', '0'},
    ...
    ERR_TABLESPACE_DISCARDED(1814, new byte[] {'H', 'Y', '0', '0', '0'},
    ...
    // Dynamic partition error code from 5100
    ERR_DYNAMIC_PARTITION_MUST_HAS_SAME_BUCKET_NUM_WITH_COLOCATE_TABLE(5100, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition buckets must equal the distribution buckets if creating a colocate table: %s"),
    ERROR_DYNAMIC_PARTITION_TIME_UNIT(5101, new byte[] {'4', '2', '0', '0', '0'},
            "Unsupported time unit %s. Expect HOUR/DAY/WEEK/MONTH."),
    ERROR_DYNAMIC_PARTITION_START_ZERO(5102, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition start must less than 0"),
    ERROR_DYNAMIC_PARTITION_START_FORMAT(5103, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition start %s"),
    ERROR_DYNAMIC_PARTITION_END_ZERO(5104, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition end must greater than 0"),
    ERROR_DYNAMIC_PARTITION_END_FORMAT(5105, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition end %s"),
    ERROR_DYNAMIC_PARTITION_END_EMPTY(5106, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition end is empty"),
    ERROR_DYNAMIC_PARTITION_BUCKETS_ZERO(5107, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition buckets must greater than 0"),
    ERROR_DYNAMIC_PARTITION_BUCKETS_FORMAT(5108, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition buckets %s"),
    ERROR_DYNAMIC_PARTITION_BUCKETS_EMPTY(5109, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition buckets is empty"),
    ERROR_DYNAMIC_PARTITION_ENABLE(5110, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition enable: %s. Expected true or false"),
    ERROR_DYNAMIC_PARTITION_PREFIX(5111, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition prefix: %s."),
    ERR_OPERATION_DISABLED(5112, new byte[] {'4', '2', '0', '0', '0'},
            "Operation %s is disabled. %s"),
    ERROR_DYNAMIC_PARTITION_REPLICATION_NUM_ZERO(5113, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic partition replication num must greater than 0"),
    ERROR_DYNAMIC_PARTITION_REPLICATION_NUM_FORMAT(5114, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition replication num: %s."),
    ERROR_CREATE_TABLE_LIKE_EMPTY(5115, new byte[] {'4', '2', '0', '0', '0'},
            "Origin create table stmt is empty"),
    ERROR_DYNAMIC_PARTITION_CREATE_HISTORY_PARTITION(5116, new byte[] {'4', '2', '0', '0', '0'},
            "Invalid dynamic partition create_history_partition: %s. Expected true or false"),
    ERROR_DYNAMIC_PARTITION_HISTORY_PARTITION_NUM_ZERO(5117, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic history partition num must greater than 0"),
    ERROR_DYNAMIC_PARTITION_RESERVED_HISTORY_PERIODS_EMPTY(5118, new byte[] {'4', '2', '0', '0', '0'},
            "Dynamic reserved history periods is empty."),
    ERROR_DYNAMIC_PARTITION_RESERVED_HISTORY_PERIODS_INVALID(5119, new byte[] {'4', '2', '0', '0', '0'},
    ...
    ERROR_DYNAMIC_PARTITION_RESERVED_HISTORY_PERIODS_START_ENDS_LENGTH_NOT_EQUAL(5120, new byte[] {'4', '2', '0', '0',
            '0'},
            "RESERVED_HISTORY_PERIODS must have pairs of date value. The input %s is not valid."),
    ERROR_DYNAMIC_PARTITION_RESERVED_HISTORY_PERIODS_START_LARGER_THAN_ENDS(5121, new byte[] {'4', '2', '0', '0', '0'},
            "The first date is larger than the second date, [%s,%s] is invalid."),
    ERROR_DYNAMIC_PARTITION_TIME_SOURCE(5122, new byte[] {'4', '2', '0', '0', '0'},
            "Not support the dynamic partition time source '%s'."),

    // LDAP error code from 5200
    ERROR_LDAP_CONFIGURATION_ERR(5200, new byte[] {'4', '2', '0', '0', '0'},
            "LDAP configuration is incorrect or LDAP admin password is not set."),
    ERROR_LDAP_USER_NOT_UNIQUE_ERR(5201, new byte[] {'4', '2', '0', '0', '0'},
            "%s is not unique in LDAP server."),

    // Catalog && schema error code from 5300
    ERR_WRONG_CATALOG_NAME(5300, new byte[] {'4', '2', '0', '0', '0'}, "Incorrect catalog name '%s'"),
    ERR_UNKNOWN_CATALOG(5301, new byte[] {'4', '2', '0', '0', '0'}, "Unknown catalog '%s'"),
    ERR_CATALOG_ACCESS_DENIED(5302, new byte[] {'4', '2', '0', '0', '0'},
            "Access denied for user '%s' to catalog '%s'"),
    ERR_ILLEGAL_COLUMN_REFERENCE_ERROR(5303, new byte[] {'4', '2', '0', '0', '1'},
            "Illegal column/field reference '%s' of semi-/anti-join"),
    ERR_EMPTY_PARTITION_IN_TABLE(5304, new byte[] {'4', '2', '0', '0', '0'},
            "data cannot be inserted into table with empty partition. "
                    + "Use `SHOW PARTITIONS FROM %s` to see the currently partitions of this table. "),
    ERROR_SQL_AND_LIMITATIONS_SET_IN_ONE_RULE(8305, new byte[] {'4', '2', '0', '0', '0'},
            "sql/sqlHash and partition_num/tablet_num/cardinality cannot be set in one rule."),
    ERR_NONSUPPORT_HMS_TABLE(5306, new byte[] {'4', '2', '0', '0', '0'},
            "Nonsupport hive metastore table named '%s' in database '%s' with catalog '%s'."),
    ERR_TABLE_NAME_LENGTH_LIMIT(5307, new byte[] {'4', '2', '0', '0', '0'}, "Table name length exceeds limit, "
            +
            "the length of table name '%s' is %d which is greater than the configuration 'table_name_length_limit' (%d)."),
    ERR_NONSUPPORT_TIME_TRAVEL_TABLE(5308, new byte[] {'4', '2', '0', '0', '0'}, "Only iceberg external"
            + " table supports time travel in current version"),

    // Other error code from 8000
    ERR_NONSSL_HANDSHAKE_RESPONSE(8000, new byte[] {'4', '2', '0', '0'},
