...
public class RegrSxx extends AggregateFunction
    ...
    public void checkLegalityBeforeTypeCoercion() {
        ...
        if (!yType.isNumericType() && !yType.isBooleanType()
            ...
            throw new AnalysisException("regr_sxx(y, x): y must be numeric, boolean or string type: " + toSql());
        }
        if (!xType.isNumericType() && !xType.isBooleanType()
                && !xType.isNullType() && !xType.isStringLikeType()) {
            throw new AnalysisException("regr_sxx(y, x): x must be numeric, boolean or string type: " + toSql());
        }
    }
