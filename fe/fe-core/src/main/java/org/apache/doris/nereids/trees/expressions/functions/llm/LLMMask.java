...
/**
 * LLM function 'LLM_Mask'
 */
public class LLMMask extends LLMFunction {
    ...
    public LLMMask(Expression arg0, Expression arg1) {
        super("llm_mask", arg0, arg1);
        if (children.size() != 2) {
            throw new AnalysisException("LLM_Mask requires exactly 2 arguments: text and fields array, "
                    + "but got " + children.size() + " arguments");
        }
    }

    @Override
    public LLMMask withChildren(List<Expression> newChildren) {
        Preconditions.checkArgument(newChildren.size() == 2,
                "LLM_Mask requires exactly 2 arguments");
        return new LLMMask(newChildren.get(0), newChildren.get(1));
    }
    ...
    public <R, C> R accept(ExpressionVisitor<R, C> visitor, C context) {
        return visitor.visitLLMMask(this, context);
    }
}
