 ...
 */
public class BoolOr extends NullableAggregateFunction implements UnaryExpression, CustomSignature, RollUpTrait {
    public BoolOr(Expression child) {
        this(false, false, child);
    }

    public BoolOr(boolean distinct, Expression arg) {
        this(distinct, false, arg);
    }

    private BoolOr(boolean distinct, boolean alwaysNullable, Expression arg) {
        super("bool_or", false, alwaysNullable, arg);
    }

    @Override
    public void checkLegalityBeforeTypeCoercion() {
        if (!getArgumentType(0).isBooleanType()) {
            throw new AnalysisException("bool_or only accepts BOOLEAN type.");
        }
    }

    @Override
    public FunctionSignature customSignature() {
        return FunctionSignature.ret(BooleanType.INSTANCE).args(BooleanType.INSTANCE);
    }
    ...
    public BoolOr withDistinctAndChildren(boolean distinct, List<Expression> children) {
        ...
        return new BoolOr(distinct, alwaysNullable, children.get(0));
    }
    ...
    public Function constructRollUp(Expression param, Expression... varParams) {
        return new BoolOr(this.distinct, this.alwaysNullable, param);
    }
