...
public abstract class StorageVault {
    ...
    private static StorageVault
        ...
        boolean setAsDefault = command.setAsDefault();
        StorageVault vault;
        switch (type) {
            case HDFS:
                vault = new HdfsStorageVault(name, ifNotExists, setAsDefault);
                ...
                break;
            case S3:
                if (!command.getProperties().containsKey(PropertyConverter.USE_PATH_STYLE)) {
                    command.getProperties().put(PropertyConverter.USE_PATH_STYLE, "true");
                }

                CreateResourceStmt resourceStmt =
                        new CreateResourceStmt(false, ifNotExists, name, command.getProperties());
                resourceStmt.analyzeResourceType();
                vault = new S3StorageVault(name, ifNotExists, setAsDefault, resourceStmt);
                break;
            default:
