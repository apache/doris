...
/**
 * logical paimon table sink for insert command.
 * This mirrors {@link LogicalIcebergTableSink} but targets Paimon external tables.
 */
public class LogicalPaimonTableSink<CHILD_TYPE extends Plan> extends LogicalTableSink<CHILD_TYPE>
    ...
    public LogicalPaimonTableSink(PaimonExternalDatabase database,
                                  ...
                                  CHILD_TYPE child) {
        super(PlanType.LOGICAL_PAIMON_TABLE_SINK, outputExprs, groupExpression, logicalProperties, cols, child);
        this.database = Objects.requireNonNull(database, "database != null in LogicalPaimonTableSink");
        this.targetTable = Objects.requireNonNull(targetTable, "targetTable != null in LogicalPaimonTableSink");
    ...
    public Plan withChildren(List<Plan> children) {
        Preconditions.checkArgument(children.size() == 1, "LogicalPaimonTableSink only accepts one child");
