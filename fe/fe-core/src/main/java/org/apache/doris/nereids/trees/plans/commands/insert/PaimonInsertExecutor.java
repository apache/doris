 ...
 *
 * This implementation follows the same pattern as {@link IcebergInsertExecutor},
 * but delegates the actual commit/rollback semantics to {@link PaimonTransaction}.
 */
public class PaimonInsertExecutor extends BaseExternalTableInsertExecutor {
    ...
    public void beginTransaction() {
        ...
        if (insertCtx.isPresent() && insertCtx.get() instanceof BaseExternalTableInsertCommandContext) {
            ...
            ctx.setTxnId(txnId);
            ctx.setCommitUser("doris_txn_" + txnId);
        }
    }
    ...
    protected void doBeforeCommit() throws UserException {
        PaimonTransaction transaction = (PaimonTransaction) transactionManager.getTransaction(txnId);
        long updateCnt = transaction.getUpdateCnt();
        if (updateCnt > 0) {
