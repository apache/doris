...
public class PushdownFilterThroughRepeat extends OneRewriteRuleFactory {
    ...
    public Rule build() {
        return logicalFilter(logicalRepeat()).then(filter -> {
            ...
            for (Expression groupByExpression : repeat.getGroupByExpressions()) {
                if (groupByExpression instanceof Slot
                        && !(groupByExpression instanceof VirtualSlotReference)) {
            ...
            for (Expression conjunct : ExpressionUtils.extractConjunction(filter.getPredicates())) {
                ...
                // must all slot in groupBySlots
                if (groupBySlots.size() != conjunctSlots.size()) {
                    continue;
                }
            ...
            return pushDownPredicate(filter, repeat, pushDownPredicates, filterPredicates);
