...
class ShuffleKeyPrunerTest extends TestWithFeService {
    ...
    @Test
    void testGlobalAggPostProcessorDoesNotPruneEquivalentNumericKeys() {
        SlotReference key1 = new SlotReference(new ExprId(60), "key1",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference key2 = new SlotReference(new ExprId(61), "key2",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference stringKey = new SlotReference(new ExprId(62), "string_key",
                new VarcharType(64), true, ImmutableList.of());
        SlotReference otherGroupKey = new SlotReference(new ExprId(63), "other_group_key",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference bridge = new SlotReference(new ExprId(64), "bridge",
                IntegerType.INSTANCE, true, ImmutableList.of());
        Statistics inputStatistics = knownStatistics(
                1000, key1, key2, stringKey, otherGroupKey, bridge);
        DistributionSpecHash hashSpec = overlappingHashSpec(key1, key2, stringKey, bridge);
        PhysicalEmptyRelation relation = (PhysicalEmptyRelation) new PhysicalEmptyRelation(
                connectContext.getStatementContext().getNextRelationId(),
                ImmutableList.of(key1, key2, stringKey, otherGroupKey, bridge), null)
        ...
        PhysicalHashAggregate<?> aggregate = new PhysicalHashAggregate<>(
                ImmutableList.of(key1, key2, stringKey, otherGroupKey),
                ImmutableList.of(key1, key2, stringKey, otherGroupKey),
                new AggregateParam(AggPhase.GLOBAL, AggMode.BUFFER_TO_RESULT),
                true, null, false, distribute);
        CascadesContext cascadesContext = Mockito.mock(CascadesContext.class);
    ...
    @Test
    void testShuffleJoinPostProcessorDoesNotPruneEquivalentNumericKeys() {
        SlotReference leftKey1 = new SlotReference(new ExprId(65), "left_key1",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference leftKey2 = new SlotReference(new ExprId(66), "left_key2",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference leftString = new SlotReference(new ExprId(67), "left_string",
                new VarcharType(64), true, ImmutableList.of());
        SlotReference leftBridge = new SlotReference(new ExprId(68), "left_bridge",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference rightKey1 = new SlotReference(new ExprId(69), "right_key1",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference rightKey2 = new SlotReference(new ExprId(70), "right_key2",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference rightString = new SlotReference(new ExprId(71), "right_string",
                new VarcharType(64), true, ImmutableList.of());
        SlotReference rightBridge = new SlotReference(new ExprId(72), "right_bridge",
                IntegerType.INSTANCE, true, ImmutableList.of());
        Statistics leftStatistics = knownStatistics(1000, leftKey1, leftKey2, leftString, leftBridge);
        Statistics rightStatistics = knownStatistics(1000, rightKey1, rightKey2, rightString, rightBridge);
        PhysicalDistribute<PhysicalEmptyRelation> left = distributeWithOverlappingHash(
                leftKey1, leftKey2, leftString, leftBridge, leftStatistics);
        PhysicalDistribute<PhysicalEmptyRelation> right = distributeWithOverlappingHash(
                rightKey1, rightKey2, rightString, rightBridge, rightStatistics);
        PhysicalHashJoin<Plan, Plan> join = new PhysicalHashJoin<>(JoinType.INNER_JOIN,
                ImmutableList.of(
                        ...
                        new EqualTo(leftString, rightString)),
        ...
        connectContext.getSessionVariable().enableShuffleKeyPrune = true;
        try {
            PhysicalHashJoin<?, ?> output = (PhysicalHashJoin<?, ?>) new ShuffleKeyPruner()
                    .processRoot(join, cascadesContext);

            Assertions.assertEquals(3,
                    getHashSpecFromJoinChild(output.left()).getOrderedShuffledColumns().size());
            Assertions.assertEquals(3,
                    getHashSpecFromJoinChild(output.right()).getOrderedShuffledColumns().size());
        } finally {
            connectContext.getSessionVariable().setBeNumberForTest(previousBeNumberForTest);
            connectContext.getSessionVariable().parallelPipelineTaskNum = previousParallelPipelineTaskNum;
            connectContext.getSessionVariable().enableShuffleKeyPrune = previousEnableShuffleKeyPrune;
    ...
    private Statistics knownStatistics(double ndv, SlotReference... slots) {
        StatisticsBuilder statistics = new StatisticsBuilder().setRowCount(1_000_000_000);
        for (SlotReference slot : slots) {
            statistics.putColumnStatistics(slot, new ColumnStatisticBuilder(1_000_000_000)
                    .setNdv(ndv)
                    .setHotValues(ImmutableMap.of())
                    .build());
        }
    ...
    private PhysicalDistribute<PhysicalEmptyRelation> distributeWithOverlappingHash(
            SlotReference key1, SlotReference key2, SlotReference stringKey,
            SlotReference bridge, Statistics statistics) {
        DistributionSpecHash hashSpec = overlappingHashSpec(key1, key2, stringKey, bridge);
        PhysicalEmptyRelation relation = (PhysicalEmptyRelation) new PhysicalEmptyRelation(
                connectContext.getStatementContext().getNextRelationId(),
                ImmutableList.of(key1, key2, stringKey, bridge), null)
                .withPhysicalPropertiesAndStats(PhysicalProperties.ANY, statistics);
        return new PhysicalDistribute<>(hashSpec, Optional.empty(), relation.getLogicalProperties(),
                PhysicalProperties.createHash(hashSpec), statistics, relation);
    }

    private DistributionSpecHash overlappingHashSpec(
            SlotReference key1, SlotReference key2, SlotReference stringKey, SlotReference bridge) {
        return new DistributionSpecHash(
                ImmutableList.of(key1.getExprId(), key2.getExprId(), stringKey.getExprId()),
                DistributionSpecHash.ShuffleType.EXECUTION_BUCKETED, -1L, ImmutableSet.of(),
                ImmutableList.of(
                        ImmutableSet.of(key1.getExprId(), bridge.getExprId()),
                        ImmutableSet.of(key2.getExprId(), bridge.getExprId()),
                        ImmutableSet.of(stringKey.getExprId())),
                ImmutableMap.of(
                        key1.getExprId(), 0,
                        key2.getExprId(), 1,
                        stringKey.getExprId(), 2,
                        bridge.getExprId(), 1));
    }
