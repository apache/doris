...
public class CacheHotspotManagerTest {
    ...
    public void testCreateEventDrivenJobUnaffected() throws AnalysisException {
        WarmUpClusterStmt eventDrivenStmt = newClusterStmt("dst", "src", false, eventDrivenProperties("load"));
    ...
    private WarmUpClusterStmt newTableStmt(String dstClusterName, boolean force,
            Triple<String, String, String>... tables) {
        List<Triple<String, String, String>> tableList = new ArrayList<>();
        for (Triple<String, String, String> table : tables) {
            tableList.add(table);
        }
        WarmUpClusterStmt stmt = Mockito.mock(WarmUpClusterStmt.class);
        Mockito.when(stmt.isWarmUpWithTable()).thenReturn(true);
        Mockito.when(stmt.getDstClusterName()).thenReturn(dstClusterName);
        Mockito.when(stmt.getTables()).thenReturn(tableList);
        Mockito.when(stmt.isForce()).thenReturn(force);
        return stmt;
    }

    private WarmUpClusterStmt newClusterStmt(String dstClusterName, String srcClusterName, boolean force) {
        return newClusterStmt(dstClusterName, srcClusterName, force, new HashMap<>());
    }

    private WarmUpClusterStmt newClusterStmt(String dstClusterName, String srcClusterName,
            boolean force, Map<String, String> properties) {
        WarmUpClusterStmt stmt = Mockito.mock(WarmUpClusterStmt.class);
        Mockito.when(stmt.isWarmUpWithTable()).thenReturn(false);
        Mockito.when(stmt.getDstClusterName()).thenReturn(dstClusterName);
        Mockito.when(stmt.getSrcClusterName()).thenReturn(srcClusterName);
        Mockito.when(stmt.isForce()).thenReturn(force);
        Mockito.when(stmt.getProperties()).thenReturn(properties);
        return stmt;
    }
