...
class RowTtlVisibilityPlanTest extends TestWithFeService {
    ...
    @Test
    void testUnrelatedOfflineBackendDoesNotBlockRowTtlBinding() throws Exception {
        SystemInfoService clusterInfo = Env.getCurrentEnv().getClusterInfo();
        Backend offlineBackend = new Backend(Env.getCurrentEnv().getNextId(), "127.0.0.250", 9050);
        clusterInfo.addBackend(offlineBackend);
        try {
            Assertions.assertFalse(offlineBackend.isAlive());
            String duplicatePlan = rewrittenPlan("select k, v from ttl_source_dup");
            assertContains(duplicatePlan, "row_ttl_is_visible");
            assertContains(duplicatePlan, Column.TTL_COL);
            assertContains(duplicatePlan, "86400000000");
            assertContains(duplicatePlan, "28800");

            String uniquePlan = rewrittenPlan("select k, v from ttl_source_mow");
            assertContains(uniquePlan, "row_ttl_is_visible");
            assertContains(uniquePlan, Column.DELETE_SIGN);

            Assertions.assertNotNull(getSQLPlanner(
                    "explain insert into ttl_source_dup values (1, '2026-01-01 00:00:00', 10)"));
            Assertions.assertNotNull(getSQLPlanner(
                    "explain insert into ttl_source_mow values (1, '2026-01-01 00:00:00', 10)"));
        } finally {
            clusterInfo.dropBackend(offlineBackend.getId());
        }
    }
    ...
    void testVisibilityFunctionIsNonDeterministic() {
        Assertions.assertFalse(new RowTtlIsVisible(new BigIntLiteral(1)).isDeterministic());
    }
