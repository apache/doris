...
class EliminateGroupByKeyTest extends TestWithFeService implements MemoPatternMatchSupported {
    ...
    @Test
    void testDerivedExpressionFdEliminatesRedundantKeys() {
    ...
    @Test
    void testDerivedAliasFdEliminatesRedundantKeys() {
    ...
    @Test
    void testDerivedScalarAliasUsesSchemaFd() {
        analyzeWithoutSimplify("select count(*) from "
                + "(select id, substring(name, 1, 3) as prefix from uni) t group by id, prefix")
                .customRewrite(new EliminateGroupByKey())
                .matches(logicalAggregate().when(agg -> agg.getGroupByExpressions().size() == 1));

        analyzeWithoutSimplify("select count(*) from "
                + "(select id, substring(name, id, id) as prefix from uni) t group by id, prefix")
                .customRewrite(new EliminateGroupByKey())
                .matches(logicalAggregate().when(agg -> agg.getGroupByExpressions().size() == 1));

        analyzeWithoutSimplify("select count(*) from "
                + "(select id, substring(name, cast(id as tinyint), 3) as prefix from uni) t "
                + "group by id, prefix")
                .customRewrite(new EliminateGroupByKey())
                .matches(logicalAggregate().when(agg -> agg.getGroupByExpressions().size() == 1));
    }
