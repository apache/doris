...
class BindExpressionTest extends TestWithFeService implements GeneratedPlanPatterns {
    ...
    @Test
    void testAutoGeneratedAggregateAliasFromDerivedTable() {
        String sql = "select `dt`, `sum(source_table_num)`, `sum(target_table_num)` from ("
                + "select r.dt, sum(r.source_table_num), sum(r.target_table_num) from ("
                + "select date(cast(col1 as datetime)) as dt, "
                + "cast(col2 as int) as source_table_num, cast(col2 as int) as target_table_num from t1"
                + ") r group by r.dt"
                + ") dataset_tb";
        PlanChecker.from(connectContext)
                .analyze(sql)
