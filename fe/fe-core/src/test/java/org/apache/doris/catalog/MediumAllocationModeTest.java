...
public class MediumAllocationModeTest extends TestWithFeService {
    ...
    /**
     * Build CREATE TABLE SQL with medium allocation policy
     */
    private String buildCreateTableSql(String dbName, String tableName, String storageMedium, String policy) {
        ...
        if (policy != null) {
            sql.append(", 'medium_allocation_mode' = '").append(policy).append("'");
        }
        sql.append(");");
        return sql.toString();
    }
     ...
     * Expected: STRICT policy succeeds on matching medium, fails on non-matching
     *           ADAPTIVE policy succeeds in all cases
     */
    @Test
     ...
     * Expected: STRICT policy succeeds when specified medium is available
     */
    @Test
