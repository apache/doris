...
class BitmapCountToBitmapOpCountTest extends ExpressionRewriteTestHelper {
    ...
    @Test
    void testBitmapOpCountNotNullableMetadata() {
        Assertions.assertFalse(new BitmapAndCount(bitmap1, bitmap2).nullable());
        Assertions.assertFalse(new BitmapOrCount(bitmap1, bitmap2).nullable());
        Assertions.assertFalse(new BitmapXorCount(bitmap1, bitmap2).nullable());
        Assertions.assertFalse(new BitmapAndNotCount(bitmap1, bitmap2).nullable());
    }

    @Test
    void testRewriteToNotNullableBitmapOpCount() {
        Expression rewrittenExpression = executor.rewrite(new BitmapCount(new BitmapAnd(bitmap1, bitmap2)), context);
        Assertions.assertInstanceOf(BitmapAndCount.class, rewrittenExpression);
        Assertions.assertFalse(rewrittenExpression.nullable());
    }
}
