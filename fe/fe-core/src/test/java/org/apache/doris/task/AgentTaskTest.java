...
public class AgentTaskTest {
    ...
    public void setUp() throws AnalysisException {
        ...
        createReplicaTask = new CreateReplicaTask(backendId1, dbId, tableId, partitionId,
                ...
                storagePageSize, TEncryptionAlgorithm.PLAINTEXT, storageDictPageSize, new HashMap<>(), null);
    ...
    public void toThriftTest() throws Exception {
        ...
        rowBinlogColumns.add(new Column("v1", ScalarType.createType(PrimitiveType.INT), false, AggregateType.NONE, "1", ""));
        MaterializedIndexMeta rowBinlogMeta = new MaterializedIndexMeta(9999L, rowBinlogColumns, 1, 1, (short) 1,
        ...
        AgentTask createWithRowBinlog = new CreateReplicaTask(backendId1, dbId, tableId, partitionId,
                indexId1, tabletId1, replicaId1, shortKeyNum, schemaHash1, version, KeysType.AGG_KEYS, storageType,
                TStorageMedium.SSD, columns, null, 0, latch, null, false, TTabletType.TABLET_TYPE_DISK, null,
                TCompressionType.LZ4F, false, "", false, false, false, "", 0, 0, 0, 0, 0, false,
                binlogConfig, null, objectPool, rowStorePageSize, false, storagePageSize,
                TEncryptionAlgorithm.PLAINTEXT, storageDictPageSize, new HashMap<>(), rowBinlogMeta);
        TAgentTaskRequest requestRow = (TAgentTaskRequest) toAgentTaskRequest.invoke(agentBatchTask, createWithRowBinlog);
        Assert.assertNotNull(requestRow.getCreateTabletReq());
        Assert.assertTrue(requestRow.getCreateTabletReq().isSetRowBinlogSchema());
