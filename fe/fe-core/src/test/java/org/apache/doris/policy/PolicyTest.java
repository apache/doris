...
public class PolicyTest extends TestWithFeService {
    ...
    public void testExistPolicyRole() throws Exception {
        ...
        Assertions.assertTrue(Env.getCurrentEnv().getPolicyMgr().canMatchPolicy("default_cluster:test_policy", "default_cluster:testRole"));
        dropPolicy("DROP ROW POLICY test_row_policy ON test.table1 FOR role 'testRole'");
        Assertions.assertFalse(Env.getCurrentEnv().getPolicyMgr().canMatchPolicy("default_cluster:test_policy", "default_cluster:testRole"));
        createPolicy("CREATE ROW POLICY test_row_policy ON test.table1 AS PERMISSIVE TO ROLE 'testRole' USING (k1 = 1)");
        dropPolicy("DROP ROW POLICY test_row_policy ON test.table1");
        Assertions.assertFalse(Env.getCurrentEnv().getPolicyMgr().canMatchPolicy("default_cluster:test_policy", "default_cluster:testRole"));
    }
