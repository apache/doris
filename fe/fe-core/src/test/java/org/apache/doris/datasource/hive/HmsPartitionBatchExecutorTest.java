...
public class HmsPartitionBatchExecutorTest {
    ...
    public void boundsLargeRequestAndRestoresOrder() {
        ...
        List<Partition> result = executor.executeWithStats(request(names)).getPartitions();
    ...
    public void parsesAndValidatesConfiguration() {
        Assertions.assertEquals(5000, ThriftHMSCachedClient.parsePartitionBatchSize((String) null));
        Assertions.assertEquals(5000, ThriftHMSCachedClient.parsePartitionBatchSize(" "));
        Assertions.assertEquals(321, ThriftHMSCachedClient.parsePartitionBatchSize("321"));
