    ...
    public void testLosslessDecimalStringComparisonCoercion() {
        ...
        try {
            for (boolean newTypeCoercionBehavior : new boolean[] {true, false}) {
                ...
                Assertions.assertEquals(decimal54, decimalChar.left().getDataType());
                Assertions.assertEquals(decimal54, decimalChar.right().getDataType());
                Cast decimalCharCast = (Cast) decimalChar.right();
                Assertions.assertTrue(decimalCharCast.isLosslessDecimalCast());
                Assertions.assertTrue(decimalCharCast.withTargetType(decimal380).isLosslessDecimalCast());
                Assertions.assertTrue(((Cast) decimalCharCast.withConstantArgs(new StringLiteral("1")))
                        .isLosslessDecimalCast());
                Assertions.assertNotEquals(
                        new Cast(decimalCharCast.child(), decimal54, false).toDigest(),
                        decimalCharCast.toDigest());

                DecimalV3Type decimal240 = DecimalV3Type.createDecimalV3Type(24, 0);
                EqualTo decimalVarchar = new EqualTo(
                        new SlotReference("decimal_col", decimal240),
                        new SlotReference("varchar_col", VarcharType.createVarcharType(510)));
                decimalVarchar = (EqualTo) TypeCoercionUtils.processComparisonPredicate(decimalVarchar);
                Assertions.assertEquals(decimal240, decimalVarchar.left().getDataType());
                Assertions.assertEquals(decimal240, decimalVarchar.right().getDataType());
                Assertions.assertTrue(((Cast) decimalVarchar.right()).isLosslessDecimalCast());
