...
public class DecommissionBackendTest extends TestWithFeService {
    ...
    public void testDecommissionBackend() throws Exception {
        ...
        ImmutableMap<Long, Backend> idToBackendRef = Env.getCurrentSystemInfo().getBackendsWithIdByCurrentCluster();
        ...
        while (System.currentTimeMillis() - startTimestamp < 90000
            && Env.getCurrentSystemInfo().getBackendsWithIdByCurrentCluster().containsKey(srcBackend.getId())) {
            Thread.sleep(1000);
        }

        Assertions.assertEquals(backendNum() - 1, Env.getCurrentSystemInfo().getBackendsWithIdByCurrentCluster().size());
        ...
        // 6. add backend
        addNewBackend();
        Assertions.assertEquals(backendNum(), Env.getCurrentSystemInfo().getBackendsWithIdByCurrentCluster().size());
    }
    ...
    public void testDecommissionBackendWithDropTable() throws Exception {
        ...
        ImmutableMap<Long, Backend> idToBackendRef = infoService.getBackendsWithIdByCurrentCluster();
        Assertions.assertEquals(backendNum(), idToBackendRef.size());
