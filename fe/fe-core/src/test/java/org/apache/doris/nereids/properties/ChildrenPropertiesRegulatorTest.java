...
public class ChildrenPropertiesRegulatorTest {
    ...
    private enum RequiredShuffleShape {
        PARENT_SINGLE_KEY,
        PARENT_TWO_KEYS,
        PARENT_TWO_KEYS_WITH_PARTIAL_EQUIVALENCE,
        FULL_GROUP_BY
    }
    ...
    @Test
    public void testUnsafeFurtherSubsetFallsBackToApprovedParentKeys() {
        double rowCount = 2_870_000_000D;
        ColumnStatistic joinDerivedKey = new ColumnStatisticBuilder(rowCount)
                .setNdv(10_000_000)
                .setNumNulls(67_000_000)
        ...
        Assertions.assertInstanceOf(DistributionSpecHash.class,
                adjustment.properties.getDistributionSpec());
        DistributionSpecHash resultHash =
                (DistributionSpecHash) adjustment.properties.getDistributionSpec();
        Assertions.assertEquals(ShuffleType.EXECUTION_BUCKETED, resultHash.getShuffleType());
    ...
    @Test
    public void testOverlappingEquivalentRequiredKeysFallBackToFullGroupBy() {
        ShuffleAdjustment adjustment = adjustParentShuffleReuseWithKeys(
                1000, 1000, 1000, 3,
        ...
        DistributionSpecHash resultHash = (DistributionSpecHash) result.getDistributionSpec();
        Assertions.assertEquals(ShuffleType.EXECUTION_BUCKETED, resultHash.getShuffleType());
        Assertions.assertEquals(adjustment.fullKeys, resultHash.getOrderedShuffledColumns());
    }

    @Test
    public void testCrossSourceEquivalentKeysFallBackToFullGroupBy() {
        ShuffleAdjustment adjustment = adjustParentShuffleReuseWithKeys(
                1000, 1000, 1000, 3,
                RequiredShuffleShape.PARENT_TWO_KEYS_WITH_PARTIAL_EQUIVALENCE,
                ActualShuffleShape.CROSS_SOURCE_EQUIVALENCE, AggPhase.GLOBAL);
    ...
    private ShuffleAdjustment adjustParentShuffleReuseWithKeys(
        ...
        DistributionSpecHash requiredChildHash;
        if (requiredShuffleShape == RequiredShuffleShape.PARENT_TWO_KEYS_WITH_PARTIAL_EQUIVALENCE) {
            requiredChildHash = new DistributionSpecHash(
                    parentRequiredKeys, ShuffleType.REQUIRE, -1L, Sets.newHashSet(),
                    Lists.newArrayList(
                            Sets.newHashSet(key1.getExprId(), equivalentKey1.getExprId()),
                            Sets.newHashSet(key2.getExprId())),
                    ImmutableMap.of(
                            key1.getExprId(), 0,
                            equivalentKey1.getExprId(), 0,
                            key2.getExprId(), 1));
        } else {
            requiredChildHash = new DistributionSpecHash(parentRequiredKeys, ShuffleType.REQUIRE);
        }
        Mockito.when(jobContext.getRequiredProperties()).thenReturn(
                PhysicalProperties.createHash(requiredChildHash));
        ...
        PhysicalProperties requiredChildProperty = PhysicalProperties.createHash(requiredChildHash);
        ...
        } else if (actualShuffleShape == ActualShuffleShape.CROSS_SOURCE_EQUIVALENCE) {
            actualChildHash = new DistributionSpecHash(
                    Lists.newArrayList(key1.getExprId(), key2.getExprId()), ShuffleType.NATURAL,
                    -1L, Sets.newHashSet(),
                    Lists.newArrayList(
                            Sets.newHashSet(key1.getExprId()),
                            Sets.newHashSet(key2.getExprId(), equivalentKey1.getExprId())),
                    ImmutableMap.of(
                            key1.getExprId(), 0,
                            key2.getExprId(), 1,
                            equivalentKey1.getExprId(), 1));
