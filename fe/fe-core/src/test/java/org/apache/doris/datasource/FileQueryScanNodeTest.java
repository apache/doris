...
public class FileQueryScanNodeTest {
    ...
    @Test
    public void testFileParentCarriesItsFeTargetSplitSize() {
        FileSplit split = Mockito.mock(FileSplit.class);
        split.targetSplitSize = 64 * MB;

        TFileRangeDesc parent = new TFileRangeDesc();
        parent.setIsFileParent(true);
        FileQueryScanNode.setFileParentTargetSplitSize(parent, split);
        Assert.assertTrue(parent.isSetTargetSplitSize());
        Assert.assertEquals(64 * MB, parent.getTargetSplitSize());

        TFileRangeDesc ordinaryRange = new TFileRangeDesc();
        FileQueryScanNode.setFileParentTargetSplitSize(ordinaryRange, split);
        Assert.assertFalse(ordinaryRange.isSetTargetSplitSize());
    }
