...
public class PushDownVirtualColumnsIntoOlapScanTest {
    ...
    @Test
    public void testIsBeneficialToPushDown_SimpleExpressions() {
        // Test that simple expressions are not beneficial to push down

        DataType intType = IntegerType.INSTANCE;
        SlotReference x = new SlotReference("x", intType);
        IntegerLiteral constant = new IntegerLiteral(10);

        PushDownVirtualColumnsIntoOlapScan rule = new PushDownVirtualColumnsIntoOlapScan();

        // Use reflection to access the private method for testing
        try {
            java.lang.reflect.Method method = rule.getClass().getDeclaredMethod("isBeneficialToPushDown",
                    org.apache.doris.nereids.trees.expressions.Expression.class);
            method.setAccessible(true);

            // Test Slot - should not be beneficial
            boolean result1 = (boolean) method.invoke(rule, x);
            assert !result1 : "Slot expressions should not be beneficial to push down";

            // Test constant - should not be beneficial
            boolean result2 = (boolean) method.invoke(rule, constant);
            assert !result2 : "Constant expressions should not be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown method", e);
        }
    }

    @Test
    public void testIsBeneficialToPushDown_CastExpressions() {
        // Test that CAST expressions are not beneficial to push down

        DataType intType = IntegerType.INSTANCE;
        DataType varcharType = VarcharType.SYSTEM_DEFAULT;
        SlotReference x = new SlotReference("x", intType);
        Cast castExpr = new Cast(x, varcharType);
        ...
        try {
            ...
            boolean result = (boolean) method.invoke(rule, castExpr);
            assert !result : "CAST expressions should not be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown with CAST", e);
        }
    }

    @Test
    public void testIsBeneficialToPushDown_EncodeFunctions() {
        // Test that encode/decode functions are not beneficial to push down

        DataType intType = IntegerType.INSTANCE;
        SlotReference x = new SlotReference("x", intType);

        // Create encode function
        EncodeAsBigInt encodeFunc = new EncodeAsBigInt(x);
        DecodeAsVarchar decodeFunc = new DecodeAsVarchar(x);
        ...
        try {
            ...
            boolean result1 = (boolean) method.invoke(rule, encodeFunc);
            assert !result1 : "EncodeAsBigInt functions should not be beneficial to push down";

            boolean result2 = (boolean) method.invoke(rule, decodeFunc);
            assert !result2 : "DecodeAsVarchar functions should not be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown with encode/decode", e);
        }
    }
    ...
    @Test
    public void testIsBeneficialToPushDown_WithIndexPushdownFunctions() {
        // Test that expressions containing index pushdown functions are not beneficial to push down
        ...
        try {
            ...
            boolean result = (boolean) method.invoke(rule, ipRangeFunc);
            assert !result : "Expressions containing index pushdown functions should not be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown with index functions", e);
        }
    }

    @Test
    public void testIsBeneficialToPushDown_WithColumnPredicates() {
        // Test that expressions that can be converted to ColumnPredicate are not beneficial to push down
        ...
        try {
            ...
            boolean result1 = (boolean) method.invoke(rule, eq);
            assert !result1 : "EqualTo expressions should not be beneficial to push down";

            boolean result2 = (boolean) method.invoke(rule, gt);
            assert !result2 : "GreaterThan expressions should not be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown with column predicates", e);
        }
    }

    @Test
    public void testIsBeneficialToPushDown_BeneficialExpressions() {
        // Test that complex expressions without blacklist conditions are beneficial to push down
        ...
        // Create complex expression: (x + y) * z - which should be beneficial
        Add xyAdd = new Add(x, y);
        Multiply complexExpr = new Multiply(xyAdd, z);
        ...
        try {
            ...
            boolean result = (boolean) method.invoke(rule, complexExpr);
            assert result : "Complex arithmetic expressions should be beneficial to push down";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test isBeneficialToPushDown with beneficial expressions", e);
        }
    }

    @Test
    public void testGetExpressionComplexity() {
        // Test expression complexity calculation
        ...
        // Simple expression: x
        Add simpleExpr = new Add(x, five);

        // Complex expression: (x + y) * 5
        Add xyAdd = new Add(x, y);
        Multiply complexExpr = new Multiply(xyAdd, five);

        PushDownVirtualColumnsIntoOlapScan rule = new PushDownVirtualColumnsIntoOlapScan();

        try {
            java.lang.reflect.Method method = rule.getClass().getDeclaredMethod("getExpressionComplexity",
                    org.apache.doris.nereids.trees.expressions.Expression.class);
            method.setAccessible(true);

            int simpleComplexity = (int) method.invoke(rule, simpleExpr);
            int complexComplexity = (int) method.invoke(rule, complexExpr);

            assert simpleComplexity > 0 : "Simple expression should have positive complexity";
            assert complexComplexity > simpleComplexity : "Complex expression should have higher complexity";

        } catch (Exception e) {
            throw new RuntimeException("Failed to test getExpressionComplexity", e);
        }
    }
}
