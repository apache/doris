...
public class CreateTableAsSelectStmtTest {
    ...
    public void testErrorType() {
        String selectFromDecimal =
            "create table `test`.`select_decimal_table` PROPERTIES(\"replication_num\" = \"1\") as select * from `test`.`decimal_table`";
    ...
    public void testErrorColumn() {
        String selectFromColumn =
            "create table `test`.`select_column_table`(test_error) PROPERTIES(\"replication_num\" = \"1\") as select * from `test`.`varchar_table`";
    ...
    public void testFunction() throws Exception {
        String selectFromFunction1 =
            "create table `test`.`select_function_1` PROPERTIES(\"replication_num\" = \"1\") as select count(*) from `test`.`varchar_table`";
        ...
        String selectFromFunction2 =
            "create table `test`.`select_function_2` PROPERTIES(\"replication_num\" = \"1\") as select sum(status), sum(status), sum(status), count(status), count(status) from `test`.`join_table`";
    ...
    public void testAlias() throws Exception {
        String selectAlias1 =
            "create table `test`.`select_alias_1` PROPERTIES(\"replication_num\" = \"1\") as select count(*) as amount from `test`.`varchar_table`";
        ...
        Assert.assertEquals("CREATE TABLE `select_alias_1` (\n" +
            ...
            ")", showResultSet1.getResultRows().get(0).get(1));
        String selectAlias2 =
            "create table `test`.`select_alias_2` PROPERTIES(\"replication_num\" = \"1\") as select userId as alias_name, username from `test`.`varchar_table`";
