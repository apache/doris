...
public class GroupExpressionMatchingTest {
    ...
    public void testLeafNode() {
        ...
        Memo memo = new Memo(new UnboundRelation(RelationId.createGenerator().getNextId(), Lists.newArrayList("test")));
    ...
    public void testDepth2() {
        ...
        Plan anotherLeaf = new UnboundRelation(RelationId.createGenerator().getNextId(), Lists.newArrayList("test2"));
    ...
    public void testAnyWithChild() {
        Plan root = new LogicalProject(Lists.newArrayList(),
                new UnboundRelation(RelationId.createGenerator().getNextId(), Lists.newArrayList("test")));
        Memo memo = new Memo(root);

        Plan anotherLeaf = new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("test2"));
        memo.copyIn(anotherLeaf, memo.getRoot().getLogicalExpression().child(0), false);
    ...
    public void testInnerLogicalJoinMatch() {
        Plan root = new LogicalJoin(JoinType.INNER_JOIN,
                new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("a")),
                new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("b"))
        );
                ...
                new LogicalFilter(new EqualTo(new UnboundSlot(Lists.newArrayList("a", "id")),
                        ...
                        new LogicalJoin(JoinType.INNER_JOIN,
                                new LogicalJoin(JoinType.LEFT_OUTER_JOIN,
                                        new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("a")),
                                        new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("b"))),
                                new UnboundRelation(RelationId.createGenerator().getNextId(), ImmutableList.of("c")))
                );
