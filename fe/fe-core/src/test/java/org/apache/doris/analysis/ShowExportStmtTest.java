...
public class ShowExportStmtTest {
    ...
    public void testNormal() throws UserException {
        ...
        Assert.assertEquals("SHOW EXPORT FROM `internal`.`testDb`", stmt.toString());
    }
    ...
    public void testWhere() throws UserException {
        ...
        stmt.analyze(analyzer);
        Assert.assertEquals("SHOW EXPORT FROM `internal`.`testDb` WHERE (`label` = 'abc') LIMIT 10", stmt.toString());
        ...
        stmt.analyze(analyzer);
        Assert.assertEquals("SHOW EXPORT FROM `internal`.`testDb` WHERE `label` LIKE 'ab%' LIMIT 10", stmt.toString());
        ...
        stmt.analyze(analyzer);
        Assert.assertEquals("SHOW EXPORT FROM `internal`.`testDb` WHERE (`state` = 'PENDING') LIMIT 10", stmt.toString());
    }
