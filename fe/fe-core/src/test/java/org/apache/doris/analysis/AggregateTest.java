...
public class AggregateTest {
    ...
    public void testCountDisintctAnalysisException() throws Exception {
        ...
        do {
            String query =
                "select count(distinct empid), count(distinct salary), count(distinct empid, salary) from " + DB_NAME +
                    "." + TABLE_NAME;
            try {
    ...
    public void testWindowFunnelAnalysisException() throws Exception {
        ...
        do {
            ...
            try {
                ...
                Assert.assertTrue(
                    e.getMessage().contains("The 4th and subsequent params of window_funnel function must be boolean"));
                break;
            } catch (Exception e) {
                Assert.fail("must be AnalysisException.");
            }
