...
public class MetaServiceTest {
    ...
    public void testDumpAllowsAdmin() throws Exception {
        ...
        HttpServletRequest request = newRequest(FE_HOST, FE_HOST, META_TOKEN);
        HttpServletResponse response = Mockito.mock(HttpServletResponse.class);
        Mockito.doReturn(authInfo()).when(service).executeCheckPassword(request, response);
    ...
    // MetaService.dump() resolves the caller from the thread-local ConnectContext, so the
