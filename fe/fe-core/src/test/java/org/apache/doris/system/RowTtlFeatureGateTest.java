...
public class RowTtlFeatureGateTest {
    @Test
    public void testConstantQueryRemainsAvailableAfterActivation() throws Exception {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class)) {
            ...
            BackendDistributedPlanWorkerManager workerManager =
                    new BackendDistributedPlanWorkerManager(null, true, false);

            DistributedPlanWorker worker = workerManager.randomAvailableWorker(internalCatalog.getId());
            Assertions.assertEquals(-1L, worker.id());
            Assertions.assertTrue(worker.available());

            Mockito.when(env.isRowTtlActivated()).thenReturn(true);
            worker = workerManager.randomAvailableWorker(internalCatalog.getId());
            Assertions.assertEquals(-1L, worker.id());
            Assertions.assertTrue(worker.available());
        }
    }
    ...
    public void testMissingBackendCapabilityDoesNotActivateBarrier() throws Exception {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class)) {
            ...
            Assertions.assertTrue(exception.getMessage().contains("does not support Row TTL"));
            ...
            Assertions.assertThrows(DdlException.class, RowTtlFeatureGate::ensureRowBinlogTtlSupported);
    ...
    public void testPreActivationTableIsNotSilentlyAdopted() throws DdlException {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class)) {
            ...
            DdlException exception = Assertions.assertThrows(
                    DdlException.class, RowTtlFeatureGate::activateForMutation);
            Assertions.assertTrue(exception.getMessage().contains("cannot be silently adopted"));
            Mockito.verify(env, Mockito.never()).activateRowTtlMetaVersion();
        }
    }
    ...
    public void testPreActivationTableUseFailsClosed() {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class)) {
            ...
            Assertions.assertTrue(exception.getMessage().contains("activation barrier"));
        }
    }
    ...
    public void testActivatedClusterMarksOldHeartbeatIncompatibleImmediately() {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class)) {
            ...
            backend.setAlive(true);
            Assertions.assertTrue(backend.isNodeFeatureIncompatible());
            Assertions.assertFalse(backend.isQueryAvailable());
            Assertions.assertTrue(frontend.isNodeFeatureIncompatible());

            backend.handleHbResponse(capableBackendHeartbeat, true);
            Assertions.assertFalse(backend.isNodeFeatureIncompatible());
            Assertions.assertTrue(backend.isQueryAvailable());
            Assertions.assertTrue(backend.isScheduleAvailable());
            Assertions.assertTrue(backend.isLoadAvailable());
            backend.handleHbResponse(oldBackendHeartbeat, true);
            Assertions.assertTrue(backend.isNodeFeatureIncompatible());
            Assertions.assertTrue(backend.isAlive());
            Assertions.assertEquals(0, backend.getHeartbeatFailureCounter());
            Assertions.assertFalse(backend.isQueryAvailable());
            Assertions.assertFalse(backend.isScheduleAvailable());
            Assertions.assertFalse(backend.isLoadAvailable());

            frontend.handleHbResponse(capableFrontendHeartbeat, true);
            Assertions.assertFalse(frontend.isNodeFeatureIncompatible());
            frontend.handleHbResponse(oldFrontendHeartbeat, true);
            Assertions.assertTrue(frontend.isNodeFeatureIncompatible());
            Assertions.assertTrue(frontend.isAlive());
        }
    }
    ...
    public void testCloudMetaServiceCapabilityIsRequired() throws Exception {
        ...
        try (MockedStatic<Env> envStatic = Mockito.mockStatic(Env.class);
            ...
            Assertions.assertTrue(exception.getMessage().contains("Every active Meta Service"));
