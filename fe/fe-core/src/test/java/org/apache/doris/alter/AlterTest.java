...
public class AlterTest {
    ...
    public static void beforeClass() throws Exception {
        ...
        // remote storage
        createRemoteStorage("alter system add remote storage \"remote_s3\"\n" +
                "properties\n" +
                "(\n" +
                "    \"type\" = \"s3\",\n" +
                "    \"s3_endpoint\" = \"bj\",\n" +
    ...
    private static void createRemoteStorage(String sql) throws Exception {
        AlterSystemStmt stmt = (AlterSystemStmt) UtFrameUtils.parseAndAnalyzeStmt(sql, connectContext);
        Catalog.getCurrentCatalog().alterCluster(stmt);
    }
    ...
    public void testBatchUpdatePartitionProperties() throws Exception {
        ...
        DataProperty oldDataProperty = new DataProperty(TStorageMedium.SSD, coolDownTimeMs, "", TStorageMedium.HDD);
        ...
        alterTable(stmt, false);
        DataProperty newDataProperty = new DataProperty(TStorageMedium.HDD, DataProperty.MAX_COOLDOWN_TIME_MS, "", TStorageMedium.HDD);
        for (Partition partition : partitionList) {
        ...
        DataProperty newDataProperty1 = new DataProperty(TStorageMedium.SSD, coolDownTimeMs, "", TStorageMedium.HDD);
    ...
    public void testAlterRemoteStorageTableDataProperties() throws Exception {
        Database db = Catalog.getCurrentCatalog().getDbOrMetaException("default_cluster:test");
        OlapTable tbl_remote = (OlapTable) db.getTableOrMetaException("tbl_remote");
        Partition p1 = tbl_remote.getPartition("p1");
        Partition p2 = tbl_remote.getPartition("p2");
        Partition p3 = tbl_remote.getPartition("p3");
        Partition p4 = tbl_remote.getPartition("p4");

        // alter storage_medium
        String stmt = "alter table test.tbl_remote modify partition (p2, p3, p4) set ('storage_medium' = 'HDD')";
        DateLiteral dateLiteral = new DateLiteral("2122-04-01 20:24:00", Type.DATETIME);
        long coolDownTimeMs = dateLiteral.unixTimestamp(TimeUtils.getTimeZone());
        DataProperty oldDataProperty = new DataProperty(TStorageMedium.SSD, coolDownTimeMs, "remote_s3", TStorageMedium.S3);
        List<Partition> partitionList = Lists.newArrayList(p2, p3, p4);
        for (Partition partition : partitionList) {
            Assert.assertEquals(oldDataProperty, tbl_remote.getPartitionInfo().getDataProperty(partition.getId()));
        }
        alterTable(stmt, false);
        DataProperty dataProperty1 = new DataProperty(TStorageMedium.HDD, coolDownTimeMs, "remote_s3", TStorageMedium.S3);
        for (Partition partition : partitionList) {
            Assert.assertEquals(dataProperty1, tbl_remote.getPartitionInfo().getDataProperty(partition.getId()));
        }
        Assert.assertEquals(oldDataProperty, tbl_remote.getPartitionInfo().getDataProperty(p1.getId()));

        // alter cooldown_time
        stmt = "alter table test.tbl_remote modify partition (p2, p3, p4) set ('storage_cooldown_time' = '2100-04-01 22:22:22')";
        alterTable(stmt, false);
        ...
        DataProperty dataProperty2 = new DataProperty(TStorageMedium.HDD, newCoolDownTimeMs, "remote_s3", TStorageMedium.S3);
        for (Partition partition : partitionList) {
            Assert.assertEquals(dataProperty2, tbl_remote.getPartitionInfo().getDataProperty(partition.getId()));
        }
        Assert.assertEquals(oldDataProperty, tbl_remote.getPartitionInfo().getDataProperty(p1.getId()));

        // alter remote_storage
        stmt = "alter table test.tbl_remote modify partition (p2, p3, p4) set ('remote_storage' = 'remote_s3_1')";
        alterTable(stmt, false);
        DataProperty dataProperty3 = new DataProperty(TStorageMedium.HDD, newCoolDownTimeMs, "remote_s3_1", TStorageMedium.S3);
        for (Partition partition : partitionList) {
            Assert.assertEquals(dataProperty3, tbl_remote.getPartitionInfo().getDataProperty(partition.getId()));
        }
        Assert.assertEquals(oldDataProperty, tbl_remote.getPartitionInfo().getDataProperty(p1.getId()));

        // alter storage_cold_medium
        stmt = "alter table test.tbl_remote modify partition (p2, p3, p4) set ('storage_cold_medium' = 'HDD', 'remote_storage' = '')";
        alterTable(stmt, false);
        DataProperty dataProperty4 = new DataProperty(TStorageMedium.HDD, DataProperty.MAX_COOLDOWN_TIME_MS, "", TStorageMedium.HDD);
        for (Partition partition : partitionList) {
            Assert.assertEquals(dataProperty4, tbl_remote.getPartitionInfo().getDataProperty(partition.getId()));
        }
        Assert.assertEquals(oldDataProperty, tbl_remote.getPartitionInfo().getDataProperty(p1.getId()));

        // alter recover to old state
        stmt = "alter table test.tbl_remote modify partition (p2, p3, p4) set ('storage_cold_medium' = 'S3', " +
                "'remote_storage' = 'remote_s3', " +
                "'storage_medium' = 'SSD', " +
                "'storage_cooldown_time' = '2122-04-01 20:24:00'" +
                ")";
