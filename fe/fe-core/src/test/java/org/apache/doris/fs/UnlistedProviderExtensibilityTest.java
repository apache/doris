...
public class UnlistedProviderExtensibilityTest {
    ...
    /** Legacy raw-SPI stub: only supports(Map)/create(Map), no typed hooks. */
    private static final class RawLegacyProvider implements FileSystemProvider<FileSystemProperties> {
        @Override
        public String name() {
            return "RAWLEGACY";
        }
        ...
        @Override
        public boolean supports(Map<String, String> properties) {
            return Boolean.parseBoolean(properties.getOrDefault("rawlegacy.marker", "false"));
        }
    ...
    @Test
    public void testLegacyRawSpiProviderIsConsultedButNotTypedBound() {
        // A provider implementing only the raw SPI (supports/create(Map)) must not be silently
        // invisible: typed binding rejects it with an actionable WARN (it inherits the throwing
        // default bind()), while the raw createFileSystem(Map) entry point still selects it.
        FileSystemPluginManager local = new FileSystemPluginManager();
        local.loadBuiltins();
        local.registerProvider(new RawLegacyProvider());
        Map<String, String> props = ImmutableMap.of("rawlegacy.marker", "true");
        Assertions.assertThrows(
                org.apache.doris.foundation.property.StoragePropertiesException.class,
                () -> local.bindPrimary(props));
        Assertions.assertTrue(local.bindAll(props).stream()
                .noneMatch(b -> "RAWLEGACY".equalsIgnoreCase(b.providerName())));
        // raw entry point still routes to it (create throws our sentinel, proving selection)
        Assertions.assertThrows(UnsupportedOperationException.class,
                () -> local.createFileSystem(new java.util.HashMap<>(props)));
    }
