...
public class CreateTableCommandTest extends TestWithFeService {
    ...
    public void testNormal() throws DdlException, ConfigException {
        ...
        Assertions.assertDoesNotThrow(() -> createTable("create table test.tbl8\n" + "(k1 varchar(40), k2 int, v1 int)\n"
                        + "unique key(k1, k2)\n"
                        + "partition by range(k2)\n" + "(partition p1 values less than(\"10\"))\n"
                        + "distributed by hash(k2) buckets 1\n" + "properties('replication_num' = '1',\n"
        ...
        Assertions.assertTrue(tbl8.getColumn(Column.SEQUENCE_COL).getAggregationType() == AggregateType.REPLACE);

        OlapTable tbl13 = (OlapTable) db.getTableOrDdlException("tbl13");
        Assertions.assertTrue(tbl13.getColumn(Column.SEQUENCE_COL).getAggregationType() == AggregateType.REPLACE);
