...
public class LineageInfoExtractorTest extends TestWithFeService {
    ...
    @Test
    public void testCteMultiConsumerLineage() throws Exception {
        String sql = "with base_orders as ("
                + " select o.o_orderkey as orderkey, o.o_orderstatus as order_status"
                + " from " + dbTable("orders") + " o"
                + " where o.o_orderstatus = 'F'"
                + " )"
                + " insert into " + dbTable("tgt_union_join")
                + " select c.c_name as customer_name, b1.order_status"
                + " from base_orders b1"
                + " join base_orders b2 on b1.orderkey = b2.orderkey"
                + " join " + dbTable("customer") + " c on b1.orderkey = c.c_custkey";
        LineageInfo lineageInfo = buildLineageInfo(sql);
        ...
        assertDirectLineageNotContainsQualifier(lineageInfo, "order_status", "base_orders");
    }

    @Test
    public void testNestedCteLineage() throws Exception {
        String sql = "with base_orders as ("
                + " select o.o_orderkey as orderkey, o.o_orderstatus as order_status"
                + " from " + dbTable("orders") + " o"
                + " ), filtered_orders as ("
                + " select orderkey, order_status"
                + " from base_orders"
                + " where orderkey > 0"
                + " )"
                + " insert into " + dbTable("tgt_union_conditional")
                + " select f.orderkey,"
                + " case when f.order_status = 'F'"
                + " then cast(1 as decimal(18,2))"
                + " else cast(0 as decimal(18,2))"
                + " end as final_price"
        ...
        assertDirectLineageNotContainsQualifier(lineageInfo, "orderkey", "base_orders");
        assertDirectLineageNotContainsQualifier(lineageInfo, "orderkey", "filtered_orders");
        assertDirectLineageNotContainsQualifier(lineageInfo, "final_price", "base_orders");
        assertDirectLineageNotContainsQualifier(lineageInfo, "final_price", "filtered_orders");
    }

    @Test
    public void testCteReferencedMultipleTimesLineage() throws Exception {
        String sql = "with base_orders as ("
                + " select o.o_orderkey as orderkey, o.o_custkey as custkey, o.o_orderstatus as order_status"
                + " from " + dbTable("orders") + " o"
                + " where o.o_orderstatus in ('F', 'O')"
                + " )"
                + " insert into " + dbTable("tgt_union_join")
                + " select c.c_name as customer_name, b.order_status"
                + " from base_orders b"
                + " join " + dbTable("customer") + " c on b.custkey = c.c_custkey"
                + " where b.orderkey in ("
                + "   select b2.orderkey from base_orders b2 where b2.orderkey > 0"
                + " )"
                + " and exists ("
                + "   select 1 from base_orders b3 where b3.custkey = b.custkey"
                + " )";
        LineageInfo lineageInfo = buildLineageInfo(sql);
    ...
    private void assertDirectLineageNotContainsQualifier(LineageInfo lineageInfo, String outputName,
                                                         String qualifier) {
        ...
        for (Map.Entry<SlotReference, SetMultimap<LineageInfo.DirectLineageType, Expression>> entry
            ...
            checked = true;
            for (Expression expression : entry.getValue().values()) {
                for (Object slot : expression.collectToList(SlotReference.class::isInstance)) {
                    SlotReference slotReference = (SlotReference) slot;
                    boolean matched = slotReference.getQualifier().stream()
                            .anyMatch(q -> q.equalsIgnoreCase(qualifier));
                    Assertions.assertFalse(matched,
                            "unexpected qualifier '" + qualifier + "' in direct lineage expression for output "
                                    + outputName + ": " + expression);
                }
            }
        }
        Assertions.assertTrue(checked, "output " + outputName + " should have direct lineage");
    }
