...
public class BackupJobTest {
    ...
    public void testSerialization() throws IOException, AnalysisException {
        ...
        tableRefs.add(
            new TableRef(new TableName(InternalCatalog.INTERNAL_CATALOG_NAME, UnitTestUtil.DB_NAME, "unknown_tbl"),
                null));
        BackupJob backupJob1 = new BackupJob("label", dbId, UnitTestUtil.DB_NAME, tableRefs, 13600 * 1000, BackupStmt.BackupContent.ALL,
            env, repo.getId());

        backupJob1.write(out);
        out.flush();
        out.close();
        ...
        BackupJob backupJob2 = BackupJob.read(in);

        Assert.assertEquals(backupJob1.getJobId(), backupJob2.getJobId());
