...
public class FillUpMissingSlotsTest extends AnalyzeCheckTestBase implements MemoPatternMatchSupported {
    ...
    public void testHavingGroupBySlot() {
        ...
        Alias sumA2 = new Alias(new ExprId(3), new Sum(a2), "sum(default_cluster:test_resolve_aggregate_functions.t1.a2)");
        PlanChecker.from(connectContext).analyze(sql)
                .applyBottomUp(new ExpressionRewrite(FunctionBinder.INSTANCE))
                .matches(
                        logicalProject(
    ...
    public void testHavingAggregateFunction() {
        ...
        sumA2 = new Alias(new ExprId(3), new Sum(a2), "sum(default_cluster:test_resolve_aggregate_functions.t1.a2)");
        PlanChecker.from(connectContext).analyze(sql)
                .matches(
                        logicalProject(
                                logicalFilter(
        ...
        Alias minPK = new Alias(new ExprId(4), new Min(pk), "min(default_cluster:test_resolve_aggregate_functions.t1.pk)");
        PlanChecker.from(connectContext).analyze(sql)
                .matches(
                        logicalProject(
                                logicalFilter(
        ...
        Alias sumA1A2 = new Alias(new ExprId(3), new Sum(new Add(a1, a2)), "sum((default_cluster:test_resolve_aggregate_functions.t1.a1 + default_cluster:test_resolve_aggregate_functions.t1.a2))");
        PlanChecker.from(connectContext).analyze(sql)
                .matches(
                        logicalProject(
                                logicalFilter(
        ...
        Alias sumA1A23 = new Alias(new ExprId(4), new Sum(new Add(new Add(a1, a2), new TinyIntLiteral((byte) 3))),
                "sum(((default_cluster:test_resolve_aggregate_functions.t1.a1 + default_cluster:test_resolve_aggregate_functions.t1.a2) + 3))");
        PlanChecker.from(connectContext).analyze(sql)
                .matches(
                        logicalProject(
                                logicalFilter(
    ...
    void testJoinWithHaving() {
        ...
        Alias sumA2 = new Alias(new ExprId(6), new Sum(a2), "sum(default_cluster:test_resolve_aggregate_functions.t1.a2)");
        Alias sumB1 = new Alias(new ExprId(7), new Sum(b1), "sum(default_cluster:test_resolve_aggregate_functions.t2.b1)");
        PlanChecker.from(connectContext).analyze(sql)
                .matches(
                        logicalProject(
                                logicalFilter(
    ...
    void testInvalidHaving() {
        ...
        ExceptionChecker.expectThrowsWithMsg(
                ...
                "Aggregate functions in having clause can't be nested:"
                        + " sum((cast(default_cluster:test_resolve_aggregate_functions.t1.a1 as DOUBLE)"
                        + " + avg(default_cluster:test_resolve_aggregate_functions.t1.a2))).",
                () -> PlanChecker.from(connectContext).analyze(
        ...
        ExceptionChecker.expectThrowsWithMsg(
                AnalysisException.class,
                "Aggregate functions in having clause can't be nested:"
                        + " sum((cast((default_cluster:test_resolve_aggregate_functions.t1.a1"
                        + " + default_cluster:test_resolve_aggregate_functions.t1.a2) as DOUBLE)"
                        + " + avg(default_cluster:test_resolve_aggregate_functions.t1.a2))).",
                () -> PlanChecker.from(connectContext).analyze(
    ...
    void testComplexQueryWithHaving() {
        ...
        SlotReference pk1 = new SlotReference(
                new ExprId(6), "(default_cluster:test_resolve_aggregate_functions.t1.pk + 1)", IntegerType.INSTANCE, true,
                ImmutableList.of()
        );
        SlotReference a1 = new SlotReference(
                new ExprId(1), "a1", TinyIntType.INSTANCE, true,
        ...
        Alias sumA1 = new Alias(new ExprId(9), new Sum(a1), "sum(default_cluster:test_resolve_aggregate_functions.t1.a1)");
        Alias countA1 = new Alias(new ExprId(13), new Count(a1), "count(default_cluster:test_resolve_aggregate_functions.t1.a1)");
        Alias countA11 = new Alias(new ExprId(10), new Add(countA1.toSlot(), Literal.of((byte) 1)), "(count(a1) + 1)");
        Alias sumA1A2 = new Alias(new ExprId(11), new Sum(new Add(a1, a2)), "sum((default_cluster:test_resolve_aggregate_functions.t1.a1 + default_cluster:test_resolve_aggregate_functions.t1.a2))");
