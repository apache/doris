...
class RequestPropertyDeriverTest {
    ...
    @Test
    void testEquivalentParentKeysDoNotMultiplyNdv() {
        SlotReference aggregateKey3 = new SlotReference(new ExprId(2), "col3",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference equivalentKey = new SlotReference(new ExprId(3), "equivalent_key",
                IntegerType.INSTANCE, true, ImmutableList.of());
        Statistics childStats = equivalentParentKeyStatistics(equivalentKey, 1000);

        Assertions.assertEquals(fullAggregateRequests(aggregateKey3),
                requestThreeKeyAggregateProperties(aggregateKey3, childStats, 3,
                        equivalentParentAggregateProperty(equivalentKey),
                        ImmutableList.of(aggregateKey1, aggregateKey2, aggregateKey3, equivalentKey)));
    }

    @Test
    void testSafeEquivalentParentRequestPreservesItsDimensions() {
        SlotReference aggregateKey3 = new SlotReference(new ExprId(2), "col3",
                IntegerType.INSTANCE, true, ImmutableList.of());
        SlotReference equivalentKey = new SlotReference(new ExprId(3), "equivalent_key",
                IntegerType.INSTANCE, true, ImmutableList.of());
        Statistics childStats = equivalentParentKeyStatistics(equivalentKey, 2000);

        List<List<PhysicalProperties>> requests = requestThreeKeyAggregateProperties(
                aggregateKey3, childStats, 3, equivalentParentAggregateProperty(equivalentKey),
                ImmutableList.of(aggregateKey1, aggregateKey2, aggregateKey3, equivalentKey));

        Assertions.assertEquals(2, requests.size());
        DistributionSpecHash parentHash = (DistributionSpecHash) requests.get(0).get(0).getDistributionSpec();
        Assertions.assertEquals(1,
                ShuffleKeyPruneUtils.getIndependentShuffleDimensions(parentHash).size());
    }

    private Statistics equivalentParentKeyStatistics(SlotReference equivalentKey, double ndv) {
        ...
        return new StatisticsBuilder()
                .setRowCount(rowCount)
                .putColumnStatistics(aggregateKey1, columnStatistic)
                .putColumnStatistics(aggregateKey2, columnStatistic)
                .putColumnStatistics(equivalentKey, columnStatistic)
                .build();
    }

    private PhysicalProperties equivalentParentAggregateProperty(SlotReference equivalentKey) {
        return PhysicalProperties.createHash(new DistributionSpecHash(
                ImmutableList.of(aggregateKey1.getExprId(), aggregateKey2.getExprId()),
                ShuffleType.REQUIRE, -1L, ImmutableSet.of(),
                ImmutableList.of(
                        ImmutableSet.of(aggregateKey1.getExprId(), equivalentKey.getExprId()),
                        ImmutableSet.of(aggregateKey2.getExprId(), equivalentKey.getExprId())),
                ImmutableMap.of(
                        aggregateKey1.getExprId(), 0,
                        aggregateKey2.getExprId(), 1,
                        equivalentKey.getExprId(), 1)));
    }
    ...
    private List<List<PhysicalProperties>> requestThreeKeyAggregateProperties(
        ...
        return requestThreeKeyAggregateProperties(aggregateKey3, childStats, beNumber,
                PhysicalProperties.createHash(
                        ImmutableList.of(aggregateKey1.getExprId(), aggregateKey2.getExprId()), ShuffleType.REQUIRE),
                ImmutableList.of(aggregateKey1, aggregateKey2, aggregateKey3));
    }

    private List<List<PhysicalProperties>> requestThreeKeyAggregateProperties(
            SlotReference aggregateKey3, Statistics childStats, int beNumber,
            PhysicalProperties parentProperty, List<SlotReference> childOutput) {
        ConnectContext testConnectContext = MemoTestUtils.createConnectContext();
        ...
        PhysicalHashAggregate<GroupPlan> aggregate = new PhysicalHashAggregate<>(
                ...
                new AggregateParam(AggPhase.GLOBAL, AggMode.BUFFER_TO_RESULT),
                true, logicalProperties, false,
                groupPlanWithOutput(childOutput.toArray(new SlotReference[0])));
        GroupExpression groupExpression = new GroupExpression(aggregate) {
            @Override
            public Statistics childStatistics(int idx) {
                return childStats;
        ...
        Mockito.when(jobContext.getRequiredProperties()).thenReturn(parentProperty);
        return new RequestPropertyDeriver(testConnectContext, jobContext)
                .getRequestChildrenPropertyList(groupExpression);
    }
