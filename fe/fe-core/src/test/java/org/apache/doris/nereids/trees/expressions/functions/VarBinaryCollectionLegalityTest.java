...
public class VarBinaryCollectionLegalityTest {
    ...
    public void testCollectSetRetainsConstantLimitCheck() {
        CollectSet function = new CollectSet(new IntegerLiteral(1), new SlotReference("limit", IntegerType.INSTANCE));
        AnalysisException error = Assertions.assertThrows(AnalysisException.class,
                function::checkLegalityBeforeTypeCoercion);
        Assertions.assertTrue(error.getMessage().contains("second parameter must be a constant"));
    }
}
