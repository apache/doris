...
/** Phase A5 golden tests freezing current fe-core MinioProperties behaviour. */
public class MinioPropertiesParityTest {

    private static Map<String, String> baseProps() {
        Map<String, String> props = new HashMap<>();
        props.put("minio.endpoint", "http://127.0.0.1:9000");
        props.put("minio.access_key", "myAk");
        props.put("minio.secret_key", "mySk");
        return props;
    }
    ...
    public void testIdentity() {
        ...
        Assertions.assertTrue(sp instanceof MinioProperties);
        Assertions.assertEquals(StorageProperties.Type.MINIO, sp.getType());
        Assertions.assertEquals("S3", sp.getStorageName());
        Assertions.assertEquals(ImmutableSet.of("s3"), ((MinioProperties) sp).schemas());
    }

    @Test
    public void testBackendMapExactWithDefaultRegion() {
        StorageProperties sp = StorageProperties.createPrimary(baseProps());
