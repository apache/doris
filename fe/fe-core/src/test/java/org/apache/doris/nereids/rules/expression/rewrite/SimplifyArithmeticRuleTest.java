...
public class SimplifyArithmeticRuleTest extends ExpressionRewriteTestHelper {
    ...
    public void testSimplifyArithmetic() {
        ...
        assertRewriteAfterTypeCoercion("IA + 2 - ((1 - IB) - (3 + IC))", "IA + IB + cast(IC as BIGINT) + 4");
    ...
    public void testSimplifyArithmeticComparison() {
        ...
        assertRewriteAfterTypeCoercion("IA + 1 - (IB * IC - 1) > 1", "cast(IA as bigint) - IB * IC > -1");
        assertRewriteAfterTypeCoercion("IA + 1 - (IB * IC - 1) > 1", "cast(IA as bigint) - IB * IC > -1");
        assertRewriteAfterTypeCoercion("IA + 1 - (IB * IC - 1) > 1", "cast(IA as bigint) - IB * IC > -1");
        assertRewriteAfterTypeCoercion("(IA - 1) + (IB + 1) - (IC - 1) > 1", "IA + IB - cast(IC as BIGINT) > 0");
        assertRewriteAfterTypeCoercion("(IA - 1) + (IB + 1) - ((IC - 1) - (ID + 1)) > 1", "IA + IB - cast(IC as BIGINT) + cast(ID as BIGINT) > -1");
        assertRewriteAfterTypeCoercion("(IA - 1) + (IB + 1) - ((IC - 1) - (ID * IE + 1)) > 1", "IA + IB - cast(IC as BIGINT) + ID * IE > -1");
        assertRewriteAfterTypeCoercion("IA + 1 - (IB - 1) > IC + 1 - 1 + ID", "IA - IB > cast(IC as bigint) + cast(ID as bigint) + -2");
        assertRewriteAfterTypeCoercion("IA + 1 - (IB - 1) > IC - 1", "IA - IB > cast(IC as bigint) - 3");
        assertRewriteAfterTypeCoercion("IA + 1 > IB - 1", "cast(IA as bigint) > cast(IB as bigint) - 2");
