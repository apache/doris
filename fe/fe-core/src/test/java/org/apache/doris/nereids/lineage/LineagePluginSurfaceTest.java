...
/**
 * Freezes the LINEAGE plugin API surface, so that changing it cannot happen without also deciding the
 * version consequence.
 *
 * <p><b>Why this exists.</b> This tree ships no lineage plugin at all, so nothing in-tree would fail when the surface moves under the third-party plugins that are this SPI's only consumer; it also sits inside fe-core, where ordinary refactoring reaches it easily. The plugin API version in
 * {@code <lineage.plugin.api.version>} is the contract that says which FE a given plugin may load into,
 ...
 * {@code src/test/resources/lineage-plugin-surface.txt}, and bump the major of {@code lineage.plugin.api.version} in
 ...
 */
public class LineagePluginSurfaceTest {

    private static final String BASELINE_RESOURCE = "/lineage-plugin-surface.txt";

    /** The types a lineage plugin implements or calls. Everything reachable on them is the contract. */
    ...
    public void pluginApiSurfaceMatchesRecordedBaseline() throws IOException {
        ...
        Assertions.assertTrue(missing.isEmpty() && added.isEmpty(),
                "The LINEAGE plugin API surface changed.\n"
                        ...
                        + BASELINE_RESOURCE + " must increment the major of <lineage.plugin.api.version>"
                        + " in fe/fe-core/pom.xml (and zero its minor).\n"
                        + "Full actual surface:\n" + String.join("\n", actual));
    }
    ...
    private static TreeSet<String> readBaseline() throws IOException {
        ...
        try (InputStream in = LineagePluginSurfaceTest.class.getResourceAsStream(BASELINE_RESOURCE)) {
