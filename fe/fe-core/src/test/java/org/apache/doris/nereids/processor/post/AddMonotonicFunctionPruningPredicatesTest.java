...
class AddMonotonicFunctionPruningPredicatesTest extends ExpressionRewriteTestHelper {
    ...
    @Test
    void addYearRangeForIcebergScans() {
        SlotReference slot = new SlotReference("dt", DateTimeV2Type.SYSTEM_DEFAULT, true);
        HMSExternalTable hmsIceberg = Mockito.mock(HMSExternalTable.class);
        Mockito.when(hmsIceberg.getDlaType()).thenReturn(DLAType.ICEBERG);
        for (ExternalTable table : ImmutableList.of(hmsIceberg, Mockito.mock(IcebergExternalTable.class))) {
            Expression original = yearEquals(slot);
            PhysicalFilter<PhysicalFileScan> filter = new PhysicalFilter<>(
                    ImmutableSet.of(original), logicalProperties(slot), fileScan(slot, table));
            assertYearRange(filter.accept(processor, cascadesContext), original, slot);
        }
    }

    @Test
    void doNotAddPredicatesForHudiOrSystemTables() {
        SlotReference slot = new SlotReference("dt", DateTimeV2Type.SYSTEM_DEFAULT, true);
        HMSExternalTable hudi = Mockito.mock(HMSExternalTable.class);
        Mockito.when(hudi.getDlaType()).thenReturn(DLAType.HUDI);
        Mockito.when(hudi.supportedHiveTopNLazyTable()).thenReturn(true);
        for (ExternalTable table : ImmutableList.of(hudi, Mockito.mock(IcebergSysExternalTable.class),
                Mockito.mock(ExternalTable.class))) {
            PhysicalFilter<PhysicalFileScan> filter = new PhysicalFilter<>(
                    ImmutableSet.of(yearEquals(slot)), logicalProperties(slot), fileScan(slot, table));
            Assertions.assertSame(filter, filter.accept(processor, cascadesContext));
        }
    }
    ...
    private PhysicalFileScan fileScan(SlotReference output, boolean supportsStoragePredicatePruning) {
        ...
        Mockito.when(table.supportedHiveTopNLazyTable()).thenReturn(supportsStoragePredicatePruning);
        return fileScan(output, table);
    }

    private PhysicalFileScan fileScan(SlotReference output, ExternalTable table) {
