...
import org.apache.doris.nereids.rules.rewrite.eageraggregation.legacy.EagerGroupBy;
...
class EagerGroupByTest implements MemoPatternMatchSupported {
    ...
    void singleSum() {
        ...
        PlanChecker.from(MemoTestUtils.createConnectContext(), agg)
                ...
                .matchesExploration(
                        logicalAggregate(
                                logicalJoin(
                                        logicalAggregate().when(cntAgg -> cntAgg.getOutputExprsSql().equals("id, sum(age) AS `sum0`")),
                                        logicalOlapScan()
                                )
                        ).when(newAgg -> newAgg.getGroupByExpressions().equals(((Aggregate) agg).getGroupByExpressions())
                                        && newAgg.getOutputExprsSql().equals("sum(sum0) AS `sum`"))
                );
    }
    ...
    void multiSum() {
        ...
        PlanChecker.from(MemoTestUtils.createConnectContext(), agg)
                ...
                .matchesExploration(
                    logicalAggregate(
                        logicalJoin(
                            logicalAggregate().when(cntAgg -> cntAgg.getOutputExprsSql().equals("id, sum(gender) AS `sum0`, sum(name) AS `sum1`, sum(age) AS `sum2`")),
                            logicalOlapScan()
                            )
                    ).when(newAgg ->
                        newAgg.getGroupByExpressions().equals(((Aggregate) agg).getGroupByExpressions())
                            && newAgg.getOutputExprsSql().equals("sum(sum0) AS `sum0`, sum(sum1) AS `sum1`, sum(sum2) AS `sum2`"))
                );
    }
}
