                ...
                .expectThrowsWithMsg(DdlException.class,
                        ...
                        () -> createTable(
                                "create table test.tb7(key1 int, key2 varchar(10)) distributed by hash(key1) \n"
                                        + "buckets 1 properties('replication_num' = '1', 'storage_medium' = 'ssd', 'medium_allocation_policy' = 'strict');"));
    ...
    public void testStorageMediumProperty() throws Exception {
        ...
        ExceptionChecker.expectThrowsNoException(
                () -> createTable("create table test.tbl_storage_medium_specified\n" + "(k1 date, k2 int)\n"
                        ...
                        + "distributed by hash(k2) buckets 1\n"
                        + "properties('replication_num' = '1', 'storage_medium' = 'HDD', 'medium_allocation_policy' = 'strict'); "));
        Database db = Env.getCurrentInternalCatalog().getDbOrDdlException("test");
        ...
        Assert.assertEquals(MediumAllocationPolicy.STRICT, table.getTableProperty().getMediumAllocationPolicy());
        ...
        Assert.assertEquals(MediumAllocationPolicy.ADAPTIVE, table2.getTableProperty().getMediumAllocationPolicy());
        ...
        ExceptionChecker.expectThrowsNoException(
                () -> createTable("create table test.tbl_explicitly_specified\n" + "(k1 int, k2 int)\n"
                        + "distributed by hash(k1) buckets 1\n"
                        + "properties('replication_num' = '1', 'storage_medium' = 'SSD', 'medium_allocation_policy' = 'adaptive'); "));
        ...
        Assert.assertEquals(MediumAllocationPolicy.ADAPTIVE, table3.getTableProperty().getMediumAllocationPolicy());
    }

    @Test
    public void testAlterPartWithMediumAllocationPolicy() throws DdlException {
        ...
        ExceptionChecker.expectThrowsNoException(
                () -> {
                    alterTableSync("alter table test.tbl_alter_allocation modify partition p1 set ('medium_allocation_policy' = 'strict')");
                });

        Database db = Env.getCurrentInternalCatalog().getDbOrDdlException("test");
        OlapTable tb = (OlapTable) db.getTableOrDdlException("tbl_alter_allocation");
        Partition p1 = tb.getPartition("p1");
        Assert.assertEquals(MediumAllocationPolicy.STRICT, tb.getPartitionInfo().getDataProperty(p1.getId()).getMediumAllocationPolicy());
    }
}
