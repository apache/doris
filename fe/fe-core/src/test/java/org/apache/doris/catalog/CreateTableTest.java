...
public class CreateTableTest extends TestWithFeService {
    ...
    public void testStorageMediumProperty() throws Exception {
        ...
        Assert.assertEquals(AllocationPolicy.STRICT, table.getTableProperty().getAllocationPolicy());
        ...
        Assert.assertEquals(AllocationPolicy.ADAPTIVE, table2.getTableProperty().getAllocationPolicy());
        ...
        Assert.assertEquals(AllocationPolicy.ADAPTIVE, table3.getTableProperty().getAllocationPolicy());
    }

    @Test
    public void testAlterPartWithAllocationPolicy() throws DdlException {
        ...
        ExceptionChecker.expectThrowsNoException(
                () -> {
                    alterTableSync("alter table test.tbl_alter_allocation modify partition p1 set ('allocation_policy' = 'strict')");
                });

        Database db = Env.getCurrentInternalCatalog().getDbOrDdlException("test");
        OlapTable tb = (OlapTable) db.getTableOrDdlException("tbl_alter_allocation");
        Partition p1 = tb.getPartition("p1");
        Assert.assertEquals(AllocationPolicy.STRICT, tb.getPartitionInfo().getDataProperty(p1.getId()).getAllocationPolicy());
    }
}
