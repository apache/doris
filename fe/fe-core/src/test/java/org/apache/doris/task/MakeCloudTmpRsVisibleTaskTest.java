...
public class MakeCloudTmpRsVisibleTaskTest {
    ...
    public void testToThriftIncludesLastActiveClusterInfo() {
        MakeCloudTmpRsVisibleTask task = new MakeCloudTmpRsVisibleTask(
                ...
                "cluster_b", List.of(300L), List.of(7L));

        TMakeCloudTmpRsVisibleRequest request = task.toThrift();

        Assertions.assertEquals("cluster_b", request.getLoadClusterId());
        Assertions.assertEquals(List.of(300L), request.getLastActiveTabletIds());
        Assertions.assertEquals(List.of(7L), request.getLastActiveEpochs());
    }
}
