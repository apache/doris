...
public class ConsistencyCheckerTest {
    ...
    public void testManualCheckRejectsWholeBatchAndPendingJobSkipsRowTtl() {
        ...
        try (MockedStatic<Env> mockedEnv = Mockito.mockStatic(Env.class)) {
            ...
            Assertions.assertTrue(exception.getMessage().contains(Long.toString(ROW_TTL_TABLET_ID)));

            Map<Long, CheckConsistencyJob> jobs = Deencapsulation.getField(checker, "jobs");
            Assertions.assertTrue(jobs.isEmpty());
            Assertions.assertFalse(new CheckConsistencyJob(ROW_TTL_TABLET_ID).sendTasks());
        }
    }
    ...
    public void testAutomaticSelectionSkipsRowTtlTable() {
        ...
        try (MockedStatic<Env> mockedEnv = Mockito.mockStatic(Env.class)) {
            ...
            Assertions.assertTrue(chosenTablets.isEmpty());
            Mockito.verify(rowTtlTable, Mockito.never()).getAllPartitions();
        }
    }

    private static TabletMeta tabletMeta(long tableId) {
        return new TabletMeta(DB_ID, tableId, PARTITION_ID, INDEX_ID, 1, TStorageMedium.HDD, false);
    }
}
