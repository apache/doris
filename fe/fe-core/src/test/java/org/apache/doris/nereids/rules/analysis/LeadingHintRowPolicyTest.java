...
public class LeadingHintRowPolicyTest extends TestWithFeService {
    ...
    protected void runBeforeAll() throws Exception {
        ...
        // Masks are asked for one column at a time, keyed by the lower-cased column name - that is the
        // shape the planner asks in and reads back, so the per-column method is stubbed.
        Mockito.doAnswer(invocation -> {
            String tbl = invocation.getArgument(3);
            String col = invocation.getArgument(4);
            if (!tbl.equalsIgnoreCase(MASKED_TABLE)) {
                return Optional.<DataMaskPolicy>empty();
            }
            String column = col.toLowerCase(Locale.ROOT);
            return Optional.of(new DataMaskPolicy() {
                @Override
                public String getMaskTypeDef() {
                    return String.format("concat(%s, '_****_', %s)", column, column);
                }

                @Override
                public String getPolicyIdent() {
                    return String.format("custom policy: concat(%s, '_****_', %s)", column, column);
                }
            });
