...
public class TableFunctionPlanTest {
    ...
    public void projectSourceColumnWhenPredicateCannotPushedDown() throws Exception {
        String sql =
            "desc verbose select a.k1, tmp1.e1 from db1.tbl1 a lateral view explode_split(k2, \",\") tmp1 as e1"
    ...
    public void invalidFunctionInLateralView() throws Exception {
        ...
        sql =
            "explain select a.k1 from db1.tbl1 a lateral view explode_split(k1=(select k1 from db1.tbl1), \",\") tmp1 as e1";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
    ...
    public void lateralViewColumnOfReduceTuple() throws Exception {
        String sql =
            "desc verbose select e1 from (select k2 as c1 from db1.tbl1) a lateral view explode_split(c1, \",\") tmp1 as e1 ";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
        Assert.assertTrue(explainString.contains("1:TABLE FUNCTION NODE"));
    ...
    public void aggInlineView() throws Exception {
        String sql =
            "desc verbose select e1 from (select k2 as c1 from db1.tbl1 group by c1) a lateral view explode_split(c1, \",\") tmp1 as e1 ";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
    ...
    public void testExplodeJsonArray() throws Exception {
        ...
        sql =
            "desc select k1, e1 from db1.tbl2 lateral view explode_json_array_string('[\"a\",\"b\",\"c\"]') tmp1 as e1 ";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
        System.out.println(explainString);
    ...
    */
    ...
    public void aggColumnInOuterQuery() throws Exception {
        String sql =
            "desc verbose select min(c1) from (select c1 from (select k1 as c1, min(k2) as c2 from db1.tbl1 group by c1) a "
    ...
    public void testLateralViewWithView() throws Exception {
        ...
        String createViewStr =
            "create view db1.v1 (k1,e1) as select k1,e1 from db1.table_for_view lateral view explode_split(k3,',') tmp as e1;";
    ...
    public void testLateralViewWithWhere() throws Exception {
        String sql =
            "select k1,e1 from db1.table_for_view lateral view explode_split(k3,',') tmp as e1 where k1 in (select k2 from db1.table_for_view);";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
    ...
    public void testLateralViewWithCTE() throws Exception {
        String sql =
            "with tmp as (select k1,e1 from db1.table_for_view lateral view explode_split(k3,',') tmp2 as e1) select * from tmp;";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
        Assert.assertTrue(
            explainString.contains("table function: explode_split(`default_cluster:db1`.`table_for_view`.`k3`, ',') "));
    }

    @Test
    public void testLateralViewWithCTEBug() throws Exception {
        String sql =
            "with tmp as (select * from db1.table_for_view where k2=1) select k1,e1 from tmp lateral view explode_split(k3,',') tmp2 as e1;";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(ctx, sql, true);
