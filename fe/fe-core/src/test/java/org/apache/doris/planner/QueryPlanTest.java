...
public class QueryPlanTest {
    ...
    public static void beforeClass() throws Exception {
        ...
        createTable("create external table test.odbc_mysql\n" +
                ...
                "\"odbc_type\" = \"mysql\"\n" +
                ");");
        createTable("create table test.tbl_runtime_exception (" +
                "`date` datetime NULL," +
                "`day` date NULL," +
                "`site_id` int(11) NULL )" +
                " ENGINE=OLAP " +
    ...
    public void testRuntimeException() throws Exception {
        connectContext.setDatabase("default_cluster:test");
        //valid date
        String sql = "select day from tbl_runtime_exception where day in ('2020-10-30')";
        String explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `day` IN ('2020-10-30 00:00:00')"));
        //valid date
        sql = "select day from tbl_runtime_exception where day in ('2020-10-30','2020-10-29')";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `day` IN ('2020-10-30 00:00:00', '2020-10-29 00:00:00')"));

        //valid datetime
        sql = "select day from tbl_runtime_exception where date in ('2020-10-30 12:12:30')";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `date` IN ('2020-10-30 12:12:30')"));
        //valid datetime
        sql = "select day from tbl_runtime_exception where date in ('2020-10-30')";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `date` IN ('2020-10-30 00:00:00')"));

        //int date
        sql = "select day from tbl_runtime_exception where day in (20201030)";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `day` IN (2.020103E7)"));
        //int datetime
        sql = "select day from tbl_runtime_exception where date in (20201030)";
        explainString = UtFrameUtils.getSQLPlanOrErrorMsg(connectContext, "EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("PREDICATES: `date` IN (2.020103E7)"));
    }
}
