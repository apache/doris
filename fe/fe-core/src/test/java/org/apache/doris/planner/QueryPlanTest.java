...
import org.apache.doris.qe.dict.DictKey;
import org.apache.doris.qe.dict.DictState;
import org.apache.doris.qe.dict.GlobalDictManger;
import org.apache.doris.qe.dict.IDict;
import org.apache.doris.qe.dict.StringDict;
...
public class QueryPlanTest extends TestWithFeService {
    ...
    protected void runBeforeAll() throws Exception {
        ...
        Deencapsulation.setField(connectContext.getSessionVariable(), "enableBucketShuffleJoin",
                false);
        ...
        createTable("create table test.tbl_int_date (" + "`date` datetime NULL,"
                + "`day` date NULL," + "`site_id` int(11) NULL )" + " ENGINE=OLAP "
                + "DUPLICATE KEY(`date`, `day`, `site_id`)"
                + "DISTRIBUTED BY HASH(`site_id`) BUCKETS 10 " + "PROPERTIES (\n"
                + "\"replication_num\" = \"1\",\n" + "\"in_memory\" = \"false\",\n"
                + "\"storage_format\" = \"V2\"\n" + ");");

        createView(
                "create view test.tbl_null_column_view AS SELECT *,NULL as add_column  FROM test.test1;");

        createView(
                "create view test.function_view AS SELECT query_id, client_ip, concat(user, db) as"
    ...
    public void testFunctionViewGroupingSet() throws Exception {
        String queryStr =
                "select query_id, client_ip, concat from test.function_view group by rollup("
                        + "query_id, client_ip, concat);";
        assertSQLPlanOrErrorMsgContains(queryStr,
                "repeat: repeat 3 lines [[], [0], [0, 1], [0, 1, 2, 3]]");
    }
    ...
    public void testBitmapInsertInto() throws Exception {
        ...
        Assert.assertTrue(
                UtFrameUtils.checkPlanResultContainsNode(explainString, 0, "OlapScanNode"));
        ...
        assertSQLPlanOrErrorMsgContains(
                "insert into test.bitmap_table select id, id from test.bitmap_table_2;",
    ...
    public void testBitmapQuery() throws Exception {
        ...
        assertSQLPlanOrErrorMsgContains("select count(id2) from test.bitmap_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select group_concat(id2) from test.bitmap_table;",
                "group_concat requires first parameter to be of type STRING: group_concat(`id2`)");

        assertSQLPlanOrErrorMsgContains("select sum(id2) from test.bitmap_table;",
                "sum requires a numeric parameter: sum(`id2`)");

        assertSQLPlanOrErrorMsgContains("select avg(id2) from test.bitmap_table;",
                "avg requires a numeric parameter: avg(`id2`)");

        assertSQLPlanOrErrorMsgContains("select max(id2) from test.bitmap_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select min(id2) from test.bitmap_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select count(*) from test.bitmap_table group by id2;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select count(*) from test.bitmap_table where id2 = 1;",
    ...
    public void testHLLTypeQuery() throws Exception {
        ...
        assertSQLPlanOrErrorMsgContains("select count(id2) from test.hll_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select group_concat(id2) from test.hll_table;",
                "group_concat requires first parameter to be of type STRING: group_concat(`id2`)");

        assertSQLPlanOrErrorMsgContains("select sum(id2) from test.hll_table;",
                "sum requires a numeric parameter: sum(`id2`)");

        assertSQLPlanOrErrorMsgContains("select avg(id2) from test.hll_table;",
                "avg requires a numeric parameter: avg(`id2`)");

        assertSQLPlanOrErrorMsgContains("select max(id2) from test.hll_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select min(id2) from test.hll_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select min(id2) from test.hll_table;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select count(*) from test.hll_table group by id2;",
                Type.OnlyMetricTypeErrorMsg);

        assertSQLPlanOrErrorMsgContains("select count(*) from test.hll_table where id2 = 1",
    ...
    public void testTypeCast() throws Exception {
        ...
        String sql =
                "select * from test.baseall a where k1 in (select k1 from test.bigtable b where k2 > 0 and k1 = 1);";
        ...
        assertSQLPlanOrErrorMsgContains("select cast(id2 as varchar) from test.hll_table;",
                "Invalid type cast of `id2` from HLL to VARCHAR(*)");
        assertSQLPlanOrErrorMsgContains("select cast(id2 as varchar) from test.bitmap_table;",
                "Invalid type cast of `id2` from BITMAP to VARCHAR(*)");
        // disable implicit cast hll/bitmap to string
        assertSQLPlanOrErrorMsgContains("select length(id2) from test.hll_table;",
                "No matching function with signature: length(hll)");
        assertSQLPlanOrErrorMsgContains("select length(id2) from test.bitmap_table;",
    ...
    public void testShowCreateDbQueryPlanWithSchemaSyntax() throws Exception {
        ...
        ShowCreateDbStmt showCreateSchemaStmt =
                (ShowCreateDbStmt) parseAndAnalyzeStmt(showCreateSchemaSql);
    ...
    public void testDateTypeEquality() throws Exception {
        ...
        Catalog.getCurrentCatalog().getLoadManager().createLoadJobV1FromStmt(loadStmt,
                EtlJobType.HADOOP, System.currentTimeMillis());
    }
    ...
    public void testJoinPredicateTransitivity() throws Exception {
        ...
        /*
         * TODO: commit on_clause and where_clause Cross-identification // test left join : left
         * table where binary predicate String sql = "select join1.id\n" + "from join1\n" +
         * "left join join2 on join1.id = join2.id\n" + "where join1.id > 1;"; String explainString
         * = getSQLPlanOrErrorMsg("explain " + sql);
         ...
         *
         * // test left join: left table where in predicate sql = "select join1.id\n" +
         * "from join1\n" + "left join join2 on join1.id = join2.id\n" + "where join1.id in (2);";
         * explainString = getSQLPlanOrErrorMsg("explain " + sql);
         ...
         *
         * // test left join: left table where between predicate sql = "select join1.id\n" +
         * "from join1\n" + "left join join2 on join1.id = join2.id\n" +
         * "where join1.id BETWEEN 1 AND 2;"; explainString = getSQLPlanOrErrorMsg("explain " +
         * sql); Assert.assertTrue(explainString.
         * contains("PREDICATES: `join1`.`id` >= 1, `join1`.`id` <= 2"));
         * Assert.assertTrue(explainString.
         * contains("PREDICATES: `join2`.`id` >= 1, `join2`.`id` <= 2"));
         *
         */
        ...
        /*
         * // test left join: right table where predicate. // If we eliminate outer join, we could
         * push predicate down to join1 and join2. // Currently, we push predicate to join1 and keep
         * join predicate for join2 sql = "select *\n from join1\n" +
         * "left join join2 on join1.id = join2.id\n" + "where join2.id > 1;"; explainString =
         * getSQLPlanOrErrorMsg("explain " + sql);
         * Assert.assertTrue(explainString.contains("PREDICATES: `join1`.`id` > 1"));
         ...
         * "where join1.id > 1;"; explainString = getSQLPlanOrErrorMsg("explain " + sql);
         * Assert.assertTrue(explainString.contains("PREDICATES: `join1`.`id` > 1"));
         * Assert.assertTrue(explainString.contains("PREDICATES: `join2`.`id` > 1"));
         */
        ...
        /*
         * // test inner join: right table where predicate, both push down left table and right
         * table sql = "select *\n from join1\n" + "join join2 on join1.id = join2.id\n" +
         * "where join2.id > 1;"; explainString = getSQLPlanOrErrorMsg("explain " + sql);
         * Assert.assertTrue(explainString.contains("PREDICATES: `join1`.`id` > 1"));
         * Assert.assertTrue(explainString.contains("PREDICATES: `join2`.`id` > 1"));
         */
        ...
        /*
         * // test anti join, left table where predicate, only push to left table sql =
         * "select join1.id\n" + "from join1\n" + "left anti join join2 on join1.id = join2.id\n" +
         * "where join1.id > 1;"; explainString = getSQLPlanOrErrorMsg("explain " + sql);
         * Assert.assertTrue(explainString.contains("PREDICATES: `join1`.`id` > 1"));
         * Assert.assertFalse(explainString.contains("PREDICATES: `join2`.`id` > 1"));
         *
         * // test semi join, left table where predicate, only push to left table sql =
         * "select join1.id\n" + "from join1\n" + "left semi join join2 on join1.id = join2.id\n" +
         * "where join1.id > 1;"; explainString = getSQLPlanOrErrorMsg("explain " + sql);
         * Assert.assertTrue(explainString.contains("PREDICATES: `join1`.`id` > 1"));
         * Assert.assertFalse(explainString.contains("PREDICATES: `join2`.`id` > 1"));
    ...
    public void testConvertCaseWhenToConstant() throws Exception {
        ...
        Assert.assertTrue(!StringUtils
                .containsIgnoreCase(getSQLPlanOrErrorMsg("explain " + caseWhenSql), "CASE WHEN"));
        ...
        String sql121 =
                "select case when false then 2 when substr(k7,2,1) then 3 else 0 end as col121 from"
                        + " test.baseall";
        ...
        String sql122 =
                "select case when substr(k7,2,1) then 2 when false then 3 else 0 end as col122"
                        + " from test.baseall";
        ...
        String sql124 =
                "select case when false then 1 when true then 2 when false then 3 else 'other' end as col124";
        ...
        String sql15 =
                "select case when case when substr(k7,2,1) then true else false end then 2 when false then 3"
        ...
        String sql212 =
                "select case 'a' when 1 then 'a' when 'a' then 'b' else 'other' end as col212;";
        ...
        String sql23 =
                "select case 'a' when 'b' then 'a' when substr(k7,2,1) then 2 when false then 3"
        ...
        String sql231 =
                "select case 'a' when substr(k7,2,1) then 2 when 1 then 'a' when false then 3 else 0 end"
        ...
        String sql232 =
                "select case k1 when substr(k7,2,1) then 2 when 1 then 'a' when false then 3 else 0 end"
        ...
        String sql31 =
                "select case cast(100 as float) when 1 then 'a' when 2 then 'b' else 'other' end as col31;";
        ...
        String sql51 =
                "select case when 132 then k7 else 'all' end as col51 from test.baseall group by col51";
        ...
        String sql52 =
                "select case when 2 < 1 then 'all' else k7 end as col52 from test.baseall group by col52";
        ...
        String sql53 =
                "select case when 2 < 1 then 'all' else k1 end as col53 from test.baseall group by col53";
        ...
        String sql54 =
                "select k2, case when 2 < 1 then 'all' else k1 end as col54, k7 from test.baseall"
        ...
        String sql55 =
                "select case when 2 < 1 then 'all' else cast(k1 as int) end as col55, k7 from"
    ...
    public void testDistinctPushDown() throws Exception {
        connectContext.setDatabase("default_cluster:test");
        String sql =
                "select distinct k1 from (select distinct k1 from test.pushdown_test) t where k1 > 1";
        String explainString = getSQLPlanOrErrorMsg("explain " + sql);
        Assert.assertTrue(explainString.contains("PLAN FRAGMENT"));
    }
    ...
    public void testConstInParitionPrune() throws Exception {
        ...
        String queryStr =
                "explain select * from (select 'aa' as kk1, sum(id) from test.join1 where dt = 9"
    ...
    public void testColocateJoin() throws Exception {
        ...
        String queryStr =
                "explain select * from test.colocate1 t1, test.colocate2 t2 where t1.k1 = t2.k1 and"
                        + " t1.k2 = t2.k2 and t1.k3 = t2.k3";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(ColocatePlanTest.COLOCATE_ENABLE));

        queryStr =
                "explain select * from test.colocate1 t1 join [shuffle] test.colocate2 t2 on t1.k1 = t2.k1 and t1.k2 = t2.k2";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        ...
        queryStr =
                "explain select * from test.colocate1 t1, test.colocate2 t2 where t1.k1 = t2.k2 and t1.k2 = t2.k1 and t1.k3 = t2.k3";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertFalse(explainString.contains(ColocatePlanTest.COLOCATE_ENABLE));
    ...
    public void testSelfColocateJoin() throws Exception {
        ...
        // single partition
        String queryStr =
                "explain select * from test.jointest t1, test.jointest t2 where t1.k1 = t2.k1";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(ColocatePlanTest.COLOCATE_ENABLE));

        // multi partition, should not be colocate
        queryStr =
                "explain select * from test.dynamic_partition t1, test.dynamic_partition t2 where t1.k1 = t2.k1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertFalse(explainString.contains(ColocatePlanTest.COLOCATE_ENABLE));
    }
    ...
    public void testBucketShuffleJoin() throws Exception {
        ...
        Database db =
                Catalog.getCurrentInternalCatalog().getDbOrMetaException("default_cluster:test");
        ...
        // single partition
        String queryStr =
                "explain select * from test.jointest t1, test.bucket_shuffle1 t2 where t1.k1 = t2.k1"
                        + " and t1.k1 = t2.k2";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains("BUCKET_SHFFULE"));
        Assert.assertTrue(
                explainString.contains("BUCKET_SHFFULE_HASH_PARTITIONED: `t1`.`k1`, `t1`.`k1`"));

        // not bucket shuffle join do not support different type
        queryStr =
                "explain select * from test.jointest t1, test.bucket_shuffle1 t2 where cast (t1.k1 as tinyint)"
        ...
        queryStr =
                "explain select * from test.jointest t1, test.bucket_shuffle2 t2 where t1.k1 = t2.k1"
                        + " and t1.k1 = t2.k2";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(!explainString.contains("BUCKET_SHFFULE"));
        ...
        queryStr =
                "explain select * from test.colocate1 t1, test.bucket_shuffle2 t2 where t1.k1 = t2.k1"
                        + " and t1.k1 = t2.k2";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(!explainString.contains("BUCKET_SHFFULE"));
        ...
        queryStr =
                "explain select * from test.jointest t1 join test.bucket_shuffle1 t2 on t1.k1 = t2.k1 and"
                        + " t1.k1 = t2.k2 join test.colocate1 t3 on t2.k1 = t3.k1 and t2.k2 = t3.k2";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(
                explainString.contains("BUCKET_SHFFULE_HASH_PARTITIONED: `t1`.`k1`, `t1`.`k1`"));
        Assert.assertTrue(
                explainString.contains("BUCKET_SHFFULE_HASH_PARTITIONED: `t3`.`k1`, `t3`.`k2`"));

        // support recurse of bucket shuffle because t4 join t2 and join column name is same as t2
        // distribute column name
        queryStr =
                "explain select * from test.jointest t1 join test.bucket_shuffle1 t2 on t1.k1 = t2.k1 and"
        ...
        Assert.assertTrue(
                explainString.contains("BUCKET_SHFFULE_HASH_PARTITIONED: `t1`.`k1`, `t1`.`k1`"));
        Assert.assertTrue(
                explainString.contains("BUCKET_SHFFULE_HASH_PARTITIONED: `t4`.`k1`, `t4`.`k1`"));

        // some column name in join expr t3 join t4 and t1 distribute column name, so should not be
        // bucket shuffle join
        queryStr =
                "explain select * from test.jointest t1 join test.bucket_shuffle1 t2 on t1.k1 = t2.k1 and t1.k1 ="
    ...
    public void testJoinWithMysqlTable() throws Exception {
        ...
        queryStr =
                "explain select * from jointest t1, mysql_table t2, mysql_table t3 where t1.k1 = t3.k1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
    ...
    public void testJoinWithOdbcTable() throws Exception {
        ...
        queryStr =
                "explain select * from jointest t1, odbc_mysql t2, odbc_mysql t3 where t1.k1 = t3.k1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertFalse(explainString.contains("INNER JOIN(PARTITIONED)"));
    ...
    public void testLimitOfExternalTable() throws Exception {
        ...
        // ODBC table (MySQL)
        String queryStr =
                "explain select * from odbc_mysql where k1 > 10 and abs(k1) > 10 limit 10";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
    ...
    public void testOdbcSink() throws Exception {
        ...
        Deencapsulation.setField(connectContext.getSessionVariable(), "enableOdbcTransaction",
                true);
    ...
    public void testPreferBroadcastJoin() throws Exception {
        connectContext.setDatabase("default_cluster:test");
        String queryStr =
                "explain select * from (select k2 from jointest group by k2)t2, jointest t1 where t1.k1 = t2.k2";
    ...
    public void testRuntimeFilterMode() throws Exception {
        ...
        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterMode",
                "REMOTE");
        explainString = getSQLPlanOrErrorMsg(queryStr);
        ...
        queryStr =
                "explain select * from jointest as a where k1 = (select count(1) from jointest as b"
                        + " where a.k1 = b.k1);";
        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterMode",
                "GLOBAL");
        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 15);
    ...
    public void testRuntimeFilterType() throws Exception {
        ...
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString
                .contains("runtime filters: RF000[in] <- `t1`.`k1`, RF001[bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString
                .contains("runtime filters: RF000[in] -> `t2`.`k1`, RF001[bloom] -> `t2`.`k1`"));
        ...
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString
                .contains("runtime filters: RF000[in] <- `t1`.`k1`, RF001[min_max] <- `t1`.`k1`"));
        Assert.assertTrue(explainString
                .contains("runtime filters: RF000[in] -> `t2`.`k1`, RF001[min_max] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 6);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] <- `t1`.`k1`, RF001[min_max] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] -> `t2`.`k1`, RF001[min_max] -> `t2`.`k1`"));
        ...
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(
                explainString.contains("runtime filters: RF000[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(
                explainString.contains("runtime filters: RF000[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 9);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] <- `t1`.`k1`, RF001[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] -> `t2`.`k1`, RF001[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 10);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] <- `t1`.`k1`, RF001[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] -> `t2`.`k1`, RF001[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 11);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] <- `t1`.`k1`, RF001[bloom] <- `t1`.`k1`, RF002[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] -> `t2`.`k1`, RF001[bloom] -> `t2`.`k1`, RF002[in_or_bloom] -> `t2`.`k1`"));


        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 12);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[min_max] <- `t1`.`k1`, RF001[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[min_max] -> `t2`.`k1`, RF001[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 13);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] <- `t1`.`k1`, RF001[min_max] <- `t1`.`k1`, RF002[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] -> `t2`.`k1`, RF001[min_max] -> `t2`.`k1`, RF002[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 14);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] <- `t1`.`k1`, RF001[min_max] <- `t1`.`k1`, RF002[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[bloom] -> `t2`.`k1`, RF001[min_max] -> `t2`.`k1`, RF002[in_or_bloom] -> `t2`.`k1`"));

        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType", 15);
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] <- `t1`.`k1`, RF001[bloom] <- `t1`.`k1`, RF002[min_max] <- `t1`.`k1`, RF003[in_or_bloom] <- `t1`.`k1`"));
        Assert.assertTrue(explainString.contains(
                "runtime filters: RF000[in] -> `t2`.`k1`, RF001[bloom] -> `t2`.`k1`, RF002[min_max] -> `t2`.`k1`, RF003[in_or_bloom] -> `t2`.`k1`"));

        // support merge in filter, and forbidden implicit conversion to bloom filter
        queryStr =
                "explain select * from jointest t2 join [shuffle] jointest t1 where t1.k1 = t2.k1";
        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterMode",
                "GLOBAL");
        Deencapsulation.setField(connectContext.getSessionVariable(), "runtimeFilterType",
                TRuntimeFilterType.IN.getValue());
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains("runtime filters: RF000[in] -> `t2`.`k1`"));
        Assert.assertTrue(explainString.contains("runtime filters: RF000[in] <- `t1`.`k1`"));
    ...
    public void testEmptyNode() throws Exception {
        ...
        sqls.add(
                "explain select rank from (select k3, dense_rank() OVER () AS rank FROM baseall) a limit 0;");
        ...
        sqls.add(
                "explain select rank from (select k3, dense_rank() OVER () AS rank FROM baseall) a where 1 =2;");
    ...
    public void testLeadAndLagFunction() throws Exception {
        ...
        String queryStr =
                "explain select time_col, lead(query_time, 1, NULL) over () as time2 from test.test1";
        String explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains("lead(`query_time`, 1, NULL)"));

        queryStr =
                "explain select time_col, lead(query_time, 1, 2) over () as time2 from test.test1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains("lead(`query_time`, 1, 2)"));

        queryStr =
                "explain select time_col, lead(time_col, 1, '2020-01-01 00:00:00') over () as time2 from test.test1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
        Assert.assertTrue(explainString.contains("lead(`time_col`, 1, '2020-01-01 00:00:00')"));

        queryStr =
                "explain select time_col, lag(query_time, 1, 2) over () as time2 from test.test1";
        explainString = getSQLPlanOrErrorMsg(queryStr);
    ...
    public void testIntDateTime() throws Exception {
        ...
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("PREDICATES: `day` IN ('2020-10-30 00:00:00', '2020-10-29 00:00:00')"));
    ...
    public void testOutJoinSmapReplace() throws Exception {
        ...
        // valid date
        String sql =
                "SELECT a.aid, b.bid FROM (SELECT 3 AS aid) a right outer JOIN (SELECT 4 AS bid) b ON (a.aid=b.bid)";
        String explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains("OUTPUT EXPRS:\n    `a`.`aid`\n    4"));
    ...
    public void testFromUnixTimeRewrite() throws Exception {
        ...
        String explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("PREDICATES: `query_time` <= 253402271999, `query_time` > 1614650488"));
        ...
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("PREDICATES: `query_time` <= 253402271999, `query_time` > 1614614400"));
        ...
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(
                explainString.contains("PREDICATES: `query_time` < 1614614400, `query_time` >= 0"));
    ...
    public void testCheckInvalidDate() throws Exception {
        ...
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains(
                "Incorrect datetime value: '2020-10-32' in expression: `day` = '2020-10-32'"));

        // invalid date
        sql = "select day from tbl_int_date where day = '20201032'";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains(
                "Incorrect datetime value: '20201032' in expression: `day` = '20201032'"));
        // invalid date
        sql = "select day from tbl_int_date where day = 20201032";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: 20201032 in expression: `day` = 20201032"));
        // invalid date
        sql = "select day from tbl_int_date where day = 'hello'";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: 'hello' in expression: `day` = 'hello'"));
        // invalid date
        sql = "select day from tbl_int_date where day = 2020-10-30";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: 1980 in expression: `day` = 1980"));
        // invalid date
        sql = "select day from tbl_int_date where day = 10-30";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(
                explainString.contains("Incorrect datetime value: -20 in expression: `day` = -20"));
        // valid datetime
        ...
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains(
                "Incorrect datetime value: '2020-10-32' in expression: `date` = '2020-10-32'"));
        // invalid datetime
        sql = "select day from tbl_int_date where date = 'hello'";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: 'hello' in expression: `date` = 'hello'"));
        // invalid datetime
        sql = "select day from tbl_int_date where date = 2020-10-30";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: 1980 in expression: `date` = 1980"));
        // invalid datetime
        sql = "select day from tbl_int_date where date = 10-30";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString
                .contains("Incorrect datetime value: -20 in expression: `date` = -20"));
        // invalid datetime
        sql = "select day from tbl_int_date where date = '2020-10-12 12:23:76'";
        explainString = getSQLPlanOrErrorMsg("EXPLAIN " + sql);
        Assert.assertTrue(explainString.contains(
                "Incorrect datetime value: '2020-10-12 12:23:76' in expression: `date` = '2020-10-12 12:23:76'"));
    ...
    public void testNullColumnViewOrderBy() throws Exception {
        ...
        String sql2 =
                "select * from tbl_null_column_view where add_column is not null order by query_id;";
    ...
    public void testCompoundPredicateWriteRule() throws Exception {
        ...
        String sql9 =
                "select * from test.test1 where (-2=2 or query_time=2) and (-2=2 or stmt_id=2);";
    ...
    public void testOutfile() throws Exception {
        ...
        String sql =
                "select * from test.outfile1 where `date` between '2021-10-07' and '2021-10-11'"
    ...
    public void testEmptyNodeWithOuterJoinAndAnalyticFunction() throws Exception {
        ...
        createTable("CREATE TABLE issue7929.`t2` (\n" + "  `j1` int(11) NULL COMMENT \"\",\n"
                ...
                + "\"in_memory\" = \"false\",\n" + "\"storage_format\" = \"V2\"\n" + ")");
        String sql =
                "select * from issue7929.t1 left join (select max(j1) over() as x from issue7929.t2) a"
    ...
    public void testGroupingSets() throws Exception {
        ...
        createTable("CREATE TABLE issue7971.`t1` (\n" + "  `k1` tinyint(4) NULL COMMENT \"\",\n"
                ...
                + "\"in_memory\" = \"false\",\n" + "\"storage_format\" = \"V2\"\n" + ")");
        String sql =
                "SELECT k1, k2, GROUPING(k1), GROUPING(k2), SUM(k3) FROM issue7971.t GROUP BY GROUPING SETS ( (k1, k2), (k2), (k1), ( ) );";
    ...
    public void testQueryWithUsingClause() throws Exception {
        ...
        String qSQL =
                "explain  select t1.* from test.tbl_using_a t1 join test.tbl_using_b t2 using(k1,k2) where t1.k1 "
    ...
    public void testMetaQuery() throws Exception {
        ...
        explainString = getSQLPlanOrErrorMsg(errorSQL);
        Assert.assertTrue(explainString.contains(
                "Cannot impose the global_dict function on the SQL which have GROUP BY clause"));
        errorSQL = "SELECT global_dict(v1), count(*) FROM test.meta_scan_test [META]";
        explainString = getSQLPlanOrErrorMsg(errorSQL);
        Assert.assertTrue(explainString
                .contains("Mix the global_dict with other aggregate function is not permitted"));
        errorSQL = "SELECT global_dict(v1), global_dict(v2) FROM test.meta_scan_test [META]";
        explainString = getSQLPlanOrErrorMsg(errorSQL);
        Assert.assertTrue(
                explainString.contains("Multiple global_dict function is not supported for now"));
    ...
    public void testOutJoinWithOnFalse() throws Exception {
        ...
        String sql =
                "explain select * from out_join_1 left join out_join_2 on out_join_1.k1 = out_join_2.k1 and 1=2;";
        String explainString = getSQLPlanOrErrorMsg(sql);
    ...
    public void testInsertDecodeNode() throws Exception {
        ...
        GlobalDictManger dictManger = Catalog.getCurrentCatalog().getGlobalDictMgr();
        Field dictMapField = GlobalDictManger.class.getDeclaredField("dictsMap");
        dictMapField.setAccessible(true);
        Map<DictKey, IDict> dictsMap = (Map<DictKey, IDict>) dictMapField.get(dictManger);
        Database database = Catalog.getCurrentInternalCatalog().getDb("default_cluster:test").get();
        Table table = database.getTable("lineitem").get();
        DictKey dictKey = new DictKey(database.getId(), table.getId(), "l_shipmode");
        new Expectations(dict) {
            {
                dict.getDictId();
                result = 0;

                dict.getState();
                result = DictState.VALID;
            }
        };
        dictsMap.put(dictKey, dict);
        connectContext.getSessionVariable().setEnableVectorizedEngine(true);
        connectContext.getSessionVariable().setEnableLowCardinalityOpt(true);

        String sql = "SELECT l_shipmode, COUNT(l_shipmode) FROM lineitem GROUP BY  l_shipmode";
        String plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertTrue(plan.contains("Decode col: <4, 0>"));
        Assert.assertTrue(plan.contains("DICT COL: <3, 0>"));

        sql = "SELECT l_shipmode FROM lineitem GROUP BY  l_shipmode";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertTrue(plan.contains("Decode"));
        Assert.assertTrue(plan.contains("DICT COL: <2, 0>"));

        sql = "SELECT count(l_shipmode) FROM lineitem WHERE l_shipmode = 'AIR' GROUP BY l_shipmode";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode"));

        sql = "SELECT count(distinct l_shipmode) FROM lineitem WHERE l_orderkey  < 20 GROUP BY l_shipmode";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode"));
        Assert.assertTrue(plan.contains("DICT COL: <6, 0>"));

        sql = "SELECT l_shipmode, l_orderkey FROM lineitem GROUP BY l_shipmode, l_orderkey";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertTrue(plan.contains("Decode"));
        ...
        // join
        sql = "SELECT * FROM (SELECT l_shipmode m, COUNT(distinct l_shipdate) "
                + "FROM lineitem GROUP BY l_shipmode) t1, (select * from lineitem) t2 WHERE t1.m = t2.l_shipmode";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode"));

        // sub-query
        sql = "SELECT * FROM (SELECT DISTINCT l_shipmode FROM lineitem) t";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertTrue(plan.contains("Decode col: <4, 0>"));
        Assert.assertTrue(plan.contains("DICT COL: <3, 0>"));

        // rollup
        sql = "SELECT l_shipmode, l_shipdate FROM lineitem GROUP BY ROLLUP(l_shipmode, l_shipdate)";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode col"));
        Assert.assertFalse(plan.contains("DICT COL"));

        // cube
        sql = "SELECT l_shipmode, l_shipdate FROM lineitem GROUP BY CUBE(l_shipmode, l_shipdate)";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode col"));
        Assert.assertFalse(plan.contains("DICT COL"));

        // min max
        sql = "SELECT MAX(l_shipmode), MIN(l_shipdate) FROM lineitem GROUP BY l_shipmode";
        plan = getSQLPlanOrErrorMsg(sql);
        Assert.assertFalse(plan.contains("Decode col"));
        Assert.assertFalse(plan.contains("DICT COL"));
