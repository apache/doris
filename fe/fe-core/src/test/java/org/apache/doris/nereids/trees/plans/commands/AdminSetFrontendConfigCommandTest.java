...
public class AdminSetFrontendConfigCommandTest extends TestWithFeService {
    ...
    public void testNormal() throws Exception {
        ...
        Assertions.assertDoesNotThrow(() -> ((AdminSetFrontendConfigCommand) plan).run(connectContext, null));
    }

    @Test
    public void testUnknownConfig() throws Exception {
        String sql = "admin set frontend config(\"unknown_config\" = \"unknown\");";
        LogicalPlan plan = new NereidsParser().parseSingle(sql);
        Assertions.assertTrue(plan instanceof AdminSetFrontendConfigCommand);

        DdlException exception = Assertions.assertThrows(DdlException.class,
                () -> ((AdminSetFrontendConfigCommand) plan).run(connectContext, null));
