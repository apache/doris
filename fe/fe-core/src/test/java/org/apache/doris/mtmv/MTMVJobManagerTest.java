...
public class MTMVJobManagerTest {
    ...
    public void testRefreshMTMVPassesCurrentComputeGroupToTaskContext() throws Exception {
        ...
        try (MockedStatic<Env> mockedEnv = Mockito.mockStatic(Env.class)) {
            ...
            Assertions.assertEquals(MTMVTaskTriggerMode.MANUAL, taskContext.getTriggerMode());
            Assertions.assertEquals(Lists.newArrayList("p1"), taskContext.getPartitions());
            Assertions.assertFalse(taskContext.isComplete());
            Assertions.assertEquals("cg1", taskContext.getComputeGroup());
        } finally {
