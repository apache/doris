...
public final class TimeStampNsLiteral extends LiteralExpr {
    ...
    public long getUnixTimestampWithNanoseconds(TimeZone timeZone) {
        Instant instant = toLocalDateTime().atZone(timeZone.toZoneId()).toInstant();
        return Math.addExact(Math.multiplyExact(instant.getEpochSecond(), NANOSECONDS_PER_SECOND),
                instant.getNano());
    }
