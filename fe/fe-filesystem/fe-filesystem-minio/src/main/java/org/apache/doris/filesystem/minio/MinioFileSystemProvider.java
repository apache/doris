...
public class MinioFileSystemProvider implements FileSystemProvider<MinioFileSystemProperties> {
    ...
    private static final String[] MINIO_GUESS_ENDPOINT_NAMES = {
            "minio.endpoint", "s3.endpoint", "AWS_ENDPOINT", "endpoint", "ENDPOINT"};
    // Endpoint markers of the non-Azure clouds whose guessIsMe fe-core's
    // MinioProperties.guessIsMe consults for mutual exclusion (COS/OSS/S3, plus GCS which the
    // dialect split adds). Azure is NOT a static marker here: its claim is host-suffix based
    // and admin-extensible (Config.azure_blob_host_suffixes), so the exclusion goes through
    // the shared AzureBlobEndpointSignals predicate instead — see supportsGuess.
    ...
    public boolean supportsGuess(Map<String, String> properties) {
        ...
        // Azure exclusion, live-mirrored: legacy MinioProperties.guessIsMe consulted
        // AzureProperties.guessIsMe directly on the full map, so the exclusion here uses the
        // SAME shared predicate as AzureFileSystemProvider.supportsGuess — Azure's own endpoint
        // alias order, host extraction, dot-anchored suffixes, and the probe-injected
        // admin-extensible suffix list (_AZURE_HOST_SUFFIXES_). Whatever Azure claims, MinIO
        // yields, so bindAll can never produce both an AZURE and a MINIO binding.
        if (AzureBlobEndpointSignals.guessIsAzureBlobEndpoint(properties)) {
            return false;
        }
