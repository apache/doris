...
@Tag("environment")
@Tag("obs")
@EnabledIfEnvironmentVariable(named = "DORIS_FS_TEST_OBS_ENDPOINT", matches = ".+")
...
class ObsFileSystemEnvTest {
    ...
    void inputOutputRoundTrip() throws IOException {
        byte[] expected = "OBS round-trip 华为云".getBytes();
    ...
    void inputFileLength() throws IOException {
        ...
        Assertions.assertEquals(555L, fs.newInputFile(loc("len.bin")).length());
    }
    ...
    void getPresignedUrl_returnsValidUrlAndUploadWorks() throws IOException {
        ...
        byte[] payload = "obs-presigned-upload".getBytes(java.nio.charset.StandardCharsets.UTF_8);
