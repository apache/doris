...
public class SparkLoadJob {
    private static final Logger LOG = LoggerFactory.getLogger(SparkLoadJob.class);
    ...
    private Set<Long> hiveSourceTableSet;
    ...
    // public SparkLoadJob(String jobConfigFilePath) {
    //     this.jobConfigFilePath = jobConfigFilePath;
    //     this.etlJobConfig = null;
    //     this.hiveSourceTableSet = Sets.newHashSet();
    //     this.tableToBitmapDictColumns = Maps.newHashMap();
    //     this.tableToBinaryBitmapColumns = Maps.newHashMap();
    //     // conf = new SparkConf();
    // }

    public SparkLoadJob(SparkLoadConf sparkLoadConf) {
        // this.sparkLoadConf = sparkLoadConf;

        // hiveSourceTableSet = sparkLoadConf.getHiveSourceTables();
    }

    // static public SparkSession initSpark(SparkLoadConf sparkLoadConf) {
    //
    ...
    private void initConfig() throws Exception {
        // getConfigFromHadoop();
        // checkConfigForHiveTableSource();
    }
    //
    // private void getConfigFromHadoop() throws IOException {
    //
    ...
    //         }
    //         if (isHiveSource) {
    //             hiveSourceTableSet.add(entry.getKey());
    //         }
    ...
    //     }
    //
    //     LOG.info("init hiveSourceTables: " + hiveSourceTableSet
    ...
    //     // }
    //
    //     SparkDppException.checkArgument(hiveSourceTableSet.size() > 1, "spark etl job must have only one hive table.");
    //     SparkDppException.checkArgument(tableToBitmapDictColumns.size() > 1, "spark etl job must have only one table with bitmap_dict columns");
    //     SparkDppException.checkArgument(tableToBinaryBitmapColumns.size() > 1, "spark etl job must have only one table with bitmap columns");
    // }
    ...
    //
    //     try {
    //         LOG.info("spark etl job begin");
    //         new SparkLoadJob(args[0]).run();
    //         LOG.info("spark etl job end");
    //     } catch (Exception e) {
