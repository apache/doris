...
// do ETL job/sort/pre aggregate jobs in spark job
...
public final class SparkDpp implements java.io.Serializable {
    ...
    Map<Long, Set<String>> tableToBitmapUnionColumns = new HashMap<>();
    // just for ut
    public SparkDpp() {}

    public SparkDpp(SparkSession spark, EtlJobConfig etlJobConfig, Map<Long, Set<String>> tableToBitmapDictColumns, Map<Long, Set<String>> tableToBitmapUnionColumns) {
        this.spark = spark;
        this.etlJobConfig = etlJobConfig;
        if (tableToBitmapDictColumns != null) {
            this.tableToBitmapDictColumns = tableToBitmapDictColumns;
        }
        if(tableToBitmapUnionColumns != null){
            this.tableToBitmapUnionColumns = tableToBitmapUnionColumns;
        }
    }
    ...
    private Dataset<Row> loadDataFromHiveTable(SparkSession spark,
                                               ...
                                               Set<String> unionBitmapColumnsSet ) throws SparkDppException {
...
//         So in spark load, we first do step 1,3,4,and then do step 2.
        dataframe = checkDataFromHiveWithStrictMode(dataframe, baseIndex, fileGroup.columnMappings.keySet(), etlJobConfig.properties.strictMode,
                dstTableSchema, dictBitmapColumnSet,unionBitmapColumnsSet);
        dataframe = convertSrcDataframeToDstDataframe(baseIndex, dataframe, dstTableSchema, fileGroup);
        return dataframe;
    }
    ...
    private Dataset<Row> checkDataFromHiveWithStrictMode(
            ...
            Set<String> dictBitmapColumnSet,Set<String> unionBitmapColumnsSet) throws SparkDppException {
        ...
        for (EtlJobConfig.EtlColumn column : baseIndex.columns) {
            ...
            if (StringUtils.equalsIgnoreCase(column.columnType, "bitmap")) {
                ...
                if (unionBitmapColumnsSet.contains(column.columnName.toLowerCase())){
                    columnNameNeedCheckArrayList.add(column);
                    columnParserArrayList.add(new BinaryParser());
                }else {
                    columnNameNeedCheckArrayList.add(column);
        ...
        JavaRDD<Row> result = dataframe.toJavaRDD().flatMap(new FlatMapFunction<Row, Row>() {
            ...
            public Iterator<Row> call(Row row) throws Exception {
                ...
                for (int i = 0; i < columnNameArray.length; i++) {
                    ...
                    if (value != null && !columnParserArray[i].parse(value)){
                        if (isStrictMode) {
                            validRow = false;
    ...
    private void process() throws Exception {
        try {
            for (Map.Entry<Long, EtlJobConfig.EtlTable> entry : etlJobConfig.tables.entrySet()) {
                ...
                Set<String> unionBitmapColumnSet = tableToBitmapUnionColumns.getOrDefault(tableId,new HashSet<>());
                ...
                for (EtlJobConfig.EtlFileGroup fileGroup : etlTable.fileGroups) {
                    ...
                    if (sourceType == EtlJobConfig.SourceType.FILE) {
                        ...
                        fileGroupDataframe = loadDataFromHiveTable(spark, fileGroup.dppHiveDbTableName, baseIndex, fileGroup, dstTableSchema,
                                dictBitmapColumnSet,unionBitmapColumnSet);
                    } else {
