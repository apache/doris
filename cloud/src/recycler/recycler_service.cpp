...
void RecyclerServiceImpl::statistics_recycle(StatisticsRecycleRequest& req, MetaServiceCode& code,
    ...
    std::ranges::for_each(instance_ids, [&](const std::string& id) {
        ...
        int64_t tablet_bytes = g_bvar_recycler_instance_last_round_to_recycle_bytes.get(
                {"global_recycler", "recycle_tablet"});
        int64_t segment_num = g_bvar_recycler_instance_last_round_to_recycle_num.get(
                {"global_recycler", "recycle_segment"});
        int64_t segment_bytes = g_bvar_recycler_instance_last_round_to_recycle_bytes.get(
                {"global_recycler", "recycle_segment"});
        // clang-format off
        ...
        std::ranges::for_each(resource_types, [&](const auto& resource_type) {
            ...
            int64_t to_recycle_kv_num =
                    g_bvar_recycler_instance_last_round_to_recycle_kv_num.get({id, resource_type});
            int64_t to_recycle_kv_bytes =
                    g_bvar_recycler_instance_last_round_to_recycle_kv_bytes.get(
                            {id, resource_type});
            int64_t recycled_kv_num =
                    g_bvar_recycler_instance_last_round_recycled_kv_num.get({id, resource_type});
            int64_t recycled_kv_bytes =
                    g_bvar_recycler_instance_last_round_recycled_kv_bytes.get({id, resource_type});
