...
int InstanceRecycler::recycle_tablet(int64_t tablet_id, RecyclerMetricsContext& metrics_context) {
    ...
    for (const auto& rs_meta : resp.rowset_meta()) {
        ...
        */
        ...
        if (!rs_meta.has_resource_id()) {
            // Skip rowset metadata without resource ID and no segments
            // This rowset was generated by compaction with multiple hole rowsets
            if (rs_meta.num_segments() == 0) {
                LOG_INFO(
                        "rowset meta does not have a resource id and no segments, skip this rowset")
                        .tag("rs_meta", rs_meta.ShortDebugString())
                        .tag("instance_id", instance_id_)
                        .tag("tablet_id", tablet_id);
                recycle_rowsets_number += 1;
