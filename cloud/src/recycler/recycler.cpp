...
int InstanceRecycler::delete_delete_bitmap_kvs(int64_t partition_id, int64_t tablet_id,
    ...
    if (config::enable_recycler_delete_bitmap_range_remove) {
        ret = txn_remove(txn_kv_.get(), delete_bitmap_start, delete_bitmap_end);
        if (ret != 0) {
            LOG(WARNING) << "failed to delete delete bitmap kv by range, instance_id="
                         << instance_id_ << " tablet_id=" << tablet_id
                         << " rowset_id=" << rowset_id;
        }
        return ret;
