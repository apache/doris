...
//                                                      0:instance_id  1:backend_id
using StatsSpillKeyInfo    = BasicKeyInfo<__LINE__, std::tuple<std::string,  int64_t>>;
...
// 0x01 "stats" ${instance_id} "spill" ${backend_id} -> SpillStatsPB (one record per BE)
