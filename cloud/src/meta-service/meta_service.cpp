...
static std::pair<MetaServiceCode, std::string> check_row_ttl_meta_service_registry(TxnKv* txn_kv) {
...
void MetaServiceImpl::get_meta_service_capability(::google::protobuf::RpcController* controller,
    ...
    response->set_feature_flags(META_SERVICE_FEATURE_ROW_TTL);
}
...
void MetaServiceImpl::update_tablet_impl(::google::protobuf::RpcController* controller,
    ...
    for (const TabletMetaInfoPB& tablet_meta_info : request->tablet_meta_infos()) {
        ...
        if (tablet_meta_info.has_is_in_memory()) { // deprecate after 3.0.0
            ...
            tablet_meta.set_row_binlog_ttl_reference_tso(0);
        } else if (tablet_meta_info.has_row_binlog_ttl_reference_tso()) {
            if (tablet_meta_info.row_binlog_ttl_reference_tso() <= 0) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "row binlog TTL reference TSO must be positive";
                return;
            }
            if (tablet_meta_info.row_binlog_ttl_reference_tso() >
                tablet_meta.row_binlog_ttl_reference_tso()) {
                tablet_meta.set_row_binlog_ttl_reference_tso(
