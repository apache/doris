...
static void get_active_unversioned_tablet(MetaServiceCode& code, std::string& msg,
    ...
    if (persisted_tablet_meta.has_schema() && persisted_tablet_meta.schema().column_size() > 0) {
...
static void get_active_tablet_schema(MetaServiceCode& code, std::string& msg,
    ...
    doris::TabletMetaCloudPB tablet_meta;
    if (!is_versioned_read) {
        get_active_unversioned_tablet(code, msg, instance_id, txn, tablet_id, &tablet_meta);
        if (code != MetaServiceCode::OK) {
            return;
        }
...
void MetaServiceImpl::commit_rowset(::google::protobuf::RpcController* controller,
    ...
    if (!check_row_ttl_policy_compatible(rowset_schema, active_schema, &row_ttl_reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
