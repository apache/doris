...
static std::pair<MetaServiceCode, std::string> check_row_ttl_meta_service_registry(TxnKv* txn_kv) {
    ...
    const int64_t now_ms = duration_cast<milliseconds>(system_clock::now().time_since_epoch())
                                   .count();
...
static bool schema_contains_row_ttl(const doris::TabletSchemaCloudPB& schema) {
    ...
    return std::any_of(schema.column().begin(), schema.column().end(), [](const auto& column) {
        return column.name() == "__DORIS_TTL_COL__";
    });
}
...
void MetaServiceImpl::get_meta_service_capability(
        ::google::protobuf::RpcController* controller,
        const GetMetaServiceCapabilityRequest* request,
        GetMetaServiceCapabilityResponse* response, ::google::protobuf::Closure* done) {
    ...
    for (const auto& tablet_meta : request->tablet_metas()) {
        ...
        if (tablet_meta.has_schema() && tablet_meta.schema().column_size() > 0) {
            ...
            if (!try_fetch_and_parse_schema(
                        schema_txn.get(), &effective_schema, instance_id, tablet_meta.index_id(),
        ...
        operation_has_row_ttl =
                operation_has_row_ttl || schema_contains_row_ttl(effective_schema);
    }
...
static void get_active_tablet_schema(MetaServiceCode& code, std::string& msg,
    ...
    if (!is_versioned_read) {
        ...
        const std::string tablet_key = meta_tablet_key(
    ...
    if (schema->schema_version() != tablet_meta.schema_version()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format(
                "active tablet {} schema version mismatch: tablet_meta={}, schema={}", tablet_id,
                tablet_meta.schema_version(), schema->schema_version());
        return;
    }
...
void MetaServiceImpl::prepare_restore_job_impl(::google::protobuf::RpcController* controller,
    ...
    if (request->tablet_meta().has_schema() &&
        request->tablet_meta().schema().column_size() > 0) {
        ...
        if (!try_fetch_and_parse_schema(
                    txn0.get(), &incoming_schema, instance_id, tablet_idx.index_id(),
                    request->tablet_meta().schema_version(), code, msg, is_versioned_read,
                    resource_mgr_.get())) {
            return;
        }
    }
...
void MetaServiceImpl::commit_restore_job_impl(::google::protobuf::RpcController* controller,
    ...
    if (restore_job_pb.state() == RestoreJobCloudPB::COMMITTED) {
        ...
        const bool committed_target_has_row_ttl =
                schema_contains_row_ttl(committed_target_schema);
    ...
    auto add_incoming_schema = [&](const doris::TabletSchemaCloudPB& schema,
        ...
        auto normalized = externalize_variant_schema
                                  ? normalize_tablet_schema_for_schema_kv(schema)
                                  : schema;
    ...
    auto resolve_incoming_schema = [&](int32_t schema_version,
...
void MetaServiceImpl::commit_rowset_impl(::google::protobuf::RpcController* controller,
    ...
    if (rowset_meta.has_index_id() && rowset_meta.index_id() != tablet_idx.index_id()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format(
                "rowset index differs from active tablet index: rowset={}, tablet={}",
    ...
    const bool target_has_row_ttl = schema_contains_row_ttl(active_schema);
    const bool operation_has_row_ttl =
            target_has_row_ttl || schema_contains_row_ttl(rowset_schema);
    if (operation_has_row_ttl != row_ttl_rpc) {
        code = MetaServiceCode::INVALID_ARGUMENT;
