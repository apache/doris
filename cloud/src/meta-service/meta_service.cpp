...
static std::pair<MetaServiceCode, std::string> check_row_ttl_meta_service_registry(
        TxnKv* txn_kv, int64_t* common_features = nullptr) {
    std::unique_ptr<Transaction> txn;
    TxnErrorCode err = txn_kv->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
        return {cast_as<ErrCategory::CREATE>(err),
                fmt::format("failed to create row ttl capability check txn, err={}", err)};
    }

    std::string value;
    err = txn->get(system_meta_service_registry_key(), &value);
    if (err != TxnErrorCode::TXN_OK) {
        return {cast_as<ErrCategory::READ>(err),
                fmt::format("failed to read meta service registry for row ttl, err={}", err)};
    }
    ServiceRegistryPB registry;
    if (!registry.ParseFromString(value)) {
        return {MetaServiceCode::PROTOBUF_PARSE_ERR,
                "malformed meta service registry while checking row ttl capability"};
    }

    const int64_t now_ms =
            duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
    int live_service_count = 0;
    int64_t features = META_SERVICE_FEATURE_ROW_TTL | META_SERVICE_FEATURE_ROW_BINLOG_TTL;
    for (const auto& item : registry.items()) {
        if (item.expiration_time_ms() < now_ms) {
            continue;
        }
        ++live_service_count;
        features &= item.feature_flags();
        if ((item.feature_flags() & META_SERVICE_FEATURE_ROW_TTL) == 0) {
            return {MetaServiceCode::INVALID_ARGUMENT,
                    fmt::format("meta service {} does not support row ttl", item.id())};
        }
    }
    if (live_service_count == 0) {
        return {MetaServiceCode::INVALID_ARGUMENT,
                "meta service registry has no live row ttl capable instance"};
    }
    if (common_features) {
...
static bool schema_contains_row_ttl(const doris::TabletSchemaCloudPB& schema) {
    if (schema.has_ttl_col_idx() && schema.ttl_col_idx() != -1) {
        return true;
    }
    return std::any_of(schema.column().begin(), schema.column().end(),
                       [](const auto& column) { return column.name() == "__DORIS_TTL_COL__"; });
}
    ...
    if (is_versioned_read) {
        ...
        if (is_table_version) {
            ...
            if (request->wait_for_pending_txn() && partition_version.pending_txn_ids_size() > 0) {
                std::vector<VersionPB> pending_versions {partition_version};
                std::tie(code, msg) = wait_for_pending_txns(source_instance, pending_versions);
    ...
    if (err == TxnErrorCode::TXN_OK) {
        if (is_table_version) {
            ...
            if (request->wait_for_pending_txn() && version_pb.pending_txn_ids_size() > 0) {
                std::vector<VersionPB> pending_versions {version_pb};
                std::tie(code, msg) = wait_for_pending_txns(instance_id, pending_versions);
                if (code != MetaServiceCode::OK) {
                    return;
                }
...
std::pair<MetaServiceCode, std::string> MetaServiceImpl::batch_get_partition_versions(
    ...
    while (response->versions_size() < num_acquired) {
        ...
        for (size_t i = response->versions_size(); i < num_acquired; i += BATCH_SIZE) {
            ...
            std::unordered_map<int64_t, std::string> partition_source_instances;
            err = reader.get_partition_versions(acquired_ids, &partition_version_map,
                                                &versionstamps, true, &partition_source_instances);
            if (err == TxnErrorCode::TXN_TOO_OLD) {
                // txn too old, fallback to non-snapshot versions.
                LOG(WARNING) << "batch_get_version execution time exceeds the txn mvcc window, "
                                "fallback to acquire non-snapshot versions, partition_ids_size="
            ...
            std::vector<VersionPB> partition_versions;
            std::vector<std::string> version_source_instances;
            for (auto& acquired_id : acquired_ids) {
                ...
                if (it == partition_version_map.end()) {
                    ...
                    version_pb.set_update_time_ms(-1);
                    partition_versions.push_back(std::move(version_pb));
                    version_source_instances.emplace_back(instance_id);
                } else {
                    partition_versions.push_back(it->second);
                    DCHECK(partition_source_instances.contains(acquired_id));
                    version_source_instances.push_back(partition_source_instances.at(acquired_id));
                }
            }
            if (request->wait_for_pending_txn()) {
                std::unordered_map<std::string, std::vector<size_t>> pending_indexes;
                for (size_t j = 0; j < partition_versions.size(); ++j) {
                    if (!partition_versions[j].pending_txn_ids().empty()) {
                        pending_indexes[version_source_instances[j]].push_back(j);
                    }
                }
                for (const auto& [source_instance, indexes] : pending_indexes) {
                    std::vector<VersionPB> pending_versions;
                    pending_versions.reserve(indexes.size());
                    for (size_t index : indexes) {
                        pending_versions.push_back(partition_versions[index]);
                    }
                    auto&& [code, msg] = wait_for_pending_txns(source_instance, pending_versions);
                    if (code != MetaServiceCode::OK) {
                        return {code, msg};
                    }
                    for (size_t j = 0; j < indexes.size(); ++j) {
                        partition_versions[indexes[j]] = std::move(pending_versions[j]);
                    }
                }
            }
            for (auto& version_pb : partition_versions) {
    ...
    if (tablet_meta.has_schema()) {
        if (config::write_schema_kv) {
            ...
            if (success) { // schema may not be saved
                normalize_tablet_schema_column_types(tablet_meta.mutable_schema());
                auto schema_key =
...
void MetaServiceImpl::get_meta_service_capability(::google::protobuf::RpcController* controller,
                                                  const GetMetaServiceCapabilityRequest* request,
                                                  GetMetaServiceCapabilityResponse* response,
                                                  ::google::protobuf::Closure* done) {
    RPC_PREPROCESS(get_meta_service_capability);
    if (request->has_cloud_unique_id()) {
        instance_id = get_instance_id(resource_mgr_, request->cloud_unique_id());
        if (instance_id.empty()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
    ...
    RPC_RATE_LIMIT(get_meta_service_capability)
    int64_t features = 0;
    std::tie(code, msg) = check_row_ttl_meta_service_registry(txn_kv_.get(), &features);
    if (code != MetaServiceCode::OK) {
        return;
    }
    response->set_feature_flags(features);
}

void MetaServiceImpl::create_tablets_row_ttl(::google::protobuf::RpcController* controller,
                                             const CreateTabletsRequest* request,
                                             CreateTabletsResponse* response,
                                             ::google::protobuf::Closure* done) {
    create_tablets_impl(controller, request, response, done, true);
}
...
void MetaServiceImpl::create_tablets(::google::protobuf::RpcController* controller,
                                     ...
                                     CreateTabletsResponse* response,
                                     ::google::protobuf::Closure* done) {
    create_tablets_impl(controller, request, response, done, false);
}

void MetaServiceImpl::create_tablets_impl(::google::protobuf::RpcController* controller,
                                          const CreateTabletsRequest* request,
                                          CreateTabletsResponse* response,
    ...
    if (row_ttl_rpc) {
        std::tie(code, msg) = check_row_ttl_meta_service_registry(txn_kv_.get());
        if (code != MetaServiceCode::OK) {
            return;
        }
    }

    std::unique_ptr<Transaction> schema_txn;
    TxnErrorCode schema_txn_err = txn_kv_->create_txn(&schema_txn);
    if (schema_txn_err != TxnErrorCode::TXN_OK) {
        code = cast_as<ErrCategory::CREATE>(schema_txn_err);
        msg = "failed to create tablet schema validation txn";
        return;
    }
    const bool is_versioned_read = is_version_read_enabled(instance_id);
    bool operation_has_row_ttl = false;
    for (const auto& tablet_meta : request->tablet_metas()) {
        doris::TabletSchemaCloudPB effective_schema;
        if (tablet_meta.has_schema() && tablet_meta.schema().column_size() > 0) {
            effective_schema.CopyFrom(tablet_meta.schema());
            if (!effective_schema.has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "create tablet inline schema has no schema_version";
                return;
            }
            if (tablet_meta.has_schema_version() &&
                tablet_meta.schema_version() != effective_schema.schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format(
                        "create tablet schema version differs between tablet meta and inline "
                        "schema: tablet_meta={}, inline={}",
                        tablet_meta.schema_version(), effective_schema.schema_version());
                return;
            }
        } else if (tablet_meta.has_schema_version()) {
            if (!try_fetch_and_parse_schema(schema_txn.get(), &effective_schema, instance_id,
                                            tablet_meta.index_id(), tablet_meta.schema_version(),
                                            code, msg, is_versioned_read, resource_mgr_.get())) {
                return;
            }
        } else {
            if (!tablet_meta.has_schema() || !tablet_meta.schema().has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "create tablet has neither inline schema nor schema_version";
                return;
            }
            // Some legacy bootstrap paths use an explicitly empty schema carrying only
            // schema_version. Treat it as an inline non-TTL schema, not a detached reference.
            effective_schema.CopyFrom(tablet_meta.schema());
        }
        std::string reason;
        if (!check_tablet_schema_compatible(effective_schema, effective_schema, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("invalid create tablet schema: {}", reason);
            return;
        }
        operation_has_row_ttl = operation_has_row_ttl || schema_contains_row_ttl(effective_schema);
    }
    if (operation_has_row_ttl != row_ttl_rpc) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = operation_has_row_ttl ? "row ttl tablet creation requires create_tablets_row_ttl"
                                    : "create_tablets_row_ttl requires a row ttl tablet schema";
        return;
    }
    ...
    // Reject conflicting schemas in the request before internal_create_tablet commits any tablet.
    // The detached schema KV is shared by every tablet with the same index and schema version.
    if (config::write_schema_kv) {
        std::map<std::pair<int64_t, int32_t>, doris::TabletSchemaCloudPB> request_schemas;
        for (const auto& tablet_meta : request->tablet_metas()) {
            if (!tablet_meta.has_schema()) {
                continue;
            }
            doris::TabletSchemaCloudPB normalized_schema = tablet_meta.schema();
            normalize_tablet_schema_column_types(&normalized_schema);
            const auto key =
                    std::make_pair(tablet_meta.index_id(), normalized_schema.schema_version());
            auto it = request_schemas.try_emplace(key, normalized_schema).first;
            std::string reason;
            if (!check_tablet_schema_compatible(normalized_schema, it->second, &reason)) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format(
                        "create-tablets request contains incompatible schemas for index {} "
                        "version {}: {}",
                        key.first, key.second, reason);
                return;
            }
        }
    }
...
static void get_active_tablet_schema(MetaServiceCode& code, std::string& msg,
                                     const std::string& instance_id, Transaction* txn,
                                     int64_t tablet_id, bool is_versioned_read,
                                     ResourceManager* resource_mgr,
    ...
    if (!is_versioned_read) {
        ...
        const std::string tablet_key =
                meta_tablet_key({instance_id, tablet_idx.table_id(), tablet_idx.index_id(),
                                 tablet_idx.partition_id(), tablet_id});
        std::string persisted_tablet_value;
        TxnErrorCode err = txn->get(tablet_key, &persisted_tablet_value);
        if (err != TxnErrorCode::TXN_OK) {
            code = err == TxnErrorCode::TXN_KEY_NOT_FOUND ? MetaServiceCode::TABLET_NOT_FOUND
                                                          : cast_as<ErrCategory::READ>(err);
            msg = fmt::format("failed to read active tablet {}, err={}", tablet_id, err);
            return;
        }
        doris::TabletMetaCloudPB persisted_tablet_meta;
        if (!persisted_tablet_meta.ParseFromString(persisted_tablet_value)) {
            code = MetaServiceCode::PROTOBUF_PARSE_ERR;
            msg = fmt::format("malformed active tablet meta, tablet_id={}", tablet_id);
            return;
        }
        if (persisted_tablet_meta.has_schema() &&
            persisted_tablet_meta.schema().column_size() > 0) {
            if (!persisted_tablet_meta.schema().has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format("active tablet {} inline schema version is missing", tablet_id);
                return;
            }
            if (persisted_tablet_meta.has_schema_version() &&
                persisted_tablet_meta.schema_version() !=
                        persisted_tablet_meta.schema().schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format(
                        "active tablet {} inline schema version mismatch: tablet_meta={}, "
                        "schema={}",
                        tablet_id, persisted_tablet_meta.schema_version(),
                        persisted_tablet_meta.schema().schema_version());
                return;
            }
        } else if (!persisted_tablet_meta.has_schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("active tablet {} detached schema version is missing", tablet_id);
            return;
        }
        internal_get_tablet(code, msg, instance_id, txn, tablet_id, &tablet_meta, false);
        if (code != MetaServiceCode::OK) {
            return;
        }
        schema->CopyFrom(tablet_meta.schema());
    } else {
        CloneChainReader reader(instance_id, resource_mgr);
        TxnErrorCode err = reader.get_tablet_meta(txn, tablet_id, &tablet_meta, nullptr);
        if (err != TxnErrorCode::TXN_OK) {
            code = cast_as<ErrCategory::READ>(err);
            msg = fmt::format("failed to get active versioned tablet meta, tablet_id={}, err={}",
                              tablet_id, err);
            return;
        }
        if (tablet_meta.has_schema() && tablet_meta.schema().column_size() > 0) {
            if (!tablet_meta.schema().has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format("active tablet {} inline schema version is missing", tablet_id);
                return;
            }
            if (tablet_meta.has_schema_version() &&
                tablet_meta.schema_version() != tablet_meta.schema().schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format(
                        "active tablet {} inline schema version mismatch: tablet_meta={}, "
                        "schema={}",
            ...
            if (!tablet_meta.has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format("active tablet {} has neither schema nor schema version",
                                  tablet_id);
                return;
            }
            err = reader.get_tablet_schema(txn, tablet_meta.index_id(),
                                           tablet_meta.schema_version(), schema);
            if (err != TxnErrorCode::TXN_OK) {
                code = cast_as<ErrCategory::READ>(err);
                msg = fmt::format(
                        "failed to get active versioned tablet schema, tablet_id={}, "
                        "schema_version={}, err={}",
                        tablet_id, tablet_meta.schema_version(), err);
                return;
            }
        }
    }

    if (!schema->has_schema_version() || !tablet_meta.has_schema_version()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("active tablet {} schema version is missing", tablet_id);
        return;
    }
    if (schema->schema_version() != tablet_meta.schema_version()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("active tablet {} schema version mismatch: tablet_meta={}, schema={}",
                          tablet_id, tablet_meta.schema_version(), schema->schema_version());
        return;
    }

    std::string reason;
    if (!check_row_ttl_policy_compatible(*schema, *schema, &reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("active tablet {} has invalid row ttl policy: {}", tablet_id, reason);
    }
}
...
static bool try_fetch_and_parse_schema(Transaction* txn, TabletSchemaCloudPB* schema,
    ...
    if (!schema->has_schema_version()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("detached schema {} has no schema_version", schema_version);
        return false;
    }
    if (schema->schema_version() != schema_version) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("detached schema version mismatch: requested={}, actual={}",
...
void MetaServiceImpl::update_tablet_row_ttl(::google::protobuf::RpcController* controller,
                                            const UpdateTabletRequest* request,
                                            UpdateTabletResponse* response,
                                            ::google::protobuf::Closure* done) {
    update_tablet_impl(controller, request, response, done, true);
}
...
void MetaServiceImpl::update_tablet(::google::protobuf::RpcController* controller,
                                    ...
                                    UpdateTabletResponse* response,
                                    ::google::protobuf::Closure* done) {
    update_tablet_impl(controller, request, response, done, false);
}

void MetaServiceImpl::update_tablet_impl(::google::protobuf::RpcController* controller,
                                         const UpdateTabletRequest* request,
                                         UpdateTabletResponse* response,
                                         ::google::protobuf::Closure* done, bool row_ttl_rpc) {
    ...
    for (const TabletMetaInfoPB& tablet_meta_info : request->tablet_meta_infos()) {
        ...
        doris::TabletSchemaCloudPB active_schema;
        get_active_tablet_schema(code, msg, instance_id, txn.get(), tablet_meta_info.tablet_id(),
                                 is_versioned_read, resource_mgr_.get(), &active_schema);
        if (code != MetaServiceCode::OK) {
            return;
        }
        const bool target_has_row_ttl = schema_contains_row_ttl(active_schema);
        if (target_has_row_ttl != row_ttl_rpc) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = target_has_row_ttl ? "row ttl tablet update requires update_tablet_row_ttl"
                                     : "update_tablet_row_ttl requires a row ttl tablet";
            return;
        }
        ...
        } else if (tablet_meta_info.has_row_binlog_ttl_reference_tso()) {
            // A legacy FE may still send these while upgrading. Do not persist
            // a cleanup reference; new BEs accept it only through heartbeat.
            continue;
        }
        int64_t table_id = tablet_meta.table_id();
        int64_t index_id = tablet_meta.index_id();
 ...
 *                                 +--------------+
 */
void MetaServiceImpl::prepare_restore_job_row_ttl(::google::protobuf::RpcController* controller,
                                                  const RestoreJobRequest* request,
                                                  RestoreJobResponse* response,
                                                  ::google::protobuf::Closure* done) {
    prepare_restore_job_impl(controller, request, response, done, true);
}
...
void MetaServiceImpl::prepare_restore_job(::google::protobuf::RpcController* controller,
                                          ...
                                          RestoreJobResponse* response,
                                          ::google::protobuf::Closure* done) {
    prepare_restore_job_impl(controller, request, response, done, false);
}

void MetaServiceImpl::prepare_restore_job_impl(::google::protobuf::RpcController* controller,
                                               const RestoreJobRequest* request,
                                               RestoreJobResponse* response,
                                               ::google::protobuf::Closure* done,
                                               bool row_ttl_rpc) {
    ...
    const bool is_versioned_read = is_version_read_enabled(instance_id);
    doris::TabletSchemaCloudPB active_target_schema;
    get_active_tablet_schema(code, msg, instance_id, txn0.get(), request->tablet_id(),
                             is_versioned_read, resource_mgr_.get(), &active_target_schema);
    if (code != MetaServiceCode::OK) {
        return;
    }
    doris::TabletSchemaCloudPB incoming_schema;
    if (request->tablet_meta().has_schema() && request->tablet_meta().schema().column_size() > 0) {
        incoming_schema.CopyFrom(request->tablet_meta().schema());
        if (!incoming_schema.has_schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "restore tablet inline schema has no schema_version";
            return;
        }
        if (request->tablet_meta().has_schema_version() &&
            request->tablet_meta().schema_version() != incoming_schema.schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "restore tablet inline schema_version differs from tablet meta";
            return;
        }
    } else {
        if (!request->tablet_meta().has_schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "restore tablet has an empty schema without schema_version";
            return;
        }
        if (!try_fetch_and_parse_schema(txn0.get(), &incoming_schema, instance_id,
                                        tablet_idx.index_id(),
                                        request->tablet_meta().schema_version(), code, msg,
                                        is_versioned_read, resource_mgr_.get())) {
            return;
        }
    }
    std::string ttl_validation_reason;
    if (!check_row_ttl_policy_compatible(incoming_schema, incoming_schema,
                                         &ttl_validation_reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("invalid restore tablet row ttl policy: {}", ttl_validation_reason);
        return;
    }
    bool operation_has_row_ttl = schema_contains_row_ttl(active_target_schema) ||
                                 schema_contains_row_ttl(incoming_schema);
    std::unordered_map<int32_t, doris::TabletSchemaCloudPB> prepare_schemas;
    prepare_schemas.emplace(incoming_schema.schema_version(), incoming_schema);
    auto resolve_prepare_schema = [&](int32_t schema_version,
                                      doris::TabletSchemaCloudPB* schema) -> bool {
        auto it = prepare_schemas.find(schema_version);
        if (it != prepare_schemas.end()) {
            schema->CopyFrom(it->second);
            return true;
        }
        if (!try_fetch_and_parse_schema(txn0.get(), schema, instance_id, tablet_idx.index_id(),
                                        schema_version, code, msg, is_versioned_read,
                                        resource_mgr_.get())) {
            return false;
        }
        if (!check_row_ttl_policy_compatible(*schema, *schema, &ttl_validation_reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("invalid restore rowset row ttl policy: {}", ttl_validation_reason);
            return false;
        }
        prepare_schemas.emplace(schema_version, *schema);
        return true;
    };
    for (const auto& rowset_meta : request->tablet_meta().rs_metas()) {
        doris::TabletSchemaCloudPB rowset_schema;
        if (rowset_meta.has_tablet_schema() && rowset_meta.tablet_schema().column_size() > 0) {
            rowset_schema.CopyFrom(rowset_meta.tablet_schema());
            if (!rowset_schema.has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "restore rowset inline schema has no schema_version";
                return;
            }
            if (rowset_meta.has_schema_version() &&
                rowset_meta.schema_version() != rowset_schema.schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "restore rowset inline schema_version differs from rowset meta";
                return;
            }
            if (!check_row_ttl_policy_compatible(rowset_schema, rowset_schema,
                                                 &ttl_validation_reason)) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format("invalid inline restore rowset row ttl policy: {}",
                                  ttl_validation_reason);
                return;
            }
        } else if (rowset_meta.has_schema_version()) {
            if (!resolve_prepare_schema(rowset_meta.schema_version(), &rowset_schema)) {
                return;
            }
        } else if (rowset_meta.has_tablet_schema() &&
                   rowset_meta.tablet_schema().has_schema_version()) {
            if (!resolve_prepare_schema(rowset_meta.tablet_schema().schema_version(),
                                        &rowset_schema)) {
                return;
            }
        } else {
            rowset_schema.CopyFrom(incoming_schema);
        }
        operation_has_row_ttl = operation_has_row_ttl || schema_contains_row_ttl(rowset_schema);
    }
    if (operation_has_row_ttl != row_ttl_rpc) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = operation_has_row_ttl
                      ? "row ttl restore preparation requires prepare_restore_job_row_ttl"
                      : "prepare_restore_job_row_ttl requires a row ttl restore";
        return;
    }
...
void MetaServiceImpl::commit_restore_job_row_ttl(::google::protobuf::RpcController* controller,
                                                 const RestoreJobRequest* request,
                                                 RestoreJobResponse* response,
                                                 ::google::protobuf::Closure* done) {
    commit_restore_job_impl(controller, request, response, done, true);
}
...
void MetaServiceImpl::commit_restore_job(::google::protobuf::RpcController* controller,
                                         ...
                                         RestoreJobResponse* response,
                                         ::google::protobuf::Closure* done) {
    commit_restore_job_impl(controller, request, response, done, false);
}

void MetaServiceImpl::commit_restore_job_impl(::google::protobuf::RpcController* controller,
                                              const RestoreJobRequest* request,
                                              RestoreJobResponse* response,
                                              ::google::protobuf::Closure* done, bool row_ttl_rpc) {
    ...
    if (restore_job_pb.state() == RestoreJobCloudPB::COMMITTED) {
        doris::TabletSchemaCloudPB committed_target_schema;
        get_active_tablet_schema(code, msg, instance_id, txn0.get(), request->tablet_id(),
                                 is_version_read_enabled(instance_id), resource_mgr_.get(),
                                 &committed_target_schema);
        if (code != MetaServiceCode::OK) {
            return;
        }
        const bool committed_target_has_row_ttl = schema_contains_row_ttl(committed_target_schema);
        if (committed_target_has_row_ttl != row_ttl_rpc) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = committed_target_has_row_ttl
                          ? "row ttl restore commit requires commit_restore_job_row_ttl"
                          : "commit_restore_job_row_ttl requires a row ttl restore";
            return;
        }
    ...
    const bool is_versioned_write = is_version_write_enabled(instance_id);
    const bool is_versioned_read = is_version_read_enabled(instance_id);
    std::unordered_map<int32_t, doris::TabletSchemaCloudPB> incoming_schemas;
    auto add_incoming_schema = [&](const doris::TabletSchemaCloudPB& schema,
                                   std::string_view location,
                                   bool externalize_variant_schema) -> bool {
        std::string reason;
        if (!check_tablet_schema_compatible(schema, schema, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("invalid restore {} schema: {}", location, reason);
            return false;
        }
        auto normalized =
                externalize_variant_schema ? normalize_tablet_schema_for_schema_kv(schema) : schema;
        if (!check_tablet_schema_compatible(normalized, normalized, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("invalid normalized restore {} schema: {}", location, reason);
            return false;
        }
        auto it = incoming_schemas.find(normalized.schema_version());
        if (it == incoming_schemas.end()) {
            incoming_schemas.emplace(normalized.schema_version(), std::move(normalized));
        } else if (!check_tablet_schema_compatible(normalized, it->second, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format("restore schemas for version {} are incompatible: {}",
                              normalized.schema_version(), reason);
            return false;
        }
        return true;
    };

    auto resolve_incoming_schema = [&](int32_t schema_version, std::string_view location) -> bool {
        if (incoming_schemas.contains(schema_version)) {
            return true;
        }
        doris::TabletSchemaCloudPB schema;
        if (!try_fetch_and_parse_schema(txn0.get(), &schema, instance_id, tablet_idx.index_id(),
                                        schema_version, code, msg, is_versioned_read,
                                        resource_mgr_.get())) {
            msg = fmt::format("failed to resolve restore {} schema version {}: {}", location,
                              schema_version, msg);
            return false;
        }
        return add_incoming_schema(schema, location, false);
    };

    const auto& incoming_tablet = restore_job_pb.tablet_meta();
    int32_t canonical_schema_version = -1;
    if (incoming_tablet.has_schema() && incoming_tablet.schema().column_size() > 0) {
        doris::TabletSchemaCloudPB tablet_schema = incoming_tablet.schema();
        normalize_tablet_schema_column_types(&tablet_schema);
        if (!tablet_schema.has_schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "restore tablet inline schema has no schema_version";
            return;
        }
        if (incoming_tablet.has_schema_version() &&
            incoming_tablet.schema_version() != tablet_schema.schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format(
                    "restore tablet schema version differs between tablet meta and inline "
                    "schema: tablet_meta={}, inline={}",
                    incoming_tablet.schema_version(), tablet_schema.schema_version());
            return;
        }
        if (!add_incoming_schema(tablet_schema, "tablet", false)) {
            return;
        }
        canonical_schema_version = tablet_schema.schema_version();
    } else if (incoming_tablet.has_schema_version()) {
        canonical_schema_version = incoming_tablet.schema_version();
        if (!resolve_incoming_schema(canonical_schema_version, "tablet")) {
            return;
        }
    } else {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = "restore tablet schema is missing";
        return;
    }

    for (const auto& [_, rowset_meta] : restore_job_rs_metas) {
        if (rowset_meta.has_tablet_schema() && rowset_meta.tablet_schema().column_size() > 0) {
            if (!rowset_meta.tablet_schema().has_schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = "restore rowset inline schema has no schema_version";
                return;
            }
            if (rowset_meta.has_schema_version() &&
                rowset_meta.schema_version() != rowset_meta.tablet_schema().schema_version()) {
                code = MetaServiceCode::INVALID_ARGUMENT;
                msg = fmt::format(
                        "restore rowset schema version differs between rowset meta and inline "
                        "schema: rowset_meta={}, inline={}",
                        rowset_meta.schema_version(), rowset_meta.tablet_schema().schema_version());
                return;
            }
            if (!add_incoming_schema(rowset_meta.tablet_schema(), "rowset",
                                     rowset_meta.has_variant_type_in_schema() &&
                                             !rowset_meta.has_schema_dict_key_list())) {
                return;
            }
            continue;
        }
        if (rowset_meta.has_schema_version()) {
            if (!resolve_incoming_schema(rowset_meta.schema_version(), "rowset")) {
                return;
            }
            continue;
        }
        if (rowset_meta.has_tablet_schema() && rowset_meta.tablet_schema().has_schema_version()) {
            if (!resolve_incoming_schema(rowset_meta.tablet_schema().schema_version(), "rowset")) {
                return;
            }
            continue;
        }
        if (!incoming_schemas.contains(canonical_schema_version)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "restore rowset schema is missing and canonical tablet schema is unavailable";
            return;
        }
    }
    ...
    bool operation_has_row_ttl = schema_contains_row_ttl(active_target_schema);
    for (const auto& [_, schema] : incoming_schemas) {
        operation_has_row_ttl = operation_has_row_ttl || schema_contains_row_ttl(schema);
    }
    if (operation_has_row_ttl != row_ttl_rpc) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = operation_has_row_ttl ? "row ttl restore commit requires commit_restore_job_row_ttl"
                                    : "commit_restore_job_row_ttl requires a row ttl restore";
        return;
    }

    for (const auto& [schema_version, schema] : incoming_schemas) {
        std::string reason;
        if (!check_row_ttl_policy_compatible(schema, active_target_schema, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format(
                    "restore row ttl policy is incompatible with active target schema, "
                    "source_schema_version={}, target_schema_version={}, reason={}",
                    schema_version, active_target_schema.schema_version(), reason);
            return;
        }
    }

    if (config::write_schema_kv) {
        std::unique_ptr<Transaction> schema_check_txn;
        err = txn_kv_->create_txn(&schema_check_txn);
        if (err != TxnErrorCode::TXN_OK) {
            code = cast_as<ErrCategory::CREATE>(err);
            msg = "failed to init schema compatibility check txn";
            return;
        }
        for (const auto& [schema_version, schema] : incoming_schemas) {
            const std::string schema_key =
                    meta_schema_key({instance_id, tablet_idx.index_id(), schema_version});
            check_schema_kv(code, msg, schema_check_txn.get(), schema_key, schema);
            if (code != MetaServiceCode::OK) {
                return;
            }
            if (is_versioned_write) {
                const std::string versioned_schema_key = versioned::meta_schema_key(
                        {instance_id, tablet_idx.index_id(), schema_version});
                check_versioned_schema_kv(code, msg, schema_check_txn.get(), versioned_schema_key,
                                          schema);
                if (code != MetaServiceCode::OK) {
                    return;
                }
    ...
    TEST_SYNC_POINT("commit_restore_job::before_final_fence");
    std::string current_restore_job_value;
    err = txn0->get(key, &current_restore_job_value);
    if (err != TxnErrorCode::TXN_OK) {
        code = cast_as<ErrCategory::READ>(err);
        msg = fmt::format("failed to read restore job for final fence, tablet_id={}, err={}",
                          tablet_idx.tablet_id(), err);
        return;
    }
    RestoreJobCloudPB current_restore_job;
    if (!current_restore_job.ParseFromString(current_restore_job_value)) {
        code = MetaServiceCode::PROTOBUF_PARSE_ERR;
        msg = fmt::format("malformed restore job in final fence, tablet_id={}",
                          tablet_idx.tablet_id());
        return;
    }
    if (current_restore_job.state() != RestoreJobCloudPB::PREPARED ||
        current_restore_job.version() != restore_job_pb.version()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format(
                "restore job changed before final commit, tablet_id={}, expected_version={}, "
                "actual_version={}, actual_state={}",
                tablet_idx.tablet_id(), restore_job_pb.version(), current_restore_job.version(),
                RestoreJobCloudPB::State_Name(current_restore_job.state()));
        return;
    }

    doris::TabletSchemaCloudPB final_target_schema;
    get_active_tablet_schema(code, msg, instance_id, txn0.get(), request->tablet_id(),
                             is_versioned_read, resource_mgr_.get(), &final_target_schema);
    if (code != MetaServiceCode::OK) {
        return;
    }
    std::string target_policy_change_reason;
    if (!check_row_ttl_policy_compatible(final_target_schema, active_target_schema,
                                         &target_policy_change_reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format(
                "active target row ttl policy changed before final restore commit, reason={}",
                target_policy_change_reason);
        return;
    }
    for (const auto& [schema_version, schema] : incoming_schemas) {
        std::string reason;
        if (!check_row_ttl_policy_compatible(schema, final_target_schema, &reason)) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format(
                    "active target row ttl policy changed before final restore commit, "
                    "source_schema_version={}, target_schema_version={}, reason={}",
                    schema_version, final_target_schema.schema_version(), reason);
            return;
        }
    }
 ...
 */
void MetaServiceImpl::commit_rowset_row_ttl(::google::protobuf::RpcController* controller,
                                            const CreateRowsetRequest* request,
                                            CreateRowsetResponse* response,
                                            ::google::protobuf::Closure* done) {
    commit_rowset_impl(controller, request, response, done, true);
}
...
void MetaServiceImpl::commit_rowset(::google::protobuf::RpcController* controller,
                                    ...
                                    CreateRowsetResponse* response,
                                    ::google::protobuf::Closure* done) {
    commit_rowset_impl(controller, request, response, done, false);
}

void MetaServiceImpl::commit_rowset_impl(::google::protobuf::RpcController* controller,
                                         const CreateRowsetRequest* request,
                                         CreateRowsetResponse* response,
                                         ::google::protobuf::Closure* done, bool row_ttl_rpc) {
    ...
    const bool is_versioned_read = is_version_read_enabled(instance_id);
    doris::TabletSchemaCloudPB active_schema;
    get_active_tablet_schema(code, msg, instance_id, txn.get(), tablet_id, is_versioned_read,
                             resource_mgr_.get(), &active_schema);
    if (code != MetaServiceCode::OK) {
        return;
    }
    ...
    if (rowset_meta.has_index_id() && rowset_meta.index_id() != tablet_idx.index_id()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("rowset index differs from active tablet index: rowset={}, tablet={}",
                          rowset_meta.index_id(), tablet_idx.index_id());
        return;
    }
    ...
    if (rowset_meta.has_tablet_schema() && rowset_meta.tablet_schema().column_size() > 0) {
        ...
        if (!rowset_schema.has_schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "commit rowset inline schema has no schema_version";
            return;
        }
        if (rowset_meta.has_schema_version() &&
            rowset_meta.schema_version() != rowset_schema.schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = fmt::format(
                    "commit rowset schema version differs between rowset meta and inline "
                    "schema: rowset_meta={}, inline={}",
                    rowset_meta.schema_version(), rowset_schema.schema_version());
            return;
        }
        ...
        if (rowset_meta.has_tablet_schema() && rowset_meta.tablet_schema().has_schema_version() &&
            rowset_meta.has_schema_version() &&
            rowset_meta.tablet_schema().schema_version() != rowset_meta.schema_version()) {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "commit rowset empty schema_version differs from rowset meta";
            return;
        }
        int32_t rowset_schema_version = -1;
        if (rowset_meta.has_schema_version()) {
            rowset_schema_version = rowset_meta.schema_version();
        } else if (rowset_meta.has_tablet_schema() &&
                   rowset_meta.tablet_schema().has_schema_version()) {
            rowset_schema_version = rowset_meta.tablet_schema().schema_version();
        } else {
            code = MetaServiceCode::INVALID_ARGUMENT;
            msg = "commit rowset schema is missing";
            return;
        }
        if (active_schema.schema_version() == rowset_schema_version) {
            rowset_schema.CopyFrom(active_schema);
        } else {
            if (!try_fetch_and_parse_schema(txn.get(), &rowset_schema, instance_id,
                                            tablet_idx.index_id(), rowset_schema_version, code, msg,
                                            is_versioned_read, resource_mgr_.get())) {
                return;
            }
        }
    }
    std::string row_ttl_reason;
    if (!check_row_ttl_policy_compatible(rowset_schema, rowset_schema, &row_ttl_reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("invalid commit rowset row ttl policy: {}", row_ttl_reason);
        return;
    }
    const bool target_has_row_ttl = schema_contains_row_ttl(active_schema);
    const bool operation_has_row_ttl = target_has_row_ttl || schema_contains_row_ttl(rowset_schema);
    if (operation_has_row_ttl != row_ttl_rpc) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = operation_has_row_ttl ? "row ttl rowset commit requires commit_rowset_row_ttl"
                                    : "commit_rowset_row_ttl requires a row ttl tablet";
        return;
    }
    if (row_ttl_rpc &&
        !check_row_ttl_policy_compatible(rowset_schema, active_schema, &row_ttl_reason)) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = fmt::format("commit rowset row ttl policy is incompatible with active tablet: {}",
                          row_ttl_reason);
        return;
    }
...
std::pair<MetaServiceCode, std::string> MetaServiceImpl::wait_for_pending_txns(
    ...
    for (auto& version : versions) {
        ...
        version.set_version(version.has_version() ? version.version() + 1 : 2);
    }
