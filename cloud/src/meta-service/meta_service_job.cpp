...
void start_compaction_job(MetaServiceCode& code, std::string& msg, std::stringstream& ss,
    ...
    // Fail-open: if we cannot read the schema, allow compaction to proceed,
    // because compaction is critical for data health and should not be blocked
    // by transient read failures.
    {
        ...
        if (is_versioned_read) {
            ...
            if (err != TxnErrorCode::TXN_OK) {
                LOG_WARNING("failed to get tablet meta for disable_auto_compaction check")
                        .tag("tablet_id", tablet_id)
                        .tag("err", err);
            } else if (tablet_meta.has_schema() && tablet_meta.schema().column_size() > 0) {
                disable = tablet_meta.schema().disable_auto_compaction();
            } else if (tablet_meta.has_schema_version()) {
                doris::TabletSchemaCloudPB schema;
                err = reader.get_tablet_schema(txn.get(), index_id, tablet_meta.schema_version(),
                                               &schema);
                if (err != TxnErrorCode::TXN_OK) {
                    LOG_WARNING("failed to get tablet schema for disable_auto_compaction check")
                            .tag("tablet_id", tablet_id)
                            .tag("err", err);
                } else {
            ...
            if (err != TxnErrorCode::TXN_OK) {
                LOG_WARNING("failed to get tablet meta for disable_auto_compaction check")
                        .tag("tablet_id", tablet_id)
                        .tag("err", err);
            } else if (!tablet_meta.ParseFromString(tablet_val)) {
                LOG_WARNING("malformed tablet meta for disable_auto_compaction check")
                        .tag("tablet_id", tablet_id);
            } else if (tablet_meta.has_schema() && tablet_meta.schema().column_size() > 0) {
                disable = tablet_meta.schema().disable_auto_compaction();
            } else if (tablet_meta.has_schema_version()) {
                ...
                if (err != TxnErrorCode::TXN_OK) {
                    LOG_WARNING("failed to get schema for disable_auto_compaction check")
                            .tag("tablet_id", tablet_id)
                            .tag("err", err);
                } else {
                    doris::TabletSchemaCloudPB schema;
                    if (!val_buf.to_pb(&schema)) {
                        LOG_WARNING("malformed schema value for disable_auto_compaction check")
                                .tag("tablet_id", tablet_id);
                    } else {
                        disable = schema.disable_auto_compaction();
                    }
        ...
        if (disable) {
            // Use STALE_TABLET_CACHE so BE triggers an immediate sync
            // (sets last_sync_time_s=0), updating its in-memory cache.
            code = MetaServiceCode::STALE_TABLET_CACHE;
