...
void MetaServiceImpl::begin_txn(::google::protobuf::RpcController* controller,
    ...
    db_id = txn_info.has_db_id() ? txn_info.db_id() : -1;
    ...
    LOG_INFO("txn->get label_key={} label={} err=", hex(label_key), label, err);
    ...
    LOG_INFO("txn->atomic_set_ver_value label_key={}", hex(label_key));
    ...
    LOG_INFO("get_txn_id_from_fdb_ts() label={} txn_id={} label_val.size()={}", label, txn_id,
             label_val.size());
    ...
    if (label_val.size() > VERSION_STAMP_LEN) {
        ...
        for (auto it = label_pb.txn_ids().rbegin(); it != label_pb.txn_ids().rend(); ++it) {
            ...
            LOG_INFO(" size={} status={} txn_id={} label={}", label_pb.txn_ids().size(),
                     cur_txn_info.status(), txn_id, label);
            if (cur_txn_info.status() == TxnStatusPB::TXN_STATUS_ABORTED) {
                if (label_pb.txn_ids().size() >= config::max_num_aborted_txn) {
                    ...
                    msg = ss.str();
                    LOG_WARNING("{} label_pb={}", msg, label_pb.ShortDebugString());
                    return;
                }
                break;
            }
    ...
    txn->atomic_set_ver_value(label_key, label_val);
    LOG_INFO("txn->atomic_set_ver_value label_key={} label={} txn_id={}", hex(label_key), label,
             txn_id);
    ...
    LOG_INFO("xxx put index_key={} txn_id{}", hex(index_key), txn_id);
...
void MetaServiceImpl::precommit_txn(::google::protobuf::RpcController* controller,
    ...
    txn_id = request->has_txn_id() ? request->txn_id() : -1;
...
void process_mow_when_commit_txn(
    ...
    for (size_t i = 0; i < total_locks; i++) {
        ...
        LOG_INFO("xxx remove delete bitmap lock, lock_key={} table_id={} txn_id={}",
                 hex(lock_keys[i]), table_id, txn_id);

        for (auto tablet_id : table_id_tablet_ids[table_id]) {
            std::string pending_key = meta_pending_delete_bitmap_key({instance_id, tablet_id});
            txn->remove(pending_key);
            LOG_INFO("xxx remove delete bitmap pending key, pending_key={} txn_id={}",
                     hex(pending_key), txn_id);
        }
    }
    lock_keys.clear();
    lock_values.clear();
...
void MetaServiceImpl::commit_txn_immediately(
    ...
    do {
        ...
        if (last_pending_txn_id > 0) {
            ...
            if (code != MetaServiceCode::OK) {
                LOG_WARNING("advance_last_txn failed last_txn={}", last_pending_txn_id)
                        .tag("code", code)
                        .tag("msg", msg);
                return;
            }
        ...
        if (code != MetaServiceCode::OK) {
            LOG_WARNING("process mow failed, txn_id={}", txn_id).tag("code", code);
            return;
        }
        ...
        if ((txn_info.prepare_time() + txn_info.timeout_ms()) < commit_time) {
            ...
            LOG_INFO("{} prepare_time={} timeout_ms={} commit_time={}", msg,
                     txn_info.prepare_time(), txn_info.timeout_ms(), commit_time);
            return;
        }
    ...
    do {
        ...
        if (need_repair_tablet_idx) {
            ...
            repair_tablet_index(txn_kv_, code, msg, instance_id, db_id, txn_id, tmp_rowsets_meta,
                                is_versioned_write);
            if (code != MetaServiceCode::OK) {
                LOG_WARNING("repair_tablet_index failed, txn_id={} code={}", txn_id, code);
                return;
            }
            continue;
        }
        ...
        if (last_pending_txn_id > 0) {
            ...
            std::tie(code, msg) = task->wait();
            if (code != MetaServiceCode::OK) {
                LOG_WARNING("advance_last_txn failed last_txn={} code={} msg={}",
                            last_pending_txn_id, code, msg);
                return;
            }
        ...
        if (ret.first != MetaServiceCode::OK) {
            LOG_WARNING("txn lazy commit failed txn_id={} code={} msg={}", txn_id, ret.first,
                        ret.second);
        }
...
void MetaServiceImpl::commit_txn(::google::protobuf::RpcController* controller,
    ...
    if (instance_id.empty()) {
        code = MetaServiceCode::INVALID_ARGUMENT;
        msg = "empty instance_id";
        LOG_INFO("{}, cloud_unique_id={} txn_id={}", msg, cloud_unique_id, txn_id);
        return;
    }
    ...
    if (code != MetaServiceCode::OK) {
        LOG_WARNING("scan_tmp_rowset failed, txn_id={} code={}", txn_id, code);
        return;
    }
    ...
    while ((!enable_txn_lazy_commit_feature ||
        ...
        LOG_INFO("txn_id={} fallthrough commit_txn_eventually", txn_id);
        break;
    }

    LOG_INFO("txn_id={} commit_txn_eventually tmp_rowsets_meta.size={}", txn_id,
...
static void _abort_txn(const std::string& instance_id, const AbortTxnRequest* request,
    ...
    LOG_INFO("check watermark conflict, txn_info={}", return_txn_info.ShortDebugString());
    txn->put(info_key, info_val);
    LOG_INFO("xxx put info_key={} txn_id={}", hex(info_key), return_txn_info.txn_id());

    std::string running_key = txn_running_key({instance_id, db_id, return_txn_info.txn_id()});
    txn->remove(running_key);
    LOG_INFO("xxx remove running_key={} txn_id={}", hex(running_key), return_txn_info.txn_id());
    ...
    txn->put(recycle_key, recycle_val);
    LOG_INFO("put recycle_txn_key={} txn_id={}", hex(recycle_key), return_txn_info.txn_id());
}
...
void MetaServiceImpl::get_txn(::google::protobuf::RpcController* controller,
    ...
    if (!label.empty()) {
        ...
        LOG_INFO("txn->get label_key={}", hex(label_key)).tag("label", label).tag("err", err);
...
void MetaServiceImpl::begin_sub_txn(::google::protobuf::RpcController* controller,
                                    ...
                                    ::google::protobuf::Closure* done) {
    int64_t txn_id {-1}, sub_txn_num {-1}, db_id {-1};
    AnnotateTag tag_txn_id("txn_id", txn_id), tag_sub_txn_num("sub_txn_num", sub_txn_num),
            tag_db_id("db_id", db_id);
    RPC_PREPROCESS(begin_sub_txn, get, put, del);
    txn_id = request->has_txn_id() ? request->txn_id() : -1;
    sub_txn_num = request->has_sub_txn_num() ? request->sub_txn_num() : -1;
    db_id = request->has_db_id() ? request->db_id() : -1;
    ...
    LOG_INFO("get_txn_id_from_fdb_ts() label={} sub_txn_id={} txn_id={} label_val.size()={}", label,
             sub_txn_id, txn_id, label_val.size());
    ...
    txn->put(info_key, info_val);
    txn->put(index_key, index_val);
    LOG_INFO("txn_id={}, sub_txn_id={}, remove label_key={}, put info_key={}, put index_key={}",
             txn_id, sub_txn_id, hex(label_key), hex(info_key), hex(index_key));
    err = txn->commit();
    if (err != TxnErrorCode::TXN_OK) {
        code = cast_as<ErrCategory::COMMIT>(err);
        ss << "failed to commit kv txn, txn_id=" << txn_id << " err=" << err;
...
void MetaServiceImpl::abort_sub_txn(::google::protobuf::RpcController* controller,
                                    ...
                                    ::google::protobuf::Closure* done) {
    int64_t txn_id {-1}, sub_txn_id {-1}, sub_txn_num {-1}, db_id {-1};
    AnnotateTag tag_txn_id("txn_id", txn_id), tag_sub_txn_id("sub_txn_id", sub_txn_id),
            tag_sub_txn_num("sub_txn_num", sub_txn_num), tag_db_id("db_id", db_id);
    RPC_PREPROCESS(abort_sub_txn, get, put);
    txn_id = request->has_txn_id() ? request->txn_id() : -1;
    sub_txn_id = request->has_sub_txn_id() ? request->sub_txn_id() : -1;
    sub_txn_num = request->has_sub_txn_num() ? request->sub_txn_num() : -1;
    db_id = request->has_db_id() ? request->db_id() : -1;
    auto& table_ids = request->table_ids();
    ...
    txn->put(info_key, info_val);
    LOG_INFO("txn_id={}, sub_txn_id={}, put info_key={}", txn_id, sub_txn_id, hex(info_key));
    err = txn->commit();
    if (err != TxnErrorCode::TXN_OK) {
        code = cast_as<ErrCategory::COMMIT>(err);
...
void MetaServiceImpl::abort_txn_with_coordinator(::google::protobuf::RpcController* controller,
    ...
    LOG_INFO("begin_info_key:{} end_info_key:{}", hex(begin_info_key), hex(end_info_key));
    ...
    LOG_INFO("abort txn count: {} total iteration count: {}", abort_txn_cnt, total_iteration_cnt);
    if (need_commit) {
        err = txn->commit();
        if (err != TxnErrorCode::TXN_OK) {
            code = cast_as<ErrCategory::COMMIT>(err);
...
std::string get_txn_info_key_from_txn_running_key(std::string_view txn_running_key) {
    ...
    if (ret != 0) [[unlikely]] {
        ...
        LOG_WARNING("failed to decode key, ret={} key={}", ret, hex(txn_running_key));
    } else {
...
void MetaServiceImpl::check_txn_conflict(::google::protobuf::RpcController* controller,
                                         ...
                                         ::google::protobuf::Closure* done) {
    int64_t end_txn_id {-1}, db_id {-1};
    AnnotateTag tag_end_txn_id("end_txn_id", end_txn_id), tag_db_id("db_id", db_id);
    ...
    db_id = request->db_id();
    end_txn_id = request->end_txn_id();
    std::string begin_running_key = txn_running_key({instance_id, db_id, 0});
    std::string end_running_key = txn_running_key({instance_id, db_id, end_txn_id});
    LOG_INFO("begin_running_key:{} end_running_key:{}", hex(begin_running_key),
             hex(end_running_key));
    ...
    do {
        ...
        while (it->has_next()) {
            total_iteration_cnt++;
            auto [k, v] = it->next();
            LOG_INFO("check watermark conflict range_get txn_run_key={}", hex(k));
            TxnRunningPB running_pb;
            ...
            if (running_pb.timeout_time() < check_time) {
                ...
                LOG_INFO("check watermark conflict range_get txn_run_key={} {} running_pb={}",
                         hex(k), k, running_pb.ShortDebugString());
    ...
    LOG_INFO("skip timeout txn count: {} conflict txn count: {} total iteration count: {}",
             skip_timeout_txn_cnt, response->conflict_txns_size(), total_iteration_cnt);
...
TxnErrorCode internal_clean_label(std::shared_ptr<TxnKv> txn_kv, const std::string_view instance_id,
    ...
    TxnErrorCode err = txn_kv->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
        LOG_WARNING("failed to create txn. err={} db_id={} label_key={}", err, db_id,
                    hex(label_key));
        return err;
    }
    DORIS_CLOUD_DEFER {
    ...
    err = txn->get(label_key, &label_val);
    if (err != TxnErrorCode::TXN_OK && err != TxnErrorCode::TXN_KEY_NOT_FOUND) {
        LOG_WARNING("failed to txn get err={} db_id={} label_key={}", err, db_id, hex(label_key));
        return err;
    }
    if (err == TxnErrorCode::TXN_KEY_NOT_FOUND) {
        LOG_INFO("txn get err={} db_id={} label_key={}", err, db_id, hex(label_key));
        return TxnErrorCode::TXN_OK;
    }

    if (label_val.size() <= VERSION_STAMP_LEN) {
        LOG_INFO("label_val.size()={} db_id={} label_key={}", label_val.size(), db_id,
                 hex(label_key));
        return TxnErrorCode::TXN_OK;
    }

    if (!label_pb.ParseFromArray(label_val.data(), label_val.size() - VERSION_STAMP_LEN)) {
        LOG_WARNING("failed to parse txn label db_id={} label_key={} label_val.size()={}", db_id,
                    hex(label_key), label_val.size());
    ...
    for (auto txn_id : label_pb.txn_ids()) {
        ...
        if ((txn_info.status() != TxnStatusPB::TXN_STATUS_ABORTED) &&
            ...
            LOG_INFO("txn not final state, label_key={} txn_id={}", hex(label_key), txn_id);
        ...
        LOG_INFO("remove index_key={} info_key={} recycle_key={}", hex(index_key), hex(info_key),
                 hex(recycle_key));
    }
    if (label_pb.txn_ids().size() == clean_txn_ids.size()) {
        txn->remove(label_key);
        key_size += label_key.size();
        LOG_INFO("remove label_key={}", hex(label_key));
    } else {
        label_pb.clear_txn_ids();
        for (auto txn_id : survival_txn_ids) {
            label_pb.add_txn_ids(txn_id);
        }
        LOG_INFO("rewrite label_pb={}", label_pb.ShortDebugString());
        label_val.clear();
        if (!label_pb.SerializeToString(&label_val)) {
            LOG_INFO("failed to serialize label_pb={} label_key={}", label_pb.ShortDebugString(),
                     hex(label_key));
            return TxnErrorCode::TXN_UNIDENTIFIED_ERROR;
        }
        txn->atomic_set_ver_value(label_key, label_val);
    ...
    if (err != TxnErrorCode::TXN_OK) {
        LOG_INFO("label_key={} key_size={} val_size={} label_pb={} clean_txn_ids={}",
                 hex(label_key), key_size, val_size, label_pb.ShortDebugString(),
                 fmt::join(clean_txn_ids, " "));
    }
    return err;
}
...
void MetaServiceImpl::clean_txn_label(::google::protobuf::RpcController* controller,
                                      ...
                                      ::google::protobuf::Closure* done) {
    int64_t db_id {-1};
    AnnotateTag tag_db_id("db_id", db_id);
    ...
    if (request->labels().empty()) {
        ...
        do {
            ...
            if (err != TxnErrorCode::TXN_OK) {
                msg = "failed to create txn";
                code = cast_as<ErrCategory::CREATE>(err);
                LOG_INFO("{} err={} begin={} end={}", msg, err, hex(begin_label_key),
                         hex(end_label_key));
                return;
            }
            DORIS_CLOUD_DEFER {
            ...
            if (err != TxnErrorCode::TXN_OK) {
                ...
                code = cast_as<ErrCategory::READ>(err);
                LOG_WARNING("{} begin={} end={}", msg, hex(begin_label_key), hex(end_label_key));
                return;
            }

            if (!it->has_next()) {
                LOG_INFO("no keys in the range. begin={} end={}", hex(begin_label_key),
                         hex(end_label_key));
                break;
            }
            while (it->has_next()) {
                ...
                if (!it->has_next()) {
                    begin_label_key = k;
                    LOG_INFO("iterator has no more kvs. key={}", hex(k));
                }
        ...
        if (err != TxnErrorCode::TXN_OK) {
            code = cast_as<ErrCategory::READ>(err);
            msg = fmt::format("failed to clean txn label. err={}", err);
            LOG_WARNING("{} db_id={} label_key={}", msg, db_id, hex(label_key));
            return;
        }
    }
...
void MetaServiceImpl::get_txn_id(::google::protobuf::RpcController* controller,
    ...
    TxnErrorCode err = txn_kv_->create_txn(&txn);
    if (err != TxnErrorCode::TXN_OK) {
        LOG_WARNING("failed to create txn. err={} db_id={} label={}", err, db_id, label);
        code = cast_as<ErrCategory::CREATE>(err);
        ss << "txn_kv_->create_txn() failed, err=" << err << " label=" << label
           << " db_id=" << db_id;
        msg = ss.str();
