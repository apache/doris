...
TableStreamReadResult read_bindings(
        const TableStreamMetadataReader& reader,
        const std::vector<TableStreamPartitionSetPB>& bindings,
        std::unordered_map<std::string, std::vector<VersionPB>>* pending_versions,
    ...
    TableStreamReadResult result = reader.read_and_validate_partitions(
            bindings, TableStreamReadIntent::SNAPSHOT, &versions, &source_instances);
    if (!result.ok()) {
        return result;
    }

    std::unordered_map<std::string, std::unordered_set<int64_t>> pending_txn_ids;
    for (const auto& stream_versions : versions) {
        for (const auto& partition_version : stream_versions.second) {
            const int64_t partition_id = partition_version.first;
            const VersionPB& version = partition_version.second;
            if (version.pending_txn_ids().empty()) {
                continue;
            }
            DCHECK_EQ(version.pending_txn_ids_size(), 1)
                    << "only support one pending txn id for now, version="
                    << version.ShortDebugString();
            DCHECK(source_instances.contains(partition_id));
            const std::string& source_instance = source_instances.at(partition_id);
            if (pending_txn_ids[source_instance].insert(version.pending_txn_ids(0)).second) {
                (*pending_versions)[source_instance].push_back(version);
            }
        }
    }
...
void MetaServiceImpl::get_table_stream_offset(::google::protobuf::RpcController* controller,
    ...
    while (true) {
        ...
        TableStreamReadResult read_result =
                read_bindings(reader, bindings, &pending_versions, &read_response);
        if (!read_result.ok()) {
        ...
        txn.reset();
        for (auto& [source_instance, versions] : pending_versions) {
            std::tie(code, msg) = wait_for_pending_txns(source_instance, versions);
            if (code != MetaServiceCode::OK) {
                return;
            }
        }
        TEST_SYNC_POINT_CALLBACK("get_table_stream_offset::after_wait_for_pending_txns");
    }
}
