...
void MetaServiceImpl::drop_partition(::google::protobuf::RpcController* controller,
    ...
    if (request->has_db_id()) {
        ...
        if (needUpdateTableVersion) {
            std::string table_ver_key = table_version_key({instance_id, request->db_id(), request->table_id()});
            txn->atomic_add(table_ver_key, 1);
            LOG_INFO("update table version").tag("table_ver_key", hex(table_ver_key));
        }
    }
