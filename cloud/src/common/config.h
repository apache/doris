...
// that never come back. Must be larger than the longest query the cluster allows; a value
// <= 0 disables the task. Incomplete multipart uploads of a crashed BE are invisible to this
// sweep: configure an AbortIncompleteMultipartUpload lifecycle rule on the bucket for those.
