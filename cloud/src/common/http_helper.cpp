...
const std::unordered_map<std::string_view, HttpHandlerInfo>& get_http_handlers() {
    ...
    static std::unordered_map<std::string_view, HttpHandlerInfo> handlers {
            ...
            {"show_storage_vaults",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_get_obj_store_info((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"add_hdfs_vault",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_alter_storage_vault((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"add_s3_vault",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_alter_storage_vault((MS*)s, c); },
            ...
            {"decode_key",
             {.handler = [](void* s, brpc::Controller* c) { return process_decode_key((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"encode_key",
             {.handler = [](void* s, brpc::Controller* c) { return process_encode_key((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"get_value",
             {.handler = [](void* s, brpc::Controller* c) { return process_get_value((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"set_value",
             {.handler = [](void* s, brpc::Controller* c) { return process_set_value((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"show_meta_ranges",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_show_meta_ranges((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"txn_lazy_commit",
             {.handler = [](void* s,
            ...
            {"fix_tablet_stats",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_fix_tablet_stats((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"fix_tablet_db_id",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_fix_tablet_db_id((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},

            {"get_instance",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_get_instance_info((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"get_obj_store_info",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_get_obj_store_info((MS*)s, c); },
            ...
            {"get_tablet_stats",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_get_tablet_stats((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"get_stage",
             {.handler = [](void* s, brpc::Controller* c) { return process_get_stage((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"get_cluster_status",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_get_cluster_status((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            ...
            {"compact_snapshot",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_compact_snapshot((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"decouple_instance",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_decouple_instance((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"abort_txn",
             {.handler = [](void* s, brpc::Controller* c) { return process_abort_txn((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"abort_tablet_job",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_abort_tablet_job((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"alter_ram_user",
             {.handler = [](void* s,
              ...
              .role = HttpRole::META_SERVICE}},
            {"alter_iam",
             {.handler = [](void* s, brpc::Controller* c) { return process_alter_iam((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"adjust_rate_limit",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_adjust_rate_limit((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},
            {"list_rate_limit",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_query_rate_limit((MS*)s, c); },
              .role = HttpRole::META_SERVICE}},

            // Recycler APIs
            {"recycle_instance",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_recycle_instance((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
            {"statistics_recycle",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_statistics_recycle((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
            {"recycle_copy_jobs",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_recycle_copy_jobs((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
            {"recycle_job_info",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_recycle_job_info((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
            {"check_instance",
             {.handler = [](void* s,
            ...
            {"check_job_info",
             {.handler = [](void* s,
              ...
              .role = HttpRole::RECYCLER}},
            {"check_meta",
             {.handler = [](void* s, brpc::Controller* c) { return process_check_meta((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
            {"adjust_rate_limiter",
             {.handler = [](void* s,
                            brpc::Controller* c) { return process_adjust_rate_limiter((RS*)s, c); },
              .role = HttpRole::RECYCLER}},
