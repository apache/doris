...
public:
    ...
    BvarLatencyRecorderWithStatus(const std::string& prefix, const std::string& metric_name)
            : BvarLatencyRecorderWithStatus(prefix + metric_name) {}
    ...
    */
    ...
    public:
        ...
        */
        bool start() {
            if (_started) {
                TEST_SYNC_POINT("ScheduledLatencyUpdater::already_start");
                LOG(INFO) << "timer already started";
                return true;
            }
            if (!schedule()) {
                LOG(WARNING) << "Failed to start bthread timer";
                return false;
            }
            TEST_SYNC_POINT("ScheduledLatencyUpdater::start");
            _started = true;
            return _started;
        }
        ...
        */
        bool schedule() {
            ...
            LOG(INFO) << "Successfully scheduled timer for ScheduledLatencyUpdater, interval: "
                      << _interval_s << "s";
            return true;
        }
        ...
        */
        static void update(void* arg) {
            ...
            LOG(INFO) << "Timer triggered for ScheduledLatencyUpdater, interval: "
...

            {
                ...
                TEST_SYNC_POINT("ScheduledLatencyUpdater::update");
                LOG(INFO) << "Updating latency recorder status for dimension, "
    ...
    private:
        ...
        bool _started {false};  // Whether the timer has been started
    };
