...
// Test: commit_txn updates last_active_cluster_id when feature is enabled
TEST(CompactionRWSeparationTest, CommitTxnUpdatesLastActiveCluster) {
    ...
    // Enable feature
    bool original = config::enable_compaction_rw_separation;
...
TEST(CompactionRWSeparationTest, FeatureDisabledNoClusterUpdate) {
    ...
    // Create tablet
    {
        ...
        auto first_rowset = tablet->add_rs_metas();
        first_rowset->set_rowset_id(0);
        first_rowset->set_rowset_id_v2("rowset_disabled_0");
        first_rowset->set_start_version(0);
        first_rowset->set_end_version(1);
        first_rowset->mutable_tablet_schema()->CopyFrom(*schema);
    ...
    int64_t txn_id = -1;
    {
        ...
        auto txn_info = req.mutable_txn_info();
        txn_info->set_db_id(db_id);
        txn_info->set_label("test_label_disabled");
        txn_info->add_table_ids(table_id);
        txn_info->set_timeout_ms(36000);
    ...
    // Prepare rowset
    {
        ...
        auto rowset = req.mutable_rowset_meta();
        rowset->set_rowset_id(0);
        rowset->set_rowset_id_v2("rowset_disabled_txn");
        rowset->set_tablet_id(tablet_id);
        rowset->set_partition_id(partition_id);
        rowset->set_txn_id(txn_id);
        rowset->set_num_segments(1);
    ...
    // Get rowset and check last_active_cluster_id is NOT set
    {
        brpc::Controller cntl;
        GetRowsetRequest req;
        GetRowsetResponse res;
        ...
        // Check that last_active_cluster_id is NOT set when feature is disabled
