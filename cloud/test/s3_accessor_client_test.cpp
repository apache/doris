...
protected:
    void SetUp() override {
        ...
        auto test_path =
                config_->get_prefix().empty() ? "s3_accessor_test_dir" : config_->get_prefix();
...
TEST_F(S3AccessorClientTest, LargeBatchDeleteTest) {
    ...
    // Create more than 1000 files to test batch deletion
    size_t num_files = 1500;
...
TEST_F(S3AccessorClientTest, ListDirectoryPaginationTest) {
    ...
    for (auto file = iter->next(); file.has_value(); file = iter->next()) {
        listed_files.insert(file->path);
        std::cout << "listed file: " << file->path << std::endl;
        count++;
    }
