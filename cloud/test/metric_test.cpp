...
TEST(MetricTest, FdbMetricExporterTest) {
    ...
    // process status
    {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        fdb_metric_exporter.stop();
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get(
        ...
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get(
                  ...
                  46551040);
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get({"09ca90b9f3f413e5816b2610ed8b465d",
        ...
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get(
                  ...
                  72359936);
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get({"0a456165f04e1ec1a2ade0ce523d54a8",
                                                         "memory", "unused_allocated_memory"}),
                  393216);
        ASSERT_EQ(g_bvar_fdb_cluster_process_status.get(
    ...
    // workload status
    {
        ...
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "started_hz"}), 12);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "started_counter"}),
                  196874980);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "committed_hz"}), 3.4);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "committed_counter"}),
                  2083958);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "conflicted_hz"}), 0);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"transactions", "conflicted_counter"}),
                  2970);
        ASSERT_EQ(
                g_bvar_fdb_cluster_workload_status.get({"transactions", "started_batch_priority_hz"}),
                0);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
                          {"transactions", "started_batch_priority_counter"}),
                  155);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
                          {"transactions", "started_default_priority_hz"}),
                  11);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
                          {"transactions", "started_default_priority_counter"}),
                  192248190);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
                          {"transactions", "started_immediate_priority_hz"}),
                  0.999999);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
        ...
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "reads_hz"}), 24.6);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "reads_counter"}),
                  324877376);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "writes_hz"}), 7.39999);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "writes_counter"}),
                  4336393);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "location_requests_hz"}),
                  0);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get(
                          {"operations", "location_requests_counter"}),
                  107543);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "read_requests_hz"}),
                  24.4);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "read_requests_counter"}),
                  324877376);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "memory_errors_hz"}), 0);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"operations", "memory_errors_counter"}),
                  0);

        // Test bytes component
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"bytes", "read_hz"}), 18468.2);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"bytes", "read_counter"}),
                  312243021624);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"bytes", "written_hz"}), 2042.2);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"bytes", "written_counter"}),
                  6678975461);

        // Test keys component
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"keys", "read_hz"}), 56.6);
        ASSERT_EQ(g_bvar_fdb_cluster_workload_status.get({"keys", "read_counter"}), 728858585);
    }
}