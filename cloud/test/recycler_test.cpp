...
TEST(CheckerTest, inverted_check_recycle_idx_file_v1) {
    ...
    for (int t = 10001; t <= 10100; ++t) {
        for (int v = 0; v < 10; ++v) {
            int ret = create_committed_rowset_with_tablet_schema(txn_kv.get(), accessor.get(), "1",
                                                                 t, v, 3, 3, false);
            ASSERT_EQ(ret, 0) << "Failed to create committed rs: " << ret;
        }
    }

    for (int t = 10001; t <= 10010; ++t) {
        for (int v = 0; v < 3; ++v) {
            std::unique_ptr<Transaction> txn;
            auto code = txn_kv->create_txn(&txn);
            ASSERT_EQ(code, TxnErrorCode::TXN_OK) << "Failed to create txn";
            txn->remove(meta_rowset_key({instance_id, t, v}));
            code = txn->commit();
            ASSERT_EQ(code, TxnErrorCode::TXN_OK) << "Failed to commit txn";
        }
    }
