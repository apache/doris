...
public class WriteBrokerConfCommand extends BrokerCommand {
    private WriteBrokerConfCommandRequestBody requestBody;

    public WriteBrokerConfCommand(WriteBrokerConfCommandRequestBody requestBody) {
    ...
    public Task setupTask() {
        ...
        WriteBrokerConfTaskHook hook = new WriteBrokerConfTaskHook();

        return new Task<TaskDesc>(desc, hook) {
            @Override
            protected int execute() throws IOException {
                try (FileOutputStream fos = new FileOutputStream(ServiceContext.getServiceMap().get(ServiceRole.BROKER).getConfigFilePath());
                     OutputStreamWriter osw = new OutputStreamWriter(fos, "UTF-8");
                     BufferedWriter bw = new BufferedWriter(osw)) {
                    bw.write(requestBody.getContent());
                    bw.flush();
                }
                return AgentConstants.COMMAND_EXECUTE_SUCCESS_CODE;
            }
        };
    }
    ...
    public CommandType setupCommandType() {
        return CommandType.WRITE_BROKER_CONF;
    }

    private static class WriteBrokerConfTaskHook extends TaskHook<TaskDesc> {
        @Override
        public void onSuccess(TaskDesc taskDesc) {
            BrokerService service = (BrokerService) ServiceContext.getServiceMap().get(ServiceRole.BROKER);
            service.load();
        }
    }
}
