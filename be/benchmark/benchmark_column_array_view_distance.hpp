...
//   2. ColumnArrayView: view handles unwrapping, get pointer via offset
// ============================================================
...
static void ArrayView_Distance_Plain_Plain(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (size_t row = 0; row < DIST_NUM_ROWS; ++row) {
            auto a1 = view1[row];
            auto a2 = view2[row];
            // Get raw pointer via: &element_data.data[offset]
            const float* p1 = &a1.data.data[a1.offset];
            const float* p2 = &a2.data.data[a2.offset];
