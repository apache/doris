...
TEST_F(SystemMetricsTest, normal) {
    MetricRegistry registry("test");
    {
        ...
        // cpu
        SimpleMetric* cpu_user =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "user"));
        ASSERT_TRUE(cpu_user != nullptr);
        // ASSERT_STREQ("57199151", cpu_user->to_string().c_str());
        SimpleMetric* cpu_nice =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "nice"));
        ASSERT_TRUE(cpu_nice != nullptr);
        ASSERT_STREQ("2616310", cpu_nice->to_string().c_str());
        SimpleMetric* cpu_system =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "system"));
        ASSERT_TRUE(cpu_system != nullptr);
        ASSERT_STREQ("10600935", cpu_system->to_string().c_str());
        SimpleMetric* cpu_idle =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "idle"));
        ASSERT_TRUE(cpu_idle != nullptr);
        ASSERT_STREQ("1517505423", cpu_idle->to_string().c_str());
        SimpleMetric* cpu_iowait =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "iowait"));
        ASSERT_TRUE(cpu_iowait != nullptr);
        ASSERT_STREQ("2137148", cpu_iowait->to_string().c_str());
        SimpleMetric* cpu_irq =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "irq"));
        ASSERT_TRUE(cpu_irq != nullptr);
        ASSERT_STREQ("0", cpu_irq->to_string().c_str());
        SimpleMetric* cpu_softirq =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "soft_irq"));
        ASSERT_TRUE(cpu_softirq != nullptr);
        ASSERT_STREQ("108277", cpu_softirq->to_string().c_str());
        SimpleMetric* cpu_steal =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "steal"));
        ASSERT_TRUE(cpu_steal != nullptr);
        ASSERT_STREQ("0", cpu_steal->to_string().c_str());
        SimpleMetric* cpu_guest =
                (SimpleMetric*)registry.get_metric("cpu", MetricLabels().add("mode", "guest"));
        ASSERT_TRUE(cpu_guest != nullptr);
        ASSERT_STREQ("0", cpu_guest->to_string().c_str());
        // memroy
        SimpleMetric* memory_allocated_bytes =
                (SimpleMetric*)registry.get_metric("memory_allocated_bytes");
        ...
        // fd
        SimpleMetric* fd_metric = (SimpleMetric*)registry.get_metric("fd_num_limit");
        ASSERT_TRUE(fd_metric != nullptr);
        ASSERT_STREQ("13052138", fd_metric->to_string().c_str());
        fd_metric = (SimpleMetric*)registry.get_metric("fd_num_used");
        ASSERT_TRUE(fd_metric != nullptr);
