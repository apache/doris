...
TEST_F(DorisBvarMetricsTest, Normal) {
    ...
    // check metric
    {
        ...
        auto metric = server_entity->get_metric("g_adder_fragment_requests_total");
    ...
    // engine request
    {
        ...
        auto metric = server_entity->get_metric("g_adder_create_tablet_requests_total");
        EXPECT_TRUE(metric != nullptr);
    ...
    }
    {
        ...
        auto metric = server_entity->get_metric("g_adder_drop_tablet_requests_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("16", metric->value_string().c_str());
    }
    {
        g_adder_report_all_tablets_requests_skip.increment(1);
        auto metric = server_entity->get_metric("g_adder_report_all_tablets_requests_skip");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("1", metric->value_string().c_str());
    }
    {
        g_adder_schema_change_requests_total.increment(19);
        auto metric = server_entity->get_metric("g_adder_schema_change_requests_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("19", metric->value_string().c_str());
    }
    {
        g_adder_create_rollup_requests_total.increment(20);
        auto metric = server_entity->get_metric("g_adder_create_rollup_requests_total");
        EXPECT_TRUE(metric != nullptr);
    ...
    }
    {
        ...
        auto metric = server_entity->get_metric("g_adder_storage_migrate_requests_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("21", metric->value_string().c_str());
    }
    {
        g_adder_delete_requests_total.increment(22);
        auto metric = server_entity->get_metric("g_adder_delete_requests_total");
        EXPECT_TRUE(metric != nullptr);
    ...
    //  compaction
    {
        ...
        auto metric = server_entity->get_metric("g_adder_base_compaction_deltas_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("30", metric->value_string().c_str());
    }
    {
        g_adder_cumulative_compaction_deltas_total.increment(31);
        auto metric = server_entity->get_metric("g_adder_cumulative_compaction_deltas_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("31", metric->value_string().c_str());
    }
    {
        g_adder_base_compaction_bytes_total.increment(32);
        auto metric = server_entity->get_metric("g_adder_base_compaction_bytes_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("32", metric->value_string().c_str());
    }
    {
        g_adder_cumulative_compaction_bytes_total.increment(33);
        auto metric = server_entity->get_metric("g_adder_cumulative_compaction_bytes_total");
        EXPECT_TRUE(metric != nullptr);
        EXPECT_STREQ("33", metric->value_string().c_str());
    }
    {
        g_adder_memory_pool_bytes_total.increment(40);
        auto metric = server_entity->get_metric("g_adder_memory_pool_bytes_total");
        EXPECT_TRUE(metric != nullptr);
