...
TEST_F(BitmapFunctionsTest, to_bitmap_invalid_argument) {
    ...
    std::string error_msg(
            "The input: xxxxxx is not valid, to_bitmap only support int value from 0 to 4294967295 "
            "currently");
...
TEST_F(BitmapFunctionsTest, to_bitmap_out_of_range) {
    ...
    std::string error_msg(
            "The input: 4294967296 is not valid, to_bitmap only support int value from 0 to "
            "4294967295 currently");
    ASSERT_EQ(error_msg, ctx->error_msg());
}
...
TEST_F(BitmapFunctionsTest, test_bitmap_intersect) {
    ...
    test_bitmap_intersect<DateTimeVal, DateTimeValue>(ctx, datatime1, datatime2);
