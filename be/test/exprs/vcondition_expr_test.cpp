...
TEST_F(VConditionExprCoalesceTest, DateTimeV2Nano) {
    auto coalesce_node =
            make_coalesce_node(TPrimitiveType::DATETIMEV2, /*is_nullable=*/true, /*scale=*/9);
    auto coalesce_expr = VectorizedCoalesceExpr::create_shared(coalesce_node);
    auto nano_type = std::make_shared<DataTypeDateTimeV2Nano>(9);
    ...
    const auto& values =
            assert_cast<const ColumnDateTimeV2Nano&>(nullable.get_nested_column()).get_data();
