...
TEST(LosslessDecimalCastTest, PreservesExecutionSemanticsInDigestAndClone) {
    TExprNode ordinary_node;
    ordinary_node.__set_node_type(TExprNodeType::CAST_EXPR);
    ordinary_node.__set_opcode(TExprOpcode::CAST);
    ordinary_node.__set_type(create_type_desc(PrimitiveType::TYPE_DECIMAL128I, 38, 0));
    ordinary_node.__set_is_nullable(true);
    ordinary_node.__set_num_children(0);

    TExprNode lossless_node = ordinary_node;
    lossless_node.__set_lossless_decimal_cast(true);
    VCastExpr ordinary_cast(ordinary_node);
    VCastExpr lossless_cast(lossless_node);

    EXPECT_NE(ordinary_cast.get_digest(0), lossless_cast.get_digest(0));

    VExprSPtr cloned;
    ASSERT_TRUE(lossless_cast.clone_node(&cloned).ok());
    auto cloned_cast = std::dynamic_pointer_cast<VCastExpr>(cloned);
    ASSERT_NE(cloned_cast, nullptr);
    EXPECT_TRUE(cloned_cast->is_lossless_decimal_cast());
}
