...
TEST_F(BlockFileCacheTest, ProbeTouchAndRemovalRevalidateTheRetainedBlock) {
            ...
            [&]() { config::enable_file_cache_async_touch_on_get_or_set = old_async_touch; }};
    {
        ...
        fs::path cache_file;
        const auto old_epoch = cache.async_write_manager()->current_write_epoch(hash);
        const uint64_t old_cache_epoch = cache.async_write_manager()->current_cache_epoch();
        {
            ...
            EXPECT_EQ(cache.async_write_manager()->current_cache_epoch(), old_cache_epoch);
            EXPECT_FALSE(cache.async_write_manager()->is_current_write_epoch(old_epoch));
