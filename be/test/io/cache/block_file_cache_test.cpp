...
TEST_F(BlockFileCacheTest, ttl_gc) {
    ...
    sp->set_call_back(
            "BlockFileCache::set_sleep_time",
            [](auto&& args) { *try_any_cast<int64_t*>(args[0]) = 200; }, &guard1);
    sp->enable_processing();
    TUniqueId query_id;
    query_id.hi = 1;
    query_id.lo = 1;
    ...
    config::file_cache_background_ttl_gc_interval_ms = 3000;
    ...
    ASSERT_TRUE(wait_until([&]() {
        auto size = cache._time_to_key.size();
        return size > 0 && size < 12;
    })) << "ttl gc should remove expired keys in batches";
    ASSERT_TRUE(wait_until([&]() { return cache._time_to_key.empty(); }))
            << "ttl gc should eventually remove all expired keys";
