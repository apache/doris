...
io::FileCacheSettings create_cached_remote_reader_tiny_settings(size_t block_size,
    ...
    const size_t capacity = std::max<size_t>(64, block_size * 4);
    settings.query_queue_size = capacity;
    settings.query_queue_elements = 16;
    settings.index_queue_size = capacity;
    settings.index_queue_elements = 16;
    settings.disposable_queue_size = capacity;
...
class RecordingFileReader : public FileReader {
public:
    explicit RecordingFileReader(FileReaderSPtr reader) : _reader(std::move(reader)) {}
    ~RecordingFileReader() override = default;
    ...
    const std::vector<PrefetchRange>& reads() const { return _reads; }

protected:
    Status read_at_impl(size_t offset, Slice result, size_t* bytes_read,
                        const IOContext* io_ctx) override {
        _reads.emplace_back(offset, offset + result.size);
        return _reader->read_at(offset, result, bytes_read, io_ctx);
    }

private:
    FileReaderSPtr _reader;
    std::vector<PrefetchRange> _reads;
};
...
TEST_F(BlockFileCacheTest,
    ...
    SourceReadBreakdown source_read_breakdown;
    IOContext io_ctx;
    ASSERT_TRUE(reader._read_remote_blocks_into_cache(empty_blocks, 2, 8, 2,
                                                      Slice(buffer.data(), buffer.size()), false,
                                                      read_stats, source_read_breakdown, &io_ctx,
                                                      indirect_read_bytes, peer_fetched_blocks)
                        .ok());

    EXPECT_EQ(peer_fetched_blocks.size(), empty_blocks.size());
...
TEST_F(BlockFileCacheTest, cached_remote_file_reader_read_remote_blocks_into_cache_dryrun_no_copy) {
    ...
    SourceReadBreakdown source_read_breakdown;
    IOContext io_ctx;
    ASSERT_TRUE(reader._read_remote_blocks_into_cache(empty_blocks, 1, 3, 0,
                                                      Slice(buffer.data(), buffer.size()), true,
                                                      read_stats, source_read_breakdown, &io_ctx,
                                                      indirect_read_bytes, peer_fetched_blocks)
                        .ok());
...
TEST_F(BlockFileCacheTest,
    ...
    ASSERT_TRUE(reader._read_remaining_blocks_from_cache(
                              holder, 1, 10, Slice(buffer.data(), buffer.size()), false,
...
TEST_F(BlockFileCacheTest,
    ...
    SourceReadBreakdown source_read_breakdown;
    ASSERT_TRUE(reader._read_remaining_blocks_from_cache(
                              holder, 1, 2, Slice(buffer.data(), buffer.size()), false,
                              peer_fetched_blocks, read_stats, source_read_breakdown,
                              indirect_read_bytes, &bytes_read, &io_ctx)
                        .ok());
...
TEST_F(BlockFileCacheTest, external_reader_buffers_small_cache_block_on_first_read) {
    const std::string content = "abcdefghijklmnop";
    const fs::path file_path =
    ...
    opts.cache_type = io::FileCachePolicy::FILE_BLOCK_CACHE;
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    opts.reader_local_cache = std::make_shared<FileScannerV2ReaderLocalCache>(16);
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader reader(local_reader, opts);
    ...
    ASSERT_TRUE(reader.read_at_from_cache(2, Slice(first.data(), first.size()), &bytes_read,
                                          &cache_hit, &first_ctx)
                        .ok());
    ASSERT_TRUE(cache_hit);
    EXPECT_EQ(first, "cdef");
    ASSERT_NE(reader._reader_local_cache, nullptr);
    EXPECT_GT(reader._reader_local_cache->entry_count(), 0);
    EXPECT_EQ(first_stats.num_reader_local_cache_total, 2);
    EXPECT_EQ(first_stats.num_reader_local_cache_miss, 2);
    EXPECT_EQ(first_stats.num_reader_local_cache_fill, 2);
    EXPECT_EQ(first_stats.bytes_reader_local_cache_request, 4);
    EXPECT_EQ(first_stats.bytes_read_into_reader_local_cache, 8);
    ...
    ASSERT_GT(reader._reader_local_cache->entry_count(), 0);
...
TEST_F(BlockFileCacheTest, reader_local_cache_zero_capacity_is_disabled) {
    auto cache = std::make_shared<FileScannerV2ReaderLocalCache>(0);
    EXPECT_EQ(cache->create_file_cache(), nullptr);
    EXPECT_EQ(cache->entry_count(), 0);
    EXPECT_EQ(cache->memory_usage(), 0);
}

TEST_F(BlockFileCacheTest, reader_local_cache_partial_probe_does_not_modify_output) {
    constexpr size_t reader_block_size = 256_kb;
    const std::string content(2 * reader_block_size, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_partial_probe", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_partial_probe_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    create_cached_remote_reader_test_cache(cache_path, 2 * reader_block_size);
    ...
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    opts.reader_local_cache = std::make_shared<FileScannerV2ReaderLocalCache>(reader_block_size);
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader reader(local_reader, opts);

    std::string warmup(reader_block_size, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(reader.read_at(0, Slice(warmup.data(), warmup.size()), &bytes_read).ok());
    bool cache_hit = false;
    ASSERT_TRUE(reader.read_at_from_cache(0, Slice(warmup.data(), warmup.size()), &bytes_read,
                                          &cache_hit)
                        .ok());
    ASSERT_TRUE(cache_hit);
    ASSERT_TRUE(reader.read_at_from_cache(0, Slice(warmup.data(), warmup.size()), &bytes_read,
    ...
    std::string output(16, '#');
    ReadStatistics stats;
    EXPECT_FALSE(reader._read_from_memory_block_cache(reader_block_size - 8,
                                                      Slice(output.data(), output.size()), &stats));
    EXPECT_EQ(output, std::string(16, '#'));
}

TEST_F(BlockFileCacheTest, reader_local_cache_fill_exception_is_best_effort) {
    const std::string content(8_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_fill_exception", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_fill_exception_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    create_cached_remote_reader_test_cache(cache_path, 8_kb);
    ...
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    opts.reader_local_cache = std::make_shared<FileScannerV2ReaderLocalCache>(8_kb);
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader reader(local_reader, opts);
    ...
    std::string buffer(8, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(reader.read_at(1_kb, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    auto* sp = SyncPoint::get_instance();
    SyncPoint::CallbackGuard guard;
    sp->set_call_back(
            "CachedRemoteFileReader::reader_local_cache_before_fill",
            [&](auto&&) { throw std::bad_alloc(); }, &guard);
    sp->enable_processing();
    Defer clear_sync_points {[&]() { sp->disable_processing(); }};

    bool cache_hit = false;
    EXPECT_NO_THROW({
        const auto status = reader.read_at_from_cache(1_kb, Slice(buffer.data(), buffer.size()),
                                                      &bytes_read, &cache_hit);
        EXPECT_TRUE(status.ok());
    });
    EXPECT_TRUE(cache_hit);
    EXPECT_EQ(buffer, std::string(8, 'x'));
}

TEST_F(BlockFileCacheTest, reader_local_cache_uses_one_cache_object_per_reader) {
    auto cache = std::make_shared<FileScannerV2ReaderLocalCache>(1_mb);
    auto first = cache->create_file_cache();
    auto second = cache->create_file_cache();
...
TEST_F(BlockFileCacheTest, reader_local_cache_is_opt_in_and_excludes_doris_tables) {
    const std::string content = "abcdefghijklmnop";
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_cache_scope", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_cache_scope_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader v1_reader(local_reader, opts);

    std::string buffer(4, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(v1_reader.read_at(2, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    for (int i = 0; i < 3; ++i) {
        bool cache_hit = false;
        ASSERT_TRUE(v1_reader
                            .read_at_from_cache(2, Slice(buffer.data(), buffer.size()), &bytes_read,
                                                &cache_hit)
                            .ok());
        ASSERT_TRUE(cache_hit);
    }
    EXPECT_EQ(v1_reader._reader_local_cache, nullptr);

    opts.enable_reader_local_cache = true;
    opts.is_doris_table = true;
    opts.tablet_id = 1;
    CachedRemoteFileReader internal_reader(local_reader, opts);
    ASSERT_TRUE(internal_reader.read_at(2, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    for (int i = 0; i < 3; ++i) {
        bool cache_hit = false;
        FileCacheStatistics stats;
        IOContext io_ctx;
        io_ctx.file_cache_stats = &stats;
        ASSERT_TRUE(internal_reader
                            .read_at_from_cache(2, Slice(buffer.data(), buffer.size()), &bytes_read,
                                                &cache_hit, &io_ctx)
                            .ok());
        ASSERT_TRUE(cache_hit);
        EXPECT_EQ(stats.num_reader_local_cache_total, 0);
    }
    EXPECT_EQ(internal_reader._reader_local_cache, nullptr);
}

TEST_F(BlockFileCacheTest, reader_local_cache_buffers_full_block_on_first_cache_hit) {
    constexpr size_t cache_block_size = 1_mb;
    constexpr size_t reader_block_size = 256_kb;
    const std::string content(600_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_cache_sub_block", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_cache_sub_block_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    create_cached_remote_reader_test_cache(cache_path, cache_block_size);
    ...
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    opts.reader_local_cache = std::make_shared<FileScannerV2ReaderLocalCache>(16_mb);
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader reader(local_reader, opts);
    ...
    std::string buffer(8, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(reader.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read).ok());

    FileCacheStatistics fill_stats;
    IOContext fill_ctx;
    fill_ctx.file_cache_stats = &fill_stats;
    bool cache_hit = false;
    ASSERT_TRUE(reader.read_at_from_cache(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read,
                                          &cache_hit, &fill_ctx)
                        .ok());
    ASSERT_TRUE(cache_hit);
    ASSERT_NE(reader._reader_local_cache, nullptr);
    EXPECT_EQ(reader._reader_local_cache->entry_count(), 1);
    EXPECT_EQ(reader._reader_local_cache->memory_usage(), reader_block_size);
    EXPECT_EQ(fill_stats.num_reader_local_cache_miss, 1);
    EXPECT_EQ(fill_stats.num_reader_local_cache_fill, 1);
    EXPECT_EQ(fill_stats.bytes_reader_local_cache_request, buffer.size());
    EXPECT_EQ(fill_stats.bytes_read_into_reader_local_cache, reader_block_size);

    FileCacheStatistics hit_stats;
    IOContext hit_ctx;
    hit_ctx.file_cache_stats = &hit_stats;
    cache_hit = false;
    ASSERT_TRUE(reader.read_at_from_cache(160_kb, Slice(buffer.data(), buffer.size()), &bytes_read,
                                          &cache_hit, &hit_ctx)
                        .ok());
    ASSERT_TRUE(cache_hit);
    ...
    // The all-or-none memory probe must leave the output untouched before FileCache serves the
    // first part from memory and promotes the missing second block.
    EXPECT_EQ(cross_block_stats.num_reader_local_cache_partial_miss, 1);
    EXPECT_EQ(cross_block_stats.num_reader_local_cache_hit, 1);
    EXPECT_EQ(cross_block_stats.num_reader_local_cache_fill, 1);
}

TEST_F(BlockFileCacheTest, remote_only_on_miss_promotes_external_cache_hit_to_reader_local) {
    constexpr size_t reader_block_size = 256_kb;
    const std::string content(600_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("remote_only_reader_local", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "remote_only_reader_local_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    CachedRemoteFileReader writer(local_reader, opts);

    std::string buffer(8, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(writer.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    ...
    IOContext fill_ctx;
    fill_ctx.file_cache_stats = &fill_stats;
    fill_ctx.file_cache_miss_policy = FileCacheMissPolicy::REMOTE_ONLY_ON_MISS;
    ASSERT_TRUE(reader.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read, &fill_ctx)
                        .ok());
    EXPECT_EQ(fill_stats.num_reader_local_cache_fill, 1);
    EXPECT_EQ(fill_stats.num_remote_io_total, 0);

    FileCacheStatistics hit_stats;
    IOContext hit_ctx;
    hit_ctx.file_cache_stats = &hit_stats;
    hit_ctx.file_cache_miss_policy = FileCacheMissPolicy::REMOTE_ONLY_ON_MISS;
    ASSERT_TRUE(reader.read_at(160_kb, Slice(buffer.data(), buffer.size()), &bytes_read, &hit_ctx)
                        .ok());
    EXPECT_EQ(hit_stats.num_reader_local_cache_hit, 1);
    EXPECT_EQ(hit_stats.num_local_io_total, 0);
    EXPECT_EQ(hit_stats.num_remote_io_total, 0);
}

TEST_F(BlockFileCacheTest, concurrent_reader_local_cache_fill_is_single_flight) {
    const std::string content(8_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_cache_single_flight", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_cache_single_flight_cache";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    opts.cache_base_path = cache_path.string();
    opts.mtime = 1;
    auto loader_reader = std::make_shared<CachedRemoteFileReader>(local_reader, opts);
    auto waiter_reader = loader_reader;

    std::string warmup(8, '#');
    size_t bytes_read = 0;
    ASSERT_TRUE(
            loader_reader->read_at(1_kb, Slice(warmup.data(), warmup.size()), &bytes_read).ok());
    ...
    auto* sp = SyncPoint::get_instance();
    sp->set_call_back("CachedRemoteFileReader::reader_local_cache_before_fill", [&](auto&&) {
        loader_entered.set_value();
        release_future.wait();
    });
    sp->set_call_back("CachedRemoteFileReader::reader_local_cache_before_wait",
                      [&](auto&&) { waiter_entered.set_value(); });
    sp->enable_processing();
    Defer clear_sync_points {[&]() {
        sp->clear_call_back("CachedRemoteFileReader::reader_local_cache_before_fill");
        sp->clear_call_back("CachedRemoteFileReader::reader_local_cache_before_wait");
        sp->disable_processing();
    }};

    auto read_cache = [](const std::shared_ptr<CachedRemoteFileReader>& reader,
                         FileCacheStatistics* stats) {
        SCOPED_INIT_THREAD_CONTEXT();
        std::string buffer(8, '#');
        size_t read_size = 0;
        ...
        IOContext io_ctx;
        io_ctx.file_cache_stats = stats;
        Status st = reader->read_at_from_cache(1_kb, Slice(buffer.data(), buffer.size()),
                                               &read_size, &cache_hit, &io_ctx);
        return std::tuple(st, cache_hit, buffer);
    };

    FileCacheStatistics loader_stats;
    FileCacheStatistics waiter_stats;
    auto loader = std::async(std::launch::async, read_cache, loader_reader, &loader_stats);
    auto loader_ready = loader_entered.get_future();
    if (loader_ready.wait_for(std::chrono::seconds(5)) != std::future_status::ready) {
        release_loader.set_value();
        FAIL() << "the first reader did not start the reader-local fill";
    }
    auto waiter = std::async(std::launch::async, read_cache, waiter_reader, &waiter_stats);
    auto waiter_ready = waiter_entered.get_future();
    if (waiter_ready.wait_for(std::chrono::seconds(5)) != std::future_status::ready) {
        release_loader.set_value();
        FAIL() << "the second reader did not wait for the in-flight fill";
    }
    release_loader.set_value();

    auto [loader_status, loader_hit, loader_buffer] = loader.get();
    auto [waiter_status, waiter_hit, waiter_buffer] = waiter.get();
    ASSERT_TRUE(loader_status.ok());
    ASSERT_TRUE(waiter_status.ok());
    EXPECT_TRUE(loader_hit);
    EXPECT_TRUE(waiter_hit);
    EXPECT_EQ(loader_buffer, std::string(8, 'x'));
    EXPECT_EQ(waiter_buffer, std::string(8, 'x'));
    EXPECT_EQ(loader_stats.num_reader_local_cache_miss, 1);
    EXPECT_EQ(loader_stats.num_reader_local_cache_fill, 1);
    EXPECT_EQ(waiter_stats.num_reader_local_cache_wait, 1);
    EXPECT_EQ(waiter_stats.num_reader_local_cache_hit, 1);
    EXPECT_EQ(loader_stats.num_reader_local_cache_fill + waiter_stats.num_reader_local_cache_fill,
              1);
}

TEST_F(BlockFileCacheTest, reader_local_cache_fill_exception_wakes_waiter) {
    const std::string content(8_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("reader_local_cache_exception_waiter", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "reader_local_cache_exception_waiter_blocks";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    auto loader = std::async(std::launch::async, read_cache, loader_reader);
    if (loader_entered.get_future().wait_for(std::chrono::seconds(5)) !=
        std::future_status::ready) {
        release_loader.set_value();
        FAIL() << "the loader did not enter the reader-local fill";
    }
    auto waiter = std::async(std::launch::async, read_cache, waiter_reader);
    if (waiter_entered.get_future().wait_for(std::chrono::seconds(5)) !=
        std::future_status::ready) {
        release_loader.set_value();
        FAIL() << "the waiter did not observe the in-flight reader-local fill";
    }
    release_loader.set_value();

    ASSERT_EQ(loader.wait_for(std::chrono::seconds(5)), std::future_status::ready);
    ASSERT_EQ(waiter.wait_for(std::chrono::seconds(5)), std::future_status::ready);
    const auto [loader_status, loader_hit, loader_buffer] = loader.get();
    const auto [waiter_status, waiter_hit, waiter_buffer] = waiter.get();
    EXPECT_TRUE(loader_status.ok());
    EXPECT_TRUE(waiter_status.ok());
    EXPECT_TRUE(loader_hit);
    EXPECT_TRUE(waiter_hit);
    EXPECT_EQ(loader_buffer, std::string(8, 'x'));
    EXPECT_EQ(waiter_buffer, std::string(8, 'x'));
    EXPECT_EQ(opts.reader_local_cache->entry_count(), 0);
...
TEST_F(BlockFileCacheTest, reader_local_cache_block_maps_are_isolated_per_stream) {
    ...
    const std::string content(600_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("shared_reader_local_cache", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "shared_reader_local_cache_blocks";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    FileReaderSPtr local_reader;
    ASSERT_TRUE(global_local_filesystem()->open_file(file_path.string(), &local_reader).ok());
    auto shared_cache = std::make_shared<FileScannerV2ReaderLocalCache>(4 * reader_block_size);
    io::FileReaderOptions opts;
    opts.cache_type = io::FileCachePolicy::FILE_BLOCK_CACHE;
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    ...
    size_t bytes_read = 0;
    ASSERT_TRUE(
            first_reader.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    ...
    ASSERT_TRUE(first_reader
                        .read_at_from_cache(128_kb, Slice(buffer.data(), buffer.size()),
                                            &bytes_read, &cache_hit, &first_ctx)
                        .ok());
    ASSERT_TRUE(cache_hit);
    EXPECT_EQ(first_stats.num_reader_local_cache_fill, 1);
    ...
    ASSERT_TRUE(second_reader
                        .read_at_from_cache(160_kb, Slice(buffer.data(), buffer.size()),
...
TEST_F(BlockFileCacheTest, reader_local_cache_releases_blocks_with_stream) {
    ...
    const fs::path file_path =
            create_cached_remote_reader_test_file("sequential_reader_local_cache", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "sequential_reader_local_cache_blocks";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    {
        CachedRemoteFileReader first_reader(local_reader, opts);
        ASSERT_TRUE(first_reader.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read)
                            .ok());
        ASSERT_TRUE(first_reader
                            .read_at_from_cache(128_kb, Slice(buffer.data(), buffer.size()),
                                                &bytes_read, &cache_hit)
...
TEST_F(BlockFileCacheTest, reader_local_cache_does_not_evict_another_reader) {
    ...
    size_t bytes_read = 0;
    ASSERT_TRUE(
            first_reader.read_at(128_kb, Slice(warmup.data(), warmup.size()), &bytes_read).ok());

    bool cache_hit = false;
    ASSERT_TRUE(first_reader
                        .read_at_from_cache(128_kb, Slice(warmup.data(), warmup.size()),
                                            &bytes_read, &cache_hit)
                        .ok());
    ASSERT_TRUE(cache_hit);
    ASSERT_TRUE(first_reader
    ...
    cache_hit = false;
    ASSERT_TRUE(second_reader
                        .read_at_from_cache(384_kb, Slice(warmup.data(), warmup.size()),
                                            &bytes_read, &cache_hit, &second_ctx)
                        .ok());
    ASSERT_TRUE(cache_hit);
...
TEST_F(BlockFileCacheTest, reader_local_cache_capacity_moves_to_later_stream) {
    ...
    const std::string content(600_kb, 'x');
    const fs::path first_file_path =
            create_cached_remote_reader_test_file("idle_reader_local_cache_a", content);
    const fs::path second_file_path =
            create_cached_remote_reader_test_file("idle_reader_local_cache_b", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(first_file_path, ignore);
        fs::remove(second_file_path, ignore);
    ...
    const fs::path cache_path = caches_dir / "idle_reader_local_cache_blocks";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    CachedRemoteFileReader second_reader(second_local_reader, opts);
    ASSERT_TRUE(
            second_reader.read_at(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read).ok());
    FileCacheStatistics second_stats;
    IOContext second_ctx;
    second_ctx.file_cache_stats = &second_stats;
    ASSERT_TRUE(second_reader
...
TEST_F(BlockFileCacheTest, reader_local_cache_bypasses_promotion_at_query_memory_limit) {
    constexpr size_t reader_block_size = 256_kb;
    const std::string content(600_kb, 'x');
    const fs::path file_path =
            create_cached_remote_reader_test_file("query_bounded_reader_local_cache", content);
    Defer cleanup_file {[&]() {
        std::error_code ignore;
        fs::remove(file_path, ignore);
    }};

    const fs::path cache_path = caches_dir / "query_bounded_reader_local_cache_blocks";
    clear_cached_remote_reader_factory();
    Defer cleanup_cache {[&]() {
        std::error_code ignore;
        fs::remove_all(cache_path, ignore);
    ...
    FileReaderSPtr local_reader;
    ASSERT_TRUE(global_local_filesystem()->open_file(file_path.string(), &local_reader).ok());
    auto query_tracker = MemTrackerLimiter::create_shared(MemTrackerLimiter::Type::QUERY,
                                                          "reader-local-cache-test", 1);
    auto shared_cache =
            std::make_shared<FileScannerV2ReaderLocalCache>(reader_block_size, query_tracker);
    io::FileReaderOptions opts;
    opts.cache_type = io::FileCachePolicy::FILE_BLOCK_CACHE;
    opts.is_doris_table = false;
    opts.enable_reader_local_cache = true;
    ...
    ASSERT_TRUE(reader.read_at_from_cache(128_kb, Slice(buffer.data(), buffer.size()), &bytes_read,
                        ...
                        .ok());
    EXPECT_TRUE(cache_hit);
    EXPECT_EQ(stats.num_reader_local_cache_miss, 1);
    EXPECT_EQ(stats.num_reader_local_cache_fill, 0);
    EXPECT_EQ(stats.num_reader_local_cache_admission_reject, 1);
    EXPECT_EQ(shared_cache->entry_count(), 0);
    EXPECT_EQ(shared_cache->memory_usage(), 0);
}
