...
TEST_F(BufferedReaderTest, normal_use) {
    ...
    std::cout << "read bytes " << read_length << " using time " << watch.elapsed_time()
              << std::endl;
}
...
TEST_F(BufferedReaderTest, test_large_gap_amplification) {
    ...
    EXPECT_LT(amplify_128kb, 1.5) << "128KB window reads only first range, minimal amplification";
    ...
    std::cout << "Amplification comparison - 8MB adaptive: " << amplify_8mb
              << "x, 128KB: " << amplify_128kb << "x" << std::endl;
...
TEST_F(BufferedReaderTest, test_complete_read_process_with_io_tracking) {
    ...
    std::cout << "\n========== Test: Complete Read Process with IO Tracking =========="
              << std::endl;
    ...
    for (size_t i = 0; i < bytes_read; ++i) {
        EXPECT_EQ((uint8_t)data[i], (uint8_t)(i % UCHAR_MAX)) << "Data mismatch at offset " << i;
    }
    ...
    std::cout << "  -> User requested: " << bytes_read << " (" << bytes_read / kb << "KB)"
              << std::endl;
    std::cout << "  -> Read amplification: " << (double)bytes_after_first_read / (double)bytes_read
              << "x" << std::endl;
    ...
    EXPECT_GE(bytes_after_first_read, 500 * kb) << "Should read at least all content (500KB)";
    ...
    for (size_t range_idx = 1; range_idx < 5; ++range_idx) {
        ...
        st = merge_reader.read_at(start_offset, Slice(data.data(), 100 * kb), &bytes_read, nullptr);
        ...
        std::cout << "  Range " << range_idx << " [" << start_offset / kb << "KB, "
                  << (start_offset + 100 * kb) / kb << "KB): " << bytes_read / kb << "KB read - ";
    ...
    EXPECT_EQ(merge_stats.request_bytes, 500 * kb) << "User requested 500KB total (5 ranges)";
    ...
    std::cout << "✓ Read amplification: " << (double)final_bytes_read / (500.0 * kb)
              << "x (expected ~1.16x)" << std::endl;
...
TEST_F(BufferedReaderTest, test_large_gap_scenario_with_tracking) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        std::cout << "Range " << i << " [" << offset / kb << "KB, " << (offset + 100 * kb) / kb
                  << "KB): read " << bytes_read / kb << "KB" << std::endl;
    }
    ...
    std::cout << "  -> Total bytes read: " << final_bytes_read << " (" << final_bytes_read / kb
              << "KB)" << std::endl;
    std::cout << "  -> User requested: " << total_user_bytes << " (" << total_user_bytes / kb
              << "KB)" << std::endl;
    ...
    std::cout << "\n✓ Large gap rejection works correctly - prevents severe amplification"
              << std::endl;
    std::cout << "========================================================\n" << std::endl;
}
...
TEST_F(BufferedReaderTest, test_sparse_gap_scenario_with_tracking) {
    ...
    auto merge_stats = merge_reader.statistics();
    std::cout << "  -> Merge efficiency: "
              << (double)merge_stats.request_bytes / (double)merge_stats.merged_bytes
              << " (higher is better)" << std::endl;

    std::cout << "\n✓ Adaptive logic successfully prevents excessive amplification" << std::endl;
    std::cout << "✓ Balanced trade-off: " << final_io_count << " IOs vs " << amplification
              << "x amplification" << std::endl;
    std::cout << "=========================================================\n" << std::endl;
}
...
TEST_F(BufferedReaderTest, test_dense_gap_scenario_with_tracking) {
    ...
    std::cout << "  -> User requested: " << total_user_bytes << " (" << total_user_bytes / kb
              << "KB)" << std::endl;
    std::cout << "  -> Amplification: " << amplification << "x" << std::endl;
    std::cout << "  -> Gap overhead: " << (final_bytes_read - total_user_bytes) / kb << "KB"
              << std::endl;
    ...
    std::cout << "\n✓ Dense ranges optimally merged - single IO with minimal amplification"
              << std::endl;
...
TEST_F(BufferedReaderTest, test_mixed_density_scenario_with_tracking) {
    ...
    std::cout << "Setup: Part 1: 5 dense ranges (10KB gaps), Part 2: 5 sparse ranges (200KB gaps)"
              << std::endl;
    ...
    std::cout << "\n✓ Mixed scenario handled correctly - adaptive to different densities"
              << std::endl;
    std::cout << "✓ Dense: " << io_after_dense
              << " IO, Sparse: " << (final_io_count - io_after_dense) << " IOs" << std::endl;
    std::cout << "==============================================\n" << std::endl;
}
...
TEST_F(BufferedReaderTest, test_progressive_reading_scenario_with_tracking) {
    ...
    for (size_t pass = 0; pass < 4; ++pass) {
        ...
        for (size_t col = 0; col < 8; ++col) {
            ...
            auto st = merge_reader.read_at(read_offset, Slice(data.data(), 50 * kb), &bytes_read,
                                           nullptr);
            ASSERT_TRUE(st.ok());
            EXPECT_EQ(bytes_read, 50 * kb);
            total_user_bytes += bytes_read;
    ...
    std::cout << "  -> User requested: " << total_user_bytes << " (" << total_user_bytes / kb
              << "KB)" << std::endl;
    std::cout << "  -> Amplification: " << amplification << "x" << std::endl;
    std::cout << "  -> Average IO size: " << (final_bytes_read / final_io_count) / kb << "KB"
              << std::endl;
    ...
    EXPECT_EQ(merge_stats.request_bytes, 1600 * kb)
            << "Request bytes should match total user bytes";
    EXPECT_EQ(merge_stats.merged_bytes, final_bytes_read)
            << "Merged bytes should match physical IO bytes";
...
TEST_F(BufferedReaderTest, test_before_vs_after_adaptive_comparison) {
    ...
    // Scenario 1: Sparse Gap (the most impacted case)
    {
        ...
        std::cout << "  - Overall amplification: " << (double)total_span / (double)total_content
                  << "x" << std::endl;
        ...
        std::cout << "  - Total user requests: " << total_user_bytes / kb << "KB (15 × 80KB)"
                  << std::endl;
        ...
        std::cout << "  - Bytes saved: " << (int)(total_span - final_bytes_read) / (int)kb << "KB ("
                  << (1.0 - (double)final_bytes_read / total_span) * 100 << "% reduction)"
                  << std::endl;
        ...
        if (overall_amp < theoretical_before_amp) {
            ...
            std::cout << "  - ✗ Overall amplification INCREASED (more IOs but less per-IO)"
                      << std::endl;
            std::cout << "  - Trade-off: " << final_io_count << "x IOs vs "
                      << (theoretical_before_amp - overall_amp) << "x less amplification"
                      << std::endl;
        }
        std::cout << std::endl;
    }
    ...
    // Scenario 3: Large Gap (before: might try to merge, after: rejects)
    {
        ...
        std::cout << "  - Overall amplification if merged: "
                  << (double)total_span / (double)total_content << "x" << std::endl;
        ...
        if (overall_amp < (double)total_span / (double)total_content) {
            std::cout << "  - ✓ Prevents catastrophic amplification (5.0x → " << overall_amp << "x)"
                      << std::endl;
        }
        std::cout << std::endl;
    }
    ...
    std::cout << "\n  Key insight: Overall amplification = Total Physical IO / Total User Requests"
              << std::endl;
