...
TEST(DataTypeWritToJsonb, test_struct) {
    ...
    params.is_strict = false;
    {
...

        {
            ...
            EXPECT_EQ("{\"int_col\":1, \"string_col\":\"hello\"}",
...

        {
            ...
            EXPECT_TRUE(type->get_serde()->deserialize_column_from_jsonb(
                    *col_struct_to, writer.getValue(), params));
            std::cout << type->to_string(*col_struct_to, 0) << std::endl;
            EXPECT_EQ("{\"int_col\":2, \"string_col\":\"world\"}",
                      type->to_string(*col_struct_to, 0));
        }
...

        {
            ...
            EXPECT_TRUE(type->get_serde()->deserialize_column_from_jsonb(
                    *col_struct_to, writer.getValue(), params));
            std::cout << type->to_string(*col_struct_to, 0) << std::endl;
            EXPECT_EQ("{\"int_col\":null, \"string_col\":\"vectorized\"}",
                      type->to_string(*col_struct_to, 0));
        }

        {
            ...
            EXPECT_TRUE(type->get_serde()->deserialize_column_from_jsonb(
                    *col_struct_to, writer.getValue(), params));
            std::cout << type->to_string(*col_struct_to, 0) << std::endl;
            EXPECT_EQ("{\"int_col\":5, \"string_col\":\"test\"}",
                      type->to_string(*col_struct_to, 0));
        }
    }
}
