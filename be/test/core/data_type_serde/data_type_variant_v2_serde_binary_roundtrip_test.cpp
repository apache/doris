...
constexpr size_t SHREDDED_RESIDUAL_LENGTH_OFFSET = SHREDDED_PAYLOAD_OFFSET + sizeof(uint8_t);
...
ColumnVariantV2::MutablePtr conflict_shredded() {
    auto residual = encoded_rows({R"({})", R"({"shape":{}})", R"({"shape":"object-conflict"})"});
    auto conflict_values = encoded_rows({"7", R"("typed-conflict")", "null"});
    ...
    fields.emplace_back(PathInData(std::vector<std::string> {"shape", "value_leaf_unique"}),
                        std::move(conflict_values), presence({1, 1, 0}));
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest, NativeShreddedWirePreservesFixedLayout) {
    auto decoded = round_trip(*conflict_shredded());
    ASSERT_TRUE(decoded->is_shredded());
    ASSERT_EQ(decoded->shredded_field_count(), 2);

    auto first_new_path = simple_shredded({{"roundtrip_union_unique"}});
    const auto first_expected = first_new_path->materialize_encoded_range(0, 1);
    const size_t first_new_row = decoded->size();
    decoded->insert_range_from(*first_new_path, 0, 1);
    EXPECT_EQ(decoded->shredded_field_count(), 2);
    const auto first_actual = decoded->materialize_encoded_range(first_new_row, 1);
    EXPECT_TRUE(canonical_equals(first_expected->get_value_ref(0), first_actual->get_value_ref(0)));

    auto decoded_again = round_trip(*decoded);
    ASSERT_TRUE(decoded_again->is_shredded());
    ASSERT_EQ(decoded_again->shredded_field_count(), 2);
    auto second_new_path = simple_shredded({{"roundtrip_residual_unique"}});
    const auto second_expected = second_new_path->materialize_encoded_range(0, 1);
    const size_t second_new_row = decoded_again->size();
    decoded_again->insert_range_from(*second_new_path, 0, 1);
    EXPECT_EQ(decoded_again->shredded_field_count(), 2);
    const auto second_actual = decoded_again->materialize_encoded_range(second_new_row, 1);
    EXPECT_TRUE(
            canonical_equals(second_expected->get_value_ref(0), second_actual->get_value_ref(0)));
}

TEST(DataTypeVariantV2SerDeBinaryRoundTripTest, ShreddedWireVersionOneDecodesWithFixedLayout) {
    auto source = conflict_shredded();
    auto legacy_bytes = serialize(*source);
    ASSERT_EQ(static_cast<uint8_t>(legacy_bytes[SHREDDED_WIRE_VERSION_OFFSET]), 2);
    uint64_t payload_size = 0;
    std::memcpy(&payload_size, legacy_bytes.data() + SHREDDED_PAYLOAD_LENGTH_OFFSET,
                sizeof(payload_size));
    ASSERT_GT(payload_size, 0);
    --payload_size;
    std::memcpy(legacy_bytes.data() + SHREDDED_PAYLOAD_LENGTH_OFFSET, &payload_size,
                sizeof(payload_size));
    legacy_bytes[SHREDDED_WIRE_VERSION_OFFSET] = 1;
    legacy_bytes.erase(legacy_bytes.begin() + SHREDDED_PAYLOAD_OFFSET);

    auto decoded = deserialize(legacy_bytes);
    ASSERT_TRUE(decoded->is_shredded());
    expect_materialized_equal(*source, *decoded);
    auto new_path = simple_shredded({{"legacy_residual_unique"}});
    decoded->insert_range_from(*new_path, 0, 1);
    EXPECT_EQ(decoded->shredded_field_count(), source->shredded_field_count());
}
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest,
    ...
    EXPECT_TRUE(decoded->shredded_field_values(0).is_encoded());
    ...
    EXPECT_EQ(conflict_presence[1], 1);
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest,
     BlockRoundTripThenCrossLayoutMergeKeepsDestinationLayout) {
    const auto round_trip_block = [](const Block& source) {
        PBlock wire;
        size_t uncompressed_bytes = 0;
        size_t compressed_bytes = 0;
        int64_t compress_time = 0;
    ...
    auto destination_source = simple_shredded({{"a"}});
    auto merge_source = simple_shredded({{"b"}});
    const ColumnVariantV2* const destination_source_identity = destination_source.get();
    const ColumnVariantV2* const merge_source_identity = merge_source.get();
    const DataTypePtr variant_type = std::make_shared<DataTypeVariantV2>();
    Block destination_block({{std::move(destination_source), variant_type, "destination"}});
    Block source_block({{std::move(merge_source), variant_type, "source"}});

    Block decoded_destination = round_trip_block(destination_block);
    Block decoded_source = round_trip_block(source_block);
    EXPECT_EQ(ColumnVariantV2::TestAccess::encoded_range_materializations(
                      *destination_source_identity),
              0);
    EXPECT_EQ(ColumnVariantV2::TestAccess::encoded_range_materializations(*merge_source_identity),
              0);

    MutableColumnPtr destination_column =
            IColumn::mutate(std::move(decoded_destination.get_by_position(0).column));
    auto& destination = assert_cast<ColumnVariantV2&>(*destination_column);
    const auto& source =
            assert_cast<const ColumnVariantV2&>(*decoded_source.get_by_position(0).column);
    ...
    EXPECT_EQ(source.shredded_field_path(0), PathInData("b"));
    ...
    const VariantField expected_residual = encode_json(R"({"b":1})");
    EXPECT_TRUE(canonical_equals(expected_residual.ref(),
                                 destination.read_view().residual_value_at(1)));
    auto expected = encoded_rows({R"({"a":1})", R"({"b":1})"});
    expect_materialized_equal(*expected, destination);
}
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest,
    ...
    bad_wire_version[SHREDDED_WIRE_VERSION_OFFSET] = 3;
    expect_corrupt_wire_rejected_without_publish(bad_wire_version);

    auto bad_sealed_flag = valid;
    bad_sealed_flag[SHREDDED_PAYLOAD_OFFSET] = 2;
    expect_corrupt_wire_rejected_without_publish(bad_sealed_flag);

    uint64_t residual_size = 0;
    std::memcpy(&residual_size, valid.data() + SHREDDED_RESIDUAL_LENGTH_OFFSET,
                sizeof(residual_size));
    const size_t field_count_offset =
            SHREDDED_RESIDUAL_LENGTH_OFFSET + sizeof(residual_size) + residual_size;
    ...
    write_u64(oversized_residual_length, SHREDDED_RESIDUAL_LENGTH_OFFSET,
              std::numeric_limits<uint64_t>::max());
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest,
    ...
    unsupported[SHREDDED_WIRE_VERSION_OFFSET] = 3;
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest,
    ...
    auto residual = encoded_rows(
            {R"({"exact_overlap_unique":{}})", R"({"ancestor_overlap_unique":"conflict"})"});
    ColumnVariantV2::ShreddedFields fields;
    fields.emplace_back(PathInData(std::vector<std::string> {"ancestor_overlap_unique", "leaf"}),
                        encoded_rows({"null", "null"}), presence({0, 0}));
    fields.emplace_back(PathInData("exact_overlap_unique"), encoded_rows({"null", "null"}),
                        presence({0, 0}));
    auto source = ColumnVariantV2::create_shredded(std::move(residual), std::move(fields));
    const auto valid = serialize(*source);

    auto exact_overlap = valid;
    exact_overlap[presence_values_offset(valid, "exact_overlap_unique")] = 1;
    expect_corrupt_wire_rejected_without_publish(exact_overlap);

    auto ancestor_overlap = valid;
    ancestor_overlap[presence_values_offset(valid, "leaf") + 1] = 1;
...
TEST(DataTypeVariantV2SerDeBinaryRoundTripTest, EncodedAndTypedDecodeToCanonicalEquality) {
    ...
    ColumnVariantV2::TestAccess::ensure_encoded(*decoded_typed.operator->());
