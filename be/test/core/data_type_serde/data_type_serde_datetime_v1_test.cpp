...
static ColumnDate::MutablePtr column_date_v1;
class DataTypeDateTimeV1SerDeTest : public ::testing::Test {
public:
    static void SetUpTestSuite() {
        auto root_dir = std::string(getenv("ROOT"));
        test_data_dir = root_dir + "/be/test/data/vec/columns";

        column_date_v1 = ColumnDate::create();
        column_datetime_v1_0 = ColumnDateTime::create();
    ...
    static void load_columns_data() {
        ...
        test_func(column_date_v1->get_ptr(), serde_date_v1, "DATEV1.csv");
        test_func(column_datetime_v1_0->get_ptr(), serde_datetime_v1, "DATETIMEV1.csv");
...
TEST_F(DataTypeDateTimeV1SerDeTest, serdes) {
    ...
    test_func(*serde_date_v1, column_date_v1);
    test_func(*serde_datetime_v1, column_datetime_v1_0);
}
} // namespace doris::vectorized
