...
TEST(VariantShreddedColumnBuilderTest, AllInactiveFieldsStillPublishLayout) {
    auto source = encoded_column({"{}", R"({"keep":1})", R"({"a":{"b":2}})"});
    VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));

    auto result = builder.build(*source);

    ASSERT_TRUE(result->is_shredded());
    ASSERT_EQ(result->size(), source->size());
    ASSERT_EQ(result->shredded_field_count(), 1);
    EXPECT_EQ(result->shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {0, 0, 0}));
    expect_materialized_equal(*source, *result);
}

TEST(VariantShreddedColumnBuilderTest, PublishesStableLayoutAndPromotesExactScalarConflict) {
    auto source = encoded_column({"{}", R"({"a":1})", R"({"a":"one"})", R"({"a":{"b":2}})"});
    const auto int_type = std::make_shared<DataTypeInt64>();
    DorisVector<VariantShreddedLayoutEntry> layout;
    layout.push_back({PathInData("a"), int_type});
    layout.push_back({PathInData("b"), int_type});
    ...
    EXPECT_EQ(result->shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {0, 1, 1, 0}));
    EXPECT_TRUE(result->shredded_field_values(0).is_encoded());
    EXPECT_EQ(residual_json(*result, 0), "{}");
    EXPECT_EQ(residual_json(*result, 1), "{}");
    EXPECT_EQ(residual_json(*result, 2), "{}");
    EXPECT_EQ(residual_json(*result, 3), R"({"a":{"b":2}})");
    ...
    EXPECT_TRUE(result->shredded_field_values(1).is_typed());
    expect_materialized_equal(*source, *result);
}

TEST(VariantShreddedColumnBuilderTest, LateFirstHitBackfillsAndPublishesNeverHitField) {
    auto source = encoded_column({"{}", R"({"a":"conflict"})", R"({"b":7,"keep":2})", "{}"});
    const auto int_type = std::make_shared<DataTypeInt64>();
    DorisVector<VariantShreddedLayoutEntry> layout;
    layout.push_back({PathInData("a"), int_type});
    layout.push_back({PathInData("b"), int_type});
    ...
    ASSERT_EQ(result->shredded_field_count(), 2);
    EXPECT_EQ(result->shredded_field_path(0), PathInData("a"));
    EXPECT_EQ(result->shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {0, 1, 0, 0}));
    EXPECT_TRUE(result->shredded_field_values(0).is_encoded());
    EXPECT_EQ(residual_json(*result, 1), "{}");
    EXPECT_EQ(result->shredded_field_path(1), PathInData("b"));
    EXPECT_EQ(result->shredded_field_presence(1).get_data(), (PaddedPODArray<UInt8> {0, 0, 1, 0}));
    const auto& child = result->shredded_field_values(1);
    ASSERT_TRUE(child.is_typed());
    EXPECT_EQ(assert_cast<const ColumnNullable&>(child.typed_column()).get_null_map_data(),
              (NullMap {1, 1, 0, 1}));
    EXPECT_EQ(residual_json(*result, 2), R"({"keep":2})");
    expect_materialized_equal(*source, *result);
}

TEST(VariantShreddedColumnBuilderTest, VariantNullFirstHitKeepsThreeNullStatesSeparate) {
    auto source = encoded_column({"{}", "{}", R"({"a":null})", R"({"a":7})"});
    VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));
    ...
    const auto& child = result->shredded_field_values(0);
    ASSERT_TRUE(child.is_typed());
    EXPECT_EQ(assert_cast<const ColumnNullable&>(child.typed_column()).get_null_map_data(),
              (NullMap {1, 1, 1, 0}));
    EXPECT_EQ(column_json(*result, 1), "{}");
    EXPECT_EQ(column_json(*result, 2), R"({"a":null})");

    auto outer_nulls = ColumnUInt8::create();
    outer_nulls->get_data() = {1, 0, 0, 0};
    auto nullable = ColumnNullable::create(std::move(result), std::move(outer_nulls));
    EXPECT_EQ(nullable->get_null_map_data(), (NullMap {1, 0, 0, 0}));
    EXPECT_TRUE(nullable->is_null_at(0));
    EXPECT_FALSE(nullable->is_null_at(1));
    EXPECT_FALSE(nullable->is_null_at(2));
}

TEST(VariantShreddedColumnBuilderTest, BoundAllMissingMaterializedFieldPublishesLayout) {
    VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));
    auto batch = builder.begin_batch(3);
    auto values = ColumnInt64::create();
    values->insert_many_defaults(3);
    auto nulls = ColumnUInt8::create();
    nulls->insert_many_vals(1, 3);
    auto owner = ColumnNullable::create(std::move(values), std::move(nulls));
    const auto& nullable = *owner;
    batch.bind_materialized_source(0, nullable.get_nested_column(),
                                   nullable.get_null_map_data().data(), std::move(owner));
    for (size_t row = 0; row < 3; ++row) {
        batch.finish_row();
    }
    auto residual = encoded_column({R"({"keep":1})", "{}", R"({"keep":3})"});

    auto result = batch.finish(std::move(residual));

    ASSERT_TRUE(result->is_shredded());
    ASSERT_EQ(result->shredded_field_count(), 1);
    EXPECT_EQ(result->shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {0, 0, 0}));
    EXPECT_EQ(column_json(*result, 0), R"({"keep":1})");
    EXPECT_EQ(column_json(*result, 1), "{}");
    EXPECT_EQ(column_json(*result, 2), R"({"keep":3})");
}

TEST(VariantShreddedColumnBuilderTest, BorrowedAllMissingMaterializedFieldPublishesLayout) {
    VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));
    auto batch = builder.begin_batch(2);
    auto values = ColumnInt64::create();
    values->insert_many_defaults(2);
    auto nulls = ColumnUInt8::create();
    nulls->insert_many_vals(1, 2);
    batch.bind_materialized_source(0, *values, nulls->get_data().data());
    batch.finish_row();
    batch.finish_row();

    auto result = batch.finish(encoded_column({R"({"keep":1})", "{}"}));

    ASSERT_TRUE(result->is_shredded());
    ASSERT_EQ(result->shredded_field_count(), 1);
    EXPECT_TRUE(result->shredded_field_values(0).is_typed());
    EXPECT_EQ(result->shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {0, 0}));
    EXPECT_EQ(column_json(*result, 0), R"({"keep":1})");
    EXPECT_EQ(column_json(*result, 1), "{}");
}

TEST(VariantShreddedColumnBuilderTest, AllInactiveBatchStillEnforcesDeclaredRowCount) {
    EXPECT_DEATH(
            {
                VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));
                auto batch = builder.begin_batch(1);
                auto residual = encoded_column({"{}"});
                static_cast<void>(batch.finish(std::move(residual)));
            },
            "did not complete every declared row");
    EXPECT_DEATH(
            {
                VariantShreddedColumnBuilder builder(int_layout(PathInData("a")));
            ...
            "completed more rows than declared");
}

// NOLINTNEXTLINE(readability-function-cognitive-complexity) -- GTest macros inflate the conflict-order matrix.
TEST(VariantShreddedColumnBuilderTest, ExactTypeConflictPromotesChildToEncoded) {
...
TEST(VariantShreddedColumnBuilderTest, PromotedChildKeepsMissingVariantNullAndOuterNullDistinct) {
    ...
    ASSERT_TRUE(shredded->is_shredded());
    const auto& presence = shredded->shredded_field_presence(0).get_data();
    ASSERT_EQ(presence, (PaddedPODArray<UInt8> {1, 1, 0, 1, 0}));
    const auto& child = shredded->shredded_field_values(0);
    ASSERT_TRUE(child.is_encoded());
    EXPECT_EQ(column_json(child, 0), "1");
    EXPECT_EQ(column_json(child, 1), R"("one")");
    ...
    EXPECT_EQ(residual_json(*shredded, 1), R"({"keep":1})");
    EXPECT_EQ(residual_json(*shredded, 2), "{}");
...
TEST(VariantShreddedColumnBuilderTest,
    ...
    EXPECT_EQ(shredded->shredded_field_presence(0).get_data(),
              (PaddedPODArray<UInt8> {1, 1, 0, 0, 1, 0}));
    EXPECT_TRUE(shredded->shredded_field_values(0).is_encoded());
    EXPECT_EQ(residual_json(*shredded, 0), R"({"keep":0})");
    EXPECT_EQ(residual_json(*shredded, 1), R"({"keep":1})");
