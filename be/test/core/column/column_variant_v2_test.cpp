...
std::vector<ColumnPtr> subcolumns(ColumnVariantV2& column) {
    ...
    column.for_each_subcolumn([&](ColumnPtr& subcolumn) { result.push_back(subcolumn); });
    return result;
}
...
const IColumn* subcolumn_address(ColumnVariantV2& column, size_t target) {
    ...
    size_t index = 0;
    column.for_each_subcolumn([&](ColumnPtr& subcolumn) {
        if (index == target) {
            result = subcolumn.get();
        }
        ++index;
    });
    EXPECT_EQ(index, 3);
...
std::vector<ETCrossCheckObservation> observe_et_cross_check(
    ...
    column.serialize_vec(batch_keys.data(), batch_keys.size());
...
TEST(ColumnVariantV2Test, CanonicalArenaAndSerializedKeyAdaptersRoundTrip) {
    ...
    column->serialize_vec(prefixed_keys.data(), prefixed_keys.size());
    for (size_t row = 0; row < column->size(); ++row) {
    ...
    batch_round_trip_first->deserialize_vec(serialized_keys.data(), serialized_keys.size());
    batch_round_trip_second->deserialize_vec(serialized_keys.data(), serialized_keys.size());
    ...
    sentinel_round_trip->deserialize_vec(sentinel_keys.data(), sentinel_keys.size());
...
TEST(ColumnVariantV2Test, TypedCanonicalHashCrcAndArenaMatchEncoded) {
    ...
    typed->serialize_vec(batch_keys.data(), batch_keys.size());
    for (size_t row = 0; row < typed->size(); ++row) {
    ...
    batch_round_trip->deserialize_vec(batch_input.data(), batch_input.size());
