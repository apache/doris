...
std::vector<int32_t> projection_ids(const std::vector<format::LocalColumnIndex>& projections) {
...
TEST_F(CastTest, ColumnMapperBuildsCastProjectionForTypeMismatch) {
    format::TableColumnMapper mapper;
...
TEST_F(CastTest, ColumnMapperDoesNotLeakRewrittenInPredicateLiteralAcrossSplits) {
    ...
    format::ColumnDefinition int_file_field;
    ...
    format::ColumnDefinition bigint_file_field;
...
TEST_F(CastTest, ColumnMapperDoesNotNestCastFilterAcrossScanRequests) {
    ...
    ASSERT_TRUE(mapper.create_scan_request({table_filter}, {}, projected_columns, &first_request,
                                           &state)
                        .ok());
    format::FileScanRequest second_request;
...
TEST_F(CastTest, ColumnMapperKeepsTableSlotIdWhenFileBlockPositionChanges) {
    ...
    format::FileScanRequest second_request;
    second_request.local_positions.emplace(format::LocalColumnId(9), format::LocalIndex(0));
    second_request.local_positions.emplace(format::LocalColumnId(10), format::LocalIndex(1));
