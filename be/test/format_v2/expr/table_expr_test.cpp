...
TEST(VLiteralTest, CreatesConstColumnWithGivenTypeAndField) {
    auto type = std::make_shared<DataTypeInt32>();
    auto literal = VLiteral::create_shared(type, Field::create_field<TYPE_INT>(123));
...
TEST(VLiteralTest, ExecutesAsConstColumn) {
    auto type = std::make_shared<DataTypeInt32>();
    auto literal = VLiteral::create_shared(type, Field::create_field<TYPE_INT>(456));
...
TEST(VLiteralTest, ExecuteAppendsConstColumnToBlock) {
    auto type = std::make_shared<DataTypeInt32>();
    auto literal = VLiteral::create_shared(type, Field::create_field<TYPE_INT>(789));
    Block block;
...
TEST(VSlotRefTest, KeepsSlotColumnIdsAndType) {
    auto type = std::make_shared<DataTypeInt32>();
    std::string name = "file_col";
    auto slot_ref = VSlotRef::create_shared(10, 20, 30, type, name);
...
TEST(VSlotRefTest, PrepareDoesNotRequireRowDescriptor) {
    auto type = std::make_shared<DataTypeInt32>();
    std::string name = "";
    auto slot_ref = VSlotRef::create_shared(10, 20, 30, type, name);
...
TEST(VSlotRefTest, ExecuteReturnsReferencedColumnId) {
    auto type = std::make_shared<DataTypeInt32>();
    std::string name = "";
    auto slot_ref = VSlotRef::create_shared(10, 1, 30, type, name);
    Block block;
    block.insert(ColumnHelper::create_column_with_name<DataTypeInt32>({1, 2, 3}));
