...
public:
    ...
    Status clone_node(VExprSPtr* cloned_expr) const override {
        DORIS_CHECK(cloned_expr != nullptr);
        *cloned_expr = std::make_shared<StatefulSequencePredicate>(_observed_invocations);
        return Status::OK();
    }
...
public:
    ...
    Status get_block(Block* file_block, size_t* rows, bool* eof) override {
        ...
        ++_returned_batches;
        *rows = 2;
        *eof = _state->eof_with_first_batch && _returned_batches >= _state->batch_count;
...
class SplitLocalPredicateTableReader final : public TableReader {
public:
    const std::vector<std::pair<size_t, size_t>>& predicate_layer_counts() const {
        return _predicate_layer_counts;
    }

protected:
    Status customize_file_scan_request(FileScanRequest* file_request) override {
        DORIS_CHECK(file_request != nullptr);
        _predicate_layer_counts.emplace_back(file_request->conjuncts.size(),
                                             _remaining_conjuncts.size());
        return TableReader::customize_file_scan_request(file_request);
    }

private:
    std::vector<std::pair<size_t, size_t>> _predicate_layer_counts;
};
...
TEST(TableReaderTest, ScannerOwnedUnsafePredicateIsPassedButNotExecutedByTableReader) {
    std::vector<ColumnDefinition> file_schema;
    file_schema.push_back(make_file_column(0, "id", std::make_shared<DataTypeInt32>()));
    std::vector<ColumnDefinition> projected_columns;
    projected_columns.push_back(make_table_column(0, "id", std::make_shared<DataTypeInt32>()));
...
TEST(TableReaderTest, ResidualExpressionStateSurvivesAcrossSplits) {
    ...
    auto stateful_predicate = std::make_shared<StatefulSequencePredicate>(&observed_invocations);
    stateful_predicate->add_child(table_int32_slot_ref(0, 0, "id"));
    auto fake_state = std::make_shared<FakeFileReaderState>();
    FakeTableReader reader(file_schema, fake_state);
    ASSERT_TRUE(reader.init({
                                    .projected_columns = projected_columns,
                                    .conjuncts = {prepared_conjunct(&state, stateful_predicate)},
                                    .format = FileFormat::PARQUET,
                                    .scan_params = nullptr,
                                    .io_ctx = nullptr,
                                    .runtime_state = &state,
...
TEST(TableReaderTest, LateConjunctFiltersAlreadyOpenSplit) {
    ...
    bool predicate_executed = false;
    auto late_predicate = std::make_shared<NonDeterministicPartitionPredicate>(&predicate_executed);
...
TEST(TableReaderTest, RecomputesPredicateExecutionLayerForEverySplit) {
    ...
    RuntimeState state {TQueryOptions(), TQueryGlobals()};
    std::vector<int> residual_invocations;
    auto residual_predicate = std::make_shared<StatefulSequencePredicate>(&residual_invocations);
    residual_predicate->add_child(table_int32_slot_ref(0, 0, "id"));
    SplitLocalPredicateTableReader reader;
    ASSERT_TRUE(reader.init({
                                    .projected_columns = projected_columns,
                                    .conjuncts =
                                            {
                                                    ...
                                                    prepared_conjunct(&state, residual_predicate),
                                            },
                                    .format = FileFormat::PARQUET,
                                    .scan_params = nullptr,
                                    .io_ctx = nullptr,
    ...
    ASSERT_EQ(reader.predicate_layer_counts().size(), 1);
    EXPECT_EQ(reader.predicate_layer_counts()[0], (std::pair<size_t, size_t> {1, 1}));
    ASSERT_TRUE(reader.close().ok());

    // The deterministic predicate was file-local for the previous split while the stateful
    // predicate remained residual. When this split omits `id`, both must be rebuilt as table-level
    // predicates instead of inheriting the previous split's file-local ownership.
    ...
    EXPECT_FALSE(eos);
    EXPECT_EQ(block.rows(), 0);
    ASSERT_EQ(reader.predicate_layer_counts().size(), 2);
    EXPECT_EQ(reader.predicate_layer_counts()[1], (std::pair<size_t, size_t> {0, 2}));
    ASSERT_TRUE(reader.get_block(&block, &eos).ok());
    EXPECT_TRUE(eos);
...
TEST(TableReaderTest, CreateScanRequestIgnoresResidualOnAnotherPhysicalColumn) {
    const auto int_type = std::make_shared<DataTypeInt32>();
    std::vector<ColumnDefinition> projected_columns = {
            make_table_column(0, "filter_key", int_type),
            make_table_column(1, "measure", int_type),
    ...
    TableFilter unrelated_residual_filter {
            .conjunct = VExprContext::create_shared(table_int32_greater_than_expr(1, 0, 2)),
            .global_indices = {GlobalIndex(1)},
            .can_localize = false,
    };

    FileScanRequest file_request;
    ASSERT_TRUE(mapper.create_scan_request({localized_filter, unrelated_residual_filter},
