...
private:
    DateV2Value<DateTimeV2ValueType> date_to_midnight(
        ...
        return DateV2Value<DateTimeV2ValueType>(date.year(), date.month(), date.day(), 0, 0, 0, 0);
    }
...
TEST_F(NewOrcReaderTest, AggregatePushdownUsesPrunedStripes) {
    ...
    column_filter.target = format::FileNestedPredicateTarget(format::LocalColumnId(0));
...
TEST_F(NewOrcReaderTest, ReaderExpressionMapFiltersRows) {
    auto reader = create_reader();
    RuntimeState state {TQueryOptions(), TQueryGlobals()};
    ASSERT_TRUE(reader->init(&state).ok());
    ...
    request->predicate_columns = {field_projection(0)};
    request->non_predicate_columns = {field_projection(1)};
    request->reader_expression_map.emplace_back(
            format::LocalColumnId(0),
            VExprContext::create_shared(std::make_shared<NullableInt32GreaterThanExpr>(0, 2)));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapStructChildPrunesStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "sarg_reader_expression_struct_child.orc").string();
    write_multi_stripe_orc_struct_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapStructChildWideningCastPrunesStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "sarg_reader_expression_struct_child_widening_cast.orc").string();
    write_multi_stripe_orc_struct_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
    ...
    request->reader_expression_map.emplace_back(
            format::LocalColumnId(0),
            VExprContext::create_shared(
                    std::make_shared<NullableStructChildInt32CastToInt64GreaterThanExpr>(
                            0, schema[0].type, "a", 500)));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapStructChildNullSafeEqualPrunesStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "sarg_reader_expression_struct_child_null_safe_equal.orc").string();
    write_multi_stripe_orc_struct_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
    ...
    request->reader_expression_map.emplace_back(
            format::LocalColumnId(0),
            VExprContext::create_shared(
                    std::make_shared<NullableStructChildInt32NullSafeEqualsLiteralExpr>(
                            0, schema[0].type, "a", 1000)));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest,
       SargReaderExpressionMapStructChildComplexNullSafeEqualDoesNotPruneStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "sarg_reader_expression_struct_child_complex_null_safe_equal.orc")
                    .string();
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapNestedStructChildPrunesStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "sarg_reader_expression_nested_struct_child.orc").string();
...
TEST_F(NewOrcReaderTest, NestedColumnPredicateBuildsSargAndPrunesStripes) {
    const auto multi_stripe_file_path = (_test_dir / "nested_column_predicate_sarg.orc").string();
    write_multi_stripe_orc_struct_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
    ...
    request->predicate_columns = {field_projection(0)};
    format::FileColumnPredicateFilter column_filter;
    column_filter.target = format::FileNestedPredicateTarget(
            format::LocalColumnId(0), std::make_unique<format::FileStructPredicateTarget>(0, "a"));
    column_filter.predicates.push_back(create_comparison_predicate<PredicateType::GT>(
            0, "a", remove_nullable(schema[0].children[0].type), Field::create_field<TYPE_INT>(500),
            false));
    request->column_predicate_filters.push_back(std::move(column_filter));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, NestedColumnPredicateUsesTargetWhenCompatibilityFieldsAreUnset) {
    const auto multi_stripe_file_path =
            (_test_dir / "target_only_nested_column_predicate.orc").string();
    write_multi_stripe_orc_struct_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
...
TEST_F(NewOrcReaderTest, NestedStringColumnPredicateBuildsSargAndPrunesStripes) {
    const auto multi_stripe_file_path =
            (_test_dir / "nested_string_column_predicate_sarg.orc").string();
    write_multi_stripe_orc_struct_string_file(multi_stripe_file_path);
    ASSERT_EQ(get_orc_stripe_count(multi_stripe_file_path), 2);
    ...
    ASSERT_EQ(schema.size(), 2);
    ASSERT_EQ(schema[0].children.size(), 1);
    EXPECT_EQ(remove_nullable(schema[0].children[0].type)->get_primitive_type(), TYPE_VARCHAR);
    ...
    column_filter.target = format::FileNestedPredicateTarget(
            format::LocalColumnId(0),
            std::make_unique<format::FileStructPredicateTarget>(0, "name"));
    column_filter.predicates.push_back(create_comparison_predicate<PredicateType::GT>(
            0, "name", remove_nullable(schema[0].children[0].type),
            Field::create_field<TYPE_STRING>("m"), false));
    request->column_predicate_filters.push_back(std::move(column_filter));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapArrayPredicateDoesNotPruneStripes) {
    const auto complex_file_path = (_test_dir / "sarg_array_reader_expression.orc").string();
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapMapPredicateDoesNotPruneStripes) {
    const auto complex_file_path = (_test_dir / "sarg_map_reader_expression.orc").string();
    write_two_stripe_orc_array_map_file(complex_file_path);
    ASSERT_EQ(get_orc_stripe_count(complex_file_path), 2);
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapElementAtDoesNotPruneStripes) {
    const auto complex_file_path =
            (_test_dir / "sarg_map_element_at_reader_expression.orc").string();
    write_two_stripe_orc_array_map_file(complex_file_path);
    ASSERT_EQ(get_orc_stripe_count(complex_file_path), 2);
    ...
    request->reader_expression_map.emplace_back(
            format::LocalColumnId(1),
            VExprContext::create_shared(std::make_shared<NullableMapElementInt32GreaterThanExpr>(
                    0, schema[1].type, "c", 200)));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapMapKeysInDoesNotPruneStripes) {
    const auto complex_file_path = (_test_dir / "sarg_map_keys_in_reader_expression.orc").string();
    write_two_stripe_orc_array_map_file(complex_file_path);
    ASSERT_EQ(get_orc_stripe_count(complex_file_path), 2);
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapArraySizeDoesNotPruneStripes) {
    const auto complex_file_path = (_test_dir / "sarg_array_size_reader_expression.orc").string();
...
TEST_F(NewOrcReaderTest, SargReaderExpressionMapListStructChildDoesNotPruneStripes) {
    const auto complex_file_path =
            (_test_dir / "sarg_array_struct_child_reader_expression.orc").string();
...
TEST_F(NewOrcReaderTest, SargDateV2CastToDateConjunctPrunesStripes) {
    ...
    request->conjuncts.push_back(
            VExprContext::create_shared(std::make_shared<NullableDateV2CastToDateGreaterThanExpr>(
                    0, DateV2Value<DateV2ValueType>(2020, 1, 1, 0, 0, 0, 0), literal)));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargDateV2CastToDateTimeV2MidnightConjunctPrunesStripes) {
    ...
    request->conjuncts.push_back(VExprContext::create_shared(
            std::make_shared<NullableDateV2CastToDateTimeV2GreaterThanExpr>(
                    0, DateV2Value<DateV2ValueType>(2020, 1, 1, 0, 0, 0, 0),
                    DateV2Value<DateTimeV2ValueType>(2020, 1, 1, 0, 0, 0, 0))));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargDateV2CastToDateTimeV2NonMidnightGreaterEqualPrunesStripes) {
    ...
    request->conjuncts.push_back(VExprContext::create_shared(
            std::make_shared<NullableDateV2CastToDateTimeV2ComparisonExpr>(
                    0, TExprOpcode::GE,
                    DateV2Value<DateTimeV2ValueType>(2020, 1, 1, 12, 0, 0, 0))));
    ASSERT_TRUE(reader->open(request).ok());
    ...
    request->conjuncts.push_back(
            VExprContext::create_shared(std::make_shared<NullableDateV2CastToDateTimeV2InExpr>(
                    0, std::vector<DateV2Value<DateTimeV2ValueType>> {
                               DateV2Value<DateTimeV2ValueType>(2021, 1, 1, 0, 0, 0, 0),
                               DateV2Value<DateTimeV2ValueType>(2020, 1, 1, 12, 0, 0, 0)})));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargDateConjunctPrunesStripes) {
    ...
    const auto literal =
            Field::create_field<TYPE_DATEV2>(DateV2Value<DateV2ValueType>(2020, 1, 1, 0, 0, 0, 0));
    auto request = std::make_shared<format::FileScanRequest>();
    request->predicate_columns = {field_projection(0)};
    request->conjuncts.push_back(
...
TEST_F(NewOrcReaderTest, SargDateCastToStringDoesNotPruneStripes) {
    ...
    request->conjuncts.push_back(
            VExprContext::create_shared(std::make_shared<NullableDateV2CastToStringGreaterThanExpr>(
                    0, "2020-01-01", DateV2Value<DateV2ValueType>(2020, 1, 1, 0, 0, 0, 0))));
    ASSERT_TRUE(reader->open(request).ok());
...
TEST_F(NewOrcReaderTest, SargTimestampConjunctPrunesStripes) {
    ...
    const auto literal = Field::create_field<TYPE_DATETIMEV2>(
            DateV2Value<DateTimeV2ValueType>(2020, 1, 1, 0, 0, 0, 0));
    auto request = std::make_shared<format::FileScanRequest>();
    request->predicate_columns = {field_projection(1)};
    request->conjuncts.push_back(
...
TEST_F(NewOrcReaderTest, ColumnPredicateTimestampBuildsSargAndPrunesStripes) {
    ...
    request->predicate_columns = {field_projection(1)};
    format::FileColumnPredicateFilter column_filter;
    column_filter.target = format::FileNestedPredicateTarget(format::LocalColumnId(1));
    column_filter.predicates.push_back(create_comparison_predicate<PredicateType::GT>(
...
TEST_F(NewOrcReaderTest, ColumnPredicateTimestampPrunesStripesByDorisStatistics) {
    ...
    const auto literal = DateV2Value<DateTimeV2ValueType>(2020, 1, 1, 0, 0, 0, 0);
    auto request = std::make_shared<format::FileScanRequest>();
    request->predicate_columns = {field_projection(1)};
    format::FileColumnPredicateFilter column_filter;
    column_filter.target = format::FileNestedPredicateTarget(format::LocalColumnId(1));
...
TEST_F(NewOrcReaderTest, ColumnPredicateTimestampInListBuildsSargAndPrunesStripes) {
    ...
    auto literal = DateV2Value<DateTimeV2ValueType>(2021, 1, 1, 0, 0, 0, 123000);
...
TEST_F(NewOrcReaderTest, ColumnPredicateTimestampNotInListBuildsSargAndPrunesStripes) {
    ...
    auto value_set = build_set<TYPE_DATETIMEV2>();
    auto literal = DateV2Value<DateTimeV2ValueType>(1970, 1, 1, 0, 0, 0, 123000);
    value_set->insert(&literal);
...
TEST_F(NewOrcReaderTest, ColumnPredicateDecimalBuildsSargAndPrunesStripes) {
    ...
    request->predicate_columns = {field_projection(2)};
    format::FileColumnPredicateFilter column_filter;
    column_filter.target = format::FileNestedPredicateTarget(format::LocalColumnId(2));
    column_filter.predicates.push_back(create_comparison_predicate<PredicateType::GT>(
