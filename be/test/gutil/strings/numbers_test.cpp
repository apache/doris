...
TEST_F(NumbersTest, test_float_to_buffer) {
    char buffer2[100];
    float v1 = 0;
    int len2 = CastToString::_fast_to_buffer(v1, buffer2);
    EXPECT_EQ(std::string("0"), std::string(buffer2, len2));

    float v2 = 0.00;
    len2 = CastToString::_fast_to_buffer(v2, buffer2);
    EXPECT_EQ(std::string("0"), std::string(buffer2, len2));

    float v3 = 20001230;
    len2 = CastToString::_fast_to_buffer(v3, buffer2);
    EXPECT_EQ(std::string("2.000123e+07"), std::string(buffer2, len2));

    float v4 = static_cast<float>(200012303131);
    len2 = CastToString::_fast_to_buffer(v4, buffer2);
...
TEST_F(NumbersTest, test_float_to_buffer2) {
    ...
    for (const auto& value : input_values) {
        std::string str;
        str.resize(64);
        auto len = CastToString::_fast_to_buffer(value.first, str.data());
        str.resize(len);
