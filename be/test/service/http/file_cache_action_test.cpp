...
TEST_F(FileCacheActionTest, clear_sync_true_runs_sync_clear) {
    cache_file_block("clear_sync_true_runs_sync_clear.dat");
    HttpRequest req(_evhttp_req);
    std::string json_metrics;
    ...
    EXPECT_NE(json_metrics.find("num_files_all=1"), std::string::npos);
    EXPECT_NE(json_metrics.find("num_cells_to_delete=1"), std::string::npos);
    EXPECT_EQ(_cache->_cur_cache_size, 0);
}
