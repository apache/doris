...
// The assertion macros account for the control-flow complexity in this service lifecycle test.
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
TEST_F(PointQueryRpcBatchTest, RpcAdmissionCompletesClosureOnceAndIsolatesLookupErrors) {
    ...
    LookupConnectionCache lookup_cache(32);
    auto* original_lookup_cache = std::exchange(exec_env->_lookup_connection_cache, &lookup_cache);
    ...
    Defer restore_env {[&]() {
        ...
        exec_env->_lookup_connection_cache = original_lookup_cache;
    }};
    add(1, 10'000);
    class AcceptedClosure final : public google::protobuf::Closure {
    public:
        void Run() override { completed.set_value(); }
        std::promise<void> completed;
    } accepted;
    auto completion = accepted.completed.get_future();
    // This direct service call has no socket for brpc's connection-based cancellation check.
    class ActiveController final : public brpc::Controller {
    public:
        bool IsCanceled() const override { return false; }
    } controller;
    PInternalService service(exec_env);
    // Exercise the real worker and PointQueryExecutor::init, including its task attachment.
    // The test has no storage engine, so the lookup must return an item error, not crash.
    service.tablet_fetch_data_batch(&controller, &request, &response, &accepted);
    ASSERT_EQ(std::future_status::ready, completion.wait_for(std::chrono::seconds(10)));
    ASSERT_EQ(1, response.results_size());
    EXPECT_EQ(0, response.status().status_code());
    EXPECT_NE(0, response.results(0).status().status_code());
    ASSERT_EQ(1, response.results(0).status().error_msgs_size());
    EXPECT_NE(std::string::npos,
              response.results(0).status().error_msgs(0).find("failed to get tablet"))
            << response.results(0).status().DebugString();
