...
TEST_F(CloudWarmUpManagerFilterTest, BuildWarmUpRowsetResultReturnsOkWithoutFailures) {
    auto st = CloudWarmUpManager::_build_warm_up_rowset_result({}, 2, 4001, 5001, "rowset-1");
    EXPECT_TRUE(st.ok());
}
...
TEST_F(CloudWarmUpManagerFilterTest, BuildWarmUpRowsetResultAggregatesAllFailures) {
    ...
    auto st = CloudWarmUpManager::_build_warm_up_rowset_result(failures, 3, 4002, 5002, "rowset-2");
    ...
    EXPECT_NE(std::string::npos, msg.find("table_id=5002"));
...
TEST_F(CloudWarmUpManagerFilterTest, BuildWarmUpRowsetResultKeepsTableNotFoundRetrySignal) {
    ...
    auto st = CloudWarmUpManager::_build_warm_up_rowset_result(failures, 2, 4003, 5003, "rowset-3");

    EXPECT_FALSE(st.ok());
    EXPECT_TRUE(st.is<ErrorCode::TABLE_NOT_FOUND>());
    std::string msg = st.to_string_no_stack();
    EXPECT_NE(std::string::npos, msg.find("table_id=5003"));
    EXPECT_NE(std::string::npos, msg.find("rpc one"));
    EXPECT_NE(std::string::npos, msg.find("missing"));
}

TEST_F(CloudWarmUpManagerFilterTest, DownstreamProgressTracksEarliestPendingTrigger) {
    g_warmup_ed_downstream_progress_tracker.reset_for_test();
    std::string job_id = "9001";

    g_warmup_ed_downstream_progress_tracker.record_task_submit(job_id, 1000);
    g_warmup_ed_downstream_progress_tracker.record_task_submit(job_id, 1500);
    g_warmup_ed_downstream_progress_tracker.record_task_submit(job_id, 1000);

    EXPECT_EQ(1000, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 1000);
    EXPECT_EQ(1000, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 1000);
    EXPECT_EQ(1500, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 1500);
    EXPECT_EQ(1500, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.reset_for_test();
}

TEST_F(CloudWarmUpManagerFilterTest, DownstreamProgressFallsBackToLatestFinishedTrigger) {
    g_warmup_ed_downstream_progress_tracker.reset_for_test();
    std::string job_id = "9002";

    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 2000);
    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 1800);
    EXPECT_EQ(2000, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.record_task_submit(job_id, 1900);
    EXPECT_EQ(1900, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    g_warmup_ed_downstream_progress_tracker.record_task_done(job_id, 1900);
    EXPECT_EQ(2000, g_warmup_ed_downstream_progress_tracker.get_progress_ts(job_id));

    auto job_ids = g_warmup_ed_downstream_progress_tracker.list_job_ids();
    EXPECT_NE(job_ids.end(), std::find(job_ids.begin(), job_ids.end(), job_id));
