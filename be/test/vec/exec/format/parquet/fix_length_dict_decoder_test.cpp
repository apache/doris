...
TEST_F(FixLengthDictDecoderTest, test_decode_with_filter_and_null) {
    ...
    // RLE encoded data: 2 zeros followed by 1, 2, 1, padded to 8 values, [0 0 1 2 1]
    std::vector<uint8_t> rle_data = {2, 4, 0, 3, 0b00011001, 0};
    ...
    size_t num_values = 7;
    std::vector<uint16_t> run_length_null_map {4, 1, 1, 1};   // data: [0 0 1 2 null 1 null]
    std::vector<uint8_t> filter_data = {1, 0, 1, 0, 1, 1, 1}; // filtered_data: [0 1 null 1 null]
    FilterMap filter_map;
    ASSERT_TRUE(filter_map.init(filter_data.data(), filter_data.size(), false).ok());
    ColumnSelectVector select_vector;
    ASSERT_TRUE(select_vector.init(run_length_null_map, num_values, nullptr, &filter_map, 0).ok());
    ...
    EXPECT_EQ(decoded_strings[3], "banana");
}
