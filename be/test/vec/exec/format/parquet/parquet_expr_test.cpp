...
TEST_F(ParquetExprTest, test_null_predicate_skips_bloom_filter) {
    const int col_idx = 2;
    NullPredicate is_null_pred(col_idx, true);
    ...
    std::function<bool(ParquetPredicate::ColumnStat*, int)> get_stat_func =
            [&](ParquetPredicate::ColumnStat* current_stat, int cid) {
                EXPECT_EQ(col_idx, cid);
                current_stat->col_schema = col_schema;
                current_stat->encoded_min_value.assign(8, '\0');
                current_stat->encoded_max_value.assign(8, '\0');
    ...
    EXPECT_TRUE(is_null_pred.evaluate_and(&stat));
    EXPECT_EQ(1, loader_calls);
}
