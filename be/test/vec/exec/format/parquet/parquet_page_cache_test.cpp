...
TEST(ParquetPageCacheTest, CacheHitReturnsDecompressedPayload) {
    ...
    config::enable_parquet_file_page_cache = true;
    config::parquet_page_cache_decompress_threshold = 100.0; // ensure decompressed pages are cached
    ...
    // insert into cache
    StoragePageCache::CacheKey key(path, static_cast<size_t>(file_end_offset), header_offset);
    ...
    ColumnChunkReader<false, false> ccr(&reader, &cc, &field_schema, nullptr, 0, nullptr);
...
TEST(ParquetPageCacheTest, MultiPagesMixedV1V2CacheHit) {
    ...
    StoragePageCache::CacheKey key1(path, static_cast<size_t>(header1_start + total1),
                                    header1_start);
    ...
    StoragePageCache::CacheKey key2(path, static_cast<size_t>(header2_start + total2),
                                    header2_start);
    ...
    ColumnChunkReader<false, false> ccr1(&reader1, &cc1, &field_schema1, nullptr, 0, nullptr);
    ...
    field_schema2.repetition_level = rl;
    field_schema2.definition_level = dl;
    ColumnChunkReader<false, false> ccr2(&reader2, &cc2, &field_schema2, nullptr, 0, nullptr);
    ASSERT_TRUE(ccr2.init().ok());
    // ASSERT_TRUE(ccr2.next_page().ok());
    ASSERT_TRUE(ccr2.load_page_data().ok());
...
TEST(ParquetPageCacheTest, CacheMissThenHit) {
    ...
    ColumnChunkReader<false, false> ccr(&reader, &cc, &fs, nullptr, 0, nullptr);
    ASSERT_TRUE(ccr.init().ok());
    // ASSERT_TRUE(ccr.next_page().ok());
    ASSERT_TRUE(ccr.load_page_data().ok());
    auto& statistics = ccr.statistics();
    ...
    ColumnChunkReader<false, false> ccr2(&reader, &cc, &fs, nullptr, 0, nullptr);
    ASSERT_TRUE(ccr2.init().ok());
    // ASSERT_TRUE(ccr2.next_page().ok());
    ASSERT_TRUE(ccr2.load_page_data().ok());
...
TEST(ParquetPageCacheTest, DecompressThresholdCachesCompressed) {
    ...
    ColumnChunkReader<false, false> ccr_small_thresh(&reader, &cc, &fs, nullptr, 0, nullptr);
    ...
    StoragePageCache::CacheKey key_small(path, /*file_end_offset*/ file_end, /*header_start*/ 0);
    bool found_small =
...
TEST(ParquetPageCacheTest, DecompressThresholdCachesDecompressed) {
    ...
    config::parquet_page_cache_decompress_threshold = 100.0;
    ColumnChunkReader<false, false> ccr_large_thresh(&reader, &cc, &fs, nullptr, 0, nullptr);
    ...
    size_t file_end = header_bytes.size() + compressed_fast.size();
    StoragePageCache::CacheKey key_large(path, /*file_end_offset*/ file_end, /*header_start*/ 0);
    bool found_large =
    ...
    ColumnChunkReader<false, false> ccr_check(&reader, &cc, &fs, nullptr, 0, nullptr);
    ASSERT_TRUE(ccr_check.init().ok());
    // ASSERT_TRUE(ccr_check.next_page().ok());
    ASSERT_TRUE(ccr_check.load_page_data().ok());
    EXPECT_EQ(ccr_check.statistics().page_cache_hit_counter, 1);
...
TEST(ParquetPageCacheTest, MultipleReadersShareCachedEntry) {
    ...
    StoragePageCache::CacheKey key(path, static_cast<size_t>(header_start + total), header_start);
    StoragePageCache::instance()->insert(key, page, &handle, segment_v2::DATA_PAGE);
