...
protected:
    ...
    const TupleDescriptor* createTupleDescriptor(DescriptorTbl** desc_tbl, ObjectPool& obj_pool,
...
// Mock implementation of IcebergTableReader for testing
class MockIcebergTableReader : public IcebergTableReader {
public:
    MockIcebergTableReader(std::unique_ptr<GenericReader> file_format_reader,
...
protected:
    ...
    std::unique_ptr<GenericReader> _create_equality_reader(const TFileRangeDesc& range) override {
        // For testing, just return nullptr
        return nullptr;
    }

    Status _read_position_delete_file(const TFileRangeDesc* range,
                                      DeleteFile* delete_file) override {
