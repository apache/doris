...
class TestScanner final : public vectorized::Scanner {
public:
    TestScanner(RuntimeState* state, pipeline::ScanLocalStateBase* local_state, int64_t limit,
                RuntimeProfile* profile)
            : vectorized::Scanner(state, local_state, limit, profile) {}

protected:
    Status _get_block_impl(RuntimeState* /*state*/, vectorized::Block* /*block*/,
                           bool* eof) override {
        *eof = true;
        return Status::OK();
    }
...
TEST_F(ScannerLateArrivalRfTest, applied_rf_num_advances_after_late_arrival) {
    ...
    auto op = std::make_shared<pipeline::MockScanOperatorX>();
    op->_runtime_filter_descs = rf_descs;
    op->_row_descriptor = row_desc;

    auto local_state =
            std::make_shared<pipeline::MockScanLocalState>(_runtime_states[0].get(), op.get());

    std::vector<std::shared_ptr<pipeline::Dependency>> rf_dependencies;
