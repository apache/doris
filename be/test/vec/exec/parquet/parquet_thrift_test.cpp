...
static void read_parquet_data_and_check(const std::string& parquet_file,
    ...
    EXPECT_TRUE(rst.ok());
    std::unique_ptr<uint8_t[]> result_buf(new uint8_t[result.size()]);
    int64_t bytes_read;
    bool eof;
    result.read(result_buf.get(), result.size(), &bytes_read, &eof);
    ASSERT_STREQ(block->dump_data(0, rows).c_str(), reinterpret_cast<char*>(result_buf.get()));
}
