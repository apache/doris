...
TEST(VGenericIteratorsTest, Union) {
    ...
    auto iter = vectorized::new_union_iterator(
            inputs, MemTracker::CreateTracker(-1, "VUnionIterator", nullptr, false));
    StorageReadOptions opts;
    auto st = iter->init(opts);
    EXPECT_TRUE(st.ok());
...
TEST(VGenericIteratorsTest, MergeAgg) {
    ...
    auto iter = vectorized::new_merge_iterator(
            inputs, MemTracker::CreateTracker(-1, "VMergeIterator", nullptr, false), -1, false,
            &merged_rows);
    StorageReadOptions opts;
    auto st = iter->init(opts);
    EXPECT_TRUE(st.ok());
...
TEST(VGenericIteratorsTest, MergeWithSeqColumn) {
    ...
    uint64_t merged_rows = 0;
    auto iter = vectorized::new_merge_iterator(
            inputs, MemTracker::CreateTracker(-1, "VMergeIterator", nullptr, false), seq_column_id,
            true, &merged_rows);
    StorageReadOptions opts;
    auto st = iter->init(opts);
    EXPECT_TRUE(st.ok());
