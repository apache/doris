...
TEST_F(JsonbDocumentTest, writer) {
    ...
    vectorized::Decimal256 decimal256_value((__int128_t(std::numeric_limits<uint64_t>::max())));
    writer.writeDecimal(decimal256_value, 30, 8);
    ...
    auto* root_obj = static_cast<ObjectVal*>(root);
    ...
    auto* jsonb_int_value = static_cast<JsonbIntVal*>(it->value());
    ASSERT_EQ(jsonb_int_value->val(), 12345);

    ++it;
    ASSERT_EQ(std::string_view(it->getKeyStr(), it->klen()), "key_float");
    ASSERT_TRUE(it->value()->isFloat());
    auto* jsonb_float_value = static_cast<JsonbFloatVal*>(it->value());
    ...
    auto* jsonb_string_value = static_cast<JsonbStringVal*>(it->value());
    ...
    auto* jsonb_array_value = static_cast<ArrayVal*>(it->value());
    ASSERT_EQ(jsonb_array_value->numElem(), 2);
    auto array_it = jsonb_array_value->begin();
    ASSERT_TRUE(array_it->isInt());
    ASSERT_EQ(static_cast<JsonbIntVal*>(static_cast<JsonbValue*>(array_it))->val(), 1);

    ++array_it;
    ASSERT_TRUE(array_it->isString());
    auto* array_string_value = static_cast<JsonbStringVal*>(static_cast<JsonbValue*>(array_it));
    ...
    auto* jsonb_int128_value = static_cast<JsonbInt128Val*>(it->value());
    ...
    auto* jsonb_decimal_value = static_cast<JsonbDecimal32*>(it->value());
    ...
    ASSERT_TRUE(it->value()->isDecimal());
    auto* jsonb_decimal64_value = static_cast<JsonbDecimal64*>(it->value());
    ...
    ASSERT_TRUE(it->value()->isDecimal());
    auto* jsonb_decimal128_value = static_cast<JsonbDecimal128*>(it->value());
    ...
    ASSERT_TRUE(it->value()->isDecimal());
    auto* jsonb_decimal256_value = static_cast<JsonbDecimal256*>(it->value());
    ASSERT_EQ(__int128_t(jsonb_decimal256_value->val()),
              __int128_t(__int128_t(std::numeric_limits<uint64_t>::max())));
    ASSERT_EQ(jsonb_decimal256_value->scale, 8);
    ASSERT_EQ(jsonb_decimal256_value->precision, 30);
    ...
    std::string expected_string(
            ...
            R"("key_decimal32":9999.9999,"key_decimal64":100000000000000,"key_decimal128":184467440737.096,)"
            R"("key_decimal256":184467440737.096})");
