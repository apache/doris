...
static DataTypeFloat32 dt_float;
static DataTypeFloat64 dt_double;
...
TEST_F(DataTypeNumberTest, get_field) {
    ...
    std::cout << "get field float\n";
    {
        ...
        EXPECT_EQ(dt_float.get_field(expr_node), value);
...
TEST_F(DataTypeNumberTest, ser_deser) {
    auto test_func = [](auto dt, const auto& column, int be_exec_version) {
        ...
        auto res_file_prefix = fmt::format("{}/datatype_{}_serialize_v{}", test_result_dir,
                                           dt.get_family_name(), be_exec_version);
        int test_index = 0;
        std::string res_file_path = fmt::format("{}_{}.out", res_file_prefix, test_index++);
        ...
        }
        {
            ...
            res_file_path = fmt::format("{}_{}.out", res_file_prefix, test_index++);
            check_or_generate_res_file(res_file_path, {{column_values}}, true);
...
TEST_F(DataTypeNumberTest, to_string) {
    auto test_func = [](auto& dt, const auto& column) {
        ...
        const auto* col_with_type = assert_cast<const ColumnType*>(&column);

        auto res_file_prefix =
                fmt::format("{}/datatype_{}_to_string", test_result_dir, dt.get_family_name());
        int test_index = 0;
        {
            ColumnString col_str;
            BufferWritable buffer(col_str);

            size_t row_count = column.size();
            for (size_t i = 0; i != row_count; ++i) {
                dt.to_string(column, i, buffer);
                buffer.commit();
            }
            std::vector<std::string> column_data;
            for (size_t i = 0; i != row_count; ++i) {
                column_data.push_back(col_str.get_data_at(i).to_string());
            }
            std::string res_file_path =
                    fmt::format("{}_{}_buffer.out", res_file_prefix, test_index++);
            check_or_generate_res_file(res_file_path, {column_data});
        }
        {
            size_t row_count = column.size();
            std::vector<std::string> column_data;
            for (size_t i = 0; i != row_count; ++i) {
                column_data.push_back(dt.to_string(column, i));
            }
            std::string res_file_path = fmt::format("{}_{}.out", res_file_prefix, test_index++);
            check_or_generate_res_file(res_file_path, {column_data});
        }
        {
            ...
            std::vector<std::string> column_data;
            for (size_t i = 0; i != row_count; ++i) {
                column_data.push_back(dt.to_string(col_with_type->get_element(i)));
            }
            std::string res_file_path =
                    fmt::format("{}_{}_value.out", res_file_prefix, test_index++);
            check_or_generate_res_file(res_file_path, {column_data});
        }
    };
