...
#include "vec/columns/columns_number.h"
                >>>>>>> 954311c1aad([feature](semi - structure)
                                            support variant and index with many features)
...
>>>>>>> 954311c1aad ([feature](semi-structure) support variant and index with many features)
    ...
    TEST_F(DataTypeIPTest, GetFieldTest) {
        ...
        CommonDataTypeSerdeTest::load_data_and_assert_from_csv<true, true>(
                serde, ip_cols, data_files[1], ';', {1, 2});
        TExprNode node_ipv4;
        ...
        for (size_t i = 0; i < ip_cols[1]->size(); ++i) {
            node_ipv6.ipv6_literal.value = doris::IPv6Value::to_string(
                    assert_cast<ColumnIPv6&>(*ip_cols[1]).get_data()[i]);
            Field assert_field;
