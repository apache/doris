...
TEST(function_string_test, function_unicode_normalize_modes_and_trim) {
    ...
    // Second argument (mode) must be constant for unicode_normalize
    // Use ConstedNotnull because the test framework has issues with nullable const columns
    // and get_arguments_that_are_always_constant check
    InputTypeSet input_types = {Notnull {TypeIndex::String}, ConstedNotnull {TypeIndex::String}};
...

    {
        ...
        static_cast<void>(check_function<DataTypeString, false>(func_name, input_types, data_set));
    }
...
TEST(function_string_test, function_unicode_normalize_mode_not_const) {
    ...
    // Second argument (mode) is NOT constant - should fail
    // Use Notnull to avoid nullable const column issues in test framework
    InputTypeSet input_types = {Notnull {TypeIndex::String}, Notnull {TypeIndex::String}};

    DataSet data_set = {
            {{std::string("abc"), std::string("NFC")}, std::string("abc")},
    };

    // expect_fail=true because mode must be constant
    static_cast<void>(
            check_function<DataTypeString, false>(func_name, input_types, data_set, true));
}

TEST(function_string_test, function_unicode_normalize_invalid_mode) {
    std::string func_name = "unicode_normalize";

    // Second argument (mode) must be constant
    // Use ConstedNotnull to avoid nullable const column issues in test framework
    InputTypeSet input_types = {Notnull {TypeIndex::String}, ConstedNotnull {TypeIndex::String}};
    ...
    // expect_fail=true because INVALID_MODE is not a valid normalization form
    static_cast<void>(
            check_function<DataTypeString, false>(func_name, input_types, data_set, true));
}
