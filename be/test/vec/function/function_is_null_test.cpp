...
TEST_F(FunctionIsNullTest, gc_binlogs_test) {
    ...
    for (int i = 0; i < rowset->num_segments(); i++) {
        ...
        EXPECT_TRUE(index_meta[0]);
        auto bkd_reader = BkdIndexReader::create_shared(index_meta[0], index_file_reader);
        ...
        EXPECT_TRUE(index_meta2[0]);
        auto string_reader =
                StringTypeInvertedIndexReader::create_shared(index_meta2[0], index_file_reader);
...
TEST_F(FunctionIsNullTest, evaluate_inverted_index_corner_cases) {
    ...
    vectorized::Field key1 = vectorized::Field(10);
    vectorized::Field key2 = vectorized::Field(20);
    vectorized::Field key3 = vectorized::Field(30);
    vectorized::Field v1 = vectorized::Field("value1");
    vectorized::Field v2 = vectorized::Field("value2");
    vectorized::Field v3 = vectorized::Field("value3");
    ...
    for (int i = 0; i < rowset->num_segments(); i++) {
        ...
        const auto* index_meta = index_metas[0];
        auto bkd_reader = BkdIndexReader::create_shared(index_meta, index_file_reader);
        EXPECT_TRUE(bkd_reader);

        std::unique_ptr<InvertedIndexIterator> iter;
        EXPECT_TRUE(bkd_reader->new_iterator(io_ctx, &stats, &runtime_state, &iter).ok());
        EXPECT_TRUE(iter);
