...
// static std::unique_ptr<std::ofstream> regression_test_const_case_file_uptr;
// static std::unique_ptr<std::ofstream> regression_test_const_case_expected_result_file_uptr;

// static std::string regression_case_test_table_template;
struct FunctionCastToDecimalTest : public FunctionCastTest {
    ...
    static void SetUpTestSuite() {
        if (FLAGS_gen_regression_case) {
            std::string from_str_const_case_name = "test_cast_str_to_decimal_const";
            auto regression_case_file_path_test_const =
                    fmt::format("{}/{}/{}.groovy", std::string(getenv("ROOT")), regression_case_dir,
                                from_str_const_case_name);
            std::string template_file_path = fmt::format(
                    "{}/be/test/vec/function/cast/"
                    "test_cast_string_to_decimal_const_template.groovy",
                    std::string(getenv("ROOT")));
            try {
                std::filesystem::copy_file(template_file_path, regression_case_file_path_test_const,
                                           std::filesystem::copy_options::overwrite_existing);
            } catch (const std::filesystem::filesystem_error& e) {
                throw std::runtime_error(fmt::format("Failed to copy file: {}, reason: {}",
                                                     template_file_path, std::string(e.what())));
            }
            regression_test_const_case_file_uptr = std::make_unique<std::ofstream>(
                    regression_case_file_path_test_const, std::ios_base::out | std::ios_base::app);
            if (!regression_test_const_case_file_uptr->is_open()) {
                throw std::runtime_error(
                        fmt::format("Cannot open file: {}", regression_case_file_path_test_const));
            }
            (*regression_test_const_case_file_uptr) << regression_notes;

            // (*regression_test_const_case_file_uptr) << kApacheLicenseHeader;
            // (*regression_test_const_case_file_uptr)
            //         << fmt::format("\n\nsuite(\"{}\") {{\n", from_str_const_case_name);

            auto regression_case_file_path_test_const_expected_result =
                    fmt::format("{}/{}/{}.out", std::string(getenv("ROOT")),
                                regression_expected_result_dir, from_str_const_case_name);
            regression_test_const_case_expected_result_file_uptr = std::make_unique<std::ofstream>(
                    regression_case_file_path_test_const_expected_result,
                    std::ios_base::out | std::ios_base::trunc);
            if (!regression_test_const_case_expected_result_file_uptr->is_open()) {
                throw std::runtime_error(
                        fmt::format("Cannot open file: {}",
                                    regression_case_file_path_test_const_expected_result));
            }
            (*regression_test_const_case_expected_result_file_uptr)
    ...
    static void TearDownTestSuite() {
        if (FLAGS_gen_regression_case) {
            if (regression_test_const_case_file_uptr &&
                regression_test_const_case_file_uptr->is_open()) {
                (*regression_test_const_case_file_uptr) << R"(})";
            }
        }
    }
    ...
    */
    void setup_regression_case_output(const std::string& case_name,
        ...
        //         std::string(getenv("ROOT")));
        // try {
        //     std::filesystem::copy_file(template_file_path, file_path,
        //                                std::filesystem::copy_options::overwrite_existing);
        // } catch (const std::filesystem::filesystem_error& e) {
        //     throw std::runtime_error(fmt::format("Failed to copy file: {}, reason: {}",
        //                                          template_file_path, std::string(e.what())));
        // }
        // ofs_case_const =
        //         std::make_unique<std::ofstream>(file_path, std::ios_base::out | std::ios_base::app);
    ...
    void from_string_test_func(std::ofstream* ofs_const_case,
        ...
        // std::string const_test_sqls;
        std::vector<std::string> const_test_strs;
        // std::string const_test_expected_results;
        ...
        int data_index = test_data_index;
        Defer defer {[&]() {
            if (FLAGS_gen_regression_case) {
                // (*ofs_const_case) << const_test_sqls;
                // (*ofs_const_expected_result) << const_test_expected_results;
                auto const_test_with_strict_arg = [&](bool enable_strict_cast) {
                    ...
                    for (int i = 0; i != value_count; ++i, ++data_index) {
                        (*ofs_const_case) << fmt::format(
                                "    qt_sql_{}_{} \"\"\"select cast(\"{}\" as decimalv3({}, "
                                "{}));\"\"\"\n",
                                data_index, enable_strict_cast ? "strict" : "non_strict",
                                const_test_strs[i], Precision, Scale);
                        (*ofs_const_expected_result)
                                << fmt::format("-- !sql_{}_{} --\n", data_index,
                                               enable_strict_cast ? "strict" : "nonstrict");
                        (*ofs_const_expected_result)
                ...
                auto table_test_with_strict_arg = [&](bool enable_strict_cast) {
                    ...
                    (*ofs_case) << fmt::format(
                            ...
                            "order by "
                            "1;'\n\n",
                            table_index, enable_strict_cast ? "strict" : "nonstrict", Precision,
                            Scale, table_name);
        ...
        auto add_regression_cases = [&](const std::string& v_str,
            ...
            // const_test_sqls += fmt::format(
            //         "    qt_sql_{} \"\"\"select cast(\"{}{}\" as decimalv3({}, "
            //         "{}));\"\"\"\n",
            //         test_data_index, is_negative ? "-" : "", v_str, Precision, Scale);
            const_test_strs.emplace_back(is_negative ? "-" : "" + v_str);
            // const_test_expected_results += fmt::format("-- !sql_{} --\n", test_data_index);
            // const_test_expected_results += fmt::format("{}\n\n", expected_result);
    ...
    void from_string_to_decimal32_test_func() {
        ...
        std::string regression_case_name = "test_cast_str_to_decimal32";
        if (FLAGS_gen_regression_case) {
        ...
        from_string_test_func<Decimal32, 1, 0>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
        from_string_test_func<Decimal32, 1, 1>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
        from_string_test_func<Decimal32, 9, 0>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;

        from_string_test_func<Decimal32, 9, 1>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
        ...
        from_string_test_func<Decimal32, 9, 3>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
        ...
        from_string_test_func<Decimal32, 9, 8>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
        ...
        from_string_test_func<Decimal32, 9, 9>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                               ofs_expected_result, regression_case_name,
                                               table_index, test_data_index);
        ++table_index;
    ...
    void from_string_to_decimal64_test_func() {
        ...
        std::unique_ptr<std::ofstream> ofs_const_case_uptr, ofs_const_expected_result_uptr;
        std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
        std::string regression_case_name = "test_cast_str_to_decimal64";
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr);
        ...
        from_string_test_func<Decimal64, 18, 0>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                                ofs_expected_result, regression_case_name,
                                                table_index, test_data_index);
        ++table_index;

        from_string_test_func<Decimal64, 18, 1>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                                ofs_expected_result, regression_case_name,
                                                table_index, test_data_index);
        ++table_index;
        ...
        from_string_test_func<Decimal64, 18, 9>(ofs_const_case, ofs_const_expected_result, ofs_case,
                                                ofs_expected_result, regression_case_name,
                                                table_index, test_data_index);
        ++table_index;
    ...
    void from_string_to_decimal128v3_test_func() {
        ...
        std::unique_ptr<std::ofstream> ofs_const_case_uptr, ofs_const_expected_result_uptr;
        std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
        std::string regression_case_name = "test_cast_str_to_decimal128v3";
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr);
    ...
    void from_string_to_decimal256_test_func() {
        ...
        std::unique_ptr<std::ofstream> ofs_const_case_uptr, ofs_const_expected_result_uptr;
        std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
        std::string regression_case_name = "test_cast_str_to_decimal256";
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr);
    ...
    void from_string_overflow_test_func(std::ofstream* ofs_const_case,
        ...
        constexpr auto max_integral =
                decimal_scale_multiplier<typename T::NativeType>(Precision - Scale) - 1;
        constexpr auto min_overflow_int_part = max_integral + 1;
        ...
        test {{
            ...
            exception "{}"
                        ...
                        for (int i = 0; i != value_count; ++i) {
                            ...
                            //         "decimalv3({}, "
                            //         "{}));\"\"\"\n",
                            //         data_index, overflow_strs[i], Precision, Scale);
                            (*ofs_const_expected_result)
                                    << fmt::format("-- !sql_{} --\n", table_name);
                ...
                std::visit(
                        [&](auto enable_strict_cast) {
                            if constexpr (enable_strict_cast) {
                                ...
                                EXPECT_THROW(
                                        (check_function_for_cast<DataTypeDecimal<T>, Scale,
...
TEST_F(FunctionCastToDecimalTest, test_from_string_overflow) {
    from_string_to_decimal32_overflow_test_func();
    /*
    from_string_overflow_test_func<Decimal32, 1, 0>();
    from_string_overflow_test_func<Decimal32, 1, 1>();
    from_string_overflow_test_func<Decimal32, 9, 0>();
