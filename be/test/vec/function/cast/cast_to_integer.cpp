...
struct FunctionCastToIntTest : public FunctionCastTest {
    ...
    void from_string_test_func() {
        ...
        */
        if (FLAGS_gen_regression_case) {
            ...
            if constexpr (PType == TYPE_TINYINT) {
                cast_type_name = "tinyint";
            } else if constexpr (PType == TYPE_SMALLINT) {
                cast_type_name = "smallint";
            } else if constexpr (PType == TYPE_INT) {
                cast_type_name = "int";
            } else if constexpr (PType == TYPE_BIGINT) {
                cast_type_name = "bigint";
            } else if constexpr (PType == TYPE_LARGEINT) {
                cast_type_name = "largeint";
            }
    ...
    void from_string_with_fraction_part_test_func() {
        ...
        if (FLAGS_gen_regression_case) {
            ...
            std::string regression_case_name =
                    fmt::format("test_cast_str_to_{}_with_fraction", cast_type_name);
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr);
            auto* ofs_const_case = ofs_const_case_uptr.get();
        ...
        test {{
            ...
            exception "{}"
                    ...
                    for (int i = 0; i != value_count; ++i) {
                        ...
                        (*ofs_const_case) << fmt::format(
                                "    qt_sql_{}_{} \"\"\"select \"{}\", cast(\"{}\" as {});\"\"\"\n",
                                i, "non_strict", v_str, v_str, cast_type_name);
                        (*ofs_const_expected_result)
                                << fmt::format("-- !sql_{}_{} --\n", i, "non_strict");
                        (*ofs_const_expected_result)
                                << fmt::format("{}\t{}\n\n", v_str, data_pairs[i].second);
    ...
    void from_string_overflow_test_func() {
        ...
        std::string dbg_str0 =
                fmt::format("test cast from string to {} overflow", TypeName<T>::get());
        ...
        test {{
            ...
            exception "{}"
                    ...
                    for (int i = 0; i != value_count; ++i) {
                        const auto& v_str = fmt::format("{}", test_vals[i]);
                        (*ofs_const_case) << fmt::format(
                                "    qt_sql_{}_{} \"\"\"select \"{}\", cast(\"{}\" as {});\"\"\"\n",
                                i, "non_strict", v_str, v_str, cast_type_name);
                        (*ofs_const_expected_result)
                                << fmt::format("-- !sql_{}_{} --\n", i, "non_strict");
                        (*ofs_const_expected_result) << fmt::format("{}\t\\N\n\n", v_str);
                    }
                }
            };
            const_test_with_strict_arg(true);
    ...
    void from_string_invalid_test_func() {
        ...
        std::vector<std::string> test_vals;
        std::string dbg_str0 =
                fmt::format("test cast from string to {} abnormal cases", TypeName<T>::get());
