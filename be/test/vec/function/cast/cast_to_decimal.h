...
struct FunctionCastToDecimalTest : public FunctionCastTest {
    ...
    void from_string_invalid_input_test_func() {
        ...
        std::unique_ptr<std::ofstream> ofs_const_case_uptr, ofs_const_expected_result_uptr;
        std::unique_ptr<std::ofstream> ofs_case_uptr, ofs_expected_result_uptr;
        std::string regression_case_name = "test_cast_str_to_decimal_invalid_input";
        if (FLAGS_gen_regression_case) {
            setup_regression_case_output(regression_case_name, ofs_const_case_uptr,
                                         ofs_const_expected_result_uptr, ofs_case_uptr,
                                         ofs_expected_result_uptr);
    ...
    template <PrimitiveType FromPT, typename ToT, int Precision, int Scale>
    void from_int_overflow_test_func_() {
        using FromT = typename PrimitiveTypeTraits<FromPT>::CppType;
        static_assert(std::numeric_limits<FromT>::is_integer, "FromT must be an integer type");
        if constexpr (NumberTraits::max_ascii_len<FromT>() > Precision - Scale) {
            DataTypeNumber<FromPT> dt_from;
    ...
    template <PrimitiveType FromPT, typename T, int Precision, int Scale>
    void from_float_double_overflow_test_func() {
        using FromT = typename PrimitiveTypeTraits<FromPT>::CppType;
        static_assert(std::numeric_limits<FromT>::is_iec559, "FromT must be a floating point type");
        DataTypeNumber<FromPT> dt_from;
        InputTypeSet input_types = {dt_from.get_primitive_type()};
    ...
    void between_decimal_with_precision_and_scale_overflow_test_func() {
        ...
        if constexpr (may_overflow) {
            ...
            for (const auto& i : from_integral_part) {
                FromT from_decimal_num {};
                if (i == to_max_integral && ToScale >= FromScale) {
                    // cases: from 999999.999
                    //              xxxxx.xxx
                    //              xxxxx.xxxxx
                    continue;
                }
                from_decimal_num = from_decimal_ctor(i, from_max_fractional, FromScale);
    ...
    void between_decimals_with_to_p_and_s_overflow_test_func() {
        ...
        static_assert(from_min_decimal_p == 1 || from_min_decimal_p > 9);
        auto aaa = [&]<int test_from_precision>() {
            between_decimal_with_precision_and_scale_overflow_test_func<
                    FromT, test_from_precision, 0, ToT, ToPrecision, ToScale>();
            between_decimal_with_precision_and_scale_overflow_test_func<
                    FromT, test_from_precision, 1, ToT, ToPrecision, ToScale>();
            if constexpr (test_from_precision != 1) {
                between_decimal_with_precision_and_scale_overflow_test_func<
                        FromT, test_from_precision, test_from_precision / 2, ToT, ToPrecision,
                        ToScale>();
                between_decimal_with_precision_and_scale_overflow_test_func<
                        FromT, test_from_precision, test_from_precision - 1, ToT, ToPrecision,
                        ToScale>();
                between_decimal_with_precision_and_scale_overflow_test_func<
                        FromT, test_from_precision, test_from_precision, ToT, ToPrecision,
                        ToScale>();
            }
        };
        aaa.template operator()<from_min_decimal_p>();
