...
TEST_F(FunctionCastToDecimalTest, test_to_decimal32_from_float_overflow) {
    ...
    from_float_double_overflow_test_func<TYPE_FLOAT, Decimal32, 9, 0>(
            table_index++, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
            ofs_const_expected_result);
    from_float_double_overflow_test_func<TYPE_FLOAT, Decimal32, 9, 3>(
            table_index++, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
            ofs_const_expected_result);
    from_float_double_overflow_test_func<TYPE_FLOAT, Decimal32, 9, 8>(
            table_index++, test_data_index, ofs_case, ofs_expected_result, ofs_const_case,
            ofs_const_expected_result);
    if (FLAGS_gen_regression_case) {
        (*ofs_const_case) << "}";
