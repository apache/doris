...
// Test for FunctionJsonExtractStringFromVarchar - the on-demand parsing optimization
// that takes VARCHAR/STRING input instead of JSONB.
// NOTE: This function uses simdjson's get_object() which requires the root to be a JSON object.
// For JSON primitives (null, true, false, numbers, strings) and arrays at root level,
...
TEST(FunctionJsonbTEST, JsonExtractStringFromVarcharAliasTest) {
    InputTypeSet input_types = {Nullable {PrimitiveType::TYPE_VARCHAR},
                                Nullable {PrimitiveType::TYPE_VARCHAR}};
    DataSet data_set = {
            {{STRING(R"({"k1":"v31", "k2": 300})"), STRING("$.k1")}, STRING("v31")},
