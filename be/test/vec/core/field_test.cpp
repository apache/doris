...
TEST(FieldTest, Test_Field_Date) {
    ...
    // Field field_Date(Int64(*reinterpret_cast<__int64_t*>(&a))); // Warning: undefined behavior
    Field field_Date(a.to_int64());

    // VecDateTimeValue b = binary_cast<Int64, VecDateTimeValue>(get<Int64>(field_Date)); // Warning: undefined behavior
    VecDateTimeValue b;
    if (!b.from_date_int64(get<Int64>(field_Date))) {
        EXPECT_EQ(1, 2);
    }
    b.cast_to_date();
