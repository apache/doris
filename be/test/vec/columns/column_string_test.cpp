...
TEST_F(ColumnStringTest, check_chars_length) {
    ...
    ColumnString::check_chars_length(0xffffffff - 1, 0);
    ColumnString::check_chars_length(0xffffffff, 0);
    EXPECT_THROW(ColumnString::check_chars_length(0xffffffffUL + 1, 0), Exception);
}
...
TEST_F(ColumnStringTest, insert_many_strings_without_reserve) {
    auto test_func = [&](size_t clone_count, auto x, const auto& source_column) {
        ...
        for (size_t i = 0; i != actual_clone_count; ++i) {
            std::cout << "row: " << i << ": " << col_vec_target->get_data_at(i).to_string() << "\n";
    ...
    auto test_func = [&](size_t clone_count, auto x, const auto& source_column) {
        ...
        for (size_t i = 0; i != sel_size; ++i) {
            ...
            if (value.size > max_value_length) {
                max_value_length = value.size;
            }
        }
        col_vec_target->insert_many_strings_overflow(strings.data(), sel_size, max_value_length);
        EXPECT_EQ(col_vec_target->size(), actual_clone_count + sel_size);
        for (size_t i = 0; i != actual_clone_count; ++i) {
            auto real_data = col_vec_target->get_data_at(i);
            auto expect_data = source_column->get_data_at(i);
...
TEST_F(ColumnStringTest, permute) {
    ...
    assert_permute(columns, column_str32->size());
...
TEST_F(ColumnStringTest, get_permutation) {
    assert_column_permutations(*column_str32, dt_str);
    assert_column_permutations(*column_str64, dt_str);
}
