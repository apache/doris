...
public:
    ...
    void SetUp() override {
        ...
        runtime_state = std::make_unique<RuntimeState>(query_globals);
...
TEST_F(ParquetExprTest, test_expr_zonemap_row_group_filter_skips_complex_parent_column) {
    auto* col_schema =
            const_cast<FieldSchema*>(p_reader->_file_metadata->schema().get_column("string_col"));
...
TEST_F(ParquetExprTest, test_expr_zonemap_row_group_filter_skips_type_mismatch_column) {
    auto* col_schema =
            const_cast<FieldSchema*>(p_reader->_file_metadata->schema().get_column("int64_col"));
    ASSERT_NE(nullptr, col_schema);
