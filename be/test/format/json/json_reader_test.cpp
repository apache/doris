...
TEST(NewJsonReaderCowTest, TruncateBlockToRowsAfterScopedColumnRestore) {
    auto first_column = ColumnInt32::create();
    first_column->insert_value(1);
    first_column->insert_value(2);
    auto second_column = ColumnInt32::create();
    second_column->insert_value(10);
    second_column->insert_value(20);

    Block block;
    block.insert({std::move(first_column), std::make_shared<DataTypeInt32>(), "c0"});
    block.insert({std::move(second_column), std::make_shared<DataTypeInt32>(), "c1"});

    auto column_guard = block.mutate_column_scoped(1);
    ASSERT_EQ(block.get_by_position(1).column.get(), nullptr);
    assert_cast<ColumnInt32&>(*column_guard.mutable_column()).insert_value(30);

    column_guard.restore();
    ASSERT_NE(block.get_by_position(1).column.get(), nullptr);

    json_reader_detail::truncate_block_to_rows(block, 2);

    ASSERT_EQ(block.rows(), 2);
    const auto& first_result = assert_cast<const ColumnInt32&>(*block.get_by_position(0).column);
    const auto& second_result = assert_cast<const ColumnInt32&>(*block.get_by_position(1).column);
    EXPECT_EQ(first_result.get_data()[0], 1);
    EXPECT_EQ(first_result.get_data()[1], 2);
    EXPECT_EQ(second_result.get_data()[0], 10);
    EXPECT_EQ(second_result.get_data()[1], 20);
}
