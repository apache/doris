...
TEST_F(VRuntimeFilterWrapperSamplingTest, deep_clone_shares_profile_counters) {
    ...
    impl->add_child(slot);
    auto wrapper = VRuntimeFilterWrapper::create_shared(node, impl, 0.4, false, /*filter_id=*/1);
    ...
    auto* cloned_wrapper = dynamic_cast<VRuntimeFilterWrapper*>(cloned_expr.get());
