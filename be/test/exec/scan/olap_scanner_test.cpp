...
TEST(OlapScannerTest, CandidateKeyBudgetIsSharedAcrossSiblingScanners) {
    SeqMapCandidateKeyBudget budget(3);

    EXPECT_TRUE(budget.try_consume(2));
    EXPECT_EQ(1, budget.remaining());
...
TEST(OlapScannerTest, CandidateCostPricesEverySegmentLookup) {
    ...
    EXPECT_EQ(3, cost_limit.segment_count);
...
TEST(OlapScannerTest, CandidateCostSaturatesOnOverflow) {
    ...
    EXPECT_EQ(std::numeric_limits<size_t>::max(), cost_limit.segment_count);
    EXPECT_EQ(std::numeric_limits<size_t>::max(), cost_limit.point_probe_cost_per_key);
}

TEST(OlapScannerTest, CandidatePointRangeEstimatePricesEveryKeyAndSegment) {
    EXPECT_EQ(32 * 2 * 3, OlapScanner::_estimate_point_range_bytes(2, 3));
    EXPECT_EQ(std::numeric_limits<size_t>::max(),
              OlapScanner::_estimate_point_range_bytes(std::numeric_limits<size_t>::max(), 2));
}
...
TEST(OlapScannerTest, CandidateNgramPatternMustContainCompleteGram) {
    EXPECT_TRUE(OlapScanner::_has_usable_ngram_bf_pattern("%have%", 3));
    EXPECT_FALSE(OlapScanner::_has_usable_ngram_bf_pattern("%ha%", 3));
    EXPECT_FALSE(OlapScanner::_has_usable_ngram_bf_pattern("%h_ve%", 3));
}
...
TEST(OlapScannerTest, CandidateStatsMergeFullFileCacheAccounting) {
    ...
    candidate_stats.file_cache_stats.inverted_index_bytes_read_from_remote = 4;
    candidate_stats.file_cache_stats.segment_footer_index_bytes_read_from_peer = 5;
    candidate_stats.file_cache_stats.bytes_read_from_cross_cg_peer = 6;
    candidate_stats.file_cache_stats.inverted_index_read_bytes = 27;
    candidate_stats.file_cache_stats.segment_footer_index_bytes_read_from_local = 8;
    candidate_stats.file_cache_stats.segment_footer_index_bytes_read_from_remote = 9;
    ...
    EXPECT_EQ(63, total_stats.seq_map_candidate_scan_bytes);
    ...
    EXPECT_EQ(4, total_stats.file_cache_stats.inverted_index_bytes_read_from_remote);
    EXPECT_EQ(5, total_stats.file_cache_stats.segment_footer_index_bytes_read_from_peer);
    EXPECT_EQ(6, total_stats.file_cache_stats.bytes_read_from_cross_cg_peer);
    EXPECT_EQ(27, total_stats.file_cache_stats.inverted_index_read_bytes);
    EXPECT_EQ(8, total_stats.file_cache_stats.segment_footer_index_bytes_read_from_local);
    EXPECT_EQ(9, total_stats.file_cache_stats.segment_footer_index_bytes_read_from_remote);
