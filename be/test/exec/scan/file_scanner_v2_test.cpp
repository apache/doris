...
TEST(FileScannerV2Test, WrappedLakeSplitsAreSupportedWhereverThePaimonOnesAre) {
    ...
    // A wrapped split whose paimon payload names a reader other than the Java one stays on the V1
    // fallback, which has no fluss_union branch: a clean refusal rather than a lake half read
    // without its suppression.
    auto non_java_split = range_with_format("fluss_union", TFileFormatType::FORMAT_JNI);
