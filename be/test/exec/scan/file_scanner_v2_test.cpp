...
TEST(FileScannerV2Test, FileCacheStatisticsArePublishedToScannerProfile) {
    ...
    file_cache_statistics.segment_footer_index_num_local_io_total = 23;
    file_cache_statistics.segment_footer_index_num_remote_io_total = 29;
    file_cache_statistics.segment_footer_index_num_peer_io_total = 31;
    file_cache_statistics.segment_footer_index_bytes_read_from_local = 37;
    file_cache_statistics.segment_footer_index_bytes_read_from_remote = 41;
    file_cache_statistics.segment_footer_index_bytes_read_from_peer = 43;
    file_cache_statistics.segment_footer_index_local_io_timer = 47;
    file_cache_statistics.segment_footer_index_remote_io_timer = 53;
    file_cache_statistics.segment_footer_index_peer_io_timer = 59;
    file_cache_statistics.num_cross_cg_peer_io_total = 61;
    file_cache_statistics.bytes_read_from_cross_cg_peer = 67;
    file_cache_statistics.cross_cg_peer_io_timer = 71;
    file_cache_statistics.num_same_cg_peer_io_total = 73;
    file_cache_statistics.bytes_read_from_same_cg_peer = 79;
    file_cache_statistics.same_cg_peer_io_timer = 83;
    file_cache_statistics.num_peer_race_peer_win = 89;
    file_cache_statistics.num_peer_race_s3_win = 97;
    file_cache_statistics.num_peer_lazy_fetch = 101;
    file_cache_statistics.peer_lazy_fetch_timer = 103;
    ...
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexNumLocalIOTotal")->value(), 23);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexNumRemoteIOTotal")->value(), 29);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexNumPeerIOTotal")->value(), 31);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexBytesScannedFromCache")->value(), 37);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexBytesScannedFromRemote")->value(), 41);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexBytesScannedFromPeer")->value(), 43);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexLocalIOUseTimer")->value(), 47);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexRemoteIOUseTimer")->value(), 53);
    EXPECT_EQ(profile.get_counter("SegmentFooterIndexPeerIOUseTimer")->value(), 59);
    EXPECT_EQ(profile.get_counter("CrossCGPeerIOTotal")->value(), 61);
    EXPECT_EQ(profile.get_counter("CrossCGPeerBytesRead")->value(), 67);
    EXPECT_EQ(profile.get_counter("CrossCGPeerIOTime")->value(), 71);
    EXPECT_EQ(profile.get_counter("SameCGPeerIOTotal")->value(), 73);
    EXPECT_EQ(profile.get_counter("SameCGPeerBytesRead")->value(), 79);
    EXPECT_EQ(profile.get_counter("SameCGPeerIOTime")->value(), 83);
    EXPECT_EQ(profile.get_counter("PeerRaceWin")->value(), 89);
    EXPECT_EQ(profile.get_counter("S3RaceWin")->value(), 97);
    EXPECT_EQ(profile.get_counter("PeerLazyFetch")->value(), 101);
    EXPECT_EQ(profile.get_counter("PeerLazyFetchTime")->value(), 103);
