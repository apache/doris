...
TEST_F(ScanOperatorTest, BinlogMergePushesOnlyKeyCommonExpr) {
    TPlanNode tnode;
    tnode.row_tuples.push_back(TTupleId(0));
    tnode.row_tuples.push_back(TTupleId(1));
    auto scan_operator = std::make_unique<OlapScanOperatorX>(obj_pool.get(), tnode, 0, *descs, 1,
                                                             TQueryCacheParam {});
    // A DUP table treats every slot as a key. Binlog merge scans must ignore that and fall back
    // to the real key list because BlockReader still merges rows above SegmentIterator.
    scan_operator->_olap_scan_node.keyType = TKeysType::DUP_KEYS;
    scan_operator->_olap_scan_node.__set_key_column_name({"k1"});
    auto local_state = OlapScanLocalState::create_unique(state.get(), scan_operator.get());

    auto key_slot = VSlotRef::create_shared(1, 0, 0, std::make_shared<DataTypeInt32>(), "k1");
    auto value_slot = VSlotRef::create_shared(2, 0, 1, std::make_shared<DataTypeInt32>(), "v1");
    for (const auto scan_type : {TBinlogScanType::MIN_DELTA, TBinlogScanType::DETAIL}) {
        ...
        EXPECT_TRUE(local_state->_should_push_down_common_expr(key_slot));
        EXPECT_FALSE(local_state->_should_push_down_common_expr(value_slot));
        EXPECT_TRUE(local_state->_should_push_down_late_runtime_filter(key_slot));
        EXPECT_FALSE(local_state->_should_push_down_late_runtime_filter(value_slot));
    }
}
