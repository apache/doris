...
TEST(PredicateLmUtilsTest, ParseEmptyString) {
    ...
    Status st = parse_predicate_lm_stage1_cols_to_column_ids("", schema, "db", "tbl", &cids);
...
TEST(PredicateLmUtilsTest, ParseBasicColumns) {
    ...
    std::vector<ColumnId> cids;
    Status st = parse_predicate_lm_stage1_cols_to_column_ids("a,b", schema, "db", "tbl", &cids);
    EXPECT_TRUE(st.ok()) << st.to_string();
...
TEST(PredicateLmUtilsTest, ParseUnknownColumnsShouldBeIgnored) {
    auto schema = build_tablet_schema_for_test();
...
TEST(PredicateLmUtilsTest, ParseTableQualifiedColumns) {
    auto schema = build_tablet_schema_for_test();
...
TEST(PredicateLmUtilsTest, ParseDbTableQualifiedColumns) {
    auto schema = build_tablet_schema_for_test();
...
TEST(PredicateLmUtilsTest, ParseDbTableQualifiedColumnsWithCatalogPrefixedDbName) {
    auto schema = build_tablet_schema_for_test();
...
TEST(PredicateLmUtilsTest, ParseQualifiedColumnsWithRollupSuffixInCurrentTableName) {
    auto schema = build_tablet_schema_for_test();
