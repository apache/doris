...
void set_lance_fetch_profile(PMultiGetBlockV2* response_block, int64_t scale) {
    ...
    profile.add_info_string("LanceRowIdReadTime", std::to_string(2 * scale) + "ns");
...
TEST_F(MaterializationSharedStateTest, TestMergeMultiResponse) {
    ...
    EXPECT_EQ("20ns, ", fmt::to_string(backend1_info.at("LanceRowIdReadTime")));
    ...
    EXPECT_EQ("2ns, ", fmt::to_string(backend2_info.at("LanceRowIdReadTime")));
