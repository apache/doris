...
TEST_F(StreamingAggOperatorTest, require_hash_shuffle_after_non_hash_local_exchange) {
    state->_query_options = create_fake_query_options();
    state->_query_options.__set_enable_local_exchange_before_agg(false);
    state->_query_options.__set_shuffled_agg_ids({op->node_id()});
    state->_query_ctx_uptr =
            MockQueryContext::create(TUniqueId(), ExecEnv::GetInstance(), state->_query_options);
    state->_query_ctx_uptr->set_mock_ai_resource();
    state->_query_ctx = state->_query_ctx_uptr.get();
    op->_needs_finalize = false;
