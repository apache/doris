...
inline void enable_row_binlog(TCreateTabletReq* request, int32_t row_binlog_schema_hash = 0) {
    ...
    row_binlog_schema.columns.push_back(
            create_tablet_column({std::string(kRowBinlogLsnColName), TPrimitiveType::BIGINT,
    ...
    row_binlog_schema.columns.push_back(
            create_tablet_column({std::string(kRowBinlogTsoColName), TPrimitiveType::BIGINT,
                                  false, true, TAggregationType::NONE, true}));
