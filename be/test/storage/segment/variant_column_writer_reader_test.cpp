...
protected:
    ...
    Status append_json_batch(ColumnWriter* writer, const std::vector<std::string>& jsons) {
        ...
        Block block = _tablet_schema->create_block();
        auto columns = block.mutate_columns();
        auto variant_col = ColumnVariant::create(_tablet_schema->column(0).variant_max_subcolumns_count());
        auto json_col = ColumnString::create();
    ...
    Status read_root_rows(const SegmentFooterPB& footer, const std::string& file_path,
        ...
        RETURN_IF_ERROR(create_variant_root_reader(footer, file_reader, _tablet_schema,
                                                   &column_reader));
...
TEST_F(VariantColumnWriterReaderTest,
    ...
    input_rowsets.push_back(create_variant_rowset(
            {{R"({"session_id": 1, "tags": ["a"], "score": 10})",
              R"({"session_id": 2, "tags": []})"}},
            1));
    input_rowsets.push_back(create_variant_rowset(
            {{R"({"score": 30})", R"({"session_id": 4})"}}, 2));
    ...
    EXPECT_EQ(collect_regular_paths(plan),
              std::set<std::string>({"score", "session_id", "tags"}));
...
TEST_F(VariantColumnWriterReaderTest,
    ...
    const std::vector<std::string> batch1 = {
            R"({"session_id": 1, "tags": ["topic_1", "topic_2"]})",
            R"({"session_id": 2, "tags": []})"};
    const std::vector<std::string> batch2 = {
            R"({"session_id": 3})", R"({"session_id": 4, "tags": [null, "topic_4"]})"};
