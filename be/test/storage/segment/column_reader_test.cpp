...
public:
    ...
    Result<TColumnAccessPaths> process_sub_access_paths(const TColumnAccessPaths& access_paths,
                                                        bool is_predicate) {
        return _normalize_access_paths(access_paths, is_predicate);
    }

    void append_deferred_defaults(MutableColumnPtr& dst, size_t count) {
...
TEST_F(ColumnReaderTest, ReadPhaseMatrix) {
    ...
    iterator.activate_read_phase(ColumnIterator::ReadPhase::FULL);
    ...
    iterator.activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);
    EXPECT_FALSE(iterator.need_to_read());
    EXPECT_TRUE(iterator.need_to_read_meta_columns());
    ...
    iterator.force_set_reading_flag(ColumnIterator::ReadingFlag::READING_FOR_PREDICATE);
    iterator.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    EXPECT_FALSE(iterator.need_to_read());
    EXPECT_FALSE(iterator.need_to_read_meta_columns());
...
TEST_F(ColumnReaderTest, DeferredDefaultsLifecycle) {
    TestColumnIterator iterator;
    iterator.force_set_reading_flag(ColumnIterator::ReadingFlag::READING_FOR_PREDICATE);
    iterator.activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);

    MutableColumnPtr dst = ColumnInt32::create();
    iterator.append_deferred_defaults(dst, 3);

    EXPECT_EQ(3, dst->size());
    EXPECT_TRUE(iterator._nested_read_plan.has_deferred_defaults);

    iterator.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    iterator.finish_deferred_read(dst);
    EXPECT_EQ(0, dst->size());
    EXPECT_FALSE(iterator._nested_read_plan.has_deferred_defaults);

    MutableColumnPtr lazy_dst = ColumnInt32::create();
    iterator.force_set_reading_flag(ColumnIterator::ReadingFlag::READING_FOR_PREDICATE);
    iterator.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    iterator.append_deferred_defaults(lazy_dst, 4);
    EXPECT_EQ(0, lazy_dst->size());
}

TEST_F(ColumnReaderTest, DeferredDefaultsRecoveryAfterColumnReplacement) {
    TestColumnIterator iterator;
    iterator.force_set_reading_flag(ColumnIterator::ReadingFlag::NEED_TO_READ);
    iterator.activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);
...
TEST_F(ColumnReaderTest, FinishDeferredReadOnNestedStruct) {
    ...
    StructFileColumnIterator struct_iter(std::make_shared<ColumnReader>(), std::move(null_iter),
                                         std::move(sub_iters));
    struct_iter.activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);
    sub_iter_ptr->activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);

    MutableColumnPtr nested_column = ColumnInt32::create();
    MutableColumnPtr nested_mut = IColumn::mutate(std::move(nested_column));
    sub_iter_ptr->append_deferred_defaults(nested_mut, 5);
    ...
    struct_iter.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    sub_iter_ptr->activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    struct_iter.finish_deferred_read(struct_mut);
...
TEST_F(ColumnReaderTest, NormalizeAccessPathsSetsPredicateFlag) {
    TestColumnIterator iterator;
    iterator.set_column_name("self");
...
TEST_F(ColumnReaderTest, NestedIteratorsPropagateReadPhase) {
    ...
    struct_iterator->activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    EXPECT_EQ(struct_iterator->_sub_column_iterators[0]->_read_phase,
              ColumnIterator::ReadPhase::DEFERRED);
    ...
    array_iterator.activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);
    ...
    MapFileColumnIterator map_iterator(std::make_shared<ColumnReader>(), std::move(map_null_iter),
                                       std::move(map_offsets_iter), std::move(map_key_iter),
                                       std::move(map_val_iter));
    map_iterator.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    EXPECT_EQ(map_iterator._key_iterator->_read_phase, ColumnIterator::ReadPhase::DEFERRED);
    EXPECT_EQ(map_iterator._val_iterator->_read_phase, ColumnIterator::ReadPhase::DEFERRED);
}
...
TEST_F(ColumnReaderTest, NestedNeedToReadInDeferredPredicatePhase) {
    auto struct_null_iterator =
            std::make_unique<FileColumnIterator>(std::make_shared<ColumnReader>());
    std::vector<ColumnIteratorUPtr> struct_sub_iters;
    struct_sub_iters.emplace_back(
    ...
    EXPECT_FALSE(struct_iterator.has_deferred_read_target());
    EXPECT_FALSE(struct_iterator.need_to_read());
    struct_iterator._sub_column_iterators[0]->set_need_to_read();
    EXPECT_TRUE(struct_iterator.has_deferred_read_target());
    ...
    ArrayFileColumnIterator array_iterator(
            std::make_shared<ColumnReader>(), std::move(array_offsets_iter),
            std::move(array_item_iterator), std::move(array_null_iter));
    array_iterator.activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
    array_iterator.set_reading_flag(ColumnIterator::ReadingFlag::READING_FOR_PREDICATE);
    EXPECT_FALSE(array_iterator.has_deferred_read_target());
    EXPECT_FALSE(array_iterator.need_to_read());
    array_iterator._item_iterator->set_need_to_read();
    EXPECT_TRUE(array_iterator.has_deferred_read_target());
    ...
    EXPECT_FALSE(map_iterator.has_deferred_read_target());
    EXPECT_FALSE(map_iterator.need_to_read());
    map_iterator._val_iterator->set_need_to_read();
    EXPECT_TRUE(map_iterator.has_deferred_read_target());
    EXPECT_TRUE(map_iterator.need_to_read());
}

TEST_F(ColumnReaderTest, NestedReadPhaseNeedToReadMatrix) {
    auto build_nested_iterator = []() {
        auto make_value_struct = []() {
            auto null_iter = std::make_unique<FileColumnIterator>(std::make_shared<ColumnReader>());
            std::vector<ColumnIteratorUPtr> sub_iters;
    ...
    auto assert_need_to_read = [](StructFileColumnIterator* top_struct) {
        ...
        auto* map_iter = static_cast<MapFileColumnIterator*>(array_iter->_item_iterator.get());
        auto* value_struct = static_cast<StructFileColumnIterator*>(map_iter->_val_iterator.get());
        auto expect_scalar = [](ColumnIterator::ReadingFlag flag, ColumnIterator::ReadPhase mode) {
            switch (mode) {
            case ColumnIterator::ReadPhase::FULL:
            ...
            case ColumnIterator::ReadPhase::DEFERRED:
                return flag == ColumnIterator::ReadingFlag::NEED_TO_READ;
            default:
                return false;
            }
        };
        auto expect_nested = [](ColumnIterator::ReadingFlag flag, ColumnIterator::ReadPhase mode) {
            switch (mode) {
            case ColumnIterator::ReadPhase::FULL:
                return flag != ColumnIterator::ReadingFlag::SKIP_READING;
            case ColumnIterator::ReadPhase::PREDICATE:
        ...
        top_struct->activate_read_phase(ColumnIterator::ReadPhase::FULL);
        EXPECT_EQ(expect_nested(top_struct->reading_flag(), ColumnIterator::ReadPhase::FULL),
                  top_struct->need_to_read());
        EXPECT_EQ(expect_nested(array_iter->reading_flag(), ColumnIterator::ReadPhase::FULL),
                  array_iter->need_to_read());
        EXPECT_EQ(expect_nested(map_iter->reading_flag(), ColumnIterator::ReadPhase::FULL),
                  map_iter->need_to_read());
        EXPECT_EQ(expect_nested(value_struct->reading_flag(), ColumnIterator::ReadPhase::FULL),
        ...
        top_struct->activate_read_phase(ColumnIterator::ReadPhase::PREDICATE);
        EXPECT_EQ(expect_nested(top_struct->reading_flag(), ColumnIterator::ReadPhase::PREDICATE),
                  top_struct->need_to_read());
        EXPECT_EQ(expect_nested(array_iter->reading_flag(), ColumnIterator::ReadPhase::PREDICATE),
                  array_iter->need_to_read());
        EXPECT_EQ(expect_nested(map_iter->reading_flag(), ColumnIterator::ReadPhase::PREDICATE),
                  map_iter->need_to_read());
        EXPECT_EQ(expect_nested(value_struct->reading_flag(), ColumnIterator::ReadPhase::PREDICATE),
                  value_struct->need_to_read());
        EXPECT_EQ(expect_scalar(map_iter->_key_iterator->reading_flag(),
        ...
        top_struct->activate_read_phase(ColumnIterator::ReadPhase::DEFERRED);
        EXPECT_EQ(top_struct->has_deferred_read_target(), top_struct->need_to_read());
        EXPECT_EQ(array_iter->has_deferred_read_target(), array_iter->need_to_read());
        EXPECT_EQ(map_iter->has_deferred_read_target(), map_iter->need_to_read());
        EXPECT_EQ(value_struct->has_deferred_read_target(), value_struct->need_to_read());
        EXPECT_EQ(expect_scalar(map_iter->_key_iterator->reading_flag(),
                                ColumnIterator::ReadPhase::DEFERRED),
                  map_iter->_key_iterator->need_to_read());
        EXPECT_EQ(map_iter->_val_iterator->has_deferred_read_target(),
                  map_iter->_val_iterator->need_to_read());
    };
