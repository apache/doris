...
TEST_F(ColumnReaderTest, MapReadByRowidsSkipReadingAppendsPlaceholders) {
    ...
    // A skipped read appends `count` placeholder rows; it must not treat `count` as the
    // absolute destination size, so reading again appends another `count` rows.
    st = map_iter.read_by_rowids(rowids, count, dst);
    ASSERT_TRUE(st.ok()) << "second read_by_rowids failed: " << st.to_string();
    ASSERT_EQ(2 * count, dst->size());
}
