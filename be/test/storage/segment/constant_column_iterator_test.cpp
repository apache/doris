...
void expect_same_field(const Field& expected, const Field& actual) {
    ...
    if (expected.get_type() == PrimitiveType::TYPE_VARIANT) {
        ASSERT_EQ(PrimitiveType::TYPE_VARIANT, actual.get_type());
        EXPECT_TRUE(canonical_equals(expected.get<TYPE_VARIANT>().ref(),
                                     actual.get<TYPE_VARIANT>().ref()));
        return;
    }
...
TEST_F(ConstantColumnIteratorTest, VariantConstantMaterializesSubcolumnsAndZoneMap) {
    // Case: a VARIANT object constant contains both a direct child and a nested object. Repeating
    // it must preserve both values, while text VARIANT defaults remain unsupported.
    TabletColumn column;
    ...
    auto root = builder_row.start_object();
    root.add_key(StringRef("number"));
    builder_row.add_int(42);
    root.add_key(StringRef("nested"));
    auto nested = builder_row.start_object();
    nested.add_key(StringRef("name"));
    builder_row.add_string(StringRef("doris"));
    nested.finish();
    root.finish();
    builder_row.finish();
    auto values = builder.finish_batch();
    Field field = Field::create_field<TYPE_VARIANT>(VariantField::from_ref(values.value_at(0)));
    assert_constant_pipeline(column, field);
    ...
    ASSERT_TRUE(st.ok()) << st;
    ASSERT_FALSE(has_null);
    const auto* variant = assert_cast<const ColumnVariantV2*>(dst.get());
    for (size_t row = 0; row < rows; ++row) {
        VariantRef actual_number;
        VariantRef actual_nested;
        VariantRef actual_name;
        const VariantRef actual = variant->get_value_ref(row);
        ASSERT_TRUE(actual.object_find(StringRef("number"), &actual_number));
        EXPECT_EQ(42, actual_number.get_int());
        ASSERT_TRUE(actual.object_find(StringRef("nested"), &actual_nested));
        ASSERT_TRUE(actual_nested.object_find(StringRef("name"), &actual_name));
        EXPECT_EQ(StringRef("doris"), actual_name.get_string());
    }
