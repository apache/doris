...
TEST(SegmentIteratorVirtualColumnTest, MaterializationExpandsConstNullableResult) {
    ...
    auto schema = std::make_shared<Schema>(tablet_schema->columns(), std::vector<ColumnId> {0, 1});
    ...
    iterator._virtual_column_exprs[1] = std::make_shared<VExprContext>(std::move(expr));
    iterator._vir_cid_to_idx_in_block[1] = 1;

    Block block;
    block.insert({ColumnInt32::create(4, 1), std::make_shared<DataTypeInt32>(), "id"});
