...
TEST(VariantV2ColumnReaderAdapterTest, AlternatingOwnedSourceLayoutsBuildOneBoundedBlockUnion) {
    ...
    EXPECT_EQ(values.shredded_field_path(1).get_path(), "b");
    EXPECT_EQ(values.shredded_field_presence(0).get_data(), (PaddedPODArray<UInt8> {1, 0, 1, 0}));
    EXPECT_EQ(values.shredded_field_presence(1).get_data(), (PaddedPODArray<UInt8> {0, 1, 0, 1}));
    EXPECT_TRUE(values.shredded_field_values(0).is_typed());
    EXPECT_TRUE(values.shredded_field_values(1).is_typed());
    EXPECT_EQ(ColumnVariantV2::TestAccess::shredded_union_rebuilds(values), 1);
