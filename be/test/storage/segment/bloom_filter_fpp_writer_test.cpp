...
Status has_bloom_filter_index(const TabletSchemaSPtr& schema, std::string_view file_name,
                              bool& has_bloom_filter) {
    std::shared_ptr<Segment> segment;
    RETURN_IF_ERROR(Segment::open(
            io::global_local_filesystem(), std::string(kTestDir) + "/" + std::string(file_name), 1,
            0, RowsetId {.version = 1}, schema, io::FileReaderOptions {}, &segment));
    ...
    RETURN_IF_ERROR(segment->_get_column_reader_for_read(schema->column_by_uid(1), &column_reader,
                                                         read_options));
...
TEST_P(VerticalSegmentWriterCompatibleBloomFilterFppWriterTest,
    ...
    rowset_ctx.tablet_schema = schema;
    opts.rowset_ctx = &rowset_ctx;
    const std::string file_name = std::string("vertical_segment_writer_") + type_case.name + ".dat";
    auto file_writer = create_file_writer(file_name);
    VerticalSegmentWriter writer(file_writer.get(), 0, schema, nullptr, nullptr, opts, nullptr);
