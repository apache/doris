...
protected:
    ...
    std::unique_ptr<SegmentIterator> make_iter(TrackingLazyColumnIterator** tracking_iter) {
        ...
        // Production fills this map in _vec_init_lazy_materialization(); the tests below call
        // _read_lazy_pruned_columns() directly, so build the same column-id -> block-position
        // map for the one-column read schema (column id 0 lives at block position 0).
        iter->_schema_block_id_map.resize(_read_schema->columns().size(), -1);
        for (int i = 0; i < _read_schema->num_column_ids(); i++) {
            iter->_schema_block_id_map[_read_schema->column_id(i)] = i;
        }
