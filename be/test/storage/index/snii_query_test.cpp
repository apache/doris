...
TEST(SniiSegmentReaderTest, OpenReadsOnlyFooterAndDirectory) {
    MemoryFile file;
    writer::SniiCompoundWriter writer(&file);
    ...
    EXPECT_EQ(file.reads()[0].offset, file.size() - format::tail_pointer_size());
    EXPECT_EQ(file.reads()[0].len, format::tail_pointer_size());
    EXPECT_EQ(file.reads()[1].offset, tp.directory_offset);
    EXPECT_EQ(file.reads()[1].len, tp.directory_length);
    EXPECT_EQ(file.read_bytes(), format::tail_pointer_size() + tp.directory_length);
}
...
TEST(SniiPhraseQueryTest, TwoTermPhraseUsesUnigramPositions) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
TEST(SniiPhraseQueryTest, TwoTermPhrasePrefixUsesUnigramPositions) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
// RQ-01: a plain literal pattern follows V3's unanchored Hyperscan semantics.
TEST(SniiRegexpQueryTest, MatchesLiteralSubstring) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
// RQ-06: patterns unsupported by Hyperscan follow V3's indexed-query behavior:
// the query succeeds without matching any term.
TEST(SniiRegexpQueryTest, UnsupportedPatternReturnsEmpty) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
// RQ-07: a syntactically invalid pattern also succeeds without matches.
TEST(SniiRegexpQueryTest, UnbalancedParenReturnsEmpty) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
// sorted order, so the first term "123" is the only expansion.
...
// RQ-10: a match-all pattern returns the union of all ordinary-term docids.
TEST(SniiRegexpQueryTest, MatchAllReturnsAllDocids) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
// RQ-11: representative patterns reproduce the V3 golden result set.
TEST(SniiRegexpQueryTest, MatchesV3Golden) {
    MemoryFile file;
    reader::SniiSegmentReader segment_reader;
    reader::LogicalIndexReader index_reader;
    assert_ok(build_reader(&file, &segment_reader, &index_reader));
...
TEST(SniiRegexpQueryTest, AnchoredPrefixIsTightened) {
    ...
    EXPECT_EQ(prefix_of("^order"), "order");
    // Non-anchored patterns cannot narrow dictionary enumeration because a match
    // may start in the middle of a term.
