...
public:
    ...
    size_t chunk_rows() const { return _chunk_rows; }
    size_t float_array_capacity() const { return _float_array.capacity(); }
    void reset_chunk_buffer_for_test(bool release) { _reset_chunk_buffer(release); }
    void force_float_array_reserve(size_t n) { _float_array.reserve(n); }
};
...
TEST_F(AnnIndexWriterTest, TestComputeChunkRowsByBytes) {
    const auto old_chunk_bytes = config::ann_index_build_chunk_bytes;
    Defer defer([old_chunk_bytes]() { config::ann_index_build_chunk_bytes = old_chunk_bytes; });

    config::ann_index_build_chunk_bytes = 64;

    EXPECT_EQ(AnnIndexColumnWriter::compute_chunk_rows(4), 4);
}

TEST_F(AnnIndexWriterTest, TestInitDoesNotReserveChunkBuffer) {
    ...
    EXPECT_EQ(writer->float_array_capacity(), 0);
}
...
TEST_F(AnnIndexWriterTest, TestAddMoreThanChunkSize) {
    ...
    // only once on the first full chunk; the remainder reuses the trained state.
...
TEST_F(AnnIndexWriterTest, TestFinishReleasesChunkBufferMemory) {
    auto mock_index = std::make_shared<MockVectorIndex>();
    auto writer = std::make_unique<TestAnnIndexColumnWriter>(_index_file_writer.get(),
                                                             _tablet_index.get());
    ...
    ASSERT_TRUE(writer->add_array_values(sizeof(float), vectors.data(), nullptr,
                        ...
                        .ok());
    EXPECT_GT(writer->float_array_capacity(), 0);
...
TEST_F(AnnIndexWriterTest, TestAddMoreThanChunkSizeIVF) {
    ...
    // IVF: train once on the first full chunk; subsequent batches only add().
    EXPECT_CALL(*mock_index, needs_training()).WillRepeatedly(testing::Return(true));
    EXPECT_CALL(*mock_index, train(10, testing::_))
            .Times(1)
            .WillOnce(testing::Return(Status::OK()));
...
TEST_F(AnnIndexWriterTest, TestLargeDataVolumeWithRemainderSkip) {
    ...
    // After the fix train() is called exactly once on the first full chunk;
    // the remaining 3 rows are added without re-training (which would otherwise
    // re-cluster IVF and invalidate the already-added vectors).
    EXPECT_CALL(*mock_index, needs_training()).WillRepeatedly(testing::Return(true));
...
TEST_F(AnnIndexWriterTest, TestPQMinTrainRows) {
    ...
    // 1000 vectors split into 100 chunks of 10. train() runs once on the first chunk,
    // then all subsequent chunks only add(). At finish() there are 0 leftover rows
    // and _need_save_index=true, so save() is invoked.
    EXPECT_CALL(*mock_index, needs_training()).WillRepeatedly(testing::Return(true));
...
TEST_F(AnnIndexWriterTest, TestPQWithSufficientData) {
    ...
    // train() runs once on the first chunk; the rest is pure add() at chunk boundary
    // plus one trailing add(2) in finish().
    EXPECT_CALL(*mock_index, needs_training()).WillRepeatedly(testing::Return(true));
    EXPECT_CALL(*mock_index, get_min_train_rows()).WillRepeatedly(testing::Return(131072));
    EXPECT_CALL(*mock_index, train(10, testing::_))
            .Times(1)
...
TEST_F(AnnIndexWriterTest, TestHnswSkipsTrainCall) {
    // For HNSW (needs_training=false), train() must never be invoked regardless
    // of how many chunks are pushed. This guards both the per-chunk path and
    // finish()'s leftover path against unintentional lock acquisition.
    auto mock_index = std::make_shared<MockVectorIndex>();
    auto writer = std::make_unique<TestAnnIndexColumnWriter>(_index_file_writer.get(),
                                                             _tablet_index.get());
...
TEST_F(AnnIndexWriterTest, TestComputeChunkRowsHandlesDimZero) {
    // Defensive guard: never divide by zero, return at least 1 row.
    EXPECT_EQ(AnnIndexColumnWriter::compute_chunk_rows(0), 1);
}

TEST_F(AnnIndexWriterTest, TestComputeChunkRowsByteBudgetSmallerThanRow) {
    // Even when chunk_bytes is below a single row's footprint, we must still
    // accept one row per chunk rather than zero.
    const auto old_chunk_bytes = config::ann_index_build_chunk_bytes;
    Defer defer([old_chunk_bytes]() { config::ann_index_build_chunk_bytes = old_chunk_bytes; });

    config::ann_index_build_chunk_bytes = 1;
    EXPECT_EQ(AnnIndexColumnWriter::compute_chunk_rows(128), 1);
}

TEST_F(AnnIndexWriterTest, TestComputeChunkRowsChunkSizeIsUpperBound) {
    // When the byte budget is generous, the per-row chunk_size cap still applies.
    // BE_TEST: chunk_size() == 10.
    const auto old_chunk_bytes = config::ann_index_build_chunk_bytes;
    Defer defer([old_chunk_bytes]() { config::ann_index_build_chunk_bytes = old_chunk_bytes; });

    config::ann_index_build_chunk_bytes = 1LL << 30; // 1 GiB
    EXPECT_EQ(AnnIndexColumnWriter::compute_chunk_rows(4), 10);
}

TEST_F(AnnIndexWriterTest, TestResetChunkBufferKeepsCapacityWhenTargetUnknown) {
    // Regression: previously `_current_chunk_capacity_elements() * 2` evaluated
    // to 0 when chunk_rows / dimension were unset, so every soft reset would
    // free the buffer. After the fix, reset(false) must keep the capacity when
    // the target is unknown; only an explicit reset(true) releases it.
    TestAnnIndexColumnWriter writer(_index_file_writer.get(), _tablet_index.get());
    // Intentionally skip init() so _chunk_rows and _dimension stay 0.
    writer.force_float_array_reserve(1024);
    const size_t reserved = writer.float_array_capacity();
    ASSERT_GE(reserved, 1024u);

    writer.reset_chunk_buffer_for_test(/*release_memory=*/false);
    EXPECT_EQ(writer.float_array_capacity(), reserved);

    writer.reset_chunk_buffer_for_test(/*release_memory=*/true);
    EXPECT_EQ(writer.float_array_capacity(), 0u);
}
