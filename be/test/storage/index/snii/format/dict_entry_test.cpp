...
// Construct a pod_ref slim dict entry.
...
TEST(SniiDictEntry, RejectsAliasedOutputAndScratch) {
    DictEntry entry = MakePodRefSlim("alias", 1);
    ByteSink sink;
    Status status = encode_dict_entry(entry, "", IndexTier::kT2, &sink, &sink);
    EXPECT_TRUE(status.is<doris::ErrorCode::INVALID_ARGUMENT>());
}
...
// An INLINE entry omits the redundant per-region crc32c byte because its
// region bytes are covered by the dict block crc32c.
...
TEST(SniiDictEntry, InlineOmitsRedundantRegionCrc) {
    ...
    inl.dd_meta.crc = 0xDEADBEEF;
    ByteSink inl_sink;
    ASSERT_TRUE(encode_dict_entry(inl, "", IndexTier::kT2, &inl_sink).ok());

    // A slim POD-ref keeps its region crc because its bytes live outside the
    // dictionary block.
    DictEntry ref = inl;
    ...
    // not carry the crc bytes by decoding and checking verify_crc is OFF.
