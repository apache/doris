...
TEST(SniiDictBlock, VisitTermRangeCrossesAnchorsAndHonorsBounds) {
    std::vector<DictEntry> entries;
    for (int i = 0; i < 24; ++i) {
        char term[8];
        snprintf(term, sizeof(term), "t%02d", i);
        entries.push_back(MakePodRef(term, static_cast<uint32_t>(i + 1), i * 10));
    }
    ...
    ASSERT_TRUE(reader.visit_term_range(
                              "t03", std::string_view("t18"),
                              [](std::string_view term) { return term.back() % 2 != 0; },
                              [&](DictEntry&& entry, bool* stop) {
                                  visited.push_back(std::move(entry.term));
                                  *stop = visited.size() == 4;
                                  return Status::OK();
                              },
                              &exhausted)
                        .ok());
    EXPECT_EQ(visited, (std::vector<std::string> {"t03", "t05", "t07", "t09"}));
    EXPECT_FALSE(exhausted);

    visited.clear();
    ASSERT_TRUE(reader.visit_term_range(
                              "t17", std::string_view("t20"), {},
                              [&](DictEntry&& entry, bool*) {
                                  visited.push_back(std::move(entry.term));
                                  return Status::OK();
                              },
                              &exhausted)
                        .ok());
    EXPECT_EQ(visited, (std::vector<std::string> {"t17", "t18", "t19"}));
