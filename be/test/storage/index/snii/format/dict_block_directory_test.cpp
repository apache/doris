...
TEST(SniiDictBlockDirectory, RejectsInvalidPhysicalBlockRanges) {
    const std::vector<std::vector<BlockRef>> invalid_directories = {
            {{.offset = 10, .length = 0, .n_entries = 1}},
            {{.offset = std::numeric_limits<uint64_t>::max(), .length = 2, .n_entries = 1}},
            {{.offset = 10, .length = 10, .n_entries = 1},
             {.offset = 19, .length = 10, .n_entries = 1}},
            {{.offset = 20, .length = 10, .n_entries = 1},
             {.offset = 10, .length = 10, .n_entries = 1}},
    };

    for (const auto& refs : invalid_directories) {
        const auto bytes = Build(refs);
        DictBlockDirectoryReader reader;
...
TEST(SniiDictBlockDirectory, WrongSectionTypeRejected) {
    ...
    SectionFramer::write(sink, static_cast<uint8_t>(SectionType::kSampledTermIndex), Slice(p, 3));
...
TEST(SniiDictBlockDirectory, TrailingFramedSectionBytesRejected) {
    auto bytes = Build(
            {BlockRef {.offset = 0, .length = 10, .n_entries = 1, .flags = 0, .checksum = 123}});
