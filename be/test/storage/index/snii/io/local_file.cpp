...
Status LocalFileReader::open(const std::string& path) {
    ...
    if (fd_ < 0) return Status::Error<ErrorCode::IO_ERROR, false>(errno_msg("open"));
    struct stat st;
    if (::fstat(fd_, &st) != 0)
        return Status::Error<ErrorCode::IO_ERROR, false>(errno_msg("fstat"));
...
Status LocalFileWriter::write_all(const uint8_t* data, size_t len) {
    ...
    while (done < len) {
        ...
        if (n < 0) {
            if (errno == EINTR) continue;
            return Status::Error<ErrorCode::IO_ERROR, false>(errno_msg("write"));
        }
        done += static_cast<size_t>(n);
    }
    return Status::OK();
...
Status LocalFileWriter::append(Slice data) {
    if (fd_ < 0) return Status::Error<ErrorCode::IO_ERROR, false>("append on unopened file");
...
Status LocalFileWriter::finalize() {
    if (fd_ < 0) return Status::Error<ErrorCode::IO_ERROR, false>("finalize on unopened file");
    RETURN_IF_ERROR(flush_buffer());
    if (::fsync(fd_) != 0) return Status::Error<ErrorCode::IO_ERROR, false>(errno_msg("fsync"));
    if (::close(fd_) != 0) {
        fd_ = -1;
        return Status::Error<ErrorCode::IO_ERROR, false>(errno_msg("close"));
    }
    fd_ = -1;
    return Status::OK();
}
