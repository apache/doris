...
Status MeteredFileReader::validate_range(uint64_t offset, size_t len) const {
    if (inner_ == nullptr)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("metered: null inner reader");
    if (block_size_ == 0)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("metered: zero block size");
...
Status MeteredFileReader::read_at(uint64_t offset, size_t len, std::vector<uint8_t>* out) {
    if (out == nullptr)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("metered: null out");
