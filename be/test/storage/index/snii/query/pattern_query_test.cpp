...
Corpus BuildRegexpParityCorpus() {
    Corpus c;
    c.doc_count = 4;
    c.docs = {{"apple123"}, {"snapple"}, {"application"}, {"banana"}};
    return c;
}
...
TEST(SniiPatternQuery, RegexpMatchesOracle) {
    ...
    for (const char* pattern :
        ...
        EXPECT_TRUE(std::ranges::is_sorted(got)) << pattern;
        EXPECT_EQ(got, corpus.matching_docs([&](std::string_view term) {
            return std::regex_search(term.begin(), term.end(), re);
        })) << pattern;
    }
...
TEST(SniiPatternQuery, RegexpMatchesV3SubstringSemantics) {
    ...
    std::vector<uint32_t> got;
    ASSERT_TRUE(query::regexp_query(idx, "app", &got).ok());
    EXPECT_EQ(got, (std::vector<uint32_t> {0, 1, 2}));

    ASSERT_TRUE(query::regexp_query(idx, "^app", &got).ok());
    EXPECT_EQ(got, (std::vector<uint32_t> {0, 2}));

    ASSERT_TRUE(query::regexp_query(idx, "ple$", &got).ok());
    EXPECT_EQ(got, (std::vector<uint32_t> {1}));

    ASSERT_TRUE(query::regexp_query(idx, "^apple123$", &got).ok());
    EXPECT_EQ(got, (std::vector<uint32_t> {0}));

    std::remove(path.c_str());
}

TEST(SniiPatternQuery, RegexpMatchesV3InvalidPatternSemantics) {
    const Corpus corpus = BuildRegexpParityCorpus();
    const std::string path = TempPath();
    WriteCorpus(corpus, path);
    ...
    std::vector<uint32_t> got;
    EXPECT_TRUE(query::regexp_query(idx, "*app*", &got).ok());
