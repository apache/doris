...
TEST(SniiPhraseFrequency, CountsOverlappingAndRepeatedTermOccurrences) {
    Corpus corpus;
    corpus.doc_count = 4;
    corpus.docs = {{"a", "b", "a", "b"},
                   {"a", "a", "a", "a"},
    ...
    ASSERT_TRUE(query::phrase_query_with_frequencies(index, {"a", "b"}, &matches).ok());
    EXPECT_EQ(matches, (std::vector<query::PhraseMatch> {{.docid = 0, .frequency = 2},
                                                         {.docid = 2, .frequency = 2}}));

    ASSERT_TRUE(query::phrase_query_with_frequencies(index, {"a", "a"}, &matches).ok());
    EXPECT_EQ(matches, (std::vector<query::PhraseMatch> {{.docid = 1, .frequency = 3}}));

    ASSERT_TRUE(query::phrase_query_with_frequencies(index, {"a", "a", "a"}, &matches).ok());
    EXPECT_EQ(matches, (std::vector<query::PhraseMatch> {{.docid = 1, .frequency = 2}}));

    ASSERT_TRUE(query::phrase_query_with_frequencies(index, {"a", "b", "c"}, &matches).ok());
    EXPECT_EQ(matches, (std::vector<query::PhraseMatch> {{.docid = 2, .frequency = 2}}));

    std::remove(path.c_str());
}

TEST(SniiPhraseFrequency, UsesFirstClauseMultiplicityWithStackedAnchorPositions) {
    SpimiTermBuffer buffer(/*has_positions=*/true);
    buffer.add_token("high", 0, 0);
    buffer.add_token("rare", 0, 1);
    buffer.add_token("rare", 0, 1);
    buffer.add_token("tail", 0, 2);
    buffer.add_token("high", 1, 0);
    buffer.add_token("high", 1, 0);
    buffer.add_token("rare", 1, 1);
    buffer.add_token("rare", 1, 1);
    buffer.add_token("tail", 1, 2);
    for (uint32_t docid = 2; docid < 8; ++docid) {
        buffer.add_token("high", docid, 0);
    }
    ...
    std::vector<query::PhraseMatch> matches;
    ASSERT_TRUE(
            query::phrase_query_with_frequencies(index, {"high", "rare", "tail"}, &matches).ok());
