...
// Pins both knobs of gram_boolean_query()'s candidate cost gate for one scope: the ratio and the
// row floor below which the ratio is not applied at all. This corpus is 30 rows, far under the
// production floor, so the gate can only be made to fire by lowering the floor deliberately.
class ScopedCandidateRatio {
public:
    ScopedCandidateRatio(int32_t bp, int32_t min_rows)
            : _saved_bp(config::gram_index_max_candidate_ratio_bp),
              _saved_min_rows(config::gram_index_candidate_ratio_min_rows) {
        config::gram_index_max_candidate_ratio_bp = bp;
        config::gram_index_candidate_ratio_min_rows = min_rows;
    }
    ...
    ~ScopedCandidateRatio() {
        config::gram_index_max_candidate_ratio_bp = _saved_bp;
        config::gram_index_candidate_ratio_min_rows = _saved_min_rows;
    }
...
void check_pattern(const std::vector<std::optional<std::string>>& corpus,
    ...
    // The truth comparison below runs on the PRODUCTION gate defaults. This corpus is far under
    // the configured row floor, so the cost gate is structurally unable to fire and the candidate
    // set really is the pruned one -- which matters, because a gate that widened everything to
    // the whole segment would satisfy the "no false negative" assertion trivially and this is the
    // one test that proves the index never loses a matching row.
    ASSERT_LT(num_docs, static_cast<uint32_t>(config::gram_index_candidate_ratio_min_rows));
    ...
    {
        // Same pattern with the gate deliberately forced on (floor dropped under this corpus, 1%
        // ratio): every gram is above 1% here, so a node with grams gives up and yields the whole
        // segment. Whatever it returns must be a superset of the ungated candidate set -- giving
        // up may only add rows.
        ScopedCandidateRatio ratio(/*bp=*/100, /*min_rows=*/1);
        doris::snii::query::LogicalIndexPostingSource source(index);
        roaring::Roaring gated;
        ASSERT_TRUE(doris::snii::query::gram_boolean_query(source, query, num_docs, &gated).ok())
                << pattern_case.pattern;
        EXPECT_TRUE((bitmap - gated).isEmpty())
                << "the candidate ratio gate dropped rows for " << pattern_case.pattern;
        EXPECT_TRUE((gated - full_range(num_docs)).isEmpty()) << pattern_case.pattern;
    }
...
void run_scheme(const std::string& analyzer_name, const std::string& segment_name) {
    ...
    // Coverage guard: at least a third of the patterns that CAN prune must actually do so,
    // otherwise this test degenerates into "everything is ALL and nothing is verified". The
    // threshold is a third rather than a half because a SPARSE scheme keeps grams by hash
    // sampling, so the share of short literals it can reliably pin down is naturally lower
    // than DENSE's; what this guards against is "the compiler or the write side broke
    // entirely", not the difference in pruning power between the two schemes.
    //
    // Non-ASCII patterns are excluded from the denominator because the index declines that
    // text outright: they are expected to compile to ALL and are still checked above for
    // returning exactly what a brute-force scan returns, which is the property that matters.
    int prunable_candidates = 0;
    for (const PatternCase& pattern_case : pattern_cases()) {
        const std::string_view p = pattern_case.pattern;
        if (std::ranges::all_of(p, [](char c) { return static_cast<unsigned char>(c) < 0x80; })) {
            ++prunable_candidates;
        }
    }
    EXPECT_GE(indexable, prunable_candidates / 3)
            << "analyzer=" << analyzer_name << ", ascii patterns=" << prunable_candidates;
}
