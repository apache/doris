...
void expect_docids_for_term(const std::vector<TermPostings>& postings, std::string_view term,
                            const std::vector<uint32_t>& expected_docids) {
    const auto found = std::ranges::find(postings, term, &TermPostings::term);
    ASSERT_NE(found, postings.end());
    EXPECT_EQ(found->docids, expected_docids);
}
...
// Without positions, docs-only postings are SETS: a repeated occurrence inside
// the same document is discarded at accumulate time (it would only cost arena /
// spill bytes -- nothing consumes a docs-only tf), so per-doc freq stays 1.
...
TEST(SniiSpimiTermBufferTest, OwnedTermInternerMaterializesDistinctTermsOnce) {
    namespace snii_testing = doris::snii::writer::testing;

    MemoryReporter reporter;
    {
        SpimiTermBuffer empty_buffer(/*has_positions=*/false, /*spill_threshold_bytes=*/0,
                                     &reporter);
        EXPECT_EQ(empty_buffer.resident_bytes_for_test(), 0U);
        EXPECT_EQ(reporter.current_bytes(), 0);
    }
    EXPECT_EQ(reporter.current_bytes(), 0);

    snii_testing::reset_vocab_string_materialization_count();
    SpimiTermBuffer buffer(/*has_positions=*/false, /*spill_threshold_bytes=*/1);
    buffer.add_token("alpha", 0, 0);
    ASSERT_EQ(buffer.run_count_for_test(), 1U);
    buffer.add_token("beta", 1, 0);
    buffer.add_token("alpha", 2, 0);
    buffer.add_token("long-materialized-term", 3, 0);
    buffer.add_token("beta", 4, 0);
    EXPECT_EQ(snii_testing::vocab_string_materialization_count(), 3U);

    std::vector<TermPostings> postings = buffer.finalize_sorted();
    ASSERT_EQ(postings.size(), 3U);
    expect_docids_for_term(postings, "alpha", {0U, 2U});
    expect_docids_for_term(postings, "beta", {1U, 4U});
    expect_docids_for_term(postings, "long-materialized-term", {3U});
}

TEST(SniiSpimiTermBufferTest, OwnedTermInternerUsesTermIdSlotsAndPreservesHashCollisions) {
    namespace snii_testing = doris::snii::writer::testing;

    EXPECT_EQ(SpimiTermBuffer::owned_term_key_size_for_test(), 4U);
    snii_testing::reset_vocab_string_materialization_count();
    snii_testing::reset_owned_term_full_byte_comparison_count();
    SpimiTermBuffer buffer(/*has_positions=*/false);
    buffer.set_owned_term_hash_mask_for_test(0);
    buffer.add_token("", 0, 0);
    buffer.add_token("short", 1, 0);
    buffer.add_token("abcdefgh-left", 2, 0);
    buffer.add_token("abcdefgh-rght", 3, 0);
    buffer.add_token("", 4, 0);
    buffer.add_token("short", 5, 0);
    buffer.add_token("abcdefgh-left", 6, 0);
    buffer.add_token("abcdefgh-rght", 7, 0);
    EXPECT_GT(snii_testing::owned_term_full_byte_comparison_count(), 0U);
    EXPECT_EQ(snii_testing::vocab_string_materialization_count(), 4U);

    const std::vector<TermPostings> postings = buffer.finalize_sorted();
    ASSERT_EQ(postings.size(), 4U);
    expect_docids_for_term(postings, "", {0U, 4U});
    expect_docids_for_term(postings, "short", {1U, 5U});
    expect_docids_for_term(postings, "abcdefgh-left", {2U, 6U});
    expect_docids_for_term(postings, "abcdefgh-rght", {3U, 7U});
}

TEST(SniiSpimiTermBufferTest, OwnedTermInternerFailureLeavesTableReusable) {
    namespace snii_testing = doris::snii::writer::testing;

    snii_testing::reset_vocab_string_materialization_count();
    MemoryReporter reporter;
    SpimiTermBuffer buffer(/*has_positions=*/false, /*spill_threshold_bytes=*/0, &reporter);

    snii_testing::fail_next_owned_term_reserve();
    EXPECT_THROW(buffer.add_token("reserve-failure-term", 0, 0), std::bad_alloc);
    EXPECT_EQ(snii_testing::vocab_string_materialization_count(), 0U);
    EXPECT_GT(buffer.resident_bytes_for_test(), 0U);
    EXPECT_EQ(reporter.current_bytes(), buffer.resident_bytes_for_test());

    snii_testing::fail_next_owned_term_emplace();
    EXPECT_THROW(buffer.add_token("emplace-failure-term", 1, 0), std::bad_alloc);
    EXPECT_EQ(snii_testing::vocab_string_materialization_count(), 0U);
    EXPECT_EQ(reporter.current_bytes(), buffer.resident_bytes_for_test());

    buffer.add_token("emplace-failure-term", 2, 0);
    buffer.add_token("", 3, 0);
    buffer.add_token("emplace-failure-term", 4, 0);
    EXPECT_EQ(snii_testing::vocab_string_materialization_count(), 2U);

    const std::vector<TermPostings> postings = buffer.finalize_sorted();
    ASSERT_EQ(postings.size(), 2U);
    expect_docids_for_term(postings, "", {3U});
    expect_docids_for_term(postings, "emplace-failure-term", {2U, 4U});
}
...
TEST(SniiSpimiTermBuffer, StreamingMatchesMaterialized) {
    ...
    Status st = strm.for_each_term_sorted([&](StreamedTermPostings&& source) {
        TermPostings postings;
        RETURN_IF_ERROR(materialize_streamed_term(std::move(source), &postings));
        streamed.push_back(std::move(postings));
        return Status::OK();
    });
    EXPECT_TRUE(st.ok());
...
TEST(SniiSpimiTermBufferTest, PositionedArenaSourceDecodesEachTokenOnce) {
    SpimiTermBuffer buffer(/*has_positions=*/true);
    buffer.add_token("term", 2, 3);
    buffer.add_token("term", 2, 7);
    buffer.add_token("term", 5, 1);
    buffer.add_token("term", 9, 2);
    buffer.add_token("term", 9, 4);
    buffer.add_token("term", 9, 8);

    doris::snii::writer::testing::reset_compact_chain_varint_decode_count();
    std::vector<uint32_t> docids;
    std::vector<uint32_t> freqs;
    std::vector<uint32_t> positions;
    const Status status = buffer.for_each_term_sorted([&](StreamedTermPostings&& term) {
        TermPostingBuffer transfer(/*memory_reporter=*/nullptr);
        bool exhausted = false;
        while (!exhausted) {
            transfer.clear_reuse();
            RETURN_IF_ERROR(term.source->fill(/*target_docs=*/2, &transfer, &exhausted));
            docids.insert(docids.end(), transfer.docids().begin(), transfer.docids().end());
            freqs.insert(freqs.end(), transfer.freqs().begin(), transfer.freqs().end());
            positions.insert(positions.end(), transfer.positions_flat().begin(),
                             transfer.positions_flat().end());
        }
        return Status::OK();
    });

    ASSERT_TRUE(status.ok()) << status;
    EXPECT_EQ(docids, (std::vector<uint32_t> {2, 5, 9}));
    EXPECT_EQ(freqs, (std::vector<uint32_t> {2, 1, 3}));
    EXPECT_EQ(positions, (std::vector<uint32_t> {3, 7, 1, 2, 4, 8}));
    EXPECT_EQ(doris::snii::writer::testing::compact_chain_varint_decode_count(), 9U);
}
...
TEST(SniiSpimiTermBuffer, StreamingDrainsAndShrinks) {
    ...
    size_t seen = 0;
    EXPECT_TRUE(buf.for_each_term_sorted([&](StreamedTermPostings&& source) {
                       TermPostings tp;
...
// Same revisit, positions disabled: docids coalesce to a unique ascending set.
// Docs-only postings carry set semantics, so the revisited doc keeps freq 1
// instead of summing occurrences.
...
// A spill's open() I/O failure surfaces as a latched error in status(). We force
// open() to fail deterministically by exhausting every free
...
TEST(SniiSpimiTermBuffer, DoubleDrainIsRejected) {
    ...
    size_t seen = 0;
    EXPECT_FALSE(buf.for_each_term_sorted([&](StreamedTermPostings&&) {
                        ++seen;
                        return Status::OK();
                    }).ok());
    EXPECT_EQ(seen, 0U);
...
TEST(SniiSpimiTermBuffer, StreamingConsumerFailurePropagatesForMemoryAndSpill) {
    for (size_t spill_threshold : {size_t {0}, size_t {1}}) {
        SpimiTermBuffer buffer(/*has_positions=*/true, spill_threshold);
        buffer.add_token("alpha", 0, 0);
        buffer.add_token("beta", 1, 0);
        size_t calls = 0;
        const Status status = buffer.for_each_term_sorted([&](StreamedTermPostings&&) {
            ++calls;
            return Status::Error<ErrorCode::IO_ERROR, false>("injected consumer failure");
        });
        EXPECT_TRUE(status.is<ErrorCode::IO_ERROR>()) << status;
        EXPECT_EQ(calls, 1U);
        EXPECT_EQ(buffer.unique_terms(), 0U);
    }
}
...
TEST(SniiSpimiTermBufferTest, NullReporterFinalizeIsCorrect) {
    ...
    // Docs-only set semantics: the 99 same-doc repeats are discarded at
    // accumulate time, so the doc's freq is 1 (not the occurrence count).
