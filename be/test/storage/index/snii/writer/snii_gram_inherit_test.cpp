...
TEST_F(SniiGramInheritTest, ReopenedContainerKeepsDistinctSchemesAndPatternCandidates) {
    // This is about a container carrying two schemes apart, not about how either was arrived
    // at, so it pins the density rather than letting the write path solve one -- a solved rate
    // would differ from the scheme the test wrote and constructed its expectation from.
    const bool saved_adaptive = doris::config::enable_gram_index_adaptive_density;
    doris::config::enable_gram_index_adaptive_density = false;
    Defer restore([&] { doris::config::enable_gram_index_adaptive_density = saved_adaptive; });
