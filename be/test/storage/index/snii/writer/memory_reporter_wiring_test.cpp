...
TEST(SniiMemoryReporterWiring, SpimiStreamingDrainNetZero) {
    ...
    ASSERT_TRUE(buf.for_each_term_sorted([&seen](StreamedTermPostings&& source) {
...
TEST(SniiMemoryReporterWiring, SpimiSpillMaterializationIsAccountedThroughRunWrite) {
    ...
    std::vector<std::string> vocab = {"wide"};
    SpimiTermBuffer buf(&vocab, /*has_positions=*/true, /*spill=*/0, &reporter);
    buf.set_forced_spill_min_arena_bytes(1);

    constexpr uint32_t kDocumentsBeforeSpill = 8192;
    for (uint32_t docid = 0; docid < kDocumentsBeforeSpill; ++docid) {
        buf.add_token(/*term_id=*/0, docid, /*pos=*/docid);
    }
    ASSERT_TRUE(buf.status().ok()) << buf.status().to_string();
    const int64_t before_spill = reporter.current_bytes();
    ASSERT_EQ(mirrored_bytes, before_spill);
    peak_bytes = before_spill;

    buf.request_global_spill_for_test();
    buf.add_token(/*term_id=*/0, kDocumentsBeforeSpill, /*pos=*/kDocumentsBeforeSpill);
    ASSERT_TRUE(buf.status().ok()) << buf.status().to_string();
    ASSERT_EQ(buf.run_count_for_test(), 1U);
