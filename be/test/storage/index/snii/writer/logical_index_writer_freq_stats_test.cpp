...
TEST(SniiWriterTest, ValidateTermRejectsFreqDocidLengthMismatch) {
    ...
    EXPECT_NE(status.to_string().find("span posting source: freqs length must equal docids"),
              std::string::npos)
            << status.to_string();
}

TEST(SniiWriterTest, ValidateTermAcceptsMissingFreqsForOrdinaryDocsOnlyTerm) {
    TermPostings tp;
    tp.term = "x";
    tp.docids = {0, 1};
    tp.retain_positions = false;
    ...
    EXPECT_EQ(index_reader.stats().sum_total_term_freq, 2U);
}
...
// FW-docs-only-spimi: the real import path for an untokenized (keyword) or
// support_phrase=false index streams docs-only terms out of SpimiTermBuffer,
// whose to_postings derives retain_positions=false from the chain shape. The
// positioned-index invariant "a positionless term must be a declared common
// gram" must not fire for a docs-only config -- it aborted the BE on the first
// keyword-index memtable flush before the fix. Direct-vector inputs cannot
// cover this: TermPostings defaults retain_positions=true, so only the
// streamed source produces the false flag the invariant sees in production.
TEST(SniiWriterTest, DocsOnlyStreamedSpimiBuildSucceeds) {
    SpimiTermBuffer buffer(/*has_positions=*/false);
    buffer.add_token(std::string_view("alpha"), /*docid=*/0, /*pos=*/0);
    buffer.add_token(std::string_view("beta"), /*docid=*/0, /*pos=*/1);
    buffer.add_token(std::string_view("alpha"), /*docid=*/1, /*pos=*/0);
    ...
    const format::DictEntry alpha = lookup_entry(index_reader, "alpha");
    EXPECT_EQ(alpha.df, 2U);
    const format::DictEntry beta = lookup_entry(index_reader, "beta");
    EXPECT_EQ(beta.df, 1U);
}
