...
TEST(SniiMemoryReporter, ZeroDeltaIsDebouncedNoOp) {
    int fire_count = 0;
    ...
    reporter.report(0);
    EXPECT_EQ(reporter.current_bytes(), 100);
    // report(0) is debounced: it neither moves the total nor invokes the
    // callback (spill paths emit frequent zero deltas; forwarding them would
    // spam the shared limiter for no state change).
