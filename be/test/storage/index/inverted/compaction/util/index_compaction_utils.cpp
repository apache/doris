...
class IndexCompactionUtils {
    ...
    static bool query_bkd(const TabletIndex* index,
        ...
        for (int i = 0; i < query_data.size(); i++) {
            Field param_value =
                    Field::create_field<TYPE_INT>(int32_t(query_data[i]));
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (const auto& row : data[i]) {
                if constexpr (std::is_same_v<T, DataRow>) {
                    Field key =
                            Field::create_field<TYPE_INT>(int32_t(row.key));
                    Field v1 = Field::create_field<TYPE_STRING>(row.word);
                    Field v2 = Field::create_field<TYPE_STRING>(row.url);
                    Field v3 =
                            Field::create_field<TYPE_INT>(int32_t(row.num));
                    ...
                    Field title =
                            Field::create_field<TYPE_STRING>(row.title);
                    Field content =
                            Field::create_field<TYPE_STRING>(row.content);
                    Field redirect =
                            Field::create_field<TYPE_STRING>(row.redirect);
                    Field space =
                            Field::create_field<TYPE_STRING>(row.space);
