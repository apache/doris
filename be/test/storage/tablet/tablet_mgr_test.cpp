...
// Keep the schema validation matrix together; assertion macros dominate the complexity count.
...
TEST_F(TabletMgrTest, ValidateRowTtlSchema) {
    ...
    ASSERT_TRUE(create_tablet(direct_without_restore).ok());
    auto created_direct = _tablet_mgr->get_tablet(direct_without_restore.tablet_id);
    ASSERT_NE(created_direct, nullptr);
    EXPECT_EQ(created_direct->tablet_schema()->row_ttl_duration_us(), -1);
    EXPECT_EQ(created_direct->tablet_schema()->row_ttl_time_zone_offset_seconds(), 0);
    ...
    ASSERT_TRUE(create_tablet(missing_offset).ok());
    EXPECT_EQ(_tablet_mgr->get_tablet(missing_offset.tablet_id)
                      ->tablet_schema()
                      ->row_ttl_time_zone_offset_seconds(),
              0);

    auto restore_without_offset = missing_offset;
    restore_without_offset.__set_tablet_id(217);
    restore_without_offset.__set_in_restore_mode(true);
    ASSERT_TRUE(create_tablet(restore_without_offset).ok());
    EXPECT_EQ(_tablet_mgr->get_tablet(restore_without_offset.tablet_id)
                      ->tablet_schema()
                      ->row_ttl_time_zone_offset_seconds(),
              0);
    ...
    expect_invalid(timestamp_non_utc, "Direct and TIMESTAMPTZ Row TTL time zone offset must be 0");
