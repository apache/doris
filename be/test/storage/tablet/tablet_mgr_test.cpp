...
TEST_F(TabletMgrTest, FindTabletWithCompact) {
...

    {
        ...
        for (auto& t : compact_tablets) {
            ...
            ASSERT_EQ(t.tablet->calc_compaction_score(CompactionType::CUMULATIVE_COMPACTION), 100 - index);
            index++;
        }
...

    {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            ASSERT_EQ(compact_tablets[i].tablet->calc_compaction_score(CompactionType::CUMULATIVE_COMPACTION), 100 - i);
        }
        ASSERT_EQ(compact_tablets[10].tablet->tablet_id(), 20102);
        ASSERT_EQ(compact_tablets[10].tablet->calc_compaction_score(CompactionType::CUMULATIVE_COMPACTION), 200);
...

    {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            ASSERT_EQ(compact_tablets[i].tablet->calc_compaction_score(CompactionType::CUMULATIVE_COMPACTION), 10 - i);
        }
