...
TEST_F(TabletIndexTest, test_schema_index_diff) {
    ...
    EXPECT_FALSE(vectorized::variant_util::has_schema_index_diff(new_tablet_schema.get(),
                                                                 old_tablet_schema.get(), 0, 0));
    EXPECT_TRUE(vectorized::variant_util::has_schema_index_diff(new_tablet_schema.get(),
