...
TEST(RowTtlTest, RestoreLegacyRowTtlFieldNumbers) {
    EXPECT_EQ(TabletSchemaPB::descriptor()->FindFieldByName("row_ttl_duration_us")->number(), 44);
    EXPECT_EQ(TabletSchemaPB::descriptor()
                      ->FindFieldByName("row_ttl_time_zone_offset_seconds")
                      ->number(),
              45);
    EXPECT_EQ(TabletSchemaPB::descriptor()->FindFieldByName("ttl_col_idx")->number(), 46);
    EXPECT_EQ(TabletSchemaCloudPB::descriptor()->FindFieldByName("row_ttl_duration_us")->number(),
              45);
    EXPECT_EQ(TabletSchemaCloudPB::descriptor()
                      ->FindFieldByName("row_ttl_time_zone_offset_seconds")
                      ->number(),
              46);
    EXPECT_EQ(TabletSchemaCloudPB::descriptor()->FindFieldByName("ttl_col_idx")->number(), 47);

    TabletSchemaPB legacy_pb;
    legacy_pb.set_keys_type(KeysType::DUP_KEYS);
    *legacy_pb.add_column() = make_column_pb(0, "k", "INT", true, false);
    *legacy_pb.add_column() = make_column_pb(1, BINLOG_TSO_COL, "BIGINT", false, false);
    *legacy_pb.add_column() = make_column_pb(2, BINLOG_LSN_COL, "BIGINT", false, false);
    *legacy_pb.add_column() = make_column_pb(3, BINLOG_OP_COL, "BIGINT", false, false);
    *legacy_pb.add_column() =
            make_column_pb(4, TTL_COL, "DATETIMEV2", false, true, "NONE", 6, "NULL");
    // In the original Row TTL wire layout fields 40..43 represented binlog_tso, binlog_lsn,
    // binlog_op, and ttl_col_idx. The current descriptor parses them as row_lsn and the three
    // binlog indexes because master inserted row_lsn_col_idx at field 40.
    legacy_pb.set_row_lsn_col_idx(1);
    legacy_pb.set_binlog_tso_col_idx(2);
    legacy_pb.set_binlog_lsn_col_idx(3);
    legacy_pb.set_binlog_op_col_idx(4);
    legacy_pb.set_row_ttl_duration_us(7);
    legacy_pb.set_row_ttl_time_zone_offset_seconds(8 * 60 * 60);
    std::string legacy_bytes;
    ASSERT_TRUE(legacy_pb.SerializeToString(&legacy_bytes));

    TabletSchemaPB parsed_legacy_pb;
    ASSERT_TRUE(parsed_legacy_pb.ParseFromString(legacy_bytes));
    TabletSchema restored;
    restored.init_from_pb(parsed_legacy_pb);
    EXPECT_EQ(restored.row_lsn_col_idx(), -1);
    EXPECT_EQ(restored.binlog_tso_col_idx(), 1);
    EXPECT_EQ(restored.binlog_lsn_col_idx(), 2);
    EXPECT_EQ(restored.binlog_op_col_idx(), 3);
    EXPECT_EQ(restored.ttl_col_idx(), 4);
    EXPECT_EQ(restored.row_ttl_duration_us(), 7);
    EXPECT_EQ(restored.row_ttl_time_zone_offset_seconds(), 8 * 60 * 60);

    TabletSchemaPB current_pb;
    restored.to_schema_pb(&current_pb);
    EXPECT_EQ(current_pb.row_lsn_col_idx(), -1);
    EXPECT_EQ(current_pb.binlog_tso_col_idx(), 1);
    EXPECT_EQ(current_pb.binlog_lsn_col_idx(), 2);
    EXPECT_EQ(current_pb.binlog_op_col_idx(), 3);
    EXPECT_EQ(current_pb.ttl_col_idx(), 4);

    TabletSchemaPB legacy_without_binlog_pb;
    legacy_without_binlog_pb.set_keys_type(KeysType::DUP_KEYS);
    *legacy_without_binlog_pb.add_column() = make_column_pb(0, "k", "INT", true, false);
    *legacy_without_binlog_pb.add_column() =
            make_column_pb(1, TTL_COL, "DATETIMEV2", false, true, "NONE", 6, "NULL");
    legacy_without_binlog_pb.set_binlog_op_col_idx(1);
    legacy_without_binlog_pb.set_row_ttl_duration_us(7);
    TabletSchema restored_without_binlog;
    restored_without_binlog.init_from_pb(legacy_without_binlog_pb);
    EXPECT_EQ(restored_without_binlog.ttl_col_idx(), 1);
    EXPECT_EQ(restored_without_binlog.row_lsn_col_idx(), -1);
    EXPECT_EQ(restored_without_binlog.binlog_tso_col_idx(), -1);
    EXPECT_EQ(restored_without_binlog.binlog_lsn_col_idx(), -1);
    EXPECT_EQ(restored_without_binlog.binlog_op_col_idx(), -1);
}
