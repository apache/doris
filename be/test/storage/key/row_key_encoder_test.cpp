...
//
// The all-type matrix checks the ordering contract and exact byte layout from
// one explicit data table. The count matrix and suffix cases retain focused
// checks for behavior that the table cannot cover independently.
...
// key(uid 0), four cluster keys(uid 1..4), two-column short-key prefix.
TabletSchemaSPtr different_key_count_schema() {
    auto s = std::make_shared<TabletSchema>();
    s->append_column(*create_int_key(0));
    for (int32_t uid = 1; uid <= 4; ++uid) {
        s->append_column(
                *create_int_value(uid, FieldAggregationMethod::OLAP_FIELD_AGGREGATION_NONE));
    }
    s->_keys_type = UNIQUE_KEYS;
    s->_cluster_key_uids = {1, 2, 3, 4};
...
//
// For the all-type matrix, kAllKeyData is the only row-data source. For key k,
// rows 3k..3k+2 are
...
TEST_F(RowKeyEncoderTest, AllKeyTypesTable) {
    ...
    // same byte-level snapshot as the all-type matrix above.
    {
        ...
        build(schema, 1, [](MutableColumns& columns) {
            fill_int(columns, 0, {42});
            fill_int(columns, 1, {7});
            fill_int(columns, 2, {9});
            fill_int(columns, 3, {11});
        ...
        EXPECT_EQ(to_hex(full_key), "02800000070280000009028000000b028000000d");
        EXPECT_EQ(to_hex(primary_key), "028000002a");
        EXPECT_EQ(to_hex(short_key), "02800000070280000009");
        EXPECT_EQ(to_hex(primary_index_key), "028000002a0200000000");

        schemas.emplace_back("unique_keys_merge_on_write_cluster_different_counts");
        physical_rows.emplace_back("0");
        sort_data_rows.emplace_back("cluster=(7,9,11,13)");
        primary_data_rows.emplace_back("primary=(42)");
        encoded_full_keys.push_back(to_hex(full_key));
        encoded_primary_keys.push_back(to_hex(primary_key));
        encoded_short_keys.push_back(to_hex(short_key));
        encoded_primary_index_keys.push_back(to_hex(primary_index_key));
    }

    // 7. Compare all encoded views with the generated byte-level snapshot.
