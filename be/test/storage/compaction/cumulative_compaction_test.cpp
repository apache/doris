...
TEST_F(CumulativeCompactionTest, TestPickExpiredRowBinlogRowsetUsesFrozenCutoff) {
    ...
    EXPECT_FALSE(compaction.pick_expired_row_binlog_rowset_for_test({expired}));
    ...
    EXPECT_EQ(compaction.input_rowsets().size(), 2);
    retained->rowset_meta()->set_commit_tso({cutoff, cutoff + 1});
    EXPECT_TRUE(compaction.pick_expired_row_binlog_rowset_for_test({expired, retained}));
    EXPECT_EQ(compaction.input_rowsets().size(), 1);

    tablet->set_visible_version(nullptr);
    TestableCumulativeCompactionMixin after_restart(storage_engine, tablet);
    after_restart.snapshot_row_binlog_ttl_for_test();
    EXPECT_FALSE(after_restart.pick_expired_row_binlog_rowset_for_test({expired}));
}
