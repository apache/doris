...
TEST_F(TestBinlogCumulativeCompactionPolicy, ttl_discovery_does_not_depend_on_schedule_score) {
    ...
    _cluster.advance_row_binlog_ttl_reference_tso(1100L << kTsoLogicalBits);
...
TEST_F(TestBinlogCumulativeCompactionPolicy, ttl_scan_honors_tablet_auto_compaction_switch) {
    ...
    _tablet_meta->set_disable_auto_compaction(true);
    EXPECT_FALSE(tablet->scan_expired_row_binlog_rowsets());
    _tablet_meta->set_disable_auto_compaction(false);
    EXPECT_TRUE(tablet->scan_expired_row_binlog_rowsets());
}
