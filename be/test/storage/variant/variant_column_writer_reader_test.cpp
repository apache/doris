...
TEST_F(VariantColumnWriterReaderTest, test_segment_rowid_read_by_reader_version) {
    ...
    const auto seek_and_read_by_rowids =
            [](const segment_v2::SegmentSharedPtr& segment, const TabletSchema& schema,
               SlotDescriptor* slot, const std::vector<uint32_t>& row_ids, MutableColumnPtr& result,
               StorageReadOptions& read_options, ColumnIteratorUPtr& iterator) -> Status {
        for (const uint32_t row_id : row_ids) {
            RETURN_IF_ERROR(segment->seek_and_read_by_rowid(schema, slot, row_id, result,
                                                            read_options, iterator));
        }
        return Status::OK();
    };
    ...
    for (size_t mode = 0; mode < 2; ++mode) {
        ...
        auto st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base], row_ids,
        ...
        st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base], second_row_ids,
                                     whole_result, read_options, whole_iterator);
        ASSERT_TRUE(st.ok()) << "use_v2=" << use_v2 << ": " << st.to_string();
        ...
        st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base + 1], row_ids,
                                     hot_result, read_options, hot_iterator);
        ASSERT_TRUE(st.ok()) << "use_v2=" << use_v2 << ": " << st.to_string();
        auto* const hot_iterator_address = hot_iterator.get();
        st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base + 1],
                                     second_row_ids, hot_result, read_options, hot_iterator);
        ASSERT_TRUE(st.ok()) << "use_v2=" << use_v2 << ": " << st.to_string();
        ...
        st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base + 2], row_ids,
                                     subpath_result, read_options, subpath_iterator);
        ASSERT_TRUE(st.ok()) << "use_v2=" << use_v2 << ": " << st.to_string();
        auto* const subpath_iterator_address = subpath_iterator.get();
        st = seek_and_read_by_rowids(segments[0], *_tablet_schema, slots[slot_base + 2],
        ...
        if (use_v2) {
            ...
            // branch-4.1 exposes a row-at-a-time Segment entry point. Sparse rows may be read
            // through different physical representations, so appending several calls can
            // canonicalize the destination to encoded Variant V2. The value/null assertions
            // above and the iterator assertion here are the compatibility contract.
        }
    ...
    for (size_t mode = 0; mode < 2; ++mode) {
        ...
        ColumnIteratorUPtr whole_iterator;
        auto st = seek_and_read_by_rowids(empty_nested_segments[0], *_tablet_schema,
                                          empty_slots[slot_base], empty_row_ids, whole_result,
        ...
        ColumnIteratorUPtr subpath_iterator;
        st = seek_and_read_by_rowids(empty_nested_segments[0], *_tablet_schema,
                                     empty_slots[slot_base + 1], empty_row_ids, subpath_result,
