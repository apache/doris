...
TEST_F(VariantColumnWriterReaderTest, doc_root_ignores_footer_layout_hints_and_rewrites_encoded) {
    constexpr int kDocBuckets = 2;
    ...
    for (size_t batch = 0; batch < batches.size(); ++batch) {
        ...
        EXPECT_EQ(reader_stats.variant_v2_shredded_output_rows, 0);
        EXPECT_EQ(column_reader_cache->path_reader_calls(), 0);
        ...
        for (size_t row = 0; row < rows; ++row) {
            EXPECT_EQ(variant_v2_json_at(values, assembled_rows - rows + row), batches[batch][row]);
        }
    }
    ...
    ASSERT_TRUE(assembled_values.is_encoded());
    size_t row = 0;
    for (const auto& batch : batches) {
    ...
    EXPECT_EQ(writer_metrics_after, writer_metrics_before);
    ASSERT_TRUE(assert_cast<const ColumnVariantV2&>(*assembled).is_encoded());
