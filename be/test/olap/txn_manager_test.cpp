...
TEST_F(TxnManagerTest, DeleteCommittedTxnForIngestingBinlog) {
    auto st = _txn_mgr->commit_txn(_meta, partition_id, transaction_id, tablet_id, schema_hash,
                                   _tablet_uid, load_id, _rowset_ingested, false);
    ...
    st = _txn_mgr->delete_txn(_meta, partition_id, transaction_id, tablet_id, schema_hash,
                              _tablet_uid);
    ASSERT_TRUE(st.ok()) << st;
    RowsetMetaSharedPtr rowset_meta2(new RowsetMeta());
    st = RowsetMetaManager::get_rowset_meta(_meta, _tablet_uid, _rowset_ingested->rowset_id(),
