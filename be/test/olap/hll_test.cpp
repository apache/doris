...
TEST_F(TestHll, Normal) {
    ...
    // empty
    {
        ...
        EXPECT_FALSE(HyperLogLog::is_valid(str));
    }
    // check unknown type
    {
        buf[0] = 60;
    ...
    // empty
    {
        ...
        EXPECT_EQ(0, test_hll.estimate_cardinality());
    ...
    HyperLogLog explicit_hll;
    {
        ...
        EXPECT_EQ(100, test_hll.estimate_cardinality());
    }
    // sparse [1024, 2048)
    HyperLogLog sparse_hll;
    {
        ...
        EXPECT_TRUE(len < HLL_REGISTERS_COUNT + 1);
        ...
        EXPECT_EQ(sparse_hll.estimate_cardinality(), cardinality);
        // 2% error rate
        EXPECT_TRUE(cardinality > 1000 && cardinality < 1045);
    }
    ...
    HyperLogLog full_hll;
    {
        ...
        EXPECT_EQ(HLL_REGISTERS_COUNT + 1, len);
        ...
        HyperLogLog test_hll(Slice((char*)buf, len));
        auto cardinality = test_hll.estimate_cardinality();
        EXPECT_EQ(full_hll.estimate_cardinality(), cardinality);
        // 2% error rate
        EXPECT_TRUE(cardinality > 62 * 1024 && cardinality < 66 * 1024);
    }
    // merge explicit to empty_hll
    {
        HyperLogLog new_explicit_hll;
        new_explicit_hll.merge(explicit_hll);
        EXPECT_EQ(100, new_explicit_hll.estimate_cardinality());
        ...
        // merge another explicit
        {
            ...
            EXPECT_TRUE(other_hll.estimate_cardinality() > 190);
        }
        // merge full
        {
            new_explicit_hll.merge(full_hll);
            EXPECT_TRUE(new_explicit_hll.estimate_cardinality() > full_hll.estimate_cardinality());
        }
    }
    // merge sparse to empty_hll
    {
        ...
        EXPECT_TRUE(new_sparse_hll.estimate_cardinality() > sparse_hll.estimate_cardinality());

        // merge full
        new_sparse_hll.merge(full_hll);
        EXPECT_TRUE(new_sparse_hll.estimate_cardinality() > full_hll.estimate_cardinality());
    }
}
