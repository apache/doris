...
TEST_F(TestEqualPredicate, TYPE_NAME##_COLUMN) { \
    ...
    SetTabletColumn(std::string("TYPE_NAME##_COLUMN"), FIELD_TYPE, \
                 "REPLACE", 1, false, true, &tablet_column); \
    schema.push_back(tablet_column); \
    int size = 10; \
...
TEST_F(TestEqualPredicate, DATE_COLUMN) {
    std::vector<TabletColumn> schema;
    TabletColumn tablet_column;
    SetTabletColumn(std::string("DATE_COLUMN"), "DATA",
                 "REPLACE", 1, false, true, &tablet_column);
    schema.push_back(tablet_column);
    int size = 6;
    std::vector<uint32_t> return_columns;
...
TEST_F(TestLessPredicate, TYPE_NAME##_COLUMN) { \
    std::vector<TabletColumn> schema; \
    TabletColumn tablet_column; \
    SetTabletColumn(std::string("TYPE_NAME_COLUMN"), FIELD_TYPE, \
                 "REPLACE", 1, false, true, &tablet_column); \
    schema.push_back(tablet_column); \
    int size = 10; \
    std::vector<uint32_t> return_columns; \
