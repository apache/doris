...
TEST(TestStream, UncompressOutStream) {
    ...
    EXPECT_TRUE(out_stream->_compressor == nullptr);

    out_stream->write(0x5a);
    out_stream->flush();

    EXPECT_EQ(out_stream->get_stream_length(), sizeof(StreamHead) + 1);

    EXPECT_EQ(out_stream->output_buffers().size(), 1);
...
TEST(TestStream, UncompressOutStream2) {
    ...
    EXPECT_EQ(out_stream->get_stream_length(), stream_length);

    EXPECT_EQ(out_stream->output_buffers().size(), 2);
...
TEST(TestStream, UncompressInStream) {
    ...
    EXPECT_NE(it, out_stream->output_buffers().end());
...
TEST(TestStream, CompressOutStream) {
    ...
    //EXPECT_EQ(out_stream->get_stream_length(), sizeof(StreamHead) + 2);
    ...
    StreamHead head;
    (*it)->get((char*)&head, sizeof(head));
    EXPECT_EQ(head.type, StreamHead::COMPRESSED);
...
TEST(TestStream, SeekUncompress) {
    ...
    EXPECT_EQ(index_entry.positions_count(), 2);
    EXPECT_EQ(index_entry.positions(0), 0);
    EXPECT_EQ(index_entry.positions(1), 1);
    out_stream->flush();
    ...
    InStream* in_stream =
            new (std::nothrow) InStream(&inputs, offsets, out_stream->get_stream_length(), nullptr,
                                        out_stream->get_total_buffer_size());
    EXPECT_EQ(in_stream->available(), 2);
    ...
    EXPECT_EQ(entry.positions_count(), 2);
    EXPECT_EQ(entry.positions(0), 0);
    EXPECT_EQ(entry.positions(1), 1);
...
TEST(TestStream, SeekCompress) {
    ...
    EXPECT_EQ(index_entry.positions_count(), 2);
    EXPECT_EQ(index_entry.positions(0), 0);
    EXPECT_EQ(index_entry.positions(1), 10);
    out_stream->flush();
...
TEST_F(TestRunLengthByte, ReadWriteMultiBytes) {
    ...
    EXPECT_EQ(_out_stream->get_stream_length(), sizeof(StreamHead) + 1 + 4);
...
TEST_F(TestRunLengthByte, ReadWriteSameBytes) {
    ...
    EXPECT_EQ(_out_stream->get_stream_length(), sizeof(StreamHead) + 1 + 1);
