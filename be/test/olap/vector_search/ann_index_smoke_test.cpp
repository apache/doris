...
class MockTabletColumn : public TabletColumn {
    MOCK_METHOD(FieldType, type, (), (const));
    MOCK_METHOD((const TabletColumn&), get_sub_column, (uint32_t), (const));
};

class MockTabletIndex : public TabletIndex {
    MOCK_METHOD(IndexType, index_type, (), (const));
    MOCK_METHOD((const std::map<string, string>&), properties, (), (const));
};

class MockIndexFileWriter : public segment_v2::IndexFileWriter {
public:
    MockIndexFileWriter(io::FileSystemSPtr fs)
            : segment_v2::IndexFileWriter(fs, "test_index", "rowset_id", 1,
                                          InvertedIndexStorageFormatPB::V2) {}

    MOCK_METHOD(Result<std::shared_ptr<segment_v2::DorisFSDirectory>>, open,
                (const TabletIndex* index_meta), (override));
};

class AnnIndexReaderWriterTest : public testing::Test {
protected:
    void SetUp() override {
        // Create a tmp_file_dirs, this will be used by
        const std::string testDir = "./ut_dir/AnnIndexReaderWriterTest";
        ...
        std::map<std::string, std::string> properties = {
                {segment_v2::AnnIndexWriter::INDEX_TYPE, "hnsw"},
                {segment_v2::AnnIndexWriter::DIM, "10"},
                {segment_v2::AnnIndexWriter::MAX_DEGREE, "32"},
                {segment_v2::AnnIndexWriter::QUANTILIZER, "flat"}};

        EXPECT_CALL(*_index_meta, properties()).WillOnce(testing::ReturnRef(properties));

        EXPECT_TRUE(segment_v2::IndexWriter::create(&field, &_ann_index_col_writer,
    ...
    std::unique_ptr<segment_v2::IndexWriter> _ann_index_col_writer;
...
TEST_F(AnnIndexReaderWriterTest, SmokeTest) {
    segment_v2::AnnIndexWriter* ann_index_writer =
            dynamic_cast<segment_v2::AnnIndexWriter*>(_ann_index_col_writer.get());
    ...
    */
    ...
    segment_v2::SearchParameters params;
    segment_v2::SearchResult result;
    ASSERT_TRUE(index2->search(query_vec.get(), 1, params, result));
    EXPECT_TRUE(result.row_ids->cardinality() == 1);
    EXPECT_TRUE(result.row_ids->contains(0));
} // namespace doris
