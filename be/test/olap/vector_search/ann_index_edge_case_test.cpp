...
class AnnIndexEdgeCaseTest : public doris::vectorized::VectorSearchTest {
protected:
    void SetUp() override { doris::vectorized::VectorSearchTest::SetUp(); }

    void TearDown() override { doris::vectorized::VectorSearchTest::TearDown(); }
};

TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexStatsInitialization) {
...
TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexStatsCopyConstructor) {
...
TEST_F(AnnIndexEdgeCaseTest, TestRangeSearchParamsToString) {
...
TEST_F(AnnIndexEdgeCaseTest, TestRangeSearchParamsWithNullRoaring) {
    doris::vectorized::RangeSearchParams params;
...
TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexParamValidation) {
...
TEST_F(AnnIndexEdgeCaseTest, TestVectorSearchUserParamsDefaultValues) {
    doris::VectorSearchUserParams params;

    // Test default values
    EXPECT_EQ(params.hnsw_ef_search, 10);
    EXPECT_EQ(params.hnsw_check_relative_distance, true);
    EXPECT_EQ(params.hnsw_bounded_queue, true);
}

TEST_F(AnnIndexEdgeCaseTest, TestVectorSearchUserParamsEquality) {
...
TEST_F(AnnIndexEdgeCaseTest, TestIndexSearchResultInitialization) {
    doris::vectorized::IndexSearchResult result;
...
TEST_F(AnnIndexEdgeCaseTest, TestRangeSearchResultInitialization) {
...
TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexWriterWithEmptyProperties) {
    ...
    auto writer =
            std::make_unique<AnnIndexColumnWriter>(mock_file_writer.get(), tablet_index.get());
...
TEST_F(AnnIndexEdgeCaseTest, TestAnnIndexReaderWithInvalidMetricType) {
    // Test reader with unknown metric type
    std::map<std::string, std::string> properties;
    properties["index_type"] = "hnsw";
    properties["metric_type"] = "unknown_metric";

    auto tablet_index = std::make_unique<doris::TabletIndex>();
    tablet_index->_properties = properties;
    tablet_index->_index_id = 1;

    auto mock_file_reader = std::make_shared<MockIndexFileReader>();

    // This should throw an exception due to unknown metric type
    EXPECT_THROW(
            auto reader = std::make_unique<AnnIndexReader>(tablet_index.get(), mock_file_reader),
            std::exception);
}

TEST_F(AnnIndexEdgeCaseTest, TestLargeVectorDimensions) {
...
TEST_F(AnnIndexEdgeCaseTest, TestZeroDimensionVectors) {
    // Test with zero dimension (should fail)
    std::map<std::string, std::string> properties;
    properties["index_type"] = "hnsw";
    properties["metric_type"] = "l2_distance";
    properties["dim"] = "0"; // Zero dimension
    ...
    // This should fail or throw an exception due to zero dimension
    EXPECT_THROW(writer->init(), std::exception);
}

TEST_F(AnnIndexEdgeCaseTest, TestNegativeMaxDegree) {
    // Test with negative max_degree (should be handled gracefully)
    std::map<std::string, std::string> properties;
    properties["index_type"] = "hnsw";
    properties["metric_type"] = "l2_distance";
    properties["dim"] = "4";
    properties["max_degree"] = "-1"; // Negative max_degree
    ...
    // This might fail or be handled gracefully depending on implementation
    try {
        Status status = writer->init();
        // If it succeeds, the negative value might be converted or handled internally
...
TEST_F(AnnIndexEdgeCaseTest, TestEmptyRoaringBitmap) {
...
TEST_F(AnnIndexEdgeCaseTest, TestLargeRoaringBitmap) {
