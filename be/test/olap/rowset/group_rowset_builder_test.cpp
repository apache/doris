...
void create_recovery_tablets(TCreateTabletReq base_request,
...
void prepare_recovery_schema(bool historical, bool key_only,
    ...
    auto base_request = testutil::create_tablet_request(
            ...
            {{.column_name = "k1", .type = TPrimitiveType::INT, .is_key = true},
             {.column_name = "v1", .type = TPrimitiveType::INT, .is_key = false}});
    if (key_only) {
    ...
    ASSERT_NO_FATAL_FAILURE(create_recovery_tablets(base_request, row_binlog_schema));

    TDescriptorTable tdesc_tbl =
            key_only ? testutil::create_descriptor_table(
                               {{.type = TYPE_INT, .column_name = "k1", .nullable = false}})
                     : testutil::create_descriptor_table(
                               {{.type = TYPE_INT, .column_name = "k1", .nullable = false},
                                {.type = TYPE_INT, .column_name = "v1", .nullable = false}});
    schema_param = testutil::create_table_schema_param(
...
void commit_recovery_rowsets(const std::shared_ptr<OlapTableSchemaParam>& schema_param,
                             bool pending_publish) {
    PUniqueId load_id;
    load_id.set_hi(0);
    load_id.set_lo(1);
    for (int64_t committed_txn = txn_id; committed_txn <= txn_id + pending_publish;
         ++committed_txn) {
        for (const auto& [base_tablet_id, row_binlog_tablet_id] : tablet_pairs) {
            WriteRequest data_req;
...
Status publish_recovery_txn(const TPublishVersionRequest& publish_request,
    ...
    discontinuous.clear();
    EnginePublishVersionTask task(*engine_ref, publish_request, &errors, &successes, &discontinuous,
                                  &delta_rows);
    return task.execute();
}

// GoogleTest macros inflate complexity in the repeated invalid-snapshot checks.
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
void check_invalid_recovery_snapshots(TPublishVersionRequest& publish_request,
                                      const std::vector<TRowBinlogWriteColumnMapping>& entries,
                                      bool historical) {
    std::vector<DiscontinuousVersionTablet> discontinuous;
    ...
    EXPECT_FALSE(publish_recovery_txn(publish_request, discontinuous).ok());
    ASSERT_NO_FATAL_FAILURE(assert_committed());
...
void recover_pending_publish(TPublishVersionRequest& publish_request, bool historical,
                             bool key_only, int32_t max_gap) {
    std::vector<DiscontinuousVersionTablet> discontinuous;
    ...
    EXPECT_TRUE(publish_recovery_txn(publish_request, discontinuous)
                        .is<ErrorCode::PUBLISH_VERSION_NOT_CONTINUOUS>());
    if (max_gap == 0) {
        EXPECT_TRUE(discontinuous.empty());
    } else {
        ASSERT_EQ(discontinuous.size(), tablet_pairs.size());
        for (const auto& item : discontinuous) {
            engine_ref->add_async_publish_task(item.partition_id, item.tablet_id,
                                               item.publish_version, txn_id, false, item.commit_tso,
                                               item.row_binlog_column_mappings);
        }
    }
    ...
    auto st = publish_recovery_txn(publish_request, discontinuous);
    ASSERT_TRUE(st.ok()) << st;
...
// GoogleTest macros inflate complexity in the per-tablet recovery assertions.
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
void check_recovered_rowsets_and_alter_schema(
...
// GoogleTest macros inflate complexity in the per-tablet snapshot assertions.
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
void check_published_recovery_rowsets(
        const std::map<TabletInfo, std::shared_ptr<TabletTxnInfo>>& txn_infos, bool historical,
        bool key_only, int64_t version) {
    for (const auto& [base_tablet_id, row_binlog_tablet_id] : tablet_pairs) {
        auto base_tablet = engine_ref->tablet_manager()->get_tablet(base_tablet_id);
        auto binlog_tablet = engine_ref->tablet_manager()->get_tablet(row_binlog_tablet_id);
        EXPECT_EQ(base_tablet->max_version().second, version);
        EXPECT_EQ(binlog_tablet->max_version().second, version);
...
// ASSERT_NO_FATAL_FAILURE expands control flow at each phase boundary.
// NOLINTNEXTLINE(readability-function-cognitive-complexity)
void recover_multiple_row_binlog_pairs(bool historical, bool key_only,
                                       std::optional<int32_t> max_gap = std::nullopt) {
    std::shared_ptr<OlapTableSchemaParam> schema_param;
    ASSERT_NO_FATAL_FAILURE(prepare_recovery_schema(historical, key_only, schema_param));
    std::vector<TRowBinlogWriteColumnMapping> entries;
    for (const auto& mapping : schema_param->indexes()[0]->row_binlog_column_mappings.entries()) {
    ...
    ASSERT_NO_FATAL_FAILURE(commit_recovery_rowsets(schema_param, max_gap.has_value()));
    restart_engine();
    ...
    ASSERT_NO_FATAL_FAILURE(check_invalid_recovery_snapshots(publish_request, entries, historical));
    if (max_gap.has_value()) {
        ASSERT_NO_FATAL_FAILURE(
                recover_pending_publish(publish_request, historical, key_only, *max_gap));
    }

    std::map<TabletInfo, std::shared_ptr<TabletTxnInfo>> txn_infos;
    ASSERT_NO_FATAL_FAILURE(check_recovered_rowsets_and_alter_schema(txn_infos));
    if (max_gap.has_value()) {
    ...
    ASSERT_NO_FATAL_FAILURE(check_published_recovery_rowsets(txn_infos, historical, key_only,
                                                             max_gap.has_value() ? 3 : 2));
}
} // namespace
