...
TEST_F(BlockBloomFilterTest, Normal) {
    ...
    auto ret = BloomFilter::create(CLASSIC_BLOOM_FILTER, &bf);
    ASSERT_FALSE(ret);
    ASSERT_EQ(nullptr, bf);
    ret = BloomFilter::create(BLOCK_BLOOM_FILTER, &bf);
    ASSERT_TRUE(ret);
    ASSERT_NE(nullptr, bf);
    ret = bf->init(bits_num / 8, HASH_MURMUR3_X64_64);
    ASSERT_TRUE(ret);
    ...
    ret = BloomFilter::create(BLOCK_BLOOM_FILTER, &bf2);
    ASSERT_TRUE(ret);
...
TEST_F(BlockBloomFilterTest, SP) {
    ...
    uint32_t bits_num = BloomFilter::optimal_bit_num(_expected_num, _fpp);
    std::unique_ptr<BloomFilter> bf;
    auto ret = BloomFilter::create(BLOCK_BLOOM_FILTER, &bf);
    ASSERT_TRUE(ret);
    ASSERT_NE(nullptr, bf);
    ret = bf->init(bits_num / 8, HASH_MURMUR3_X64_64);
    ASSERT_TRUE(ret);
