...
TEST(ExternalColMetaUtilTest, PointersAndUidMapValidation) {
    ...
    EXPECT_TRUE(ExternalColMetaUtil::parse_external_meta_pointers(footer, &ptrs));
    ...
    EXPECT_TRUE(ExternalColMetaUtil::parse_uid_to_colid_map(footer, ptrs, &uid2colid));
    EXPECT_EQ(uid2colid.size(), 2U);
    EXPECT_EQ(uid2colid[10], 0U);
    EXPECT_EQ(uid2colid[20], 1U);

    // Negative cases for pointers
    footer.set_num_columns(0);
    EXPECT_FALSE(ExternalColMetaUtil::parse_external_meta_pointers(footer, &ptrs));
    footer.set_num_columns(2);
    ...
    // Negative case for uid‑>col_id sizing
    footer.clear_column_meta_entries();
    auto* e2 = footer.add_column_meta_entries();
    e2->set_unique_id(10);
    e2->set_length(50);
    ptrs.num_columns = 2;
    uid2colid.clear();
    EXPECT_FALSE(ExternalColMetaUtil::parse_uid_to_colid_map(footer, ptrs, &uid2colid));
...
TEST(ExternalColMetaUtilTest, VariantExtMetaWriterAndReaderInteropAndCompatibility) {
...

    {
        ...
        st = ext_writer.externalize_from_footer(&footer);
        ASSERT_TRUE(st.ok()) << st.to_json();
    }
    ...
    // 3. Verify that ext meta keys/values metas were flushed into footer.file_meta_datas.
    bool found_keys_meta = false;
    bool found_vals_meta = false;
    std::string keys_meta_bytes;
    std::string vals_meta_bytes;
    for (const auto& m : footer.file_meta_datas()) {
        ...
        } else if (m.key() == "variant_meta_values." + std::to_string(root_uid)) {
            found_vals_meta = true;
            vals_meta_bytes = m.value();
        }
    }
    ASSERT_TRUE(found_keys_meta);
    ASSERT_TRUE(found_vals_meta);
    ...
    //      "variant_meta_keys" / "variant_meta_values"
    ...
    legacy_footer.clear_file_meta_datas();
    {
        ...
        auto* p2 = legacy_footer.add_file_meta_datas();
        p2->set_key("variant_meta_values");
        p2->set_value(vals_meta_bytes);
    }
...
TEST(ExternalColMetaUtilTest, WriteAndReadV3SegmentExternalMetaRoundTrip) {
    ...
    ExternalColMetaUtil::ExternalMetaPointers ptrs;
    ASSERT_TRUE(ExternalColMetaUtil::parse_external_meta_pointers(loaded_footer, &ptrs));
    std::unordered_map<int32_t, size_t> uid2colid;
    ASSERT_TRUE(ExternalColMetaUtil::parse_uid_to_colid_map(loaded_footer, ptrs, &uid2colid));
