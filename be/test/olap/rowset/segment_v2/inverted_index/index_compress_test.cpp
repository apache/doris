...
class IndexCompressTest : public testing::Test {
public:
    ...
    IndexCompressTest() = default;
    ~IndexCompressTest() override = default;
...
static void write_index(const std::string& name, const std::string& path,
                        IndexVersion index_version, const std::vector<std::string>& datas) {
...
static void read_index(const std::string& path, int32_t doc_count) {
...
TEST_F(IndexCompressTest, test_write_v2) {
    ...
    write_index(name, path, IndexVersion::kV2, datas);

    try {
        read_index(path, doc_count);
    } catch (...) {
        EXPECT_TRUE(false);
    }
}

TEST_F(IndexCompressTest, test_write_v3) {
    std::srand(getDaySeed());
    ...
    std::string path = kTestDir + "/index1";
    write_index(name, path, IndexVersion::kV3, datas);
...
TEST_F(IndexCompressTest, test_compaction_v2) {
    std::srand(getDaySeed());
    ...
    // index compaction v3
    {
        ...
        destDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index5").c_str()));
        destDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index6").c_str()));

        std::string path = kTestDir + "/index0";
        std::exception_ptr eptr;
        try {
            index_compaction(path, srcDirs, destDirs, doc_count);
        } catch (...) {
            eptr = std::current_exception();
        }
        FINALLY(eptr, {
        ...
        try {
            read_index(std::string(kTestDir + "/index4"), doc_count * 3);
            read_index(std::string(kTestDir + "/index5"), doc_count * 3);
            read_index(std::string(kTestDir + "/index6"), doc_count * 3);
        } catch (const CLuceneError& e) {
            std::cout << "e: " << e.number() << ", " << e.what() << std::endl;
            EXPECT_TRUE(false);
        } catch (...) {
            EXPECT_TRUE(false);
        }
...
TEST_F(IndexCompressTest, test_compaction_v3) {
    std::srand(getDaySeed());
    std::string name = "field_name";

    // index v3
    {
        std::string path = kTestDir + "/index3";
        std::vector<std::string> datas;
        for (int32_t i = 0; i < doc_count; i++) {
            std::string ip_v4 = generateRandomIP();
            datas.emplace_back(ip_v4);
    ...
    {
        std::vector<lucene::store::Directory*> srcDirs;
        srcDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index3").c_str()));
        srcDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index3").c_str()));
        srcDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index3").c_str()));
        srcDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index3").c_str()));
        srcDirs.push_back(FSDirectory::getDirectory(std::string(kTestDir + "/index3").c_str()));
...
TEST_F(IndexCompressTest, test_compaction_exception) {
    std::srand(getDaySeed());
    std::string name = "field_name";
    ...
    // index v1
    {
        ...
        write_index(name, path, IndexVersion::kV1, datas);
    }
