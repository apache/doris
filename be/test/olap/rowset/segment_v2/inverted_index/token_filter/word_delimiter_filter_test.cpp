...
#include "olap/rowset/segment_v2/inverted_index/token_filter/word_delimiter_iterator.h"
...
TokenStreamPtr create_filter(const std::string& text, int32_t flags,
    ...
    return std::make_shared<WordDelimiterFilter>(
...
TEST(WordDelimiterFilterTest, PreserveOriginalAndGenerateParts) {
    ...
    std::vector<ExpectedToken> expected = {{"foo-bar", 1}, {"foo", 1}, {"bar", 1}};
