...
constexpr static std::string_view dest_dir = "/ut_dir/variant_multi_index_test";
constexpr static std::string_view tmp_dir = "./ut_dir/tmp";

class VariantMultiIndexTest : public testing::Test {
public:
    void SetUp() override {
        // absolute dir
...
TEST_F(VariantMultiIndexTest, test_variant_multi_index) {
    ...
    SchemaUtils::construct_column(schema_pb.add_column(), 2, "VARIANT", "V1", 2, false);
    _tablet_schema = std::make_shared<TabletSchema>();
    _tablet_schema->init_from_pb(schema_pb);

    TabletColumn& variant = _tablet_schema->mutable_column_by_uid(2);
    // add subcolumn
    TabletColumn subcolumn2;
    subcolumn2.set_name("key1");
    subcolumn2.set_type(FieldType::OLAP_FIELD_TYPE_STRING);
    variant.add_sub_column(subcolumn2);

    TabletColumn subcolumn3;
    subcolumn3.set_name("key2");
    subcolumn3.set_type(FieldType::OLAP_FIELD_TYPE_INT);
    variant.add_sub_column(subcolumn3);

    TabletColumn subcolumn4;
    subcolumn4.set_name("key3");
    subcolumn4.set_type(FieldType::OLAP_FIELD_TYPE_STRING);
    variant.add_sub_column(subcolumn4);

    TabletIndex index2;
    index2._properties["field_pattern"] = "key1";
    index2._col_unique_ids = {2};
    index2._index_type = IndexType::INVERTED;
    index2._index_id = 1001;
    _tablet_schema->append_index(std::move(index2));

    TabletIndex index3;
    index3._properties["field_pattern"] = "key2";
    index3._col_unique_ids = {2};
    index3._index_type = IndexType::INVERTED;
    index3._index_id = 1002;
    _tablet_schema->append_index(std::move(index3));

    TabletIndex index4;
    index4._properties["field_pattern"] = "key3";
    index4._col_unique_ids = {2};
    index4._index_type = IndexType::INVERTED;
    index4._index_id = 1003;
    _tablet_schema->append_index(std::move(index4));

    TabletIndex index5;
    index5._properties["field_pattern"] = "key3";
    index5._properties[INVERTED_INDEX_PARSER_KEY] = INVERTED_INDEX_PARSER_UNICODE;
    index5._col_unique_ids = {2};
    index5._index_type = IndexType::INVERTED;
    index5._index_id = 1004;
    _tablet_schema->append_index(std::move(index5));

    TabletIndex index6;
    index6._properties[INVERTED_INDEX_PARSER_KEY] = INVERTED_INDEX_PARSER_UNICODE;
    index6._col_unique_ids = {2};
    index6._index_type = IndexType::INVERTED;
    index6._index_id = 1005;
    _tablet_schema->append_index(std::move(index6));
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int seg_id = 0; seg_id < rowset->num_segments(); seg_id++) {
            ...
            EXPECT_TRUE(io::global_local_filesystem()
                                ->file_size(inverted_index_file_path, &file_size)
                                .ok());
            EXPECT_TRUE(file_size > 0);
            auto inverted_index_file_reader = std::make_unique<InvertedIndexFileReader>(
                    io::global_local_filesystem(), std::string(index_file_path_prefix),
                    InvertedIndexStorageFormatPB::V2);
            EXPECT_TRUE(inverted_index_file_reader->init().ok());
            auto dirs = inverted_index_file_reader->get_all_directories();
            EXPECT_TRUE(dirs.has_value()) << dirs.error();
            EXPECT_EQ(dirs.value().size(), 6);
            total_index += dirs.value().size();
        }
        rowsets.push_back(rowset);
    }
    EXPECT_EQ(total_index, 150);
    ...
    for (int seg_id = 0; seg_id < output_rowset->num_segments(); seg_id++) {
        auto seg_path = output_rowset->segment_path(seg_id);
        EXPECT_TRUE(seg_path.has_value()) << seg_path.error();
        auto index_file_path_prefix =
                InvertedIndexDescriptor::get_index_file_path_prefix(seg_path.value());
        int64_t file_size = 0;
