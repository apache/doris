...
TEST_F(ColumnReaderWriterTest, test_cast_iterator_int_to_double_next_batch) {
    ...
    for (size_t i = 0; i < n; ++i) {
        auto v = assert_cast<vectorized::ColumnVector<vectorized::Float64>*>(dst.get())->get_element(i);
...
TEST_F(ColumnReaderWriterTest, test_cast_iterator_nullable_src_to_nullable_dst) {
    ...
    vectorized::MutableColumnPtr dst = vectorized::make_nullable(std::make_shared<vectorized::DataTypeInt64>())
                                               ->create_column();
    size_t n = 4;
    bool has_null = false;
    st = cast_iter.next_batch(&n, dst, &has_null);
