...
TEST_F(VariantColumnWriterReaderTest, test_write_data_normal) {
    ...
    auto check_leaf_reader = [&]() {
        for (int i = 0; i < 3; ++i) {
            ...
            st = variant_column_reader->new_iterator(&it, &subcolumn, &storage_read_opts,
                                                     &column_reader_cache);
            EXPECT_TRUE(st.ok()) << st.msg();
            EXPECT_TRUE(assert_cast<FileColumnIterator*>(it.get()) != nullptr);
            st = it->init(column_iter_opts);
            EXPECT_TRUE(st.ok()) << st.msg();
    ...
    subcolumn_compaction.set_path_info(PathInData(parent_column.name_lower_case() + ".key10"));
    ...
    // test VariantRootColumnIterator for next_batch and read_by_rowids
    {
        ...
        auto nullable_dt = std::make_shared<vectorized::DataTypeNullable>(
                std::make_shared<vectorized::DataTypeObject>(3));
...

    {
        ...
        MutableColumnPtr sparse_dst1 = ColumnObject::create(3);
        ...
        MutableColumnPtr sparse_dst2 =
                ColumnNullable::create(ColumnObject::create(3), ColumnUInt8::create());
...
TEST_F(VariantColumnWriterReaderTest, test_no_sub_in_sparse_column) {
    ...
    std::cout << "column_object size: "
              << assert_cast<ColumnObject*>(column_object.get())->debug_string() << std::endl;
...
void test_write_variant_column(StorageEngine* _engine_ref, std::string _absolute_dir,
    ...
    TabletSchemaPB schema_pb;
    schema_pb.set_keys_type(KeysType::DUP_KEYS);
    SchemaUtils::construct_column(schema_pb.add_column(), 1, "VARIANT", "v", 3, false, nullable);
    _tablet_schema->init_from_pb(schema_pb);
...
TEST_F(VariantColumnWriterReaderTest, test_nested_subcolumn) {
    ...
    // make some error senior
    {
        ...
        EXPECT_TRUE(st.ok()) << st.msg();
        st = nested_iter11->seek_to_first();
        EXPECT_TRUE(st.ok()) << st.msg();
        MutableColumnPtr wrong_arr = ColumnObject::create(3);
        size_t nrows = 1000;
...
TEST_F(VariantColumnWriterReaderTest, test_nested_iter) {
    ...
    MutableColumnPtr new_column_object1 = ColumnObject::create(3);
    ...
    {
        // fill with nullable columnObject target
        MutableColumnPtr new_column_object12 = ColumnObject::create(3);
    ...
    // read dst is nullable column object
    {
        ...
        EXPECT_TRUE(st.ok()) << st.msg();
        // fill with nullable columnObject target
        MutableColumnPtr new_column_object2 = ColumnObject::create(3);
    ...
    // test _process_with_nested_column for offsets not equals
    {
        ...
        MutableColumnPtr nested_column_object = ColumnObject::create(3);
        auto& container_variant = assert_cast<ColumnObject&>(*nested_column_object);
