...
TEST(TabletMetaTest, DeserializeFromMemoryWithOldHeader) {
    ...
    old_header.file_length = 100;
    old_header.checksum = 200;
    old_header.protobuf_length = 300;
    old_header.protobuf_checksum = 400;
    memcpy(buffer.data(), &old_header, sizeof(old_header));
...
TEST(TabletMetaTest, DeserializeFromMemoryWithInvalidChecksum) {
    ...
    buffer[sizeof(FixedFileHeaderV2) + 1] ^= 0xFF;
