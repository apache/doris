    ...
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back([&]() {
            for (int j = 0; j < 100; ++j) {
                ...
                uint32_t id = id_file_map.add_file_mapping(mapping);
                auto retrieved_mapping = id_file_map.get_file_mapping(id);
                EXPECT_EQ(retrieved_mapping->type, FileMappingType::DORIS_FORMAT);
                EXPECT_EQ(retrieved_mapping->value, "file" + std::to_string(id));
            }
        });
    }
    ...
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back([&]() {
            for (int j = 0; j < 10; ++j) {
                ...
                auto id_file_map = id_manager.add_id_file_map(query_id);
                EXPECT_NE(id_file_map, nullptr);

                id_manager.remove_id_file_map(query_id);
                auto retrieved_id_file_map = id_manager.add_id_file_map(query_id);
