...
TEST_F(BlockColumnPredicateTest, PARQUET_NULL_PREDICATE_BLOOM_FILTER) {
    const int col_idx = 0;
    ...
    std::function<bool(vectorized::ParquetPredicate::ColumnStat*, int)> get_stat_func =
            [&](vectorized::ParquetPredicate::ColumnStat* current_stat, int cid) {
                EXPECT_EQ(col_idx, cid);
                current_stat->col_schema = parquet_field.get();
                current_stat->is_all_null = true;
                current_stat->has_null = true;
                current_stat->encoded_min_value.assign(sizeof(int64_t), '\0');
                current_stat->encoded_max_value.assign(sizeof(int64_t), '\0');
                return true;
            };
    stat.get_stat_func = &get_stat_func;
