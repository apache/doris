...
void single_column_eq_test_func(vectorized::MutableColumnPtr&& column_data, float check_value) {
    vectorized::MutableColumns block;
    block.emplace_back(std::move(column_data));

    int col_idx = 0;
    std::unique_ptr<ColumnPredicate> pred(
            new ComparisonPredicateBase<T, PredicateType::EQ>(col_idx, check_value));
    SingleColumnBlockPredicate single_column_block_pred(pred.get());

    int rows = column_data->size();
    std::vector<uint16_t> sel_idx(rows);
    uint16_t selected_size = rows;
    block[col_idx]->reserve(rows);
    for (int i = 0; i < rows; i++) {
    ...
    selected_size = single_column_block_pred.evaluate(block, sel_idx.data(), selected_size);
    EXPECT_EQ(selected_size, 1);
    auto* pred_col = reinterpret_cast<vectorized::PredicateColumnType<T>*>(block[col_idx].get());
    EXPECT_EQ(pred_col->get_data()[sel_idx[0]], check_value);
}
std::vector<double> double_special_values {
        std::numeric_limits<double>::lowest(),    std::numeric_limits<double>::max(),
        std::numeric_limits<double>::infinity(),  -std::numeric_limits<double>::infinity(),
        std::numeric_limits<double>::quiet_NaN(),
};
std::vector<float> float_special_values {
        std::numeric_limits<float>::lowest(),    std::numeric_limits<float>::max(),
        std::numeric_limits<float>::infinity(),  -std::numeric_limits<float>::infinity(),
        std::numeric_limits<float>::quiet_NaN(),
};
vectorized::MutableColumnPtr create_and_fill_test_column() {
    auto column = vectorized::PredicateColumnType<TYPE_DOUBLE>::create();
    for (int i = 0; i < 10; i++) {
        double v = i;
        column->insert_data((char*)&v, 0);

        v = -v;
        column->insert_data((char*)&v, 0);
    }
    for (auto v : double_special_values) {
        column->insert_data((char*)&v, 0);
    }
    return column;
}
