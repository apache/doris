...
private:
    std::shared_ptr<QueryContext> _generate_on_query(std::shared_ptr<WorkloadGroup>& wg) {
        ...
        st = query_context->set_workload_group(wg);
        EXPECT_TRUE(st.ok()) << "set query's workload group failed: " << st.to_string();
        return query_context;
    }

    void _run_checking_loop(const std::shared_ptr<WorkloadGroup>& wg) {
        CountDownLatch latch(1);
        size_t check_times = 100;
        while (--check_times > 0) {
...
TEST_F(WorkloadGroupManagerTest, add_paused_query) {
    auto wg = _wg_manager->get_or_create_workload_group({});
    auto query_context = _generate_on_query(wg);

    query_context->update_paused_reason(Status::Error(ErrorCode::QUERY_MEMORY_EXCEEDED, "test"));
    query_context->set_mem_limit(1024 * 1024);

    _wg_manager->add_paused_query(query_context, 1024);

    {
        ...
        SCOPED_ATTACH_TASK(query_context->query_mem_tracker);
        query_context->query_mem_tracker->consume(1024 * 1024 * 2);
    }
...
TEST_F(WorkloadGroupManagerTest, wg_exceed1) {
    ...
    query_context->update_paused_reason(
...
TEST_F(WorkloadGroupManagerTest, wg_exceed3) {
    WorkloadGroupInfo wg_info {.id = 1, .memory_limit = 1024L * 1024};
    ...
    ASSERT_EQ(query_context->exec_status().code(), ErrorCode::MEM_LIMIT_EXCEEDED);
...
TEST_F(WorkloadGroupManagerTest, slot_memory_policy_disabled) {
    WorkloadGroupInfo wg_info {.id = 1,
                               ...
                               .slot_mem_policy = TWgSlotMemoryPolicy::DISABLED};
    auto wg = _wg_manager->get_or_create_workload_group(wg_info);
    EXPECT_EQ(wg->id(), wg_info.id);
    EXPECT_EQ(wg->enable_memory_overcommit(), false);
    EXPECT_EQ(wg->slot_memory_policy(), TWgSlotMemoryPolicy::DISABLED);
...
TEST_F(WorkloadGroupManagerTest, query_released) {
    ...
    canceled_query->update_paused_reason(
            Status::Error(ErrorCode::WORKLOAD_GROUP_MEMORY_EXCEEDED, "test for canceled"));

    _wg_manager->add_paused_query(query_context, 1024L * 1024 * 1024);
    _wg_manager->add_paused_query(canceled_query, 1024L * 1024 * 1024);
