...
class FailingKinesisPipe : public io::KinesisConsumerPipe {
public:
    Status append_json(const char*, size_t) override {
        return Status::InternalError("append failed");
    }
};
...
TEST_F(KinesisBatchProgressReproduction, AppendFailureDoesNotPublishProgressOrEof) {
    auto ctx = StreamLoadContext::create_shared(nullptr);
    ctx->load_type = TLoadType::ROUTINE_LOAD;
    ctx->load_src_type = TLoadSourceType::KINESIS;
    ctx->format = TFileFormatType::FORMAT_JSON;
    ...
    KinesisQueueItem record_item;
    record_item.shard_id = "shard-0";
    record_item.record = std::make_shared<Aws::Kinesis::Model::Record>();
    record_item.record->SetSequenceNumber("1");
    const std::string payload = "{\"id\":1}";
    record_item.record->SetData(Aws::Utils::ByteBuffer(
            reinterpret_cast<const unsigned char*>(payload.data()), payload.size()));
    ASSERT_TRUE(group._queue.blocking_put(record_item));
    ...
    auto pipe = std::make_shared<FailingKinesisPipe>();
    Status result = Status::OK();
    ASSERT_FALSE(group._run_consume_loop(ctx, pipe, result).ok());
    ASSERT_TRUE(ctx->kinesis_info->cmt_sequence_number.empty());
    ASSERT_TRUE(ctx->kinesis_info->closed_shard_ids.empty());
}

TEST_F(KinesisBatchProgressReproduction, EndMarkerCarriesAllChildParents) {
    auto ctx = StreamLoadContext::create_shared(nullptr);
    ctx->load_type = TLoadType::ROUTINE_LOAD;
    ctx->load_src_type = TLoadSourceType::KINESIS;
    ctx->format = TFileFormatType::FORMAT_JSON;
    ...
    KinesisQueueItem end_marker;
    end_marker.shard_id = "parent-1";
    end_marker.end_of_shard = true;
    end_marker.child_shard_parent_ids["child"] = {"parent-1", "parent-2"};
    ASSERT_TRUE(group._queue.blocking_put(end_marker));
    group._queue.shutdown();

    auto pipe = std::make_shared<RecordingKinesisPipe>();
    Status result = Status::OK();
    ASSERT_TRUE(group._run_consume_loop(ctx, pipe, result).ok());
    ASSERT_EQ(1, ctx->kinesis_info->closed_shard_ids.count("parent-1"));
    ASSERT_EQ(1, ctx->kinesis_info->child_shard_parent_ids.count("child"));
    const auto& parents = ctx->kinesis_info->child_shard_parent_ids.at("child");
    ASSERT_EQ(2, parents.size());
    ASSERT_TRUE(parents.count("parent-1"));
    ASSERT_TRUE(parents.count("parent-2"));
}
