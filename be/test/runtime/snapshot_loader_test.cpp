...
TEST_F(SnapshotLoaderTest, TestLinkSameRowsetFiles) {
    ...
    // 9. Verify file linking - check if files were properly linked rather than downloaded
    ...
    // Look for linked files (files containing the new rowset ID that should be linked)
    ...
    for (const auto& file : downloaded_files) {
        if (file.file_name.find(".dat") != std::string::npos) {
            ...
            ASSERT_GT(file_stat.st_size, 0) << "Linked file is empty: " << file_path;

            std::cout << "Found linked file: " << file_path << ", size: " << file_stat.st_size
                      << " bytes" << std::endl;
            break;
        }
    }

    ASSERT_TRUE(found_linked_file) << "Linked rowset file not found in download directory";
}
} // namespace doris
