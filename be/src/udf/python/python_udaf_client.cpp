...
static Status validate_and_cast_column(const std::shared_ptr<arrow::RecordBatch>& batch,
                                       int column_index, const std::string& operation_name,
                                       std::shared_ptr<ArrowArrayType>* out) {
    if (UNLIKELY(column_index >= batch->num_columns())) {
        return Status::InternalError("{} response: column index {} out of range (total {} columns)",
                                     operation_name, column_index, batch->num_columns());
    }
    ...
    if (UNLIKELY(!column->type()->Equals(expected_type))) {
        return Status::InternalError(
                "{} response: expected column {} to be of type {}, but got type {}", operation_name,
                column_index, expected_type->ToString(), column->type()->ToString());
    }
...
Status PythonUDAFClient::create(int64_t place_id) {
    ...
    // Create unified batch for CREATE operation (no metadata or data needed)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(
            create_unified_batch(UDAFOperation::CREATE, place_id, nullptr, nullptr, &batch));
    ...
    if (output->num_columns() != 1 || output->num_rows() != 1) {
        return Status::InternalError("Invalid CREATE response from Python UDAF server");
    }

    std::shared_ptr<arrow::BooleanArray> bool_array;
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "CREATE", &bool_array));
...
Status PythonUDAFClient::accumulate(int64_t place_id, bool is_single_place,
    ...
    // Add places array if needed
    if (!is_single_place && places != nullptr) {
    ...
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "ACCUMULATE", &int64_array));
...
Status PythonUDAFClient::serialize(int64_t place_id,
    ...
    // Create unified batch for SERIALIZE operation (no metadata or data needed)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(
            create_unified_batch(UDAFOperation::SERIALIZE, place_id, nullptr, nullptr, &batch));

    // Send and receive serialized state
    std::shared_ptr<arrow::RecordBatch> output;
    RETURN_IF_ERROR(_send_operation(batch.get(), &output));
    ...
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "SERIALIZE", &binary_array));
...
Status PythonUDAFClient::merge(int64_t place_id,
    ...
    // Create unified batch for MERGE operation (data = serialized_state)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(create_unified_batch(UDAFOperation::MERGE, place_id, nullptr, serialized_state,
                                         &batch));
    ...
    // Validate response: [success: bool]
    if (output->num_columns() != 1 || output->num_rows() != 1) {
        return Status::InternalError("Invalid MERGE response from Python UDAF server");
    }

    std::shared_ptr<arrow::BooleanArray> bool_array;
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "MERGE", &bool_array));
...
Status PythonUDAFClient::finalize(int64_t place_id, std::shared_ptr<arrow::RecordBatch>* output) {
    ...
    // Create unified batch for FINALIZE operation (no metadata or data needed)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(
            create_unified_batch(UDAFOperation::FINALIZE, place_id, nullptr, nullptr, &batch));

    // Send and receive final result
    RETURN_IF_ERROR(_send_operation(batch.get(), output));
...
Status PythonUDAFClient::reset(int64_t place_id) {
    ...
    // Create unified batch for RESET operation (no metadata or data needed)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(create_unified_batch(UDAFOperation::RESET, place_id, nullptr, nullptr, &batch));
    ...
    // Validate response: [success: bool]
    if (output->num_columns() != 1 || output->num_rows() != 1) {
        return Status::InternalError("Invalid RESET response from Python UDAF server");
    }

    std::shared_ptr<arrow::BooleanArray> bool_array;
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "RESET", &bool_array));
...
Status PythonUDAFClient::destroy(int64_t place_id) {
    ...
    // Create unified batch for DESTROY operation (no metadata or data needed)
    std::shared_ptr<arrow::RecordBatch> batch;
    RETURN_IF_ERROR(
    ...
    std::shared_ptr<arrow::BooleanArray> bool_array;
    RETURN_IF_ERROR(validate_and_cast_column(output, 0, "DESTROY", &bool_array));

    if (!bool_array->Value(0)) {
        // Log warning but don't fail - cleanup should be best-effort
...
Status PythonUDAFClient::_send_operation(const arrow::RecordBatch* input,
    ...
    if (output != nullptr) {
        ...
        std::shared_ptr<arrow::BinaryArray> binary_array;
        RETURN_IF_ERROR(
                validate_and_cast_column(unified_response, 0, "UNIFIED_RESPONSE", &binary_array));
