...
class UDAFClassLoader:
    ...
    @staticmethod
    def load_from_module(python_udf_meta: PythonUDFMeta, loader: ModuleUDFLoader) -> type:
        """
...
class UDAFStateManager:
    ...
    def create_state(self, place_id: int) -> None:
        ...
        with self.lock:
            if place_id in self.states:
                ...
                logging.warning(
                    "State for place_id %s already exists, destroying old state before recreating",
                    place_id,
                )
            ...
            try:
                ...
                logging.info("Created UDAF state for place_id: %s", place_id)
            except Exception as e:
    ...
    def has_state(self, place_id: int) -> bool:
        """
        Check if a state exists for the given place_id.
        ...
        """
        with self.lock:
            return place_id in self.states
    ...
    def accumulate(self, place_id: int, *args) -> None:
        ...
        try:
            ...
            logging.debug("Accumulated values for place_id %s: %s", place_id, args)
        except Exception as e:
    ...
    def serialize(self, place_id: int) -> bytes:
        ...
        with self.lock:
            if place_id not in self.states:
                ...
                if self.udaf_class is None:
                    raise RuntimeError("UDAF class not set")
                try:
                    self.states[place_id] = self.udaf_class()
                except Exception as e:
    ...
    def reset(self, place_id: int) -> None:
        ...
        with self.lock:
            if place_id in self.states:
                ...
                try:
                    self.states[place_id] = self.udaf_class()
                    logging.debug("Reset state for place_id %s", place_id)
                except Exception as e:
                    logging.error(
    ...
    def destroy(self, place_id: int) -> None:
        ...
        with self.lock:
            if place_id in self.states:
                del self.states[place_id]
                logging.debug("Destroyed state for place_id %s", place_id)
            else:
                logging.warning("Attempted to destroy non-existent state: %s", place_id)
...
class FlightServer(flight.FlightServerBase):
    ...
    def _handle_udaf_accumulate(
        ...
        try:
            ...
            logging.info("ACCUMULATE: data_schema=%s", data_batch.schema)
            ...
            if is_single_place:
                ...
                for i in range(loop_start, loop_end):
                    ...
                    logging.debug("ACCUMULATE: Processing row %d with args=%s", i, args)
