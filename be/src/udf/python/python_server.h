...
private:
    ...
    // Global shared process pools (instead of thread-local)
    // All threads share the same pool per Python version for:
    // 1. Better load balancing (uniform distribution across all threads)
    // 2. Lower total process count (O(pool_size) vs O(threads × pool_size))
    // 3. Higher resource utilization (no idle processes in rarely-used threads)
