...
BuilderScannerMemtable::BuilderScannerMemtable(TabletSharedPtr tablet,
                                               const std::string &build_dir,
                                               const std::string &file_type)
    : _runtime_state(TQueryGlobals()), _tablet(tablet), _build_dir(build_dir),
  ...
  _runtime_state.init(_query_ctx->query_id, _options, TQueryGlobals(),
                      _exec_env);
...
void BuilderScannerMemtable::create_expr_info() {
  ...
  for (int i = 0; i < _tablet->num_columns(); i++) {
    ...
    // default_value_of_src_slot
    {
      ...
      _params.default_value_of_src_slot.emplace(i + _tablet->num_columns(),
                                                expr);
    }

    _params.dest_sid_to_src_sid_without_trans.emplace(
        i, _tablet->num_columns() + i);
...
void BuilderScannerMemtable::doSegmentBuild(
  ...
  DeltaWriter::open(&write_req, &delta_writer, _runtime_state.runtime_profile(),
                    load_id);
  ...
  std::filesystem::path segment_path(
      std::filesystem::path(_build_dir + "/segment"));
  ...
  RowsetMetaSharedPtr rowset_meta =
      delta_writer->get_cur_rowset()->rowset_meta();
