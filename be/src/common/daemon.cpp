...
void Daemon::memtable_mem_limit_mgr_tracker_refresh_thread() {
    ...
    while (!_stop_background_threads_latch.wait_for(
        ...
        if (ExecEnv::GetInstance()->initialized()) {
            doris::ExecEnv::GetInstance()->memtable_mem_limit_mgr()->refresh_mem_tracker();
        }
    }
}
...
void Daemon::start() {
    ...
    CHECK(st.ok()) << st;
    st = Thread::create(
            "Daemon", "memtable_mem_limit_mgr_tracker_refresh_thread",
            [this]() { this->memtable_mem_limit_mgr_tracker_refresh_thread(); },
            &_memtable_mem_limit_mgr_tracker_refresh_thread);
    CHECK(st.ok()) << st;
    st = Thread::create(
...
void Daemon::stop() {
    ...
    if (_memtable_mem_limit_mgr_tracker_refresh_thread) {
        _memtable_mem_limit_mgr_tracker_refresh_thread->join();
    }
