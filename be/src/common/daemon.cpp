...
void Daemon::memory_proc_refresh_thread() {
    int64_t interval_milliseconds = 1000;
    while (!_stop_background_threads_latch.wait_for(
        ...
        // If system available memory is not enough, or the process memory exceeds the limit,
        // call gc and adjust the refresh interval.
...
void Daemon::load_channel_tracker_refresh_thread() {
    int64_t interval_milliseconds = 100;
    while (!_stop_background_threads_latch.wait_for(std::chrono::seconds(interval_milliseconds))) {
