...
void Daemon::start() {
    ...
    CHECK(st.ok()) << st;
    st = Thread::create(
            "Daemon", "memtable_flush_mgr_tracker_refresh_thread",
            [this]() { this->memtable_flush_mgr_tracker_refresh_thread(); },
            &_memtable_flush_mgr_tracker_refresh_thread);
    CHECK(st.ok()) << st;
    st = Thread::create(
...
void Daemon::stop() {
    ...
    if (_memtable_flush_mgr_tracker_refresh_thread) {
        _memtable_flush_mgr_tracker_refresh_thread->join();
    }
