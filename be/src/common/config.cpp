...
DEFINE_mString(variant_duplicate_key_mode, "STRICT");
DEFINE_mBool(enable_vertical_compact_variant_subcolumns, "true");

DEFINE_Validator(variant_max_json_key_length,
                 [](const int config) -> bool { return config > 0 && config <= 65535; });
DEFINE_Validator(variant_duplicate_key_mode, [](const std::string& config) -> bool {
    return config == "STRICT" || config == "KEEP_FIRST" || config == "KEEP_LAST";
});
