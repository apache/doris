...
#define DORIS_CHECK(condition, ...)                                                       \
    do {                                                                                  \
        if (!(condition)) {                                                               \
            throw doris::Exception(doris::ErrorCode::FATAL_ERROR, "Check failed: {}, {}", \
                                   condition __VA_OPT__(, fmt::format(__VA_ARGS__)), ""); \
        }                                                                                 \
    } while (0)
...
T* DorisCheckNotNull(const char* expr, T* ptr) {
    if (ptr == nullptr) {
        throw doris::Exception(doris::ErrorCode::FATAL_ERROR, "Check failed: '{}' must be not null",
                               expr);
    }
    return ptr;
}
