...
bool is_tls_protocol_enabled(std::string_view protocol) {
    return config::enable_tls &&
           !comma_separated_protocols_contains(config::tls_excluded_protocols, protocol);
}

} // namespace

const char* get_internal_http_scheme() {
    return is_tls_protocol_enabled("http") ? "https://" : "http://";
}
