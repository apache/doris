...
private:
    ...
    std::shared_ptr<HybridSetBase> _try_convert_to_bitset_impl(RuntimeState* state) {
        ...
        if constexpr (T == TYPE_TINYINT || T == TYPE_SMALLINT || T == TYPE_INT ||
                      T == TYPE_BIGINT) {
            const size_t max_bitset_size = config::in_set_to_bitset_max_size;
            const size_t max_range = config::in_set_to_bitset_max_range;
            ...
            LOG_WARNING("yxc test _try_convert_to_bitset_impl")
                    .tag("size", this->size())
                    .tag("min_value", min_value)
                    .tag("max_value", max_value)
                    .tag("range", range)
                    .tag("bitset_size", set.size());
