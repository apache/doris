...
void CaseExpr::get_child_val(int child_idx, ExprContext* ctx, TupleRow* row, AnyVal* dst) {
    ...
    case TYPE_DECIMAL_V2:
        *reinterpret_cast<Decimal_V2Val*>(dst) = _children[child_idx]->get_decimal_v2_val(ctx, row);
        break;
    case TYPE_LARGEINT:
...
bool CaseExpr::any_val_eq(const TypeDescriptor& type, const AnyVal* v1, const AnyVal* v2) {
    ...
    case TYPE_DECIMAL_V2:
        return AnyValUtil::equals(type, *reinterpret_cast<const Decimal_V2Val*>(v1),
                                  *reinterpret_cast<const Decimal_V2Val*>(v2));
    case TYPE_LARGEINT:
...
CASE_COMPUTE_FN_WAPPER(Decimal_V2Val, decimal_v2_val)
