...
Status CaseExpr::get_codegend_compute_fn(RuntimeState* state, Function** fn) {
    ...
    BasicBlock* eval_first_when_expr_block =
            BasicBlock::Create(context, "eval_first_when_expr", function, default_value_block);
    ...
    if (has_case_expr()) {
        ...
        eval_case_expr_block =
                BasicBlock::Create(context, "eval_case_expr", function, eval_first_when_expr_block);
        builder.SetInsertPoint(eval_case_expr_block);
        case_val = CodegenAnyVal::create_call_wrapped(codegen, &builder, children()[0]->type(),
                                                      child_fns[0], args, "case_val");
        builder.CreateCondBr(case_val.get_is_null(), default_value_block,
                             eval_first_when_expr_block);
    } else {
    ...
    for (int i = (has_case_expr()) ? 1 : 0; i < loop_end; i += 2) {
        BasicBlock* check_when_expr_block =
                BasicBlock::Create(context, "check_when_expr_block", function, default_value_block);
        ...
        if (i == last_loop_iter) {
            ...
            continue_or_exit_block = BasicBlock::Create(context, "eval_next_when_expr", function,
                                                        default_value_block);
        }
        ...
        builder.CreateCondBr(when_val.get_is_null(), continue_or_exit_block, check_when_expr_block);
        ...
        if (has_case_expr()) {
            ...
            builder.CreateCondBr(when_val.get_val(), return_then_expr_block,
                                 continue_or_exit_block);
        }
        ...
        Value* then_val =
                CodegenAnyVal::create_call(codegen, &builder, child_fns[i + 1], args, "then_val");
    ...
    if (has_else_expr()) {
        Value* else_val = CodegenAnyVal::create_call(codegen, &builder, child_fns[num_children - 1],
                                                     args, "else_val");
...
#define CASE_COMPUTE_FN_WAPPER(TYPE, TYPE_NAME) CASE_COMPUTE_FN(TYPE, TYPE_NAME)
