...
Status Expr::create_tree_from_thrift(ObjectPool* pool, const vector<TExprNode>& nodes, Expr* parent,
...
llvm::Function* Expr::create_ir_function_prototype(LlvmCodeGen* codegen, const std::string& name,
                                                   llvm::Value* (*args)[2]) {
    ...
    prototype.add_argument(LlvmCodeGen::NamedVariable(
...
llvm::Function* Expr::get_static_get_val_wrapper(const TypeDescriptor& type, LlvmCodeGen* codegen) {
    switch (type.type) {
    case TYPE_BOOLEAN:
...
Value* Expr::get_ir_constant(LlvmCodeGen* codegen, ExprConstant c, int i) {
    ...
    case ARG_TYPE_SIZE:
        ...
        return ConstantInt::get(codegen->get_type(TYPE_INT), _children[i]->_type.get_byte_size());
    default:
        CHECK(false) << "NYI";
        return NULL;
    }
...
int Expr::inline_constants(LlvmCodeGen* codegen, Function* fn) {
    int replaced = 0;
    for (llvm::inst_iterator iter = llvm::inst_begin(fn), end = llvm::inst_end(fn); iter != end;) {
...
Status Expr::get_codegend_compute_fn_wrapper(RuntimeState* state, llvm::Function** fn) {
    ...
    llvm::Value* ret = CodegenAnyVal::create_call(codegen, &builder, static_getval_fn,
                                                  compute_fn_args, "ret", NULL);
...
Status Expr::create(const TExpr& texpr, const RowDescriptor& row_desc, RuntimeState* state,
                    ObjectPool* pool, Expr** scalar_expr, MemTracker* tracker) {
    *scalar_expr = nullptr;
    Expr* root;
...
Status Expr::create(const vector<TExpr>& texprs, const RowDescriptor& row_desc, RuntimeState* state,
                    ObjectPool* pool, vector<Expr*>* exprs, MemTracker* tracker) {
    exprs->clear();
...
Status Expr::create(const vector<TExpr>& texprs, const RowDescriptor& row_desc, RuntimeState* state,
                    vector<Expr*>* exprs, MemTracker* tracker) {
...
Status Expr::create_tree_internal(const vector<TExprNode>& nodes, ObjectPool* pool, Expr* root,
                                  int* child_node_idx) {
    // propagate error case
