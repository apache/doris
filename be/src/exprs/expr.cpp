...
Status Expr::create(const vector<TExpr>& texprs, const RowDescriptor& row_desc,
    RuntimeState* state, ObjectPool* pool, vector<Expr*>* exprs, std::shared_ptr<MemTracker> tracker) {
  exprs->clear();
...
Status Expr::create(const TExpr& texpr, const RowDescriptor& row_desc,
    RuntimeState* state, Expr** scalar_expr, std::shared_ptr<MemTracker> tracker) {
  return Expr::create(texpr, row_desc, state, state->obj_pool(), scalar_expr, tracker);
}

Status Expr::create(const vector<TExpr>& texprs, const RowDescriptor& row_desc,
    RuntimeState* state, vector<Expr*>* exprs, std::shared_ptr<MemTracker> tracker) {
