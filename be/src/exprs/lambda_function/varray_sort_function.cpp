...
public:
    ...
    Status execute(VExprContext* context, const Block* block, Selector* expr_selector, size_t count,
        ...
        for (int i = 0; i < 2; i++) {
            auto& lambda_column = lambda_block.get_by_position(i).column;
            auto mutable_lambda_column = IColumn::mutate(std::move(lambda_column));
            auto* temp_column = assert_cast<ColumnNullable*>(mutable_lambda_column.get());
            ...
            lambda_column = std::move(mutable_lambda_column);
        };
