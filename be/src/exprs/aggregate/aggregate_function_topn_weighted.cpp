...
AggregateFunctionPtr create_aggregate_function_topn_weighted(const std::string& name,
    ...
    if (has_default_param) {
        return topn_weighted_creator::create<AggregateFunctionTopNArray, ImplWeightWithDefault>(
                argument_types, result_is_nullable, attr);
    } else {
        return topn_weighted_creator::create<AggregateFunctionTopNArray, ImplWeight>(
                argument_types, result_is_nullable, attr);
    }
}
