...
AggregateFunctionPtr create_aggregate_function_window_funnel_v2(const std::string& name,
    ...
    } else if (argument_types[2]->get_primitive_type() == TYPE_DATETIMEV2_NANO) {
        return creator_without_type::create<AggregateFunctionWindowFunnelV2<TYPE_DATETIMEV2_NANO>>(
                argument_types, result_is_nullable, attr);
