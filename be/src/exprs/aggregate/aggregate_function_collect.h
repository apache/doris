...
struct AggregateFunctionCollectSetDataV2 {
    ...
    void read(BufferReadable& buf, Arena& arena) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            StringRef key;
            key.size = s.size;
            key.data = arena.insert(s.data, s.size);
            seen.insert(key);
            column_data->deserialize_and_insert_from_arena(key.data);
        }
        read_var_int(max_size, buf);
    }
...
class AggregateFunctionCollect final
    ...
    static constexpr bool ENABLE_ARENA =
            ...
            (IsV2 && !Data::is_collect_list && Data::PType == INVALID_TYPE);
    static constexpr bool DESERIALIZE_USE_ARENA =
            IsV2 && !Data::is_collect_list && Data::PType == INVALID_TYPE;
...
public:
    ...
    void deserialize(AggregateDataPtr __restrict place, BufferReadable& buf,
                     Arena& arena) const override {
        auto& data = this->data(place);
        if constexpr (DESERIALIZE_USE_ARENA) {
            data.read(buf, arena);
        } else {
            data.read(buf);
        }
