...
template <CastModeType CastMode, typename FromDataType, typename ToDataType>
    requires(CastUtil::IsPureDigitType<FromDataType> &&
             std::is_same_v<ToDataType, DataTypeTimeStampNs>)
class CastToImpl<CastMode, FromDataType, ToDataType> : public CastToBase {
public:
    Status execute_impl(FunctionContext* /*context*/, Block& block, const ColumnNumbers& arguments,
                        uint32_t result, size_t input_rows_count,
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
            ...
            if constexpr (IsDataTypeDecimal<FromDataType>) {
                parsed = CastToDatetimeV2::from_decimal(
                        col_from.get_intergral_part(i), col_from.get_fractional_part(i),
                        col_from.get_scale(), value, TimeStampNsValue::FRACTIONAL_DIGITS, params);
                ...
                parsed = CastToDatetimeV2::from_float(col_from.get_element(i), value,
                                                      TimeStampNsValue::FRACTIONAL_DIGITS, params);
            }
            if (!parsed && status.ok()) {
...
template <CastModeType CastMode, typename FromDataType, typename ToDataType>
    requires(IsDatelikeTypes<FromDataType> && std::is_same_v<ToDataType, DataTypeTimeStampNs>)
class CastToImpl<CastMode, FromDataType, ToDataType> : public CastToBase {
public:
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
                        uint32_t result, size_t input_rows_count,
