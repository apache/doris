...
struct CastToDecimal {
    ...
    static inline bool from_string_lossless(const StringRef& from, ToCppT& to, UInt32 precision,
        ...
        return is_lossless_parse_result(result);
    }

    static bool is_lossless_parse_result(StringParser::ParseResult result) {
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        if constexpr (Mode == CastModeType::NonStrictMode) {
            ...
            for (size_t i = 0; i < col_from->size(); ++i) {
                ...
                bool parsed = false;
                if (!source_is_null) {
                    bool lossless = CastToDecimal::is_lossless_decimal_string(
                            source.data, source.size, precision, scale);
                    if (lossless || Mode == CastModeType::StrictLosslessMode) {
                        StringParser::ParseResult parse_result = CastToDecimal::parse_string(
                                source, values_to[i], precision, scale);
                        parsed = lossless && CastToDecimal::is_lossless_parse_result(parse_result);
                        if constexpr (Mode == CastModeType::StrictLosslessMode) {
                            if (parse_result != StringParser::PARSE_SUCCESS && !parsed) {
