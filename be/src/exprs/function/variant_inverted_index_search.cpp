...
Status VariantNestedSearchEvaluator::evaluate(
    ...
    if (variant_reader == nullptr) {
        if (dynamic_cast<segment_v2::ConstantColumnReader*>(column_reader.get()) != nullptr) {
            // For example, a segment written before `ADD COLUMN v VARIANT` contains only NULL for
            // v, so `NESTED(v.items, ...)` has no matching nested documents in this segment.
            return Status::OK();
        }
