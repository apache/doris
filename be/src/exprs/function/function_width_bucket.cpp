...
private:
    ...
    void _execute_timestamp_ns(const IColumn& expr_column, const IColumn& min_value_column,
        ...
        for (size_t i = 0; i < expr_column.size(); ++i) {
            ...
            if (expr < min_value) {
                ...
                const __int128 range = static_cast<__int128>(max_value) - min_value;
                if (range / num_buckets == 0) {
                    continue;
                }
                const __int128 offset = static_cast<__int128>(expr) - min_value;
                nested_data[i] = 1 + static_cast<int64_t>(offset * num_buckets / range);
            }
        }
    }
