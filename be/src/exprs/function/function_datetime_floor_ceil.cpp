...
#undef EXPANDER
    ...
    static bool time_round_two_args(const DateValueType& ts_arg, const Int32 period,
        ...
        if constexpr (std::is_same_v<DateValueType, VecDateTimeValue>) {
        ...
        } else if constexpr (std::is_same_v<DateValueType, DateTimeV2NanoValue>) {
            const auto nanos_since_midnight = [](const DateTimeV2NanoValue& value) {
                return value.time_part_to_seconds() * DateTimeV2NanoValue::NANOS_PER_SECOND +
                       value.nanosecond();
            };
            const auto nanos_since_date = [&](const DateTimeV2NanoValue& value, uint8_t month,
                                              uint8_t day) {
                return (value.daynr() - calc_daynr(value.year(), month, day)) * HOUR_PER_DAY *
                               SECOND_PER_HOUR * DateTimeV2NanoValue::NANOS_PER_SECOND +
            ...
            if constexpr (Flag::Unit == MINUTE) {
                diff = (ts_arg.daynr() - ts_origin.daynr()) * HOUR_PER_DAY * SECOND_PER_MINUTE +
                       ...
                       ts_origin.minute();
                trivial_part_ts_arg = ts_arg.second() * DateTimeV2NanoValue::NANOS_PER_SECOND +
                                      ts_arg.nanosecond();
                trivial_part_ts_res = ts_origin.second() * DateTimeV2NanoValue::NANOS_PER_SECOND +
                                      ts_origin.nanosecond();
            }
            if constexpr (Flag::Unit == SECOND) {
                diff = ts_arg.datetime_diff_in_seconds(ts_origin);
...
#define TIME_ROUND_WITH_DELTA_TYPE(IMPL, NAME, UNIT, TYPE, DELTA)                                \
    ...
    using FunctionDateTimeV2NanoOneArg##IMPL##DELTA =                                            \
            FunctionDateTimeFloorCeil<IMPL, TYPE_DATETIMEV2_NANO, 1>;                            \
    using FunctionDateTimeV2NanoTwoArg##IMPL##DELTA =                                            \
            FunctionDateTimeFloorCeil<IMPL, TYPE_DATETIMEV2_NANO, 2>;                            \
    using FunctionDateTimeV2NanoThreeArg##IMPL##DELTA =                                          \
            FunctionDateTimeFloorCeil<IMPL, TYPE_DATETIMEV2_NANO, 3>;                            \
...
#define TIME_ROUND_DECLARE(IMPL, NAME, UNIT, TYPE)                                            \
    ...
    using FunctionDateTimeV2NanoTwoArg##IMPL =                                                \
            FunctionDateTimeFloorCeil<IMPL, TYPE_DATETIMEV2_NANO, 2, true>;                   \
...
#define REGISTER_FUNC_WITH_DELTA_TYPE(IMPL, DELTA)                            \
    ...
    factory.register_function<FunctionDateTimeV2NanoOneArg##IMPL##DELTA>();   \
    factory.register_function<FunctionDateTimeV2NanoTwoArg##IMPL##DELTA>();   \
    factory.register_function<FunctionDateTimeV2NanoThreeArg##IMPL##DELTA>(); \
    ...
    factory.register_function<FunctionDateTimeV2NanoTwoArg##IMPL>();          \
