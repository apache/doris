...
public:
    ...
    ColumnNumbers get_arguments_that_are_always_constant() const override { return {2}; }
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        if (arguments.size() != 2 && arguments.size() != 3) {
            return Status::InvalidArgument("{} requires 2 or 3 arguments", get_name());
        }
        const auto& ttl_entry = block.get_by_position(arguments[0]);
        ColumnPtr ttl_owner = ttl_entry.column->convert_to_full_column_if_const();
        const NullMap* null_map = VectorizedUtils::get_null_map(ttl_owner);
        ColumnPtr ttl_column = remove_nullable(ttl_owner);
        ...
        if (source_time != (arguments.size() == 3)) {
            return Status::InvalidArgument(
                    "{} requires a time zone offset only for a temporal TTL column", get_name());
        }
        ...
        if (source_time) {
            const auto* offset = check_and_get_column_constData<ColumnInt32>(
                    block.get_by_position(arguments[2]).column.get());
            if (offset == nullptr) {
                return Status::InvalidArgument("{} time zone offset must be a constant INT",
                                               get_name());
            }
            const int32_t offset_seconds = offset->get_data()[0];
            if (!is_valid_row_ttl_time_zone_offset_seconds(offset_seconds)) {
                return Status::InvalidArgument(
                        "{} time zone offset {} must be a whole minute in [{}, {}]", get_name(),
                        offset_seconds, MIN_ROW_TTL_TIME_ZONE_OFFSET_SECONDS,
                        MAX_ROW_TTL_TIME_ZONE_OFFSET_SECONDS);
            }
            if (!requires_time_zone && offset_seconds != 0) {
                return Status::InvalidArgument("{} TIMESTAMPTZ time zone offset must be 0",
                                               get_name());
            }
        ...
        for (size_t row = 0; row < input_rows_count; ++row) {
            ...
            if (source_time) {
                RETURN_IF_ERROR(calculate_row_ttl_expiration_us(
                        *ttl_column, ttl_type, row, ttl_time_zone, duration[row], &expiration_us));
            } else {
...
// The framework propagates NULLs and materializes constant inputs. Reuse the same temporal
// conversion as source-column TTL, with no duration added to an absolute expiration.
class FunctionRowTtlExpiration final : public IFunction {
public:
    static constexpr auto name = "row_ttl_expiration";

    static FunctionPtr create() { return std::make_shared<FunctionRowTtlExpiration>(); }
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
                        uint32_t result, size_t input_rows_count) const override {
        const auto& input = block.get_by_position(arguments[0]);
        const FieldType type =
                primitive_type_to_storage_field_type(input.type->get_primitive_type());
        if (type == FieldType::OLAP_FIELD_TYPE_BIGINT) {
            block.replace_by_position(result, input.column);
            return Status::OK();
        }
        auto expiration = ColumnInt64::create(input_rows_count);
        auto& data = expiration->get_data();
        for (size_t row = 0; row < input_rows_count; ++row) {
            RETURN_IF_ERROR(calculate_row_ttl_expiration_us(
                    *input.column, type, row, context->state()->timezone_obj(), 0, &data[row]));
        }
        block.replace_by_position(result, std::move(expiration));
        return Status::OK();
    }
};
...
void register_function_row_ttl(SimpleFunctionFactory& factory) {
    ...
    factory.register_function<FunctionRowTtlExpiration>();
}
