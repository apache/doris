...
Status JdbcConnector::get_next(bool* eos, Block* block, int batch_size) {
    ...
    jboolean has_next = JNI_FALSE;
    {
        ...
        //        _executor_obj.call_nonvirtual_void_method()
...
//    RETURN_IF_ERROR(Jni::find_class(env, "org/apache/doris/thrift/TOdbcTableType",  &enum_class));
//
//    Jni::MethodId find_by_value_method;
//    RETURN_IF_ERROR(enum_class.get_static_method(env, "findByValue", "(I)Lorg/apache/doris/thrift/TOdbcTableType;", &find_by_value_method));
//////    jclass enum_class = env->FindClass();
////    jmethodID find_by_value_method = env->GetStaticMethodID(
////            enum_class,
//
//    Jni::LocalObject java_enum_local_obj;
//    RETURN_IF_ERROR(enum_class.call_static_method(env, find_by_value_method,&java_enum_local_obj,static_cast<jint>(table_type)));
//
//    RETURN_ERROR_IF_EXC(env);
