...
/*static*/ HiveOrcNestedColumnUtils::SchemaAndColumnResult
HiveOrcNestedColumnUtils::_extract_schema_and_columns_efficiently(
...
HiveOrcNestedColumnUtils::SchemaAndColumnResult
HiveOrcNestedColumnUtils::_extract_schema_and_columns_efficiently_by_top_level_col_index(
        const orc::Type* orc_type,
...
void HiveOrcNestedColumnUtils::_extract_nested_column_ids_efficiently(
    ...
    for (const auto& path : paths) {
        if (!path.empty()) {
            std::string first_table_name = path[0];
    ...
    for (uint64_t i = 0; i < type.getSubtypeCount(); ++i) {
        ...
        const auto exact_it = child_paths_by_table_name.find(to_lower(child_field_name));
        // const auto star_it = child_paths_by_table_name.find("*");

        if (exact_it != child_paths_by_table_name.end()) {
            // 合并两类来源的路径（若均存在）
            ...
            // if (star_it != child_paths_by_table_name.end()) {
...
/*static*/ HiveOrcNestedColumnUtils::SchemaAndColumnResult
HiveOrcNestedColumnUtils::_extract_schema_and_columns_efficiently_by_index(
        const orc::Type* orc_type,
