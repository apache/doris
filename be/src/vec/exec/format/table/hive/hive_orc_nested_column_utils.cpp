    ...
    for (uint64_t i = 0; i < type.getSubtypeCount(); ++i) {
        ...
        case orc::TypeKind::MAP:
            ...
            if (i == 0 && only_access_keys) {
                ...
                std::cout << "[HiveOrcNestedColumnUtils] MAP KEYS optimization: Processing key "
                             "field (column_id: "
                          << child->getColumnId() << ")" << std::endl;
            } else if (i == 1 && only_access_values) {
                child_field_name = "VALUES";
                std::cout << "[HiveOrcNestedColumnUtils] MAP VALUES optimization: Processing value "
                             "field (column_id: "
                          << child->getColumnId() << ")" << std::endl;
            }
            ...
            // // to correctly determine MAP boundaries and null information in ORC format
            ...
            //     uint64_t key_max_column_id = child->getMaximumColumnId();
            //     std::cout << "[HiveOrcNestedColumnUtils] MAP VALUES optimization: Adding key column_ids "
            //              << key_start_id << " to " << key_max_column_id
            //              << " for structure information" << std::endl;
            //     for (uint64_t id = key_start_id; id <= key_max_column_id; ++id) {
            //         column_ids.insert(id);
            //     }
            //     has_child_columns = true;
            //     // Still set child_field_name for further processing if needed
            //     child_field_name = "";  // Skip normal processing since we handled it above
            // }
