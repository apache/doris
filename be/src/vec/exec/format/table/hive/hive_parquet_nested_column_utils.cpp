    ...
    for (uint64_t i = 0; i < field_schema.children.size(); ++i) {
        ...
        if (is_list) {
            ...
            if (i == 0 && only_access_keys) {
                ...
                std::cout << "[HiveParquetNestedColumnUtils] MAP KEYS optimization: Processing key "
                             "field "
                          << child.name << " (column_id: " << child.get_column_id() << ")"
                          << std::endl;
            } else if (i == 1 && only_access_values) {
                child_field_name = "VALUES";
                std::cout << "[HiveParquetNestedColumnUtils] MAP VALUES optimization: Processing "
                             "value field "
                          << child.name << " (column_id: " << child.get_column_id() << ")"
                          << std::endl;
            }
            ...
            // to correctly determine MAP boundaries and null information in Parquet/ORC format
            if (i == 0 && only_access_values) {
                ...
                std::cout << "[HiveParquetNestedColumnUtils] MAP VALUES optimization: Adding key "
                             "column_ids "
