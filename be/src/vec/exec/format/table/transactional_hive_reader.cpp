...
Status TransactionalHiveReader::init_reader(
    ...
    Status status =
            orc_reader->init_reader(&_col_names, colname_to_value_range, vconjunct_ctx, true);
    return status;
}
...
Status TransactionalHiveReader::init_row_filters(const TFileRangeDesc& range) {
    ...
    for (auto& delete_delta : range.table_format_params.transactional_hive_params.delete_deltas) {
        ...
        OrcReader delete_reader(_profile, _state, _params, delete_range,
                                ...
                                _MIN_BATCH_SIZE, _state->timezone(), _io_ctx, false);

        RETURN_IF_ERROR(delete_reader.init_reader(
                &TransactionalHive::DELETE_ROW_COLUMN_NAMES_LOWER_CASE, nullptr, nullptr, false));
