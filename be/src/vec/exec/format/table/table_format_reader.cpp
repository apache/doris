...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_table_field_id(
    ...
    case TPrimitiveType::MAP: {
        ...
        RETURN_IF_ERROR(by_table_field_id(*table_schema.nestedField.map_field.key_field,
                                          *file_schema.nestedField.map_field.key_field, key_node));

        std::shared_ptr<TableSchemaChangeHelper::Node> value_node = nullptr;
        RETURN_IF_ERROR(by_table_field_id(*table_schema.nestedField.map_field.value_field,
                                          *file_schema.nestedField.map_field.value_field,
                                          value_node));
    ...
    case TPrimitiveType::ARRAY: {
        ...
        RETURN_IF_ERROR(by_table_field_id(*table_schema.nestedField.array_field.item_field,
                                          *file_schema.nestedField.array_field.item_field,
                                          item_node));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_table_field_id(
    ...
    for (size_t idx = 0; idx < file_schema.fields.size(); ++idx) {
        file_field_id_to_idx.emplace(file_schema.fields[idx].id, idx);
    }
    auto struct_node = std::make_shared<TableSchemaChangeHelper::StructNode>();

    for (const auto& table_field : table_schema.fields) {
        const auto& table_column_name = table_field.name;

        if (file_field_id_to_idx.contains(table_field.id)) {
            const auto& file_field = file_schema.fields.at(file_field_id_to_idx[table_field.id]);

            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_table_field_id(table_field, file_field, field_node));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_field_id(
    ...
    for (const auto& table_field : table_schema.fields) {
        ...
        if (file_column_id_idx_map.contains(table_field.id)) {
            auto file_column_idx = file_column_id_idx_map[table_field.id];
            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_parquet_field_id(table_field, parquet_fields_schema[file_column_idx],
                                                field_node, exist_field_id));
            struct_node->add_children(table_column_name,
                                      parquet_fields_schema[file_column_idx].name, field_node);
        } else {
            struct_node->add_not_exist_children(table_column_name);
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_parquet_field_id(
    ...
    case TPrimitiveType::MAP: {
        ...
        RETURN_IF_ERROR(by_parquet_field_id(*table_schema.nestedField.map_field.key_field,
                                            parquet_field.children[0].children[0], key_node,
                                            exist_field_id));

        RETURN_IF_ERROR(by_parquet_field_id(*table_schema.nestedField.map_field.value_field,
                                            parquet_field.children[0].children[1], value_node,
                                            exist_field_id));
        ...
        for (const auto& table_field : table_schema.nestedField.struct_field.fields) {
            ...
            if (file_column_id_idx_map.contains(table_field.id)) {
                const auto& file_field =
                        parquet_field.children.at(file_column_id_idx_map[table_field.id]);
                std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
                RETURN_IF_ERROR(
                        by_parquet_field_id(table_field, file_field, field_node, exist_field_id));
                struct_node->add_children(table_column_name, file_field.name, field_node);
            } else {
                struct_node->add_not_exist_children(table_column_name);
            }
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_field_id(
    ...
    for (const auto& table_field : table_schema.fields) {
        const auto& table_column_name = table_field.name;
        if (file_column_id_idx_map.contains(table_field.id)) {
            auto file_field_idx = file_column_id_idx_map[table_field.id];
            const auto& file_field = orc_root->getSubtype(file_field_idx);
            std::shared_ptr<TableSchemaChangeHelper::Node> field_node = nullptr;
            RETURN_IF_ERROR(by_orc_field_id(table_field, file_field, field_id_attribute_key,
                                            field_node, exist_field_id));
...
Status TableSchemaChangeHelper::BuildTableInfoUtil::by_orc_field_id(
    ...
    case TPrimitiveType::MAP: {
        ...
        RETURN_IF_ERROR(by_orc_field_id(*table_schema.nestedField.map_field.key_field,
                                        orc_root->getSubtype(0), field_id_attribute_key, key_node,
                                        exist_field_id));

        RETURN_IF_ERROR(by_orc_field_id(*table_schema.nestedField.map_field.value_field,
