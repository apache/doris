...
static Status convert_data_type_from_parquet(tparquet::Type::type parquet_type,
    ...
    Status convert(const IColumn* src_col, IColumn* dst_col) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            auto& value = reinterpret_cast<DateV2Value<DateV2ValueType>&>(
                    reinterpret_cast<ColumnDateV2*>(dst_col)->get_data()[i]);
            //            value = reinterpret_cast<const ColumnVector<uint32>*>(src_col)->get_data()[i];
            int64_t date_value =
                    reinterpret_cast<const ColumnVector<uint32>*>(src_col)->get_data()[i] +
                    doc->_decode_params->offset_days;
...
static Status get_converter_impl(std::shared_ptr<const IDataType> src_data_type,
...
static Status get_converter(std::shared_ptr<const IDataType> src_type,
