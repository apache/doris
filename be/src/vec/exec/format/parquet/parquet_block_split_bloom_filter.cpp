...
Status ParquetBlockSplitBloomFilter::init(const char* buf, size_t size,
    ...
    // bool input_has_null_byte = true;
    // size_t payload_bytes = size - 1;
    // if ((payload_bytes & (payload_bytes - 1)) != 0) {
    //     if ((size & (size - 1)) == 0) {
    //         input_has_null_byte = false;
    //         payload_bytes = size;
    //     } else {
    //         return Status::InvalidArgument("size - 1 must be power of two");
    //     }
    // }
    // _size = payload_bytes + 1;
    // _num_bytes = payload_bytes;
    // _data = new char[_size];
    // memcpy(_data, buf, input_has_null_byte ? _size : size);
    // if (!input_has_null_byte) {
    //     // Backfill the extra byte when upstream does not reserve space for null tracking.
    //     _data[_num_bytes] = 0;
    // }
