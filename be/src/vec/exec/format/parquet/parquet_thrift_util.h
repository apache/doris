...
//        uint8_t* buff_ptr = buff;
        ...
        LOG(WARNING) << "footer_read_size: " << footer_read_size;
//          _TIMER(stats->footer_read_time);
        int64_t bytes_read = 0;
        RETURN_IF_ERROR(file->readat(file_size - footer_read_size, footer_read_size, &bytes_read, buff));
        ...
        if (memcmp(magic_ptr, PARQUET_VERSION_NUMBER, sizeof(PARQUET_VERSION_NUMBER)) != 0) {
            return Status::Corruption("magic value error");
        }
        ...
        if (metadata_size > file_size - PARQUET_FOOTER_SIZE) {
            Status::Corruption(
                    ...
                    " bytes, smaller than the size reported by footer's (", metadata_size, "bytes)");
        }
        LOG(WARNING) << "metadata_size: " << metadata_size;
        LOG(WARNING) << "=====================================";
        tparquet::FileMetaData t_metadata;
        // deserialize footer
        RETURN_IF_ERROR(deserialize_thrift_msg(footer_buff - metadata_size, &metadata_size, true, &t_metadata));
        file_metadata.reset(new FileMetaData());
        file_metadata->init(t_metadata);
        return Status::OK();
    }
