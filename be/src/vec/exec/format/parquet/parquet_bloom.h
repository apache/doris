...
// Ref: https://github.com/apache/parquet-cpp/blob/642da055adf009652689b20e68a198cffb857651/src/parquet/bloom_filter.h#L35
class BloomFilter {
public:
    ...
    static constexpr uint32_t K_MAX_BLOOM_FILTER_BYTES = 128 * 1024 * 1024;
    ...
    virtual void insert_hashes(const uint64_t* hashes, int num_values) = 0;

    /// Get the number of bytes of bitset
    virtual uint32_t get_bitset_size() const = 0;
    ...
    /// @param hashes a pointer to the output hash values, its length should be equal to
    /// num_values.
    virtual void hashes(const parquet::Int96* values, int num_values, uint64_t* hashes) const = 0;
    ...
    /// @param hashes a pointer to the output hash values, its length should be equal to
    /// num_values.
    virtual void hashes(const parquet::FLBA* values, uint32_t type_len, int num_values,
                        uint64_t* hashes) const = 0;
...
constexpr bool is_multiple_of8(int64_t n) {
    return (n & 7) == 0;
}
...
public:
    ...
    static constexpr uint32_t K_MIN_BLOOM_FILTER_BYTES = 32;
    ...
    static uint32_t optimal_num_of_bytes(uint32_t ndv, double fpp) {
        uint32_t optimal_bits_num = optimal_num_of_bits(ndv, fpp);
        DCHECK(is_multiple_of8(optimal_bits_num));
        return optimal_bits_num >> 3;
    }
    ...
    static uint32_t optimal_num_of_bits(uint32_t ndv, double fpp) {
        DCHECK(fpp > 0.0 && fpp < 1.0);
        const double m = -8.0 * ndv / log(1 - pow(fpp, 1.0 / 8));
        uint32_t num_bits;

        // Handle overflow.
        if (m < 0 || m > K_MAX_BLOOM_FILTER_BYTES << 3) {
            num_bits = static_cast<uint32_t>(K_MAX_BLOOM_FILTER_BYTES << 3);
        } else {
            num_bits = static_cast<uint32_t>(m);
        }

        // Round up to lower bound
        if (num_bits < K_MIN_BLOOM_FILTER_BYTES << 3) {
            num_bits = K_MIN_BLOOM_FILTER_BYTES << 3;
        }

        // Get next power of 2 if bits is not power of 2.
        if ((num_bits & (num_bits - 1)) != 0) {
            num_bits = static_cast<uint32_t>(next_power2(num_bits));
        }

        // Round down to upper bound
        if (num_bits > K_MAX_BLOOM_FILTER_BYTES << 3) {
            num_bits = K_MAX_BLOOM_FILTER_BYTES << 3;
        }

        return num_bits;
    }

    bool find_hash(uint64_t hash) const override;
    void insert_hash(uint64_t hash) override;
    void insert_hashes(const uint64_t* hashes, int num_values) override;
    uint32_t get_bitset_size() const override { return num_bytes_; }
    ...
    uint64_t hash(const parquet::Int96* value) const override { return hasher_->Hash(value); }
    uint64_t hash(const parquet::ByteArray* value) const override { return hasher_->Hash(value); }
    ...
    void hashes(const parquet::Int96* values, int num_values, uint64_t* hashes) const override {
        hasher_->Hashes(values, num_values, hashes);
    }
    void hashes(const parquet::ByteArray* values, int num_values, uint64_t* hashes) const override {
        hasher_->Hashes(values, num_values, hashes);
    }
    ...
    static BlockSplitBloomFilter deserialize(io::FileReaderSPtr file_reader_s, int64_t bloom_offset,
                                             const io::IOContext* io_context);

private:
    inline void insert_hash_impl(uint64_t hash) const;

    // Bytes in a tiny Bloom filter block.
    static constexpr int K_BYTES_PER_FILTER_BLOCK = 32;

    // The number of bits to be set in each tiny Bloom filter
    static constexpr int K_BITS_SET_PER_BLOCK = 8;

    // A mask structure used to set bits in each tiny Bloom filter.
    struct BlockMask {
        uint32_t item[K_BITS_SET_PER_BLOCK];
    };
    ...
    static constexpr uint32_t SALT[K_BITS_SET_PER_BLOCK] = {0x47b6137bU, 0x44974d91U, 0x8824ad5bU,
    ...
    std::unique_ptr<parquet::Hasher> hasher_;
};
