...
bool ColumnTypeConverter::is_numeric_type(PrimitiveType type) {
    switch (type) {
    case TYPE_BOOLEAN:
    case TYPE_TINYINT:
    case TYPE_SMALLINT:
...
bool ColumnTypeConverter::is_decimal_type(doris::PrimitiveType type) {
    switch (type) {
    case TYPE_DECIMALV2:
    case TYPE_DECIMAL32:
    case TYPE_DECIMAL64:
...
ColumnPtr ColumnTypeConverter::get_column(const TypeDescriptor& src_type, ColumnPtr& dst_column,
    ...
    if (_cached_src_column == nullptr) {
        _cached_src_type = DataTypeFactory::instance().create_data_type(src_type, false);
...
std::unique_ptr<ColumnTypeConverter> ColumnTypeConverter::get_converter(
    ...
    // example: float -> int
    if (is_numeric_type(src_primitive_type) && is_numeric_type(dst_primitive_type)) {
...
#define DISPATCH(SRC_PTYPE)                                                                 \
    case SRC_PTYPE: {                                                                       \
        ...
        default:                                                                            \
            return std::make_unique<UnsupportedConverter>();                                \
        }                                                                                   \
    }
...
#undef DISPATCH
    ...
    if (is_string_type(dst_primitive_type)) {
        ...
        if (is_numeric_type(src_primitive_type)) {
            switch (src_primitive_type) {
#define DISPATCH(SRC_PTYPE) \
    case SRC_PTYPE:         \
...
#undef DISPATCH
        ...
        } else if (is_decimal_type(src_primitive_type)) { // decimal type to string
            switch (src_primitive_type) {
#define DISPATCH(SRC_PTYPE) \
    case SRC_PTYPE:         \
...
#undef DISPATCH
    ...
    if (src_primitive_type == TYPE_DATEV2 && dst_primitive_type == TYPE_DATETIMEV2) {
        return std::make_unique<V2DateConverter<TYPE_DATEV2, TYPE_DATETIMEV2>>();
    }
    if (src_primitive_type == TYPE_DATETIMEV2 && dst_primitive_type == TYPE_DATEV2) {
        return std::make_unique<V2DateConverter<TYPE_DATETIMEV2, TYPE_DATEV2>>();
    }

    // numeric to decimal
    if (is_numeric_type(src_primitive_type) && is_decimal_type(dst_primitive_type)) {
...
#undef DISPATCH
    ...
    // decimal to numeric
    if (is_decimal_type(src_primitive_type) && is_numeric_type(dst_primitive_type)) {
