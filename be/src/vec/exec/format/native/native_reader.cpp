...
Status NativeReader::get_next_block(Block* block, size_t* read_rows, bool* eof) {
    ...
    // deserialize PBlock into Block. Older Block API only exposes a single-argument
...
Status NativeReader::get_columns(std::unordered_map<std::string, TypeDescriptor>* name_to_type,
