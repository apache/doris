                    ...
                    if constexpr (std::is_same_v<typename HashTableCtxType::Mapped,
                        ...
                        while (iter != iter_end) {
                            ...
                            if constexpr (keep_matched) { //intersected
                                ...
                                if (!mapped.visited && is_need_shrink) {
                                    tmp_hash_table.hash_table.insert(iter->get_value());
                                }
                                ++iter;
                            }
...
struct HashTableProbe {
    ...
    Status mark_data_in_hashtable(HashTableContext& hash_table_ctx) {
        ...
        if constexpr (std::is_same_v<typename HashTableContext::Mapped, RowRefListWithFlag>) {
    ...
    void add_result_columns(RowRefListWithFlag& value, int& block_size) {
        auto it = value.begin();
        ...
        if constexpr (std::is_same_v<typename HashTableContext::Mapped, RowRefListWithFlag>) {
            for (; iter != hash_table_ctx.hash_table.end() && block_size < _batch_size; ++iter) {
                ...
                if constexpr (is_intersected) {
                    if (value.visited) { //intersected: have done probe, so visited values it's the result
                        add_result_columns(value, block_size);
                    }
                } else {
                    if (!value.visited) { //except: haven't visited values it's the needed result
                        add_result_columns(value, block_size);
                    }
                }
            }
