...
static std::string get_es_host_port(const std::vector<doris::TNetworkAddress>& es_hosts) {
...
Status NewEsScanNode::_init_scanners(std::list<VScannerSPtr>* scanners) {
    ...
    for (auto& es_scan_range : _scan_ranges) {
        ...
        properties[ESScanReader::KEY_HOST_PORT] = get_es_host_port(es_scan_range->es_hosts);
