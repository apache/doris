    ...
    if (is_duplicate_key) {
        auto build_new_scanner =
                [&](const TPaloScanRange& scan_range, const std::vector<OlapScanRange*>& key_ranges,
                    ...
                    std::shared_ptr<NewOlapScanner> scanner = NewOlapScanner::create_shared(
                            ...
                            scan_range, key_ranges, rs_readers_with_segments,
                            _scanner_profile.get(), scanner_idx++);
        ...
        for (int i = 0; i < _scan_ranges.size(); ++i) {
            ...
            while (rowset_idx < rs_seg_count.size()) {
                ...
                rs_readers_with_segments.emplace({});
                rs_readers_with_segments.back().rs_reader =
                        rowset_readers_vector[i][rowset_idx]->clone();
                ...
                if (num_segments_assigned + max_add_seg_nums > avg_segment_count_by_scanner) {
                    ...
                    rs_readers_with_segments.back().segment_offsets = {
                            segment_idx_to_scan,
...
#ifndef NDEBUG
            for (const auto& rs_reader_with_segments : rs_readers_with_segments) {
            ...
            if (!rs_readers_with_segments.empty()) {
                build_new_scanner(*scan_range, scanner_ranges, rs_readers_with_segments);
            }
        }
    } else {
