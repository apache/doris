...
Status FileScanner::_process_runtime_filters_partition_prune(bool& can_filter_all) {
    ...
    for (auto const& partition_col_desc : _partition_col_descs) {
        ...
        Slice slice(value_to_deserialize.data(), value_to_deserialize.size());
        uint64_t num_deserialized = 0;
        DataTypeSerDe::FormatOptions options {};
        if (_partition_value_is_null.contains(source_column_name)) {
            ...
            if (_partition_value_is_null[source_column_name]) {
