    ...
    while (true) {
        ...
        case TFileFormatType::FORMAT_JNI: {
            if (range.__isset.table_format_params &&
                ...
                if (_state->query_options().enable_paimon_cpp_reader) {
                    _cur_reader = PaimonCppReader::create_unique(_file_slot_descs, _state, _profile,
                                                                 range, _params);
                    init_status = ((PaimonCppReader*)(_cur_reader.get()))->init_reader();
                } else {
