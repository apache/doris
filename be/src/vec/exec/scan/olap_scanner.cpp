...
Status OlapScanner::init() {
    ...
    for (auto pair : local_state->_slot_id_to_virtual_column_expr) {
        ...
        LOG_INFO("OlapScanner copy virtual column, slot id: {}, expr: {}", pair.first,
                 pair.second->root()->debug_string());
    }
...
Status OlapScanner::_init_tablet_reader_params(
    ...
    _tablet_reader_params.ann_topn_predicate = _ann_topn_descriptor;
    ...
    if (_tablet_reader_params.direct_mode) {
        ...
        LOG_INFO("Aggregation mode, schema num_key_columns {}", tablet_schema->num_key_columns());
        // 这段代码为什么没有导致 block_idx 出错
    ...
    LOG_INFO("Direct mode {}", _tablet_reader_params.direct_mode);
...
Status OlapScanner::_init_return_columns() {
    ...
    if (_return_columns.empty()) {
        ...
        LOG_INFO("OlapScanner init return columns, return columns: {}", msg);
    }
