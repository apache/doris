...
class NewDorisAdbcScanNode;

class NewDorisAdbcScanner : public VScanner {
    ENABLE_FACTORY_CREATOR(NewDorisAdbcScanner);

public:
    friend class JdbcConnector;

    NewDorisAdbcScanner(RuntimeState* state, NewDorisAdbcScanNode* parent, int64_t limit, TupleId tuple_id,
                 const std::string& location_uri, const std::string& ticket,
