...
Status VOlapScanNode::change_fixed_value_range(ColumnValueRange<primitive_type>& temp_range,
    ...
    case TYPE_DATETIME: {
        ...
        reinterpret_cast<VecDateTimeValue*>(value)->convert_vec_dt_to_dt(&date_value);
        func(temp_range,
             reinterpret_cast<typename PrimitiveTypeTraits<primitive_type>::CppType*>(&date_value));
        break;
    }
    case TYPE_DECIMALV2:
    case TYPE_CHAR:
...
bool VOlapScanNode::_should_push_down_function(
        VectorizedFnCall* fn_call, const std::function<bool(const std::string&)>& fn_checker) {
...
Status VOlapScanNode::_normalize_in_and_eq_predicate(VExpr* expr, VExprContext* expr_ctx,
    ...
    if (TExprNodeType::IN_PRED == expr->node_type()) {
        ...
        while (iter->has_next()) {
            ...
            RETURN_IF_ERROR(change_fixed_value_range(temp_range, value,
        ...
        if (_should_push_down_binary_predicate(reinterpret_cast<VectorizedFnCall*>(expr), expr_ctx,
            ...
            if (value.data != nullptr) {
                if constexpr (T == TYPE_CHAR || T == TYPE_VARCHAR || T == TYPE_STRING ||
                    ...
                    RETURN_IF_ERROR(
                            change_fixed_value_range(temp_range, reinterpret_cast<void*>(&val),
                                                     ColumnValueRange<T>::add_fixed_value_range));
                } else {
...
Status VOlapScanNode::_normalize_is_null_predicate(VExpr* expr, VExprContext* expr_ctx,
    ...
    if (TExprNodeType::FUNCTION_CALL == expr->node_type()) {
        if (_should_push_down_function(reinterpret_cast<VectorizedFnCall*>(expr),
                                       is_null_checker)) {
            ...
            range.intersection(temp_range);
            *push_down = true;
        } else if (_should_push_down_function(reinterpret_cast<VectorizedFnCall*>(expr),
                                              is_not_null_checker)) {
            auto temp_range = ColumnValueRange<T>::create_empty_column_value_range(
                    slot->type().precision, slot->type().scale);
...
Status VOlapScanNode::_normalize_noneq_binary_predicate(VExpr* expr, VExprContext* expr_ctx,
    ...
    if (TExprNodeType::BINARY_PRED == expr->node_type()) {
        ...
        if (_should_push_down_binary_predicate(reinterpret_cast<VectorizedFnCall*>(expr), expr_ctx,
            ...
            DCHECK(slot_ref_child >= 0);
            auto fn_name = reinterpret_cast<VectorizedFnCall*>(expr)->fn().name.function_name;
            ...
            if (value.data != nullptr) {
                *push_down = true;
                ...
                case TYPE_DATE: {
                    DateTimeValue date_value;
                    reinterpret_cast<VecDateTimeValue*>(const_cast<char*>(value.data))
                            ->convert_vec_dt_to_dt(&date_value);
                    ...
                    range.add_range(to_olap_filter_type(fn_name, slot_ref_child),
                ...
                case TYPE_DATEV2: {
                    DateV2Value<DateTimeV2ValueType> datetimev2_value =
                            *reinterpret_cast<DateV2Value<DateTimeV2ValueType>*>(
                    ...
                    if constexpr (T == PrimitiveType::TYPE_DATEV2) {
                        range.add_range(to_olap_filter_type(fn_name, slot_ref_child), date_v2);
                        break;
                    } else {
                        __builtin_unreachable();
                    }
                ...
                case TYPE_HLL:
                case TYPE_STRING: {
                    if constexpr (T == TYPE_CHAR || T == TYPE_VARCHAR || T == TYPE_HLL ||
                                  T == TYPE_STRING) {
                        range.add_range(to_olap_filter_type(fn_name, slot_ref_child),
                                        StringValue(value.data, value.size));
                    } else {
                        LOG(FATAL) << "Invalid type " << slot->type().type;
                    }
                    break;
                }
...
VExpr* VOlapScanNode::_normalize_predicate(RuntimeState* state, VExpr* conjunct_expr_root) {
    ...
    auto in_predicate_checker = [&](const std::vector<VExpr*>& children, const VSlotRef** slot,
        ...
        if (VExpr::expr_without_cast(children[0])->node_type() != TExprNodeType::SLOT_REF) {
            // not a slot ref(column)
            return false;
        }
    ...
    if (conjunct_expr_root != nullptr) {
        if (is_leaf(conjunct_expr_root)) {
            ...
            eval_const_conjuncts(conjunct_expr_root, *(_vconjunct_ctx_ptr.get()), &push_down);
            if (!push_down && (_is_predicate_acting_on_slot(conjunct_expr_root,
                                                            in_predicate_checker, &slot, &range) ||
                               _is_predicate_acting_on_slot(conjunct_expr_root,
                                                            eq_predicate_checker, &slot, &range))) {
