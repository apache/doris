...
Status VNativeTransformer::write(const Block& block) {
    ...
    VLOG_ROW << "VNativeTransformer::write start, rows=" << block.rows();
    ...
    Status st = block.serialize(BeExecVersionManager::get_newest_version(), &pblock,
                                ...
                                _compression_type);
    VLOG_ROW << "VNativeTransformer serialize block uncompressed_bytes: " << uncompressed_bytes
             << ", compressed_bytes: " << compressed_bytes
             << ", compression_type: " << _compression_type
             << ", compressed_time: " << compressed_time << ", block rows: " << block.rows();
    if (!st.ok()) {
        LOG(WARNING) << "VNativeTransformer serialize block failed: " << st;
        return st;
    }
    ...
    if (!pblock.SerializeToString(&buff)) {
        ...
        LOG(WARNING) << "VNativeTransformer SerializeToString failed: " << err;
        return err;
    }
    ...
    st = _file_writer->append(len_slice);
    if (!st.ok()) {
        LOG(WARNING) << "VNativeTransformer append length failed, len=" << len << ", st=" << st;
        return st;
    }
    st = _file_writer->append(buff);
    if (!st.ok()) {
        LOG(WARNING) << "VNativeTransformer append block body failed, bytes=" << buff.size()
                     << ", st=" << st;
        return st;
    }
    ...
    VLOG_ROW << "VNativeTransformer::write end, rows=" << block.rows()
             << ", written_len=" << _written_len;
    return Status::OK();
}
...
Status VNativeTransformer::close() {
    ...
    if (_file_writer != nullptr && _file_writer->state() != doris::io::FileWriter::State::CLOSED) {
        ...
        if (!st.ok()) {
            LOG(WARNING) << "VNativeTransformer close underlying file writer failed: " << st;
            return st;
        }
    }

    VLOG_ROW << "VNativeTransformer::close, total_written_len=" << _written_len
             << ", total_rows=" << _cur_written_rows;
    return Status::OK();
}
