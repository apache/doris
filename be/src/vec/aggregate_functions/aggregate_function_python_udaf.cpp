...
Status AggregatePythonUDAFData::add(int64_t place_id, const IColumn** columns,
    ...
    // Each place has its own client, so we always use is_single_place=true
    RETURN_IF_ERROR(
...
Status AggregatePythonUDAFData::write(BufferWritable& buf, int64_t place) const {
    ...
    // Cache in serialize_data and write to buffer
    serialize_data.assign(reinterpret_cast<const char*>(serialized_state->data()),
...
Status AggregatePythonUDAFData::get(IColumn& to, const DataTypePtr& result_type,
    ...
    if (result->num_columns() != 1 || result->num_rows() != 1) {
        return Status::InternalError("Invalid result from Python UDAF: expected 1x1 RecordBatch");
    }
...
void AggregatePythonUDAF::create(AggregateDataPtr __restrict place) const {
    ...
    PythonUDAFClient* client = get_shared_client();
    if (UNLIKELY(!client)) {
        this->data(place).~Data();
        throw doris::Exception(ErrorCode::INTERNAL_ERROR, "Failed to get shared Python UDAF client");
    }
...
Status AggregatePythonUDAF::init_shared_client() const {
    if (_shared_client) {
        return Status::OK();
    }
...
PythonUDAFClient* AggregatePythonUDAF::get_shared_client() const {
    // Double-checked locking for thread-safe lazy initialization
    if (!_client_initialized) {
        std::lock_guard<std::mutex> lock(_client_init_mutex);
        if (!_client_initialized) {
            Status st = init_shared_client();
            if (!st.ok()) {
