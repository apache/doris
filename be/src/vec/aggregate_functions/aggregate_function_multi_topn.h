...
class AggregateFunctionMultiTopN final
        : public IAggregateFunctionDataHelper<AggregateFunctionTopKGenericData,
                                              AggregateFunctionMultiTopN<is_include_counts>> {
private:
...
public:
    ...
    DataTypePtr get_return_type() const override {
        // if constexpr (is_include_counts) {
        //     DataTypes types {
        //             this->get_argument_types()[0],
        ...
        //     return std::make_shared<DataTypeArray>(make_nullable(
        //             std::make_shared<DataTypeStruct>(std::move(types), std::move(names))));
        // } else {
        //     return std::make_shared<DataTypeArray>(make_nullable(this->get_argument_types()[0]));
        // }
    ...
    void insert_result_into(ConstAggregateDataPtr __restrict place, IColumn& to) const override {
        ...
        // auto _insert_result_into = [&result_vec](IColumn& data_to) {
        //     if constexpr (is_include_counts) {
        //         auto& column_tuple = assert_cast<ColumnStruct&, TypeCheckOnRelease::DISABLE>(data_to);
        //         IColumn& column_key = column_tuple.get_column(0);
        //         IColumn& column_count = column_tuple.get_column(1);
        //         IColumn& column_error = column_tuple.get_column(2);
        //         for (auto& elem : result_vec) {
        //             column_count.insert(elem.count);
        //             column_error.insert(elem.error);
        //             column_key.deserialize_and_insert_from_arena(elem.key.data);
        //         }
        //     } else {
        //         for (auto& elem : result_vec) {
        //             data_to.deserialize_and_insert_from_arena(elem.key.data);
        //         }
        //     }
        // };

        // if (!data_to.is_nullable()) {
        //     _insert_result_into(data_to);
        // } else {
        //     auto* col_null = assert_cast<ColumnNullable*, TypeCheckOnRelease::DISABLE>(&data_to);
        //     _insert_result_into(col_null->get_nested_column());
