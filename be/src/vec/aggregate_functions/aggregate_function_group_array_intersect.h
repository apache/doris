...
class NullableNumericOrDateSet
        : public HybridSet<T, DynamicContainer<typename PrimitiveTypeTraits<T>::CppType>> {
public:
    NullableNumericOrDateSet()
            : HybridSet<T, DynamicContainer<typename PrimitiveTypeTraits<T>::CppType>>(true) {}
...
public:
    ...
    void insert_result_into(ConstAggregateDataPtr __restrict place, IColumn& to) const override {
        ...
        auto insert_values = [](typename State::ColVecType& nested_col, auto& set,
            ...
            while (it->has_next()) {
                const auto value =
                        *reinterpret_cast<const typename PrimitiveTypeTraits<T>::CppNativeType*>(
                                it->get_value());
