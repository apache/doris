...
public:
    using ResultDataType =
            std::conditional_t<is_decimal(T),
                               typename PrimitiveTypeTraits<Data::ResultPType>::DataType,
                               DataTypeFloat64>;
    using ColVecType = typename PrimitiveTypeTraits<T>::ColumnType;
    using ColVecResult =
            std::conditional_t<is_decimal(T),
                               typename PrimitiveTypeTraits<Data::ResultPType>::ColumnType,
                               ColumnFloat64>;
    ...
    AggregateFunctionAvg(const DataTypes& argument_types_)
            : IAggregateFunctionDataHelper<Data, AggregateFunctionAvg<T, Data>>(argument_types_),
              ...
              output_scale(std::max(DEFAULT_MIN_AVG_DECIMAL128_SCALE, input_scale)) {
