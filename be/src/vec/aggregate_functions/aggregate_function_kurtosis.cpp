...
template <typename T>
AggregateFunctionPtr type_dispatch_for_aggregate_function_kurtosis(const DataTypes& argument_types,
    ...
    return creator_without_type::create<AggregateFunctionVarianceSimple<StatFunctionTemplate>>(
            argument_types, result_is_nullable, StatisticsFunctionKind::kurtPop);
};

AggregateFunctionPtr create_aggregate_function_kurtosis(const std::string& name,
...
#define DISPATCH(TYPE)                                                             \
    if (type.idx == TypeIndex::TYPE)                                               \
        return type_dispatch_for_aggregate_function_kurtosis<TYPE>(argument_types, \
                                                                   result_is_nullable);
...
void register_aggregate_function_kurtosis_pop(AggregateFunctionSimpleFactory& factory) {
    factory.register_function_both("kurt", create_aggregate_function_kurtosis);
