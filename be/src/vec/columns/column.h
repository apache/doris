...
public:
     ...
     *  NOTICE: only column_nullable and predict_column, column_vector, column_dictionary now support filter_by_selector
     */
    virtual Status filter_by_selector(const uint16_t* sel, size_t sel_size, IColumn* col_ptr) {
        LOG(FATAL) << get_name() << " do not support filter_by_selector, only column_nullable and predict_column support";
        __builtin_unreachable();
    }
