...
ColumnStruct::ColumnStruct(MutableColumns&& mutable_columns) {
    ...
    for (auto& column : mutable_columns) {
        if (is_column_const(*column)) {
            throw doris::Exception(ErrorCode::INTERNAL_ERROR, "ColumnStruct cannot have ColumnConst as its element");
...
void ColumnStruct::insert(const Field& x) {
    ...
    if (tuple.size() != tuple_size) {
        throw doris::Exception(ErrorCode::INTERNAL_ERROR, "Cannot insert value of different size into tuple. field tuple size {}, columns size {}", tuple.size(), tuple_size);
    }
...
void ColumnStruct::insert_from(const IColumn& src_, size_t n) {
    ...
    if (src.columns.size() != tuple_size) {
        throw doris::Exception(ErrorCode::INTERNAL_ERROR, "Cannot insert value of different size into tuple.");
        __builtin_unreachable();
    }
