...
void ColumnObject::serialize_one_row_to_string(int row, std::string* output) const {
...
Status ColumnObject::finalize(FinalizeMode mode) {
    ...
    for (auto&& entry : subcolumns) {
        ...
        if (get_base_type_of_array(least_common_type)->get_type_id() == TypeIndex::Nothing) {
            continue;
        }
