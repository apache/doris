...
ColumnNullable::ColumnNullable(MutableColumnPtr&& nested_column_, MutableColumnPtr&& null_map_)
    ...
    if (is_column_const(*null_map)) {
        throw doris::Exception(ErrorCode::INTERNAL_ERROR, "ColumnNullable cannot have constant null map");
...
void ColumnNullable::apply_null_map_impl(const ColumnUInt8& map) {
    ...
    if (arr1.size() != arr2.size()) {
        throw doris::Exception(ErrorCode::INTERNAL_ERROR, "Inconsistent sizes of ColumnNullable objects");
        __builtin_unreachable();
    }
...
void ColumnNullable::check_consistency() const {
    if (null_map->size() != get_nested_column().size()) {
        throw Exception(ErrorCode::INTERNAL_ERROR, "Sizes of nested column and null map of Nullable column are not equal");
    }
}
