...
Status IndexChannel::close_wait(
    ...
    while (true) {
        ...
        for (auto& it : _node_channels) {
            if (!status.ok() || (it.second->is_closed() && !it.second->is_cancelled())) {
                continue;
            }
            bool node_channel_closed = false;
            auto s = it.second->close_wait(state, &node_channel_closed);
            if (node_channel_closed) {
                s = it.second->after_close_handle(state, writer_stats, node_add_batch_counter_map);
            } else {
                all_node_channel_closed = false;
            }
            if (!s.ok()) {
                status = cancel_channel_and_check_intolerable_failure(
                        std::move(status), s.to_string(), *this, *it.second);
            }
        }
        if (all_node_channel_closed || !status.ok()) {
            break;
        }
