...
private:
    std::shared_ptr<VPaimonPartitionWriter> _create_partition_writer(
    ...
    // Indices into the output block for partition columns
    std::vector<int> _partition_columns_input_index;
    // Column indices that should NOT be written to the data file (partition cols)
    std::set<size_t> _non_write_columns_indices;

    std::unordered_map<std::string, std::shared_ptr<VPaimonPartitionWriter>> _partitions_to_writers;
