...
public:
    ...
    VPaimonPartitionWriter(std::vector<std::string> partition_values,
    ...
    inline size_t written_len() const {
        return _file_format_transformer ? _file_format_transformer->written_len() : 0;
    }
...
private:
    ...
    void _build_paimon_commit_data(TPaimonCommitData* commit_data);
