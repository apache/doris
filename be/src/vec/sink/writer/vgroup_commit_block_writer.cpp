...
VGroupCommitBlockWriter::VGroupCommitBlockWriter(const TDataSink& t_sink,
                                                 const VExprContextSPtrs& output_exprs)
        : AsyncResultWriter(output_exprs), _filter_bitmap(1024), _t_sink(t_sink) {
    DCHECK(t_sink.__isset.olap_table_sink);
}

VGroupCommitBlockWriter::~VGroupCommitBlockWriter() {
...
Status VGroupCommitBlockWriter::_init(RuntimeState* state, RuntimeProfile* profile) {
    ...
    _profile = profile;
    _mem_tracker = std::make_shared<MemTracker>("VGroupCommitBlockWriter:" +
...
Status VGroupCommitBlockWriter::open(RuntimeState* state, RuntimeProfile* profile) {
    // Prepare the exprs to run.
    RETURN_IF_ERROR(_init(state, profile));
    return Status::OK();
}

Status VGroupCommitBlockWriter::write(Block& input_block) {
    SCOPED_CONSUME_MEM_TRACKER(_mem_tracker.get());
    ...
    RETURN_IF_ERROR(_block_convertor->validate_and_convert_block(
            _state, &input_block, block, _output_vexpr_ctxs, rows, has_filtered_rows));
    ...
    return _add_block(_state, block);
}

Status VGroupCommitBlockWriter::close(Status close_status) {
    RETURN_IF_ERROR(close_status);
    int64_t total_rows = _state->num_rows_load_total();
    int64_t loaded_rows = _state->num_rows_load_total();
    _state->set_num_rows_load_total(loaded_rows + _state->num_rows_load_unselected() +
                                    _state->num_rows_load_filtered());
    _state->update_num_rows_load_filtered(_block_convertor->num_filtered_rows() + total_rows -
                                          loaded_rows);
    if (!_is_block_appended) {
        // if not meet the max_filter_ratio, we should return error status directly
        int64_t num_selected_rows =
                _state->num_rows_load_total() - _state->num_rows_load_unselected();
        if (num_selected_rows > 0 &&
            (double)_state->num_rows_load_filtered() / num_selected_rows > _max_filter_ratio) {
            return Status::DataQualityError("too many filtered rows");
        }
        RETURN_IF_ERROR(_add_blocks(_state, true));
    }
    if (_load_block_queue) {
        _remove_estimated_wal_bytes();
        _load_block_queue->remove_load_id(_load_id);
...
Status VGroupCommitBlockWriter::_add_block(RuntimeState* state,
                                           std::shared_ptr<vectorized::Block> block) {
    if (block->rows() == 0) {
        return Status::OK();
    }
...
Status VGroupCommitBlockWriter::_add_blocks(RuntimeState* state,
...
size_t VGroupCommitBlockWriter::_calculate_estimated_wal_bytes(
        bool is_blocks_contain_all_load_data) {
...
void VGroupCommitBlockWriter::_remove_estimated_wal_bytes() {
