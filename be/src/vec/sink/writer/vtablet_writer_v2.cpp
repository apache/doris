...
void VTabletWriterV2::_cancel(Status status) {
    ...
    if (_load_stream_map) {
        _load_stream_map->for_each([status](int64_t dst_id, LoadStreamStubs& streams) {
...
void VTabletWriterV2::_close_wait(bool incremental) {
    ...
    auto st = _load_stream_map->for_each_st(
            [this, incremental](int64_t dst_id, LoadStreamStubs& streams) -> Status {
                ...
                if (!st.ok()) {
                    LOG(WARNING) << "close_wait timeout on streams to dst_id=" << dst_id << ", load_id="
                                    << print_id(_load_id) << ": " << st;
                }
                return st;
            });
    if (!st.ok()) {
