...
#include "vec/sink/vtablet_finder.h"
    ...
    Status GroupCommitBlockSink::init(const TDataSink& t_sink) {
        ...
        _vpartition =
                std::make_unique<doris::VOlapTablePartitionParam>(_schema, table_sink.partition);
    ...
    Status GroupCommitBlockSink::send(RuntimeState* state, vectorized::Block* input_block,
                                      bool eos) {
        SCOPED_CONSUME_MEM_TRACKER(_mem_tracker.get());
