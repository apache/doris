...
public:
    OpenPartitionClosure(VNodeChannelV2* vnode_channel, IndexChannelV2* index_channel,
    ...
    VNodeChannelV2* vnode_channel;
    IndexChannelV2* index_channel;
    int64_t partition_id;
};

IndexChannelV2::~IndexChannelV2() {}

Status IndexChannelV2::init(RuntimeState* state, const std::vector<TTabletWithPartition>& tablets) {
    ...
    for (auto& tablet : tablets) {
        ...
        std::vector<std::shared_ptr<VNodeChannelV2>> channels;
        for (auto& node_id : location->node_ids) {
            std::shared_ptr<VNodeChannelV2> channel;
            ...
            if (it == _node_channels.end()) {
                ...
                channel = std::make_shared<VNodeChannelV2>(_parent, this, node_id);
...
void IndexChannelV2::mark_as_failed(int64_t node_id, const std::string& host,
                                    const std::string& err, int64_t tablet_id) {
...
Status IndexChannelV2::check_intolerable_failure() {
    std::lock_guard<doris::SpinLock> l(_fail_lock);
    return _intolerable_failure_status;
}

void IndexChannelV2::set_error_tablet_in_state(RuntimeState* state) {
...
void IndexChannelV2::set_tablets_received_rows(
...
Status IndexChannelV2::check_tablet_received_rows_consistency() {
...
VNodeChannelV2::VNodeChannelV2(VOlapTableSinkV2* parent, IndexChannelV2* index_channel,
                               int64_t node_id)
...
void VNodeChannelV2::clear_all_blocks() {
...
Status VNodeChannelV2::init(RuntimeState* state) {
    ...
    _name = fmt::format("VNodeChannelV2[{}-{}]", _index_channel->_index_id, _node_id);
...
Status VNodeChannelV2::open_wait() {
    _open_closure->join();
    SCOPED_CONSUME_MEM_TRACKER(_node_channel_tracker.get());
    ...
    // add block closure
    _add_block_closure = ReusableClosureV2<PTabletWriterAddBlockResult>::create();
...
void VNodeChannelV2::open_partition(int64_t partition_id) {
...
void VNodeChannelV2::open_partition_wait() {
    for (auto& open_partition_closure : _open_partition_closures) {
        open_partition_closure->join();
    }
}

Status VNodeChannelV2::add_block(vectorized::Block* block, const Payload* payload, bool is_append) {
    SCOPED_CONSUME_MEM_TRACKER(_node_channel_tracker.get());
    ...
    if (is_append || _cur_mutable_block->rows() >= _batch_size ||
        _cur_mutable_block->bytes() > config::doris_scanner_row_bytes) {
        {
            ...
            VLOG_DEBUG << "VOlapTableSinkV2:" << _parent << " VNodeChannelV2:" << this
...
int VNodeChannelV2::try_send_and_fetch_status(RuntimeState* state,
    ...
    if (_pending_batches_num > 0) {
        auto s = thread_pool_token->submit_func(
                std::bind(&VNodeChannelV2::try_send_block, this, state));
        if (!s.ok()) {
...
void VNodeChannelV2::_cancel_with_msg(const std::string& msg) {
...
Status VNodeChannelV2::none_of(std::initializer_list<bool> vars) {
...
void VNodeChannelV2::try_send_block(RuntimeState* state) {
    ...
    if (_parent->_transfer_large_data_by_brpc && request.has_block() &&
        ...
        Status st = request_embed_attachment_contain_block<
                PTabletWriterAddBlockRequest, ReusableClosureV2<PTabletWriterAddBlockResult>>(
...
void VNodeChannelV2::cancel(const std::string& cancel_msg) {
    SCOPED_CONSUME_MEM_TRACKER(_node_channel_tracker.get());
...
Status VNodeChannelV2::close_wait(RuntimeState* state) {
    SCOPED_CONSUME_MEM_TRACKER(_node_channel_tracker.get());
    // set _is_closed to true finally
    Defer set_closed {[&]() {
        std::lock_guard<std::mutex> l(_closed_lock);
...
VOlapTableSinkV2::~VOlapTableSinkV2() {
    ...
    for (const auto& index_channel : _channels) {
        index_channel->for_each_node_channel(
                [](const std::shared_ptr<VNodeChannelV2>& ch) { ch->clear_all_blocks(); });
    }
}
...
Status VOlapTableSinkV2::prepare(RuntimeState* state) {
    ...
    for (int i = 0; i < _schema->indexes().size(); ++i) {
        ...
        _channels.emplace_back(new IndexChannelV2(this, index->index_id, index->where_clause));
...
void VOlapTableSinkV2::_open_partition(const VOlapTablePartition* partition) {
...
void VOlapTableSinkV2::_send_batch_process() {
    ...
    while (true) {
        ...
        for (auto index_channel : _channels) {
            index_channel->for_each_node_channel([&running_channels_num,
                                                  this](const std::shared_ptr<VNodeChannelV2>& ch) {
...
size_t VOlapTableSinkV2::get_pending_bytes() const {
...
Status VOlapTableSinkV2::close(RuntimeState* state, Status exec_status) {
    ...
    if (status.ok()) {
        ...
        std::unordered_map<int64_t, AddBatchCounterV2> node_add_batch_counter_map;
        int64_t serialize_batch_ns = 0, queue_push_lock_ns = 0, actual_consume_ns = 0,
                ...
                num_node_channels = 0;
        VNodeChannelV2Stat channel_stat;
        {
            ...
            for (auto index_channel : _channels) {
                index_channel->for_each_node_channel(
                        [](const std::shared_ptr<VNodeChannelV2>& ch) { ch->mark_close(); });
            ...
            for (auto index_channel : _channels) {
                ...
                index_channel->for_each_node_channel(
                        [&index_channel, &state, &node_add_batch_counter_map, &serialize_batch_ns,
                         ...
                         &total_add_batch_num](const std::shared_ptr<VNodeChannelV2>& ch) {
        ...
        for (auto channel : _channels) {
            channel->for_each_node_channel([&status](const std::shared_ptr<VNodeChannelV2>& ch) {
