...
public:
    ...
    Status add_rows(Block* block, const std::vector<uint32_t>& rows, bool eos) {
        ...
        RETURN_IF_ERROR(_serializer.next_serialized_block(block, _pblock.get(), 1, &serialized, eos,
                                                          &rows));
        if (serialized) {
            RETURN_IF_ERROR(_send_current_block(eos, Status::OK()));
        }
...
protected:
    Status _send_local_block(Status exec_status, bool eos = false);
    Status _send_current_block(bool eos, Status exec_status);
