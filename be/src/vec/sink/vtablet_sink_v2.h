...
struct AddBatchCounterV2 {
    ...
    AddBatchCounterV2& operator+=(const AddBatchCounterV2& rhs) {
    ...
    friend AddBatchCounterV2 operator+(const AddBatchCounterV2& lhs, const AddBatchCounterV2& rhs) {
        AddBatchCounterV2 sum = lhs;
        sum += rhs;
        return sum;
    }
};
...
// Delete this point is safe, don't worry about RPC callback will run after ReusableClosureV2 deleted.
template <typename T>
class ReusableClosureV2 final : public google::protobuf::Closure {
public:
    ReusableClosureV2() : cid(INVALID_BTHREAD_ID) {}
    ~ReusableClosureV2() override {
    ...
    static ReusableClosureV2<T>* create() { return new ReusableClosureV2<T>(); }
...
public:
    VNodeChannelV2Stat& operator+=(const VNodeChannelV2Stat& stat) {
...
class VNodeChannelV2 {
public:
    VNodeChannelV2(VOlapTableSinkV2* parent, IndexChannelV2* index_channel, int64_t node_id);
    ...
    void time_report(std::unordered_map<int64_t, AddBatchCounterV2>* add_batch_counter_map,
                     int64_t* serialize_batch_ns, VNodeChannelV2Stat* stat,
...
protected:
    ...
    VOlapTableSinkV2* _parent = nullptr;
    IndexChannelV2* _index_channel = nullptr;
    int64_t _node_id = -1;
    ...
    AddBatchCounterV2 _add_batch_counter;
    std::atomic<int64_t> _serialize_batch_ns {0};
    std::atomic<int64_t> _queue_push_lock_ns {0};
    std::atomic<int64_t> _actual_consume_ns {0};

    VNodeChannelV2Stat _stat;
    // lock to protect _is_closed.
    // The methods in the IndexChannelV2 are called back in the RpcClosure in the NodeChannel.
    // However, this rpc callback may occur after the whole task is finished (e.g. due to network latency),
    // and by that time the IndexChannelV2 may have been destructured, so we should not call the
    // IndexChannelV2 methods anymore, otherwise the BE will crash.
    // Therefore, we use the _is_closed and _closed_lock to ensure that the RPC callback
    // function will not call the IndexChannelV2 method after the NodeChannel is closed.
    // The IndexChannelV2 is definitely accessible until the NodeChannel is closed.
    ...
    ReusableClosureV2<PTabletWriterAddBlockResult>* _add_block_closure = nullptr;
};

class IndexChannelV2 {
public:
    IndexChannelV2(VOlapTableSinkV2* parent, int64_t index_id,
        ...
        _index_channel_tracker =
                std::make_unique<MemTracker>("IndexChannelV2:indexID=" + std::to_string(_index_id));
    }
    ~IndexChannelV2();
    ...
    void for_each_node_channel(
            const std::function<void(const std::shared_ptr<VNodeChannelV2>&)>& func) {
...
private:
    friend class VNodeChannelV2;
    friend class VOlapTableSinkV2;

    VOlapTableSinkV2* _parent;
    int64_t _index_id;
    ...
    // BeId -> channel
    std::unordered_map<int64_t, std::shared_ptr<VNodeChannelV2>> _node_channels;
    // from tablet_id to backend channel
    std::unordered_map<int64_t, std::vector<std::shared_ptr<VNodeChannelV2>>> _channels_by_tablet;
...
private:
    friend class VNodeChannelV2;
    friend class IndexChannelV2;

    using ChannelDistributionPayload = std::vector<std::unordered_map<VNodeChannelV2*, Payload>>;
    ...
    // index_channel
    std::vector<std::shared_ptr<IndexChannelV2>> _channels;
