...
Status Channel::send_remote_block(std::unique_ptr<PBlock>&& block, bool eos, Status exec_status) {
    ...
    if (eos || block->column_metas_size()) {
        RETURN_IF_ERROR(_buffer->add_block({this, std::move(block), eos, exec_status}));
    }
    return Status::OK();
}
...
Status Channel::_send_current_block(bool eos, Status exec_status) {
    if (is_local()) {
        return _send_local_block(exec_status, eos);
    }
    return send_remote_block(std::move(_pblock), eos, exec_status);
}

Status Channel::_send_local_block(Status exec_status, bool eos) {
