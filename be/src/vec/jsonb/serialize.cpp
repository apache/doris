...
void JsonbSerializeUtil::block_to_jsonb(const TabletSchema& schema, const Block& block,
    ...
    for (int i = 0; i < num_rows; ++i) {
        ...
        for (int j = 0; j < num_cols; ++j) {
            ...
            if (Status st = block.get_data_type(j)->get_serde()->write_column_to_jsonb(
                        *column, jsonb_writer, &pool, tablet_column.unique_id(), i);
                !st) {
                LOG(FATAL) << block.get_data_type(j)->get_name()
                           << " write_column_to_jsonb err: " << st.to_string();
            }
            //            const auto& data_ref =
...
void JsonbSerializeUtil::jsonb_to_block(const TupleDescriptor& desc, const char* data, size_t size,
    ...
    for (int j = 0; j < desc.slots().size(); ++j) {
        ...
        //            continue;
        //        }
        if (Status st = dst.get_data_type(j)->get_serde()->read_column_from_jsonb(*dst_column,
                                                                                  slot_value);
            !st) {
            LOG(FATAL) << dst.get_data_type(j)->get_name()
                       << " read_column_from jsonb err: " << st.to_string();
        }
