...
template <typename T>
char* DataTypeNumberBase<T>::serialize2(const IColumn& column, char* buf,
                                        int be_exec_version) const {
    // const flag
    ...
    *reinterpret_cast<uint32_t*>(buf) = mem_size;
    ...
    const auto* origin_data = assert_cast<const ColumnVector<T>&>(*data_column).get_data().data();
...
template <typename T>
const char* DataTypeNumberBase<T>::deserialize2(const char* buf, MutableColumnPtr* column,
                                                int be_exec_version) const {
    //const flag
