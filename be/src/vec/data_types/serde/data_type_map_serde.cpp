...
Status DataTypeMapSerDe::from_string(StringRef& str, IColumn& column,
    ...
    for (int i = 0; i < key_value_size; i++) {
        RETURN_IF_ERROR(ComplexTypeDeserializeUtil::process_column(key_serde, nested_key_column,
                                                                   key_slices[i], options));
        RETURN_IF_ERROR(ComplexTypeDeserializeUtil::process_column(value_serde, nested_val_column,
                                                                   value_slices[i], options));
    }
