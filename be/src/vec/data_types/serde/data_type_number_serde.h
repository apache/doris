...
Status DataTypeNumberSerDe<T>::_write_column_to_mysql(
    ...
    for (auto i = start; i < end; ++i) {
        ...
        if constexpr (std::is_same_v<T, Int8> || std::is_same_v<T, UInt8>) {
            buf_ret = result[i].push_tinyint(data[col_index]);
        } else if constexpr (std::is_same_v<T, Int16> || std::is_same_v<T, UInt16>) {
            buf_ret = result[i].push_smallint(data[col_index]);
        } else if constexpr (std::is_same_v<T, Int32> || std::is_same_v<T, UInt32>) {
            buf_ret = result[i].push_int(data[col_index]);
        } else if constexpr (std::is_same_v<T, Int64> || std::is_same_v<T, UInt64>) {
            buf_ret = result[i].push_bigint(data[col_index]);
        } else if constexpr (std::is_same_v<T, Int128>) {
            buf_ret = result[i].push_largeint(data[col_index]);
        } else if constexpr (std::is_same_v<T, float>) {
            buf_ret = result[i].push_float(data[col_index]);
        } else if constexpr (std::is_same_v<T, double>) {
            buf_ret = result[i].push_double(data[col_index]);
        }
    }
    return Status::OK();
}
