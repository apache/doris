...
const uint8_t* DataTypeSerDe::deserialize_binary_to_non_nullable_column(const uint8_t* data,
                                                                        IColumn& column) {
    const FieldType type = static_cast<FieldType>(*data++);
    const uint8_t* end = data;
...
#define HANDLE_SIMPLE_SERDE(FT, SERDE)                           \
    case FieldType::FT: {                                        \
        end = SERDE::deserialize_binary_to_column(data, column); \
        break;                                                   \
    }

#define HANDLE_T_NUM_SERDE(FT, TYPEID)                                                 \
    case FieldType::FT: {                                                              \
        end = DataTypeNumberSerDe<TYPEID>::deserialize_binary_to_column(data, column); \
        break;                                                                         \
    }

#define HANDLE_T_DEC_SERDE(FT, TYPEID)                                                  \
    case FieldType::FT: {                                                               \
        end = DataTypeDecimalSerDe<TYPEID>::deserialize_binary_to_column(data, column); \
        break;                                                                          \
    }
    ...
    case FieldType::OLAP_FIELD_TYPE_NONE: {
        // The encoded value is NULL, but the target column cannot represent NULL.
        // Preserve best-effort semantics by inserting default.
        end = data;
    ...
    default:
        throw doris::Exception(ErrorCode::OUT_OF_BOUND,
                               "Type ({}) for deserialize_binary_to_non_nullable_column is invalid",
                               type);
    }
