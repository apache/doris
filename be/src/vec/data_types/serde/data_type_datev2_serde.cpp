...
Status DataTypeDateV2SerDe::write_column_to_jsonb(const IColumn& column, JsonbWriter** results,
                                                  const size_t num_rows,
                                                  const uint32_t* indexes) const {
    const auto& column_datetime = assert_cast<const ColumnDateV2&>(column);
    for (uint32_t i = 0; i != num_rows; ++i) {
        auto row_idx = indexes ? indexes[i] : i;
        auto value = binary_cast<UInt32, DateV2Value<DateV2ValueType>>(
