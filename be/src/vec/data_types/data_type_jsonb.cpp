...
Status DataTypeJsonb::from_string(ReadBuffer& rb, IColumn* column) const {
    ...
    column_jsonb->insert_data(value.value(), value.size());
...
int64_t DataTypeJsonb::get_uncompressed_serialized_bytes(const IColumn& column,
    ...
    const auto& column_jsonb = assert_cast<const ColumnJsonb&>(*ptr.get());
    return data_type_string.get_uncompressed_serialized_bytes(
        *column_jsonb.get_column_string(), data_version);
}
...
char* DataTypeJsonb::serialize(const IColumn& column, char* buf, int data_version) const {
    ...
    return data_type_string.serialize(*column_jsonb.get_column_string(), buf, data_version);
}
...
const char* DataTypeJsonb::deserialize(const char* buf, IColumn* column, int data_version) const {
    ...
    return data_type_string.deserialize(buf, column_jsonb->get_column_string(), data_version);
}
