...
bool DataTypeVariant::equals(const IDataType& rhs) const {
    ...
    if (rhs_type &&
        _enable_typed_paths_to_sparse != rhs_type->variant_enable_typed_paths_to_sparse()) {
        VLOG_DEBUG << "_enable_typed_paths_to_sparse is " << _enable_typed_paths_to_sparse
                   << " rhs_type->variant_enable_typed_paths_to_sparse() "
                   << rhs_type->variant_enable_typed_paths_to_sparse();
        return false;
    }
...
MutableColumnPtr DataTypeVariant::create_column() const {
    auto col = ColumnVariant::create(_max_subcolumns_count);
    auto* variant = assert_cast<ColumnVariant*>(col.get());
    variant->set_variant_enable_typed_paths_to_sparse(_enable_typed_paths_to_sparse);
    return col;
}
