...
void SpillReader::_init_profile() {
...
// The returned block is owned by SpillReader and is
...
Status SpillReader::read(Block* block, bool* eos) {
    ...
    RETURN_IF_ERROR(read_at_offset(block_start_offsets_[read_block_index_], bytes_to_read, block));

    return Status::OK();
}

Status SpillReader::read_at_offset(size_t offset, size_t bytes_to_read, Block* block) {
...

    {
        ...
        RETURN_IF_ERROR(file_reader_->read_at(offset, result, &bytes_read));
    }
