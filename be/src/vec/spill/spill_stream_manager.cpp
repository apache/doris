...
Status SpillStreamManager::spill_stream(SpillStreamSPtr spill_stream) {
    RETURN_IF_ERROR(io_thread_pool_->submit_func([spill_stream] { spill_stream->spill(); }));
...
Status SpillStreamManager::register_spill_stream(SpillStreamSPtr& spill_stream,
    ...
    spill_stream = std::make_shared<SpillStream>(id, spill_dir, batch_rows, batch_bytes, profile);
