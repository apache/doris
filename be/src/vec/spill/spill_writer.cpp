...
void SpillWriter::_init_profile() {
...
Status SpillWriter::open() {
    RETURN_IF_ERROR(io::global_local_filesystem()->create_directory(dir_));
...
int SpillWriter::get_fd() const {
    return ((io::LocalFileWriter*)file_writer_.get())->get_fd();
}
