...
void Allocator<clear_memory_, mmap_populate, use_mmap, MemoryAllocator>::consume_memory(
        size_t size) {
    // save tracker of the first alloc, all subsequent allocs must have the same tracker in thread context.
...
void Allocator<clear_memory_, mmap_populate, use_mmap, MemoryAllocator>::release_memory(
    ...
    if ((thread_context && thread_context->thread_mem_tracker()->label() != "Orphan") ||
        ...
        // If thread_context not exist and the label of thread_mem_tracker is equal to
        // Allocator mem_tracker label, this means that in the scope of SCOPED_ATTACH_TASK,
        // so thread_mem_tracker should be used to release memory.

        // DEBUG 将在 RELEASE_THREAD_MEM_TRACKER 时报错
        // there is a scenario where an object that inherits Allocator has never called alloc, but frees memory.
        ...
        // if thread_context does not exist or the label of thread_mem_tracker is not equal to
        // Allocator tracker label, it usually happens during object destruction. This means that
