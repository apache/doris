...
Status send_fetch_full_base_schema_view_rpc(FullBaseSchemaView* schema_view) {
    ...
    for (const auto& column : res.allColumns) {
        ...
        std::transform(column.column_name.begin(), column.column_name.end(),
                       std::back_inserter(column_name_lower_case), ::tolower);
        schema_view->column_name_to_column[column_name_lower_case] = column;
    }
