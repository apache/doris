...
size_t FullSorter::get_reserve_mem_size(RuntimeState* state, bool eos) const {
    ...
    if (rows != 0) {
        ...
        const auto allocated_bytes = _state->unsorted_block_->allocated_bytes();
        ...
        auto sort = new_rows > buffered_block_size_ || new_block_bytes > buffered_block_bytes_;
        if (sort) {
            // new column is created when doing sort, reserve average size of one column
            // for estimation
            size_to_reserve += new_block_bytes / _state->unsorted_block_->columns();

            // helping data structures used during sorting
            size_to_reserve += new_rows * sizeof(IColumn::Permutation::value_type);

            auto sort_columns_count = _vsort_exec_exprs.lhs_ordering_expr_ctxs().size();
