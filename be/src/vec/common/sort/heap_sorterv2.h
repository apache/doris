...
class HeapSorterV2 final : public Sorter {
    ENABLE_FACTORY_CREATOR(HeapSorterV2);

public:
    HeapSorterV2(VSortExecExprs& vsort_exec_exprs, int64_t limit, int64_t offset, ObjectPool* pool,
                 std::vector<bool>& is_asc_order, std::vector<bool>& nulls_first,
                 const RowDescriptor& row_desc);

    ~HeapSorterV2() override = default;
