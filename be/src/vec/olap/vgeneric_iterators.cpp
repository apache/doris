...
Status VStatisticsIterator::init(const StorageReadOptions& opts, long tz_offset) {
    if (!_init) {
...
Status VAutoIncrementIterator::init(const StorageReadOptions& opts, long tz_offset) {
    return Status::OK();
}
...
Status VMergeIterator::init(const StorageReadOptions& opts, long tz_offset) {
...
Status VUnionIterator::init(const StorageReadOptions& opts, long tz_offset) {
    if (_origin_iters.empty()) {
        return Status::OK();
    }
    ...
    RETURN_IF_ERROR(_cur_iter->init(_read_options, tz_offset));
