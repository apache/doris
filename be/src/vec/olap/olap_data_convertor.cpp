...
OlapBlockDataConvertor::create_olap_column_data_convertor(const TabletColumn& column) {
    ...
    const void* OlapBlockDataConvertor::OlapColumnDataConvertorObject::get_data_at(size_t offset)
            const {
        UInt8 null_flag = 0;
        if (_nullmap) {
    ...
    Status OlapBlockDataConvertor::OlapColumnDataConvertorBitMap::convert_to_olap() {
        ...
        if (_nullmap) {
            ...
            column_bitmap =
                    assert_cast<const vectorized::ColumnBitmap*>(_typed_column.column.get());
        }
    ...
    const void* OlapBlockDataConvertor::OlapColumnDataConvertorChar::get_data_at(size_t offset)
            const {
        UInt8 null_flag = 0;
        if (_nullmap) {
            null_flag = _nullmap[offset];
    ...
    Status OlapBlockDataConvertor::OlapColumnDataConvertorChar::convert_to_olap() {
        ...
        if (_nullmap) {
            ...
            column_string =
                    assert_cast<const vectorized::ColumnString*>(_typed_column.column.get());
        }
...
>>>>>>> 7f323a214 (resolve rebase master conflicts)
    ...
    const void* OlapBlockDataConvertor::OlapColumnDataConvertorVarChar::get_data_at(size_t offset)
            const {
    ...
    const void* OlapBlockDataConvertor::OlapColumnDataConvertorJson::get_data_at(size_t offset)
            const {
        ...
        if (from_datetime_v2_) {
            ...
            if (_nullmap) {
                auto nullable_column =
                        assert_cast<const vectorized::ColumnNullable*>(_typed_column.column.get());
                column_datetimev2 =
                        assert_cast<const vectorized::ColumnVector<vectorized::UInt64>*>(
                                nullable_column->get_nested_column_ptr().get());
            } else {
                column_datetimev2 =
                        assert_cast<const vectorized::ColumnVector<vectorized::UInt64>*>(
            ...
            const DateV2Value<DateTimeV2ValueType>* datetime_cur =
                    (const DateV2Value<DateTimeV2ValueType>*)(column_datetimev2->get_data()
                                                                      .data()) +
                    _row_pos;
    ...
    Status OlapBlockDataConvertor::OlapColumnDataConvertorArray::convert_to_olap() {
        ...
        if (_nullmap) {
            ...
            data_type_array = assert_cast<const DataTypeArray*>(
                    (assert_cast<const DataTypeNullable*>(_typed_column.type.get())
                             ->get_nested_type())
                            .get());
        } else {
