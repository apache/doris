...
Status VerticalBlockReader::_unique_key_next_block(Block* block, bool* eof) {
    if (_reader_context.is_key_column_group) {
        ...
        if (block_rows != row_buffer_size_cur_batch - merged_rows_in_rs_buffer) {
            std::stringstream ss;
            ss << "block_rows:" << block_rows
               << ",row_buffer_size_cur_batch:" << row_buffer_size_cur_batch
               << ",merged_rows_in_rs_buffer:" << merged_rows_in_rs_buffer;
            LOG(WARNING) << ss.str();
        ...
        if (_filter_delete && block_rows > 0) {
            ...
            while (row_source_idx < _row_sources_buffer->buffered_size()) {
                DCHECK(_row_sources_buffer->get_agg_flag(row_source_idx))
                        << ", row_source_idx: " << row_source_idx
                        << ", buffer size: " << _row_sources_buffer->buffered_size();
                if (!_row_sources_buffer->get_agg_flag(row_source_idx)) {
                    std::stringstream ss;
                    ss << "row_source_idx:" << row_source_idx
                       << ",buffer size:" << _row_sources_buffer->buffered_size();
                    LOG(WARNING) << ss.str();
                    return Status::InternalError(ss.str());
                }
        ...
        if (filtered_rows_in_rs_buffer != filtered_rows_cur_batch) {
            LOG(INFO) << "ERROR: "
                      ...
                      << ", should be: " << rows_should_be_deleted;
        }
        ...
        if (row_buffer_size_cur_batch !=
            block->rows() + merged_rows_cur_batch + filtered_rows_cur_batch) {
            std::stringstream ss;
            ss << "row buffer size: " << row_buffer_size_cur_batch
