...
bool BlockReader::_rowsets_overlapping(const ReaderParams& read_params) {
    ...
    const std::vector<RowSetReaderWithSegments>& rs_readers_with_segments =
            read_params.rs_readers_with_segments;
    for (const auto& rs_reader : rs_readers_with_segments) {
        // version 0-1 of every tablet is empty, just skip this rowset
...
Status BlockReader::_init_collect_iter(const ReaderParams& read_params) {
    ...
    for (int i = 0; i < read_params.rs_readers_with_segments.size(); ++i) {
        auto& rs_reader = read_params.rs_readers_with_segments[i];
