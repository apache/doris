...
public:
    ...
    FunctionLLMSentiment() {
        Status status = init();
        if (!status.ok()) {
            throw Status::InternalError("Failed to initialize FunctionLLMSentiment: " +
                                        status.to_string());
        }
    }
    ...
    Status build_prompt(const Block& block, const ColumnNumbers& arguments, size_t row_num,
        ...
        StringRef text = assert_cast<const ColumnString*>(text_column.column.get())->get_data_at(row_num);
