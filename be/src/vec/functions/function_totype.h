    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        if (auto col_left = check_and_get_column<ColVecLeft>(lcol.get())) {
            if (auto col_right = check_and_get_column<ColVecRight>(rcol.get())) {
                if (left_const) {
                    RETURN_IF_ERROR((Impl<LeftDataType, RightDataType>::scalar_vector(
                            col_left->get_data()[0], col_right->get_data(), vec_res)));
                } else if (right_const) {
                    RETURN_IF_ERROR((Impl<LeftDataType, RightDataType>::vector_scalar(
                            col_left->get_data(), col_right->get_data()[0], vec_res)));
                } else {
                    RETURN_IF_ERROR((Impl<LeftDataType, RightDataType>::vector_vector(
