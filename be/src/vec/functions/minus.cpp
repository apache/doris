...
struct MinusIntegralImpl {
    ...
    using Arg = std::conditional_t<Type == TYPE_BOOLEAN, UInt8,
                                   typename PrimitiveTypeTraits<Type>::CppNativeType>;
    ...
    static inline typename PrimitiveTypeTraits<Type>::CppNativeType apply(Arg a, Arg b) {
        return static_cast<typename PrimitiveTypeTraits<Type>::CppNativeType>(a) - b;
    }

    /// Apply operation and check overflow. It's used for Decimal operations. @returns true if overflowed, false otherwise.
    static inline bool apply(Arg a, Arg b, typename PrimitiveTypeTraits<Type>::CppNativeType& c) {
        return common::sub_overflow(
                static_cast<typename PrimitiveTypeTraits<Type>::CppNativeType>(a), b, c);
    }
    ...
    static ColumnPtr vector_constant(ColumnPtr column_left, Arg b) {
        const auto* column_left_ptr =
                assert_cast<const ColumnType*>(remove_nullable(column_left).get());
    ...
    static ColumnPtr constant_vector(Arg a, ColumnPtr column_right) {
        const auto* column_right_ptr =
                assert_cast<const ColumnType*>(remove_nullable(column_right).get());
...
struct MinusDecimalImpl {
    ...
    static ColumnPtr constant_constant(
        ...
        column_result->get_element(0) = typename PrimitiveTypeTraits<ResultType>::ColumnItemType(
                apply<true>(a, b, *type_left, *type_right, res_data_type, max_result_number,
                            scale_diff_multiplier, check_overflow_for_decimal));
        return column_result;
    }
    ...
    static ColumnPtr vector_constant(
            ...
            const DataTypeDecimal<ResultType>& res_data_type, bool check_overflow_for_decimal) {
        const auto* column_left_ptr =
                assert_cast<const ColumnTypeA*>(remove_nullable(column_left).get());
        auto column_result =
                ...
                [&](auto need_adjust_scale) {
                    for (size_t i = 0; i < column_left->size(); ++i) {
                        c[i] = typename DataTypeDecimal<ResultType>::FieldType(
                                apply<need_adjust_scale>(a[i], b, *type_left, *type_right,
    ...
    static ColumnPtr constant_vector(
            ...
            const DataTypeDecimal<ResultType>& res_data_type, bool check_overflow_for_decimal) {
        const auto* column_right_ptr =
                assert_cast<const ColumnTypeB*>(remove_nullable(column_right).get());
        auto column_result =
        ...
        std::visit(
                [&](auto need_adjust_scale) {
                    for (size_t i = 0; i < column_right->size(); ++i) {
                        c[i] = typename DataTypeDecimal<ResultType>::FieldType(
                                apply<need_adjust_scale>(a, b[i], *type_left, *type_right,
                                                         res_data_type, max_result_number,
                                                         scale_diff_multiplier,
                                                         check_overflow_for_decimal));
                    }
                ...
                [&](auto need_adjust_scale) {
                    for (size_t i = 0; i < sz; i++) {
                        c[i] = typename ColumnDecimal<ResultType>::value_type(
                                apply<need_adjust_scale>(a[i], b[i], *type_left, *type_right,
                                                         res_data_type, max_result_number,
                                                         scale_diff_multiplier,
                                                         check_overflow_for_decimal));
                    }
        ...
        if constexpr (ResultType == TYPE_DECIMALV2) {
            ...
            if (check_overflow) {
                ...
                if (UNLIKELY(apply<ResultType>(a, b, res))) {
                    THROW_DECIMAL_BINARY_OP_OVERFLOW_EXCEPTION(
                            type_left.to_string(ArgA(a)), "subtract", type_right.to_string(ArgB(b)),
                            type_to_string(ResultType), type_result.get_name());
                    // multiply
...
private:
    ColumnPtr constant_constant(ColumnPtr column_left, ColumnPtr column_right,
        ...
        const auto* column_left_ptr =
                assert_cast<const ColumnConst*>(remove_nullable(column_left).get());
        const auto* column_right_ptr =
                assert_cast<const ColumnConst*>(remove_nullable(column_right).get());
...
void register_function_minus(SimpleFunctionFactory& factory) {
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMALV2, TYPE_DECIMALV2>>>();

    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL32, TYPE_DECIMAL32>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL32, TYPE_DECIMAL64>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL32, TYPE_DECIMAL128I>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL32, TYPE_DECIMAL256>>>();

    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL64, TYPE_DECIMAL32>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL64, TYPE_DECIMAL64>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL64, TYPE_DECIMAL128I>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL64, TYPE_DECIMAL256>>>();

    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL128I, TYPE_DECIMAL32>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL128I, TYPE_DECIMAL64>>>();
    factory.register_function<
            FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL128I, TYPE_DECIMAL128I>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL128I, TYPE_DECIMAL256>>>();

    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL256, TYPE_DECIMAL32>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL256, TYPE_DECIMAL64>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL256, TYPE_DECIMAL128I>>>();
    factory.register_function<FunctionMinus<MinusDecimalImpl<TYPE_DECIMAL256, TYPE_DECIMAL256>>>();

    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_TINYINT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_SMALLINT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_INT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_BIGINT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_LARGEINT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_FLOAT>>>();
    factory.register_function<FunctionMinus<MinusIntegralImpl<TYPE_DOUBLE>>>();
}
