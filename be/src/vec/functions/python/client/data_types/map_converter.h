...
/**
 * {@link MapConverter} is the specific implementation of {@link DataTypeConverter}, which is
 * responsible for converting between MAP<K,V> column data and Python objects.
 */
class MapConverter final : public DataTypeConverter {
public:
...
private:
    static Status generate_key_or_value_data_column(const vectorized::DataTypePtr& type,
                                                    vectorized::ColumnPtr& column);

    static Status convert_key_or_value_column_to_python_object(const vectorized::ColumnPtr& column,
                                                               const vectorized::DataTypePtr& type,
                                                               size_t row_index,
                                                               PyObject*& py_object);

    static Status add_to_key_or_value_column(vectorized::DataTypePtr& type,
                                             vectorized::ColumnPtr& column,
