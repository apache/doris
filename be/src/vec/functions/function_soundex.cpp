...
public:
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
            ...
            std::string word = std::string(ref.data, ref.size);
            std::string soundex_code = calculate_soundex(word);
...
private:
    std::string calculate_soundex(const std::string& word) const {
        if (word.empty()) {
            return "";
        }
        ...
        for (char c : word) {
            ...
            if (result.empty()) {
                result += c;
            } else if (std::isdigit(code) && code != pre_code) {
                result += code;
                if (result.size() == 4) {
                    return result;
                }
    ...
    char get_soundex_code(char c) const {
