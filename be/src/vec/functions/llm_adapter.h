...
public:
    ...
    // TODO: Add a function to handle common LLM generation parameters like `max_tokens`, `temperature`,
    //       `top_p`, `frequency_penalty`, etc. These parameters should be properly configured for
    //       each provider type and included in the request payload. This can be directly called
    //       in `execute_impl` to standardize parameter handling across different LLM providers.
...
// The OpenAI API format can be reused with some compatible LLMs. For format details, please refer to:
...
public:
    ...
    Status set_authentication(HttpClient* client) const override {
        client->set_authorization("Bearer " + _config.api_key);
        client->set_content_type("application/json");
...
public:
    ...
    Status set_authentication(HttpClient* client) const override {
        ...
        if (url.find('?') == std::string::npos) {
            url += "?key=" + _config.api_key;
        } else {
            url += "&key=" + _config.api_key;
        }

        Status status = client->init(url);
        if (!status.ok()) {
            return status;
        }
...
public:
    ...
    Status set_authentication(HttpClient* client) const override {
        ...
        client->set_header("anthropic-version", "2023-06-01");
    ...
    Status build_request_payload(const std::vector<std::string>& inputs,
        ...
        // TODO: when the extra parameters are supported, modify this together
        doc.AddMember("max_tokens", 4096, allocator);
