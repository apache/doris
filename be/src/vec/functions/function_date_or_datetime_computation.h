...
#define ADD_DATE_TIME_FUNCTION_IMPL(CLASS, NAME, UNIT)                                             \
...
ADD_DATE_TIME_FUNCTION_IMPL(AddMicrosecondsImpl, microseconds_add, MICROSECOND);
ADD_DATE_TIME_FUNCTION_IMPL(AddMillisecondsImpl, milliseconds_add, MILLISECOND);
ADD_DATE_TIME_FUNCTION_IMPL(AddSecondsImpl, seconds_add, SECOND);
ADD_DATE_TIME_FUNCTION_IMPL(AddMinutesImpl, minutes_add, MINUTE);
ADD_DATE_TIME_FUNCTION_IMPL(AddHoursImpl, hours_add, HOUR);
ADD_DATE_TIME_FUNCTION_IMPL(AddDaysImpl, days_add, DAY);
ADD_DATE_TIME_FUNCTION_IMPL(AddWeeksImpl, weeks_add, WEEK);
ADD_DATE_TIME_FUNCTION_IMPL(AddMonthsImpl, months_add, MONTH);
ADD_DATE_TIME_FUNCTION_IMPL(AddYearsImpl, years_add, YEAR);
    ...
    struct CLASS {                                                                                 \
        ...
        ReturnNativeType execute(const InputType1& arg1, const InputType2& arg2) {                 \
            if constexpr (PType == TYPE_DATETIMEV2) {                                              \
                ...
                TimeValue::TimeType tv2 = static_cast<TimeValue::TimeType>(arg2);                  \
                ...
                TimeValue::TimeType tv1 = static_cast<TimeValue::TimeType>(arg1);                  \
                TimeValue::TimeType tv2 = static_cast<TimeValue::TimeType>(arg2);                  \
                bool neg = std::string_view(name) == "sub_time";                                   \
                double res = TimeValue::limit_with_bound(neg ? tv1 - tv2 : tv1 + tv2);             \
                return res;                                                                        \
            } else {                                                                               \
                throw Exception(ErrorCode::INVALID_ARGUMENT, "not support type for function {}",   \
                                name);                                                             \
            }                                                                                      \
        }                                                                                          \
                                                                                                   \
