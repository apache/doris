la // Licensed to the Apache Software Foundation (ASF) under one
...
#include "common/compile_check_begin.h"
    ...
    struct RegexpPositionImpl {
        ...
        static void execute_impl(FunctionContext* context, ColumnPtr argument_columns[],
            ...
            for (size_t i = 0; i < input_rows_count; ++i) {
                ...
                Int64 start_pos = 1;
                if (start_str_col) {
                    auto s = start_str_col->get_data_at(i);
                    std::string tmp(s.data, s.size);
                    try {
                        start_pos = std::stoll(tmp);
                    } catch (...) {
                        result_data[i] = -1;
                        continue;
                    }
                ...
                if (!re) {
                    auto p = pattern_col->get_data_at(i);
                    std::string pat(p.data, p.size);
                    std::string err;
                    if (!StringFunctions::compile_regex(pat, &err, StringRef(), StringRef(),
                                                        scoped_re)) {
                        context->add_warning(err.c_str());
                        result_data[i] = -1;
                        continue;
                    }
                    re = scoped_re.get();
                ...
                auto s = str_col->get_data_at(i);
                re2::StringPiece hay(s.data, s.size);
                size_t off = static_cast<size_t>(start_pos - 1);
                if (off >= s.size) {
                    result_data[i] = -1;
                    continue;
                }
                ...
                if (!re->Match(hay, off, s.size, re2::RE2::UNANCHORED, &match, 1)) {
                    result_data[i] = -1;
                    continue;
                }

                result_data[i] = static_cast<Int64>(match.data() - s.data + 1);
            }
        }
    };
    ...
    struct RegexpReplaceImpl {
        ...
        static void execute_impl(FunctionContext* context, ColumnPtr argument_columns[],
            ...
            for (size_t i = 0; i < input_rows_count; ++i) {
                ...
                _execute_inner_loop<false>(context, str_col, pattern_col, replace_col,
    ...
    struct RegexpExtractImpl {
        ...
        static void _execute_inner_loop(FunctionContext* context, const ColumnString* str_col,
            ...
            if (index_data >= max_matches) {
                ReturnNull ? StringOP::push_null_string(index_now, result_data, result_offset,
                                                        null_map)
    ...
    struct RegexpExtractAllImpl {
        ...
        static void execute_impl(FunctionContext* context, ColumnPtr argument_columns[],
            ...
            for (int i = 0; i < input_rows_count; ++i) {
                ...
                _execute_inner_loop<false>(context, str_col, pattern_col, result_data,
                                           result_offset, null_map, i);
            }
        }
        ...
        static void _execute_inner_loop(FunctionContext* context, const ColumnString* str_col,
            ...
            res += "]";
            StringOP::push_value_string(std::string_view(res), index_now, result_data,
                                        result_offset);
        }
    };
    ...
    void register_function_regexp_extract(SimpleFunctionFactory & factory) {
        ...
        factory.register_function<FunctionVariadicAdaptor<RegexpPositionImpl, RegexpPositionImpl,
                                                          TwoParamTypes, ThreeParamTypes>>();
        // <<< Added
    }
