...
public:
    ...
    Status build_prompt(const Block& block, const ColumnNumbers& arguments, size_t row_num,
        ...
        int text_pos = arguments.size() - 2;

        // text
        const ColumnWithTypeAndName& text_column = block.get_by_position(arguments[text_pos]);
        std::string text_str;
        ...
        // target language
        const ColumnWithTypeAndName& lang_column = block.get_by_position(arguments[text_pos + 1]);
