...
public:
    ...
    Status init(const std::string& resource_name) {
        ...
        request.__set_function_name(this->get_name());
    ...
    Status init_from_resource(const Block& block, const ColumnNumbers& arguments,
        ...
        if (arguments.size() == assert_cast<const Derived&>(*this).max_args_num) {
            ...
            status = const_cast<Derived*>(assert_cast<const Derived*>(this))->init("");
        }
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        Status st = ThreadPoolBuilder("LLMRequestPool")
                            .set_min_threads(1)
                            .set_max_threads(config::llm_max_concurrent_requests)
                            .build(&thread_pool);
        if (!st.ok()) {
