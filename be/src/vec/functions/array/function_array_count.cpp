...
public:
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        if (check_column<ColumnUInt8>(*nested_column)) {
            return_column = _execute_number<ColumnUInt8>(offsets, nested_null_map, *nested_column,
                                                         array_null_map);
        } else if (check_column<ColumnInt8>(*nested_column)) {
            return_column = _execute_number<ColumnInt8>(offsets, nested_null_map, *nested_column,
                                                        array_null_map);
        } else if (check_column<ColumnInt16>(*nested_column)) {
            return_column = _execute_number<ColumnInt16>(offsets, nested_null_map, *nested_column,
                                                         array_null_map);
        } else if (check_column<ColumnInt32>(*nested_column)) {
            return_column = _execute_number<ColumnInt32>(offsets, nested_null_map, *nested_column,
                                                         array_null_map);
        } else if (check_column<ColumnInt64>(*nested_column)) {
            return_column = _execute_number<ColumnInt64>(offsets, nested_null_map, *nested_column,
                                                         array_null_map);
        } else if (check_column<ColumnInt128>(*nested_column)) {
            return_column = _execute_number<ColumnInt128>(offsets, nested_null_map, *nested_column,
                                                          array_null_map);
        }
...
private:
    ...
    ColumnPtr _execute_number(const ColumnArray::Offsets64& offsets, const UInt8* nested_null_map,
                              const IColumn& nested_column, const UInt8* outer_null_map) {
        ...
        return ColumnNullable::create(std::move(dst_column), std::move(dst_null_column));
    }
};
