...
private:
    ...
    ColumnPtr _apply_internal(const IColumn& src_column, const ColumnArray::Offsets64& src_offsets,
        ...
        const uint8_t* src_column_null_map_data = nullptr;
        if (!src_column.is_nullable()) {
            src_column_data_ptr = src_column.get_raw_data().data;
        } else if (const auto* nullable_column = check_and_get_column<ColumnNullable>(src_column)) {
            src_column_data_ptr = nullable_column->get_nested_column().get_raw_data().data;
            src_column_null_map_data = nullable_column->get_null_map_data().data();
        }
        ...
        for (size_t i = 0; i < src_column_size; ++i) {
            if (src_column_null_map_data && src_column_null_map_data[i]) {
                continue;
            }
