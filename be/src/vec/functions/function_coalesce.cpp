...
public:
    ...
    Status execute_impl(FunctionContext* context, Block& block, const ColumnNumbers& arguments,
        ...
        std::vector<int> record_idx(input_rows_count, 0); //used to save column idx, record the result data of each row from which column 
        std::vector<int> filled_flag(input_rows_count, 0); //used to save filled flag, in order to check current row whether have filled data
        ...
        for (size_t i = 0; i < argument_size && remaining_rows; ++i) {
            ...
            if (remaining_rows == 0) {
                ...
                for (size_t row = 0; row < input_rows_count; ++row) {
                    is_same_column = is_same_column & (record_idx[row] == record_idx[0]);
                }
    ...
    Status insert_result_data(MutableColumnPtr& result_column, ColumnPtr& argument_column,
                              UInt8* null_map_data, std::vector<int>& filled_flag,
                              size_t& input_rows_count) {
        auto* __restrict result_raw_data =
                reinterpret_cast<ColumnType*>(result_column.get())->get_data().data();
        auto* __restrict column_raw_data =
                reinterpret_cast<ColumnType*>(argument_column->assume_mutable().get())->get_data().data();
    ...
    Status filled_result_column(const DataTypePtr& data_type, MutableColumnPtr& result_column,
                                ColumnPtr& argument_column, UInt8* null_map_data,
                                std::vector<int>& filled_flag, size_t& input_rows_count) {
