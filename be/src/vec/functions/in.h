        ...
        if (in_state->use_set) {
            if (materialized_column->is_nullable()) {
                ...
                if (nested_col_ptr->is_column_string()) {
                    ...
                    search_hash_set_check_null(in_state, input_rows_count, vec_res, null_bitmap,
                                               column_string_ptr, col_const);
                } else {
                    //TODO: support other column type
                    search_hash_set_check_null(in_state, input_rows_count, vec_res, null_bitmap,
                                               nested_col_ptr, col_const);
                }
...
private:
    template <typename T>
    static ALWAYS_INLINE void search_hash_set_check_null(InState* in_state, size_t input_rows_count,
                                                         ColumnUInt8::Container& vec_res,
                                                         const ColumnUInt8::Container& null_bitmap,
                                                         T* col_ptr, bool col_const) {
        if (col_const) {
            const auto& ref_data = col_ptr->get_data_at(0);
            for (size_t i = 0; i < input_rows_count; ++i) {
                vec_res[i] = !null_bitmap[i] &&
    ...
    template <typename T>
    static ALWAYS_INLINE void search_hash_set(InState* in_state, size_t input_rows_count,
                                              ColumnUInt8::Container& vec_res, T* col_ptr,
                                              bool col_const) {
        if (col_const) {
            const auto& ref_data = col_ptr->get_data_at(0);
            for (size_t i = 0; i < input_rows_count; ++i) {
                vec_res[i] = in_state->hybrid_set->find((void*)ref_data.data, ref_data.size);
                if constexpr (negative) {
                    vec_res[i] = !vec_res[i];
                }
            }
    ...
    static ALWAYS_INLINE void impl_without_set(FunctionContext* context,
                                               const std::vector<ColumnPtr>& set_columns,
        ...
        for (size_t i = 0; i < input_rows_count; ++i) {
            ...
            std::unique_ptr<HybridSetBase> hybrid_set(create_set(context->get_arg_type(0)->type));
            bool null_in_set = false;
