...
public:
    ...
    FunctionLLMClassify() {
        Status status = init();
        if (!status.ok()) {
            throw Status::InternalError("Failed to initialize FunctionLLMClassify: " +
                                        status.to_string());
        }
    }
    ...
    Status build_prompt(const Block& block, const ColumnNumbers& arguments, size_t row_num,
        ...
        prompt = "Classify the text below into one of the following JSON encoded labels: " + labels_str + "\n"
