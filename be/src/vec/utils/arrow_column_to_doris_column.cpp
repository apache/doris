...
Status arrow_column_to_doris_column(const arrow::Array* arrow_column,
    ...
    if (doirs_column.column->is_nullable()) {
        auto* nullable_column = reinterpret_cast<vectorized::ColumnNullable*>((*std::move(doirs_column.column)).mutate().get());
    ...
    switch (arrow_column->type()->id())
    {
        ...
        case arrow::Type::BINARY:
            return convert_column_with_string_data(arrow_column, arrow_batch_cur_idx, data_column, num_elements);
        case arrow::Type::FIXED_SIZE_BINARY:
            return convert_column_with_fixed_size_data(arrow_column, arrow_batch_cur_idx, data_column, num_elements);
#    define DISPATCH(ARROW_NUMERIC_TYPE, CPP_NUMERIC_TYPE) \
        case ARROW_NUMERIC_TYPE: \
            return convert_column_with_numeric_data<CPP_NUMERIC_TYPE>(arrow_column, arrow_batch_cur_idx, data_column, num_elements);
        FOR_ARROW_NUMERIC_TYPES(DISPATCH)
#    undef DISPATCH
        case arrow::Type::BOOL:
            return convert_column_with_boolean_data(arrow_column, arrow_batch_cur_idx, data_column, num_elements);
        ...
        case arrow::Type::DECIMAL:
            return convert_column_with_decimal_data(arrow_column, arrow_batch_cur_idx, data_column, num_elements);
        default:
            break;
    }
    return Status::NotSupported(fmt::format(
        "Not support arrow type:{}", arrow_column->type()->name()));
}
} // namespace doris
