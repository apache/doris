...
Status AnnTopNDescriptor::prepare(RuntimeState* state, const RowDescriptor& row_desc) {
    ...
    */
    ...
    _dest_column_id = vir_slot_ref->column_id();
    ...
    _src_column_id = slot_ref->column_id();
...
Status AnnTopNDescriptor::evaluate_vector_ann_search(
        segment_v2::IndexIterator* ann_index_iterator, roaring::Roaring& row_bitmap,
    ...
    segment_v2::AnnIndexParam ann_query_params {
            ...
            .limit = _limit,
            .roaring = &row_bitmap,
            .distance = std::make_shared<std::vector<float>>(0),
    };
...
std::string AnnTopNDescriptor::debug_string() const {
    return "AnnTopNDescriptor: limit=" + std::to_string(_limit) +
           ", src_column_id=" + std::to_string(_src_column_id) +
           ", dest_column_id=" + std::to_string(_dest_column_id) +
