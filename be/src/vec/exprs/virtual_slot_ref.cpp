...
Status VirtualSlotRef::execute(VExprContext* context, Block* block, int* result_column_id) {
    ...
    const ColumnWithTypeAndName& column = block->get_by_position(_column_id);
    ...
    const vectorized::ColumnNothing* col_nothing =
            check_and_get_column<ColumnNothing>(column.column.get());
    if (this->_virtual_column_expr != nullptr) {
        if (col_nothing != nullptr) {
            ...
            VLOG_DEBUG << fmt::format(
                    "Virtual column materialization, slot_id {}, column_id {}, tmp_column_id {}, "
                    "column_name {}",
                    _slot_id, _column_id, tmp_column_id, *_column_name);
        }

#ifndef NDEBUG
        if (!_column_data_type->equals(*column.type)) {
            throw doris::Exception(doris::ErrorCode::FATAL_ERROR,
                                   ...
                                   _column_id, *_column_name, column.type->get_name(),
