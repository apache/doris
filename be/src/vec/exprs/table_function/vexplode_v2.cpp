...
void VExplodeV2TableFunction::get_same_many_values(MutableColumnPtr& column, int length) {
    ...
    if (_generate_row_index) {
        auto* nullable_column =
                assert_cast<ColumnNullable*>(struct_column->get_column(0).get_ptr().get());
        auto* pos_column =
                assert_cast<ColumnInt32*>(nullable_column->get_nested_column_ptr().get());
        nullable_column->get_null_map_column_ptr()->insert_many_defaults(length);
...
int VExplodeV2TableFunction::get_value(MutableColumnPtr& column, int max_step) {
    ...
    if (current_empty()) {
        ...
        if (_generate_row_index) {
            ...
            auto* pos_column =
                    assert_cast<ColumnInt32*>(nullable_column->get_nested_column_ptr().get());
            nullable_column->get_null_map_column_ptr()->insert_many_defaults(max_step);
