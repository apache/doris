...
Status VectorizedIfExpr::execute_column(VExprContext* context, const Block* block, size_t count,
    ...
    RETURN_IF_ERROR(_children[0]->execute_checked(context, block, count, cond_column));
    ...
    if (true_count == item_count) {
        RETURN_IF_ERROR(_children[1]->execute_checked(context, block, count, then_column));
        result_column = _data_type->is_nullable() ? make_nullable(then_column) : then_column;
        return Status::OK();
    } else if (true_count == 0) {
        RETURN_IF_ERROR(_children[2]->execute_checked(context, block, count, else_column));
...
Status VectorizedIfNullExpr::execute_column(VExprContext* context, const Block* block, size_t count,
    ...
    RETURN_IF_ERROR(_children[0]->execute_checked(context, block, count, first_column));
    ...
    if (first_column->only_null()) {
        RETURN_IF_ERROR(_children[1]->execute_checked(context, block, count, result_column));
        return Status::OK();
    }

    ColumnPtr second_column;
    RETURN_IF_ERROR(_children[1]->execute_checked(context, block, count, second_column));
...
Status VectorizedCoalesceExpr::execute_column(VExprContext* context, const Block* block,
    ...
    for (size_t i = 0; i < _children.size() && remaining_rows; ++i) {
        ...
        RETURN_IF_ERROR(_children[i]->execute_checked(context, block, count, original_columns[i]));
