...
Status CGroupUtil::find_global_cgroup(const string& subsystem, string* path) {
    ...
    while (true) {
        if (proc_cgroups.fail()) {
            return Status::IOError(strings::Substitute("Error reading /proc/self/cgroup: $0", get_str_err_msg()));
...
static Status unescape_path(const string& escaped, string* unescaped) {
    ...
    if (!CUnescape(escaped, unescaped, &err)) {
        return Status::InvalidArgument(strings::Substitute("Could not unescape path '$0': $1", escaped, err));
    }
    return Status::OK();
}
...
static Status read_cgroup_value(const string& limit_file_path, int64_t* val) {
    ...
    if (limit_file.fail() || limit_file.bad()) {
        return Status::IOError(strings::Substitute("Error reading $0: $1", limit_file_path, get_str_err_msg()));
    }
...
Status CGroupUtil::find_cgroup_mounts(
    ...
    while (true) {
        if (mountinfo.fail() || mountinfo.bad()) {
            return Status::IOError(strings::Substitute("Error reading /proc/self/mountinfo: $0", get_str_err_msg()));
        } else if (mountinfo.eof()) {
            return Status::NotFound(
                       strings::Substitute("Could not find subsystem $0 in /proc/self/mountinfo",
                                  subsystem));
        }
...
Status CGroupUtil::find_abs_cgroup_path(const string& subsystem, string* path) {
    ...
    if (path->compare(0, system_path.size(), system_path) != 0) {
        return Status::InvalidArgument(
                   strings::Substitute("Expected CGroup path '$0' to start with '$1'",
                              *path, system_path));
    }
...
std::string CGroupUtil::debug_string() {
    ...
    return strings::Substitute("Process CGroup Info: memory.limit_in_bytes=$0, cpu cfs limits: $1", mem_limit_str, cpu_limit_str);
}
