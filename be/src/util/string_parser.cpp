...
T StringParser::string_to_decimal(const char* __restrict s, int len, int type_precision,
    ...
    static_assert(std::is_same_v<T, int32_t> || std::is_same_v<T, int64_t> ||
                  ...
                  "wide::Int256.");
    const auto* orign_s = s;
    auto origin_len = len;
    // std::cout << fmt::format("casting \"{}\" to Decimal({}, {})\n",
    ...
    if (i != len) {
        ...
        if (s[i] == 'e' || s[i] == 'E') {
            ...
            if (i == len) {
                ...
                *result = StringParser::PARSE_FAILURE;
                std::cout << fmt::format(
                        "cast \"{}\" to Decimal({}, {}), invalid exponent format 1\n",
                        std::string(orign_s, origin_len), type_precision, type_scale);
                return 0;
            }
            for (; i != len; ++i) {
    ...
    // std::cout << fmt::format("cast \"{}\" to Decimal({}, {}), int part: {}, frac part: {}\n",
    //                          std::string(orign_s, origin_len), type_precision, type_scale,
    //                          int_part_number, frac_part_number);
