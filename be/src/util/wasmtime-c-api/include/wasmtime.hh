...
public:
    ...
    Span(Container<T, std::allocator<T>>& range) : ptr_ {range.data()}, size_ {range.size()} {}
...
public:
    ...
    void profiler(ProfilingStrategy profiler) {
        wasmtime_config_profiler_set(ptr.get(),
                                     static_cast<wasmtime_profiling_strategy_t>(profiler));
    }
...
class ExternRef {
    ...
    struct deleter {
        void operator()(wasmtime_externref_t* p) const { wasmtime_externref_delete(p); }
    };
...
public:
    ...
    explicit ExternRef(T val)
            : ExternRef(wasmtime_externref_new(std::make_unique<std::any>(std::move(val)).release(),
                                               finalizer)) {}
    ...
    std::any& data() { return *static_cast<std::any*>(wasmtime_externref_data(ptr.get())); }
...
struct WasmType<std::optional<ExternRef>> {
    ...
    static std::optional<ExternRef> load(Store::Context cx, wasmtime_val_raw_t* p) {
        ...
        return ExternRef(wasmtime_externref_from_raw(cx.raw_context(), p->externref));
    }
};
...
struct WasmHostFunc<R (*)(A...)> {
    ...
    static std::optional<Trap> invoke(F& f, Caller cx, wasmtime_val_raw_t* raw) {
        ...
        return std::apply(
                [&](const auto&... val) { return WasmHostRet<R>::invoke(f, cx, raw, val...); },
                params);
    }
};
...
struct WasmHostFunc<R (*)(Caller, A...)> : public WasmHostFunc<R (*)(A...)> {
    ...
    static std::optional<Trap> invoke(F& f, Caller cx, wasmtime_val_raw_t* raw) {
        ...
        return std::apply(
                [&](const auto&... val) { return WasmHostRet<R>::invoke(f, cx, raw, cx, val...); },
                params);
    }
};
...
public:
   ...
   */
    template <typename F,
              std::enable_if_t<std::is_invocable_r_v<Result<std::monostate, Trap>, F, Caller,
...
public:
   ...
   */
    TrapResult<Results> call(Store::Context cx, Params params) const {
        ...
        wasm_trap_t* trap = nullptr;
        auto* error = wasmtime_func_call_unchecked(cx.raw_context(), &f.func, storage.data(),
                                                   storage.size(), &trap);
        if (error != nullptr) {
            return TrapError(Error(error));
        }
...
public:
   ...
   */
    ...
    size_t size(Store::Context cx) const { return wasmtime_table_size(cx.ptr, &table); }
...
public:
    ...
    Result<std::monostate> module(Store::Context cx, std::string_view name, const Module& m) {
        auto* error =
                wasmtime_linker_module(ptr.get(), cx.ptr, name.data(), name.size(), m.ptr.get());
        if (error != nullptr) {
            return Error(error);
        }
        return std::monostate();
    ...
    Result<std::monostate> func_wrap(std::string_view module, std::string_view name, F f) {
        ...
        auto* error = wasmtime_linker_define_func_unchecked(
                ptr.get(), module.data(), module.length(), name.data(), name.length(), ty.ptr.get(),
                Func::raw_callback_unchecked<F>, std::make_unique<F>(f).release(),
                Func::raw_finalize<F>);
    ...
    Result<Func> get_default(Store::Context cx, std::string_view name) {
        ...
        auto* error =
                wasmtime_linker_get_default(ptr.get(), cx.ptr, name.data(), name.size(), &item);
        if (error != nullptr) {
            return Error(error);
        }
        return Func(item);
