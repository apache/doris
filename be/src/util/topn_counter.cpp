...
void TopNCounter::serialize(std::string* buffer) {
    ...
    for (std::vector<Counter>::const_iterator it = _counter_vec->begin(); it != _counter_vec->end();
         ++it) {
...
bool TopNCounter::deserialize(const doris::Slice& src) {
    ...
    for (int i = 0; i < topn_counter.counter_size(); ++i) {
        ...
        _counter_map->insert(
                std::make_pair(counter.item(), Counter(counter.item(), counter.count())));
...
void TopNCounter::sort_retain(uint32_t capacity, std::vector<Counter>* sort_vec) {
    for (std::unordered_map<std::string, Counter>::const_iterator it = _counter_map->begin();
         it != _counter_map->end(); ++it) {
...
void TopNCounter::merge(doris::TopNCounter&& other) {
    ...
    for (auto& other_entry : *(other._counter_map)) {
        ...
        if (itr != _counter_map->end()) {
            ...
            this->_counter_map->insert(std::make_pair(
                    other_entry.first,
...
void TopNCounter::finalize(std::string& finalize_str) {
    ...
    uint32_t k = 0;
    writer.StartObject();
    for (std::vector<Counter>::const_iterator it = _counter_vec->begin();
         it != _counter_vec->end() && k < _top_num; ++it, ++k) {
