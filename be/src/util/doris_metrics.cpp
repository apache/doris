...
void DorisMetrics::initialize(const std::string& name, const std::vector<std::string>& paths,
                              bool init_system_metrics, const std::set<std::string>& disk_devices,
...
#define REGISTER_ENGINE_REQUEST_METRIC(type, status, metric) \
    _metrics->register_metric("engine_requests_total",       \
                              MetricLabels().add("type", #type).add("status", #status), &metric)
    ...
    _metrics->register_metric("segment_read",
                              MetricLabels().add("type", "segment_total_read_times"),
                              &segment_read_total);
    _metrics->register_metric("segment_read", MetricLabels().add("type", "segment_total_row_num"),
                              &segment_row_total);
    _metrics->register_metric("segment_read",
                              MetricLabels().add("type", "segment_rows_by_short_key"),
                              &segment_rows_by_short_key);
    _metrics->register_metric("segment_read",
                              MetricLabels().add("type", "segment_rows_read_by_zone_map"),
    ...
    _metrics->register_metric("txn_request", MetricLabels().add("type", "exec"),
                              &txn_exec_plan_total);
    ...
    for (auto& path : paths) {
        ...
        _metrics->register_metric("disks_data_used_capacity", MetricLabels().add("path", path),
                                  gauge);
