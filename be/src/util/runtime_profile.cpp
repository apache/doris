...
void RuntimeProfile::update(const std::vector<PRuntimeProfileNode>& nodes, int* idx) {
    ...
    ++*idx;
    {
        ...
        for (int i = 0; i < node.num_children(); ++i) {
            ...
            ChildMap::iterator j = _child_map.find(pchild.name());
            RuntimeProfile* child = nullptr;
            ...
            if (j != _child_map.end()) {
                ...
                _children.push_back(std::make_pair(child, pchild.indent()));
            }
...
void RuntimeProfile::to_proto(PRuntimeProfileTree* tree) {
    tree->clear_nodes();
    to_proto(tree->mutable_nodes());
}

void RuntimeProfile::to_proto(
        google::protobuf::RepeatedPtrField<PRuntimeProfileNode>* nodes) const {
    PRuntimeProfileNode* node = nodes->Add();
    node->set_name(_name);
    node->set_num_children(_children.size());
    ...
    CounterMap counter_map;
    {
        std::lock_guard<std::mutex> l(_counter_map_lock);
        counter_map = _counter_map;
        PChildCountersName p_child_counter;
        for (auto& child_counter : _child_counter_map) {
            p_child_counter.clear_child_counters_name();
            *p_child_counter.mutable_child_counters_name() = {child_counter.second.begin(),
                                                              child_counter.second.end()};
            node->mutable_child_counters_map()->insert({child_counter.first, p_child_counter});
        }
    }

    for (std::map<std::string, Counter*>::const_iterator iter = counter_map.begin();
         iter != counter_map.end(); ++iter) {
        PProfileCounter* counter = node->add_counters();
        counter->set_name(iter->first);
        counter->set_value(iter->second->value());
        counter->set_type(static_cast<PUnit>(iter->second->type()));
    }

    {
        std::lock_guard<std::mutex> l(_info_strings_lock);
        *node->mutable_info_strings() = {_info_strings.begin(), _info_strings.end()};
        *node->mutable_info_strings_display_order() = {_info_strings_display_order.begin(),
                                                       _info_strings_display_order.end()};
    }
    ...
    for (int i = 0; i < children.size(); ++i) {
        ...
        children[i].first->to_proto(nodes);
        // fix up indentation flag
        (*nodes)[child_idx].set_indent(children[i].second);
    }
}
