...
public:
    ...
    Status compress(const Slice& input, Slice* output) const override {
        ...
        if (LZ4F_isError(compressed_len)) {
            return Status::InvalidArgument(Substitute("Fail to do LZ4F compress frame, msg=$0",
...
private:
    Status _compress(LZ4F_compressionContext_t ctx, const std::vector<Slice>& inputs,
        ...
        if (LZ4F_isError(wbytes)) {
            return Status::InvalidArgument(Substitute("Fail to do LZ4F compress begin, res=$0",
        ...
        for (auto input : inputs) {
            ...
            if (LZ4F_isError(wbytes)) {
                return Status::InvalidArgument(Substitute("Fail to do LZ4F compress update, res=$0",
                                                          LZ4F_getErrorName(wbytes)));
            }
            offset += wbytes;
        }
    ...
    Status _decompress(LZ4F_decompressionContext_t ctx, const Slice& input, Slice* output) const {
        ...
        if (LZ4F_isError(lres)) {
            ...
            return Status::InvalidArgument(
                    Substitute("Fail to do LZ4F decompress: trailing data left in compressed data, "
                               "read=$0 vs given=$1",
