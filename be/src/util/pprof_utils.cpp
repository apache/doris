...
Status PprofUtils::get_readable_profile(const std::string& file_or_content, bool is_file,
    ...
    // delete raw file
    RETURN_IF_ERROR(io::global_local_filesystem()->delete_file(file_or_content));
...
Status PprofUtils::generate_flamegraph(int32_t sample_seconds,
    ...
    if (!rc) {
        RETURN_IF_ERROR(io::global_local_filesystem()->delete_file(tmp_file.str()));
    ...
    if (return_file) {
        ...
        if (!rc) {
            RETURN_IF_ERROR(io::global_local_filesystem()->delete_file(tmp_file.str()));
            RETURN_IF_ERROR(io::global_local_filesystem()->delete_file(graph_file.str()));
