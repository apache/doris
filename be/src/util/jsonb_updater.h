...
class JsonbUpdater {
 public:
  ...
  JsonbErrType pushPathKey(int idx){
    ...
    if(nullptr == path_node_.back().jsonb_value ||
       !path_node_.back().jsonb_value->isArray()){
  ...
  JsonbErrType pushPathKey(const char* key,
                           unsigned int klen,
                           hDictFind handler = nullptr){
    // Check node validity
    if(nullptr == key){
  ...
  // Update a field
  JsonbErrType updateValue(const JsonbValue *value){
  ...
  JsonbErrType appendValue(const JsonbValue *value){
    ...
    return insertArrayInternal(
      ((ArrayVal*)path_node_.back().jsonb_value)->end(),
      (const char*)value,
  ...
  JsonbErrType insertValue(int idx,
                           ArrayVal::const_iterator beg,
                           ArrayVal::const_iterator end){
    ArrayVal::iterator ite;
    JsonbErrType ret = getElemInArray(idx,
                                      path_node_.back().jsonb_value,
                                      ite);
    if(JsonbErrType::E_NONE != ret)
      return ret;
  ...
  JsonbErrType insertValue(ObjectVal::const_iterator beg,
    ...
    ObjectVal::const_iterator obj_end =
      ((ObjectVal*)path_node_.back().jsonb_value)->end();
 ...
 private:
  ...
  JsonbErrType insertArrayInternal(ArrayVal::iterator ite,
                                   const char *val_beg,
                                   const char *val_end){
 ...
 private:
  JsonbErrType getElemInArray(int idx,
                              JsonbValue *array,
                              ArrayVal::iterator &ite){
    if(nullptr == array ||
       !array->isArray())
  ...
  void updatePackageSize(int inc_size){
    ...
    */
    for(auto ite = path_node_.begin();
      ...
      if(value->type() >= JsonbType::T_Null &&
         value->type() <= JsonbType::T_Double){
        ...
        uint32_t *size_pointer =
          (uint32_t*)((char*)value + sizeof(JsonbTypeUnder));
