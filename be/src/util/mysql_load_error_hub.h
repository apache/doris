...
public:
    struct MysqlInfo {
        ...
        MysqlInfo(const std::string& h, int32_t p, const std::string& u,
                   const std::string& pwd, const std::string& d, const std::string& t) :
            host(h), port(p), user(u), passwd(pwd), db(d), table(t) {
        }

        MysqlInfo(const TMysqlErrorHubInfo& t_info) :
                host(t_info.host), port(t_info.port),
                user(t_info.user), passwd(t_info.passwd),
                db(t_info.db), table(t_info.table) {
        }
    };
