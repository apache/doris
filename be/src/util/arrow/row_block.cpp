...
Status convert_to_doris_schema(const arrow::Schema& schema, std::shared_ptr<Schema>* result) {
    ...
    auto doris_schema = new Schema();
    if (doris_schema->init(columns, col_ids) != OLAP_SUCCESS) {
        return Status::InternalError("Failed to convert to doris schema");
    }
    result->reset(doris_schema);
    return Status::OK();
}
