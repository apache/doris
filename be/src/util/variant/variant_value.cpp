...
VariantBasicType VariantValueRef::basic_type() const {
    require_bytes(size, 1, "header");
    return static_cast<VariantBasicType>(static_cast<uint8_t>(data[0]) & VARIANT_BASIC_TYPE_MASK);
}

VariantPrimitiveId VariantValueRef::primitive_id() const {
    ...
    const uint8_t id = static_cast<uint8_t>(data[0]) >> VARIANT_VALUE_HEADER_SHIFT;
...
size_t VariantValueRef::value_size() const {
    const VariantBasicType type = basic_type();
    if (type == VariantBasicType::SHORT_STRING) {
        const size_t string_size = static_cast<uint8_t>(data[0]) >> VARIANT_VALUE_HEADER_SHIFT;
        require_bytes(size - 1, string_size, "short string");
        return 1 + string_size;
    }
    if (type == VariantBasicType::PRIMITIVE) {
        const size_t payload_size = primitive_payload_size(primitive_id(), data + 1, size - 1);
        require_bytes(size - 1, payload_size, "primitive payload");
...
int64_t VariantValueRef::get_int() const {
    const VariantPrimitiveId id = primitive_id();
    uint8_t width = 0;
    switch (id) {
    case VariantPrimitiveId::INT8:
    ...
    require_bytes(size - 1, width, "integer");
    return read_signed(data + 1, width);
}

float VariantValueRef::get_float() const {
    require_primitive(primitive_id(), VariantPrimitiveId::FLOAT, "float");
    require_bytes(size - 1, sizeof(uint32_t), "float");
    return std::bit_cast<float>(static_cast<uint32_t>(read_unsigned(data + 1, sizeof(uint32_t))));
}

double VariantValueRef::get_double() const {
    require_primitive(primitive_id(), VariantPrimitiveId::DOUBLE, "double");
    require_bytes(size - 1, sizeof(uint64_t), "double");
    return std::bit_cast<double>(read_unsigned(data + 1, sizeof(uint64_t)));
}

VariantDecimal VariantValueRef::get_decimal() const {
    const VariantPrimitiveId id = primitive_id();
    uint8_t width = 0;
    switch (id) {
    case VariantPrimitiveId::DECIMAL4:
    ...
    require_bytes(size - 1, static_cast<size_t>(width) + 1, "decimal");
    const auto scale = static_cast<uint8_t>(data[1]);
    if (scale > 38) {
        throw Exception(ErrorCode::CORRUPTION, "Invalid Variant decimal scale {}", scale);
    }
    return {.unscaled = read_signed_128(data + 2, width), .scale = scale, .width = width};
}

int32_t VariantValueRef::get_date() const {
    require_primitive(primitive_id(), VariantPrimitiveId::DATE, "date");
    require_bytes(size - 1, sizeof(uint32_t), "date");
    return static_cast<int32_t>(read_signed(data + 1, sizeof(uint32_t)));
}

int64_t VariantValueRef::get_timestamp_micros() const {
    require_primitive(primitive_id(), VariantPrimitiveId::TIMESTAMP_MICROS, "timestamp micros");
    require_bytes(size - 1, sizeof(uint64_t), "timestamp micros");
    return read_signed(data + 1, sizeof(uint64_t));
}

int64_t VariantValueRef::get_timestamp_ntz_micros() const {
    require_primitive(primitive_id(), VariantPrimitiveId::TIMESTAMP_NTZ_MICROS,
                      "timestamp NTZ micros");
    require_bytes(size - 1, sizeof(uint64_t), "timestamp NTZ micros");
    return read_signed(data + 1, sizeof(uint64_t));
}

int64_t VariantValueRef::get_time_ntz_micros() const {
    require_primitive(primitive_id(), VariantPrimitiveId::TIME_NTZ_MICROS, "time NTZ micros");
    require_bytes(size - 1, sizeof(uint64_t), "time NTZ micros");
    return read_signed(data + 1, sizeof(uint64_t));
}

int64_t VariantValueRef::get_timestamp_nanos() const {
    require_primitive(primitive_id(), VariantPrimitiveId::TIMESTAMP_NANOS, "timestamp nanos");
    require_bytes(size - 1, sizeof(uint64_t), "timestamp nanos");
    return read_signed(data + 1, sizeof(uint64_t));
}

int64_t VariantValueRef::get_timestamp_ntz_nanos() const {
    require_primitive(primitive_id(), VariantPrimitiveId::TIMESTAMP_NTZ_NANOS,
                      "timestamp NTZ nanos");
    require_bytes(size - 1, sizeof(uint64_t), "timestamp NTZ nanos");
    return read_signed(data + 1, sizeof(uint64_t));
}

StringRef VariantValueRef::get_binary() const {
    require_primitive(primitive_id(), VariantPrimitiveId::BINARY, "binary");
    const size_t total_size = value_size();
    return {data + 1 + sizeof(uint32_t), total_size - 1 - sizeof(uint32_t)};
}

StringRef VariantValueRef::get_string() const {
    if (basic_type() == VariantBasicType::SHORT_STRING) {
        const size_t total_size = value_size();
        return {data + 1, total_size - 1};
    }
...
std::array<uint8_t, 16> VariantValueRef::get_uuid() const {
    require_primitive(primitive_id(), VariantPrimitiveId::UUID, "UUID");
    require_bytes(size - 1, 16, "UUID");
    std::array<uint8_t, 16> result {};
    std::transform(data + 1, data + 17, result.begin(),
                   [](char byte) { return static_cast<uint8_t>(byte); });
    return result;
}

VariantValueRef::ContainerLayout VariantValueRef::_container_layout(
        VariantBasicType expected_type) const {
    ...
    const uint8_t value_header = static_cast<uint8_t>(data[0]) >> VARIANT_VALUE_HEADER_SHIFT;
    ...
    require_bytes(size - 1, count_width, "container element count");
    const auto count = static_cast<uint32_t>(read_unsigned(data + 1, count_width));

    const size_t ids_offset = 1 + count_width;
    if (id_width != 0 && count > (size - ids_offset) / id_width) {
    ...
    if (static_cast<uint64_t>(count) + 1 > (size - offsets_offset) / offset_width) {
    ...
    if (!is_object) {
        const auto first_offset =
                static_cast<uint32_t>(read_unsigned(data + offsets_offset, offset_width));
        if (first_offset != 0) {
    ...
    const auto values_size = static_cast<uint32_t>(read_unsigned(
            data + offsets_offset + static_cast<size_t>(count) * offset_width, offset_width));
    require_bytes(size - values_offset, values_size, "container values");
...
uint32_t VariantValueRef::num_elements() const {
    const VariantBasicType type = basic_type();
...
uint32_t VariantValueRef::_object_field_id(const ContainerLayout& layout, uint32_t index) const {
    if (index >= layout.count) {
        throw Exception(ErrorCode::INVALID_ARGUMENT,
                        "Variant object index {} is out of range [0, {})", index, layout.count);
    }
    const auto field_id = static_cast<uint32_t>(
            read_unsigned(data + layout.ids_offset + static_cast<size_t>(index) * layout.id_width,
                          layout.id_width));
...
VariantValueRef VariantValueRef::_container_value_at(const ContainerLayout& layout, uint32_t index,
    ...
    const auto offset = static_cast<uint32_t>(read_unsigned(
            data + layout.offsets_offset + static_cast<size_t>(index) * layout.offset_width,
            layout.offset_width));
    ...
    VariantValueRef child {.metadata = metadata,
                           .data = data + layout.values_offset + offset,
                           .size = layout.values_size - offset};
    const size_t child_size = child.value_size();
    if (require_array_boundary) {
        const auto next_offset = static_cast<uint32_t>(
                read_unsigned(data + layout.offsets_offset +
...
VariantValueRef VariantValueRef::object_value_at(uint32_t index, uint32_t* field_id_out) const {
...
VariantValueRef VariantValueRef::array_at(uint32_t index) const {
    return _container_value_at(_container_layout(VariantBasicType::ARRAY), index, true);
}

bool VariantValueRef::object_find(StringRef key, VariantValueRef* out) const {
    if (out == nullptr) {
...
bool VariantValueRef::object_find_by_id(uint32_t field_id, VariantValueRef* out) const {
    if (out == nullptr) {
        throw Exception(ErrorCode::INVALID_ARGUMENT, "Variant object lookup output is null");
    }
...
bool VariantValueRef::_object_find_by_id(const ContainerLayout& layout, uint32_t field_id,
