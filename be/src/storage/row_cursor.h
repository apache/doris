...
public:
    ...
    Status init(ReadSchemaSPtr schema, const OlapTuple& tuple);
    Status init(ReadSchemaSPtr schema, std::vector<Field>&& fields);

    // Initialize from typed Fields directly.
    Status init_scan_key(TabletSchemaSPtr schema, std::vector<Field> fields);

    static ReadSchemaSPtr create_shared_schema(const TabletSchemaSPtr& schema,
...
struct PointKeySet {
    explicit PointKeySet(ReadSchemaSPtr key_schema) : schema(std::move(key_schema)) {}
