...
std::string resolved_default_value(const TabletColumn& column, int64_t timestamp_ms,
    ...
    if (UNLIKELY(column.type() == FieldType::OLAP_FIELD_TYPE_TIMESTAMP_NS &&
                 to_lower(column.has_default_value_expr() ? column.default_value_expr()
                                                          : column.default_value())
                                 .find(to_lower("CURRENT_TIMESTAMP")) != std::string::npos)) {
        ...
        if (pos == std::string::npos) {
            ...
            // FE validates CURRENT_TIMESTAMP precision against the nanosecond range.
            DORIS_CHECK(precision >= 0 && precision <= 9);
            dtv.from_unixtime(timestamp_ms / 1000, nano_seconds, timezone, precision);
            if (precision > 6) {
                const auto factor = static_cast<int64_t>(int_exp10(9 - precision));
                // DORIS_CHECK above makes factor 1, 10 or 100; the analyzer cannot see
                // that its out-of-line failure handler terminates this path.
...
Status PartialUpdateInfo::init(int64_t tablet_id, int64_t txn_id, const TabletSchema& tablet_schema,
    ...
    if (tablet_schema.has_ttl_col() && tablet_schema.column(tablet_schema.ttl_col_idx()).type() !=
...
Status BlockAggregator::aggregate_for_flexible_partial_update(
    ...
    if (_tablet_schema.has_ttl_col() && row_ttl_uses_source_time(_tablet_schema)) {
        auto& skip_bitmaps =
                get_mutable_skip_bitmap_column(block, _tablet_schema.skip_bitmap_col_idx())
                        ->get_data();
        const int32_t source_uid = _partial_update_info.row_ttl_source_uid();
