    ...
    NP parse_group() {
        ...
        if (peek() == '?') {
            ...
            if (peek() == ':') {
                ...
                // `(?<=` and `(?<!` are lookbehind assertions, which Boost supports when
                // enable_extended_regex is on. They must not be mistaken for a named group: the
                // scan below stops at the first '>', and a '>' inside the assertion body would
                // leave the rest of that body to be parsed as an ordinary consuming
                // subexpression. A lookbehind consumes nothing, so its body would become a
                // mandatory literal and prune rows the engine really matches -- for example
                // `aq>x(?<=q>x)y` would drop the row `aq>xy`. Give up on the whole pattern
                // instead; the caller turns a parse failure into ALL, which filters nothing.
                if (i + 1 < p.size() && (p[i + 1] == '=' || p[i + 1] == '!')) {
                    ok = false;
                    err = "lookbehind is not supported";
                    return nullptr;
                }
                while (!eof() && peek() != '>') {
                    i++;
