...
public:
    ...
    // Total number of gram leaves the whole compiled tree may carry. kMaxSet bounds a single
    // set, which does not bound the tree: a wide alternation can multiply several bounded sets
    // into a query with more than a hundred leaves. Every leaf costs at least one term dictionary
    // lookup and one posting read, and on object storage each of those is a separate remote
...
private:
    ...
    // compile entry points, so no caller can be handed a query whose index IO outweighs the scan.
    static void apply_gram_budget(GramQuery* q);
