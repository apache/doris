...
public:
    ...
    // Return the original relative byte boundaries when token normalization changed them.
    virtual std::span<const int32_t> get_source_byte_offsets(std::string_view term) const {
        return {};
    }
};
