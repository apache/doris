...
public:
    ...
    void add_many(std::span<const uint32_t> docids);
...
public:
    ...
    // Exact pre-allocation charge for CRoaring::readSafe, excluding the caller's
    // framed input bytes and decoded docid output vector.
    static Status decoded_memory_bytes(Slice framed, uint64_t* out);
    ...
    // Appends decoded docids in ascending order without cloning the bitmap.
    void append_docids(std::vector<uint32_t>& out) const;
