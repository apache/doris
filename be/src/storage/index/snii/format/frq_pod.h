...
// .frq region codec (FrqPod): doc-delta (dd) postings encoded with PFOR.
//
// Windowed payloads are not self-describing and use [prelude][dd-block]. The
// dd-block concatenates every window's dd_region. Each region is encoded raw or
// with zstd, and its codec metadata is stored in the prelude. Region bytes carry
// no header, so the whole payload is one contiguous range.
//
...
// PFOR runs are segmented at 256 docs (kFrqBaseUnit); a partial segment writes
// the remainder. Variable-length integers reuse
...
// Codec metadata for one encoded dd region, hoisted into the prelude.
...
// oversized count all return a non-OK Status.
