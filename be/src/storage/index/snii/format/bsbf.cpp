...
Status BsbfBuilder::create(uint32_t ndv, double fpp, BsbfBuilder* out) {
    if (out == nullptr) return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bsbf: null out");
    if (!(fpp > 0.0 && fpp < 1.0))
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bsbf: fpp out of (0,1)");
    if (ndv == 0) ndv = 1;
...
Status BsbfBuilder::serialize(ByteSink* sink) const {
    if (sink == nullptr)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bsbf: null sink");
    if (num_bytes_ == 0)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bsbf: not built");
...
Status BsbfHeader::parse(Slice h, uint64_t section_base, BsbfHeader* out) {
    ...
    if (h.size() < kBsbfHeaderSize)
        return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>("bsbf: short header");
    const uint8_t* p = h.data();
    if (p[0] != 'B' || p[1] != 'S' || p[2] != 'B' || p[3] != 'F')
        return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>("bsbf: bad magic");
    if (p[4] != 1)
        return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>("bsbf: bad version");
    if (p[5] != 0)
...
Status bsbf_probe(io::FileReader* reader, const BsbfHeader& header, uint64_t hash,
                  bool* maybe_present) {
    if (reader == nullptr || maybe_present == nullptr)
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bsbf: null arg");
