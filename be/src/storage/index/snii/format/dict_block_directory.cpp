...
Status decode_payload(Slice payload, std::vector<BlockRef>* refs) {
    ...
    for (uint32_t i = 0; i < n_blocks; ++i) {
        ...
        if (ref.length == 0) {
            return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>(
                    "dict_block_directory: zero-length block");
        }
        if (ref.length > std::numeric_limits<uint64_t>::max() - ref.offset) {
            return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>(
                    "dict_block_directory: block range end overflow");
        }
        const uint64_t block_end = ref.offset + ref.length;
        if (i != 0 && ref.offset < previous_end) {
            return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>(
                    "dict_block_directory: blocks overlap or are out of physical order");
        }
...
Status DictBlockDirectoryReader::open(Slice section, DictBlockDirectoryReader* out) {
    ...
    if (!src.eof()) {
        return Status::Error<ErrorCode::INVERTED_INDEX_FILE_CORRUPTED, false>(
                "dict_block_directory: trailing framed section bytes");
    }
