...
void DictBlockBuilder::encode_covered(ByteSink* sink) const {
    // header.
    ...
    ByteSink entry_body_scratch;
    std::string_view prev;
    for (uint32_t i = 0; i < n_entries_; ++i) {
        const bool anchor = is_anchor(i);
        if (anchor) {
            anchor_offsets.push_back(static_cast<uint32_t>(sink->size()));
        }
    ...
    for (uint32_t off : anchor_offsets) {
        sink->put_fixed32(off);
    }
    sink->put_fixed32(static_cast<uint32_t>(anchor_offsets.size()));
}
...
std::vector<uint8_t> DictBlockBuilder::finish_owned() const {
    ByteSink block;
    encode_covered(&block);
    const uint32_t checksum = crc32c(block.view());
    block.reserve(kFooterBytes);
    block.put_fixed32(checksum);
    return block.take();
}
...
Status DictBlockReader::visit_term_range(std::string_view lower_inclusive,
                                         std::optional<std::string_view> upper_exclusive,
                                         const std::function<bool(std::string_view)>& accept_key,
                                         const std::function<Status(DictEntry&&, bool*)>& on_hit,
                                         bool* range_exhausted) const {
    if (!on_hit || range_exhausted == nullptr) {
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>(
                "dict_block: null visit_term_range args");
    }
    *range_exhausted = false;
    ...
    if (!lower_inclusive.empty()) {
        locate_anchor(lower_inclusive, &anchor_idx);
    }
    const size_t seg_begin = anchor_offsets_[anchor_idx];
    const size_t entries_end =
            block_.size() - kNAnchorsBytes - anchor_offsets_.size() * kAnchorOffBytes;
    ...
    while (!src.eof()) {
        ...
        size_t body_start = 0;
        uint64_t entry_total = 0;
        RETURN_IF_ERROR(decode_dict_entry_key(&src, std::string_view(prev), &entry, &body_start,
                                              &entry_total));
        const std::string_view term(entry.term);
        if (term < lower_inclusive) {
            RETURN_IF_ERROR(skip_dict_entry_body(&src, body_start, entry_total));
            prev = std::move(entry.term);
            continue;
        }
        if (upper_exclusive.has_value() && term >= *upper_exclusive) {
            *range_exhausted = true;
            return Status::OK();
        }
        if (accept_key && !accept_key(term)) {
            RETURN_IF_ERROR(skip_dict_entry_body(&src, body_start, entry_total));
            prev = std::move(entry.term);
            continue;
        }
