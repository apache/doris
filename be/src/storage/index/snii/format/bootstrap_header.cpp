...
Status encode_bootstrap_header(const BootstrapHeader& header, ByteSink* sink) {
    if (sink == nullptr) {
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bootstrap_header: null sink");
    }
    ByteSink fields;
...
Status decode_bootstrap_header(Slice data, BootstrapHeader* out) {
    if (out == nullptr) {
        return Status::Error<ErrorCode::INVALID_ARGUMENT, false>("bootstrap_header: null out");
    }
