...
Status decode_leaf_head(ByteSource* src, uint32_t bytes_per_dim, uint32_t expected_point_count,
    ...
    if (point_count == 0) {
        return leaf_codec_corrupted("leaf carries no points");
    }

    uint8_t raw_mode = 0;
    RETURN_IF_ERROR(src->get_u8(&raw_mode));
    if (raw_mode > static_cast<uint8_t>(kMaxLeafValueMode)) {
        return leaf_codec_corrupted("unknown value_mode");
    }
    ...
    if (common_prefix_len > bytes_per_dim) {
        return leaf_codec_corrupted("common_prefix_len exceeds bytes_per_dim");
    }
    Slice common_prefix;
    ...
    if ((value_mode == LeafValueMode::kAllEqual) != (suffix_width == 0)) {
        return leaf_codec_corrupted("value_mode disagrees with common_prefix_len");
    }
...
Status decode_leaf_tail(Slice block, uint32_t* docid_block_offset, size_t* docid_block_end) {
    if (block.empty()) {
        return leaf_codec_corrupted("leaf block is empty");
    }
    ...
    if (length == 0 || length > kMaxVarint32Bytes ||
        ...
        return leaf_codec_corrupted("docid_block_offset length byte is out of range");
    }
    ...
    if (!offset_src.eof()) {
        ...
        return leaf_codec_corrupted("docid_block_offset is not exactly its declared length");
    }
    if (*docid_block_offset > offset_start) {
        return leaf_codec_corrupted("docid_block_offset points past the leaf tail");
    }
...
Status read_value_area(ByteSource* src, const LeafHead& head, std::vector<LeafValueRun>* runs) {
    ...
    if (head.value_mode == LeafValueMode::kRle) {
        ...
        if (run_count == 0 || run_count > head.point_count) {
            return leaf_codec_corrupted("run_count is out of range");
        }
        ...
        if (run_count > src->remaining() / (static_cast<size_t>(head.suffix_width) + 1)) {
            return leaf_codec_corrupted("run count exceeds the bytes remaining in the leaf");
        }
        ...
        for (uint32_t run = 0; run < run_count; ++run) {
            ...
            if (run_length == 0) {
                return leaf_codec_corrupted("run_len is zero");
            }
            ...
            if (run != 0 && std::memcmp(previous.data(), suffix.data(), head.suffix_width) >= 0) {
                return leaf_codec_corrupted("run suffixes are not strictly ascending");
            }
            if (run_length > head.point_count - covered) {
                return leaf_codec_corrupted("run lengths overrun point_count");
            }
        ...
        if (covered != head.point_count) {
            return leaf_codec_corrupted("run lengths do not sum to point_count");
        }
        return Status::OK();
    }
    ...
    for (uint32_t point = 0; point < head.point_count; ++point) {
        ...
        if (point != 0 && std::memcmp(previous.data(), suffix.data(), head.suffix_width) > 0) {
            return leaf_codec_corrupted("suffixes are not ascending");
        }
...
Status decode_doc_id_block(Slice region, const LeafHead& head, std::span<const LeafValueRun> runs,
    ...
    if (!src.eof()) {
        return leaf_codec_corrupted("trailing bytes between the doc id block and the leaf tail");
    }
    if (head.value_mode == LeafValueMode::kRaw) {
        return Status::OK();
    }
    ...
    for (const LeafValueRun& run : runs) {
        ...
        for (uint32_t i = 1; i < run.count; ++i) {
            ...
            if (doc_id > std::numeric_limits<uint32_t>::max()) {
                return leaf_codec_corrupted("doc id deltas overflow a 32-bit row id");
            }
...
Status decode_leaf_block(Slice block, uint32_t bytes_per_dim, uint32_t expected_point_count,
    ...
    if (src.position() != docid_block_offset) {
        return leaf_codec_corrupted("docid_block_offset disagrees with the end of the value area");
    }
