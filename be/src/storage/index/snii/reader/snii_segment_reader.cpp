...
Status ReadBootstrap(io::FileReader* reader, BootstrapHeader* bh) {
    std::vector<uint8_t> buf;
    RETURN_IF_ERROR(reader->read_at(0, format::kBootstrapHeaderSize, &buf));
    return format::decode_bootstrap_header(Slice(buf), bh);
}
