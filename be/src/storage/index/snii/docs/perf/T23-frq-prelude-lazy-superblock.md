...
- `be/src/storage/index/snii/format/frq_prelude.h`
...
## 4. 依赖
- 硬依赖：无。
- 软依赖 / shared infra：`doris::snii::resize_uninitialized`（T19）用于 window_region 拷贝；缺失时 `std::vector::resize` 替代，功能不受影响。
