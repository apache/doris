...
- be/src/storage/index/snii/format/dict_block_directory.cpp:67,74
...
注意：be/src/storage/index/snii/format/tail_pointer.h:29 明确**不**使用 SectionFramer（固定布局，需在无前置知识时可解析，见 bootstrap_header.h:18 同理），这是格式有意为之，不应"统一"到 framer。
...
3. 依赖关系：仅依赖 doris::snii::crc32c（R05）。SectionFramer 自身无需改动；其命运绑定 R05——若 R05 保留或做到字节一致替换，本组件零影响。
