...
struct SniiIndexInput {
    ...
    // Present when the dictionary was generated by a gram-family tokenizer.
    std::optional<segment_v2::gram::GramScheme> gram_scheme;
    // stop-gram: a term whose df exceeds this many documents keeps its dictionary entry
    // (term key + df, marked posting_dropped) but has no posting list written. 0 disables.
    //
    // The caller owns the policy, because the safety argument is the caller's: dropping a
    // posting turns the term into one that matches every document, which is sound only
    // where the index is an approximate filter whose candidates get re-checked. The
    // caller is also the one that knows doc_count and can decide a row floor, so it hands
    // down an absolute document count rather than a ratio.
    //
    // Measured motivation: on real log text the terms above the query-side cost gate's
    // threshold are 0.7% of the vocabulary and hold 88.6% of the posting entries, and the
    // gate already refuses to read every one of them.
    uint32_t stop_gram_df_threshold = 0;
};
...
private:
    ...
    // High-df digest, built as terms stream past. See format::HighDfTerms for what a reader
    // does with it, and note_high_df_term for how it is maintained.
    struct HighDfEntry {
        uint32_t df = 0;
        uint64_t term_hash = 0;
    };
    // Derived from this segment's own doc_count, so the digest scales with the index rather
    // than with a configured constant: a small segment records the terms that are common
    // FOR IT, which is the only sense in which "common" means anything to a cost gate.
    uint32_t high_df_floor_ = 0;
    std::vector<HighDfEntry> high_df_terms_; // min-heap on df, capped
    void note_high_df_term(uint64_t term_hash, uint32_t df);
