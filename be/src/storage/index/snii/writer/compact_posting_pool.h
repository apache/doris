...
public:
    ...
    // Encodes and appends one unsigned LEB128 value. The common case copies all
    // encoded bytes into the current slice in one operation; only a slice-boundary
    // value enters the cold allocation loop.
    void append_varint(SliceWriter* w, uint8_t* level, uint64_t value);
...
// ---- Inlined per-byte hot paths --------------------------------------------
// append_byte (one call per encoded payload byte during SPIMI ingest) and
// Cursor::has_next/next (one call per arena byte during finalize drain/merge)
// are the writer's two hottest per-byte loops. Their callers live in a DIFFERENT
// translation unit (spimi_term_buffer.cpp), and the build has no LTO/IPO/unity
// build, so an out-of-line .cpp definition forces a non-inlinable cross-TU call
// (plus stack spill of cur_/slice_end_/budget_) on every single byte. Defining
// the bodies inline HERE lets the caller's TU inline them, keeping the cursor
// state in registers and eliminating the per-byte call/ret. They are placed
// AFTER the class so every private member they touch (at, read_ptr, alloc_slice,
// write_ptr, kSliceSizes, kNextLevel, payload_bytes_) is already declared and
// visible; Cursor is a nested class, so it may use the enclosing pool's privates
// (read_ptr/at) through pool_. The cold slice-overflow helpers (alloc_slice,
// write_ptr) stay out-of-line in the .cpp -- only the per-byte work is inlined.
// Pure code move from the .cpp: behavior and produced bytes are identical.

inline void CompactPostingPool::append_byte(SliceWriter* w, uint8_t* level, uint8_t value) {
...
inline void CompactPostingPool::append_varint(SliceWriter* w, uint8_t* level, uint64_t value) {
    ...
    while (copied < size) {
        ...
        const size_t available = w->slice_end - w->cur;
        const size_t count = std::min(size - copied, available);
        std::memcpy(at(w->cur), encoded.data() + copied, count);
        w->cur += static_cast<uint32_t>(count);
        copied += count;
        payload_bytes_ += count;
    }
}

inline bool CompactPostingPool::Cursor::has_next() const {
    if (budget_ == 0) {
        return false;
    }
    ...
    if (cur_ == slice_end_) {
        return pool_->read_ptr(slice_end_) != 0;
    }
    return true;
}

inline uint8_t CompactPostingPool::Cursor::next() {
