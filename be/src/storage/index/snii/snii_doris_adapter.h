...
private:
    ...
    // Folds a per-segment private stats slot back into the shared sink (disjoint
    // physical reads never race on it). Delegates to FileCacheStatistics::merge_from
    // so the field list can never drift from io_common.h.
    static void _merge_file_cache_statistics(io::FileCacheStatistics* dst,
                                             const io::FileCacheStatistics& src);

    io::FileReaderSPtr _reader;
    const bool _direct_remote_io = false;
