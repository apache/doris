...
Status BaseTablet::generate_new_block_for_partial_update(
    ...
    const bool has_ttl_source = partial_update_info->row_ttl_source_cid() >= 0;
    std::vector<bool> row_ttl_recompute(has_ttl_source ? update_rows : 0, false);
    ...
    for (auto i = 0; i < missing_cids.size(); ++i) {
        ...
        for (auto idx = 0; idx < update_rows; ++idx) {
            ...
            if (new_row_delete_sign) {
                ...
                if (use_default) {
                    if (has_ttl_source &&
                        std::cmp_equal(missing_cids[i], rowset_schema->ttl_col_idx())) {
    ...
    if (std::ranges::any_of(row_ttl_recompute, [](bool recompute) { return recompute; })) {
        copy_row_ttl_source(output_block, *rowset_schema, partial_update_info->row_ttl_source_cid(),
                            row_ttl_recompute);
    }
        ...
        for (auto idx = 0; idx < update_rows; ++idx) {
            if (cid < rowset_schema->num_key_columns()) {
                ...
                if (rids_be_overwritten.contains(idx)) {
                    ...
                    if (has_ttl_source && std::cmp_equal(cid, rowset_schema->ttl_col_idx()) &&
