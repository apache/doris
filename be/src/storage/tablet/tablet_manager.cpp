...
Status TabletManager::create_tablet(const TCreateTabletReq& request, std::vector<DataDir*> stores,
    ...
    if (has_row_ttl) {
        ...
        const int64_t duration_us = request.tablet_schema.row_ttl_duration_us;
        ...
        const int32_t offset_seconds = request.tablet_schema.row_ttl_time_zone_offset_seconds;
        ...
        if ((is_direct_expiration || ttl_type == TPrimitiveType::TIMESTAMPTZ) &&
            offset_seconds != 0) {
            return Status::InvalidArgument(
                    "Direct and TIMESTAMPTZ Row TTL time zone offset must be 0");
        }
    }
