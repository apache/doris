...
ReadSchemaSPtr RowCursor::create_shared_schema(const TabletSchemaSPtr& schema,
...
Status RowCursor::init(ReadSchemaSPtr schema, const OlapTuple& tuple) {
    _schema = std::move(schema);
    return _from_tuple(tuple);
}

Status RowCursor::init(ReadSchemaSPtr schema, std::vector<Field>&& fields) {
    if (fields.size() != schema->num_read_columns()) {
        return Status::Error<INVALID_ARGUMENT>(
                "column count does not match. fields_size={}, schema_column_count={}",
                fields.size(), schema->num_read_columns());
    }
    _schema = std::move(schema);
