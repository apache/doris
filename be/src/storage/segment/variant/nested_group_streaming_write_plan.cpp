...
Status append_plan_from_rowset_reader(const RowsetReaderSharedPtr& input_rs_reader,
    ...
    for (const auto& segment : segment_cache.get_segments()) {
        ...
        if (variant_reader == nullptr) {
            if (dynamic_cast<ConstantColumnReader*>(column_reader.get()) != nullptr) {
                // For example, a segment written before `ADD COLUMN v VARIANT` has only v's
                // nullable default and therefore contributes no NestedGroup paths to the plan.
                continue;
            }
            return Status::InternalError("column uid {} has a non-VARIANT physical reader",
                                         variant_uid);
        }
