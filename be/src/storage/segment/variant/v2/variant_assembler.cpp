...
// Keep one execution Block's prefix-free S layout independent of the persisted DOC writer layout.
// This matches the existing 2048-path Variant schema defaults without inheriting mutable config or
// treating DOC max_subcolumns_count=0 as an unbounded execution layout.
constexpr size_t MAX_SHREDDED_EXECUTION_LAYOUT_PATHS = 2048;
...
// 4. Whole-root reads with a fixed scalar layout route visible scalar leaves directly into owned
//    shredded fields while ObjectEmitter writes the residual E. Other reads retain the encoded
//    result.
...
Status assemble_hierarchical(StorageMapKind storage_map_kind, bool has_root,
    ...
    if (shredded_builder != nullptr) {
        ...
        DorisVector<uint8_t> bound_paths(shredded_builder->layout().size(), 0);
        for (size_t index = 0; index < materialized_slots.size(); ++index) {
            const auto& path_index = materialized_slots[index].shredded_path_index;
            if (!path_index.has_value()) {
                continue;
            }
            DORIS_CHECK_EQ(bound_paths[*path_index], 0)
                    << "multiple materialized streams cannot bind one shredded path";
            bound_paths[*path_index] = 1;
...
Status check_options(const VariantAssemblerOptions& options) {
    ...
    if (!options.shredded_layout_hints.empty() &&
        (options.storage_map_kind != StorageMapKind::DOC || !options.requested_path.empty())) {
        return Status::InvalidArgument(
                "Variant shredded layout hints require whole-root DOC assembly");
    }
    ...
    RETURN_IF_ERROR(validate_paths(options.shredded_layout_hints, "shredded layout hint"));

    const auto materialized = sorted_path_refs(options.materialized_paths);
    const auto hints = sorted_path_refs(options.shredded_layout_hints);
    ...
    RETURN_IF_ERROR(check_duplicates(hints, "shredded layout hint"));
    size_t materialized_index = 0;
    size_t hint_index = 0;
    while (materialized_index < materialized.size() && hint_index < hints.size()) {
        const PathInData& materialized_path = materialized[materialized_index]->path;
        const PathInData& hint_path = hints[hint_index]->path;
        if (same_path(materialized_path, hint_path)) {
            return Status::InvalidArgument(
                    "Variant path '{}' is both materialized and a shredded layout hint",
                    materialized_path.get_path());
        }
        if (variant_shredded_path_less(materialized_path, hint_path)) {
            ++materialized_index;
        } else {
            ++hint_index;
        }
...
std::unique_ptr<VariantShreddedColumnBuilder> build_shredded_builder(
        ...
        std::span<const VariantAssemblerOptions::MaterializedPath> layout_hints) {
    if (!requested.empty()) {
        return nullptr;
    }
    ...
    candidates.reserve(materialized.size() + layout_hints.size());
    ...
    for (const auto& hint : layout_hints) {
        const DataTypePtr scalar_type = remove_nullable(hint.type);
        PathInData relative_path = hint.path.copy_pop_nfront(requested.get_parts().size());
        if (eligible_path(relative_path) &&
            VariantShreddedColumnBuilder::supports_direct_typed_append(
                    scalar_type->get_primitive_type())) {
            candidates.push_back({.path = std::move(relative_path), .scalar_type = scalar_type});
        }
    }
    ...
    layout.reserve(std::min(candidates.size(), MAX_SHREDDED_EXECUTION_LAYOUT_PATHS));
    for (size_t index = 0; index < candidates.size(); ++index) {
        ...
        if (layout.size() == MAX_SHREDDED_EXECUTION_LAYOUT_PATHS) {
            break;
        }
    }
    if (layout.empty()) {
...
Result<std::unique_ptr<VariantAssembler>> VariantAssembler::create(
    ...
    auto shredded_builder = build_shredded_builder(options.requested_path, materialized,
...
size_t VariantAssembler::TestAccess::max_shredded_execution_layout_paths() {
    return MAX_SHREDDED_EXECUTION_LAYOUT_PATHS;
}
#endif
