...
public:
    ...
    explicit SubcolumnToDocCompactIterator(std::vector<LeafEntry>&& entries)
        ...
        // Sort entries by path so that doc-value ColumnMap keys are always in
        // sorted order. serialize_from_doc_value_to_json_format() relies on
        // sorted keys for correct Prefix-based JSON nesting.
        // Use PathInData comparison (part-by-part) to match the ordering used by
        // _serialize_field_to_doc_value, convert_subcolumns_to_doc_value, and
        // get_sorted_subcolumns throughout the codebase.
        std::sort(_entries.begin(), _entries.end(), [](const LeafEntry& a, const LeafEntry& b) {
            return PathInData(a.path) < PathInData(b.path);
        });
    }
