...
Status read_columns_by_rowids_with_read_ahead(Segment& segment, const TabletSchema& schema,
    ...
    for (auto& column : columns) {
        ...
        const int index = column.slot->col_unique_id() >= 0
                                  ? schema.field_index(column.slot->col_unique_id())
                                  : schema.field_index(column.slot->col_name());
        // A newly added column can be absent from the tablet schema until the next load.
        // Preserve the point-query path's slot-derived default in that interval.
        TabletColumn column_awaiting_a_load;
        if (index < 0) {
            column_awaiting_a_load = variant_util::get_column_by_type(
                    column.slot->type(), column.slot->col_name(),
                    variant_util::ExtraInfo {.unique_id = column.slot->col_unique_id()});
            if (!column.slot->col_default_value().empty()) {
                column_awaiting_a_load.set_default_value(column.slot->col_default_value());
            }
        }
        const TabletColumn& read_column =
                index >= 0 ? schema.column(index) : column_awaiting_a_load;
        RETURN_IF_ERROR(segment.seek_and_read_by_rowid(read_column, column.slot, row_ids,
                                                       *column.result, *column.read_options,
                                                       *column.iterator));
    }
    return Status::OK();
}
