...
Status segment_zone_maps_can_answer_agg(Segment* segment, const Schema& schema,
    ...
    *usable = true;
    for (size_t i = 0; i < schema.num_column_ids(); ++i) {
        std::shared_ptr<ColumnReader> reader;
        Status st = segment->get_column_reader(*schema.column(schema.column_id(i)), &reader,
                                               read_options.stats, &read_options.io_ctx);
        if (st.is<ErrorCode::NOT_FOUND>()) {
            continue;
        }
        RETURN_IF_ERROR(st);
...
Status Segment::new_index_iterator(const TabletColumn& tablet_column, const TabletIndex* index_meta,
    ...
    if (index_meta) {
        ...
        Status iter_status = reader->new_index_iterator(_index_file_reader, index_meta, iter);
        if (!iter_status.ok()) {
            if (need_binding_diagnostic) {
                const auto diagnostic = fmt::format(
