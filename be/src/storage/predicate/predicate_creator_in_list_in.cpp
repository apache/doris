...
static std::shared_ptr<ColumnPredicate> create_in_list_predicate_impl(
        ...
        bool is_opposite, ColumnPredicateLiteralTypeInfo literal_type_info) {
    ...
    if constexpr (!is_string_type(TYPE)) {
        return InListPredicateBase<TYPE, PT, FIXED_CONTAINER_MAX_SIZE + 1>::create_shared(
                cid, col_name, set, is_opposite, literal_type_info);
    } else {
...
std::shared_ptr<ColumnPredicate> create_in_list_predicate<PredicateType::IN_LIST>(
    ...
    case TYPE_INT: {
        return create_in_list_predicate_impl<TYPE_INT, PredicateType::IN_LIST>(
                cid, col_name, set, is_opposite, literal_type_info);
    }
    case TYPE_BIGINT: {
    ...
    case TYPE_CHAR: {
        return create_in_list_predicate_impl<TYPE_CHAR, PredicateType::IN_LIST>(
                cid, col_name, set, is_opposite, literal_type_info);
    }
    case TYPE_VARCHAR: {
