...
Status DataDir::load() {
    ...
    auto load_pending_publish_info_func =
            [&engine = _engine](int64_t tablet_id, int64_t publish_version, std::string_view info) {
                ...
                auto st = engine.add_async_publish_task(pending_publish_info_pb.partition_id(),
                                                        tablet_id, publish_version,
                                                        pending_publish_info_pb.transaction_id(),
                                                        true, pending_publish_info_pb.commit_tso());
                CHECK(st.ok()) << st; // Recovery only registers an already durable marker.
                return true;
            };
