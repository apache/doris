...
public:
    ...
    size_t generate(const Block& block, size_t row_pos, size_t max_rows, size_t max_bytes,
        ...
        // Rows are read from the COW snapshot, indexed by the caller's positions in `block`;
        // the two must describe the same rows or the serialized cells silently mismatch.
