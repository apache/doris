...
int64_t ScannerContext::acquire_limit_quota(int64_t desired) {
    DCHECK(desired > 0);
    int64_t remaining = _remaining_limit.load(std::memory_order_acquire);
    while (true) {
        if (remaining < 0) {
        ...
        if (_remaining_limit.compare_exchange_weak(remaining, remaining - granted,
...
Status ScannerContext::get_block_from_queue(RuntimeState* state, Block* block, bool* eos, int id) {
    ...
    if (_tasks_queue.empty() &&
        (_num_finished_scanners == _all_scanners.size() ||
         (_remaining_limit.load(std::memory_order_acquire) == 0 && _num_scheduled_scanners == 0))) {
        _set_scanner_done();
        _is_finished = true;
    }
...
std::string ScannerContext::debug_string() {
    return fmt::format(
            ...
            _free_blocks.size_approx(), limit, _remaining_limit.load(std::memory_order_relaxed),
