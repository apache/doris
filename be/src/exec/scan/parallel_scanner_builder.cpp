...
io::IOContext ParallelScannerBuilder::_create_preload_io_context(
        RuntimeState* state, OlapReaderStatistics* preload_stats) {
    io::IOContext io_ctx;
    ...
    io_ctx.is_disposable = !state->query_options().enable_file_cache_for_olap_table;
...
Status ParallelScannerBuilder::_load() {
    ...
    for (auto&& [tablet, version] : _tablets) {
        ...
        for (auto& rs_split : read_source.rs_splits) {
            ...
            auto preload_io_ctx = _create_preload_io_context(_state, &preload_stats);
