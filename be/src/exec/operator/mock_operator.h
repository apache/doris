...
public:
    ...
    ~MockLocalState() override = default;

private:
    friend class MockOperatorX;
};

class MockOperatorX final : public OperatorX<MockLocalState> {
public:
    ...
    Status get_block(RuntimeState* state, vectorized::Block* block, bool* eos) override {
        ...
        *eos = false;
        block->swap(_outout_blocks.front());
        _outout_blocks.pop_front();
        if (_outout_blocks.empty()) {
            *eos = true;
        }
        return Status::OK();
