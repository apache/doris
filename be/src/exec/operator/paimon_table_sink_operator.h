...
/// determines the number of concurrent Paimon writer sessions per table. FE's
/// Paimon write provider currently requires GATHER so bucket assignment sees
/// one ordered input stream.
///
/// Partition and bucket routing is performed internally by the Paimon Java SDK
/// through JNI. Doris does not compute partition values or bucket ids; it
/// passes complete Blocks through the backend to the SDK,
...
extern template class AsyncWriterSink<PaimonTableWriter, PaimonTableSinkOperatorX>;
