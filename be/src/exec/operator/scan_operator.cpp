...
Status ScanLocalState<Derived>::_normalize_conjuncts(RuntimeState* state) {
    ...
    for (auto& slot : slots) {
        ...
        if (type == TYPE_ARRAY) {
            type = assert_cast<const DataTypeArray*>(remove_nullable(slot->type()).get())
                           ->get_nested_type()
                           ->get_primitive_type();
            if (type == TYPE_ARRAY) {
                continue;
...
std::atomic<int64_t>* ScanLocalState<Derived>::shared_scan_limit_ptr() {
    ...
    // -1 means "no SQL LIMIT" — return nullptr so downstream code
    // (Scanner, SegmentIterator) naturally skips all limit logic.
