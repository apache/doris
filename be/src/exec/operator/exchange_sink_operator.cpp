...
Status ExchangeSinkLocalState::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    if (_part_type == TPartitionType::HASH_PARTITIONED) {
        ...
        _partitioner = std::make_unique<TabletSinkHashPartitioner>(
...
Status ExchangeSinkLocalState::open(RuntimeState* state) {
    ...
    if ((_part_type == TPartitionType::UNPARTITIONED) && !_only_local_exchange) {
        _broadcast_pb_mem_limiter =
                BroadcastPBlockHolderMemLimiter::create_shared(_queue_dependency);
...
Status ExchangeSinkOperatorX::sink_impl(RuntimeState* state, Block* block, bool eos) {
    ...
    if (_part_type == TPartitionType::UNPARTITIONED) {
        ...
        if (local_state._only_local_exchange) {
            ...
            auto block_holder = BroadcastPBlockHolder::create_shared();
            {
    ...
    } else if (_part_type == TPartitionType::EXTERNAL_TABLE_SINK_UNPARTITIONED) {
