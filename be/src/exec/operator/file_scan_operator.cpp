...
bool FileScanLocalState::_should_use_file_scanner_v2(const TQueryOptions& query_options,
    ...
    // The V1 reader cannot distinguish an explicit wall-clock contract from an absent legacy
    // marker, so every versioned Parquet query must stay on V2 even if the session toggle is off.
