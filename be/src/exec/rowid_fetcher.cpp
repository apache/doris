...
Status RowIdStorageReader::read_external_row_from_file_mapping(
    ...
    scan_blocks[idx] = vectorized::Block(scan_slots, read_ids.size());
...
Status RowIdStorageReader::submit_external_scan_tasks(
        vectorized::ScannerScheduler* scheduler, std::counting_semaphore<>& semaphore,
        size_t task_count, const std::function<std::string(size_t)>& make_task_id,
    ...
    for (size_t idx = 0; idx < task_count; ++idx) {
        ...
        Status submit_st = scheduler->submit_scan_task(
                vectorized::SimplifiedScanTask(run_one_task, nullptr, nullptr), make_task_id(idx));
        if (!submit_st.ok()) {
...
Status RowIdStorageReader::read_batch_external_row(
    ...
    for (size_t column_id = 0; column_id < result_column_count; column_id++) {
        vectorized::MutableColumnPtr dst_col =
                result_block.get_by_position(column_id).column->assume_mutable();

        bool dst_is_nullable = dst_col->is_nullable();
        std::vector<vectorized::ColumnPtr> nullable_src_columns(scan_blocks.size());
        ...
        for (const auto& [pos_block, block_idx] : row_id_block_idx) {
            ...
            if (dst_is_nullable && !src_col->is_nullable()) {
                if (!nullable_src_columns[pos_block]) {
                    nullable_src_columns[pos_block] = vectorized::make_nullable(src_column_ptr);
                }
