...
Status KuduScanner::open_next_scan_token(const string& scan_token) {
    ...
    kudu::client::KuduScanner::ReadMode mode = MODE_READ_AT_SNAPSHOT == config::kudu_read_mode
    ...
    VLOG_ROW << "Starting KuduScanner with ReadMode=" << mode
             << " timeout=" << config::kudu_operation_timeout_ms;
...
Status KuduScanner::handle_empty_projection(RowBatch* row_batch, bool* batch_done) {
    ...
    int rows_to_add = std::min(row_batch->capacity() - row_batch->num_rows(), num_rows_remaining);
...
Status KuduScanner::decode_rows_into_row_batch(RowBatch* row_batch, Tuple** tuple_mem,
    ...
    *batch_done = false;
    ...
    for (int krow_idx = _cur_kudu_batch_num_read; krow_idx < num_rows; ++krow_idx) {
        ...
        if (has_conjuncts && !ExecNode::eval_conjuncts(&_conjunct_ctxs[0], _conjunct_ctxs.size(),
                                                       reinterpret_cast<TupleRow*>(&kudu_tuple))) {
            continue;
        }
        // Deep copy the tuple, set it in a new row, and commit the row.
        kudu_tuple->deep_copy(*tuple_mem, *_scan_node->tuple_desc(), row_batch->tuple_data_pool());
