...
void GroupByAggContext::init_hash_method() {
    auto st = doris::init_hash_method<AggregatedDataVariants>(_hash_table_data.get(),
                                                              _key_data_types, _is_first_phase);
    if (!st.ok()) {
...
Status GroupByAggContext::serialize(RuntimeState* state, Block* block, bool* eos) {
    ...
    agg_context_utils::visit_agg_method(*_hash_table_data, [&](auto& agg_method) {
        ...
        // NOTE: Unlike finalize(), serialize() does NOT defer null key when the
        // batch is already full. This means serialize may output batch_size+1 rows
        // when a null key is present.
...
Status GroupByAggContext::finalize(RuntimeState* state, Block* block, bool* eos) {
    ...
    agg_context_utils::visit_agg_method(*_hash_table_data, [&](auto& agg_method) {
        ...
        // key handling to the next call. init_once() ensures the hash table iterator
