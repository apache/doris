...
Status OlapScanNode::normalize_in_and_eq_predicate(SlotDescriptor* slot,
    ...
    for (int conj_idx = 0; conj_idx < _conjunct_ctxs.size(); ++conj_idx) {
        ...
        if (TExprOpcode::FILTER_IN == _conjunct_ctxs[conj_idx]->root()->op()) {
            ...
            // If the pred is generated by runtime filter, then try to push it to storage engine.
            auto rf_conj = _conjunctid_to_runtime_filter_ctxs.find(conj_idx);
            if (rf_conj != _conjunctid_to_runtime_filter_ctxs.end()) {
                std::vector<TCondition> conditions;
                temp_range.to_olap_filter(conditions);
                // The normalize method should not modify the global state of olap scannode. It
                // is very ugly here, but have no other method.
                _olap_filter.push_back(conditions[0]);
            }
