...
void OlapScanNode::_init_counter(RuntimeState* state) {
    ...
    _bitmap_index_filter_counter =
            ADD_COUNTER(_runtime_profile, "BitmapIndexFilterCount", TUnit::UNIT);
        ...
        if (TExprOpcode::FILTER_IN == _conjunct_ctxs[conj_idx]->root()->op()) {
            ...
            if (1 == pred->get_child(0)->get_slot_ids(&slot_ids)) {
                ...
                VLOG(1) << slot->col_name()
                        << " fixed_values add num: " << pred->hybird_set()->size();
                ...
                while (iter->has_next()) {
                    ...
                    case TYPE_TINYINT: {
                        int32_t v =
                                *reinterpret_cast<int8_t*>(const_cast<void*>(iter->get_value()));
                ...
                if (1 == pred->get_child(child_idx)->get_slot_ids(&slot_ids)) {
                    ...
                    default: {
                        ...
                        return Status::InternalError(
                                "Normalize filter fail, Unsupport Primitive type");
                    }
                    }
                }
            }
...
void OlapScanNode::scanner_thread(OlapScanner* scanner) {
    ...
    while (!eos && raw_rows_read < raw_rows_threshold) {
        ...
        RowBatch* row_batch = new RowBatch(this->row_desc(), state->batch_size(),
                                           _runtime_state->fragment_mem_tracker());
