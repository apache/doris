...
Status SchemaFileCacheInfoScanner::_fill_block_impl(vectorized::Block* block) {
    ...
    std::vector<std::tuple<std::string, int64_t, int64_t, std::string>> cache_entries;
    ...
    for (const auto& cache : caches) {
        ...
        while (iterator->valid()) {
            ...
            cache_entries.emplace_back(hash_str, key.tablet_id, value.size, cache_path);
    ...
    for (size_t col_idx = 0; col_idx < _s_tbls_columns.size(); ++col_idx) {
        ...
        for (size_t row_idx = 0; row_idx < row_num; ++row_idx) {
            ...
            const auto& [hash, tablet_id, size, cache_path] = entry;
