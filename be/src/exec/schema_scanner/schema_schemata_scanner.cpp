...
std::vector<SchemaScanner::ColumnDesc> SchemaSchemataScanner::_s_columns = {
        ...
        {"CREATED_BY", TYPE_VARCHAR, sizeof(StringRef), true},
        {"CREATION_TIME", TYPE_DATETIME, sizeof(int128_t), true},
};
...
Status SchemaSchemataScanner::_fill_block_impl(vectorized::Block* block) {
    ...
    // CREATED_BY
    {
        if (!_db_result.__isset.creators) {
            RETURN_IF_ERROR(fill_dest_column_for_range(block, 6, null_datas));
        } else {
            std::vector<StringRef> strs(dbs_num);
            for (int i = 0; i < dbs_num; ++i) {
                const std::string& src = _db_result.creators[i];
                if (src.empty()) {
                    datas[i] = nullptr;
                } else {
