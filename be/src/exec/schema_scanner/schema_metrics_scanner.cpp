...
std::vector<SchemaScanner::ColumnDesc> SchemaMetricsScanner::_s_metric_columns = {
...
SchemaMetricsScanner::SchemaMetricsScanner()
        : SchemaScanner(_s_metric_columns, TSchemaTableType::SCH_TABLE_OPTIONS) {}

Status SchemaMetricsScanner::start(RuntimeState* state) {
    if (!_is_init) {
        return Status::InternalError("used before initialized.");
    }

    TGetMetricsRequest request;
    RETURN_IF_ERROR(SchemaHelper::get_metrics(*(_param->common_param->ip), _param->common_param->port,
                                            request, &_metrics_result));
    return Status::OK();
}

Status SchemaMetricsScanner::get_next_block_internal(vectorized::Block* block, bool* eos) {
    if (!_is_init) {
...
Status SchemaMetricsScanner::_fill_block_impl(vectorized::Block* block) {
