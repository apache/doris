...
#include "exec/schema_scanner/schema_compaction_tasks_scanner.h"
...
Status SchemaCompactionTasksScanner::get_next_block_internal(vectorized::Block* block, bool* eos) {
    if (!_is_init) {
        return Status::InternalError("Used before initialized.");
    }
...
Status SchemaCompactionTasksScanner::_fill_block_impl(vectorized::Block* block) {
