...
bool can_tree_publish(const std::vector<TRuntimeFilterTargetParamsV2>& targets) {
    return std::all_of(targets.begin(), targets.end(), [](const auto& target) {
        return target.__isset.target_fragment_ids && !target.target_fragment_ids.empty();
    });
}
...
void set_request_direct_publish_target(const TRuntimeFilterTargetParamsV2& target,
    ...
    if (target.__isset.target_fragment_ids && !target.target_fragment_ids.empty()) {
        for (const auto& target_fragment_id : target.target_fragment_ids) {
            request->add_fragment_ids(target_fragment_id);
        }
    } else {
        for (const auto& target_fragment_instance_id : target.target_fragment_instance_ids) {
            PUniqueId* id = request->add_fragment_instance_ids();
            id->set_hi(target_fragment_instance_id.hi);
            id->set_lo(target_fragment_instance_id.lo);
        }
    }
}
...
Status RuntimeFilterMergeControllerEntity::_send_rf_to_target(
    ...
    *apply_request.mutable_query_id() = query_id;
    ...
    if (use_tree_publish) {
        ...
        for (const auto& task : tasks) {
            Status rpc_st =
                    send_runtime_filter_relay_rpc(task, request_attachment, timeout_ms, ctx);
            if (!rpc_st.ok()) {
                st = std::move(rpc_st);
            }
        }
