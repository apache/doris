...
Status ParquetReaderWrap::init_parquet_reader(
...
Status ParquetReaderWrap::handle_timestamp(const std::shared_ptr<arrow::TimestampArray>& ts_array,
    ...
    case arrow::TimeUnit::type::NANO: { // INT96
        timestamp = (time_t)((int64_t)ts_array->Value(_current_line_of_group) /
        ...
        for (size_t i = 0; i < slots; ++i) {
            ...
            case arrow::Type::type::DATE32: {
                ...
                if (ts_array->IsNull(_current_line_of_group)) {
                    ...
                    time_t timestamp = (time_t)((int64_t)ts_array->Value(_current_line_of_group) *
                                                24 * 60 * 60);
                    struct tm local;
                    localtime_r(&timestamp, &local);
            ...
            case arrow::Type::type::DATE64: {
                ...
                if (ts_array->IsNull(_current_line_of_group)) {
                    ...
                    time_t timestamp =
                            (time_t)((int64_t)ts_array->Value(_current_line_of_group) / 1000);
                    struct tm local;
                    localtime_r(&timestamp, &local);
                    char* to = reinterpret_cast<char*>(&tmp_buf);
