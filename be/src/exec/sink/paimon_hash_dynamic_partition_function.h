...
// Routes each HASH_DYNAMIC row to the unique SDK HashBucketAssigner which owns
// its partition/key state. Bucket assignment itself remains in PaimonJniWriter.
class PaimonHashDynamicPartitionFunction final : public PaimonRowHashPartitionFunction {
public:
    PaimonHashDynamicPartitionFunction(HashValType partition_count,
...
private:
    TPaimonHashDynamicInfo _dynamic_info;
};
