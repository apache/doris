...
paimon::Status io_status(const Status& status) {
    if (status.ok()) return paimon::Status::OK();
    if (status.is<ErrorCode::NOT_FOUND>()) return paimon::Status::NotExist(status.to_string());
    if (status.is<ErrorCode::ALREADY_EXIST>()) return paimon::Status::Exist(status.to_string());
    if (status.is<ErrorCode::CANCELLED>()) return paimon::Status::Cancelled(status.to_string());
    if (status.is<ErrorCode::MEM_LIMIT_EXCEEDED>() || status.is<ErrorCode::MEM_ALLOC_FAILED>() ||
        ...
        return paimon::Status::OutOfMemory("Doris Paimon IO allocation failed");
    }
...
auto io_call(const std::shared_ptr<ResourceContext>& context, F&& f) -> decltype(f()) {
    try {
        ...
        return paimon::Status::IOError("Exception in Doris Paimon IO");
    } catch (...) {
        return paimon::Status::IOError("Unknown exception in Doris Paimon IO");
    }
}
...
PResult<std::map<std::string, io::FileInfo>> list_children(const io::FileSystemSPtr& fs,
    ...
    for (auto& file : files) {
        ...
        if (file.file_name.front() == '/' || !safe_suffix(file.file_name)) {
            return paimon::Status::Invalid("Invalid entry in Doris directory listing");
        }
...
public:
    ...
    paimon::Status Seek(int64_t offset, paimon::SeekOrigin origin) override {
        if (_closed) return paimon::Status::Invalid("Input stream is closed");
        int64_t base = 0;
        ...
        if (offset < -base || offset > std::numeric_limits<int64_t>::max() - base) {
            return paimon::Status::Invalid("Invalid seek offset");
        }
        _pos = base + offset;
        return paimon::Status::OK();
    }
    ...
    PResult<int64_t> Read(char* buffer, int64_t size, int64_t offset) override {
        return io_call(_context, [&]() -> PResult<int64_t> {
            if (_closed || size < 0 || offset < 0 || (size > 0 && buffer == nullptr)) {
                return paimon::Status::Invalid("Invalid input read");
            }
    ...
    void ReadAsync(char* buffer, int64_t size, int64_t offset,
        ...
        callback(!result.ok()             ? result.status()
                 : result.value() == size ? paimon::Status::OK()
                                          : paimon::Status::IOError("Short async read"));
    }
    ...
    PResult<int64_t> Write(const char* buffer, int64_t size) override {
        ...
        auto result = io_call(_context, [&]() -> PResult<int64_t> {
            if (!_writer || size < 0 || (size > 0 && buffer == nullptr) ||
                ...
                return paimon::Status::Invalid("Invalid output write");
            }
...
paimon::Status DorisPaimonFileSystem::cleanup_owned_files() {
...
PResult<std::string> DorisPaimonFileSystem::storage_path(const std::string& path) const {
    ...
    if (_table_root == "/" && path.front() == '/')
        ...
        return paimon::Status::Invalid("Paimon IO path is outside the bound table");
    if (!safe_suffix(suffix)) return paimon::Status::Invalid("Ambiguous Paimon IO path");
    ...
    return io_call(_context, [&]() -> PResult<std::unique_ptr<paimon::OutputStream>> {
        ...
        PAIMON_RETURN_NOT_OK(io_status(_fs->exists(mapped, &exists)));
        {
            ...
            if (_ownership_finished)
                return paimon::Status::Invalid("Paimon output ownership is finished");
            if (exists && (!overwrite || !_owned_files.contains(mapped))) {
                return paimon::Status::Exist("Paimon output already exists and cannot be replaced");
            }
...
paimon::Status DorisPaimonFileSystem::Rename(const std::string&, const std::string&) const {
    return paimon::Status::NotImplemented("Native append adapter does not provide atomic rename");
}

paimon::Status DorisPaimonFileSystem::AtomicStore(const std::string&, const std::string&) {
    return paimon::Status::NotImplemented("Paimon metadata commit remains owned by FE");
}
...
paimon::Status DorisPaimonFileSystem::ListFileStatus(
...
paimon::Status DorisPaimonFileSystem::ListDir(
        const std::string& path,
...
paimon::Status DorisPaimonFileSystem::WriteFile(const std::string& path, const std::string& content,
                                                bool overwrite) {
    return io_call(_context, [&]() -> paimon::Status {
        ...
        if (written != static_cast<int64_t>(content.size()))
            return paimon::Status::IOError("Short file write");
