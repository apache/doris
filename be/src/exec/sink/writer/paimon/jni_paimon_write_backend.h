...
public:
    ...
    /// Abort prepared files without retaining the original Java writer object.
    static Status abort_prepared_commit(const TPaimonTableSink& sink,
                                        const std::vector<TPaimonCommitMessage>& commit_messages);

private:
    static Status _check_jni_exception(JNIEnv* env, const std::string& method_name);
    static Status _load_writer_class(JNIEnv* env, jclass* writer_class);
