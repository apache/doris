...
#include "exec/sink/writer/paimon/paimon_bucket_partitioner.h"
...
Status PaimonBucketPartitioner::open(RuntimeState* state) {
    ...
    _pool = std::make_shared<PaimonDorisMemoryPool>(state->query_mem_tracker());
...
Status PaimonBucketPartitioner::do_partitioning(RuntimeState* state, Block* block) const {
    ...
    RETURN_IF_ERROR(_compute_bucket_ids(state, *block, bucket_ids.data()));

    _channel_ids.resize(rows);
    for (size_t i = 0; i < rows; ++i) {
        int32_t b = bucket_ids[i];
        HashValType u = b < 0 ? 0 : static_cast<HashValType>(b);
        _channel_ids[i] = u % _partition_count;
    }
    return Status::OK();
}
