...
PaimonBackendType PaimonWriteBackendFactory::select_backend_type(
    ...
    //   }
    //   return PaimonBackendType::JNI;  // default fallback
    //
    // The FFI backend is preferred when:
    //   - Table is append-only (no primary keys)
    //   - No Lookup/FullCompaction changelog producer
    //   - No DeletionVectors + PartialUpdate/Aggregation
    if (sink.__isset.backend_type) {
        if (sink.backend_type == TPaimonWriteBackendType::FFI) {
            return PaimonBackendType::FFI;
        }
    }
