...
Status JniPaimonWriter::_write_projected_block(RuntimeState* state, Block& block) {
    ...
    // Variant V2 is transported losslessly as its value/metadata pair. The representation option
    // is propagated recursively so Variant nested in arrays, maps, or structs uses the same layout.
