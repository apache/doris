...
Status JniPaimonWriteBackend::abort_prepared_commit(
        const TPaimonTableSink& sink, const std::vector<TPaimonCommitMessage>& commit_messages) {
    ...
    jclass writer_class = nullptr;
    RETURN_IF_ERROR(_load_writer_class(env, &writer_class));
    jmethodID abort_id =
            env->GetStaticMethodID(writer_class, "abortPreparedCommit",
                                   "(Ljava/lang/String;Ljava/util/Map;Ljava/lang/String;[[B)V");
    Status method_status = _check_jni_exception(env, "resolve abortPreparedCommit");
    if (!method_status.ok()) {
    ...
    jstring serialized_table = env->NewStringUTF(sink.serialized_table.c_str());
    jobject hadoop_config =
            _to_java_options(env, sink.__isset.hadoop_config ? sink.hadoop_config : empty_config);
    jstring commit_user = env->NewStringUTF(sink.commit_user.c_str());
    jclass byte_array_class = env->FindClass("[B");
    auto payloads = env->NewObjectArray(static_cast<jsize>(commit_messages.size()),
                                        byte_array_class, nullptr);
    Status allocation_status = _check_jni_exception(env, "allocate abortPreparedCommit arguments");
    if (allocation_status.ok()) {
        for (size_t i = 0; i < commit_messages.size(); ++i) {
            const auto& message = commit_messages[i];
            DORIS_CHECK(message.__isset.payload);
            auto payload = env->NewByteArray(static_cast<jsize>(message.payload.size()));
            env->SetByteArrayRegion(payload, 0, static_cast<jsize>(message.payload.size()),
                                    reinterpret_cast<const jbyte*>(message.payload.data()));
            env->SetObjectArrayElement(payloads, static_cast<jsize>(i), payload);
            env->DeleteLocalRef(payload);
        }
        allocation_status = _check_jni_exception(env, "populate abortPreparedCommit payloads");
    }

    Status abort_status = allocation_status;
    if (abort_status.ok()) {
        env->CallStaticVoidMethod(writer_class, abort_id, serialized_table, hadoop_config,
                                  commit_user, payloads);
        abort_status = _check_jni_exception(env, "abort prepared Paimon commit");
    }
    ...
    if (commit_user != nullptr) {
        env->DeleteLocalRef(commit_user);
    }
    if (hadoop_config != nullptr) {
        env->DeleteLocalRef(hadoop_config);
    }
    if (serialized_table != nullptr) {
        env->DeleteLocalRef(serialized_table);
    }
    env->DeleteLocalRef(writer_class);
    return abort_status;
}
