...
// Builds exactly the byte representation produced by Paimon 1.4.2
// BinaryRowWriter on the current host. Paimon's MemorySegment primitive access
// is native-endian, so using native-endian stores here also preserves behavior
// on a non-little-endian host.
...
// Reproduces Paimon 1.4.2 DefaultBucketFunction. nullopt means the
