...
int32_t java_abs_remainder(int32_t value, int32_t divisor) {
...
int32_t binary_row_hash(std::string_view bytes) {
    ...
    // This is intentionally Paimon's per-byte tail loop, not the standard
    // Murmur3 packed tail. BinaryRows are word-aligned, but keeping this exact
    // makes the primitive independently testable.
...
std::optional<uint32_t> default_bucket(int32_t bucket_key_hash, int32_t num_buckets) {
    ...
    return static_cast<uint32_t>(java_abs_remainder(bucket_key_hash, num_buckets));
}
