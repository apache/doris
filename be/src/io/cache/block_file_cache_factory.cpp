...
Status FileCacheFactory::reload_or_create_file_cache(const std::string& cache_base_path,
                                                     FileCacheSettings file_cache_settings) {
    std::unique_lock lock(_mtx);
    auto cache_map_iter = _path_to_cache.find(cache_base_path);
    if (cache_map_iter == _path_to_cache.end()) {
        lock.unlock();
        return create_file_cache(cache_base_path, file_cache_settings);
    }
    auto cache_iter =
            std::find_if(_caches.begin(), _caches.end(), [cache_map_iter](const auto& cache_uptr) {
    ...
    *cache_iter = std::make_unique<BlockFileCache>(cache_base_path, file_cache_settings);
