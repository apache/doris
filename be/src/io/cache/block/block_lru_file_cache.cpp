...
std::string LRUFileCache::read_file_cache_version() const {
    ...
    bool exists = false;
    THROW_IF_ERROR(fs->exists(version_path, &exists));
    if (!exists) {
        return "1.0";
    }
    FileReaderSPtr version_reader;
    int64_t file_size = -1;
    THROW_IF_ERROR(fs->file_size(version_path, &file_size));
    char version[file_size];

    THROW_IF_ERROR(fs->open_file(version_path, &version_reader));
    size_t bytes_read = 0;
    THROW_IF_ERROR(version_reader->read_at(0, Slice(version, file_size), &bytes_read));
    THROW_IF_ERROR(version_reader->close());
