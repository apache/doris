...
ObjectStorageResponse OSSv2ObjStorageClient::delete_objects(const ObjectStoragePathOptions& opts,
    ...
    for (size_t i = 0; i < objs.size(); i += OSS_BATCH_DELETE_LIMIT) {
        ...
        // Verbose mode (setQuiet=false) makes OSS return the list of successfully deleted keys,
        // enabling partial-failure detection. Quiet mode (default) returns only errors.
        ...
        // keyList() is populated in Verbose mode with the keys that were successfully deleted.
