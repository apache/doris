...
LocalFileReader::LocalFileReader(Path path, size_t file_size, int fd,
    ...
    DorisBvarMetrics::instance()->local_file_open_reading->increment(1);
    DorisBvarMetrics::instance()->local_file_reader_total->increment(1);
}
...
Status LocalFileReader::read_at_impl(size_t offset, Slice result, size_t* bytes_read,
    ...
    *bytes_read = 0;
    ...
    DorisBvarMetrics::instance()->local_bytes_read_total->increment(*bytes_read);
    return Status::OK();
}
