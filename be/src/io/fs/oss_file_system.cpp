...
Result<std::shared_ptr<OSSFileSystem>> OSSFileSystem::create(OSSConf oss_conf, std::string id) {
    // Normalize prefix before construction so RemoteFileSystem and _prefix are consistent.
    if (!oss_conf.prefix.empty()) {
        size_t start = oss_conf.prefix.find_first_not_of('/');
        if (start == std::string::npos) {
            oss_conf.prefix = "";
        } else {
            size_t end = oss_conf.prefix.find_last_not_of('/');
            oss_conf.prefix = oss_conf.prefix.substr(start, end - start + 1);
        }
    }
