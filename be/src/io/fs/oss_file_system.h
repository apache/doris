...
// OSSClientHolder holds a shared OSS client and provides thread-safe access
// In runtime, AK and SK may be modified via vault updates, and the original OssClient will be replaced.
// The OSSFileReader cached by the Segment must hold a shared OSSClientHolder in order to
// access OSS data with latest credentials.
class OSSClientHolder {
public:
    explicit OSSClientHolder(OSSClientConf conf);
    ~OSSClientHolder();

    Status init();

    // Update OSS conf and reset client if `conf` is different. This method is threadsafe.
    Status reset(const OSSClientConf& conf);

    std::shared_ptr<AlibabaCloud::OSS::OssClient> get() const {
    ...
    // For error msg
    std::string full_oss_path(std::string_view bucket, std::string_view key) const;

    const OSSClientConf& oss_client_conf() { return _conf; }
...
// File system for Alibaba Cloud OSS (Object Storage Service)
// When creating OSSFileSystem, all required info should be set in OSSConf,
// such as ak, sk, region, endpoint, bucket.
// And the root_path of OSSFileSystem is oss_conf.prefix.
// When using OSSFileSystem, it accepts 2 kinds of path:
//  1. Full path: oss://bucket/path/to/file.txt
...
class OSSFileSystem final : public RemoteFileSystem {
public:
    static Result<std::shared_ptr<OSSFileSystem>> create(OSSConf oss_conf, std::string id);

    ~OSSFileSystem() override;

    const std::shared_ptr<OSSClientHolder>& client_holder() const { return _client; }
...
private:
    OSSFileSystem(OSSConf oss_conf, std::string id);

    Status init();

    // For error msg
    std::string full_oss_path(std::string_view key) const;
