...
Status LocalExchangeSourceLocalState::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _get_block_failed_counter =
            ADD_COUNTER_WITH_LEVEL(operator_custom_profile(), "GetBlockFailedTime", TUnit::UNIT, 1);
    ...
    if (_exchanger->get_type() == ExchangeType::LOCAL_MERGE_SORT && _channel_id == 0) {
        ...
        _wait_for_dependency_timer =
                ADD_TIMER_WITH_LEVEL(operator_common_profile(), wait_for_dependency_timer_name, 1);
        for (size_t i = 0; i < _deps_counter.size(); i++) {
            _deps_counter[i] = operator_common_profile()->add_nonzero_counter(
