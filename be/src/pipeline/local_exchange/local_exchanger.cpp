...
void Exchanger<QueueType>::_enqueue_data_and_set_ready(
        ...
        LocalExchangeSinkLocalState* local_state, typename QueueType::BlockType&& block) {
    if (local_state == nullptr) {
        _enqueue_data_and_set_ready(enqueue_rows_counter, channel_id, std::move(block));
        return;
    }
    ...
    // one queue.
    if constexpr (std::is_same_v<PartitionedBlock, typename QueueType::BlockType> ||
                  std::is_same_v<BroadcastBlock, typename QueueType::BlockType>) {
    ...
    if (_data_queue[channel_id]->enqueue(std::move(block))) {
        ...
        if constexpr (std::is_same_v<PartitionedBlock, typename QueueType::BlockType> ||
                      std::is_same_v<BroadcastBlock, typename QueueType::BlockType>) {
            block.first->unref(this, local_state->_shared_state, allocated_bytes, channel_id);
        } else {
            block->unref(this, local_state->_shared_state, allocated_bytes, channel_id);
...
template <typename QueueType>
bool Exchanger<QueueType>::_dequeue_data(RuntimeProfile::Counter* dequeue_rows_counter,
    ...
    if (local_state == nullptr) {
        return _dequeue_data(dequeue_rows_counter, get_block_failed_counter, block, eos, data_block,
                             channel_id);
    }
    int64_t rows = 0;
    Defer defer {[&]() {
    ...
    if (_data_queue[channel_id]->try_dequeue(block)) {
        ...
        if constexpr (std::is_same_v<PartitionedBlock, typename QueueType::BlockType> ||
            ...
            block->unref(this, local_state->_shared_state, data_block->allocated_bytes(),
                         channel_id);
            DCHECK_EQ(block->ref_value(), 0);
        }
        return true;
...
void Exchanger<QueueType>::_enqueue_data_and_set_ready(
    ...
    if (_data_queue[channel_id]->enqueue(std::move(block))) {
        ...
        if constexpr (std::is_same_v<PartitionedBlock, typename QueueType::BlockType> ||
                      std::is_same_v<BroadcastBlock, typename QueueType::BlockType>) {
            block.first->unref(this);
        } else {
            block->unref(this);
            DCHECK_EQ(block->ref_value(), 0);
        }
        throw Exception(ErrorCode::INTERNAL_ERROR,
                        "Exception occurs in data queue [eos = {}] of local exchange.",
                        _data_queue[channel_id]->eos());
    }
}
...
template <typename QueueType>
Status ShuffleExchanger<QueueType>::sink(RuntimeState* state, vectorized::Block* in_block, bool eos,
                                         Profile&& profile, SinkInfo&& sink_info) {
    sink_info.channel_selector->reset_selected_channel();
    if (in_block->empty()) {
        return Status::OK();
    }
    {
    ...
    }
    {
        ...
        const auto& channel_filed = sink_info.partitioner->get_channel_ids();
        if (channel_filed.len == sizeof(uint32_t)) {
            RETURN_IF_ERROR(_split_rows(std::move(profile), state,
                                        sink_info.partitioner->get_channel_ids().get<uint32_t>(),
                                        in_block, sink_info.channel_id, sink_info.channel_selector,
                                        sink_info.local_state));
        } else {
            RETURN_IF_ERROR(_split_rows(std::move(profile), state,
                                        sink_info.partitioner->get_channel_ids().get<int64_t>(),
                                        in_block, sink_info.channel_id, sink_info.channel_selector,
                                        sink_info.local_state));
        }
    }
...
template <typename QueueType>
void ShuffleExchanger<QueueType>::close(SourceInfo&& source_info) {
    PartitionedBlock partitioned_block;
    bool eos;
    vectorized::Block block;
    Parent::_data_queue[source_info.channel_id]->set_eos();
    while (Parent::_dequeue_data(nullptr, nullptr, source_info.local_state, partitioned_block, &eos,
                                 &block, source_info.channel_id)) {
        partitioned_block.first->unref(
                this, source_info.local_state ? source_info.local_state->_shared_state : nullptr,
                source_info.channel_id);
    }
}

template <typename QueueType>
Status ShuffleExchanger<QueueType>::get_block(RuntimeState* state, vectorized::Block* block,
    ...
    auto get_data = [&]() -> Status {
        do {
            ...
            RETURN_IF_ERROR(Parent::_mutable_blocks[source_info.channel_id]->add_rows(
                    &block_wrapper->data_block, offset_start,
                    offset_start + partitioned_block.second.length));
            block_wrapper->unref(
                    this,
                    source_info.local_state ? source_info.local_state->_shared_state : nullptr,
                    source_info.channel_id);
        } while (!QueueType::IsSerialized &&
                 Parent::_mutable_blocks[source_info.channel_id]->rows() < state->batch_size() &&
                 !*eos &&
                 Parent::_dequeue_data(profile.dequeue_rows_counter,
                                       profile.get_block_failed_counter, source_info.local_state,
                                       partitioned_block, eos, block, source_info.channel_id));
        return Status::OK();
    };

    if (Parent::_dequeue_data(profile.dequeue_rows_counter, profile.get_block_failed_counter,
                              source_info.local_state, partitioned_block, eos, block,
                              source_info.channel_id)) {
        SCOPED_TIMER(profile.copy_data_timer);
        if (Parent::_mutable_blocks[source_info.channel_id] == nullptr) {
            Parent::_mutable_blocks[source_info.channel_id] =
                    vectorized::MutableBlock::create_unique(
                            partitioned_block.first->data_block.clone_empty());
        }
        if (source_info.accumulated_block) {
            if (source_info.accumulated_block->columns() == 0) {
                *source_info.accumulated_block = partitioned_block.first->data_block.clone_empty();
            }
            Parent::_mutable_blocks[source_info.channel_id]->swap(*source_info.accumulated_block);
        } else if (!block->mem_reuse()) {
            Parent::_mutable_blocks[source_info.channel_id] =
                    vectorized::MutableBlock::create_unique(
                            partitioned_block.first->data_block.clone_empty());
        } else {
            Parent::_mutable_blocks[source_info.channel_id]->set_mutable_columns(
                    block->mutate_columns());
        }
        RETURN_IF_ERROR(get_data());
        *block = Parent::_mutable_blocks[source_info.channel_id]->to_block();
    }
    return Status::OK();
}
...
Status ShuffleExchanger<QueueType>::_split_rows(Profile&& profile, RuntimeState* state,
                                                const ChannelIdType* __restrict channel_ids,
                                                vectorized::Block* block, int channel_id,
                                                ChannelSelector* channel_selector,
                                                LocalExchangeSinkLocalState* local_state) {
    if (local_state == nullptr) {
        return _split_rows(std::move(profile), state, channel_ids, block, channel_id,
                           channel_selector);
    }
    ...
    {
        partition_rows_histogram.assign(Parent::_num_partitions + 1, 0);
        for (int32_t i = 0; i < rows; ++i) {
            partition_rows_histogram[channel_ids[i]]++;
        }
        for (int32_t i = 1; i <= Parent::_num_partitions; ++i) {
    ...
    if (Parent::_free_blocks.try_dequeue(data_block)) {
    ...
    if (get_type() == ExchangeType::HASH_SHUFFLE) {
        ...
        for (const auto& it : map) {
            DCHECK(it.second >= 0 && it.second < Parent::_num_partitions)
                    << it.first << " : " << it.second << " " << Parent::_num_partitions;
            uint32_t start = partition_rows_histogram[it.first];
            uint32_t size = partition_rows_histogram[it.first + 1] - start;
            if (size > 0) {
                Parent::_enqueue_data_and_set_ready(profile.enqueue_rows_counter, it.second,
                                                    local_state,
                                                    {new_block_wrapper, {row_idx, start, size}});
            } else {
                new_block_wrapper->unref(this, local_state->_shared_state, channel_id);
            }
        }
    } else {
        DCHECK(!bucket_seq_to_instance_idx.empty());
        new_block_wrapper->ref(Parent::_num_partitions);
        for (int i = 0; i < Parent::_num_partitions; i++) {
            uint32_t start = partition_rows_histogram[i];
            uint32_t size = partition_rows_histogram[i + 1] - start;
            if (size > 0) {
                Parent::_enqueue_data_and_set_ready(profile.enqueue_rows_counter,
                                                    bucket_seq_to_instance_idx[i], local_state,
                                                    {new_block_wrapper, {row_idx, start, size}});
            } else {
                new_block_wrapper->unref(this, local_state->_shared_state, channel_id);
            }
...
Status ShuffleExchanger<QueueType>::_split_rows(Profile&& profile, RuntimeState* state,
    ...
    auto& partition_rows_histogram = _partition_rows_histogram[channel_id];
    {
        partition_rows_histogram.assign(Parent::_num_partitions + 2, 0);
        for (int32_t i = 0; i < rows; ++i) {
            partition_rows_histogram[channel_ids[i] + 1]++;
        }
        for (int32_t i = 1; i <= Parent::_num_partitions + 1; ++i) {
            partition_rows_histogram[i] += partition_rows_histogram[i - 1];
        }
        for (int32_t i = rows - 1; i >= 0; --i) {
    ...
    std::shared_ptr<BlockWrapper> new_block_wrapper = BlockWrapper::create_shared(*block);
    if (new_block_wrapper->data_block.empty()) {
        return Status::OK();
    }
    new_block_wrapper->ref(cast_set<int>(Parent::_num_partitions));
    for (int i = 0; i < Parent::_num_partitions; i++) {
        uint32_t start = partition_rows_histogram[i + 1];
        uint32_t size = partition_rows_histogram[i + 2] - start;
        if (size > 0) {
            channel_selector->select_channel(i);
            Parent::_enqueue_data_and_set_ready(profile.enqueue_rows_counter, i,
                                                {new_block_wrapper, {row_idx, start, size}});
        } else {
            new_block_wrapper->unref(this);
        }
    }
...
template <typename QueueType>
Status PassthroughExchanger<QueueType>::sink(RuntimeState* state, vectorized::Block* in_block,
    ...
    if (sink_info.local_state) {
        if (!Parent::_free_blocks.try_dequeue(new_block)) {
            new_block = {in_block->clone_empty()};
        }
        new_block.swap(*in_block);
        wrapper = BlockWrapper::create_shared(std::move(new_block));
    } else {
        wrapper = BlockWrapper::create_shared(*in_block);
    }

    auto channel_id = sink_info.channel_selector->next_channel_ids()[0];
    Parent::_enqueue_data_and_set_ready(profile.enqueue_rows_counter, channel_id,
                                        sink_info.local_state, std::move(wrapper));

    return Status::OK();
}

template <typename QueueType>
void PassthroughExchanger<QueueType>::close(SourceInfo&& source_info) {
    vectorized::Block next_block;
    BlockWrapperSPtr wrapper;
    bool eos;
    Parent::_data_queue[source_info.channel_id]->set_eos();
    while (Parent::_dequeue_data(nullptr, nullptr, source_info.local_state, wrapper, &eos,
                                 &next_block, source_info.channel_id)) {
        // do nothing
    }
}
...
void PassToOneExchanger::close(SourceInfo&& source_info) {
    ...
    while (_dequeue_data(nullptr, nullptr, source_info.local_state, wrapper, &eos, &next_block,
                         source_info.channel_id)) {
        // do nothing
    }
}
...
template <typename QueueType>
Status PassthroughExchanger<QueueType>::get_block(RuntimeState* state, vectorized::Block* block,
                                                  bool* eos, Profile&& profile,
                                                  SourceInfo&& source_info) {
    BlockWrapperSPtr next_block;
    Parent::_dequeue_data(profile.dequeue_rows_counter, profile.get_block_failed_counter,
                          source_info.local_state, next_block, eos, block, source_info.channel_id);
    return Status::OK();
}
...
Status PassToOneExchanger::sink(RuntimeState* state, vectorized::Block* in_block, bool eos,
    ...
    _enqueue_data_and_set_ready(profile.enqueue_rows_counter, 0, sink_info.local_state,
                                std::move(wrapper));
...
Status PassToOneExchanger::get_block(RuntimeState* state, vectorized::Block* block, bool* eos,
    ...
    BlockWrapperSPtr next_block;
    _dequeue_data(profile.dequeue_rows_counter, profile.get_block_failed_counter,
                  source_info.local_state, next_block, eos, block, source_info.channel_id);
    return Status::OK();
}
...
Status LocalMergeSortExchanger::sink(RuntimeState* state, vectorized::Block* in_block, bool eos,
    ...
    if (!in_block->empty()) {
        ...
        new_block.swap(*in_block);
        _enqueue_data_and_set_ready(profile.enqueue_rows_counter, sink_info.channel_id,
                                    sink_info.local_state,
                                    BlockWrapper::create_shared(std::move(new_block)));
    }
    if (eos && sink_info.local_state) {
        sink_info.local_state->_shared_state->source_deps[sink_info.channel_id]->set_always_ready();
    }
    return Status::OK();
}
...
void LocalMergeSortExchanger::finalize() {
    ...
    for (auto& data_queue : _data_queue) {
        data_queue->set_eos();
        while (_dequeue_data(nullptr, nullptr, next_block, &eos, &block, id)) {
...
Status LocalMergeSortExchanger::build_merger(Profile&& profile, RuntimeState* state,
    ...
    for (int channel_id = 0; channel_id < _num_partitions; channel_id++) {
        vectorized::BlockSupplier block_supplier =
                [&, local_state, dequeue_rows_counter = profile.dequeue_rows_counter,
                 get_block_failed_counter = profile.get_block_failed_counter,
                 id = channel_id](vectorized::Block* block, bool* eos) {
                    BlockWrapperSPtr next_block;
                    _dequeue_data(dequeue_rows_counter, get_block_failed_counter, local_state,
                                  next_block, eos, block, id);
                    return Status::OK();
                };
...
Status LocalMergeSortExchanger::get_block(RuntimeState* state, vectorized::Block* block, bool* eos,
    ...
    if (!_merger) {
        ...
        RETURN_IF_ERROR(build_merger(std::move(profile), state, source_info.local_state));
    }
...
template <typename QueueType>
Status BroadcastExchanger<QueueType>::sink(RuntimeState* state, vectorized::Block* in_block,
                                           bool eos, Profile&& profile, SinkInfo&& sink_info) {
    if (in_block->empty()) {
        return Status::OK();
    }
    ...
    wrapper->ref(Parent::_num_partitions);
    for (int i = 0; i < Parent::_num_partitions; i++) {
        Parent::_enqueue_data_and_set_ready(profile.enqueue_rows_counter, i, sink_info.local_state,
...
template <typename QueueType>
void BroadcastExchanger<QueueType>::close(SourceInfo&& source_info) {
...
template <typename QueueType>
Status BroadcastExchanger<QueueType>::get_block(RuntimeState* state, vectorized::Block* block,
                                                bool* eos, Profile&& profile,
                                                SourceInfo&& source_info) {
    BroadcastBlock partitioned_block;

    if (Parent::_dequeue_data(profile.dequeue_rows_counter, profile.dequeue_rows_counter,
                              source_info.local_state, partitioned_block, eos, block,
                              source_info.channel_id)) {
        SCOPED_TIMER(profile.copy_data_timer);
        if (Parent::_mutable_blocks[source_info.channel_id] == nullptr) {
        ...
        auto block_wrapper = partitioned_block.first;
        RETURN_IF_ERROR(Parent::_mutable_blocks[source_info.channel_id]->add_rows(
                &block_wrapper->data_block, partitioned_block.second.offset_start,
...
Status AdaptivePassthroughExchanger::_passthrough_sink(Profile&& profile, RuntimeState* state,
    ...
    new_block.swap(*in_block);
    auto channel_id = sink_info.channel_selector->next_channel_ids()[0];
    _enqueue_data_and_set_ready(profile.enqueue_rows_counter, channel_id, sink_info.local_state,
                                BlockWrapper::create_shared(std::move(new_block)));

    return Status::OK();
}

Status AdaptivePassthroughExchanger::_shuffle_sink(Profile&& profile, RuntimeState* state,
    ...
    return _split_rows(std::move(profile), state, channel_ids.data(), block, std::move(sink_info));
}
...
Status AdaptivePassthroughExchanger::_split_rows(Profile&& profile, RuntimeState* state,
    ...
    for (int32_t i = 0; i < _num_partitions; i++) {
        ...
        if (size > 0) {
            ...
            _enqueue_data_and_set_ready(profile.enqueue_rows_counter, i, sink_info.local_state,
                                        BlockWrapper::create_shared(std::move(new_block)));
        }
    }
    return Status::OK();
...
Status AdaptivePassthroughExchanger::sink(RuntimeState* state, vectorized::Block* in_block,
    ...
    if (_is_pass_through) {
        return _passthrough_sink(std::move(profile), state, in_block, std::move(sink_info));
    } else {
        if (_total_block++ > _num_partitions) {
            _is_pass_through = true;
        }
        return _shuffle_sink(std::move(profile), state, in_block, std::move(sink_info));
    }
}
...
Status AdaptivePassthroughExchanger::get_block(RuntimeState* state, vectorized::Block* block,
                                               ...
                                               SourceInfo&& source_info) {
    BlockWrapperSPtr next_block;
    _dequeue_data(profile.dequeue_rows_counter, profile.dequeue_rows_counter,
                  source_info.local_state, next_block, eos, block, source_info.channel_id);
    return Status::OK();
}
...
template class BroadcastExchanger<SerializedBlockQueue<BroadcastBlock>>;
template class BroadcastExchanger<ConcurrentBlockQueue<BroadcastBlock>>;
template class ShuffleExchanger<SerializedBlockQueue<PartitionedBlock>>;
template class ShuffleExchanger<ConcurrentBlockQueue<PartitionedBlock>>;
template class PassthroughExchanger<SerializedBlockQueue<BlockWrapperSPtr>>;
template class PassthroughExchanger<ConcurrentBlockQueue<BlockWrapperSPtr>>;
