    ...
    while (!_shutdown) {
        ...
        while (iter != local_blocked_tasks.end()) {
            ...
            if (state == PipelineTaskState::PENDING_FINISH) {
                ...
                task->query_context()->cancel(PPlanFragmentCancelReason::TIMEOUT, "",
                                              Status::Cancelled(""));
                _make_task_run(local_blocked_tasks, iter, ready_tasks);
...
void TaskScheduler::_try_close_task(PipelineTask* task, PipelineTaskState state) {
    ...
    if (!status.ok() && state != PipelineTaskState::CANCELED) {
        ...
        task->close();
        task->query_context()->cancel(PPlanFragmentCancelReason::INTERNAL_ERROR, status.to_string(),
        ...
        status = task->close();
        if (!status.ok() && state != PipelineTaskState::CANCELED) {
            task->query_context()->cancel(PPlanFragmentCancelReason::INTERNAL_ERROR,
                                          status.to_string(),
                                          Status::Cancelled(status.to_string()));
            state = PipelineTaskState::CANCELED;
        }
