...
Status PartitionedHashJoinProbeLocalState::spill_build_block(RuntimeState* state,
    ...
    return spill_io_pool->submit_func([query_id, mem_data, mem_tracker, execution_context, state,
                                       &build_spilling_stream, &mutable_block, submit_timer, this] {
...
Status PartitionedHashJoinProbeLocalState::spill_probe_blocks(RuntimeState* state,
    ...
    if (!blocks.empty()) {
        ...
        auto mem_tracker = state->get_query_ctx()->query_mem_tracker;
        std::weak_ptr<HashJoinMemData> mem_data = _shared_state->mem_data;
        return spill_io_pool->submit_func([query_id, mem_data, mem_tracker, execution_context,
                                           state, &blocks, spilling_stream, submit_timer, this] {
            SCOPED_ATTACH_TASK_WITH_ID(mem_tracker, query_id);
            auto execution_context_lock = execution_context.lock();
            auto mem_data_sptr = mem_data.lock();
            if (!mem_data_sptr || !execution_context_lock) {
...
Status PartitionedHashJoinProbeLocalState::recovery_build_blocks_from_disk(RuntimeState* state,
    ...
    auto mem_tracker = state->get_query_ctx()->query_mem_tracker;
    std::weak_ptr<HashJoinMemData> mem_data = _shared_state->mem_data;
    auto read_func = [this, state, query_id, mem_data, mem_tracker, &spilled_stream, &mutable_block,
...
Status PartitionedHashJoinProbeLocalState::recovery_probe_blocks_from_disk(RuntimeState* state,
    ...
    auto mem_tracker = state->get_query_ctx()->query_mem_tracker;
    std::weak_ptr<HashJoinMemData> mem_data = _shared_state->mem_data;
    auto read_func = [this, query_id, mem_data, mem_tracker, execution_context, &spilled_stream,
                      &blocks, submit_timer] {
        SCOPED_ATTACH_TASK_WITH_ID(mem_tracker, query_id);
        auto execution_context_lock = execution_context.lock();
        auto mem_data_sptr = mem_data.lock();
