...
Status PartitionedHashJoinProbeLocalState::spill_probe_blocks(RuntimeState* state) {
    ...
    auto exception_catch_func = [query_id, shared_state_holder, execution_context, spill_func,
                                 this]() {
...
Status PartitionedHashJoinProbeLocalState::recovery_build_blocks_from_disk(RuntimeState* state,
    ...
    auto read_func = [this, query_id, mem_tracker, state, spilled_stream = spilled_stream,
                      &mutable_block, shared_state_holder, submit_timer, partition_index] {
        SCOPED_ATTACH_TASK_WITH_ID(mem_tracker, query_id);
        auto shared_state_sptr = shared_state_holder.lock();
    ...
    auto exception_catch_func = [read_func, shared_state_holder, execution_context, state, query_id,
                                 this]() {
        std::shared_ptr<TaskExecutionContext> execution_context_lock;
        auto shared_state_sptr = shared_state_holder.lock();
        if (shared_state_sptr) {
...
Status PartitionedHashJoinProbeLocalState::recovery_probe_blocks_from_disk(RuntimeState* state,
    ...
    auto read_func = [this, query_id, mem_tracker, &spilled_stream, &blocks, submit_timer] {
        SCOPED_ATTACH_TASK_WITH_ID(mem_tracker, query_id);
        _spill_wait_in_queue_timer->update(submit_timer.elapsed_time());
    ...
    auto exception_catch_func = [read_func, shared_state_holder, execution_context, query_id,
                                 this]() {
        std::shared_ptr<TaskExecutionContext> execution_context_lock;
        auto shared_state_sptr = shared_state_holder.lock();
        if (shared_state_sptr) {
