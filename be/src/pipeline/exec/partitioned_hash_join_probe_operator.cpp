...
Status PartitionedHashJoinProbeLocalState::acquire_spill_writer(
        RuntimeState* state, int partition_index, vectorized::SpillFileWriterSPtr& writer) {
...
Status PartitionedHashJoinProbeLocalState::spill_probe_blocks(RuntimeState* state, bool flush_all) {
    ...
    for (uint32_t partition_index = 0; partition_index != p._partition_count; ++partition_index) {
        ...
        if (!flush_all && partitioned_block->allocated_bytes() <
...
Status PartitionedHashJoinProbeLocalState::repartition_current_partition(
    ...
    std::unique_ptr<vectorized::PartitionerBase> build_fanout_clone;
    ...
    std::vector<vectorized::SpillFileSPtr> build_output_spill_files;
...
Status PartitionedHashJoinProbeOperatorX::push(RuntimeState* state, vectorized::Block* input_block,
    ...
    for (uint32_t i = 0; i != _partition_count; ++i) {
        ...
        if (bytes >= vectorized::SpillFile::MIN_SPILL_WRITE_BATCH_MEM) {
            vectorized::SpillFileWriterSPtr writer;
