...
Status JoinProbeLocalState<SharedStateArg, Derived>::init(RuntimeState* state,
    ...
    _join_filter_timer = ADD_TIMER(Base::operator_custom_profile(), "JoinFilterTimer");
    _build_output_block_timer = ADD_TIMER(Base::operator_custom_profile(), "BuildOutputBlock");
    _probe_rows_counter =
            ADD_COUNTER_WITH_LEVEL(Base::operator_custom_profile(), "ProbeRows", TUnit::UNIT, 1);
    _finish_probe_phase_timer = ADD_TIMER(Base::operator_custom_profile(), "FinishProbePhaseTime");
    return Status::OK();
}
