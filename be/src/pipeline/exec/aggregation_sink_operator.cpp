...
Status AggSinkLocalState::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    _agg_data = Base::_shared_state->agg_data;
    _agg_arena_pool = Base::_shared_state->agg_arena_pool;
    ...
    if (Base::_shared_state->probe_expr_ctxs.empty()) {
        ...
        std::visit(vectorized::Overload {
                           ...
                           [&](auto& agg_method) {
                               ...
                               Base::_shared_state->reset_agg_data_container(
...
Status AggSinkOperatorX::reset_hash_table(RuntimeState* state) {
    ...
    local_state._agg_arena_pool = ss.agg_arena_pool;
    return Status::OK();
}
