...
Status AggLocalState::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _get_results_timer = ADD_TIMER(operator_custom_profile(), "GetResultsTime");
    _hash_table_iterate_timer = ADD_TIMER(operator_custom_profile(), "HashTableIterateTime");
    _insert_keys_to_column_timer = ADD_TIMER(operator_custom_profile(), "InsertKeysToColumnTime");
    _insert_values_to_column_timer =
            ADD_TIMER(operator_custom_profile(), "InsertValuesToColumnTime");

    _merge_timer = ADD_TIMER(Base::operator_custom_profile(), "MergeTime");
    _deserialize_data_timer = ADD_TIMER(Base::operator_custom_profile(), "DeserializeAndMergeTime");
    _hash_table_compute_timer = ADD_TIMER(Base::operator_custom_profile(), "HashTableComputeTime");
    _hash_table_emplace_timer = ADD_TIMER(Base::operator_custom_profile(), "HashTableEmplaceTime");
    _hash_table_input_counter = ADD_COUNTER_WITH_LEVEL(Base::operator_custom_profile(),
                                                       "HashTableInputCount", TUnit::UNIT, 1);
    _hash_table_memory_usage = ADD_COUNTER_WITH_LEVEL(Base::operator_custom_profile(),
                                                      "MemoryUsageHashTable", TUnit::BYTES, 1);
    _hash_table_size_counter = ADD_COUNTER_WITH_LEVEL(Base::operator_custom_profile(),
                                                      "HashTableSize", TUnit::UNIT, 1);

    _memory_usage_container =
            ADD_COUNTER(operator_custom_profile(), "MemoryUsageContainer", TUnit::BYTES);
    _memory_usage_arena = ADD_COUNTER(operator_custom_profile(), "MemoryUsageArena", TUnit::BYTES);
