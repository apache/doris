...
Status ExchangeSinkBuffer::_send_rpc(InstanceLoId id) {
    ...
    if (!instance_q.empty()) {
        ...
        PSenderBlockInfo* sender_block_info = brpc_request->add_multi_sender();
        if (request.block && !request.block->column_metas().empty()) {
            sender_block_info->set_allocated_block(request.block.get());
        }

        sender_block_info->set_sender_id(brpc_request->sender_id());
        sender_block_info->set_eos(brpc_request->eos());
        sender_block_info->set_packet_seq(brpc_request->packet_seq());
        ...
        if (request.block) {
            ...
            static_cast<void>(sender_block_info->release_block());
        }
        instance_q.pop();
        _total_queue_size--;
        if (_queue_dependency && _total_queue_size <= _queue_capacity) {
        ...
        if (request.block_holder->get_block() &&
            ...
            sender_block_info->set_allocated_block(request.block_holder->get_block());
        }
        auto send_callback = request.channel->get_send_callback(id, request.eos);
