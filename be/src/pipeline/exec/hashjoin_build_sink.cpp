...
Status HashJoinBuildSinkLocalState::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    if (!_should_build_hash_table) {
        ...
        {
            std::lock_guard<std::mutex> guard(_shared_state->shared_hash_table_context->mutex);
            _shared_state->shared_hash_table_context->finish_dependencies.push_back(
                    _finish_dependency);
        }
    } else {
    ...
    if (eos) {
        ...
        if (build_block_rows > 0) {
            ...
            std::visit(vectorized::Overload {[&](std::monostate& arg) {
                       ...
                       _shared_state->hash_table_variant_vector[0]->method_variant);
        }
    }
...
Status HashJoinBuildSinkLocalState::close(RuntimeState* state, Status exec_status) {
    ...
    Defer defer {[&]() {
        ...
        {
            std::unique_lock(_shared_state->shared_hash_table_context->mutex);
            _shared_state->shared_hash_table_context->signaled = true;
            for (auto& dep : _shared_state->sink_deps) {
                dep->set_ready();
            }
            for (auto& dep : _shared_state->shared_hash_table_context->finish_dependencies) {
                dep->set_ready();
            }
        }
    }};
    ...
    try {
        RETURN_IF_ERROR(_runtime_filter_producer_helper->process(
                ...
                p._use_shared_hash_table ? _shared_state->shared_hash_table_context : nullptr));
    } catch (Exception& e) {
        bool blocked_by_shared_hash_table_signal =
                !_should_build_hash_table && p._use_shared_hash_table &&
                !_shared_state->shared_hash_table_context->signaled;
...
Status HashJoinBuildSinkLocalState::process_build_block(RuntimeState* state,
    ...
    Status st = std::visit(
            ...
            _shared_state->hash_table_variant_vector[0]->method_variant, p._join_op_variants,
...
Status HashJoinBuildSinkLocalState::_hash_table_init(RuntimeState* state) {
    ...
    return init_hash_method<JoinDataVariants>(
            _shared_state
                    ->hash_table_variant_vector[_shared_state->hash_table_variant_vector.size() == 1
                                                        ? 0
                                                        : _task_idx]
                    .get(),
            data_types, true);
...
Status HashJoinBuildSinkOperatorX::sink(RuntimeState* state, vectorized::Block* in_block,
    ...
    if (local_state._should_build_hash_table && eos) {
        ...
        if (!local_state._shared_state->shared_hash_table_context->signaled ||
            state->get_task()->wake_up_early()) {
            return Status::Error<ErrorCode::END_OF_FILE>("source have closed");
        }

        if (!local_state._shared_state->shared_hash_table_context->status.ok()) {
            return local_state._shared_state->shared_hash_table_context->status;
        }
