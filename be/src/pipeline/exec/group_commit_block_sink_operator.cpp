...
Status GroupCommitBlockSinkLocalState::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    _init_load_queue_timer = ADD_TIMER(operator_custom_profile(), "InitLoadQueueTime");
    _valid_and_convert_block_timer =
            ADD_TIMER(operator_custom_profile(), "ValidAndConvertBlockTime");
    _find_partition_timer = ADD_TIMER(operator_custom_profile(), "FindPartitionTime");
    _append_blocks_timer = ADD_TIMER(operator_custom_profile(), "AppendBlocksTime");
    return Status::OK();
}
