...
Status EsScanLocalState::_init_profile() {
    ...
    _blocks_read_counter = ADD_COUNTER(_operator_custom_profile, "BlocksRead", TUnit::UNIT);
    _read_timer = ADD_TIMER(_operator_custom_profile, "TotalRawReadTime(*)");
    _materialize_timer = ADD_TIMER(_operator_custom_profile, "MaterializeTupleTime(*)");
    return Status::OK();
}
