...
void PartitionedAggSinkLocalState::_init_counters() {
    ...
    _memory_usage_reserved = ADD_COUNTER_WITH_LEVEL(Base::operator_custom_profile(),
                                                    "MemoryUsageReserved", TUnit::BYTES, 1);

    _spill_serialize_hash_table_timer =
            ADD_TIMER_WITH_LEVEL(Base::operator_custom_profile(), "SpillSerializeHashTableTime", 1);
}
#define UPDATE_PROFILE(name) \
    update_profile_from_inner_profile<spilled>(name, Base::operator_custom_profile(), child_profile)
...
Status PartitionedAggSinkOperatorX::sink(doris::RuntimeState* state, vectorized::Block* in_block,
    ...
    if (!local_state._shared_state->is_spilled) {
        ...
        local_state.update_profile<false>(sink_local_state->operator_custom_profile());
    }
...
Status PartitionedAggSinkLocalState::revoke_memory(
    ...
    if (!_shared_state->is_spilled) {
        ...
        update_profile<false>(sink_local_state->operator_custom_profile());
    } else {
        update_profile<true>(sink_local_state->operator_custom_profile());
    }
