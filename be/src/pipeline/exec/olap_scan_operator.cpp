...
Status OlapScanLocalState::_init_profile() {
    ...
    _scan_rows = ADD_COUNTER(_operator_custom_profile, "ScanRows", TUnit::UNIT);
    ...
    _process_conjunct_timer = ADD_TIMER(_operator_custom_profile, "ProcessConjunctTime");
    ...
    _tablet_counter = ADD_COUNTER(_operator_custom_profile, "TabletNum", TUnit::UNIT);
    _key_range_counter = ADD_COUNTER(_operator_custom_profile, "KeyRangesNum", TUnit::UNIT);
    _runtime_filter_info =
            ADD_LABEL_COUNTER_WITH_LEVEL(_operator_custom_profile, "RuntimeFilterInfo", 1);
...
Status OlapScanLocalState::_init_scanners(std::list<vectorized::VScannerSPtr>* scanners) {
    ...
    if (!_conjuncts.empty() && RuntimeFilterConsumer::_state->enable_profile()) {
        ...
        _operator_custom_profile->add_info_string("RemainedDownPredicates", message);
    }
    auto& p = _parent->cast<OlapScanOperatorX>();
...
Status OlapScanLocalState::_build_key_ranges_and_filters() {
    ...
    if (p._push_down_agg_type == TPushAggOp::NONE ||
        ...
        _operator_custom_profile->add_info_string("PushDownAggregate",
                                                  push_down_agg_to_string(p._push_down_agg_type));
    }

    if (state()->enable_profile()) {
        _operator_custom_profile->add_info_string("PushDownPredicates",
                                                  olap_filters_to_string(_olap_filters));
        _operator_custom_profile->add_info_string("KeyRanges", _scan_keys.debug_string());
        _operator_custom_profile->add_info_string("TabletIds", tablets_id_to_string(_scan_ranges));
    }
...
void OlapScanLocalState::add_filter_info(int id, const PredicateFilterInfo& update_info) {
    ...
    // add counter
    auto* input_count = ADD_CHILD_COUNTER_WITH_LEVEL(_operator_custom_profile, rf_name + "input",
                                                     TUnit::UNIT, "RuntimeFilterInfo", 1);
    auto* filtered_count = ADD_CHILD_COUNTER_WITH_LEVEL(
            _operator_custom_profile, rf_name + "filtered", TUnit::UNIT, "RuntimeFilterInfo", 1);
