...
Status OlapScanLocalState::open(RuntimeState* state) {
    ...
    if (_ann_topn_descriptor) {
        ...
        _ann_topn_descriptor->get_order_by_expr_ctx()->root()->prepare_virtual_slots(
                _slot_id_to_virtual_column_expr);
    }

    RETURN_IF_ERROR(ScanLocalState<OlapScanLocalState>::open(state));

    for (auto exp : _common_expr_ctxs_push_down) {
        auto root = exp->root();
        root->prepare_virtual_slots(_slot_id_to_virtual_column_expr);
    }

    for (auto exp : _conjuncts) {
        auto root = exp->root();
        root->prepare_virtual_slots(_slot_id_to_virtual_column_expr);
    }
    return Status::OK();
