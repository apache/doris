...
Status PipelineXLocalState<SharedStateArg>::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    */
    ...
    operator_profile()->set_metadata(_parent->node_id());
    operator_profile()->set_is_sink(false);
    // indent is false so that source operator will have same
    // indentation_level with its parent operator.
    info.parent_profile->add_child(operator_profile(), /*indent=*/false, nullptr);

    _operator_common_profile = std::make_unique<RuntimeProfile>("CommonCounters");
    _operator_custom_profile = std::make_unique<RuntimeProfile>("CustomCounters");
    operator_profile()->add_child(_operator_common_profile.get(), /*indent=*/true);
    operator_profile()->add_child(_operator_custom_profile.get(), /*indent=*/true);
        ...
        if constexpr (std::is_same_v<LocalExchangeSharedState, SharedStateArg>) {
            ...
            _wait_for_dependency_timer =
                    ADD_TIMER_WITH_LEVEL(operator_common_profile(),
                                         "WaitForDependency[" + _dependency->name() + "]Time", 1);
    ...
    _rows_returned_counter =
            ADD_COUNTER_WITH_LEVEL(operator_common_profile(), "RowsProduced", TUnit::UNIT, 1);
    _blocks_returned_counter =
            ADD_COUNTER_WITH_LEVEL(operator_common_profile(), "BlocksProduced", TUnit::UNIT, 1);
    _projection_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "ProjectionTime", 1);
    _init_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "InitTime", 1);
    _open_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "OpenTime", 1);
    _close_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "CloseTime", 1);
    _exec_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "ExecTime", 1);
    _memory_used_counter =
            operator_common_profile()->AddHighWaterMarkCounter("MemoryUsage", TUnit::BYTES, "", 1);
    return Status::OK();
}
...
Status PipelineXSinkLocalState<SharedState>::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    info.parent_profile->add_child(operator_profile(), /*indent=*/true, nullptr);

    _operator_common_profile = state->obj_pool()->add(new RuntimeProfile("CommonCounters"));
    _operator_custom_profile = state->obj_pool()->add(new RuntimeProfile("CustomCounters"));
    operator_profile()->add_child(_operator_common_profile, /*indent=*/true);
    operator_profile()->add_child(_operator_custom_profile, /*indent=*/true);

    operator_profile()->set_metadata(_parent->node_id());
    operator_profile()->set_is_sink(true);
    _wait_for_finish_dependency_timer =
            ADD_TIMER(operator_common_profile(), "PendingFinishDependency");
    ...
    _rows_input_counter =
            ADD_COUNTER_WITH_LEVEL(operator_common_profile(), "InputRows", TUnit::UNIT, 1);
    _init_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "InitTime", 1);
    _open_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "OpenTime", 1);
    _close_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "CloseTime", 1);
    _exec_timer = ADD_TIMER_WITH_LEVEL(operator_common_profile(), "ExecTime", 1);
...
Status AsyncWriterSink<Writer, Parent>::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    _wait_for_dependency_timer = ADD_TIMER_WITH_LEVEL(
            operator_common_profile(),
            "WaitForDependency[" + _async_writer_dependency->name() + "]Time", 1);
    return Status::OK();
}
...
Status AsyncWriterSink<Writer, Parent>::open(RuntimeState* state) {
    ...
    RETURN_IF_ERROR(_writer->start_writer(state, operator_custom_profile()));
    return Status::OK();
}
