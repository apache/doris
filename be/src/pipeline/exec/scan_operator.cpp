...
Status ScanLocalState<Derived>::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _wait_for_dependency_timer =
            ADD_TIMER_WITH_LEVEL(operator_common_profile(),
                                 "WaitForDependency[" + _scan_dependency->name() + "]Time", 1);
    SCOPED_TIMER(exec_time_counter());
    SCOPED_TIMER(_init_timer);
    auto& p = _parent->cast<typename Derived::Parent>();
    RETURN_IF_ERROR(RuntimeFilterConsumer::init(state, p.is_serial_operator()));
    // init profile for runtime filter
    RuntimeFilterConsumer::_init_profile(operator_custom_profile());
    ...
    _prepare_rf_timer(operator_common_profile());

    _wait_for_rf_timer = ADD_TIMER(operator_common_profile(), "WaitForRuntimeFilter");
    return Status::OK();
}
...
Status ScanLocalState<Derived>::_init_profile() {
    ...
    _rows_read_counter = ADD_COUNTER(operator_custom_profile(), "RowsRead", TUnit::UNIT);
    _num_scanners = ADD_COUNTER(operator_custom_profile(), "NumScanners", TUnit::UNIT);
    ...
    operator_custom_profile()->add_child(_scanner_profile.get(), true, nullptr);
    ...
    _scanner_wait_worker_timer = ADD_TIMER(operator_custom_profile(), "ScannerWorkerWaitTime");

    _max_scan_concurrency =
            ADD_COUNTER(operator_custom_profile(), "MaxScanConcurrency", TUnit::UNIT);
    _min_scan_concurrency =
            ADD_COUNTER(operator_custom_profile(), "MinScanConcurrency", TUnit::UNIT);
    ...
    _scan_rows = ADD_COUNTER(operator_custom_profile(), "ScanRows", TUnit::UNIT);
    // Size of data that read from storage.
    // Does not include rows that are cached by doris page cache.
    _scan_bytes = ADD_COUNTER(operator_custom_profile(), "ScanBytes", TUnit::BYTES);
    return Status::OK();
}
