...
Status PartitionSortSinkLocalState::init(RuntimeState* state, LocalSinkStateInfo& info) {
    ...
    _serialize_key_arena_memory_usage = operator_custom_profile()->AddHighWaterMarkCounter(
    ...
    _build_timer = ADD_TIMER(operator_custom_profile(), "HashTableBuildTime");
    _selector_block_timer = ADD_TIMER(operator_custom_profile(), "SelectorBlockTime");
    _emplace_key_timer = ADD_TIMER(operator_custom_profile(), "EmplaceKeyTime");
    _sorted_data_timer = ADD_TIMER(operator_custom_profile(), "SortedDataTime");
    _passthrough_rows_counter =
            ADD_COUNTER(operator_custom_profile(), "PassThroughRowsCounter", TUnit::UNIT);
    _sorted_partition_input_rows_counter =
            ADD_COUNTER(operator_custom_profile(), "SortedPartitionInputRows", TUnit::UNIT);
    _partition_sort_info = std::make_shared<PartitionSortInfo>(
            ...
            p._child->row_desc(), state, operator_custom_profile(), p._has_global_limit,
            p._partition_inner_limit, p._top_n_algorithm, p._topn_phase);
    operator_custom_profile()->add_info_string("PartitionTopNPhase", to_string(p._topn_phase));
    operator_custom_profile()->add_info_string("PartitionTopNLimit",
