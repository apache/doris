...
Status PartitionedHashJoinSinkLocalState::init(doris::RuntimeState* state,
    ...
    _partition_timer = ADD_TIMER_WITH_LEVEL(operator_custom_profile(), "SpillPartitionTime", 1);
    _partition_shuffle_timer =
            ADD_TIMER_WITH_LEVEL(operator_custom_profile(), "SpillPartitionShuffleTime", 1);
    _spill_build_timer = ADD_TIMER_WITH_LEVEL(operator_custom_profile(), "SpillBuildTime", 1);
    _in_mem_rows_counter =
            ADD_COUNTER_WITH_LEVEL(operator_custom_profile(), "SpillInMemRow", TUnit::UNIT, 1);
...
Status PartitionedHashJoinSinkLocalState::_revoke_unpartitioned_block(
    ...
    auto spill_runnable = std::make_shared<SpillSinkRunnable>(
            state, spill_context, _spill_dependency, operator_custom_profile(), _shared_state->shared_from_this(),
...
Status PartitionedHashJoinSinkLocalState::revoke_memory(
    ...
    auto spill_runnable = std::make_shared<SpillSinkRunnable>(
            state, nullptr, nullptr, operator_custom_profile(), _shared_state->shared_from_this(),
            [this, query_id] {
...
#define UPDATE_COUNTER_FROM_INNER(name) \
    update_profile_from_inner_profile<false>(name, operator_custom_profile(), inner_profile)
...
void PartitionedHashJoinSinkLocalState::update_profile_from_inner() {
    ...
    if (sink_local_state) {
        ...
        auto* inner_profile = inner_sink_state->operator_custom_profile();
