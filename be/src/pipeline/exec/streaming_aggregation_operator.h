...
private:
    ...
    friend class AggSinkLocalStateHelper<StreamingAggLocalState, StreamingAggOperatorX>;
    friend class AggLocalStateHelper<StreamingAggLocalState, StreamingAggOperatorX>;
    friend class AggSourceLocalStateHelper<StreamingAggLocalState, StreamingAggOperatorX>;
    AggSinkLocalStateHelper<StreamingAggLocalState, StreamingAggOperatorX> _agg_sink_helper;
    AggSourceLocalStateHelper<StreamingAggLocalState, StreamingAggOperatorX> _agg_source_helper;
    ...
    std::unique_ptr<vectorized::Arena> agg_arena_pool = nullptr;
    ...
    std::unique_ptr<vectorized::AggregateDataContainer> aggregate_data_container = nullptr;
    ...
    struct Executor final : public ExecutorBase {
        Status get_result(StreamingAggLocalState* local_state, RuntimeState* state,
            ...
            if constexpr (WithoutKey) {
                if constexpr (NeedFinalize) {
                    return local_state->_agg_source_helper._get_without_key_result(state, block,
                                                                                   eos);
                } else {
                    return local_state->_agg_source_helper._serialize_without_key(state, block,
                                                                                  eos);
                }
            } else {
                if constexpr (NeedFinalize) {
                    return local_state->_agg_source_helper._get_with_serialized_key_result(
                            state, block, eos);
                } else {
                    return local_state->_agg_source_helper._serialize_with_serialized_key_result(
                            state, block, eos);
                }
            }
        }
        ...
        Status execute(StreamingAggLocalState* local_state, vectorized::Block* block) override {
            if constexpr (WithoutKey) {
                if constexpr (NeedToMerge) {
                    return local_state->_agg_sink_helper._merge_without_key(block);
                } else {
                    return local_state->_agg_sink_helper._execute_without_key(block);
                }
            } else {
                if constexpr (NeedToMerge) {
                    return local_state->_agg_sink_helper._merge_with_serialized_key(block);
                } else {
                    return local_state->_agg_sink_helper._execute_with_serialized_key(block);
                }
            }
        }
        ...
        void update_memusage(StreamingAggLocalState* local_state) override {
            if constexpr (WithoutKey) {
                local_state->_agg_sink_helper._update_memusage_without_key();
            } else {
                local_state->_agg_sink_helper._update_memusage_with_serialized_key();
            }
        }
    };
        ...
        std::visit(
                [&](auto&& agg_method) -> void {
                    ...
                    data.for_each_mapped([&](auto& mapped) {
                        if (mapped) {
                            (void)_agg_sink_helper._destroy_agg_status(mapped);
                            mapped = nullptr;
                        }
                    });
                    if (data.has_null_key_data()) {
                        (void)_agg_sink_helper._destroy_agg_status(
