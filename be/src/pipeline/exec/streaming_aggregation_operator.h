...
private:
    ...
    bool _should_not_do_pre_agg(size_t rows);
...
private:
    ...
    Status _init_probe_expr_ctx(RuntimeState* state);

    Status _init_aggregate_evaluators(RuntimeState* state);

    Status _calc_aggregate_evaluators();
