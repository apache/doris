...
Status SpillSortSinkLocalState::open(RuntimeState* state) {
    ...
    _shared_state->setup_shared_profile(operator_custom_profile());
...
void SpillSortSinkLocalState::_init_counters() {
    ...
    _spill_merge_sort_timer =
            ADD_TIMER_WITH_LEVEL(operator_custom_profile(), "SpillMergeSortTime", 1);
}

#define UPDATE_PROFILE(name) update_profile_from_inner_profile<true>(name, operator_custom_profile(), child_profile)
...
Status SpillSortSinkLocalState::setup_in_memory_sort_op(RuntimeState* state) {
    ...
    operator_custom_profile()->add_info_string(
            "TOP-N", *sink_local_state->operator_custom_profile()->get_info_string("TOP-N"));
    return Status::OK();
}
...
Status SpillSortSinkLocalState::revoke_memory(RuntimeState* state,
    ...
    if (!_shared_state->is_spilled) {
        ...
        operator_custom_profile()->add_info_string("Spilled", "true");
    }
    ...
    auto spill_func = [this, state, query_id, &parent] {
        ...
        auto* sink_local_state = _runtime_state->get_sink_local_state();
        update_profile(sink_local_state->operator_custom_profile());
    ...
    if (status.ok()) {
        ...
        status = ExecEnv::GetInstance()->spill_stream_mgr()->get_spill_io_thread_pool()->submit(
                std::make_shared<SpillSinkRunnable>(state, spill_context, _spill_dependency,
                    operator_custom_profile(), _shared_state->shared_from_this(),
