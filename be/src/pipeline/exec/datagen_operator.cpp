...
Status DataGenLocalState::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _table_function_execution_timer =
            ADD_TIMER(operator_custom_profile(), "TableFunctionExecutionTime");
    _filter_timer = ADD_TIMER(_operator_custom_profile.get(), "FilterTime");
    ...
    for (const auto& filter_desc : p._runtime_filter_descs) {
        ...
        runtime_filter->init_profile(_operator_custom_profile.get());
    }
    return Status::OK();
}
