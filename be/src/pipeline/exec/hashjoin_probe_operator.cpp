...
Status HashJoinProbeLocalState::init(RuntimeState* state, LocalStateInfo& info) {
    ...
    _probe_arena_memory_usage = operator_custom_profile()->AddHighWaterMarkCounter(
            "MemoryUsageProbeKeyArena", TUnit::BYTES, "", 1);
    // Probe phase
    _probe_expr_call_timer = ADD_TIMER(operator_custom_profile(), "ProbeExprCallTime");
    _search_hashtable_timer = ADD_TIMER(operator_custom_profile(), "ProbeWhenSearchHashTableTime");
    _build_side_output_timer = ADD_TIMER(operator_custom_profile(), "ProbeWhenBuildSideOutputTime");
    _probe_side_output_timer = ADD_TIMER(operator_custom_profile(), "ProbeWhenProbeSideOutputTime");
    _non_equal_join_conjuncts_timer =
            ADD_TIMER(operator_custom_profile(), "NonEqualJoinConjunctEvaluationTime");
    _init_probe_side_timer = ADD_TIMER(operator_custom_profile(), "InitProbeSideTime");
    return Status::OK();
}
