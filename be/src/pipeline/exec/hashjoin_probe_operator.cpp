...
Status HashJoinProbeOperatorX::prepare(RuntimeState* state) {
    ...
    const size_t right_col_idx = (_is_right_semi_anti && !_have_other_join_conjunct &&
    ...
    for (const auto* slot : slots_to_check) {
        ...
        } else if (idx - right_col_idx >= _right_table_data_types.size()) {
            return Status::InternalError(
                    "Join node(id={}, OP={}) intermediate slot({}, #{})'s on right table "
                    "idx out bound of _right_table_data_types: {} vs {}(idx = {}, right_col_idx = "
                    "{})",
                    _node_id, _join_op, slot->col_name(), slot->id(), idx - right_col_idx,
                    _right_table_data_types.size(), idx, right_col_idx);
        }
