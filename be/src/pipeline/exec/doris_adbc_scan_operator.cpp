...
Status DorisAdbcScanLocalState::_init_scanners(std::list<vectorized::VScannerSPtr>* scanners) {
    ...
    auto& p = _parent->cast<DorisAdbcScanOperatorX>();
    for (auto& doris_arrow_scan_range : _scan_ranges) {
        ...
        std::shared_ptr<vectorized::NewDorisAdbcScanner> scanner = vectorized::NewDorisAdbcScanner::create_shared(
                vectorized::RuntimeFilterConsumer::_state, this, p._limit, p._tuple_id, doris_arrow_scan_range->uri_str,
