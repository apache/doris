...
struct MultiCastBlock {
    ...
    int _used_count;
    std::atomic_int _un_finish_copy;
    size_t _mem_size;
};
...
private:
    ...
    void _copy_block(vectorized::Block* block, std::atomic_int& un_finish_copy);

    void _wait_copy_block(vectorized::Block* block, std::atomic_int& un_finish_copy);
