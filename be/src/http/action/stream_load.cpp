...
Status StreamLoadAction::_process_put(HttpRequest* http_req, StreamLoadContext* ctx) {
    ...
    if (!http_req->header(HTTP_EXEC_JSONPATH).empty()) {
        request.__set_jsonpath(http_req->header(HTTP_EXEC_JSONPATH));
    }
    if (!http_req->header(HTTP_EXEC_JSONPATH_FILE).empty()) {
        request.__set_jsonpath_file(http_req->header(HTTP_EXEC_JSONPATH_FILE));
    }
