...
int HttpLoadAction::on_header(HttpRequest* req) {
    std::shared_ptr<HttpLoadAction> ctx = std::make_shared<HttpLoadAction>(_exec_env);
    if (req->header(HTTP_SQL).empty()) {
        return Status::InvalidArgument("sql must exist.");
    }
    std::string sql_str = req->header(HTTP_SQL);
    LOG(INFO) << "SQL to be executed:" << sql_str;
    return 0;
}
