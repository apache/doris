...
void PInternalServiceImpl::transmit_data(google::protobuf::RpcController* controller,
    ...
    _transmit_data(controller, request, response, new_done, Status::OK());
}
...
void PInternalServiceImpl::transmit_data_by_http(google::protobuf::RpcController* controller,
    ...
    _transmit_data(controller, new_request, response, new_done, st);
}
...
void PInternalServiceImpl::tablet_writer_add_batch(google::protobuf::RpcController* controller,
    ...
    bool ret = _heavy_work_pool.try_offer([this, controller, request, response, done]() {
        google::protobuf::Closure* new_done = new NewHttpClosure<PTransmitDataParams>(done);
        _tablet_writer_add_batch(controller, request, response, new_done);
    });
    if (!ret) {
        LOG(WARNING) << "fail to offer request to the work pool";
        brpc::ClosureGuard closure_guard(done);
...
void PInternalServiceImpl::tablet_writer_add_batch_by_http(
    ...
    bool ret = _heavy_work_pool.try_offer([this, controller, request, response, done]() {
        ...
        brpc::Controller* cntl = static_cast<brpc::Controller*>(controller);
        Status st = attachment_extract_request_contain_tuple<PTabletWriterAddBatchRequest>(
                new_request, cntl);
        if (st.ok()) {
            _tablet_writer_add_batch(controller, new_request, response, new_done);
        } else {
            st.to_protobuf(response->mutable_status());
        }
    });
...
void PInternalServiceImpl::_tablet_writer_add_batch(google::protobuf::RpcController* controller,
                                                    const PTabletWriterAddBatchRequest* request,
                                                    PTabletWriterAddBatchResult* response,
                                                    google::protobuf::Closure* done) {
