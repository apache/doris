...
int DiskannVectorIndex::calculate_num_pq_chunks() {
    double final_index_ram_limit =
            diskann::get_memory_budget(builderParameterPtr->get_search_ram_budget());
    int num_pq_chunks = diskann::calculate_num_pq_chunks(final_index_ram_limit, npt_num,
                                                         builderParameterPtr->dim);
...
doris::Status DiskannVectorIndex::build() {
    try {
        diskann::generate_quantized_data<float>(
                _data_stream, _pq_pivots_stream, _pq_compressed_stream,
                builderParameterPtr->get_metric_type(), builderParameterPtr->get_sample_rate(),
                calculate_num_pq_chunks(), false, "");
        _data_stream.seekg(0, _data_stream.beg);
        diskann::build_merged_vamana_index<float>(
                _data_stream, builderParameterPtr->get_metric_type(), builderParameterPtr->get_l(),
                builderParameterPtr->get_r(), 1, builderParameterPtr->get_indexing_ram_budget(),
                _vamana_index_stream, "", "", 0, false, builderParameterPtr->get_num_threads(),
                false, "", "", "", 0);
        _data_stream.seekg(0, _data_stream.beg);
...
doris::Status DiskannVectorIndex::save() {
    try {
        ...
        //把stream刷到存储层
        lucene::store::IndexOutput* pq_pivots_output =
                _dir->createOutput(DiskannFileDesc::get_pq_pivots_file_name());
        lucene::store::IndexOutput* pq_compressed_output =
                _dir->createOutput(DiskannFileDesc::get_pq_compressed_file_name());
        lucene::store::IndexOutput* vamana_index_output =
                _dir->createOutput(DiskannFileDesc::get_vamana_index_file_name());
        lucene::store::IndexOutput* disk_layout_output =
                _dir->createOutput(DiskannFileDesc::get_disklayout_file_name());
        lucene::store::IndexOutput* tag_output =
                _dir->createOutput(DiskannFileDesc::get_tag_file_name());
...
doris::Status DiskannVectorIndex::stream_write_to_output(std::stringstream& stream,
    ...
    try {
        ...
        while (stream) { // 只要 stream 仍然有效，就继续读取
            ...
            if (bytes_read > 0) {
                output->writeBytes(reinterpret_cast<const uint8_t*>(buffer.data()),
                                   static_cast<int32_t>(bytes_read));
            }
        }
        return doris::Status::OK();
    } catch (const std::exception& e) {
        return doris::Status::Corruption(std::string("failed stream write to output, message=") +
                                         e.what());
    }
}
...
doris::Status DiskannVectorIndex::load(VectorIndex::Metric dist_fn) {
    ...
    if (dist_fn == VectorIndex::Metric::L2) {
        ...
        return doris::Status::InternalError(
                "Error. Only l2 and mips distance functions are supported");
    }
    lucene::store::IndexInput* pq_pivots_input =
            _dir->openInput(DiskannFileDesc::get_pq_pivots_file_name());
    std::cout << "Actual type: " << typeid(*pq_pivots_input).name() << std::endl;

    lucene::store::IndexInput* pq_compressed_input =
            _dir->openInput(DiskannFileDesc::get_pq_compressed_file_name());
    lucene::store::IndexInput* vamana_index_input =
            _dir->openInput(DiskannFileDesc::get_vamana_index_file_name());
    lucene::store::IndexInput* disk_layout_input =
            _dir->openInput(DiskannFileDesc::get_disklayout_file_name());
    ...
    std::shared_ptr<IndexInputReaderWrapper> pq_pivots_reader(
            new IndexInputReaderWrapper(pq_pivots_input));
    std::shared_ptr<IndexInputReaderWrapper> pq_compressed_reader(
            new IndexInputReaderWrapper(pq_compressed_input));
    std::shared_ptr<IndexInputReaderWrapper> vamana_index_reader(
            new IndexInputReaderWrapper(vamana_index_input));
    std::shared_ptr<IndexInputReaderWrapper> disk_layout_reader(
            new IndexInputReaderWrapper(disk_layout_input));
    std::shared_ptr<IndexInputReaderWrapper> tag_reader(new IndexInputReaderWrapper(tag_input));

    _pFlashIndex =
            std::make_shared<diskann::PQFlashIndex<float, uint16_t>>(disk_layout_reader, metric);
    _pFlashIndex->load(8, pq_pivots_reader, pq_compressed_reader, vamana_index_reader,
                       disk_layout_reader, tag_reader);
    return doris::Status::OK();
}

doris::Status DiskannVectorIndex::search(const float* query_vec, int topk, SearchResult* result,
    ...
    try {
        ...
        uint32_t k = _pFlashIndex->cached_beam_search(
                query_vec, topk, search_list, query_result_ids_64.data(), query_result_dists.data(),
