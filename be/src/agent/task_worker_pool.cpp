...
#define TRACE_POINT() \
    ...
    watch.start(); \
    SCOPED_CLEANUP({ \
        if (config::agent_task_trace_threshold_sec > 0 && \
            watch.elapsed_time() / 1e9 > config::agent_task_trace_threshold_sec) { \
...
void TaskWorkerPool::_drop_tablet_worker_thread_callback() {
    while (_is_work) {
        ...
        if (dropped_tablet != nullptr) {
            ...
            TRACE("StorageEngine drop tablet finish, status: $0", drop_status);
            ...
            TRACE("force drop all txns related with the tablet");
        } else {
...
void TaskWorkerPool::_alter_tablet_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do alter table task");
        ...
        if (!is_task_timeout) {
            ...
            TRACE("finish to do alter tablet task");
            _finish_task(finish_task_request);
        }
        _remove_task_info(agent_task_req.task_type, agent_task_req.signature);
    }
...
void TaskWorkerPool::_publish_version_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do publish version task");
        Status st;
        ...
        while (retry_time < PUBLISH_VERSION_MAX_RETRY) {
            ...
            TRACE("finish do publish version task");
            if (res == OLAP_SUCCESS) {
                break;
            } else {
...
void TaskWorkerPool::_clear_transaction_task_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do clear transaction task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        if (clear_transaction_task_req.transaction_id > 0) {
            ...
            TRACE("finish to clear transaction task");
...
void TaskWorkerPool::_update_tablet_meta_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do update tablet meta task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        TRACE("finish to do update tablet meta task");
...
void TaskWorkerPool::_storage_medium_migrate_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do storage medium migrate");
        ...
        if (_check_migrate_requset(storage_medium_migrate_req, tablet, &dest_store) !=
            ...
            TRACE("finish to do storage medium migrate");
            if (res != OLAP_SUCCESS) {
...
void TaskWorkerPool::_check_consistency_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do check consistency task");
        ...
        OLAPStatus res = _env->storage_engine()->execute_task(&engine_task);
        TRACE("finish to do check consistency task");
        if (res != OLAP_SUCCESS) {
...
void TaskWorkerPool::_report_disk_state_worker_thread_callback() {
    ...
    while (_is_work) {
        ...
        _is_doing_work = true;
        TRACE_POINT();
        TRACE("begin to do disk state report task");
        ...
        TRACE("finish to do disk state report task");
    }
...
void TaskWorkerPool::_report_tablet_worker_thread_callback() {
    ...
    while (_is_work) {
      ...
      _is_doing_work = true;
        TRACE_POINT();
        TRACE("begin to do tablet worker report task");
        ...
        TRACE("finish to do tablet worker report task");
    }
    StorageEngine::instance()->deregister_report_listener(this);
}
...
void TaskWorkerPool::_upload_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE("finish to do upload task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        if (!status.ok()) {
            status_code = TStatusCode::RUNTIME_ERROR;
...
void TaskWorkerPool::_download_worker_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do download task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        TRACE("finish to do download task");
        if (!status.ok()) {
            status_code = TStatusCode::RUNTIME_ERROR;
...
void TaskWorkerPool::_make_snapshot_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do snapshot task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        TRACE("finish to do snapshot task");
...
void TaskWorkerPool::_release_snapshot_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do release snapshot task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        TRACE("finish to do release snapshot task");
...
void TaskWorkerPool::_move_dir_thread_callback() {
    while (_is_work) {
        ...
        TRACE_POINT();
        TRACE("begin to do move dir task");
        TStatusCode::type status_code = TStatusCode::OK;
        std::vector<string> error_msgs;
        TStatus task_status;
        ...
        TRACE("finish to do move dir task");
        if (status != DORIS_SUCCESS) {
            status_code = TStatusCode::RUNTIME_ERROR;
...
void TaskWorkerPool::_handle_report(TReportRequest& request, ReportType type) {
    TRACE("begin to do report task");
    TMasterResult result;
    AgentStatus status = _master_client->report(request, &result);
    TRACE("finish to do report task");
