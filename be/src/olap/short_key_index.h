...
public:
    ...
    uint64_t size() { return _key_buf.size() + _offset_buf.size() + _footer_buf.size(); }
...
public:
    ...
    ShortKeyIndexIterator lower_bound(const Slice& key) const { return seek<true>(key); }

    // Return the iterator which locates the first item greater than the
    // input key.
    ShortKeyIndexIterator upper_bound(const Slice& key) const { return seek<false>(key); }
