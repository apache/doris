...
public:
    ...
    OLAPStatus check_condition_valid(TabletPtr table, const TCondition& cond);
    ...
    OLAPStatus delete_cond(
            TabletPtr table, const int32_t version, bool delete_smaller_version_conditions);
    ...
    // 返回值：
    //     OLAP_SUCCESS：调用成功
    OLAPStatus log_conds(TabletPtr table);
private:

    // 检查指定的删除条件版本是否符合要求；
    // 如果不符合要求，返回OLAP_ERR_DELETE_INVALID_VERSION；符合要求返回OLAP_SUCCESS
    OLAPStatus _check_version_valid(TabletPtr table, const int32_t filter_version);

    // 检查指定版本的删除条件是否已经存在。如果存在，返回指定版本删除条件的数组下标；不存在返回-1
    int _check_whether_condition_exist(TabletPtr, int cond_version);
};
...
public:
    ...
    OLAPStatus init(TabletPtr olap_table, int32_t version);
