...
public:
    ...
    // 3. segments_overlap() flag is not NONOVERLAPPING and not OVERLAP_NONE (rowset is created before v0.11) 
    //  (OVERLAP_UNKNOWN and OVERLAPPING are OK)
    bool is_segments_overlapping() const {
        return num_segments() > 1 && is_singleton_delta() &&
               !(segments_overlap() == NONOVERLAPPING || segments_overlap() == OVERLAP_NONE);
    }
