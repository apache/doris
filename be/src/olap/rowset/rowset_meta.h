...
public:
    ...
    // so segments non overlapping is defined as
    ...
    // 3. there is no segments in rowset (no data, or a delete version)
    bool is_segments_overlapping() const {
        if (end_version() > start_version() || segments_overlap() == NONOVERLAPPING) {
            return false;
        }

        // some of rowsets may have no segments.(no data, or a delete version)
        if (num_segments() == 0) {
            return false;
        }
        return true;
