...
public:
    ...
    void set_segment_rows(const std::vector<uint32_t>& segment_rows) {
        _rowset_meta_pb.mutable_segment_rows()->Assign(segment_rows.cbegin(), segment_rows.cend());
    }

    void get_segment_rows(std::vector<uint32_t>* segment_rows) const {
        segment_rows->assign(_rowset_meta_pb.segment_rows().cbegin(),
                             _rowset_meta_pb.segment_rows().cend());
    }

    auto& get_segment_rows() const { return _rowset_meta_pb.segment_rows(); }
