...
Status SegmentReader::_load_segment_file() {
...
Status SegmentReader::_set_decompressor() {
...
Status SegmentReader::_set_segment_info() {
...
Status SegmentReader::init(bool is_using_cache) {
...
Status SegmentReader::get_block(VectorizedRowBatch* batch, uint32_t* next_block_id, bool* eof) {
    if (_eof) {
        *eof = true;
        return Status::OK();
    }
...
Status SegmentReader::_pick_columns() {
...
Status SegmentReader::_pick_delete_row_groups(uint32_t first_block, uint32_t last_block) {
...
Status SegmentReader::_init_include_blocks(uint32_t first_block, uint32_t last_block) {
...
Status SegmentReader::_pick_row_groups(uint32_t first_block, uint32_t last_block) {
    ...
    Status res = _init_include_blocks(first_block, last_block);
    if (!res.ok()) {
        return res;
    }
...
Status SegmentReader::_load_index(bool is_using_cache) {
    Status res = Status::OK();
    ...
    res = stream.init();
    if (!res.ok()) {
        LOG(WARNING) << "fail to init stream. res = " << res;
        return res;
    }
    ...
    for (int64_t stream_index = 0; stream_index < _header_message().stream_info_size();
        ...
        if (message.kind() == StreamInfoMessage::ROW_INDEX) {
            ...
            if (!res.ok()) {
                LOG(WARNING) << "fail to init bloom filter reader. res = " << res;
                return res;
            }
...
Status SegmentReader::_read_all_data_streams(size_t* buffer_size) {
...
Status SegmentReader::_create_reader(size_t* buffer_size) {
...
Status SegmentReader::_reset_readers() {
...
Status SegmentReader::_load_to_vectorized_row_batch(VectorizedRowBatch* batch, size_t size) {
