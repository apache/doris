...
Status BetaRowsetWriter::add_block(const vectorized::Block* block) {
    ...
    if (UNLIKELY(_segment_writer == nullptr)) {
        OLAP_RETURN_NOT_OK(_create_segment_writer(&_segment_writer, block));
    }
...
Status BetaRowsetWriter::_find_longest_consecutive_small_segment(
    ...
    OLAP_RETURN_NOT_OK(_load_noncompacted_segments(&all_segments, _num_segment - 1));
    ...
    for (int64_t i = 0; i < all_segments.size(); ++i) {
        ...
        if (seg->num_rows() > small_threshold) {
            if (let_big_terminate) {
                ...
                OLAP_RETURN_NOT_OK(_rename_compacted_segment_plain(_segcompacted_point++));
            }
        } else {
...
Status BetaRowsetWriter::_rename_compacted_segments(int64_t begin, int64_t end) {
    ...
    OLAP_RETURN_NOT_OK(_rename_compacted_indices(begin, end, 0));
...
Status BetaRowsetWriter::_rename_compacted_segment_plain(uint64_t seg_id) {
    ...
    OLAP_RETURN_NOT_OK(_rename_compacted_indices(-1, -1, seg_id));
...
Status BetaRowsetWriter::_get_segcompaction_candidates(SegCompactionCandidatesSharedPtr& segments,
    ...
    if (is_last) {
        ...
        OLAP_RETURN_NOT_OK(_load_noncompacted_segments(segments.get(), _num_segment));
        for (int i = 0; i < segments->size(); ++i) {
            OLAP_RETURN_NOT_OK(_rename_compacted_segment_plain(_segcompacted_point++));
        }
        segments->clear();
    } else {
        OLAP_RETURN_NOT_OK(_find_longest_consecutive_small_segment(segments));
    }
    return Status::OK();
}
...
Status BetaRowsetWriter::_add_block(const vectorized::Block* block,
    ...
    do {
        ...
        if (UNLIKELY(max_row_add < 1)) {
            ...
            OLAP_RETURN_NOT_OK(_flush_segment_writer(segment_writer));
            OLAP_RETURN_NOT_OK(_create_segment_writer(segment_writer, block));
...
Status BetaRowsetWriter::add_rowset(RowsetSharedPtr rowset) {
    ...
    OLAP_RETURN_NOT_OK(rowset->link_files_to(_context.rowset_dir, _context.rowset_id));
...
Status BetaRowsetWriter::flush() {
    if (_segment_writer != nullptr) {
        OLAP_RETURN_NOT_OK(_flush_segment_writer(&_segment_writer));
    }
    return Status::OK();
}
...
Status BetaRowsetWriter::flush_single_memtable(const vectorized::Block* block, int64* flush_size) {
    ...
    OLAP_RETURN_NOT_OK(_create_segment_writer(&writer, block));
    OLAP_RETURN_NOT_OK(_add_block(block, &writer));
    OLAP_RETURN_NOT_OK(_flush_segment_writer(&writer, flush_size));
    OLAP_RETURN_NOT_OK(_segcompaction_if_necessary());
    return Status::OK();
}
