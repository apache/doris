...
Status BetaRowset::upload_to(io::RemoteFileSystem* dest_fs, const RowsetId& new_rowset_id) {
    ...
    if (st.ok()) {
        ...
        DorisBvarMetrics::instance()->upload_rowset_count->increment(1);
        DorisBvarMetrics::instance()->upload_total_byte->increment(data_disk_size());
    } else {
        DorisMetrics::instance()->upload_fail_count->increment(1);
        DorisBvarMetrics::instance()->upload_fail_count->increment(1);
    }
    return st;
}
