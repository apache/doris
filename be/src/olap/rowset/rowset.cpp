...
int64_t Rowset::approximate_cache_data_size() {
    if (!config::enable_file_cache) {
        return 0;
    }
    ...
    for (int seg_id = 0; seg_id < num_segments(); ++seg_id) {
        ...
        LOG(INFO) << "get segment cache file size by path, rowset_id=" << rowset_id()
                  << ", seg_id=" << seg_id << ", cache_size=" << cache_size;
    }
...
int64_t Rowset::approximate_cache_index_size() {
    ...
    for (const auto& file_name : file_names) {
        ...
        int64_t cache_size =
                io::FileCacheFactory::instance()->get_cache_file_size_by_path(cache_key);
        total_cache_size += cache_size;
        LOG(INFO) << "get inverted index cache file size by path, file_name=" << file_name
