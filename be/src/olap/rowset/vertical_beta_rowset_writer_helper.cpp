...
Status VerticalBetaRowsetWriterHelper::_flush_columns(
...
Status VerticalBetaRowsetWriterHelper::flush_columns(bool is_key) {
    if (_segment_writers->empty()) {
        return Status::OK();
    }
...
Status VerticalBetaRowsetWriterHelper::final_flush() {
    for (auto& segment_writer : *_segment_writers) {
