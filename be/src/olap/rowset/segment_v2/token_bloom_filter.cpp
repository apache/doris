...
#include "olap/rowset/segment_v2/token_bloom_filter.h"
...
TokenBloomFilter::TokenBloomFilter(size_t size)
        : _size(size),
          words((size + sizeof(UnderType) - 1) / sizeof(UnderType)),
          filter(words, 0) {}

// for read
Status TokenBloomFilter::init(const char* buf, uint32_t size, HashStrategyPB strategy) {
    if (size == 0) {
...
void TokenBloomFilter::add_bytes(const char* data, uint32_t len) {
...
bool TokenBloomFilter::contains(const BloomFilter& bf_) const {
    const TokenBloomFilter& bf = static_cast<const TokenBloomFilter&>(bf_);
    for (size_t i = 0; i < words; ++i) {
