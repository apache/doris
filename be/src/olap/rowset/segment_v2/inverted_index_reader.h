...
public:
    InvertedIndexVisitor(roaring::Roaring* hits, InvertedIndexQueryBase* range_query,
    ...
    //bool _matches(uint8_t* packed_value, const uint8_t* query_max, const uint8_t* query_min);
    bool _matches(const BinaryType& packed_value, const BinaryType& query_max,
                  const BinaryType& query_min);
    lucene::util::bkd::relation compare(std::vector<uint8_t>& min_packed,
                                        std::vector<uint8_t>& max_packed) override;
    //lucene::util::bkd::relation _compare(std::vector<uint8_t>& min_packed,
    //                                     std::vector<uint8_t>& max_packed, const uint8_t* query_max,
    //                                     const uint8_t* query_min);
...
public:
    ...
    Status try_query(OlapReaderStatistics* stats, const std::string& column_name,
                     InvertedIndexQueryBase* query_value, uint32_t* count) override;
    Status bkd_query(std::shared_ptr<lucene::util::bkd::bkd_reader> r,
