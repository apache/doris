...
Status SegmentWriter::append_block_with_partial_content(const vectorized::Block* block,
    ...
    if (is_unique_key_replace_if_not_null) {
        ...
        calc_indicator_maps(row_pos, num_rows, indicator_maps_vertical);
    } else {
    ...
    RETURN_IF_ERROR(_olap_data_convertor->set_source_content_with_specifid_columns(
            &full_block, row_pos, num_rows, determistic_cids));
    ...
    RETURN_IF_ERROR(_fill_missing_columns(
            &full_block, read_plan, missing_cids, point_read_cids, use_default_or_null_flag,
            has_default_or_nullable, segment_start_pos, is_unique_key_replace_if_not_null, block));
    ...
    RETURN_IF_ERROR(_olap_data_convertor->set_source_content_with_specifid_columns(
            &full_block, row_pos, num_rows, remaining_cids));
