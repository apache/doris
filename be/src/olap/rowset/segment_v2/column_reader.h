...
struct SharedColumnCache {
    const ColumnIteratorUPtr _column_iterator = nullptr;
    vectorized::MutableColumnPtr _column = nullptr;
    ...
    SharedColumnCache(ColumnIteratorUPtr column_iterator, vectorized::MutableColumnPtr column)
            : _column_iterator(std::move(column_iterator)), _column(std::move(column)) {}
    ...
    Status seek_to_ordinal(ordinal_t ord) {
        if (_state == State::SEEKED_NEXT_BATCHED) {
            return Status::OK();
        }
        _state = State::SEEKED_NEXT_BATCHED;
        _column->clear();
        _length = -1;
        _has_null = false;
        return _column_iterator->seek_to_ordinal(ord);
    }

    Status next_batch(size_t* n, bool* has_null) {
        if (_length != -1) {
            DCHECK(_state == State::SEEKED_NEXT_BATCHED);
            *has_null = _has_null;
            *n = static_cast<size_t>(_length);
            return Status::OK();
        }
        _column->clear();
        DCHECK(_state == State::SEEKED_NEXT_BATCHED);
        RETURN_IF_ERROR(_column_iterator->next_batch(n, _column, has_null));
        _length = static_cast<int64_t>(*n);
        _has_null = *has_null;
        return Status::OK();
    }

    Status read_by_rowids(const rowid_t* rowids, const size_t count) {
        if (_state == State::READ_BY_ROWIDS) {
            DCHECK(_length == count);
            return Status::OK();
        }
        _column->clear();
        _state = State::READ_BY_ROWIDS;
        RETURN_IF_ERROR(_column_iterator->read_by_rowids(rowids, count, _column));
        _length = static_cast<int64_t>(count);
        return Status::OK();
    }
...
using SharedColumnCacheSPtr = std::shared_ptr<SharedColumnCache>;
using PathToSharedColumnCacheUPtr =
        std::unique_ptr<std::unordered_map<std::string, SharedColumnCacheSPtr>>;
