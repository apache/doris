...
Status HierarchicalDataIterator::seek_to_first() {
    throw doris::Exception(INTERNAL_ERROR,
                           "HierarchicalDataIterator::seek_to_first is not implemented");
}
...
Status HierarchicalDataIterator::_process_nested_columns(
    ...
    for (const auto& entry : nested_subcolumns) {
        ...
        MutableColumnPtr nested_object =
                ColumnObject::create(0 /*no sparse column*/, base_array->get_data().size());
...
Status HierarchicalDataIterator::_process_sparse_column(vectorized::ColumnObject& container_variant,
                                                        size_t nrows) {
    using namespace vectorized;
