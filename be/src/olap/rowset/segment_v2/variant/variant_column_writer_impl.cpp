...
Status UnifiedSparseColumnWriter::init(const TabletColumn* parent_column, int bucket_num,
    ...
    if (_bucket_num <= 1) {
        TabletColumn sparse_column = vectorized::schema_util::create_sparse_column(*parent_column);
...
Status UnifiedSparseColumnWriter::append_bucket_sparse(
    ...
    for (ssize_t row = 0; row < static_cast<ssize_t>(num_rows); ++row) {
        ...
        for (size_t i = start; i < end; ++i) {
            ...
            uint32_t b = vectorized::schema_util::variant_binary_shard_of(path, bucket_num);
...
Status VariantDocSnapShotWriter::init(const TabletColumn* parent_column, int bucket_num,
                                      int& column_id, const ColumnWriterOptions& opts,
                                      SegmentFooterPB* footer) {
...
Status VariantDocSnapShotWriter::append_data(const TabletColumn* parent_column,
    ...
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        for (size_t i = start; i < end; ++i) {
            StringRef path = paths_col->get_data_at(i);
            uint32_t bucket = vectorized::schema_util::variant_binary_shard_of(path, _bucket_num);
...
Status VariantColumnWriterImpl::_process_sparse_column(
    ...
    RETURN_IF_ERROR(
            _sparse_writer.init(_tablet_column, bucket_num, column_id, _opts, _opts.footer));
    RETURN_IF_ERROR(_sparse_writer.append_data(_tablet_column, *ptr, num_rows, converter));
    return Status::OK();
}
...
uint64_t VariantColumnWriterImpl::estimate_buffer_size() {
    ...
    size += _doc_snapshot_writer.estimate_buffer_size();
    return size;
}
...
Status VariantColumnWriterImpl::finish() {
    ...
    RETURN_IF_ERROR(_doc_snapshot_writer.finish());
    return Status::OK();
}
Status VariantColumnWriterImpl::write_data() {
    ...
    RETURN_IF_ERROR(_doc_snapshot_writer.write_data());
    return Status::OK();
}
Status VariantColumnWriterImpl::write_ordinal_index() {
    ...
    RETURN_IF_ERROR(_doc_snapshot_writer.write_ordinal_index());
    return Status::OK();
}
...
Status VariantCompactionDocSnapshotWriter::append_data(const uint8_t** ptr, size_t num_rows) {
    const auto* column = reinterpret_cast<const vectorized::VariantColumnData*>(*ptr);
    const auto& src = *reinterpret_cast<const vectorized::ColumnVariant*>(column->column_data);
...
uint64_t VariantCompactionDocSnapshotWriter::estimate_buffer_size() {
    return _column->byte_size();
}
