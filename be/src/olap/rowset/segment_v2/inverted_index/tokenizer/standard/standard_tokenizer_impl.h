        ...
        while (true) {
            ...
            while (true) {
                if (zzCurrentPosL < zzEndReadL) {
                    size_t len = 0;
                    zzInput = decode_utf8_to_codepoint(
                            std::string_view(zzBuffer.data() + zzCurrentPosL, zzEndReadL), len);
                    if (zzInput == -1) {
                        return YYEOF;
                    }
                    zzCurrentPosL += len;
...
private:
    uint32_t decode_utf8_to_codepoint(const std::string_view& utf8, size_t& bytesRead) {
        if (utf8.empty()) {
            return -1;
        }

        auto firstByte = static_cast<uint8_t>(utf8[0]);
        bytesRead = 0;
