...
public:
    ...
    virtual bool isTokenChar(UChar32 c) = 0;
};
...
public:
    ...
    bool isTokenChar(UChar32 c) override {
        switch (_type) {
        case Type::LETTER:
            return u_isalpha(c);
        case Type::DIGIT:
...
public:
    ...
    bool isTokenChar(UChar32 c) override { return _code_points.containsNone(c); }
...
public:
    ...
    bool isTokenChar(UChar32 c) override {
        return std::ranges::any_of(_matchers,
                                   [c](const auto& matcher) { return matcher->isTokenChar(c); });
    }
