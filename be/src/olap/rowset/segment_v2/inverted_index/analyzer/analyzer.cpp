...
#include "olap/rowset/segment_v2/inverted_index/analyzer/simple/simple_analyzer.h"
...
std::unique_ptr<lucene::analysis::Analyzer> InvertedIndexAnalyzer::create_analyzer(
    ...
    } else if (analyser_type == InvertedIndexParserType::PARSER_SIMPLE) {
        analyzer = std::make_unique<SimpleAnalyzer>();
    } else {
        // default
        analyzer = std::make_unique<lucene::analysis::SimpleAnalyzer<char>>();
    }
