...
template <typename TDocSet, typename TOtherDocSet>
class Intersection final : public DocSet {
public:
    Intersection(TDocSet left, TDocSet right, std::vector<TOtherDocSet> others);
...
private:
    static uint32_t go_to_first_doc(const std::vector<TDocSet>& docsets);
