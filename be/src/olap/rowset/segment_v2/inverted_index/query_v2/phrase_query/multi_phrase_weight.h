...
public:
    MultiPhraseWeight(IndexQueryContextPtr context, std::wstring field,
                      std::vector<TermInfo> term_infos, bool nullable)
    ...
    ScorerPtr phrase_scorer(const QueryExecutionContext& ctx, const std::string& binding_key) {
        ...
        for (const auto& term_info : _term_infos) {
            ...
            if (term_info.is_single_term()) {
                auto posting = create_position_posting(
                        reader.get(), _field, term_info.get_single_term(), _context->io_ctx);
                if (posting) {
                ...
                for (const auto& term : terms) {
                    auto posting =
                            create_position_posting(reader.get(), _field, term, _context->io_ctx);
                    if (posting) {
                        if (posting->size_hint() <= SPARSE_TERM_DOC_THRESHOLD) {
        ...
        return PhraseScorer<PostingsPtr>::create(term_postings_list, 0, num_docs);
    }
