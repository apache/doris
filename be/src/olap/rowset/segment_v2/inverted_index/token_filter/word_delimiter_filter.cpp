    ...
    while (true) {
        ...
        if (_iterator->_end == WordDelimiterIterator::DONE) {
            ...
            if (_buffered_pos < _buffered_len) {
                ...
                const auto& term = _buffered[_buffered_pos++];
        ...
        if (_iterator->is_single_word()) {
            ...
            _iterator->next();
            _first = false;
            t->setNoCopy(_attribute->termBuffer<char>(), 0, _attribute->termLength<char>());
            return t;
        }
...
void WordDelimiterFilter::buffer() {
    if (_buffered_len == _buffered.size()) {
        _buffered.resize(_buffered.size() * 2);
        _pos_inc.resize(_buffered.size() * 2);
    }
    _pos_inc[_buffered_len] = _attribute->getPositionIncrement();
    _buffered[_buffered_len] =
