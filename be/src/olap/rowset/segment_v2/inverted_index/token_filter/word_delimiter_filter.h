...
public:
    WordDelimiterFilter(const TokenStreamPtr& in, std::vector<char> char_type_table,
            ...
            : DorisTokenFilter(in),
              ...
              _buffered(8),
              _pos_inc(8) {
        _iterator = std::make_unique<WordDelimiterIterator>(
                char_type_table, has(SPLIT_ON_CASE_CHANGE), has(SPLIT_ON_NUMERICS),
...
private:
    ...
    std::vector<std::string> _buffered;
    std::vector<int32_t> _pos_inc;
...
public:
    ...
    void write(Token* t) {
        t->setNoCopy(_buffer.data(), 0, _buffer.size());
        t->setPositionIncrement(_filter.position(true));
