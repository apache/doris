...
std::vector<float> BM25Similarity::LENGTH_TABLE = initialize_length_table();

std::vector<float> BM25Similarity::initialize_length_table() {
    std::vector<float> length_table(256);
    for (int32_t i = 0; i < 256; i++) {
        LENGTH_TABLE[i] = int_to_byte4(i);
    }
    return length_table;
}
