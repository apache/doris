...
private:
    ...
    Status _output_column_by_ret_flags(vectorized::Block* block,
                                       const std::vector<uint32_t>& column_ids, uint8_t* ret_flags,
                                       uint16_t batch_size, uint16_t selected_size) {
        SCOPED_RAW_TIMER(&_opts.stats->output_col_ns);
        if (0 != selected_size){
            for (auto cid : column_ids) {
                int block_cid = _schema_block_id_map[cid];
