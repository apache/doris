...
public:
    ...
    std::string to_string() {
        if constexpr (Type == TYPE_LARGEINT || Type == TYPE_DECIMAL128I) {
            std::stringstream ss;
            fmt::memory_buffer buffer;

            if (is_point_query()) {
                ss << inverted_index_query_op_to_string(point_op()) << " ";
                for (auto& value : get_fixed_value_set()) {
                    buffer.clear();
                ...
                if (has_lower_bound()) {
                    buffer.clear();
                    fmt::format_to(buffer, "{}", lower_value());
                    ss << std::string(buffer.data(), buffer.size()) << " "
                ...
                if (has_upper_bound()) {
                    ...
                    fmt::format_to(buffer, "{}", lower_value());
                    ss << std::string(buffer.data(), buffer.size()) << " "
                       << inverted_index_query_op_to_string(upper_op()) << " ";
                }
            }
            return ss.str();
        } else {
            ...
            if (is_point_query()) {
                ...
                if (has_lower_bound()) {
                    ss << lower_value() << " " << inverted_index_query_op_to_string(lower_op())
                       << " ";
                }
                if (has_upper_bound()) {
                    ss << upper_value() << " " << inverted_index_query_op_to_string(upper_op())
                       << " ";
                }
            }
            return ss.str();
