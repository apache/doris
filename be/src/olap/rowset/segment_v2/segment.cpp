...
Status Segment::new_iterator(SchemaSPtr schema, const StorageReadOptions& read_options,
    ...
    for (auto& entry : read_options.col_id_to_predicates) {
        ...
        std::shared_ptr<ColumnReader> reader;
        RETURN_IF_ERROR(_get_column_reader(col, &reader));
...
Status Segment::_new_iterator_with_variant_root(const TabletColumn& tablet_column,
    ...
    RETURN_IF_ERROR(_get_column_reader(vectorized::PathInDataRef(&root->path),
                                       ...
                                       &reader));
    RETURN_IF_ERROR(reader->new_iterator(&it));
...
Status Segment::new_column_iterator_with_path(const TabletColumn& tablet_column,
    ...
    auto new_default_iter_with_same_nested = [&](const TabletColumn& tablet_column,
        ...
        if (parent && SubcolumnColumnReaders::find_parent(
            ...
            RETURN_IF_ERROR(reader->new_iterator(&sibling_iter_ptr));
    ...
    if (node != nullptr) {
        if (node->is_leaf_node() && sparse_node == nullptr) {
            ...
            RETURN_IF_ERROR(HierarchicalDataReader::create(iter, relative_path, unique_id, node,
                                                           root, read_type, shared_from_this()));
        }
    } else {
...
Status Segment::_get_column_reader(const TabletColumn& col,
    ...
    if (col.has_path_info()) {
        ...
        return _get_column_reader(vectorized::PathInDataRef(&relative_path), col_uid,
                                  column_reader);
    }
    return _get_column_reader(vectorized::PathInDataRef(), col_uid, column_reader);
}
