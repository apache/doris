...
Status Segment::open(io::FileSystemSPtr fs, const std::string& path, int64_t tablet_id,
    ...
    auto s = _open(fs, path, segment_id, rowset_id, tablet_schema, reader_options, output,
                   idx_file_info, stats, is_limit_io);
    if (!s.ok()) {
...
Status Segment::_open(io::FileSystemSPtr fs, const std::string& path, uint32_t segment_id,
    ...
    auto st = segment->_open(stats, is_limit_io);
    ...
    if (st.is<ErrorCode::CORRUPTION>() &&
        ...
        RETURN_IF_ERROR(fs->open_file(path, &file_reader, &reader_options));
        segment->_file_reader = std::move(file_reader);
        st = segment->_open(stats, is_limit_io);
...
Status Segment::_open(OlapReaderStatistics* stats, bool is_limit_io) {
    std::shared_ptr<SegmentFooterPB> footer_pb_shared;
    RETURN_IF_ERROR(_get_segment_footer(footer_pb_shared, stats, is_limit_io));
...
Status Segment::new_iterator(SchemaSPtr schema, const StorageReadOptions& read_options,
    ...
    RETURN_IF_ERROR(
            _create_column_readers_once(read_options.stats, read_options.io_ctx.is_limit_io));
...

    {
        ...
        RETURN_IF_ERROR(load_index(read_options.stats, read_options.io_ctx.is_limit_io));
    }
...
Status Segment::load_index(OlapReaderStatistics* stats, bool is_limit_io) {
    return _load_index_once.call([this, stats, is_limit_io] {
...
Status Segment::_create_column_readers_once(OlapReaderStatistics* stats, bool is_limit_io) {
...
Status Segment::new_column_iterator(const TabletColumn& tablet_column,
    ...
    RETURN_IF_ERROR(_create_column_readers_once(opt->stats, opt->io_ctx.is_limit_io));
...
Status Segment::_get_segment_footer(std::shared_ptr<SegmentFooterPB>& footer_pb,
                                    OlapReaderStatistics* stats, bool is_limit_io) {
    std::shared_ptr<SegmentFooterPB> footer_pb_shared = _footer_pb.lock();
    ...
    if (!segment_footer_cache->lookup(cache_key, &cache_handle,
        ...
        RETURN_IF_ERROR(_parse_footer(footer_pb_shared, stats, is_limit_io));
