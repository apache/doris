...
Status Segment::new_iterator(SchemaSPtr schema, const StorageReadOptions& read_options,
                             std::unique_ptr<RowwiseIterator>* iter, long tz_offset) {
    ...
    if (config::ignore_always_true_predicate_for_segment &&
        ...
        if (pruned) {
            ...
            return iter->get()->init(options_with_pruned_predicates, tz_offset);
        }
    }
    return iter->get()->init(read_options, tz_offset);
}
...
static Status new_default_iterator(const TabletColumn& tablet_column,
    ...
    RETURN_IF_ERROR(default_value_iter->init(iter_opts, 0)); //TODOZY
...
Status Segment::seek_and_read_by_rowid(const TabletSchema& schema, SlotDescriptor* slot,
    ...
    if (!slot->column_paths().empty()) {
        ...
        if (iterator_hint == nullptr) {
            ...
            RETURN_IF_ERROR(iterator_hint->init(opt, tz_offset));
        }
        RETURN_IF_ERROR(
