...
Status SegmentIterator::_get_row_ranges_by_keys() {
    ...
    if (_opts.split_key_range.valid) {
        ...
        if (_opts.split_key_range.upper_key.cursor_key) {
            RETURN_IF_ERROR(_lookup_ordinal(*(_opts.split_key_range.upper_key.cursor_key),
                                            !_opts.split_key_range.upper_key.include, num_rows(),
                                            &upper_rowid));
        } else if (!_opts.split_key_range.upper_key.key.empty()) {
        ...
        if (_opts.split_key_range.lower_key.cursor_key) {
            RETURN_IF_ERROR(_lookup_ordinal(*(_opts.split_key_range.lower_key.cursor_key),
                                            _opts.split_key_range.lower_key.include, upper_rowid,
                                            &lower_rowid));
        } else if (!_opts.split_key_range.lower_key.key.empty()) {
            RETURN_IF_ERROR(_lookup_ordinal_from_sk_index<false>(
...
Status SegmentIterator::_lookup_ordinal_from_sk_index(const Slice& index_key, bool is_include,
    ...
    while (start < end) {
        ...
        if constexpr (HasRowCursor) {
            ...
            auto seek_index = _encode_seek_block_to_short_key(key_schema->column_ids());
...
std::string SegmentIterator::_encode_seek_block_to_short_key(const std::vector<ColumnId>& col_ids) {
    std::string key;
