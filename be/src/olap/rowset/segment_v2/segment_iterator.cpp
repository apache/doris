...
void SegmentIterator::_evaluate_short_circuit_predicate(uint16_t* vec_sel_rowid_idx,
    ...
    for (auto predicate : _short_cir_eval_predicate) {
        ...
        // If lazy materialization is disabled, the engine will read all columns data from
        // filesystem, if we already read all columns data, it is useless to elimate the block
        // using bloomfilter. Because the bloom filter is generated by join condition, evaluate
        // the column using bloom filter is more heavy than evaluate using hash table during join
        // exec node.
        if (!_lazy_materialization_read && predicate->type() == PredicateType::BF) {
            continue;
        }
