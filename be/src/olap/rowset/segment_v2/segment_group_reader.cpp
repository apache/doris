...
SegmentGroupIterator::SegmentGroupIterator(SegmentGroup* segment_group)
...
SegmentGroupIterator::~SegmentGroupIterator() {
    _segment_group->release();
    SAFE_DELETE(_segment_reader);
}

Status SegmentGroupIterator::init() {
...
Status SegmentGroupIterator::get_next_block(RowBlock** row_block) {
...
Status SegmentGroupIterator::_next_row(const RowCursor** row, bool without_filter) {
    _read_block->pos_inc();
    do {
...
Status SegmentGroupIterator::_seek_to_block(const RowBlockPosition& block_pos, bool without_filter) {
...
Status SegmentGroupIterator::_find_position_by_short_key(
...
Status SegmentGroupIterator::_find_position_by_full_key(
        const RowCursor& key, bool find_last_key, RowBlockPosition *position) {
    RowBlockPosition tmp_pos;
...
Status SegmentGroupIterator::_seek_to_row(const RowCursor& key, bool find_last_key, bool is_end_key) {
...
Status SegmentGroupIterator::_get_first_row_block(RowBlock** row_block) {
    DCHECK(!_end_key_is_set) << "end key is set while use block interface.";
    _is_normal_read = true;
    _eof = false;

    RETURN_IF_ERROR(_schema_change_init());

    // to be same with OLAPData, we use segment_group.
    RowBlockPosition block_pos;
    auto res = segment_group()->find_first_row_block(&block_pos);
    if (res != OLAP_SUCCESS) {
        if (res == OLAP_ERR_INDEX_EOF) {
            *row_block = nullptr;
            _eof = true;
...
OLAPStatus SegmentGroupIterator::get_next_row_block(RowBlock** row_block) {
    auto st = _get_next_row_block(row_block);
    if (res != OLAP_SUCCESS) {
        if (res != OLAP_ERR_DATA_EOF) {
            OLAP_LOG_WARNING("fail to load data to row block. [res=%d]", res);
        }
...
Status SegmentGroupIterator::_get_next_row_block(RowBlock** row_block) {
    _is_normal_read = true;
    RETURN_IF_ERROR(_get_block(false));
    *row_block = _read_block.get();
    return Status::OK();
}

bool SegmentGroupIterator::delta_pruning_filter() {
...
int SegmentGroupIterator::delete_pruning_filter() {
    if (empty() || zero_num_rows()) {
...
uint64_t SegmentGroupIterator::get_filted_rows() {
    return _stats->rows_del_filtered;
}

Status SegmentGroupIterator::_schema_change_init() {
    _is_using_cache = false;
...
Status SegmentGroupIterator::_get_block_from_reader(
    ...
    do {
        ...
        if (!_segment_eof) {
            ...
            RETURN_IF_ERROR(_segment_iterator->next_batch(vec_batch, &_segment_eof));
            // Normal case
            *got_batch = vec_batch;
            return Status::OK();
        }
        ...
        RETURN_IF_ERROR(_seek_to_block(block_pos, without_filter));
    } while (true);

    return OLAP_SUCCESS;
}

Status SegmentGroupIterator::_get_block(bool without_filter, int rows_read) {
    do {
        VectorizedRowBatch* vec_batch = nullptr;
        RETURN_IF_ERROR(_get_block_from_reader(&vec_batch, without_filter, rows_read));
        // evaluate predicates
