...
Status ColumnReader::merge_zone_maps(const std::vector<ZoneMapPB>& zone_maps,
    ...
    for (const auto& zone_map_ : zone_maps) {
        ...
        RETURN_IF_ERROR(_parse_zone_map(zone_map_, field_type, field_length, min, max));
        ...
        zone_map.set_has_not_null(true);
    }

    if (min_value) {
        *(zone_map.mutable_min()) = min_value->to_string();
    }
    if (max_value) {
        *(zone_map.mutable_max()) = max_value->to_string();
    }
...
bool ColumnReader::match_zone_map_condition(const AndBlockColumnPredicate* predicate,
    ...
    std::unique_ptr<WrapperField> min_value;
    std::unique_ptr<WrapperField> max_value;
    THROW_IF_ERROR(_parse_zone_map(zone_map, field_type, field_length, min_value, max_value));
