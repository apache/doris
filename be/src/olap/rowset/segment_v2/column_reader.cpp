...
Status ColumnReader::init() {
    ...
    for (int i = 0; i < _meta.indexes_size(); i++) {
        ...
        case BLOOM_FILTER_INDEX:
            ...
            break;
        default:
            return Status::Corruption(Substitute("Bad file $0: invalid column index type $1",
                                                 _file_name, index_meta.type()));
        }
    }
    if (_ordinal_index_meta == nullptr) {
        return Status::Corruption(Substitute("Bad file $0: missing ordinal index for column $1",
                                             _file_name, _meta.column_id()));
    }
    return Status::OK();
}
