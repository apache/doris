...
Status ColumnReader::get_row_ranges_by_bloom_filter(CondColumn* cond_column,
    ...
    for (int i = 0; i < range_size; ++i) {
        ...
        RETURN_IF_ERROR(bf_iter->read_range_bloom_filters(
                row_ranges->get_range_from(i), row_ranges->get_range_to(i), &bfs));
        // filter cond against bloom filters
        for (auto& bf : bfs) {
            if (cond_column->eval(bf.second.get())) {
                bf_row_ranges.add(bf.first);
            } else {
                stats->rows_stats_filtered += bf.first.to() - bf.first.from();
            }
        }
    }
