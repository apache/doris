...
int64_t InvertedIndexFileWriter::write_v1() {
    ...
    for (const auto& entry : _indices_dirs) {
        ...
        try {
            ...
            LOG(ERROR) << "CLuceneError occurred when closing index file "
...
int64_t InvertedIndexFileWriter::write_v2() {
    try {
        ...
        // Create output stream
        auto [out_dir, compound_file_output] = create_output_stream_v2();
...
InvertedIndexFileWriter::create_output_stream_v2() {
    ...
    return {out_dir, std::move(compound_file_output)};
}
