...
public:
    ...
    Status read_range_bloom_filters(rowid_t from, rowid_t to, std::map<RowRange,
            std::unique_ptr<BloomFilter>, RowRange::Comparator>* bfs);

    inline size_t current_bloom_filter_index() const {
        return _bloom_filter_iter.current_page_index();
    }
