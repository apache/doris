...
Status LazyInitSegmentIterator::init(const StorageReadOptions& /*opts*/, long tz_offset) {
    ...
    RETURN_IF_ERROR(_segment->new_iterator(_schema, _read_options, &_inner_iterator, tz_offset));
    return _inner_iterator->init(_read_options, tz_offset);
}
