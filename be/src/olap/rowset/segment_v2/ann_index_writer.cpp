...
AnnIndexWriter::AnnIndexWriter(const std::string& field_name, IndexFileWriter* index_file_writer,
...
AnnIndexWriter::~AnnIndexWriter() {}

Status AnnIndexWriter::init() {
...
Status AnnIndexWriter::add_values(const std::string fn, const void* values, size_t count) {
    return Status::OK();
}

void AnnIndexWriter::close_on_error() {}

Status AnnIndexWriter::add_array_values(size_t field_size, const void* value_ptr,
...
Status AnnIndexWriter::add_array_values(size_t field_size, const CollectionValue* values,
                                        size_t count) {
    return Status::OK();
}

Status AnnIndexWriter::add_nulls(uint32_t count) {
    // 实现逻辑
    return Status::OK();
}

Status AnnIndexWriter::add_array_nulls(const uint8_t* null_map, size_t row_id) {
    // 实现逻辑
    return Status::OK();
}

int64_t AnnIndexWriter::size() const {
    return 0; // TODO: 获取倒排索引的内存大小
}

Status AnnIndexWriter::finish() {
