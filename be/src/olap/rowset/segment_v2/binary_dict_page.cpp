                ...
                if (src->size > 0) {
                    ...
                    if (item_mem == nullptr) {
                        return Status::MemoryAllocFailed(
                                Substitute("memory allocate failed, size:$0", src->size));
                    }
...
Status BinaryDictPageDecoder::init() {
    ...
    if (_data.size < BINARY_DICT_PAGE_HEADER_SIZE) {
        return Status::Corruption(Substitute("invalid data size:$0, header size:$1", _data.size,
    ...
    for (int i = 0; i < *n; ++i) {
        ...
        if (element.size > 0) {
            ...
            if (destination == nullptr) {
                return Status::MemoryAllocFailed(
                        Substitute("memory allocate failed, size:$0", element.size));
            }
