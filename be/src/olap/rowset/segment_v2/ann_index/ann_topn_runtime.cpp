    ...
    */
    ...
    if (auto* nullable_col =
        ...
        if (nullable_col->has_null()) {
            return Status::InvalidArgument("Ann topn query vector cannot be NULL");
        }
    ...
    if (value_count == 0) {
        return Status::InvalidArgument("Ann topn query vector cannot be empty");
    }
    ...
    const auto* value_col = assert_cast<const vectorized::ColumnFloat32*>(values_holder_col);
...
Status AnnTopNRuntime::evaluate_vector_ann_search(segment_v2::IndexIterator* ann_index_iterator,
    ...
    if (ann_index_reader->get_dimension() != _query_array_size) {
        return Status::InvalidArgument(
                "Ann topn query vector dimension {} does not match index dimension {}",
                _query_array_size, ann_index_reader->get_dimension());
    }
