...
template <FieldType field_type>
Result<std::unique_ptr<lucene::analysis::Analyzer>>
...
Status InvertedIndexColumnWriter<field_type>::add_array_values(size_t field_size,
                                                               ...
                                                               size_t count) {
    DBUG_EXECUTE_IF("InvertedIndexColumnWriter::add_array_values_count_is_zero", { count = 0; })
    if (count == 0) {
    ...
    if constexpr (field_is_slice_type(field_type)) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (!_doc->getFields()->empty()) {
                ...
                if (st != Status::OK()) {
                    LOG(ERROR) << "create field " << string(_field_name.begin(), _field_name.end())
                               << " error:" << st;
                    return st;
                }
                _doc->add(*new_field);
