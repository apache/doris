...
Status VerticalBetaRowsetWriter::add_columns(const vectorized::Block* block,
    ...
    if (_segment_writers.empty()) {
        ...
        if (num_rows_key_group == 0) {
            std::stringstream ss;
            ss << "num_rows_key_group:" << num_rows_key_group;
            LOG(WARNING) << ss.str();
        ...
        if (num_rows_written + num_rows >= num_rows_key_group &&
            ...
            if (start_offset > num_rows) {
                std::stringstream ss;
                ss << "start_offset:" << start_offset << ",num_rows: " << num_rows;
                LOG(WARNING) << ss.str();
                return Status::InternalError(ss.str());
            }
...
Status VerticalBetaRowsetWriter::flush_columns(bool is_key) {
    ...
    if (_cur_writer_idx >= _segment_writers.size()) {
        std::stringstream ss;
        ss << "_cur_writer_idx:" << _cur_writer_idx
           << ",_segment_writers.size():" << _segment_writers.size();
        LOG(WARNING) << ss.str();
        return Status::InternalError(ss.str());
    }
