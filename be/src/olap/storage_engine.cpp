...
void StorageEngine::perform_cumulative_compaction(OlapStore* store) {
    TabletSharedPtr best_table = _find_best_tablet_to_compaction(CompactionType::CUMULATIVE_COMPACTION, store);
...
void StorageEngine::perform_base_compaction(OlapStore* store) {
    TabletSharedPtr best_table = _find_best_tablet_to_compaction(CompactionType::BASE_COMPACTION, store);
    if (best_table == nullptr) { return; }
