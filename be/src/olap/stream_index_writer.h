...
public:
    ...
    // 设置统计信息
    Status set_statistic(ColumnStatistics* statistic);
    // 判断当前entry是否写入了统计信息
    ...
    // ，目前不会有超出这个数量的位置信息
    Status add_position(uint32_t position);
    // 实际上是重置position，便于重写时写入正确的位置
    void reset_write_offset();
    // 移除从from起，count个已经写入的位置，用于去除present流
    Status remove_written_position(uint32_t from, size_t count);
    // 将内容输出到outbuffer中。
...
public:
    ...
    Status add_index_entry(const PositionEntryWriter& entry);
