...
// `SegmentGroup` is the 'largest' group of segments from the tablet.
// All segments in the `SegmentGroup` are not overlapped.
struct SegmentGroup {
...
private:
    ...
    Status _convert_key_ranges(const TabletSPtr& tablet, std::vector<std::string>& start_keys,
