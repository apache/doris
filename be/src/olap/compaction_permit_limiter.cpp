...
void CompactionPermitLimiter::request(int64_t permits) {
    ...
    DorisBvarMetrics::instance()->compaction_waitting_permits->set_value(permits);
    ...
    DorisBvarMetrics::instance()->compaction_waitting_permits->set_value(0);
    DorisBvarMetrics::instance()->compaction_used_permits->set_value(_used_permits);
}
