...
Status EngineCloneTask::clone_missing_rowset_from_peer(const TReplicaInfo& addr,
                                                       const std::string& token,
                                                       const std::vector<Version>& rowset_versions,
                                                       Tablet* tablet) {
    LOG(INFO) << "begin to fetch compaction result, tablet_id=" << tablet->tablet_id()
              << ", addr=" << addr.host << ", version=" << rowset_versions[0];
    std::shared_lock migration_rlock(tablet->get_migration_lock(), std::try_to_lock);
    if (!migration_rlock.owns_lock()) {
    ...
    int timeout_s = 0;
    bool is_incremental_clone = false;
    // 1: make snapshot
    RETURN_IF_ERROR(_make_snapshot(addr.host, addr.be_port, tablet->tablet_id(),
                                   tablet->schema_hash(), timeout_s, rowset_versions,
                                   &snapshot_path, &is_incremental_clone));
    Defer defer {[&, this] {
        // TODO(plat1ko): Async release snapshot
    ...
    _pending_rs_guards = DORIS_TRY(_engine.snapshot_mgr()->convert_rowset_ids(
            local_path, tablet->tablet_id(), tablet->replica_id(), tablet->table_id(),
            tablet->partition_id(), tablet->schema_hash()));
    // 4: finish_clone: create output_rowset and link file
    return _finish_clone(tablet, local_data_path, rowset_versions.back().second,
