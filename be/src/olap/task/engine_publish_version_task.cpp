                ...
                if (version.first != max_version + 1) {
                    if (tablet->check_version_exist(version)) {
                        StorageEngine::instance()->txn_manager()->remove_txn_tablet_info(
                                partition_id, transaction_id, tablet->tablet_id(),
                                tablet->tablet_uid());
                        continue;
                    }
...
void TabletPublishTxnTask::handle() {
    ...
    _result = publish_version_and_add_rowset(StorageEngine::instance(), _partition_id, _tablet,
                                             _rowset, _transaction_id, _version,
...
void AsyncTabletPublishTask::handle() {
    ...
    auto publish_status =
            publish_version_and_add_rowset(StorageEngine::instance(), _partition_id, _tablet,
                                           rowset, _transaction_id, version, nullptr, _stats);
