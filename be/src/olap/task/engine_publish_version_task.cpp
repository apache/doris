...
OLAPStatus EnginePublishVersionTask::finish() {
    ...
    // each partition
    for (auto& partitionVersionInfo : _publish_version_req.partition_version_infos) {
        ...
        for (auto& tablet_rs : tablet_related_rs) {
            ...
            if (publish_status != OLAP_SUCCESS &&
                ...
                LOG(WARNING) << "add visible rowset to tablet failed rowset_id:"
                             << rowset->rowset_id() << "tablet id: " << tablet_info.tablet_id
                             << "txn id:" << transaction_id << "res:" << publish_status;
                _error_tablet_ids->push_back(tablet_info.tablet_id);
                res = publish_status;
                continue;
            }
