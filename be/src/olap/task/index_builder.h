...
public:
    ...
    Status do_build_vector_index();
    Status update_inverted_index_info();
    Status update_vector_index_info();
    Status handle_inverted_index_data();
    Status handle_vector_index_data();
    Status handle_single_rowset_for_inverted_index(RowsetMetaSharedPtr output_rowset_meta,
                                std::vector<segment_v2::SegmentSharedPtr>& segments);
    Status handle_single_rowset_for_vector_index(RowsetMetaSharedPtr output_rowset_meta,
                                                   std::vector<segment_v2::SegmentSharedPtr>& segments);
...
private:
    ...
    Status _write_vector_index_data(TabletSchemaSPtr tablet_schema, int32_t segment_idx,
                                      vectorized::Block* block);
    ...
    Status _add_data_to_vector(const std::string& column_name,
                               const std::pair<int64_t, int64_t>& index_writer_sign, Field* field,
                               const uint8_t** ptr, size_t num_rows);
...
private:
    ...
    std::vector<doris::TOlapTableIndex> _alter_vector_indexes;
    std::vector<TabletIndex> _dropped_inverted_indexes;
    std::vector<TabletIndex> _dropped_vector_indexes;
    bool _is_drop_op;
    std::set<int64_t> _alter_inverted_index_ids;
    std::set<int64_t> _alter_vector_index_ids;
    ...
    // "<segment_id, index_id>" -> VectorIndexWriter
    std::unordered_map<std::pair<int64_t, int64_t>,
                       std::unique_ptr<VectorIndexWriter>>
            _vector_index_builders;
    std::unordered_map<int64_t, std::unique_ptr<VectorIndexFileWriter>>
            _vector_index_file_writers;
};
