...
public:
    ...
    // Get table pointer
    TabletPtr get_table(TTabletId tablet_id, SchemaHash schema_hash, bool load_table = true);

    OLAPStatus get_tables_by_id(TTabletId tablet_id, std::list<TabletPtr>* table_list);    
    ...
    TabletPtr create_table(const TCreateTabletReq& request,
    ...
    OLAPStatus add_table(TTabletId tablet_id, SchemaHash schema_hash,
                         const TabletPtr& table, bool force = false);
    ...
    void get_transactions_by_tablet(TabletPtr tablet, int64_t* partition_id,
    ...
    OLAPStatus clone_incremental_data(TabletPtr tablet, OLAPHeader& clone_header,
                                     int64_t committed_version);

    OLAPStatus clone_full_data(TabletPtr tablet, OLAPHeader& clone_header);
    ...
    virtual std::string get_info_before_incremental_clone(TabletPtr tablet,
        int64_t committed_version, std::vector<Version>* missing_versions);

    virtual OLAPStatus finish_clone(TabletPtr tablet, const std::string& clone_dir,
...
private:
    ...
    OLAPStatus _prepare_snapshot_dir(const TabletPtr& ref_olap_table,
    ...
    OLAPStatus _create_init_version(TabletPtr olap_table, const TCreateTabletReq& request);

private:
    struct TableInstances {
        Mutex schema_change_lock;
        std::list<TabletPtr> table_arr;
    };
    ...
    TabletPtr _get_table_with_no_lock(TTabletId tablet_id, SchemaHash schema_hash);
    ...
    TabletPtr _find_best_tablet_to_compaction(CompactionType compaction_type);
    bool _can_do_compaction(TabletPtr table);
    ...
    void _build_tablet_info(TabletPtr olap_table, TTabletInfo* tablet_info);
