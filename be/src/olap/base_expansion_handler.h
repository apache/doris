...
// @brief 实现对START_BASE_EXPANSION命令的处理逻辑，并返回处理结果
...
public:
    ...
    // 初始化BaseExpansion, 主要完成以下工作：
    // 1. 检查是否满足base expansion策略
    // 2. 如果满足，计算需要合并哪些版本
    //
    // 输入参数：
...
private:
    ...
    // 检验当前情况是否满足base expansion的触发策略
    //
    // 输入参数：
    // - is_manual_trigger: 是否是手动执行START_BASE_EXPANSION命令
    // 输出参数
    ...
    // 在此次base expansion执行过程中，将被合并的cumulative文件版本
