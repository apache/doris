...
protected:
    ...
    std::unique_ptr<std::thread> _evict_querying_rowset_thread;
...
private:
    ...
    std::unique_ptr<std::thread> _unused_rowset_monitor_thread;
    // thread to monitor snapshot expiry
    std::unique_ptr<std::thread> _garbage_sweeper_thread;
    // thread to monitor disk stat
    std::unique_ptr<std::thread> _disk_stat_monitor_thread;
    // thread to produce both base and cumulative compaction tasks
    std::unique_ptr<std::thread> _compaction_tasks_producer_thread;
    std::unique_ptr<std::thread> _update_replica_infos_thread;
    std::unique_ptr<std::thread> _cache_clean_thread;
    // threads to clean all file descriptor not actively in use
    std::vector<std::shared_ptr<std::thread>> _path_gc_threads;
    // thread to produce tablet checkpoint tasks
    std::unique_ptr<std::thread> _tablet_checkpoint_tasks_producer_thread;
    // thread to check tablet path
    std::unique_ptr<std::thread> _tablet_path_check_thread;
    // thread to clean tablet lookup cache
    std::unique_ptr<std::thread> _lookup_cache_clean_thread;
    ...
    std::unique_ptr<std::thread> _cooldown_tasks_producer_thread;
    std::unique_ptr<std::thread> _remove_unused_remote_files_thread;
    std::unique_ptr<std::thread> _cold_data_compaction_producer_thread;

    std::unique_ptr<std::thread> _cache_file_cleaner_tasks_producer_thread;
    ...
    std::unique_ptr<std::thread> _async_publish_thread;
    ...
    std::unique_ptr<std::thread> _check_delete_bitmap_score_thread;
