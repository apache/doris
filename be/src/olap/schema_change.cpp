...
OLAPStatus SchemaChangeHandler::_check_and_clear_schema_change_info(
    ...
    OLAPStatus res = OLAP_SUCCESS;
    // Alter task state is not finished, but we come into this code. it means got schema change lock
    // 2 case
    // case 1: be restart when do schema change
    // case 2: previous schema change failed
    // if alter state is finished, not drop old tablet return success
    // if alter state is not finished, it means be not return success to fe, could drop the new tablet and
    // redo alter job
    if (alter_task != nullptr && alter_task->alter_state() != ALTER_FINISHED) {
        LOG(INFO) << "alter state not finished and received another alter task and get schema change lock, previous"
                  << " alter task is not running, should drop new tablet generated by previous task" 
                  << ", new_tablet_id=" << request.new_tablet_req.tablet_id
                  << ", new_schema_hash=" << request.new_tablet_req.tablet_schema.schema_hash
                  << ", base_tablet_id=" << tablet->tablet_id()
                  << ", base_schema_hash=" << tablet->schema_hash();
