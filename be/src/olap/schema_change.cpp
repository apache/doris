...
bool RowBlockMerger::merge(
    ...
    while (_heap.size() > 0) {
        row_cursor.allocate_memory_for_string_type(_tablet->tablet_schema(), rowset_builder->mem_pool());
        ...
        if (KeysType::DUP_KEYS == _tablet->keys_type()) {
            rowset_builder->add_row(&row_cursor);
            continue;
        }
    ...
    if (rowset_builder->flush() != OLAP_SUCCESS) {
...
bool LinkedSchemaChange::process(
    ...
    OLAPStatus status = new_rowset_builder->add_rowset(rowset_reader->rowset());
    if (status != OLAP_SUCCESS) {
        LOG(WARNING) << "fail to reload index. "
                     << "[tablet='"<< _new_tablet->full_name() << "'"
                     << " version='%d" << new_rowset_builder->version().first
                     << "-" << new_rowset_builder->version().second << "']";
        return false;
    }
...
bool SchemaChangeDirectly::process(RowsetReaderSharedPtr rowset_reader, RowsetBuilderSharedPtr rowset_builder,
                                   TabletSharedPtr tablet) {
    ...
    if (need_create_empty_version) {
        res = rowset_builder->flush();
        if (res != OLAP_SUCCESS) {
            LOG(WARNING) << "create empty version for schema change failed."
                << "version=" << rowset_builder->version().first << "-" << rowset_builder->version().second;
            return false;
        }
        return true;
    }
    ...
    if (OLAP_SUCCESS != rowset_builder->flush()) {
        result = false;
        goto DIRECTLY_PROCESS_ERR;
    }
...
bool SchemaChangeWithSorting::process(
    ...
    if (need_create_empty_version) {
        res = new_rowset_builder->flush();
        if (res != OLAP_SUCCESS) {
            LOG(WARNING) << "create empty version for schema change failed."
                << "version=" << new_rowset_builder->version().first
                << "-" << new_rowset_builder->version().second;
            return false;
        }
        return true;
    ...
    if (!_external_sorting(src_rowsets, new_rowset_builder, tablet)) {
    ...
    if (config::row_nums_check) {
        if (rowset_reader->num_rows()
            != new_rowset_builder->num_rows() + merged_rows() + filted_rows()) {
            OLAP_LOG_WARNING("fail to check row num! "
                             "[source_rows=%lu merged_rows=%lu filted_rows=%lu new_index_rows=%lu]",
                             new_rowset_builder->num_rows(),
                             merged_rows(), filted_rows(), new_rowset_builder->num_rows());
            result = false;
        }
    } else {
        LOG(INFO) << "all row nums. source_rows=" << rowset_reader->num_rows()
                  << ", merged_rows=" << merged_rows()
                  << ", filted_rows=" << filted_rows()
                  << ", new_index_rows=" << new_rowset_builder->num_rows();
    }
...
bool SchemaChangeWithSorting::_external_sorting(
    ...
    if (OLAP_SUCCESS != merger.merge(rs_readers, &merged_rows, &filted_rows)) {
        OLAP_LOG_WARNING("fail to merge deltas. [tablet='%s' version='%d-%d']",
                         ...
                         rowset_builder->version().first,
                         rowset_builder->version().second);
        goto EXTERNAL_SORTING_ERR;
    }
    add_merged_rows(merged_rows);
    add_filted_rows(filted_rows);
...
OLAPStatus SchemaChangeHandler::_get_versions_to_be_changed(
    ...
    if (request_version >= 0) {
        ...
        if (0 == all_versions.size()) {
            OLAP_LOG_WARNING("there'is no any version in the tablet. [tablet='%s']",
                             ref_tablet->full_name().c_str());
...
OLAPStatus SchemaChangeHandler::_alter_tablet(SchemaChangeParams* sc_params) {
    ...
    for (vector<RowsetReaderSharedPtr>::iterator it = sc_params->ref_rowset_readers.end() - 1;
        ...
        context_builder.set_rowset_id(rowset_id)
                       ...
                       .set_num_key_fields(new_tablet->num_key_fields())
                       .set_num_short_key_fields(new_tablet->num_short_key_fields())
                       .set_num_rows_per_row_block(new_tablet->num_rows_per_row_block())
                       .set_compress_kind(new_tablet->compress_kind())
                       .set_bloom_filter_fpp(new_tablet->bloom_filter_fpp())
                       .set_rowset_state(VISIBLE)
                       .set_version((*it)->version())
                       .set_version_hash((*it)->version_hash());
        ...
        RowsetBuilderSharedPtr rowset_builder(new AlphaRowsetBuilder());
        OLAPStatus status = rowset_builder->init(builder_context);
        if (status != OLAP_SUCCESS) {
            res = OLAP_ERR_ROWSET_BUILDER_INIT;
            goto PROCESS_ALTER_EXIT;
        }

        if (!sc_procedure->process(*it, rowset_builder, sc_params->new_tablet)) {
            LOG(WARNING) << "failed to process the version. "
                         << " version=" << (*it)->version().first
                         << "-" << (*it)->version().second;
            rowset_builder->release();
    ...
    *sc_sorting = false;
    ...
    for (size_t i = 0; i < new_tablet->num_fields(); ++i) {
        ...
        if (column_mapping->ref_column < 0) {
            ...
            if (new_tablet_schema[i].type != ref_tablet_schema[column_mapping->ref_column].type) {
                *sc_directly = true;
                return OLAP_SUCCESS;
            } else if (
                (new_tablet_schema[i].type == ref_tablet_schema[column_mapping->ref_column].type)
                    && (new_tablet_schema[i].length 
                        != ref_tablet_schema[column_mapping->ref_column].length)) {
                *sc_directly = true;
                return OLAP_SUCCESS;

            } else if (new_tablet_schema[i].is_bf_column != ref_tablet_schema[i].is_bf_column) {
                *sc_directly = true;
                return OLAP_SUCCESS;
            }
        }
