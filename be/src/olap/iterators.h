...
public:
    ...
    struct SplitKey {
        ...
        SplitKey(const std::shared_ptr<RowCursor>& cursor, const std::string& key_,
                 const std::shared_ptr<Schema>& schema_, bool include_)
                : cursor_key(cursor), key(key_), schema(schema_), include(include_) {}
    };
