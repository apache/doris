...
public:
    ...
    // If the new position is greater than or equal to limit, return Status::Error<E_INVALID_ARGUMENT>()
    ...
    //set new limit
    //If limit is greater than capacity, return Status::Error<E_INVALID_ARGUMENT>()
    ...
    // If position >= limit before writing, return Status::Error<BUFFER_OVERFLOW>()
    Status put(char src);

    // Write data at the index position without changing the position
    // Returns:
    //   Status::Error<BUFFER_OVERFLOW>() : index >= limit
    ...
    // Returns:
    //   Status::Error<BUFFER_OVERFLOW>(): remaining() < length
    //   Status::Error<OUT_OF_BOUND>(): offset + length > src_size
