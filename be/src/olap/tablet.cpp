...
>>>>>>> d9a9c0d79 ([Schema Change] support fast add/drop column  (#49))
...
namespace doris {
    ...
    void Tablet::save_meta() {
        ...
        CHECK_EQ(res, Status::OK())
                << "fail to save tablet_meta. res=" << res << ", root=" << _data_dir->path();
    }
    ...
    const RowsetSharedPtr Tablet::get_rowset_by_version(const Version& version, bool find_in_stale)
            const {
    ...
    RowsetSharedPtr Tablet::_rowset_with_largest_size() {
        ...
        for (auto& it : _rs_version_map) {
            ...
            if (largest_rowset == nullptr ||
                it.second->rowset_meta()->index_disk_size() >
    ...
    Status Tablet::capture_consistent_versions(
            const Version& spec_version, std::vector<Version>* version_path, bool quiet) const {
        Status status = _timestamped_version_tracker.capture_consistent_versions(spec_version,
                                                                                 version_path);
        if (!status.ok() && !quiet) {
            std::vector<Version> missed_versions;
            ...
            if (missed_versions.empty()) {
                ...
                if (version_path != nullptr) {
                    LOG(WARNING) << "tablet:" << full_name()
                                 << ", version already has been merged. spec_version: "
                                 << spec_version;
                }
    ...
    void Tablet::acquire_version_and_rowsets(std::vector<std::pair<Version, RowsetSharedPtr>> *
                                             version_rowsets) const {
    ...
    void Tablet::_max_continuous_version_from_beginning_unlocked(
            Version * version, Version * max_version, bool* has_version_cross) const {
    ...
    void Tablet::pick_candidate_rowsets_to_cumulative_compaction(std::vector<RowsetSharedPtr> *
                                                                 candidate_rowsets) {
    ...
    void Tablet::pick_candidate_rowsets_to_base_compaction(vector<RowsetSharedPtr> *
                                                           candidate_rowsets) {
        std::shared_lock rdlock(_meta_lock);
    ...
    void Tablet::get_compaction_status(std::string * json_result) {
        ...
        for (int i = 0; i < rowsets.size(); ++i) {
            ...
            std::string disk_size = PrettyPrinter::print(
                    rowsets[i]->rowset_meta()->total_disk_size(), TUnit::BYTES);
        ...
        for (auto& rs_meta : _tablet_meta->all_rs_metas()) {
            ...
            if (RowsetMetaManager::check_rowset_meta(_data_dir->get_meta(), tablet_uid(),
                ...
                RowsetMetaManager::remove(_data_dir->get_meta(), tablet_uid(),
                                          rs_meta->rowset_id());
                VLOG_NOTICE
                        << "remove rowset id from meta store because it is already persistent with "
    ...
    Status Tablet::create_rowset_writer(
            const Version& version, const RowsetStatePB& rowset_state,
            const SegmentsOverlapPB& overlap, const doris::TabletSchema* tablet_schema,
    ...
    Status Tablet::create_rowset_writer(
            const int64_t& txn_id, const PUniqueId& load_id, const RowsetStatePB& rowset_state,
            const SegmentsOverlapPB& overlap, const doris::TabletSchema* tablet_schema,
            std::unique_ptr<RowsetWriter>* rowset_writer) {
        RowsetWriterContext context;
    ...
    std::shared_ptr<MemTracker>& Tablet::get_compaction_mem_tracker(
            CompactionType compaction_type) {
        if (compaction_type == CompactionType::CUMULATIVE_COMPACTION) {
    ...
    Status Tablet::cooldown() {
        ...
        RETURN_IF_ERROR(old_rowset->upload_to(
                reinterpret_cast<io::RemoteFileSystem*>(dest_fs.get()), new_rowset_id));
        ...
        RowsetSharedPtr new_rowset;
        RowsetFactory::create_rowset(&_schema, _tablet_path, std::move(new_rowset_meta),
                                     &new_rowset);
    ...
    bool Tablet::need_cooldown(int64_t * cooldown_timestamp, size_t * file_size) {
        ...
        VLOG_DEBUG << "tablet does not need cooldown, tablet id: " << tablet_id()
                   << " ttl sec: " << cooldown_ttl_sec
                   << " cooldown datetime: " << cooldown_datetime
        ...
        for (auto& it : _rs_version_map) {
            ...
            if (!rs->is_local()) {
                ...
                LOG_IF(WARNING, !st.ok()) << "Failed to remove rowset " << rs->version() << " "
                                          << rs->rowset_id().to_string() << " in tablet "
