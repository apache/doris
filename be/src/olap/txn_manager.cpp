...
OLAPStatus TxnManager::prepare_txn(TPartitionId partition_id, TTransactionId transaction_id,
    ...
    if (_txn_partition_map.size() > config::max_runnings_transactions) {
        LOG(WARNING) << "too many transactions: " << _txn_tablet_map.size()
                     << ", limit: " << config::max_runnings_transactions;
        ...
        if (it.second.find(tablet_info) != it.second.end()) {
            *partition_id = it.first.first;
            ...
            VLOG(3) << "find transaction on tablet."
                    << "partition_id: " << it.first.first << ", transaction_id: " << it.first.second
                    << ", tablet: " << tablet_info.to_string();
        }
    }
}
...
bool TxnManager::has_txn(TPartitionId partition_id, TTransactionId transaction_id,
    ...
    ReadLock txn_rdlock(&_txn_map_lock);
    auto it = _txn_tablet_map.find(key);
    bool found = it != _txn_tablet_map.end() && it->second.find(tablet_info) != it->second.end();
