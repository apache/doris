...
Status ShortKeyIndexBuilder::finalize(uint32_t num_segment_rows, std::vector<Slice>* body,
    ...
    page_footer->set_uncompressed_size(static_cast<uint32_t>(_key_buf.size() + _offset_buf.size()));

    segment_v2::ShortKeyFooterPB* footer = page_footer->mutable_short_key_page_footer();
    footer->set_num_items(_num_items);
    footer->set_key_bytes(static_cast<uint32_t>(_key_buf.size()));
    footer->set_offset_bytes(static_cast<uint32_t>(_offset_buf.size()));
