...
Status SnapshotManager::convert_rowset_ids(const std::string& clone_dir, int64_t tablet_id,
    ...
    for (auto& visible_rowset : cloned_tablet_meta_pb.rs_metas()) {
        ...
        if (!visible_rowset.has_resource_id()) {
            ...
            OLAP_RETURN_NOT_OK(_rename_rowset_id(visible_rowset, clone_dir, tablet_schema, rowset_id,
                                            rowset_meta));
            RowsetId src_rs_id;
    ...
    for (auto& stale_rowset : cloned_tablet_meta_pb.stale_rs_metas()) {
        ...
        if (!stale_rowset.has_resource_id()) {
            // src be local rowset
            RowsetId rowset_id = StorageEngine::instance()->next_rowset_id();
            OLAP_RETURN_NOT_OK(_rename_rowset_id(stale_rowset, clone_dir, tablet_schema, rowset_id,
                                            rowset_meta));
            RowsetId src_rs_id;
...
Status SnapshotManager::_rename_rowset_id(const RowsetMetaPB& rs_meta_pb,
    ...
    OLAP_RETURN_NOT_OK(org_rowset->load(false));
    ...
    OLAP_RETURN_NOT_OK(RowsetFactory::create_rowset_writer(context, false, &rs_writer));
    ...
    OLAP_RETURN_NOT_OK(new_rowset->load(false));
...
Status SnapshotManager::_link_index_and_data_files(
    ...
    for (auto& rs : consistent_rowsets) {
        OLAP_RETURN_NOT_OK(rs->link_files_to(schema_hash_path, rs->rowset_id()));
    }
    return res;
}
