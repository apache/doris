...
Status Merger::vmerge_rowsets(TabletSharedPtr tablet, ReaderType reader_type,
    ...
    std::copy_if(
            src_rowset_readers.begin(), src_rowset_readers.end(),
            std::inserter(reader_params.delete_predicates, reader_params.delete_predicates.begin()),
            [](const auto& rowset) {
                return rowset->rowset()->rowset_meta()->has_delete_predicate();
            });
    {
