...
void Merger::vertical_split_columns(TabletSharedPtr tablet,
                                    std::vector<std::vector<uint32_t>>* column_groups) {
    uint32_t num_columns_per_group = config::vertical_compaction_num_columns_per_group;
    uint32_t num_key_cols = tablet->num_key_columns();
    uint32_t total_cols = tablet->num_columns();
    ...
    if (tablet->keys_type() == KeysType::UNIQUE_KEYS) {
        if (tablet->tablet_schema()->has_sequence_col()) {
            sequence_col_idx = tablet->tablet_schema()->sequence_col_idx();
            key_columns.emplace_back(sequence_col_idx);
        }
        delete_sign_idx = tablet->tablet_schema()->field_index(DELETE_SIGN);
...
Status Merger::vertical_merge_rowsets(TabletSharedPtr tablet, ReaderType reader_type,
    ...
    std::vector<std::vector<uint32_t>> column_groups;
    vertical_split_columns(tablet, &column_groups);
