...
Status Merger::vertical_compact_one_group(
    ...
    while (!eof && !StorageEngine::instance()->stopped()) {
        ...
        if (!is_key && eof) {
            ...
            if (dst_rowset_writer->num_rows() != stats_output->output_rows) {
                std::stringstream ss;
                ss << "dst_rowset_writer->num_rows() != stats_output->output_rows:"
            ...
            DCHECK_EQ(dst_rowset_writer->num_rows(),
                     row_source_buf->total_size() - reader.merged_rows())
                    << "dst rowset writer key group rows: " << dst_rowset_writer->num_rows()
                    << "row source buf total size: " << row_source_buf->total_size()
                    << "row source buf size: " << row_source_buf->buffered_size()
                    << ", merged rows: " << reader.merged_rows();
            if (dst_rowset_writer->num_rows() !=
                row_source_buf->total_size() - reader.merged_rows()) {
                std::stringstream ss;
                ss << "dst rowset writer key group rows: " << dst_rowset_writer->num_rows()
                   << "row source buf total size: " << row_source_buf->total_size()
                   << "row source buf size: " << row_source_buf->buffered_size()
                   << ", merged rows: " << reader.merged_rows();
                LOG(WARNING) << ss.str();
            ...
            DCHECK_EQ(output_rows + block.rows(), dst_rowset_writer->num_rows())
                    << "output_rows: " << output_rows << "block rows: " << block.rows()
                    << "dst rowset writer key group rows: " << dst_rowset_writer->num_rows()
                    << "row source buf size: " << row_source_buf->buffered_size()
                    << ", merged rows: " << reader.merged_rows();
            if (output_rows + block.rows() != dst_rowset_writer->num_rows()) {
                std::stringstream ss;
                ss << "output_rows: " << output_rows << "block rows: " << block.rows()
                   << "dst rowset writer key group rows: " << dst_rowset_writer->num_rows()
                   << "row source buf size: " << row_source_buf->buffered_size()
                   << ", merged rows: " << reader.merged_rows();
                LOG(WARNING) << ss.str();
    ...
    if (is_key && tablet_schema->keys_type() == KeysType::UNIQUE_KEYS) {
        ...
        DCHECK_EQ(row_source_buf->total_size(), stats_output->output_rows +
                                                        stats_output->merged_rows +
                                                        stats_output->filtered_rows);
        if (row_source_buf->total_size() !=
            stats_output->output_rows + stats_output->merged_rows + stats_output->filtered_rows) {
            std::stringstream ss;
            ss << "row source buf total size: " << row_source_buf->total_size()
               << "stats_output output_rows: " << stats_output->output_rows
               << "stats_output merged_rows: " << stats_output->merged_rows
               << "stats_output filtered_rows: " << stats_output->filtered_rows;
            LOG(WARNING) << ss.str();
            return Status::InternalError(ss.str());
        }
        DCHECK_EQ(stats_output->output_rows, dst_rowset_writer->num_rows());
        if (stats_output->output_rows != dst_rowset_writer->num_rows()) {
            std::stringstream ss;
            ss << "stats_output output_rows:" << stats_output->output_rows
               << "dst_rowset_writer num_rows:" << dst_rowset_writer->num_rows();
            LOG(WARNING) << ss.str();
            return Status::InternalError(ss.str());
        }
    }
