...
public:
    ...
    // 初始化CumulativeHandler对象，包括:
    // - 检查是否触发cumulative expansion
    // - 计算可合并的delta文件
    //
    // 输入参数：
    ...
    //
    // 返回值：
    // - 如果触发cumulative expansion，返回OLAP_SUCCESS
    // - 否则，返回对应错误码
...
private:
    // 检查是否满足cumulative expansion触发策略
    //
    // 返回值：
    // - 如果满足，返回OLAP_SUCCESS
    ...
    // CumulativeHandler对象是否初始化
    bool _is_init;
    // header文件是否加锁
    bool _header_locked;
    // table现有的cumulative层的标识点
    int32_t _old_cumulative_layer_point;
    // 待cumulative expansion完成之后，新的cumulative层的标识点
