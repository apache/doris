...
void TabletSchema::flatten_variant_cols() {
    std::vector<TabletColumn> subcolumns;
    for (const TabletColumn& col : _cols) {
        if (col.is_variant_type()) {
            for (const TabletColumn& subcol : col.get_sub_columns()) {
                subcolumns.push_back(subcol);
            }
        }
    }
    for (const TabletColumn& subcol : subcolumns) {
        append_column(subcol);
    }
}
