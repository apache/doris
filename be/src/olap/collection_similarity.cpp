...
void CollectionSimilarity::get_bm25_scores(roaring::Roaring* row_bitmap,
    ...
    std::vector<uint32_t> filtered_row_ids;
    std::vector<float> filtered_scores;
    filtered_row_ids.reserve(row_bitmap->cardinality());
    ...
    for (uint32_t row_id : *row_bitmap) {
        ...
        filtered_row_ids.push_back(row_id);
        filtered_scores.push_back(score);
    }

    size_t num_results = filtered_row_ids.size();
    ...
    for (size_t i = 0; i < num_results; ++i) {
        (*row_ids)[i] = filtered_row_ids[i];
        score_data[i] = filtered_scores[i];
        new_bitmap.add(filtered_row_ids[i]);
    }
