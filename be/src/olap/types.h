...
class TypeInfo {
public:
    inline bool equal(const void* left, const void* right) const { return _equal(left, right); }

    inline int cmp(const void* left, const void* right) const { return _cmp(left, right); }

    inline void shallow_copy(void* dest, const void* src) const { _shallow_copy(dest, src); }
    ...
    inline void direct_copy(void* dest, const void* src) const { _direct_copy(dest, src); }
...
OLAPStatus convert_int_from_varchar(void* dest, const void* src) {
    ...
    T result = StringParser::string_to_int<T>(src_value->get_data(), src_value->get_size(),
                                              &parse_res);
...
OLAPStatus convert_float_from_varchar(void* dest, const void* src) {
    ...
    auto src_value = reinterpret_cast<const SrcType*>(src);
    StringParser::ParseResult parse_res;
    T result = StringParser::string_to_float<T>(src_value->get_data(), src_value->get_size(),
                                                &parse_res);
    if (UNLIKELY(parse_res != StringParser::PARSE_SUCCESS)) {
        return OLAPStatus::OLAP_ERR_INVALID_SCHEMA;
    }
...
struct FieldTypeTraits<OLAP_FIELD_TYPE_TINYINT>
        : public BaseFieldtypeTraits<OLAP_FIELD_TYPE_TINYINT> {
    static std::string to_string(const void* src) {
        char buf[1024] = {'\0'};
...
struct FieldTypeTraits<OLAP_FIELD_TYPE_SMALLINT>
        : public BaseFieldtypeTraits<OLAP_FIELD_TYPE_SMALLINT> {
    static std::string to_string(const void* src) {
        char buf[1024] = {'\0'};
...
struct FieldTypeTraits<OLAP_FIELD_TYPE_BIGINT>
        : public BaseFieldtypeTraits<OLAP_FIELD_TYPE_BIGINT> {
    static std::string to_string(const void* src) {
        char buf[1024] = {'\0'};
...
struct FieldTypeTraits<OLAP_FIELD_TYPE_LARGEINT>
        : public BaseFieldtypeTraits<OLAP_FIELD_TYPE_LARGEINT> {
    static OLAPStatus from_string(void* buf, const std::string& scan_key) {
        int128_t value = 0;
...
struct FieldTypeTraits<OLAP_FIELD_TYPE_DOUBLE>
        : public BaseFieldtypeTraits<OLAP_FIELD_TYPE_DOUBLE> {
    static OLAPStatus from_string(void* buf, const std::string& scan_key) {
        CppType value = 0.0;
        if (scan_key.length() > 0) {
