...
OLAPStatus olap_compress(const char* src_buf,
    ...
    *written_len = dest_len;
    ...
    case OLAP_COMP_TRANSPORT: {
        ...
        int lzo_res = 0;
        if (LZO_E_OK != (lzo_res = lzo1x_1_compress(reinterpret_cast<const lzo_byte*>(src_buf),
                                                    src_len,
                                                    reinterpret_cast<unsigned char*>(dest_buf),
                                                    written_len,
                                                    mem))) {
            OLAP_LOG_WARNING("compress failed."
                             "[src_len=%lu; dest_len=%lu; written_len=%lu; lzo_res=%d]",
                             src_len,
                             dest_len,
                             *written_len,
                             lzo_res);
    ...
    case OLAP_COMP_STORAGE: {
        ...
        int lzo_res = 0;
        if (LZO_E_OK != (lzo_res = lzo1c_99_compress(reinterpret_cast<const lzo_byte*>(src_buf),
                                                     src_len,
                                                     reinterpret_cast<unsigned char*>(dest_buf),
                                                     written_len,
                                                     mem))) {
    ...
    *written_len = dest_len;
    ...
    case OLAP_COMP_TRANSPORT: {
        int lzo_res = lzo1x_decompress_safe(reinterpret_cast<const lzo_byte*>(src_buf),
                                            ...
                                            NULL);
        if (LZO_E_OK != lzo_res) { 
            OLAP_LOG_WARNING("decompress failed."
                             "[src_len=%lu; dest_len=%lu; written_len=%lu; lzo_res=%d]",
                             src_len,
                             dest_len,
                             *written_len,
            ...
            return OLAP_ERR_DECOMPRESS_ERROR;
        } else if (*written_len > dest_len) {
    ...
    case OLAP_COMP_STORAGE: {
        int lzo_res = lzo1c_decompress_safe(reinterpret_cast<const lzo_byte*>(src_buf),
                                            src_len,
                                            reinterpret_cast<unsigned char*>(dest_buf),
                                            written_len,
                                            NULL);
...
OLAPStatus move_to_trash(const boost::filesystem::path& schema_hash_root,
    ...
    new_file_dir_stream <<  storage_root
                        << TRASH_PREFIX
                        << "/" << time_str << "." << delete_counter++
...
OLAPStatus copy_file(const string& src, const string& dest) {
    ...
    while (true) {
        ...
        if (wr_size != rd_size) {
            OLAP_LOG_WARNING("failed to write to file. [err=%m file_name=%s fd=%d rd_size=%ld "
                             "wr_size=%ld]",
                             dest.c_str(),
                             dest_fd,
                             rd_size,
...
OLAPStatus copy_dir(const string &src_dir, const string &dst_dir) {
    ...
    try {
        ...
        OLAP_LOG_WARNING("input invalid[src_path=%s dst_path=%s]", 
                         src_path.string().c_str(),
    ...
    for (boost::filesystem::directory_iterator file(src_path); 
        ...
        try {
            ...
            if (boost::filesystem::is_directory(current)) {
                ...
                OLAPStatus res = OLAP_SUCCESS;
                if (OLAP_SUCCESS != (res = copy_dir(current.string(),
                                                    (dst_path / current.filename()).string()))) {
...
bool valid_signed_number<int128_t>(const std::string& value_str) {
    ...
    if (*endptr != 0) {
        ...
        if ((!sign && current > max_int128)
                ||  (sign && current > max_int128 + 1)) {
            return false;
        }
...
bool valid_datetime(const string &value_str) {
    const char *datetime_pattern = "((?:\\d){4})-((?:\\d){2})-((?:\\d){2})[ ]*"
                                   "(((?:\\d){2}):((?:\\d){2}):((?:\\d){2}))?";
