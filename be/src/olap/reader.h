...
public:
    ...
    // Return E_OK and set `*eof` to false when next row is read into `row_cursor`.
    // Return E_OK and set `*eof` to true when no more rows can be read.
    ...
    // Return E_OK and set `*eof` to false when next block is read
    // Return E_OK and set `*eof` to true when no more rows can be read.
    // Return others when unexpected error happens.
    ...
    virtual Status next_block_with_aggregation(vectorized::Block* block, MemPool* mem_pool,
        ...
        return Status::Error<READER_INITIALIZE_ERROR>();
    }
