...
Status CloudMetaMgr::do_commit_rowset(RowsetMeta& rs_meta, const std::string& job_id,
    ...
    CloudTabletSPtr tablet =
            DORIS_TRY(ExecEnv::GetInstance()->storage_engine().to_cloud().tablet_mgr().get_tablet(
                    rs_meta.tablet_id()));
    if (tablet == nullptr) {
    ...
    Status st = retry_rpc(MetaServiceRPC::COMMIT_ROWSET, req, &resp, commit_rowset_method,
                          {
                                  .host_limiters = host_level_ms_rpc_rate_limiters_,
                                  .backpressure_handler = ms_backpressure_handler_,
                                  .table_id = table_id,
...
Status CloudMetaMgr::prepare_restore_job(const TabletMetaPB& tablet_meta) {
    ...
    return retry_rpc(MetaServiceRPC::PREPARE_RESTORE_JOB, req, &resp, prepare_restore_method,
                     {
                             .host_limiters = host_level_ms_rpc_rate_limiters_,
                             .backpressure_handler = ms_backpressure_handler_,
                     });
...
Status CloudMetaMgr::commit_restore_job(const int64_t tablet_id) {
    ...
    return retry_rpc(MetaServiceRPC::COMMIT_RESTORE_JOB, req, &resp, commit_restore_method,
                     {
                             .host_limiters = host_level_ms_rpc_rate_limiters_,
                             .backpressure_handler = ms_backpressure_handler_,
                     });
