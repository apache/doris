...
Status CloudCumulativeCompaction::prepare_compact() {
    ...
    std::string compaction_tablet_rowset;
    cloud_tablet()->traverse_rowsets([&](const RowsetSharedPtr& rs) {
        compaction_tablet_rowset += "[" + std::to_string(rs->start_version()) + "-" +
                                    std::to_string(rs->end_version()) + "], ";
    });

    LOG(INFO) << "wyx_debug, before compact rowset and sync rowset"
              << ". tablet=" << _tablet->tablet_id() << " rowset meta" << compaction_tablet_rowset
              << " need_sync_tablet=" << need_sync_tablet;
...
Status CloudCumulativeCompaction::execute_compact() {
    ...
    LOG(INFO) << "wyx_debug, after compact rowset"
              << ". tablet=" << _tablet->tablet_id() << " rowset meta" << compaction_tablet_rowset;
...
Status CloudCumulativeCompaction::modify_rowsets() {
    ...
    std::string rs_meta;
    cloud_tablet()->traverse_rowsets([&rs_meta](const RowsetSharedPtr& rs) {
        return rs_meta += "[" + std::to_string(rs->start_version()) + "-" +
                          std::to_string(rs->end_version()) + "], ";
    });
    LOG(INFO) << "wyx_debug" << ", before modify rowset"
              << ". tablet=" << _tablet->tablet_id() << " input rowsets"
              << std::accumulate(_input_rowsets.begin(), _input_rowsets.end(), std::string(),
                                 [](const std::string& a, const RowsetSharedPtr& b) {
                                     return a + "[" + std::to_string(b->start_version()) + "-" +
                                            std::to_string(b->end_version()) + "], ";
                                 })
              << " output rowset " << "[" << _output_rowset->start_version() << "-"
              << _output_rowset->end_version() << " tablet rowset meta" << rs_meta;
    {
    ...
    LOG(INFO) << "wyx_debug" << ", after modify rowset"
              << ". tablet=" << _tablet->tablet_id() << " input rowsets"
              << std::accumulate(_input_rowsets.begin(), _input_rowsets.end(), std::string(),
                                 [](const std::string& a, const RowsetSharedPtr& b) {
                                     return a + "[" + std::to_string(b->start_version()) + "-" +
...
Status CloudCumulativeCompaction::pick_rowsets_to_compact() {
    ...
    LOG(INFO) << "wyx_debug, pick_rowsets_to_compact: tablet_id=" << _tablet->tablet_id()
              << ", candidate_rowsets=" << candidate_rowsets.size()
              << std::accumulate(_input_rowsets.begin(), _input_rowsets.end(), std::string(),
                                 [](const std::string& a, const RowsetSharedPtr& b) {
                                     return a + (a.empty() ? "" : ",") +
                                            fmt::format("[{}-{}]", b->start_version(),
                                                        b->end_version());
                                 });
    return Status::OK();
}
