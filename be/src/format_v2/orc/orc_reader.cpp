...
//   - 4 类 filter 引用了非 predicate_columns 的列（callback 时拿不到那些列）
...
//   2. reader_expression_map VExpr 表达式（key = file_column_id）
//   3. column_predicate_filters  Doris ColumnPredicate
//
...
Status OrcReader::_init_search_argument_from_local_filters() {
    if (_request->conjuncts.empty() && _request->reader_expression_map.empty() &&
    ...
    try {
        ...
        // 来源 2: reader_expression_map
        for (const auto& reader_expression_entry : _request->reader_expression_map) {
            const auto& reader_expression = reader_expression_entry.second;
            if (reader_expression == nullptr) {
                continue;
            }
            has_pushdown = build_orc_search_argument(*_request, *_state->root_type,
                                                     reader_expression->root(), builder) ||
                           has_pushdown;
        }
        // 来源 3: column_predicate_filters
...
//          ↓   2. 跑 4 类 filter → keep_filter (4096 个 0/1)
...
Status OrcReader::_filter_orc_batch(::orc::ColumnVectorBatch& data, uint16_t* sel, uint16_t size,
    ...
    // 步骤 3：跑 4 类 filter（顺序：column_predicate → reader_expr → conjuncts → delete）
...
//      callback 里跑了 4 类 filter，sel/keep_filter 缓存在 _state 里
...
Status OrcReader::_validate_reader_expression_map() const {
    for (const auto& [file_column_id, reader_expression] : _request->reader_expression_map) {
        if (reader_expression == nullptr) {
            return Status::InvalidArgument("ORC reader expression is null for column {}",
                                           file_column_id.value());
        }
    }
    return Status::OK();
...
//
// 给定 ORC callback 已能看到的列 (= predicate_columns)，能不能跑全部 4 类 filter？
...
//   reader_expression : 表达式引用的列必须都 decode + key 列已 decode
...
bool OrcReader::_can_filter_with_decoded_columns(
    ...
    for (const auto& column_filter : _request->column_predicate_filters) {
        if (!column_filter.effective_file_child_id_path().empty()) {
            return false; // nested pruning hint 不能 row-level 跑（需要嵌套 decode）
        }
        if (!decoded_columns.contains(column_filter.effective_file_column_id())) {
            return false;
        }
    }
    // 2. reader_expression_map
    for (const auto& [file_column_id, reader_expression] : _request->reader_expression_map) {
        if (!decoded_columns.contains(file_column_id) || !expr_can_run(reader_expression)) {
            return false;
        }
    }
    // 3. conjuncts
...
bool OrcReader::_filter_has_row_level_predicates() const {
    return !_request->column_predicate_filters.empty() ||
           !_request->reader_expression_map.empty() || !_request->conjuncts.empty() ||
           !_request->delete_conjuncts.empty();
}

// _build_keep_filter —— 4 类 row-level filter 串行 AND 入口
//
// 4 类来源（详见 file_reader.h FileScanRequest）：
//   1. column_predicate_filters  Doris ColumnPredicate 形态（zonemap-friendly + Bloom/Bitmap/HybridSet）
//   2. reader_expression_map     column_mapper 改写不了的 file-local 表达式（fallback）
//   3. conjuncts                 标准 SQL VExpr 表达式（主通道）
//   4. delete_conjuncts          Iceberg/Hudi 表层删除（不参与 SARG，row-level 兜底）
//
...
Status OrcReader::_build_keep_filter(Block* file_block, size_t rows,
    ...
    RETURN_IF_ERROR(_execute_reader_expression_filters(file_block, rows, keep_filter));
...
Status OrcReader::_execute_column_predicate_filters(Block* file_block, size_t rows,
    ...
    for (const auto& column_filter : _request->column_predicate_filters) {
        ...
        const auto position_it =
                _request->local_positions.find(column_filter.effective_file_column_id());
        DORIS_CHECK(position_it != _request->local_positions.end());
        const auto block_position = position_it->second;
        DORIS_CHECK(block_position.value() < file_block->columns());
...
Status OrcReader::_execute_reader_expression_filters(Block* file_block, size_t rows,
                                                     IColumn::Filter* keep_filter) const {
    DORIS_CHECK(file_block != nullptr);
    DORIS_CHECK(keep_filter != nullptr);
    for (const auto& reader_expression_entry : _request->reader_expression_map) {
        const auto& reader_expression = reader_expression_entry.second;
        DORIS_CHECK(reader_expression != nullptr);
        IColumn::Filter expression_filter(rows, 1);
        bool can_filter_all = false;
        RETURN_IF_ERROR(reader_expression->execute_filter(file_block, expression_filter.data(),
                                                          rows, false, &can_filter_all));
        if (can_filter_all) {
            std::fill(keep_filter->begin(), keep_filter->end(), 0);
            return Status::OK();
        }
        for (size_t row = 0; row < rows; ++row) {
            (*keep_filter)[row] &= expression_filter[row];
        }
    }
    return Status::OK();
}
