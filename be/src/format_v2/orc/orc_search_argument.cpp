...
std::optional<OrcSargColumn> sarg_column_for_column_filter(
    ...
    if (!column_filter.target.is_valid()) {
        return std::nullopt;
    }
...
bool can_build_search_argument(const format::FileScanRequest& request, const ::orc::Type& root_type,
    ...
    case TExprOpcode::COMPOUND_NOT:
        ...
        return expr->children().size() == 1 &&
               can_build_search_argument(request, root_type, expr->children()[0]);
    case TExprOpcode::GE:
    case TExprOpcode::GT:
    case TExprOpcode::LE:
    case TExprOpcode::LT:
...
bool build_search_argument(const format::FileScanRequest& request, const ::orc::Type& root_type,
    ...
    case TExprOpcode::COMPOUND_NOT:
        builder->startNot();
        DORIS_CHECK(build_search_argument(request, root_type, expr->children()[0], builder));
        builder->end();
        return true;
    case TExprOpcode::GE:
    case TExprOpcode::GT:
