...
private:
    ...
    // 在这里 decode predicate 列、跑 4 类 filter、写 sel[] 给 FOLLOWERS phase 用。
    ...
    Status _validate_reader_expression_map() const;

    // ============ Filter 子系统（4 类 filter 的执行 + ORC lazy 决策）============
    //
    // _can_filter_with_decoded_columns
    //     给定一组已 decode 的列，能不能跑所有 row-level filter？
    //     仅用于判断 ORC lazy callback 能否安全运行全部 filter。
    //     4 类 filter 引用的所有列都已 decode → callback 可用
    ...
    //
    // _build_keep_filter
    //     4 类 filter 串行 AND 入口，按顺序：
    //       1. _execute_column_predicate_filters  (Doris ColumnPredicate)
    //       2. _execute_reader_expression_filters (file-local VExpr fallback)
    //       3. _execute_conjuncts                 (标准 SQL VExpr)
    //       4. _execute_delete_conjuncts          (Iceberg/Hudi delete)
    //
    ...
    Status _execute_reader_expression_filters(Block* file_block, size_t rows,
                                              IColumn::Filter* keep_filter) const;
