...
struct NativeFieldSchema {
    ...
    // The index order in NativeFieldDescriptor._physical_fields
    int physical_column_index = -1;
    int16_t definition_level = 0;
    int16_t repetition_level = 0;
    int16_t repeated_parent_def_level = 0;
    std::vector<NativeFieldSchema> children;
    ...
    NativeFieldSchema()
            : data_type(std::make_shared<DataTypeNothing>()),
              column_id(NATIVE_UNASSIGNED_COLUMN_ID) {}
    ~NativeFieldSchema() = default;
    NativeFieldSchema(const NativeFieldSchema& fieldSchema) = default;
    std::string debug_string() const;

    int32_t field_id = -1;
    uint64_t column_id = NATIVE_UNASSIGNED_COLUMN_ID;
    ...
    const NativeFieldSchema* find_column_by_id(uint64_t target_id) const;
...
private:
    ...
    std::vector<NativeFieldSchema> _fields;
    // The leaf node of schema elements
    std::vector<NativeFieldSchema*> _physical_fields;
    // Name to _fields, not all schema elements
    std::unordered_map<std::string, NativeFieldSchema*> _name_to_field;
...
public:
    ...
    /**
     * Parse NativeFieldDescriptor from parquet thrift FileMetaData.
     ...
     */
    const NativeFieldSchema* get_column(size_t index) const { return &_fields[index]; }

    /**
     * Get the column(the first level schema element, maybe nested field) by name.
     * @param name Column name
     * @return NativeFieldSchema or nullptr if not exists
     */
    NativeFieldSchema* get_column(const std::string& name) const;
     ...
     * After calling this, each NativeFieldSchema will have column_id and
