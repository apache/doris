...
// These cursor types belong to the native v2 decoder so its execution path never reaches into
// the legacy Parquet reader for filtering or level materialization.
...
public:
    ...
    size_t get_next_run(DataReadType* data_read_type) {
        ...
        if constexpr (has_filter) {
            ...
            size_t run_length = 1;
            while (_read_index < _num_values && _data_map[_read_index] == type) {
                ++run_length;
                ++_read_index;
            }
            *data_read_type = type;
