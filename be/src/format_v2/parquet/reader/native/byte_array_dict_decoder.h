...
public:
    ...
    size_t dictionary_size() const override { return _dict_items.size(); }
    Status decode_dictionary(ParquetFixedValueConsumer& fixed_consumer,
                             ParquetBinaryValueConsumer& binary_consumer) override;

protected:
    // StringRef entries point into BaseDictDecoder::_dict. The dictionary buffer lives for the
    // entire column chunk, so retaining a second copy of every byte is unnecessary.
