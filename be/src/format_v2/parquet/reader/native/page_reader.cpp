...
Status PageReader<IN_COLLECTION, OFFSET_INDEX>::_validate_page_header(uint32_t header_size) const {
    if (UNLIKELY(_cur_page_header.compressed_page_size < 0 ||
                 _cur_page_header.uncompressed_page_size < 0)) {
        return Status::Corruption("Parquet page has a negative compressed or uncompressed size");
    }
    ...
    if (_cur_page_header.__isset.data_page_header_v2) {
        ...
        if (UNLIKELY(v2.num_values < 0 || v2.num_rows < 0 || v2.num_nulls < 0 ||
                     ...
                     v2.definition_levels_byte_length < 0)) {
            return Status::Corruption("Parquet data page v2 has negative counts or level sizes");
        }
        if (UNLIKELY(v2.num_nulls > v2.num_values || v2.num_rows > v2.num_values)) {
            return Status::Corruption(
                    "Parquet data page v2 null or row count exceeds its value count");
        }
        const uint64_t level_bytes = static_cast<uint64_t>(v2.repetition_levels_byte_length) +
                                     static_cast<uint64_t>(v2.definition_levels_byte_length);
        if (UNLIKELY(level_bytes > static_cast<uint64_t>(_cur_page_header.compressed_page_size) ||
                     level_bytes >
...
PageReader<IN_COLLECTION, OFFSET_INDEX>::PageReader(io::BufferedStreamReader* reader,
    ...
    _state = INITIALIZED;
    _page_cache_key_builder.init(_page_read_ctx.page_cache_file_key);

    if constexpr (OFFSET_INDEX) {
        _end_row = _offset_index != nullptr && _offset_index->page_locations.size() >= 2
