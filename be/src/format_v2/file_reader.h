...
struct FileScanRequest {
    ...
    // Logical Variant object paths retained after table-to-file mapping. Parquet uses their unique
    // count to choose single-path direct seek or one cached full-root materialization; other readers
    // may ignore them. The all-path map is the union required from a physical column, while the
