...
public:
    ...
    virtual Status on_fill_partition_columns(Block* block, size_t rows,
        ...
        LOG(INFO) << "[DEBUG on_fill_partition_columns] _fill_partition_values.size="
                  << _fill_partition_values.size() << " cols.size=" << cols.size()
                  << " rows=" << rows;
        for (const auto& col_name : cols) {
            auto it = _fill_partition_values.find(col_name);
            if (it == _fill_partition_values.end()) {
                LOG(INFO) << "[DEBUG on_fill_partition_columns] col=" << col_name
                          << " NOT FOUND in _fill_partition_values";
                continue;
            }
