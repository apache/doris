...
Status IcebergPositionDeleteSysTableReader::_init_position_delete_reader() {
    ...
    if (_delete_file_desc->file_format == TFileFormatType::FORMAT_PARQUET) {
        ...
        // predicates, so pass empty filter state while preserving the schema mapping built above.
        VExprContextSPtrs conjuncts;
        phmap::flat_hash_map<int, std::vector<std::shared_ptr<ColumnPredicate>>>
                slot_id_to_predicates;
        RETURN_IF_ERROR(parquet_reader->init_reader(
                read_column_names, &_read_col_name_to_block_idx, conjuncts, slot_id_to_predicates,
                nullptr, nullptr, nullptr, nullptr, nullptr, std::move(table_info_node), false));
    ...
    if (_delete_file_desc->file_format == TFileFormatType::FORMAT_ORC) {
        ...
        VExprContextSPtrs conjuncts;
        RETURN_IF_ERROR(orc_reader->init_reader(&read_column_names, &_read_col_name_to_block_idx,
                                                conjuncts, false, nullptr, nullptr, nullptr,
                                                nullptr, std::move(table_info_node)));
...
Status IcebergPositionDeleteSysTableReader::get_next_block(Block* block, size_t* read_rows,
                                                           bool* eof) {
