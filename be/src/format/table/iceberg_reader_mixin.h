...
protected:
    ...
    Status on_before_read_block(Block* block) override {
        ...
        // Auto-detect whether the block contains ICEBERG_ROWID_COL on first call.
        if (!_need_row_id_column_detected) {
            _need_row_id_column_detected = true;
            int pos = block->get_position_by_name(BeConsts::ICEBERG_ROWID_COL);
            if (pos >= 0) {
                _need_row_id_column = true;
                _row_id_column_position = pos;
            }
            LOG(INFO) << "[IcebergDebug] on_before_read_block: _need_row_id_column="
                      << _need_row_id_column << ", pos=" << pos;
        }

        return Status::OK();
    }

    // Called by the base reader to fill synthesized columns (e.g. Iceberg $row_id).
    Status on_fill_synthesized_columns(Block* block, size_t rows,
                                       const std::vector<std::string>& /*cols*/) override {
        if (!_need_row_id_column || _row_id_column_position < 0) {
            return Status::OK();
        }
        ...
        auto& col_with_type = block->get_by_position(static_cast<size_t>(_row_id_column_position));
    ...
    Status on_after_read_block(Block* block, size_t* read_rows) override {
        if (_equality_delete_impl != nullptr) {
            RETURN_IF_ERROR(_equality_delete_impl->filter_data_block(
                    block, this->col_name_to_block_idx_ref()));
    ...
    bool _need_row_id_column_detected = false;
...
Status IcebergReaderMixin<BaseReader>::_equality_delete_base(
    ...
    for (const auto& delete_file : delete_files) {
        ...
        if (auto* parquet_reader = typeid_cast<ParquetReader*>(delete_reader.get())) {
            ...
            RETURN_IF_ERROR(parquet_reader->_do_init_reader(
            ...
            RETURN_IF_ERROR(orc_reader->_do_init_reader(&equality_delete_col_names,
    ...
    return _equality_delete_impl->init(this->get_profile());
}
