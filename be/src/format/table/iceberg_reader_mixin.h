...
Status IcebergReaderMixin<BaseReader>::_equality_delete_base(
    ...
    for (const auto& delete_file : delete_files) {
        ...
        // Build delete col names/types/ids for columns that need to be read
        ...
        for (uint32_t idx = 0; idx < equality_delete_col_names.size(); ++idx) {
            ...
            // Use idx as a placeholder col_id (actual field_id mapping is format-specific)
            if (idx < read_column_field_ids.size()) {
                delete_col_ids.emplace_back(read_column_field_ids[idx]);
            }
        }
