...
Status PaimonRustReader::get_next_block(Block* block, size_t* read_rows, bool* eof) {
    if (_push_down_agg_type == TPushAggOp::type::COUNT && _remaining_table_level_row_count >= 0) {
...
Status PaimonRustReader::get_columns(std::unordered_map<std::string, DataTypePtr>* name_to_type,
