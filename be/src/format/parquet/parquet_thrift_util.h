...
static Status parse_thrift_footer(io::FileReaderSPtr file,
                                  ...
                                  bool retain_serialized_metadata = false) {
    ...
    if (retain_serialized_metadata) {
        // Small-file v2 opens still need Arrow's page-index adapter. Retaining the bytes already
        // fetched from storage avoids serializing the just-decoded Thrift footer a second time.
        serialized_metadata.assign(meta_ptr, meta_ptr + metadata_size);
    }
