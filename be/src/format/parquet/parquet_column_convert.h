...
inline IColumn* get_mutable_inner_column(ColumnPtr& column) {
    ...
    IColumn* inner_column = nullptr;
    if (mutable_column->is_nullable()) {
        inner_column = &assert_cast<ColumnNullable*>(mutable_column.get())->get_nested_column();
    } else {
        inner_column = mutable_column.get();
    }
...
inline void align_null_map(ColumnPtr& src_column, ColumnPtr& dst_column, size_t old_null_map_size,
    ...
    auto mutable_dst_column = IColumn::mutate(std::move(dst_column));
    auto* dst_nullable = assert_cast<ColumnNullable*>(mutable_dst_column.get());
    ...
    if (dst_null_map.size() == expected_rows) {
        dst_column = std::move(mutable_dst_column);
        return;
    }
