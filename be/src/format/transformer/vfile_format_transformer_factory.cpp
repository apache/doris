...
#include "format/transformer/vfile_format_transformer_factory.h"
...
Status create_tvf_format_transformer(const TTVFTableSink& tvf_sink, RuntimeState* state,
    ...
    if (tvf_sink.__isset.writer_type && tvf_sink.writer_type == TTVFWriterType::JNI) {
        ...
        result->reset(new VJniFormatTransformer(state, output_vexpr_ctxs, writer.substr(0, sep),
                                                writer.substr(sep + 1), std::move(writer_params)));
        return Status::OK();
    }
