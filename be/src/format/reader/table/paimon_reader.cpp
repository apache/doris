...
void annotate_column_from_field(reader::ColumnDefinition* column,
                                const schema::external::TField& field);

void annotate_struct_children(reader::ColumnDefinition* column,
...
Status PaimonReader::prepare_split(const reader::SplitReadOptions& options) {
    _split_schema_id = -1;
    ...
    return reader::TableReader::prepare_split(options);
}
...
Status PaimonReader::annotate_file_schema(std::vector<reader::ColumnDefinition>* file_schema) {
    DORIS_CHECK(file_schema != nullptr);
    if (mapping_mode() != reader::TableColumnMappingMode::BY_FIELD_ID) {
        return Status::OK();
    }
