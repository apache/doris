...
Status PlanFragmentExecutor::prepare(const TExecPlanFragmentParams& request,
    ...
    RETURN_IF_CATCH_EXCEPTION(RETURN_IF_ERROR(ExecNode::create_tree(
    ...
    if (request.fragment.__isset.output_sink) {
        RETURN_IF_CATCH_EXCEPTION(RETURN_IF_ERROR(DataSink::create_data_sink(
                obj_pool(), request.fragment.output_sink, request.fragment.output_exprs, params,
                row_desc(), runtime_state(), &_sink, *desc_tbl)));
        RETURN_IF_CATCH_EXCEPTION(RETURN_IF_ERROR(_sink->prepare(runtime_state())));
