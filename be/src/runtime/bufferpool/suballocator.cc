...
Status Suballocator::Allocate(int64_t bytes, unique_ptr<Suballocation>* result) {
    ...
    if (UNLIKELY(bytes > MAX_ALLOCATION_BYTES)) {
        ...
        err_stream << "Requested memory allocation of " << bytes << " bytes, larger than max "
...
Status Suballocator::SplitToSize(unique_ptr<Suballocation> free_node, int64_t target_bytes,
                                 unique_ptr<Suballocation>* result) {
...
unique_ptr<Suballocation> Suballocator::RemoveFromFreeList(Suballocation* node) {
    ...
    unique_ptr<Suballocation>* ptr_from_prev =
...
unique_ptr<Suballocation> Suballocator::CoalesceBuddies(unique_ptr<Suballocation> b1,
                                                        unique_ptr<Suballocation> b2) {
    DCHECK(!b1->in_use_);
    DCHECK(!b2->in_use_);
