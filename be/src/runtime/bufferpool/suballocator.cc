...
Status Suballocator::SplitToSize(std::unique_ptr<Suballocation> free_node,
    int64_t target_bytes, std::unique_ptr<Suballocation>* result) {
...
std::unique_ptr<Suballocation> Suballocator::RemoveFromFreeList(Suballocation* node) {
  ...
  std::unique_ptr<Suballocation>* ptr_from_prev = node->prev_free_ == nullptr ?
...
std::unique_ptr<Suballocation> Suballocator::CoalesceBuddies(
