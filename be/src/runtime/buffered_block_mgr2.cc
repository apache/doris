...
Status BufferedBlockMgr2::create(RuntimeState* state, MemTracker* parent, RuntimeProfile* profile,
...
Status BufferedBlockMgr2::register_client(int num_reserved_buffers, MemTracker* tracker,
...
bool BufferedBlockMgr2::consume_memory(Client* client, int64_t size) {
    ...
    if (std::max<int64_t>(0, remaining_unreserved_buffers()) + client->_num_tmp_reserved_buffers <
        buffers_needed) {
        return false;
    }
