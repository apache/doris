...
// The first four layers are MemTrackerLimiter with limit, and the fifth layer is NewMemTracker without limit.
// Specific parts of the fragment (exec nodes, sinks, etc) will add a fifth level when they are initialized.
class MemTrackerLimiter final : public NewMemTracker {
public:
    ...
    void make_snapshot(std::vector<NewMemTracker::Snapshot>* snapshots, size_t cur_level,
...
private:
    ...
    static std::string limit_exceeded_errmsg_sys_str(int64_t bytes) {
        ...
        ExecEnv::GetInstance()->new_process_mem_tracker()->print_log_usage(err_msg);
        return err_msg;
    }
...
private:
    ...
    // Group number in NewMemTracker::mem_tracker_pool, generated by the timestamp.
    int64_t _group_num;
