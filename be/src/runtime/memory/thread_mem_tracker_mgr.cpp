...
void ThreadMemTrackerMgr::exceeded(int64_t failed_consume_size, Status failed_try_consume_rt) {
    ...
    if (is_attach_task()) {
        if (_exceed_cb.cancel_task) {
            auto st = _limiter_tracker->mem_limit_exceeded(
                    fmt::format("query mem limit exceeded and cancel it, {}", _exceed_cb.cancel_msg),
                    failed_consume_size, failed_try_consume_rt, _fragment_instance_id);
