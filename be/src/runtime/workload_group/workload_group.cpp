...
WorkloadGroupInfo WorkloadGroupInfo::parse_topic_info(
    ...
    int min_flush_thread_num = std::max(1, config::wg_flush_thread_num_per_store);
...
Status WorkloadGroup::create_dummy_wg(std::shared_ptr<WorkloadGroup>* dummy_wg) {
    int exec_thread_num = config::pipeline_executor_size <= 0 ? CpuInfo::num_cores()
    ...
    WorkloadGroupInfo wg_info = {
            ...
            .memory_limit = 0,
            .enable_memory_overcommit = ENABLE_MEMORY_OVERCOMMIT_DEFAULT_VALUE,
            .version = 0,
            .cpu_hard_limit = CPU_HARD_LIMIT_DEFAULT_VALUE,
            .scan_thread_num = config::doris_scanner_thread_pool_thread_num,
            .max_remote_scan_thread_num =
                    vectorized::ScannerScheduler::get_remote_scan_thread_num(),
            .min_remote_scan_thread_num = config::doris_scanner_min_thread_pool_thread_num,
            .memory_low_watermark = MEMORY_LOW_WATERMARK_DEFAULT_VALUE,
            .memory_high_watermark = MEMORY_HIGH_WATERMARK_DEFAULT_VALUE,
            .read_bytes_per_second = -1,
            .remote_read_bytes_per_second = -1,
            .total_query_slot_count = TOTAL_QUERY_SLOT_COUNT_DEFAULT_VALUE,
            .slot_mem_policy = TWgSlotMemoryPolicy::NONE,
            .write_buffer_ratio = LOAD_BUFFER_RATIO_DEFAULT_VALUE,
            .pipeline_exec_thread_num = exec_thread_num,
            .max_flush_thread_num = max_flush_thread_num,
            .min_flush_thread_num = 1};

    *dummy_wg = std::make_shared<WorkloadGroup>(wg_info);

#ifndef BE_TEST
    RETURN_IF_ERROR((*dummy_wg)->upsert_task_scheduler(&wg_info));
#endif
