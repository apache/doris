...
void WorkloadGroupMgr::init_internal_workload_group() {
    WorkloadGroupPtr internal_wg = nullptr;
    {
        ...
        if (_workload_groups.find(INTERNAL_WORKLOAD_GROUP_ID) == _workload_groups.end()) {
            ...
            LOG(INFO) << "create internal group succ, wg_id=" << internal_wg_info.id;
        }
    }
    DCHECK(internal_wg != nullptr);
    if (internal_wg) {
        internal_wg->create_cgroup_cpu_ctl();
        internal_wg->set_need_create_query_thread_pool(false);
        if (std::shared_ptr<CgroupCpuCtl> ptr = internal_wg->get_cgroup_cpu_ctl_wptr().lock()) {
            LOG(INFO) << "internal group init cgroup cpu ctl succ.";
        } else {
            LOG(INFO) << "internal group init cgroup cpu ctl failed.";
        }
    }
}
