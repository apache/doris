...
private:
    ...
    std::mutex _mutex;
    std::shared_ptr<vectorized::HashTableVariants> _hash_table_variants;
    std::shared_ptr<std::vector<vectorized::Block>> _build_blocks;
    std::shared_ptr<std::unordered_map<const vectorized::Block*, std::vector<int>>> _inserted_rows;
    std::shared_ptr<vectorized::Arena> _arena;
    std::shared_ptr<vectorized::Sizes> _probe_key_sz;
    std::shared_ptr<vectorized::Sizes> _build_key_sz;
...
public:
    ...
    bool shared_hash_table_operate(vectorized::SharedHashTableContext& shared_hash_table_ctx);
    bool shared_hash_table_barrier();
    Status get_callback(vectorized::shared_hash_table_operator* hash_table_operator,
