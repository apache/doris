...
double GeoLine::Length() const {
    if (_polyline == nullptr || _polyline->num_vertices() < 2) {
        return 0.0;
    }
    
    // Calculate the total length of the polyline by summing distances between consecutive points
...
double GeoPolygon::Length() const {
    if (_polygon == nullptr || _polygon->num_loops() == 0) {
        return 0.0;
    }
