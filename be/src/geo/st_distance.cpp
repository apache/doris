...
double GeoPoint::Distance(const GeoShape* rhs) const {
    // rhs is guaranteed to be valid by StDualGeoDoubleFunction (functions_geo.cpp) which checks shapes[0] && shapes[1]
    switch (rhs->type()) {
...
double GeoCircle::Distance(const GeoShape* rhs) const {
    // Both rhs and self are guaranteed to be valid by StDualGeoDoubleFunction (functions_geo.cpp) which checks shapes[0] && shapes[1]
