...
namespace doris {
    ...
    std::unique_ptr<GeoShape> GeoJsonParse::read_geometry(rapidjson::Document& document ,GeoParseStatus& parse_status){
    ...
    std::unique_ptr<GeoPoint> GeoJsonParse::read_point(rapidjson::Document& document ,GeoParseStatus& parse_status){
        ...
        if(document.HasMember("coordinates") && document["coordinates"].IsArray()){
            if(document["coordinates"].Size() == 0 ){
                ...
                std::unique_ptr<GeoCoordinate> coord = read_coordinate(document["coordinates"],parse_status);
    ...
    std::unique_ptr<GeoLineString> GeoJsonParse::read_linestring(rapidjson::Document& document ,GeoParseStatus& parse_status){
        ...
        if(document.HasMember("coordinates") && document["coordinates"].IsArray()){
            if(document["coordinates"].Size() == 0 ){
                ...
                std::unique_ptr<GeoCoordinates> coords= read_coordinates(document["coordinates"],parse_status);
                if(parse_status==GEO_PARSE_OK){
    ...
    std::unique_ptr<GeoPolygon> GeoJsonParse::read_polygon(rapidjson::Document& document ,GeoParseStatus& parse_status){
        ...
        if(document.HasMember("coordinates") && document["coordinates"].IsArray()){
            if(document["coordinates"].Size() == 0 ){
                ...
                std::unique_ptr<GeoCoordinateLists> coords_list= read_coords_list(document["coordinates"],parse_status);
                if(parse_status==GEO_PARSE_OK){
    ...
    std::unique_ptr<GeoMultiPoint> GeoJsonParse::read_multi_point(rapidjson::Document& document ,GeoParseStatus& parse_status){
    ...
    std::unique_ptr<GeoMultiLineString> GeoJsonParse::read_multi_linestring(rapidjson::Document& document ,GeoParseStatus& parse_status){
    ...
    std::unique_ptr<GeoMultiPolygon> GeoJsonParse::read_multi_polygon(rapidjson::Document& document ,GeoParseStatus& parse_status){
        ...
        if(document.HasMember("coordinates") && document["coordinates"].IsArray()){
            if(document["coordinates"].Size() == 0 ){
                ...
                std::unique_ptr<GeoCoordinateListCollections> coords_list_collections= read_coords_list_collections(document["coordinates"],parse_status);
                if(parse_status==GEO_PARSE_OK){
    ...
    std::unique_ptr<GeoCollection> GeoJsonParse::read_geometry_collection(rapidjson::Document& document ,GeoParseStatus& parse_status){
        ...
        if(document.HasMember("geometries") && document["geometries"].IsArray()){
            if(document["geometries"].Size() == 0 ){
                ...
                for(std::size_t i = 0; i < document["geometries"].Size(); ++i){
                    ...
                    auto status = collection->add_one_geometry(read_geometry(doc,parse_status).release());
    ...
    std::unique_ptr<GeoCoordinateListCollections> GeoJsonParse::read_coords_list_collections(rapidjson::Value& coords_array ,GeoParseStatus& parse_status){
        std::unique_ptr<GeoCoordinateListCollections> coords_list_collections(new GeoCoordinateListCollections());
        for (int i = 0; i < coords_array.Size(); ++i) {
            std::unique_ptr<GeoCoordinateLists> coords_list = read_coords_list(coords_array[i],parse_status);
            if(parse_status!=GEO_PARSE_OK){
                return nullptr;
            }
    ...
    std::unique_ptr<GeoCoordinateLists> GeoJsonParse::read_coords_list(rapidjson::Value& coords_array ,GeoParseStatus& parse_status){
    ...
    std::unique_ptr<GeoCoordinates> GeoJsonParse::read_coordinates(rapidjson::Value& coords_array ,GeoParseStatus& parse_status){
    ...
    std::unique_ptr<GeoCoordinate> GeoJsonParse::read_coordinate(rapidjson::Value& coord_array ,GeoParseStatus& parse_status){
        if(LIKELY(coord_array.Size() == 2 && (coord_array[0].IsDouble() || coord_array[0].IsInt()) && (coord_array[1].IsDouble() || coord_array[1].IsInt()))){
            parse_status = GEO_PARSE_OK;
            return std::make_unique<GeoCoordinate>(coord_array[0].GetDouble(),coord_array[1].GetDouble());
        }else {
