...
Status truncate_orc_timestamp_to_microseconds(int64_t seconds, int64_t nanoseconds,
    ...
    // Doris exposes ORC timestamps at microsecond precision. Discard sub-microsecond digits so
    // native ORC and JNI readers retain the same visible value and predicate semantics.
    result->seconds = seconds;
    result->microseconds = cast_set<uint64_t>(nanoseconds / NANOS_PER_MICROSECOND);
    return Status::OK();
}
