...
ColumnArrayDataOffsets filter_return_new_dispatch(const Filter& filt, ssize_t result_size_hint,
    ...
    if (check_and_get_column<ColumnDateTimeV2Nano>(data.get())) {
        return filter_number_return_new<TYPE_DATETIMEV2_NANO>(filt, result_size_hint, data,
                                                              offsets);
    }
...
size_t filter_inplace_dispatch(const Filter& filter, IColumn& src_data,
    ...
    if (check_and_get_column<ColumnDateTimeV2Nano>(&src_data)) {
        return filter_number_inplace<TYPE_DATETIMEV2_NANO>(filter, src_data, src_offsets);
    }
