...
public:
    ...
    // are copied into owned shredded field children as they arrive; structural values stay in the caller's
    // residual row. The builder retains neither source columns nor borrowed Variant refs.
    class Batch {
    public:
        ~Batch();
        ...
        // Returns false without mutating the field when the value is structural or cannot be
        // appended losslessly to the fixed typed layout; the caller must keep it in the residual.
        bool append_value(size_t path_index, VariantRef value);
        void append_scalar(size_t path_index, const VariantScalarRef& scalar);
    ...
    // Exact scalar identities accepted by the row-local typed appender without an encoded bridge.
    static bool supports_direct_typed_append(PrimitiveType type) noexcept;
