...
public:
    ...
    struct TestAccess {
        ...
        static size_t shredded_union_rebuilds(const ColumnVariantV2& column);
        static size_t shredded_conflict_slow_rows(const ColumnVariantV2& column);
        static size_t shredded_union_existing_child_rows_copied(const ColumnVariantV2& column);
        static size_t shredded_union_existing_presence_rows_copied(const ColumnVariantV2& column);
    ...
    // dense 0/1 presence column with the same row count. Active child rows must be scalars. Paths
    // are normalized, sorted, unique, and prefix-free before the new state is published. Every
    // active shredded field is also checked to be disjoint from residual without materializing an encoded
    // copy.
...
private:
    ...
    void _append_same_shredded_layout_rows(const ColumnVariantV2& source,
                                           const Selection& selection);
    template <typename Selection>
    void _append_shredded_mapped_rows(const ColumnVariantV2& source,
                                      const DorisVector<uint8_t>& source_active,
                                      const ColumnVariantV2& residual, const Selection& selection);
    template <typename Selection>
    void _append_shredded_conflicts_to_residual(
    ...
    template <typename Selection>
    void _replace_with_shredded_union(MutablePtr residual, const ColumnVariantV2& source,
                                      const DorisVector<uint8_t>& source_active,
                                      const DorisVector<const ShreddedField*>& source_only,
                                      const Selection& selection);
    ...
    // In S, the encoded members above are the residual rows. Each dense field applies only where
    // presence is one. An inner Variant null is therefore distinct from a missing field.
    ShreddedFields _shredded_fields;
    // A Block may admit at most one cross-source union, capped at the original field count. Thus
    // the first layout can grow by at most 2x; later physical paths are folded into residual so
    // dense child count cannot grow with every segment layout.
    bool _shredded_layout_frozen = false;
#ifdef BE_TEST
    size_t _test_shredded_union_rebuilds = 0;
    size_t _test_shredded_conflict_slow_rows = 0;
    size_t _test_shredded_union_existing_child_rows_copied = 0;
    size_t _test_shredded_union_existing_presence_rows_copied = 0;
