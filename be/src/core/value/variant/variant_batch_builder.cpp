...
public:
    ...
    void add_scalar(const VariantScalarRef& scalar) {
        if (scalar.encoded_size() <= INLINE_SCALAR_CAPACITY) {
            prepare_value_node();
            std::array<char, INLINE_SCALAR_CAPACITY> inline_bytes {};
            scalar.write_physical(inline_bytes.data(), scalar.encoded_size());
            complete_inline_scalar(inline_bytes, scalar.encoded_size());
            return;
        }

        const uint32_t offset = prepare_arena_scalar(scalar.encoded_size());
        scalar_bytes.resize(scalar_bytes.size() + scalar.encoded_size());
        scalar.write_physical(scalar_bytes.data() + offset, scalar.encoded_size());
        complete_scalar(offset);
    }

    void add_null() { add_scalar(VariantScalarRef::null_value()); }
    ...
    void import_primitive(VariantRef value) {
        ...
        case VariantPrimitiveId::DOUBLE:
            add_scalar(VariantScalarRef::float64(value.get_double()));
            return;
        ...
        case VariantPrimitiveId::DATE:
            add_scalar(VariantScalarRef::date(value.get_date()));
            return;
        case VariantPrimitiveId::TIMESTAMP_MICROS:
            add_scalar(VariantScalarRef::timestamp_micros(value.get_timestamp_micros(), true));
            return;
        case VariantPrimitiveId::TIMESTAMP_NTZ_MICROS:
            add_scalar(VariantScalarRef::timestamp_micros(value.get_timestamp_ntz_micros(), false));
            return;
        case VariantPrimitiveId::BINARY:
            add_scalar(VariantScalarRef::binary(value.get_binary()));
            return;
        case VariantPrimitiveId::STRING: {
            const StringRef string = value.get_string();
            require_import_utf8(string, "string");
            add_scalar(VariantScalarRef::string(string));
            return;
        }
        case VariantPrimitiveId::TIME_NTZ_MICROS:
            add_scalar(VariantScalarRef::time_ntz_micros(value.get_time_ntz_micros()));
            return;
        case VariantPrimitiveId::TIMESTAMP_NANOS:
            add_scalar(VariantScalarRef::timestamp_nanos(value.get_timestamp_nanos(), true));
            return;
        case VariantPrimitiveId::TIMESTAMP_NTZ_NANOS:
            add_scalar(VariantScalarRef::timestamp_nanos(value.get_timestamp_ntz_nanos(), false));
            return;
        case VariantPrimitiveId::UUID:
            add_scalar(VariantScalarRef::uuid(value.get_uuid()));
            return;
        }
    }
...
void VariantBatchBuilder::_add_scalar(uint64_t generation, const VariantScalarRef& scalar) {
    _impl->ensure_active(generation);
    _impl->collection.add_scalar(scalar);
}
...
void VariantBatchBuilder::Row::add_uuid(const std::array<uint8_t, 16>& value) {
    _add_scalar(VariantScalarRef::uuid(value));
}

void VariantBatchBuilder::Row::add_largeint(__int128 value) {
    if (variant_unsigned_magnitude(value) <= VARIANT_DECIMAL16_MAX) {
        _add_scalar(VariantScalarRef::decimal(value, 0, 16));
        return;
    }
    std::array<char, 40> buffer {};
    const auto size = static_cast<size_t>(LargeIntValue::to_buffer(value, buffer.data()));
    _add_scalar(VariantScalarRef::string({buffer.data(), size}));
}

void VariantBatchBuilder::Row::add_scalar(const VariantScalarRef& scalar) {
    _add_scalar(scalar);
}
...
void VariantBatchBuilder::Row::_add_scalar(const VariantScalarRef& scalar) {
    if (_builder == nullptr) {
        throw Exception(ErrorCode::INVALID_ARGUMENT, "Variant block row handle is moved-from");
    }
    _builder->_add_scalar(_generation, scalar);
}
