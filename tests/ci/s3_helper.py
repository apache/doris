...
# https://github.com/ClickHouse/ClickHouse/blob/master/tests/ci/s3_helper.py
...
from env_helper import S3_TEST_REPORTS_BUCKET, S3_BUILDS_BUCKET, RUNNER_TEMP
...
class S3Helper:
    def __init__(self, host):
        ...
        role_confidential = requests.get(url='http://100.100.100.200/latest/meta-data/Ram/security-credentials/doris-build')
        self.session = boto3.session.Session(region_name='beijing',
            aws_access_key_id = role_confidential.json()['AccessKeyId'],
            aws_secret_access_key = role_confidential.json()['AccessKeySecret'],
            aws_session_token = role_confidential.json()['SecurityToken'])
    ...
    def _upload_file_to_s3(self, bucket_name, file_path, s3_path):
        ...
        return self.host + "/{bucket}/{path}".format(bucket=bucket_name, path=s3_path) \
