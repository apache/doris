...
# https://github.com/ClickHouse/ClickHouse/blob/master/docker/test/performance-comparison/perf.py
...
# Open connections
servers = zip(args.host, args.port)
all_connections = [pymysql.connect(host=server, port=int(port), db="tests", charset='utf8') for server, port in servers]
...
for t in tables:
    for c in all_connections:
        try:
            res = c.query("select 1 from {} limit 1".format(t))
        except:
...
if not args.use_existing_tables:
    ...
    def do_create(connection, index, queries):
        for q in queries:
            q_begin_time = time.perf_counter()
            print(f'create\t{index}\t{tsv_escape(q)}')
            try:
                connection.query(q)
            except pymysql.err.OperationalError as e:
                e.detailMessage = e.detailMessage + q
                raise
            exec_time = time.perf_counter() - q_begin_time
            print(f'create\t{index}\t{exec_time}\t{tsv_escape(q)}')
...
for query_index in queries_to_run:
    ...
    for conn_index, c in enumerate(all_connections):
        try:
            query_begin_time = time.perf_counter()
            try:
                ...
                e.message = q + e.message
                raise
            exec_time = time.perf_counter() - query_begin_time
            print(f'prewarm\t{query_index}\t{conn_index}\t{exec_time}')
    ...
    while True:
        ...
        for conn_index, c in enumerate(this_query_connections):
            ...
            elapsed = time.perf_counter() - query_begin_time
...
if not args.keep_created_tables and not args.use_existing_tables:
    ...
    for conn_index, c in enumerate(all_connections):
        for q in drop_queries:
            ...
            c.query(q)
            exec_time = time.perf_counter() - query_begin_time
            print(f'drop\t{conn_index}\t{exec_time}\t{tsv_escape(q)}')
