...
# https://github.com/ClickHouse/ClickHouse/blob/master/docker/test/performance-comparison/report.py
...
{{ text-align: right; }}
  ...
  <title>apache doris performance comparison</title>
</head>
<body>
<div class="main">

<h1>apache doris performance comparison</h1>
"""
...
def tableStart(title):
    ...
    return f"""
        <h2 id="{anchor}">
            ...
            <a class="cancela" href="https://github.com/apache/incubator-doris/tree/master/tests/ci/performance-comparison#{help_anchor}"><sup style="color: #888">?</sup></a>
        </h2>
        <table class="{cls}">
    """
    ...
    try:
        addSimpleTable('Tested Commits', ['Old', 'New'],
            [['<pre>{}</pre>'.format(x) for x in
                [open('left-commit.txt', encoding='utf-8').read(),
                 open('right-commit.txt', encoding='utf-8').read()]]])
    except:
