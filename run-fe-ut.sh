...
# every plugin has a probe that loads it out of a built directory - so leaving it unrun means the
# next change to a plugin's dependency closure is only caught by a user's query.
#
