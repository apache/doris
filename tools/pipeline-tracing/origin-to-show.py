...
class Record:
    ...
    def to_json(self) :
        json = {"name": self.task_name, "cat": self.state_name, "ph": "X", "ts": self.start_time, "dur": self.end_time - self.start_time,
                "pid": self.core_id, "tid": self.thread_id}
        return json
...
parser = argparse.ArgumentParser(description='Accept file to analyse. And how to illustrate it')
parser.add_argument('-s', '--source', help='the path of tracing record file to analyze')
parser.add_argument('-d', '--dest', help='the path of json result file you want to save')
