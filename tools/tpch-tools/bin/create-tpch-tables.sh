...
if [[ ${SCALE_FACTOR} -eq 1 ]]; then
    ...
    if ! mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" \
        <"${CURDIR}/../ddl/create-tpch-tables-sf1.sql" 2>&1; then
        echo "Error: Failed to execute SQLs from create-tpch-tables-sf1.sql" >&2
        exit 1
    fi
    ...
    if ! mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" \
        ...
        echo "Error: Failed to execute SQLs from create-tpch-tables-sf100.sql" >&2
        exit 1
    fi
    ...
    if ! mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" \
        ...
        echo "Error: Failed to execute SQLs from create-tpch-tables-sf1000.sql" >&2
        exit 1
    fi
elif [[ ${SCALE_FACTOR} -eq 10000 ]]; then
    echo "Run SQLs from ${CURDIR}/../ddl/create-tpch-tables-sf10000.sql"
    if ! mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" \
        <"${CURDIR}/../ddl/create-tpch-tables-sf10000.sql" 2>&1; then
        echo "Error: Failed to execute SQLs from create-tpch-tables-sf10000.sql" >&2
        exit 1
    fi
else
