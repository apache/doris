...
run_sql "set global enable_nereids_planner=true;"
run_sql "select @@enable_nereids_planner;"

sum=0
IFS=';'
qs=$(sed $'/--/d;s/`//g;s/\'/\"/g' "${QUERIES_DIR}/tpch-query-from-starrocks.sql")
i=1
for q in $qs; do
    total=0
    run=3
    ...
    for ((j = 0; j < run; j++)); do
        start=$(date +%s%3N)
        mysql -h"${FE_HOST}" -u "${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" --comments -e"$q" >/dev/null
        end=$(date +%s%3N)
...
run_sql "set global enable_nereids_planner=false;"
