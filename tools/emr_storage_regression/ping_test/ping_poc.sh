...
run_spark_create_sql() {
    if [[ ${SERVICE} == 'ali' ]]; then
        PARAM="--conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions \
            ...
            --conf spark.sql.catalog.iceberg.dlf.endpoint=${ENDPOINT} \
...
case "${SERVICE}" in
    ali)
        ...
        # sed -e 's#DLF_ENDPOINT#'"${DLF_ENDPOINT}"'#g' create_catalog_aliyun.sql > emr_catalog.sql
        # JINDO_ENDPOINT=cn-beijing.oss-dls.aliyuncs.com
        # sed -e 's#JINDO_ENDPOINT#'"${JINDO_ENDPOINT}"'#g' create_catalog_aliyun.sql > emr_catalog.sql
        ;;
    tx)
