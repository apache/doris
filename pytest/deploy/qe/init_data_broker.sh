...
mysql -h ${FE_HOST} -P${FE_QUERY_PORT} -uroot -p${FE_PASSWORD} -e "create table ${FE_DB}.baseall(k1 tinyint, k2 smallint, k3 int, k4 bigint, k5 decimal(9,3), k6 char(5), k10 date, k11 datetime, k7 varchar(20), k8 double max, k9 float sum) engine=olap distributed by hash(k1) buckets 5 properties('storage_type'='column')"
...
mysql -h ${FE_HOST} -P${FE_QUERY_PORT} -uroot -p${FE_PASSWORD} -e "LOAD LABEL ${FE_DB}.label_5 (DATA INFILE('$hdfs_data_path/sys/all_type.txt') INTO TABLE all_type_table COLUMNS TERMINATED BY '$separator') $broker_property PROPERTIES('max_filter_ratio'='0.05')"
