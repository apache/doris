...
def test_change_partition_rollup():
    """
    {
    "title": "test_sys_partition_schema_change_rollup_aggregate.test_change_partition_rollup",
    ...
    util.assert_return(False, 'Rollup should contains all keys if there is a REPLACE value',
    ...
    util.assert_return(True, '',
                       ...
                       add_column_list=['v4 float sum NULL to ' + index_name],
                       is_wait=True)
    ...
    util.assert_return(False, 'Can not add column which already exists in base table: v2',
                       client.schema_change, table_name,
                       add_column_list=['v2 float sum NULL to ' + index_name],
                       is_wait=True)

    util.assert_return(False, 'Float or double can not used as a key, use decimal instead.',
                       client.schema_change, table_name,
                       add_column_list=['v5 float NULL to ' + index_name],
                       is_wait=True)

    util.assert_return(True, '',
                       client.schema_change, table_name,
                       add_column_list=['v6 decimal(9,3) sum NULL to ' + index_name],
                       is_wait=True)

    assert client.get_column('v6', table_name, database_name)
    client.clean(database_name)
