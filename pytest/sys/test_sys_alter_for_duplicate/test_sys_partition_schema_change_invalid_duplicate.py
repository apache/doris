...
def partition_check(table_name, column_name, partition_name_list, partition_value_list,
                    distribution_type, bucket_num, storage_type):
    """
    检查，验证
    """
...
def test_change_partition_key_and_bucket_key_invalid():
    """
    {
    "title": "test_sys_partition_schema_change_invalid_duplicate.test_change_partition_key_and_bucket_key_invalid",
    "describe": "分区列和分桶列不能修改",
    "tag": "system,p1,fuzz"
    }
    """
    """
    ...
    util.assert_return(False, 'Can not modify partition column',
                       client.schema_change_modify_column, table_name, 'k1', 'BIGINT KEY NULL',
    ...
    util.assert_return(False, 'Can not assign aggregation method on column in Duplicate data model table',
                       client.schema_change_modify_column, table_name, 'v2', 'float',
    ...
    util.assert_return(False, 'Can not change default value',
                       ...
                       aggtype='-', column_info='NOT NULL DEFAULT "0.1"',
                       is_wait_job=True, is_wait_delete_old_schema=True)
