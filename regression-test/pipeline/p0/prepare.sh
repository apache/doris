...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/p0/prepare.sh ]]; then
    ...
    echo "Build Step file missing: regression-test/pipeline/p0/prepare.sh" && exit 1
fi
EOF
#####################################################################################
## prepare.sh content ##
...
set -x
if ! [[ -d "${teamcity_build_checkoutDir}"/regression-test/pipeline/p0/ &&
