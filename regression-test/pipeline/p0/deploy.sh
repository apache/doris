...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/p0/deploy.sh ]]; then
    cd "${teamcity_build_checkoutDir}"/regression-test/pipeline/p0
    bash -x deploy.sh
else
    echo "Build Step file missing: regression-test/pipeline/p0/deploy.sh" && exit 1
fi
EOF
#####################################################################################
...
(
    ...
    set -e
    echo "#### 3. copy conf from regression-test/pipeline/p0/conf/ and modify"
    ...
    cp -f "${teamcity_build_checkoutDir}"/regression-test/pipeline/p0/conf/fe_custom.conf "${DORIS_HOME}"/fe/conf/
    cp -f "${teamcity_build_checkoutDir}"/regression-test/pipeline/p0/conf/be_custom.conf "${DORIS_HOME}"/be/conf/
    print_doris_conf
    ...
    session_variables_file="${teamcity_build_checkoutDir}/regression-test/pipeline/p0/conf/session_variables.sql"
