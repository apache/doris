...
bash "${teamcity_build_checkoutDir}"/tools/tpch-tools/bin/run-tpch-queries.sh | tee run-tpch-queries.log
line_end=$(sed -n '/^Total hot run time/=' run-tpch-queries.log)
...
$(sed -n "${line_begin},${line_end}" run-tpch-queries.log)"
