...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/external/run.sh ]]; then
    cd "${teamcity_build_checkoutDir}"/regression-test/pipeline/external/
    bash -x run.sh
else
    echo "Build Step file missing: regression-test/pipeline/external/run.sh" && exit 1
fi
EOF
############################# run.sh content ########################################
...
echo "#### Run external test on Doris ####"
...
run() {
    ...
    args="${RUN_THIRDPARTIES_DOCKER_ARGS:-" -c mysql,es,hive2,hive3,pg,oracle,sqlserver,clickhouse,mariadb,iceberg,db2,oceanbase,kerberos,kafka"}"
    if grep 'polaris' "${teamcity_build_checkoutDir}"/docker/thirdparties/run-thirdparties-docker.sh; then
        args+=',polaris'
    fi
    if "${teamcity_build_checkoutDir}"/docker/thirdparties/run-thirdparties-docker.sh -h | grep -q -- --reserve-ports; then
        args+=" --reserve-ports"
    fi
    # shellcheck disable=SC2086
    if bash "${teamcity_build_checkoutDir}"/docker/thirdparties/run-thirdparties-docker.sh ${args}; then
        echo
    else
        echo "ERROR: start external docker failed"
    fi
