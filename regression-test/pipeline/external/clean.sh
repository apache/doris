...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/external/clean.sh ]]; then
    cd "${teamcity_build_checkoutDir}"/regression-test/pipeline/external/
    bash -x clean.sh
else
    echo "Build Step file missing: regression-test/pipeline/external/clean.sh" && exit 1
fi
EOF
############################# clean.sh content ########################################
