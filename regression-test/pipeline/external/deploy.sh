...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/external/deploy.sh ]]; then
    cd "${teamcity_build_checkoutDir}"/regression-test/pipeline/external
    bash -x deploy.sh
else
    echo "Build Step file missing: regression-test/pipeline/external/deploy.sh" && exit 1
fi
EOF
#####################################################################################
...
(
    ...
    set -e
    echo "#### 3. copy conf from regression-test/pipeline/external/conf/ and modify"
    ...
    cp -f "${teamcity_build_checkoutDir}"/regression-test/pipeline/external/conf/fe_custom.conf "${DORIS_HOME}"/fe/conf/
    cp -f "${teamcity_build_checkoutDir}"/regression-test/pipeline/external/conf/be_custom.conf "${DORIS_HOME}"/be/conf/
    print_doris_conf
    ...
    session_variables_file="${teamcity_build_checkoutDir}/regression-test/pipeline/external/conf/session_variables.sql"
