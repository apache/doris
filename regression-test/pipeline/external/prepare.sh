...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/external/prepare.sh ]]; then
    ...
    echo "Build Step file missing: regression-test/pipeline/external/prepare.sh" && exit 1
fi
EOF
#####################################################################################
## prepare.sh content ##
...
if _get_pr_changed_files "${pr_num_from_trigger}"; then
    if ! file_changed_regression_external; then
        bash "${teamcity_build_checkoutDir}"/regression-test/pipeline/common/get-or-set-tmp-env.sh 'set' "export skip_pipeline=true"
        exit 0
    fi
fi
...
set -x
if ! [[ -d "${teamcity_build_checkoutDir}"/regression-test/pipeline/external/ &&
