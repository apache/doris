...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/vault_p0/prepare.sh ]]; then
    ...
    echo "Build Step file missing: regression-test/pipeline/vault_p0/prepare.sh" && exit 1
fi
EOF
#####################################################################################
## prepare.sh content ##
