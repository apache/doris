...
if [[ -f "${teamcity_build_checkoutDir:-}"/regression-test/pipeline/vault_p0/deploy.sh ]]; then
    cd "${teamcity_build_checkoutDir}"/regression-test/pipeline/vault_p0
    bash -x deploy.sh
else
    echo "Build Step file missing: regression-test/pipeline/vault_p0/deploy.sh" && exit 1
fi
EOF
#####################################################################################
...
(
    ...
    set -e
    echo "#### 3. copy conf from regression-test/pipeline/vault_p0/conf/ and modify"
    ...
    cat "${teamcity_build_checkoutDir}"/regression-test/pipeline/vault_p0/conf/ms_custom.conf >>"${DORIS_HOME}"/ms/conf/doris_cloud.conf
    echo >>"${DORIS_HOME}"/ms/conf/doris_cloud.conf
    cat "${teamcity_build_checkoutDir}"/regression-test/pipeline/vault_p0/conf/recycler_custom.conf >>"${DORIS_HOME}"/recycler/conf/doris_cloud.conf
