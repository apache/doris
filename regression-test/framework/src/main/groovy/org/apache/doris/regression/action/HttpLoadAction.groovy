...
@Slf4j
class HttpLoadAction implements SuiteAction {
    ...
    HttpLoadAction(SuiteContext context) {
    ...
    void run() {
        ...
        try {
            def uri = "http://${address.hostString}:${address.port}/api/v2/_load"
            ...
            HttpClients.createDefault().withCloseable { client ->
                ...
                String beLocation = httpLoadToFe(client, requestBuilder)
                responseText = httpLoadToBe(client, requestBuilder, beLocation, httpEntity)
            }
        } catch (Throwable t) {
            ex = t
        }
        ...
        log.info("Http load elapsed ${endTime - startTime} ms, response: ${responseText}".toString() +
                 ex.toString())
    ...
    private HttpEntity prepareHttpEntity(CloseableHttpClient client) {
        ...
        if (inputStream != null) {
            ...
            if (fileName.startsWith("http://") || fileName.startsWith("https://")) {
                log.info("Set Http load input: ${fileName}".toString())
            ...
            if (!file.exists()) {
                log.warn("Http load input file not exists: ${file}".toString())
                throw new IllegalStateException("Http load input file not exists: ${file}");
            }
            log.info("Set Http load input: ${file.canonicalPath}".toString())
            entity = new FileEntity(file)
        }
        return entity
    }

    private String httpLoadToFe(CloseableHttpClient client, RequestBuilder requestBuilder) {
        log.info("Http load to ${requestBuilder.uri}".toString())
        String backendhttpLoadUri = null
        client.execute(requestBuilder.build()).withCloseable { resp ->
            resp.withCloseable {
                ...
                if (respCode != 307) {
                    throw new IllegalStateException("Expect frontend Http load response code is 307, " +
                            "but meet ${respCode}\nbody: ${body}")
                }
                backendhttpLoadUri = resp.getFirstHeader("location").getValue()
            }
        }
        return backendhttpLoadUri
    }

    private String httpLoadToBe(CloseableHttpClient client, RequestBuilder requestBuilder, String beLocation, HttpEntity httpEntity) {
        log.info("Redirect Http load to ${beLocation}".toString())
        ...
        try{
            client.execute(requestBuilder.build()).withCloseable { resp ->
                resp.withCloseable {
                    ...
                    def respCode = resp.getStatusLine().getStatusCode()
                    if (respCode != 200) {
                        throw new IllegalStateException("Expect backend Http load response code is 200, " +
                                "but meet ${respCode}\nbody: ${body}")
                    }
                    responseText = body
                }
            ...
            log.info("HttpLoadAction Exception: ", t)
        }
        return responseText
    }
    ...
    private void checkResult(String responseText, Throwable ex, long startTime, long endTime) {
        ...
        log.info("The origin Http load result: ${responseText}, final status: ${finalStatus}")
        ...
        if (check != null) {
            ...
            if (!"Success".equalsIgnoreCase(status)) {
                ...
                if (errorUrl != null) {
                    ...
                    throw new IllegalStateException("Http load failed:\n${responseText}\n${errorDetails}")
                }
                throw new IllegalStateException("Http load failed:\n${responseText}")
            }
            ...
            if (numberTotalRows != numberLoadedRows) {
                throw new IllegalStateException("Http load rows mismatch:\n${responseText}")
            }
    ...
    // Sometime the Http load may return "PUBLISH TIMEOUT"
    ...
    private String waitForPublishOrFailure(String responseText) {
        try {
            ...
            log.info("Http load with txn ${txnId} is publish timeout")
            ...
            log.info("Http load with txn ${txnId} is ${st}")
