...
Suite.metaClass.uuidMatrixRows = { ->
...
Suite.metaClass.uuidMatrixSchema = { ->
...
Suite.metaClass.uuidMatrixValues = { ->
    delegate.uuidMatrixRows().withIndex().collect { row, i -> "(${i},${row.values().join(',')})" }.join(',\n')
}
...
Suite.metaClass.uuidRunMatrix = { String group, String table, List<String> columns, Closure expressions,
    ...
    for (String mode : ['fe', 'be', 'runtime']) {
        ...
        for (int mask = 0; mask <= allColumns; ++mask) {
            List<Integer> samples = mask == allColumns ? [0] : (0..<rows.size()).toList()
            for (int sample : samples) {
                List<String> args = columns.withIndex().collect { column, i ->
                    (mask & (1 << i)) != 0 ? column : rows[sample][column]
                }
