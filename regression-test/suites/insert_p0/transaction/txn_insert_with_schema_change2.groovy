...
suite("txn_insert_with_schema_change2") {
    def table = "txn_insert_with_schema_change2"
    ...
    sql """ insert into ${table}_1 values(1, '21', 11), (2, '22', 12) """
    ...
    def txnInsert = { sqls ->
        try (Connection conn = DriverManager.getConnection(url, context.config.jdbcUser, context.config.jdbcPassword);
            ...
            try {
                ...
                logger.info("execute sql: ${sqls[2]}")
                statement.execute(sqls[2])
    ...
    def insert_sqls =  [
        "insert into ${table}_0(id, name, score) select id, name, score from ${table}_1 where id < 2;",
        "insert into ${table}_0(id, name, score) select id, name, score from ${table}_1 where id >= 2;",
        "insert into ${table}_1(id, name, score) select id, name, score from ${table}_0;"
    ]

    Thread insert_thread = new Thread(() -> txnInsert(insert_sqls))
    Thread schema_change_thread = new Thread(() -> schemaChange("alter table ${table}_0 order by (id, score, name);", "${table}_0", "WAITING_TXN"))
    insert_thread.start()
    ...
    getAlterTableState("${table}_0", "FINISHED")
    order_qt_select3 """select id, name, score from ${table}_0 """
    order_qt_select4 """select id, name, score from ${table}_1 """
    check_table_version_continuous(dbName, table + "_" + 0)
    check_table_version_continuous(dbName, table + "_" + 1)
}
