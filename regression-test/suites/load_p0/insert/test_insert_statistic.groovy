...
suite("test_insert_statistic", "p0") {
    // Get master FE connection info
    // SHOW LOAD requires master FE because load metadata is only maintained on master
    def masterFe = sql_return_maparray("show frontends").find { it.IsMaster == "true" }
    if (masterFe == null) {
        logger.warn("No master FE found, skip test")
        return
    }
    def masterUrl = "jdbc:mysql://${masterFe.Host}:${masterFe.QueryPort}/"
    logger.info("Connect to master FE: ${masterUrl}")

    connect(context.config.jdbcUser, context.config.jdbcPassword, masterUrl) {
