...
suite("test_s3_load_with_where", "load_p0") {
    ...
    def load_from_s3_with_or_predicate = {testTablex, label, s3FilePath, format ->
        def result1= sql """
                        LOAD LABEL ${label} (
                            DATA INFILE("${s3FilePath}")
                            INTO TABLE ${testTablex}
    ...
    def s3_csv_file_path = "s3://${getS3BucketName()}/regression/load/data/broker_load_with_where.csv"
    ...
    load_from_s3_with_or_predicate.call(testTable, test_load_label, s3_csv_file_path, "CSV")
