...
suite("test_kinesis_routine_load_condition") {
    ...
    def streamName = "doris-test-cond-${UUID.randomUUID().toString().substring(0, 8)}"
    def tableName = "test_kinesis_condition"
    def jobName = "testKinesisCondition"
    ...
    try {
        ...
        logger.info("Filtered rows: ${result[0][0]}")
        assertTrue(result[0][0] >= 30 && result[0][0] < 50)

        result = sql "SELECT MIN(age), MAX(age) FROM ${tableName}"
