...
suite("test_kinesis_routine_load_swap_table_restart", "nonConcurrent") {
    ...
    def streamName = "doris-swap-${UUID.randomUUID().toString().substring(0, 8)}"
    def tableName = "test_kinesis_swap"
    def tableName2 = "test_kinesis_swap2"
    def jobName = "testKinesisSwap"
    ...
    try {
        ...
        logger.info("Before swap: ${count1[0][0]} rows")

        sql "ALTER TABLE ${tableName} SWAP WITH ${tableName2}"

        Thread.sleep(10000)

        def count2 = sql "SELECT COUNT(*) FROM ${tableName2}"
        logger.info("After swap: ${count2[0][0]} rows")
