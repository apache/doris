...
suite("test_kinesis_routine_load_offset") {
    ...
    def streamName = "doris-offset-${UUID.randomUUID().toString().substring(0, 8)}"
    def tableName = "test_kinesis_offset"
    def jobName = "testKinesisOffset"
    ...
    try {
        ...
        // Write more data after job started
        for (int i = 31; i <= 50; i++) {
            def data = "{\"id\": ${i}, \"name\": \"user_${i}\"}"
            kinesisClient.putRecord(new PutRecordRequest()
                .withStreamName(streamName)
        ...
        logger.info("Loaded rows with LATEST: ${result[0][0]}")
        assertTrue(result[0][0] >= 20 && result[0][0] < 50)  // Should only load new data
