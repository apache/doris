...
suite("test_kinesis_routine_load_task_exception_recovery", "nonConcurrent") {
    ...
    def streamName = "doris-exception-${UUID.randomUUID().toString().substring(0, 8)}"
    def tableName = "test_kinesis_exception"
    def jobName = "testKinesisException"
    ...
    try {
        ...
        for (int i = 1; i <= 60; i++) {
            def data = (i % 10 == 0) ?
                "{\"id\": \"invalid\", \"value\": ${i}}" :  // Invalid every 10th record
                "{\"id\": ${i}, \"value\": ${i * 10}}"
        ...
        assertTrue(result[0][0] >= 54)  // At least 54 valid records (60 - 6 invalid)

        def jobInfo = sql "SHOW ROUTINE LOAD FOR ${jobName}"
        logger.info("Job state: ${jobInfo[0][8]}")
        assertTrue(jobInfo[0][8] == "RUNNING")
