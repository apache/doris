...
suite("test_kinesis_routine_load_with_sc") {
    ...
    def streamName = "doris-sc-${UUID.randomUUID().toString().substring(0, 8)}"
    def tableName = "test_kinesis_sc"
    def jobName = "testKinesisSC"
    ...
    try {
        ...
        logger.info("Before schema change: ${count1[0][0]} rows")

        // Perform schema change
        sql "ALTER TABLE ${tableName} ADD COLUMN age INT DEFAULT 0"

        Thread.sleep(10000)

        // Write more data with new column
        for (int i = 31; i <= 50; i++) {
        ...
        logger.info("After schema change: ${count2[0][0]} rows")
        assertTrue(count2[0][0] >= 50)
