            ...
            while (true) {
                ...
                httpTest {
                    ...
                    check { code, body ->
                        ...
                        if (body.contains("doris_fe_routine_load_task_execute_time")){
                            log.info("contain doris_fe_routine_load_task_execute_time")
                            metricCount++
                        }
                        if (body.contains("doris_fe_routine_load_task_execute_count")){
                            log.info("contain doris_fe_routine_load_task_execute_count")
                            metricCount++
                        }
                        if (body.contains("doris_fe_routine_load_get_meta_latency")){
                            log.info("contain doris_fe_routine_load_get_meta_latency")
                            metricCount++
                        }
                        if (body.contains("doris_fe_routine_load_get_meta_count")){
                            log.info("contain doris_fe_routine_load_get_meta_count")
                            metricCount++
                        }
                        if (body.contains("doris_fe_routine_load_get_meta_fail_count")){
                            log.info("contain doris_fe_routine_load_get_meta_fail_count")
                            metricCount++
                        }
                        if (body.contains("doris_fe_routine_load_receive_bytes")){
                            log.info("contain doris_fe_routine_load_receive_bytes")
                            metricCount++
                        }
