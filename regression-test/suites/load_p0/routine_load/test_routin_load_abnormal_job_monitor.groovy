...
suite("test_routine_load_abnormal_job_monitor","nonConcurrent") {
    def kafkaCsvTpoics = [
                  "test_user",
                ]
    ...
    def jobName3 = "test_abnormal_job_monitor3"
    ...
    if (enabled != null && enabled.equalsIgnoreCase("true")) {
        try {
            ...
            sql "sync"
            sql """
                CREATE ROUTINE LOAD ${jobName3} on ${tableName}
                COLUMNS(k00,k01,k02,k03,k04,k05,k06,k07,k08,k09,k10,k11,k12,k13,k14,k15,k16,k17,k18),
                COLUMNS TERMINATED BY "|"
                PROPERTIES
                (
            ...
            // 2. check abnormal jobs
            def count = 0
            while (true) {
                def command = "curl -X GET --location-trusted -u ${context.config.feHttpUser}:${context.config.feHttpPassword} http://${context.config.feHttpAddress}/api/routine_load/abnormal_jobs"
                log.info("abnormal jobs: ${command}")
                def process = command.execute()
                def code = process.waitFor()
                def out = process.text
                log.info("result: ${out}".toString())
                def jsonSlurper = new JsonSlurper()
                def result = jsonSlurper.parseText(out)
                log.info("result: ${result}".toString())

                def name1 = context.config.getDbNameByFile(context.file) + "." + jobName1
                def name2 = context.config.getDbNameByFile(context.file) + "." + jobName2
                def name3 = context.config.getDbNameByFile(context.file) + "." + jobName3
                log.info("except: ${name1}, ${name2}, ${name3}".toString())

                if (result.toString().contains(name1.toString())
                        && result.toString().contains(name2.toString())
                        && result.toString().contains(name3.toString())) {
                    break;
                }
            ...
            // 3. make job normal
            try {
                sql "pause routine load for ${jobName1}"
            } catch (Exception e) {
                log.info("exception: ${e}".toString())
            }
            sql "ALTER ROUTINE LOAD FOR ${jobName1} FROM KAFKA(\"kafka_topic\" = \"test_abnormal_job_monitor\");"
            sql "resume routine load for ${jobName1}"
            sql "stop routine load for ${jobName2}"
            GetDebugPoint().disableDebugPointForAllBEs("FragmentMgr.exec_plan_fragment.failed")

            // 4. check doris_fe_routine_load_abnormal_job_nums is zero
            count = 0
            while (true) {
                def end = false
                httpTest {
                    ...
                    check { code, body ->
                        ...
                        def entry = result.find { it.tags?.metric == "doris_fe_routine_load_abnormal_job_nums" }
                        def value = entry ? entry.value : null
                        log.info("Contains routine_load_abnormal_job_nums: ${entry != null}".toString())
                        log.info("Value of routine_load_abnormal_job_nums: ${value}".toString())
                        if (value == 0) {
                            end = true
                        }
                    }
            ...
            sql "stop routine load for ${jobName3}"
