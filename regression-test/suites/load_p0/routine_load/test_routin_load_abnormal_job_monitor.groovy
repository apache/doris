...
suite("test_routine_load_abnormal_job_monitor","nonConcurrent") {
    ...
    def jobName = "test_abnormal_job_monitor"
    ...
    if (enabled != null && enabled.equalsIgnoreCase("true")) {
        try {
            ...
            while (true) {
                ...
                def name = context.config.getDbNameByFile(context.file) + "." + jobName
                log.info("except: ${name}".toString())

                if (result.toString().contains(name.toString())) {
                    break;
                }
            ...
            sql "ALTER ROUTINE LOAD FOR ${jobName} FROM KAFKA(\"kafka_topic\" = \"test_abnormal_job_monitor\");"
            ...
            // case 2: abort txn and job progress
            sql """
                CREATE ROUTINE LOAD ${jobName} on ${tableName}
                COLUMNS(k00,k01,k02,k03,k04,k05,k06,k07,k08,k09,k10,k11,k12,k13,k14,k15,k16,k17,k18),
                COLUMNS TERMINATED BY "|"
