...
suite("test_stream_load_audit_log", "nonConcurrent") {
    def tableName = "test_stream_load_record"
    def label = UUID.randomUUID().toString().replaceAll("-", "")

    try {
        set_be_param("enable_stream_load_record_to_audit_log_table", "true")
        ...
        while (true) {
            sleep(1000)
            def res = sql"select * from __internal_schema.audit_log where query_id = '${label}'"
            log.info("Audit log result: ${res.toString()}")
            if (res.size() > 0) {
                break
            }
            count++
            if (count > 150) {
                throw new RuntimeException("Audit log not found after 150 seconds")
            }
        }
    } finally {
        set_be_param("enable_stream_load_record_to_audit_log_table", "false")
    }
}