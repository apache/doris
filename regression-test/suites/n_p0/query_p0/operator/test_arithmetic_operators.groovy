...
suite("test_arithmetic_operators", "query,p0") {
    ...
    qt_arith_op1 "select k1, k4 div k1, k4 div k2, k4 div k3, k4 div k4 \
		    from test order by k1, k2, k3, k4"
    qt_arith_op2 "select k1, k1+ '1', k5,100000*k5 from test order by k1, k2, k3, k4"
    qt_arith_op3 "select k1,k5,k2*k5 from test order by k1, k2, k3, k4"
    qt_arith_op4 "select k1,k5,k8*k5,k5*k9,k2*k9,k2*k8 from test order by k1, k2, k3, k4"
    qt_arith_op5 "select k1, k5*0.1, k8*0.1, k9*0.1 from test order by k1, k2, k3, k4"
    qt_arith_op6 "select k1, k2*(-0.1), k3*(-0.1), k4*(-0.1), \
		    k5*(-0.1), k8*(-0.1), k9*(-0.1) from  test order by k1, k2, k3, k4"
    qt_arith_op7 "select k1, k5*(9223372036854775807/100), k8*9223372036854775807, \
		    k9*9223372036854775807 from  test order by k1, k2, k3, k4"
    ...
    qt_arith_op11 "select k1, k5/0.000001, k8/0.000001, \
		    k9/0.000001 from  test order by k1, k2, k3, k4"
    ...
    qt_arith_op16 "select k1+10, k2+10.0, k3+1.6, k4*1, k5-6, k8-234.66, k9-0 \
		    from test order by k1, k2, k3, k4"
    qt_arith_op17 "select * from test where k1+k9<0 order by k1, k2, k3, k4"
    qt_arith_op18 "select k1*k2*k3*k5 from test order by k1, k2, k3, k4"
    qt_arith_op19 "select k1*k2*k3*k5*k8*k9 from test order by k1, k2, k3, k4"
    qt_arith_op20 "select k1*10000/k4/k8/k9 from test order by k1, k2, k3, k4"

    for( i in [1, 2, 3, 5, 8, 9]) {
        for( j in [1, 2, 3, 5, 8, 9]) {
            qt_arith_op21 "select k${i}*k${j}, k${i}+k${j}, k${i}-k${j}, k${i}/k${j} from test \
			    where abs(k${i})<9223372036854775807 and k${j}<>0 and\
			    abs(k${i})<922337203685477580 order by k1, k2, k3, k4"
        }
    }

    qt_arith_op22 "select 1.1*1.1 + k2 from test order by 1 limit 10"
    qt_arith_op23 "select 1.1*1.1 + k5 from test order by 1 limit 10"
    ...
    qt_arith_op28 "select k5 / 0, k8 / 0, k9 / 0 from test order by k1,k2,k3,k4"
    qt_arith_op29 "select k5 % 0, k8 % 0, k9 % 0 from test order by k1,k2,k3,k4"
}
