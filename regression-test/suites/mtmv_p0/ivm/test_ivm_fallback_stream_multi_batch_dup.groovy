...
suite("test_ivm_fallback_stream_multi_batch_dup", "nonConcurrent") {
    ...
    sql """drop table if exists ivm_fbs_dup_o;"""
    sql """drop table if exists ivm_fbs_dup_c;"""

    sql """
        CREATE TABLE ivm_fbs_dup_o (
            order_id INT,
            dt INT,
            cid INT,
            amount INT
    ...
    sql """
        INSERT INTO ivm_fbs_dup_c VALUES
            (1, 'alice'),
            (101, 'bob');
    """
    sql """
        INSERT INTO ivm_fbs_dup_o VALUES
            (10, 20240101, 1, 10),
            (20, 20240102, 101, 20);
    """
    waitVisible()
    ...
    sql """
        CREATE MATERIALIZED VIEW ivm_fbs_dup_mv
        ...
        AS
        SELECT
            ivm_fbs_dup_o.dt AS dt,
            ivm_fbs_dup_o.order_id AS order_id,
            ivm_fbs_dup_o.cid AS cid,
            ivm_fbs_dup_o.amount AS amount,
            ivm_fbs_dup_c.name AS name
        FROM ivm_fbs_dup_o
        INNER JOIN ivm_fbs_dup_c
            ON ivm_fbs_dup_o.cid = ivm_fbs_dup_c.cid;
    """
    ...
    def queryBaseRows = {
        ...
        try {
            sql """
                ...
                FROM ivm_fbs_dup_o o
                INNER JOIN ivm_fbs_dup_c c
                    ON o.cid = c.cid
                ORDER BY o.dt, o.order_id, o.cid, o.amount, c.name
            """
        } finally {
    ...
    def queryMvRows = {
        sql """
            ...
            FROM ivm_fbs_dup_mv
            ORDER BY dt, order_id, cid, amount, name
        """
    }
    ...
    sql """REFRESH MATERIALIZED VIEW ivm_fbs_dup_mv COMPLETE"""
    waitingMTMVTaskFinishedByMvName("ivm_fbs_dup_mv")
    ...
    sql """INSERT INTO ivm_fbs_dup_o VALUES (12, 20240101, 1, 18);"""
    waitVisible()
    assertLagUnits(["p20240101"])
    sql """REFRESH MATERIALIZED VIEW ivm_fbs_dup_mv INCREMENTAL"""
    waitingMTMVTaskFinishedByMvName("ivm_fbs_dup_mv")
    assertMvEqualsBase()
    assertLagUnits([])
    ...
    sql """INSERT INTO ivm_fbs_dup_c VALUES (3, 'erin');"""
    waitVisible()
    assertLagUnits(["p100"])
    sql """REFRESH MATERIALIZED VIEW ivm_fbs_dup_mv INCREMENTAL"""
    waitingMTMVTaskFinishedByMvName("ivm_fbs_dup_mv")
    ...
    sql """INSERT INTO ivm_fbs_dup_o VALUES (13, 20240102, 3, 19);"""
    waitVisible()
