...
suite("test_rollup_partition_mtmv_date_add", "mtmv") {
    ...
    sql """drop materialized view if exists mv_test_rollup_partition_mtmv_date_add_raw_partition_expr"""
    ...
    // raw partition expression: PARTITION BY (date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day'))
    sql """
        CREATE MATERIALIZED VIEW mv_test_rollup_partition_mtmv_date_add_raw_partition_expr
            ...
            GROUP BY day_alias;
    """
    waitingMTMVTaskFinishedByMvName("mv_test_rollup_partition_mtmv_date_add_raw_partition_expr")
    def showCreate = sql """show create materialized view mv_test_rollup_partition_mtmv_date_add_raw_partition_expr"""
    assertTrue(showCreate.toString().toLowerCase().contains("partition by"))
    assertTrue(showCreate.toString().toLowerCase().contains("date_trunc"))
    assertTrue(showCreate.toString().toLowerCase().contains("date_add"))
    assertTrue(showCreate.toString().toLowerCase().contains("interval 3 hour"))
    assertFalse(showCreate.toString().toLowerCase().contains("date_add(date_add"))
    ...
    // DATE partition columns are not supported for hour-offset rollup.
    sql """drop materialized view if exists mv_test_rollup_partition_mtmv_date_add_date_col"""
    sql """drop table if exists t_test_rollup_partition_mtmv_date_add_date_col"""
    sql """
        CREATE TABLE t_test_rollup_partition_mtmv_date_add_date_col (
          id BIGINT NOT NULL,
          k2 DATE NOT NULL
        ) ENGINE=OLAP
        DUPLICATE KEY(id)
    ...
    test {
        sql """
            CREATE MATERIALIZED VIEW mv_test_rollup_partition_mtmv_date_add_date_col
                BUILD IMMEDIATE REFRESH AUTO ON MANUAL
                partition by (date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day'))
                DISTRIBUTED BY RANDOM BUCKETS 1
                PROPERTIES ('replication_num' = '1')
                AS
                SELECT date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day') AS day_alias, count(*) AS cnt
                FROM t_test_rollup_partition_mtmv_date_add_date_col
                GROUP BY day_alias;
        """
        exception "partitionColumnType should be datetime"
    }

    // TIMESTAMPTZ partition columns are not supported for hour-offset rollup.
    sql """drop materialized view if exists mv_test_rollup_partition_mtmv_date_add_timestamptz_col"""
    sql """drop table if exists t_test_rollup_partition_mtmv_date_add_timestamptz_col"""
    sql """
        CREATE TABLE t_test_rollup_partition_mtmv_date_add_timestamptz_col (
          id BIGINT NOT NULL,
          k2 TIMESTAMPTZ(6) NOT NULL
        ) ENGINE=OLAP
        DUPLICATE KEY(id)
        PARTITION BY range(k2)
        (
          PARTITION p_20250724 VALUES [("2025-07-24 21:00:00+00:00"),("2025-07-25 21:00:00+00:00")),
          PARTITION p_20250725 VALUES [("2025-07-25 21:00:00+00:00"),("2025-07-26 21:00:00+00:00"))
        )
        DISTRIBUTED BY HASH(id) BUCKETS 1
        PROPERTIES ('replication_num' = '1');
    """
    test {
        sql """
            CREATE MATERIALIZED VIEW mv_test_rollup_partition_mtmv_date_add_timestamptz_col
                BUILD IMMEDIATE REFRESH AUTO ON MANUAL
                partition by (date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day'))
                DISTRIBUTED BY RANDOM BUCKETS 1
                PROPERTIES ('replication_num' = '1')
                AS
                SELECT date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day') AS day_alias, count(*) AS cnt
                FROM t_test_rollup_partition_mtmv_date_add_timestamptz_col
                GROUP BY day_alias;
        """
        exception "partitionColumnType should be datetime"
    }
    ...
    // DATETIMEV2(6) partition boundaries within the first second (epsilon upper bound handling)
    sql """drop materialized view if exists mv_test_rollup_partition_mtmv_date_add_datetimev2_epsilon"""
    sql """drop table if exists t_test_rollup_partition_mtmv_date_add_datetimev2_epsilon"""
    sql """
        CREATE TABLE t_test_rollup_partition_mtmv_date_add_datetimev2_epsilon (
          id BIGINT NOT NULL,
          k2 DATETIMEV2(6) NOT NULL
        ) ENGINE=OLAP
        DUPLICATE KEY(id)
        PARTITION BY range(k2)
        (
          PARTITION p_20250724 VALUES [("2025-07-24 21:00:00.100000"),("2025-07-24 21:00:00.900000")),
          PARTITION p_20250725 VALUES [("2025-07-25 21:00:00.100000"),("2025-07-25 21:00:00.900000"))
        )
        DISTRIBUTED BY HASH(id) BUCKETS 1
        PROPERTIES ('replication_num' = '1');
    """
    sql """
        INSERT INTO t_test_rollup_partition_mtmv_date_add_datetimev2_epsilon VALUES
            (1, "2025-07-24 21:00:00.100000"),
            (2, "2025-07-24 21:00:00.500000"),
            (3, "2025-07-25 21:00:00.200000");
    """
    sql """
        CREATE MATERIALIZED VIEW mv_test_rollup_partition_mtmv_date_add_datetimev2_epsilon
            BUILD IMMEDIATE REFRESH AUTO ON MANUAL
            partition by (date_trunc(day_alias, 'day'))
            DISTRIBUTED BY RANDOM BUCKETS 1
            PROPERTIES ('replication_num' = '1')
            AS
            SELECT date_trunc(date_add(k2, INTERVAL 3 HOUR), 'day') AS day_alias, count(*) AS cnt
            FROM t_test_rollup_partition_mtmv_date_add_datetimev2_epsilon
            GROUP BY day_alias;
    """
    waitingMTMVTaskFinishedByMvName("mv_test_rollup_partition_mtmv_date_add_datetimev2_epsilon")
    def epsilonShowPartitions = sql """show partitions from mv_test_rollup_partition_mtmv_date_add_datetimev2_epsilon"""
    assertEquals(2, epsilonShowPartitions.size())
    assertTrue(epsilonShowPartitions.toString().contains("2025-07-25 00:00:00"))
    assertTrue(epsilonShowPartitions.toString().contains("2025-07-26 00:00:00"))
