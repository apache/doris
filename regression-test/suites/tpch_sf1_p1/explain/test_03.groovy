...
suite("test_explain_tpch_sf_1_q03", "tpch_sf1") {
    ...
    explain {
		...
		explainStr.contains("VTOP-N\n" + 
				"  |  order by: <slot 14> <slot 13> sum(<slot 32> * (1 - <slot 33>)) DESC, <slot 15> <slot 11> `o_orderdate` ASC") && 
		explainStr.contains("VAGGREGATE (update finalize)\n" + 
				"  |  output: sum(<slot 32> * (1 - <slot 33>))\n" + 
				"  |  group by: <slot 31>, <slot 36>, <slot 37>") && 
		explainStr.contains("join op: INNER JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 27> = `c_custkey`\n" + 
				"  |  runtime filters: RF000[in_or_bloom] <- `c_custkey`") && 
		explainStr.contains("vec output tuple id: 6") && 
		explainStr.contains("output slot ids: 31 32 33 36 37 \n" + 
				"  |  hash output slot ids: 20 21 22 25 26 ") && 
		...
		explainStr.contains("output slot ids: 20 21 22 25 26 27 \n" + 
		...
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.lineitem(lineitem), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts. conjunct on `L_SHIPDATE` which is StorageEngine value column\n" + 
				"     PREDICATES: `l_shipdate` > '1995-03-15 00:00:00', `default_cluster:regression_test_tpch_sf1_p1.lineitem`.`__DORIS_DELETE_SIGN__` = 0\n" + 
		...
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.customer(customer), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts.\n" + 
				"     PREDICATES: `c_mktsegment` = 'BUILDING', `default_cluster:regression_test_tpch_sf1_p1.customer`.`__DORIS_DELETE_SIGN__` = 0") && 
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.orders(orders), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts.\n" + 
				"     PREDICATES: `o_orderdate` < '1995-03-15 00:00:00', `default_cluster:regression_test_tpch_sf1_p1.orders`.`__DORIS_DELETE_SIGN__` = 0\n" + 
