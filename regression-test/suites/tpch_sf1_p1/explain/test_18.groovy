...
suite("test_explain_tpch_sf_1_q18", "tpch_sf1") {
    ...
    explain {
		...
		explainStr.contains("VAGGREGATE (update finalize)\n" + 
				"  |  output: sum(<slot 66>)\n" + 
				"  |  group by: <slot 73>, <slot 74>, <slot 68>, <slot 69>, <slot 70>") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 53> = <slot 8> `l_orderkey`\n" + 
				"  |  runtime filters: RF000[in_or_bloom] <- <slot 8> `l_orderkey`") && 
		explainStr.contains("vec output tuple id: 14") && 
		explainStr.contains("output slot ids: 66 68 69 70 73 74 \n" + 
				"  |  hash output slot ids: 54 56 57 58 61 62 ") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 44> = <slot 2> `l_orderkey`\n" + 
				"  |  runtime filters: RF001[in_or_bloom] <- <slot 2> `l_orderkey`") && 
		explainStr.contains("vec output tuple id: 13") && 
		explainStr.contains("output slot ids: 53 54 56 57 58 61 62 \n" + 
				"  |  hash output slot ids: 49 50 41 42 44 45 46 ") && 
		explainStr.contains("join op: INNER JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 38> = `c_custkey`\n" + 
				"  |  runtime filters: RF002[in_or_bloom] <- `c_custkey`") && 
		explainStr.contains("vec output tuple id: 12") && 
		explainStr.contains("output slot ids: 41 42 44 45 46 49 50 \n" + 
				"  |  hash output slot ids: 32 33 35 36 37 12 13 ") && 
		...
		explainStr.contains("output slot ids: 32 33 35 36 37 38 \n" + 
				"  |  hash output slot ids: 16 17 5 11 14 15 ") && 
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.lineitem(lineitem), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts. Aggregate Operator not match: SUM <--> REPLACE\n" + 
