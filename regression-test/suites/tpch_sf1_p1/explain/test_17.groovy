...
suite("test_explain_tpch_sf_1_q17", "tpch_sf1") {
    ...
    explain {
		...
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 20> = <slot 2> `l_partkey`\n" + 
				"  |  other join predicates: <slot 41> < 0.2 * <slot 45>") && 
		explainStr.contains("other join predicates: <slot 41> < 0.2 * <slot 45>\n" + 
		...
		explainStr.contains("output slot ids: 16 17 20 \n" + 
				"  |  hash output slot ids: 6 7 8 ") && 
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.lineitem(lineitem), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts. conjunct on `L_QUANTITY` which is StorageEngine value column\n" + 
		...
		explainStr.contains("VAGGREGATE (update finalize)\n" + 
				...
				"  |  group by: `l_partkey`") && 
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.lineitem(lineitem), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts. Invalid Aggregate Operator: avg\n" + 
				"     PREDICATES: `default_cluster:regression_test_tpch_sf1_p1.lineitem`.`__DORIS_DELETE_SIGN__` = 0") && 
		explainStr.contains("TABLE: default_cluster:regression_test_tpch_sf1_p1.part(part), PREAGGREGATION: OFF. Reason: __DORIS_DELETE_SIGN__ is used as conjuncts.\n" + 
				"     PREDICATES: `p_brand` = 'Brand#23', `p_container` = 'MED BOX', `default_cluster:regression_test_tpch_sf1_p1.part`.`__DORIS_DELETE_SIGN__` = 0\n" + 
