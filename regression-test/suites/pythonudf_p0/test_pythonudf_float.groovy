...
suite("test_pythonudf_float") {
    def tableName = "test_pythonudf_float"
    ...
    try {
        ...
        qt_select """ SELECT user_id,python_udf_float_test(float_1, float_2) as sum FROM ${tableName} order by user_id; """
        createMV("create materialized view udf_mv as SELECT user_id,python_udf_float_test(float_1, float_2) as sum FROM test_pythonudf_float order by user_id;")
        qt_select """ SELECT user_id,python_udf_float_test(float_1, float_2) as sum FROM ${tableName} order by user_id; """

        explain {
            sql("SELECT user_id,python_udf_float_test(float_1, float_2) as sum FROM ${tableName} order by user_id; ")
            contains "(udf_mv)"
        }
        ...
        qt_select """ SELECT user_id,python_udf_double_test(double_1, double_1) as sum FROM ${tableName} order by user_id; """
