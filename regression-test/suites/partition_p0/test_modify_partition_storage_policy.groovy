...
suite("test_modify_partition_storage_policy") {
    if (isCloudMode()) {
        return
    }

    def resource_name = "test_modify_partition_storage_policy_resource"
    def policy_name= "test_modify_partition_storage_policy"
    def tableName = "test_modify_partition_storage_policy_tbl_synced"
	...
	test{
		sql """alter table ${tableName} modify partition p1 set ("storage_policy" = "${policy_name}");"""
		exception "Table is being synced, cannot set storage policy"
	}
}