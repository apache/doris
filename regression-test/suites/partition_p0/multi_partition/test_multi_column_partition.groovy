...
suite("test_multi_partition_key", "p0") {
    ...
    assertEquals(ret_drop_p.size(), 0)
    ...
    // partition columns is not adjacent, add key columns
    testPartitionTbl(
    ...
    assertEquals(getAlterColumnFinalState("test_multi_col_test_partition_key_add_col"), "FINISHED")
    ...
    assertEquals(getAlterColumnFinalState("test_multi_column_drop_partition_column"), "FINISHED")
    test {
        sql "ALTER TABLE test_multi_column_drop_partition_column DROP COLUMN k1"
    ...
    // test partition delete, 多列分区，删除分区中的数据
    qt_sql11 "DELETE FROM test_multi_column_drop_partition_column PARTITION partition_d WHERE k2 = '10' AND k3 > '10'"
    // test partition rollup, 多列分区，创建rollup
    testPartitionTbl(
    ...
    assertEquals(getAlterRollupFinalState("test_multi_col_test_rollup"), "FINISHED")
    qt_sql12 "desc test_multi_col_test_rollup all"
    // test partition drop rollup, 多列分区，删除rollup
    sql "ALTER TABLE test_multi_col_test_rollup DROP ROLLUP idx"
    assertEquals(getAlterRollupFinalState("test_multi_col_test_rollup"), "FINISHED")
    qt_sql13 "desc test_multi_col_test_rollup all"
    // test partition modify, check partition replication_num 修改分区列类型
    ...
    qt_sql14 "select * from test_multi_col_insert order by k1, k2"
