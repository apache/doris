...
suite("txn_insert") {
    ...
    if (backends.size() < 3) {
        logger.info("backends num is ${backends.size()} < 3, skip")
        return
    }

    def table = "txn_insert"
    ...
    for (def insert_sqls: sqls) {
        ...
        if (true) {
            ...
            Thread schema_change_thread = new Thread(() -> schemaChange("ALTER TABLE ${table}_0 SET ( \"default.replication_allocation\" = \"tag.location.default: 1\" ) ", null))
            insert_thread.start()
