...
suite("test_need_read_data", "p0"){
    ...
    qt_sql "SELECT /*+SET_VAR(enable_segment_limit_pushdown = false,inverted_index_skip_threshold=100) */ id FROM ${indexTblName3} WHERE value<0 and abs(value)>1;"
    qt_sql "SELECT /*+SET_VAR(enable_segment_limit_pushdown=true,inverted_index_skip_threshold=100) */ id FROM ${indexTblName3} WHERE value<0 and abs(value)>1;"
