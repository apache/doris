...
// a variant path whatever the property says. Norms are not visible from outside a SNII file, so
// this suite reads them off the scores: in every table, rows 1 and 2 match "alpha" once each, but
// row 1 is three tokens long and row 2 one. With norms the longer row scores lower; without norms
// both rows score the same, and neither is NaN.
...
suite("test_storage_format_snii_norms", "p0,nonConcurrent") {
    ...
    // index hands to its subcolumns: rows 1 and 2 score the same
    order_qt_snii_field_pattern_no_norms_score """
        select id, round(score(), 4) from test_storage_format_snii_norms
        where cast(v["t_note"] as string) match_any "alpha" order by score() desc limit 10
    """
    order_qt_snii_whole_column_no_norms_score """
        select id, round(score(), 4) from test_storage_format_snii_norms
    ...
    order_qt_snii_config_plain_column_score """
        ...
        where content match_any "alpha" order by score() desc limit 10
    """
    order_qt_snii_config_field_pattern_score """
        select id, round(score(), 4) from test_storage_format_snii_norms_config
        where cast(v["s_note"] as string) match_any "alpha" order by score() desc limit 10
    """
    order_qt_snii_config_norms_true_score """
        select id, round(score(), 4) from test_storage_format_snii_norms_config
