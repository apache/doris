...
suite("test_materialized_view_date", "rollup") {
    ...
    sql "CREATE materialized index amt_max1 AS SELECT store_id, max(sale_date1) FROM ${tbName1} GROUP BY store_id;"
    while (max_try_secs--) {
    ...
    Thread.sleep(2000)
    max_try_secs = 120
    sql "CREATE materialized index amt_max2 AS SELECT store_id, max(sale_datetime1) FROM ${tbName1} GROUP BY store_id;"
    while (max_try_secs--) {
        String res = getJobState(tbName1)
        if (res == "FINISHED") {
            sleep(3000)
    ...
    Thread.sleep(2000)
    max_try_secs = 120
    sql "CREATE materialized index amt_max3 AS SELECT store_id, max(sale_datetime2) FROM ${tbName1} GROUP BY store_id;"
    while (max_try_secs--) {
        String res = getJobState(tbName1)
        if (res == "FINISHED") {
            sleep(3000)
    ...
    Thread.sleep(2000)
    max_try_secs = 120
    sql "CREATE materialized index amt_max4 AS SELECT store_id, max(sale_datetime3) FROM ${tbName1} GROUP BY store_id;"
    while (max_try_secs--) {
        String res = getJobState(tbName1)
        if (res == "FINISHED") {
            sleep(3000)
