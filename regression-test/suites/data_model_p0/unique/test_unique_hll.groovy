...
suite("test_unique_table_hll") {
    ...
    for (def enable_mow : [true, false]) {
        for (def use_nereids : [true, false]) {
            ...
            qt_from_uniq "select k, hll_cardinality(v) from ${tbName} order by k, hll_cardinality(v);"
            qt_from_uniq "select k, hll_cardinality(hll_union(v)) from ${tbName} group by k order by k, hll_cardinality(hll_union(v));"
            ...
            sql "DROP TABLE IF EXISTS ${tbName}"
            test {
                sql """ CREATE TABLE IF NOT EXISTS ${tbName} (k hll, v int) UNIQUE KEY(k)
                    DISTRIBUTED BY HASH(k) BUCKETS 1 properties("replication_num" = "1"); """
