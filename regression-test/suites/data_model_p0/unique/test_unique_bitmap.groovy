...
suite("test_unique_table_bitmap") {
    ...
    qt_sql "select k,bitmap_count(id_bitmap) from ${tbName} order by k;"
    sql "insert into ${tbName} values(3,to_bitmap(7));"
    sql "insert into ${tbName} values(1,bitmap_or(to_bitmap(3),to_bitmap(1000)));"
    qt_sql "select k,bitmap_count(id_bitmap) from ${tbName} order by k;"
    ...
    qt_sql "select k,bitmap_count(id_bitmap) from ${tbName2} order by k;"
    sql "insert into ${tbName2} values(3,to_bitmap(7));"
    sql "insert into ${tbName2} values(1,bitmap_or(to_bitmap(3),to_bitmap(1000)));"
    qt_sql "select k,bitmap_count(id_bitmap) from ${tbName2} order by k;"
