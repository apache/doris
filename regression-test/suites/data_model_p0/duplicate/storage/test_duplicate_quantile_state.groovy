...
suite("test_duplicate_table_quantile_state") {
    ...
    for (def use_nereids : [true, false]) {
        ...
        qt_from_dup "select k, quantile_percent(v, 0) c1, quantile_percent(v, 0.5) c2, quantile_percent(v, 1) c3 from ${tbName} order by k, c1, c2, c3;"
        qt_from_dup """ select k, quantile_percent(QUANTILE_UNION(v), 0) c1, quantile_percent(QUANTILE_UNION(v), 0.5) c2, quantile_percent(QUANTILE_UNION(v), 1) c3
                from ${tbName} group by k order by k, c1, c2, c3; """
