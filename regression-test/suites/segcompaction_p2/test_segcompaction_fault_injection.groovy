...
suite("test_segcompaction_correctness") {
    ...
    try {
        ...
        // (TODO) make it a helper func
        for ( e in backendId_to_backendIP ) {
            def beid = e.key
            def host = e.value
            def port = backendId_to_backendHttpPort.get(beid)
            def url = 'http://' + host + ':' + port + '/api/debug_point/add/' + 'SegcompactionWorker_check_correctness_wrong_sum_src_row'
            def httpresult = Http.http_post(url, null, true)
            assert httpresult.status == 'OK' : httpresult.toString()
        }
        ...
        for ( e in backendId_to_backendIP ) {
            ...
            def host = e.value
            def port = backendId_to_backendHttpPort.get(beid)
            def url = 'http://' + host + ':' + port + '/api/debug_point/remove/' + 'SegcompactionWorker_check_correctness_wrong_sum_src_row'
            def httpresult = Http.http_post(url, null, true)
            assert httpresult.status == 'OK' : httpresult.toString()
        }
    }
