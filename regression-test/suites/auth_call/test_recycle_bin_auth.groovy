...
suite("test_recycle_bin_auth", "p0,auth_call") {
    ...
    sql """
        ...
        """
    // give the user full privileges on the database. erasing the recycle bin must still be
    // refused: it takes a raw object id and is irreversible, so it is ADMIN only.
    ...
    connect(user, "${pwd}", context.config.jdbcUrl) {
        ...
        test {
            sql """DROP CATALOG RECYCLE BIN WHERE 'PartitionId' = ${tableId};"""
            exception "denied"
        }
    }
    ...
    def erasedRes = sql """SHOW CATALOG RECYCLE BIN WHERE NAME = "${tableName}" """
