...
suite("test_file_cache_write_index_file_only", "docker") {
    ...
    def tableName = "test_file_cache_write_index_file_only"
    def loadRows = 1200
    ...
    def waitForLatestAlterOnTableFinish = { String table ->
        for (int i = 0; i < 300; i++) {
            def alterResult = sql """
                SHOW ALTER TABLE COLUMN WHERE TableName = "${table}" ORDER BY CreateTime DESC LIMIT 1
            """
            logger.info("latest alter table column result: ${alterResult}")
            if (alterResult.size() > 0) {
                def alterResultString = alterResult.toString()
                assert !alterResultString.contains("CANCELLED") :
                        "schema change was cancelled, result=${alterResultString}"
                if (alterResultString.contains("FINISHED")) {
                    sleep(3000)
                    return
                }
            }
        ...
        assert false : "wait alter table column timeout, table=${table}"
    }
    ...
    docker(options) {
        ...
        def loadProfileTag = "file_cache_write_index_only_load_profile"
        ...
        profile(loadProfileTag) {
            ...
            run {
                logRowsetsLayout("before load query")
            ...
            check { profileString, exception ->
                ...
                assertIndexOnlyProfile(profileString, "load", true, true)
            }
        }
        assert loadProfileChecked : "load profile check was not executed"
        ...
        def compactionProfileTag = "file_cache_write_index_only_compaction_profile"
        ...
        profile(compactionProfileTag) {
            ...
            sql """ SET inverted_index_max_expansions = 4096 """
            run {
                logRowsetsLayout("before full compaction query")
                sql """
            ...
            check { profileString, exception ->
                ...
                logger.info("profile snippet: {}", profileString.take(3000))
                assertIndexOnlyProfile(profileString, "full compaction", false, true)
            }
        }
        assert compactionProfileChecked : "full compaction profile check was not executed"

        clearFileCache(beHost, beHttpPort)
        sql """ ALTER TABLE ${tableName} ADD COLUMN sc_key BIGINT KEY DEFAULT "0" AFTER id """
        waitForLatestAlterOnTableFinish(tableName)
        sql """ SYNC """

        def schemaChangeProfileTag = "file_cache_write_index_only_schema_change_profile"
        def schemaChangeProfileChecked = false
        profile(schemaChangeProfileTag) {
            sql """ SET enable_profile = true """
            sql """ SET profile_level = 2 """
            sql """ SET parallel_pipeline_task_num = 1 """
            sql """ SET inverted_index_max_expansions = 4096 """
            run {
                logRowsetsLayout("before heavy schema change query")
                sql """
                    /* ${schemaChangeProfileTag} */
                    SELECT COUNT(*), SUM(id), SUM(sort_key), SUM(LENGTH(payload))
                    FROM ${tableName}
                    WHERE body MATCH_REGEXP '^${indexTokenPrefix}.*'
                """
            ...
            check { profileString, exception ->
                schemaChangeProfileChecked = true
                if (exception != null) {
                    throw exception
                }
                logger.info("profile snippet: {}", profileString.take(3000))
                assertIndexOnlyProfile(profileString, "heavy schema change", false, true)
            }
        }
        assert schemaChangeProfileChecked : "heavy schema change profile check was not executed"
    }
}
