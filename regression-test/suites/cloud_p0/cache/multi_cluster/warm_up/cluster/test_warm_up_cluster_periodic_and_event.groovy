...
suite('test_warm_up_cluster_periodic_and_event', 'docker') {
    ...
    docker(options) {
        ...
        logger.info("Periodic warm-up job ID: ${jobId1}")
        ...
        logger.info("Event driven warm-up job ID: ${jobId2}")
        ...
        def jobInfo = sql """SHOW WARM UP JOB WHERE ID = ${jobId1}"""
        assertEquals(jobInfo[0][0], jobId1)
        ...
        assertEquals(jobInfo2[0][0], jobId2)
        assertEquals(jobInfo2[0][1], clusterName1)
        assertEquals(jobInfo2[0][2], clusterName2)
        assertEquals(jobInfo2[0][4], "CLUSTER")
        assertTrue(jobInfo2[0][3] in ["RUNNING", "PENDING"])
        assertEquals(jobInfo2[0][5], "EVENT_DRIVEN (LOAD)")

        // Cancel job and confirm
        sql """CANCEL WARM UP JOB WHERE ID = ${jobId1}"""
        def cancelInfo1 = sql """SHOW WARM UP JOB WHERE ID = ${jobId1}"""
        assertEquals(cancelInfo1[0][3], "CANCELLED")

        // Cancel job and confirm
        sql """CANCEL WARM UP JOB WHERE ID = ${jobId2}"""
        def cancelInfo2 = sql """SHOW WARM UP JOB WHERE ID = ${jobId2}"""
        assertEquals(cancelInfo2[0][3], "CANCELLED")
