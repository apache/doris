...
suite('test_warm_up_cluster_event_cancel_active', 'docker') {
    ...
    docker(options) {
        ...
        sleep(15000)
        def cacheSize1 = getClusterTTLCacheSizeSum(clusterName1);
        def cacheSize2 = getClusterTTLCacheSizeSum(clusterName2);
        assertTrue(cacheSize1 > cacheSize0, "cache size in src cluster should increase")
        assertEquals(cacheSize0, cacheSize2, "no more syncs after job cancel is expected")
