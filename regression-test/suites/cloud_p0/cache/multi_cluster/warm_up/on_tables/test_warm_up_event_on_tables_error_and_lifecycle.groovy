...
suite('test_warm_up_event_on_tables_error_and_lifecycle', 'docker') {
    ...
    docker(options) {
        ...
        try {
            ...
            // ===== Lifecycle Test 1: Cluster-level and table-level jobs coexist =====
            logger.info("===== Lifecycle Test 1: Coexistence =====")
            ...
            // Create table-level event-driven job (should succeed alongside cluster-level)
            def tableJobId_ = sql """
                WARM UP CLUSTER ${clusterName2} WITH CLUSTER ${clusterName1}
                ON TABLES (
            ...
            // Cluster-level job should have empty TableFilter and MatchedTables
