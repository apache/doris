...
"""
        ...
        def checkNodesAlive = { String certDir ->
            ...
            frontends.each { fe ->
                ...
                def httpCode = runCommand(curlCmd, "Failed to check FE ${fe.host}:${fe.httpPort}").trim()
            ...
            backends.each { be ->
                ...
                def curlCmd = "curl --noproxy '*' --cacert ${certDir}/ca.crt --cert ${certDir}/certificate.crt --key ${certDir}/certificate.key -s -o /dev/null -w '%{http_code}' --connect-timeout 5 ${url}"
                def httpCode = runCommand(curlCmd, "Failed to check BE ${be.host}:${be.httpPort}").trim()
            ...
            metaservices.each { ms ->
                ...
                def curlCmd = "curl --noproxy '*' --cacert ${certDir}/ca.crt --cert ${certDir}/certificate.crt --key ${certDir}/certificate.key -s -o /dev/null -w '%{http_code}' --connect-timeout 5 ${url}"
                def httpCode = runCommand(curlCmd, "Failed to check MS ${ms.host}:${ms.httpPort}").trim()
            ...
            recyclers.each { rc ->
                ...
                def curlCmd = "curl --noproxy '*' --cacert ${certDir}/ca.crt --cert ${certDir}/certificate.crt --key ${certDir}/certificate.key -s -o /dev/null -w '%{http_code}' --connect-timeout 5 ${url}"
                def httpCode = runCommand(curlCmd, "Failed to check Recycler ${rc.host}:${rc.httpPort}").trim()
