...
suite("test_cloud_show_inverted_index_storage_format", "p0, docker") {
    ...
    docker(options) {
        ...
        sql "DROP TABLE IF EXISTS test_cloud_show_inverted_format_snii"
        sql """
            CREATE TABLE test_cloud_show_inverted_format_snii (
                k INT,
                v VARCHAR(20),
                INDEX idx_v (v) USING INVERTED
            )
        ...
        """
        sql """ALTER TABLE test_cloud_show_inverted_format_snii
                SET ("partition.inverted_index_storage_format" = "SNII")"""
        sql """ALTER TABLE test_cloud_show_inverted_format_snii
                ADD PARTITION p_snii VALUES [("10"), ("20"))"""
        def sniiPartitions = sql_return_maparray("SHOW PARTITIONS FROM test_cloud_show_inverted_format_snii")
        assertEquals("V2", sniiPartitions.find { it.PartitionName == "p_v2" }.InvertedIndexStorageFormat)
        assertEquals("SNII", sniiPartitions.find { it.PartitionName == "p_snii" }.InvertedIndexStorageFormat)
