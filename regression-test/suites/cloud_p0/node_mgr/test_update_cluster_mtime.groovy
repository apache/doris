...
suite('test_update_cluster_mtime', 'docker') {
    ...
    docker(options) {
        ...
        assertNotNull(normalBefore.cluster_status_mtime,
                'A newly created compute cluster must initialize cluster_status_mtime')
        assertNotNull(suspended.cluster_status_mtime,
                'Suspending a cluster must persist cluster_status_mtime')
        assertNotNull(normalBefore.mtime, 'A newly created compute cluster must initialize mtime')
        assertNotNull(suspended.mtime, 'Suspending a cluster must persist mtime')
        assertTrue(suspended.cluster_status_mtime > normalBefore.cluster_status_mtime,
                "Cluster status mtime did not advance: before=${normalBefore.cluster_status_mtime}, "
                        + "after=${suspended.cluster_status_mtime}")
        ...
        assertNotNull(normalAfter.cluster_status_mtime,
                'Resuming a cluster must persist cluster_status_mtime')
        assertNotNull(normalAfter.mtime, 'Resuming a cluster must persist mtime')
        assertTrue(normalAfter.cluster_status_mtime > suspended.cluster_status_mtime,
                "Cluster status mtime did not advance on recovery: "
                        + "suspended=${suspended.cluster_status_mtime}, "
                        + "normal=${normalAfter.cluster_status_mtime}")
        assertTrue(normalAfter.mtime > suspended.mtime,
                "Cluster mtime did not advance on recovery: suspended=${suspended.mtime}, "
                        + "normal=${normalAfter.mtime}")
    }
}
