...
suite("test_cumulative_compaction_status") {
    def tableName = "test_cumulative_compaction_status"
    ...
    try {
        ...
        for (def tablet in tablets) {
            ...
            logger.info("Trigger cumulative compaction: code=" + code + ", out=" + out + ", err=" + err)
            assertEquals(code, 0)
            ...
            assert checkCount < 10, "Cumulative compaction didn't complete within expected time"
            ...
            assertTrue(out.contains("last cumulative status\": \"[OK]\""))
            assertFalse(out.contains("last cumulative success time\": \"1970-01-01"))
            assertFalse(out.contains("last cumulative schedule time\": \"1970-01-01"))
        }
