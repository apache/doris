...
suite("test_filecache_with_base_compaction_thresthold", "docker") {
    ...
    options.beConfigs.add('file_cache_each_block_size=1024')
    options.beConfigs.add('file_cache_path=[{"path":"/opt/apache-doris/be/storage/file_cache","total_size":9216,"query_limit":9216}]')
    ...
    docker(options) {
        ...
        for (int i = 2; i < 20000; i++) {
            valueStr = valueStr + """ , (${i}, ${i}, "${randomStr}", ${i}) """
        }
        sql """ insert into ${testTable}(siteid, citycode, username, pv) values (1, 1, "xxx", 1) ${valueStr}; """
