...
suite("test_base_compaction_status") {
    def tableName = "test_base_compaction_status"
    ...
    try {
        ...
        for (def tablet in tablets) {
            ...
            def (code, out, err) = be_run_base_compaction(
                backendId_to_backendIP.get(backend_id), 
                backendId_to_backendHttpPort.get(backend_id), 
                tablet_id
            )
            logger.info("Trigger base compaction: code=" + code + ", out=" + out + ", err=" + err)
            assertEquals(code, 0)
            ...
            assert checkCount < 10, "Base compaction didn't complete within expected time"
            ...
            assertTrue(out.contains("last base status\": \"[OK]\""))
            assertFalse(out.contains("last base success time\": \"1970-01-01"))
            assertFalse(out.contains("last base schedule time\": \"1970-01-01"))
        }
