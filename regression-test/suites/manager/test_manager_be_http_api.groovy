...
suite("test_manager_be_http_api") {
    ...
    def get_be_health_status = {
        for (String id in backendIdToBackendIP.keySet()) {
            ...
            def (code, out, err) = curl("GET", String.format("http://%s:%s/api/health", beIp, bePort))
    ...
    def show_be_config = {
        for (String id in backendIdToBackendIP.keySet()) {
            def beIp = backendIdToBackendIP.get(id)
            def bePort = backendIdToBackendHttpPort.get(id)
            def (code, out, err) = curl("GET", String.format("http://%s:%s/api/show_config", beIp, bePort))
            assertFalse(out.isBlank())
        }
    }

    get_be_health_status()
    show_be_config()
}