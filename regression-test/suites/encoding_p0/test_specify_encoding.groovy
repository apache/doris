suite("test_specify_encoding")  {
    ...
    def checkScalar = { java.util.Map column, String encoding->
        ...
        assertEquals(realEncoding, exceptedEncoding)
    }

    def checkColumn = {Map column ->
        if ("ARRAY".equalsIgnoreCase(column.type)) {
            checkScalar(column.children_columns[0], "BIT_SHUFFLE")
        } else if ("MAP".equalsIgnoreCase(column.type)) {
            checkScalar(column.children_columns[0], "PLAIN_ENCODING")
            checkScalar(column.children_columns[1], "BIT_SHUFFLE")
    ...
    def assertEncoding = { String tableName ->
    ...
    sql """
        CREATE TABLE IF NOT EXISTS ${tableName} (
            ...
            `boolean_rle` variant encoding "rle" NULL,
            `boolean_bs` variant encoding "bit_shuffle" NULL,
            `boolean_plain` variant encoding "plain" NULL,
            ...
            `map_test` map<int encoding "plain", int encoding "bit_shuffle"> NULL,
            `struct_test` struct<`a_plain`: int encoding "plain", `b_bs`: int encoding "bit_shuffle"> NULL
       ) DUPLICATE KEY(`tiny_bs`, `tiny_for`, `tiny_plain`)
         DISTRIBUTED BY RANDOM BUCKETS 1
         PROPERTIES (
           "replication_allocation" = "tag.location.default: 1"
       )
    """
    assertEncoding(tableName)
   ...
   """
    ...
    """
    assertEncoding(defaultTableName)
}