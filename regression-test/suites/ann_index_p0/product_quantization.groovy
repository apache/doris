...
suite("product_quantization") {
    ...
    sql """create table product_quantization (
            ...
            distributed by hash(id) buckets 1
            properties('replication_num' = '1');"""
    // PQ training needs 2^pq_nbits * 100 points; with fewer rows the ANN index
    // build is skipped for the segment and the load succeeds instead of failing.
    sql """insert into product_quantization values (1, [1.0, 2.0, 3.0, 4.0])"""
    sql """select * from product_quantization order by id"""
