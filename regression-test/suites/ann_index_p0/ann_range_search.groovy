...
suite("ann_range_search") {
    sql "drop table if exists ann_range_search_l2"
    sql "drop table if exists ann_range_search_ip"

    sql """
        create table ann_range_search_l2 (
            id int not null,
    ...
    sql """
        create table ann_range_search_ip (
            id int not null,
            embedding array<float> not null,
            comment String not null,
            value int null,
            INDEX idx_comment(`comment`) USING INVERTED PROPERTIES("parser" = "english") COMMENT 'inverted index for comment',
            INDEX ann_embedding(`embedding`) USING ANN PROPERTIES("index_type"="hnsw","metric_type"="inner_product","dim"="8")
        ) duplicate key (`id`) 
        distributed by hash(`id`) buckets 1
        properties("replication_num"="1");
    """
    ...
    sql """
        INSERT INTO ann_range_search_l2 (id, embedding, comment, value) VALUES
    ...
    sql """
        INSERT INTO ann_range_search_ip (id, embedding, comment, value) VALUES
            (0, [39.906116, 10.495334, 54.08394, 88.67262, 55.243687, 10.162686, 36.335983, 38.684258], "This example illustrates how subtle differences can influence perception. It's more about interpretation than right or wrong.", 100),
            (1, [62.759315, 97.15586, 25.832521, 39.604908, 88.76715, 72.64085, 9.688437, 17.721428], "Thanks for all the comments, good and bad. They help us refine our test. Keep in mind that we're attempting to figure you out in 40 pairs of pictures. We did this so that lots of people could take it, just to introduce the idea.<p>A <i>real</i> test would have more like 200 pairs, which is what the YC founders took when we assessed their attributes in the first place.", 101),
            (2, [15.447449, 59.7771, 65.54516, 12.973712, 99.685135, 72.080734, 85.71118, 99.35976], "At a glance, these might seem obvious, but there’s nuance in every choice. Don’t rush.", 102),
            (3, [72.26747, 46.42257, 32.368374, 80.50209, 5.777631, 98.803314, 7.0915947, 68.62693], "We're testing how consistent your judgments are over a range of visual impressions. There's no single 'correct' answer.", 103),
    ...
    """
    qt_q0 """
        select id, comment from ann_range_search_l2 ORDER BY id;
    """

    qt_q1 """
        select id, comment from ann_range_search_l2 where l2_distance_approximate(embedding, [26.360261917114258,7.05784273147583,32.361351013183594,86.39714050292969,58.79527282714844,27.189321517944336,99.38946533203125,80.19270324707031]) < 105.66439056396484 ORDER BY id;
    """
    ...
    qt_q3 """
        select id, comment from ann_range_search_ip where inner_product_approximate(embedding, [26.360261917114258,7.05784273147583,32.361351013183594,86.39714050292969,58.79527282714844,27.189321517944336,99.38946533203125,80.19270324707031]) < 10000 ORDER BY id;
    """

    qt_q4 """
        select id, comment from ann_range_search_ip where inner_product_approximate(embedding, [26.360261917114258,7.05784273147583,32.361351013183594,86.39714050292969,58.79527282714844,27.189321517944336,99.38946533203125,80.19270324707031]) > 105.66439056396484 ORDER BY id;
    """   
}