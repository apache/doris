...
suite("create_tbl_with_ann_index_test") {
    // Test cloud mode - should fail when in cloud mode
    if (isCloudMode()) {
        // Test CREATE TABLE with ANN index in cloud mode
        sql "drop table if exists ann_tbl_cloud"
        test {
            sql """
                CREATE TABLE ann_tbl_cloud (
                    id INT NOT NULL COMMENT "",
                    vec ARRAY<FLOAT> NOT NULL COMMENT "",
                    INDEX ann_idx_cloud (vec) USING ANN PROPERTIES(
                        "index_type" = "hnsw",
        ...
        // Test ALTER TABLE ADD INDEX with ANN index in cloud mode
        sql "drop table if exists ann_tbl_alter_cloud"
        sql """
            CREATE TABLE ann_tbl_alter_cloud (
                id INT NOT NULL COMMENT "",
                vec ARRAY<FLOAT> NOT NULL COMMENT ""
            ) ENGINE=OLAP
            DUPLICATE KEY(id) COMMENT "OLAP"
        ...
        test {
            sql """
                ALTER TABLE ann_tbl_alter_cloud ADD INDEX ann_idx_alter_cloud (vec) USING ANN PROPERTIES(
                    "index_type" = "hnsw",
                    "metric_type" = "l2_distance",
                    "dim" = "128"
                );
