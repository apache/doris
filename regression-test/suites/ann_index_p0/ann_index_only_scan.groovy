...
suite("ann_index_only_scan") {
	/// Fobid topn-lazy materialization, since it will break index-only scan
	sql "set experimental_topn_lazy_materialization_threshold=0;"
	sql "set enable_common_expr_pushdown=true;"
	sql "drop table if exists ann_only_scan"

	sql """
        create table ann_only_scan (
            id int not null,
    ...
    sql """
        INSERT INTO ann_only_scan (id, embedding, comment, value) VALUES
	...
	"""
		select id,
			   ...
			   l2_distance_approximate(
			   		embedding, [26.360261917114258,7.05784273147583,32.361351013183594,86.39714050292969,58.79527282714844,27.189321517944336,99.38946533203125,80.19270324707031]) as dist 
		from ann_only_scan 
