...
suite("spm_tpch_sf01_q21", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q21 query
    // (SQL unchanged from sql/q21.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
LIMIT 100""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("supplier"),
                "q21 plan_sql should reference supplier: ${own[0][4]}")
  ...
  s_name
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q21 query")
  ...
  s_name
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q21 query")
  ...
  s_name
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q21 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q21 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
