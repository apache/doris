...
suite("spm_tpch_sf01_q09", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q09 query
    // (SQL unchanged from sql/q09.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
ORDER BY
  ...
  o_year DESC""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("part"),
                "q09 plan_sql should reference part: ${own[0][4]}")
