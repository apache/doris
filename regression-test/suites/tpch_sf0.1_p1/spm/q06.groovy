...
suite("spm_tpch_sf01_q06", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q06 query
    // (SQL unchanged from sql/q06.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
AND l_quantity < 24"
        WITH "SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1994-01-01'
...
AND l_quantity < 24""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("lineitem"),
                "q06 plan_sql should reference lineitem: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1994-01-01'
...
AND l_discount BETWEEN 0.06 - 0.01 AND .06 + 0.01
    AND l_quantity < 24"""
    order_qt_orig_with_spm """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1994-01-01'
...
AND l_discount BETWEEN 0.06 - 0.01 AND .06 + 0.01
    AND l_quantity < 24"""
    order_qt_orig_without_spm """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1994-01-01'
...
AND l_discount BETWEEN 0.06 - 0.01 AND .06 + 0.01
    AND l_quantity < 24"""
        ...
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
...
AND l_discount BETWEEN 0.07 - 0.01 AND .07 + 0.01
    AND l_quantity < 30"""
    order_qt_similar_with_spm """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1995-01-01'
...
AND l_discount BETWEEN 0.07 - 0.01 AND .07 + 0.01
    AND l_quantity < 30"""
    order_qt_similar_without_spm """SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1995-01-01'
...
AND l_discount BETWEEN 0.07 - 0.01 AND .07 + 0.01
    AND l_quantity < 30"""
        ...
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1994-01-01'
...
AND l_discount BETWEEN 0.06 - 0.01 AND .06 + 0.01
    AND l_quantity < 24"""
    def explainSimilar = sql """EXPLAIN SELECT sum(l_extendedprice * l_discount) AS revenue
FROM
  lineitem
WHERE
  l_shipdate >= DATE '1995-01-01'
...
AND l_discount BETWEEN 0.07 - 0.01 AND .07 + 0.01
    AND l_quantity < 30"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q06 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q06 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
