...
suite("spm_tpch_sf01_q14", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q14 query
    // (SQL unchanged from sql/q14.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
  ...
  AND l_shipdate < DATE '1995-09-01' + INTERVAL '1' MONTH""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("lineitem"),
                "q14 plan_sql should reference lineitem: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1995-09-01'
      AND l_shipdate < DATE '1995-09-01' + INTERVAL '1' MONTH"""
    order_qt_orig_with_spm """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1995-09-01'
      AND l_shipdate < DATE '1995-09-01' + INTERVAL '1' MONTH"""
    order_qt_orig_without_spm """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1995-09-01'
      AND l_shipdate < DATE '1995-09-01' + INTERVAL '1' MONTH"""
        ...
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1996-09-01'
    ...
    order_qt_similar_with_spm """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1996-09-01'
      AND l_shipdate < DATE '1996-09-01' + INTERVAL '1' MONTH"""
    order_qt_similar_without_spm """SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1996-09-01'
      AND l_shipdate < DATE '1996-09-01' + INTERVAL '1' MONTH"""
        ...
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
...
WHERE
  ...
  AND l_shipdate >= DATE '1995-09-01'
      AND l_shipdate < DATE '1995-09-01' + INTERVAL '1' MONTH"""
    def explainSimilar = sql """EXPLAIN SELECT 100.00 * sum(CASE
                    WHEN p_type LIKE 'PROMO%'
                      THEN l_extendedprice * (1 - l_discount)
                    ELSE 0
                    END) / sum(l_extendedprice * (1 - l_discount)) AS promo_revenue
  ...
  AND l_shipdate >= DATE '1996-09-01'
      AND l_shipdate < DATE '1996-09-01' + INTERVAL '1' MONTH"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q14 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q14 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
