...
suite("spm_tpch_sf01_q19", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q19 query
    // (SQL unchanged from sql/q19.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
WHERE
  ...
  (
    ...
    AND l_shipinstruct = 'DELIVER IN PERSON'
        ...
        WITH "SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("lineitem"),
                "q19 plan_sql should reference lineitem: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    order_qt_orig_with_spm """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
...
WHERE
  ...
  (
    ...
    order_qt_orig_without_spm """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    AND l_shipinstruct = 'DELIVER IN PERSON'
        ...
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  (
    ...
    AND p_container IN ('SM BAG', 'SM BOX', 'SM PACK', 'SM PKG')
    AND l_quantity >= 2 AND l_quantity <= 2 + 10
    AND p_size BETWEEN 2 AND 6
  ...
  (
    p_partkey = l_partkey
    AND p_brand = 'Brand#26'
    AND p_container IN ('MED JAR', 'MED BOX', 'MED PKG', 'MED PACK')
    AND l_quantity >= 11 AND l_quantity <= 11 + 11
    AND p_size BETWEEN 2 AND 11
    AND l_shipmode IN ('AIR', 'TRUCK')
    AND l_shipinstruct = 'COLLECT COD'
  )
  OR
  (
    p_partkey = l_partkey
    AND p_brand = 'Brand#37'
    AND p_container IN ('LG CAN', 'LG BOX', 'LG PACK', 'LG PKG')
    AND l_quantity >= 21 AND l_quantity <= 21 + 10
    AND p_size BETWEEN 2 AND 16
    AND l_shipmode IN ('AIR', 'TRUCK')
    AND l_shipinstruct = 'COLLECT COD'
      )"""
    order_qt_similar_with_spm """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
...
WHERE
  ...
  (
    ...
    order_qt_similar_without_spm """SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    AND l_shipinstruct = 'COLLECT COD'
        ...
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    def explainSimilar = sql """EXPLAIN SELECT sum(l_extendedprice * (1 - l_discount)) AS revenue
FROM
  lineitem,
  part
WHERE
  ...
  (
    ...
    AND l_shipinstruct = 'COLLECT COD'
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q19 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q19 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
