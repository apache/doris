...
suite("spm_tpch_sf01_q20", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q20 query
    // (SQL unchanged from sql/q20.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
ORDER BY s_name""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("supplier"),
                "q20 plan_sql should reference supplier: ${own[0][4]}")
