...
suite("spm_tpch_sf01_q17", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q17 query
    // (SQL unchanged from sql/q17.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
  ...
  AND l_quantity < (
    ...
    WHERE
      l_partkey = p_partkey
        ...
        WITH "SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("lineitem"),
                "q17 plan_sql should reference lineitem: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    ...
    order_qt_orig_with_spm """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
...
WHERE
  ...
  AND l_quantity < (
    ...
    order_qt_orig_without_spm """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    ...
    WHERE
      l_partkey = p_partkey
        ...
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    SELECT 0.3 * avg(l_quantity)
    FROM
      lineitem
    WHERE
      l_partkey = p_partkey
      )"""
    order_qt_similar_with_spm """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
...
WHERE
  ...
  AND l_quantity < (
    ...
    order_qt_similar_without_spm """SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    ...
    WHERE
      l_partkey = p_partkey
        ...
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
  ...
  AND l_quantity < (
    ...
    def explainSimilar = sql """EXPLAIN SELECT sum(l_extendedprice) / 7.0 AS avg_yearly
FROM
  lineitem,
  part
WHERE
