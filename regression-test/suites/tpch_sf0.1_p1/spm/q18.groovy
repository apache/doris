...
suite("spm_tpch_sf01_q18", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q18 query
    // (SQL unchanged from sql/q18.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
LIMIT 100""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q18 plan_sql should reference customer: ${own[0][4]}")
  ...
  o_orderdate
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q18 query")
  ...
  o_orderdate
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q18 query")
  ...
  o_orderdate
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q18 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q18 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
