...
suite("spm_tpch_sf01_q08", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q08 query
    // (SQL unchanged from sql/q08.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
  ...
  o_year""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("part"),
                "q08 plan_sql should reference part: ${own[0][4]}")
