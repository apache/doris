...
suite("spm_tpch_sf01_q01", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q01 query
    // (SQL unchanged from sql/q01.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
l_linestatus""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("lineitem"),
                "q01 plan_sql should reference lineitem: ${own[0][4]}")
...
FROM
  lineitem
WHERE
  l_shipdate <= DATE '1998-06-01' - INTERVAL '90' DAY
GROUP BY
l_returnflag,
l_linestatus
ORDER BY
