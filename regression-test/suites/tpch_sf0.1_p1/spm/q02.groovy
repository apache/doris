...
suite("spm_tpch_sf01_q02", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q02 query
    // (SQL unchanged from sql/q02.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
LIMIT 100""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("part"),
                "q02 plan_sql should reference part: ${own[0][4]}")
  ...
  p_partkey
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q02 query")
  ...
  p_partkey
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q02 query")
  ...
  p_partkey
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q02 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q02 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
