...
suite("spm_tpch_sf01_q12", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q12 query
    // (SQL unchanged from sql/q12.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
  ...
  l_shipmode""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("orders"),
                "q12 plan_sql should reference orders: ${own[0][4]}")
...
ORDER BY
      l_shipmode"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q12 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q12 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
