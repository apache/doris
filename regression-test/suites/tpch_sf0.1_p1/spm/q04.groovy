...
suite("spm_tpch_sf01_q04", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q04 query
    // (SQL unchanged from sql/q04.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
o_orderpriority""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("orders"),
                "q04 plan_sql should reference orders: ${own[0][4]}")
...
ORDER BY
    o_orderpriority"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q04 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q04 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
