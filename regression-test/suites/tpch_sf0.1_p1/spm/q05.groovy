...
suite("spm_tpch_sf01_q05", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q05 query
    // (SQL unchanged from sql/q05.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
revenue DESC""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q05 plan_sql should reference customer: ${own[0][4]}")
