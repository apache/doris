...
suite("spm_tpch_sf01_q03", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q03 query
    // (SQL unchanged from sql/q03.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
...
LIMIT 10""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q03 plan_sql should reference customer: ${own[0][4]}")
  ...
  o_orderdate
    LIMIT 10"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q03 query")
...
WHERE
  ...
  AND c_custkey = o_custkey
  AND l_orderkey = o_orderkey
  AND o_orderdate < DATE '1996-03-15'
  AND l_shipdate > DATE '1996-03-15'
GROUP BY
  l_orderkey,
  o_orderdate,
  o_shippriority
  ...
  o_orderdate
    LIMIT 10"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q03 query")
  ...
  o_orderdate
    LIMIT 10"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q03 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q03 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
