...
suite("spm_tpch_sf01_q16", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCH q16 query
    // (SQL unchanged from sql/q16.sql) against the REAL TPCH sf0.1 data loaded
    // by load.groovy.
  ...
  p_size""""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("partsupp"),
                "q16 plan_sql should reference partsupp: ${own[0][4]}")
...
WHERE
  ...
  AND ps_suppkey NOT IN (
    ...
    FROM
      supplier
    WHERE
      s_comment LIKE '%Special%Requests%'
  )
GROUP BY
  p_brand,
  p_type,
  ...
  p_type,
      p_size"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q16 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q16 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
