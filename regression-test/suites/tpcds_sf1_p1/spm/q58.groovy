...
suite("spm_tpcds_sf1_q58", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q58 query
    // (SQL unchanged from sql/q58.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q58 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY ss_items.item_id ASC, ss_item_rev ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q58 query")
  ...
  ss_items AS (
   ...
   WHERE (ss_item_sk = i_item_sk)
      ...
      WHERE (d_week_seq = (
            ...
            FROM
              date_dim
            WHERE (d_date = CAST('1999-01-03' AS DATE))
         ))
   ))
      AND (ss_sold_date_sk = d_date_sk)
   GROUP BY i_item_id
...
WHERE (ss_items.item_id = cs_items.item_id)
   AND (ss_items.item_id = ws_items.item_id)
   AND (ss_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * cs_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * cs_item_rev))
   AND (ss_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * ws_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * ws_item_rev))
   AND (cs_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * ss_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * ss_item_rev))
   AND (cs_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * ws_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * ws_item_rev))
   AND (ws_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * ss_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * ss_item_rev))
   AND (ws_item_rev BETWEEN (CAST('0.8' AS DECIMAL(2,1)) * cs_item_rev) AND (CAST('1.2' AS DECIMAL(2,1)) * cs_item_rev))
ORDER BY ss_items.item_id ASC, ss_item_rev ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY ss_items.item_id ASC, ss_item_rev ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q58 query")
...
ORDER BY ss_items.item_id ASC, ss_item_rev ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q58 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q58 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
