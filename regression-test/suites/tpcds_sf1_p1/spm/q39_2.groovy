...
suite("spm_tpcds_sf1_q39_2", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q39_2 query
    // (SQL unchanged from sql/q39_2.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("inventory"),
                "q39_2 plan_sql should reference inventory: ${own[0][4]}")
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        assertEquals(normRows(origWithSpm), normRows(origWithoutSpm),
                "SPM rewrite must preserve the result of the q39_2 query")
...
WHERE (inv1.i_item_sk = inv2.i_item_sk)
   ...
   AND (inv1.d_moy = 2)
   AND (inv2.d_moy = (2 + 1))
   AND (inv1.cov > CAST('1.2' AS DECIMAL(2,1)))
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        assertEquals(normRows(similarWithSpm), normRows(similarWithoutSpm),
                "SPM rewrite must preserve the result of a similar q39_2 query")
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q39_2 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q39_2 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
