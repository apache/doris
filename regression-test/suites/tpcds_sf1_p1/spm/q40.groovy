...
suite("spm_tpcds_sf1_q40", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q40 query
    // (SQL unchanged from sql/q40.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q40 plan_sql should reference catalog_sales: ${own[0][4]}")
...
ORDER BY w_state ASC, i_item_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q40 query")
  ...
  w_state
, i_item_id
, sum((CASE WHEN (CAST(d_date AS DATE) < CAST('2001-03-11' AS DATE)) THEN (cs_sales_price - COALESCE(cr_refunded_cash, 0)) ELSE 0 END)) sales_before
, sum((CASE WHEN (CAST(d_date AS DATE) >= CAST('2001-03-11' AS DATE)) THEN (cs_sales_price - COALESCE(cr_refunded_cash, 0)) ELSE 0 END)) sales_after
FROM
  catalog_sales
LEFT JOIN catalog_returns ON (cs_order_number = cr_order_number)
   AND (cs_item_sk = cr_item_sk)
...
, item
, date_dim
WHERE (i_current_price BETWEEN CAST('1.50' AS DECIMAL(3,2)) AND CAST('2.49' AS DECIMAL(3,2)))
   AND (i_item_sk = cs_item_sk)
   AND (cs_warehouse_sk = w_warehouse_sk)
   AND (cs_sold_date_sk = d_date_sk)
   AND (CAST(d_date AS DATE) BETWEEN (CAST('2001-03-11' AS DATE) - INTERVAL  '30' DAY) AND (CAST('2001-03-11' AS DATE) + INTERVAL  '30' DAY))
GROUP BY w_state, i_item_id
ORDER BY w_state ASC, i_item_id ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY w_state ASC, i_item_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q40 query")
...
ORDER BY w_state ASC, i_item_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q40 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q40 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
