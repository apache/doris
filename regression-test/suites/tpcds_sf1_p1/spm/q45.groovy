...
suite("spm_tpcds_sf1_q45", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q45 query
    // (SQL unchanged from sql/q45.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q45 plan_sql should reference web_sales: ${own[0][4]}")
...
ORDER BY ca_zip ASC, ca_city ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q45 query")
...
WHERE (ws_bill_customer_sk = c_customer_sk)
   ...
   AND ((substr(ca_zip, 1, 5) IN ('85669~'   , '86197'   , '88274'   , '83405'   , '86475'   , '85392'   , '85460'   , '80348'   , '81792'))
      ...
      FROM
        item
      WHERE (i_item_sk IN (37     , 1081   , 2149   , 2965   , 3091   , 4861   , 6907   , 8629   , 10657  , 15091))
   )))
   AND (ws_sold_date_sk = d_date_sk)
   AND (d_qoy = 3)
   AND (d_year = 2000)
...
ORDER BY ca_zip ASC, ca_city ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q45 query")
...
ORDER BY ca_zip ASC, ca_city ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q45 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q45 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
