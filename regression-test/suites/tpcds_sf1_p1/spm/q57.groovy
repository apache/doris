...
suite("spm_tpcds_sf1_q57", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q57 query
    // (SQL unchanged from sql/q57.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q57 plan_sql should reference item: ${own[0][4]}")
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, 3 ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q57 query")
...
FROM
  v2
WHERE (d_year = 2000)
   AND (avg_monthly_sales > -1)
   AND ((CASE WHEN (avg_monthly_sales > -1) THEN (abs((sum_sales - avg_monthly_sales)) / avg_monthly_sales) ELSE null END) > CAST('0.0' AS DECIMAL(2,1)))
ORDER BY (sum_sales - avg_monthly_sales) ASC, 3 ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, 3 ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q57 query")
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, 3 ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q57 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q57 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
