...
suite("spm_tpcds_sf1_q51", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q51 query
    // (SQL unchanged from sql/q51.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q51 plan_sql should reference web_sales: ${own[0][4]}")
...
ORDER BY item_sk ASC, d_date ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q51 query")
...
ORDER BY item_sk ASC, d_date ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q51 query")
...
ORDER BY item_sk ASC, d_date ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q51 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q51 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
