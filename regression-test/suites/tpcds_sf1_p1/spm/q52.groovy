...
suite("spm_tpcds_sf1_q52", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q52 query
    // (SQL unchanged from sql/q52.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("date_dim"),
                "q52 plan_sql should reference date_dim: ${own[0][4]}")
...
ORDER BY dt.d_year ASC, ext_price DESC, brand_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q52 query")
...
ORDER BY dt.d_year ASC, ext_price DESC, brand_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q52 query")
...
ORDER BY dt.d_year ASC, ext_price DESC, brand_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q52 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q52 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
