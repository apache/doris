...
suite("spm_tpcds_sf1_q71", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q71 query
    // (SQL unchanged from sql/q71.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY ext_price DESC, i_brand_id ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q71 plan_sql should reference item: ${own[0][4]}")
...
GROUP BY i_brand, i_brand_id, t_hour, t_minute
    ORDER BY ext_price DESC, i_brand_id ASC"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q71 query")
...
GROUP BY i_brand, i_brand_id, t_hour, t_minute
    ORDER BY ext_price DESC, i_brand_id ASC"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q71 query")
...
GROUP BY i_brand, i_brand_id, t_hour, t_minute
    ORDER BY ext_price DESC, i_brand_id ASC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q71 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q71 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
