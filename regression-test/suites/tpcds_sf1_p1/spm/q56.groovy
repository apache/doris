...
suite("spm_tpcds_sf1_q56", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q56 query
    // (SQL unchanged from sql/q56.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q56 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY total_sales ASC, i_item_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q56 query")
...
ORDER BY total_sales ASC, i_item_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q56 query")
...
ORDER BY total_sales ASC, i_item_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q56 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q56 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
