...
suite("spm_tpcds_sf1_q22", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q22 query
    // (SQL unchanged from sql/q22.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("inventory"),
                "q22 plan_sql should reference inventory: ${own[0][4]}")
