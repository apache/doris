...
suite("spm_tpcds_sf1_q62", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q62 query
    // (SQL unchanged from sql/q62.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q62 plan_sql should reference web_sales: ${own[0][4]}")
...
WHERE (d_month_seq BETWEEN 1201 AND (1201 + 11))
   ...
   AND (ws_ship_mode_sk = sm_ship_mode_sk)
   AND (ws_web_site_sk = web_site_sk)
GROUP BY substr(w_warehouse_name, 1, 21), sm_type, web_name
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, web_name ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, web_name ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q62 query")
...
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, web_name ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q62 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q62 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
