...
suite("spm_tpcds_sf1_q93", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q93 query
    // (SQL unchanged from sql/q93.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q93 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY sumsales ASC, ss_customer_sk ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q93 query")
...
ORDER BY sumsales ASC, ss_customer_sk ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q93 query")
...
ORDER BY sumsales ASC, ss_customer_sk ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q93 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q93 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
