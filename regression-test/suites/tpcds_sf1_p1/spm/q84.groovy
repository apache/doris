...
suite("spm_tpcds_sf1_q84", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q84 query
    // (SQL unchanged from sql/q84.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q84 plan_sql should reference customer: ${own[0][4]}")
...
ORDER BY c_customer_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q84 query")
...
WHERE (ca_city = 'Fairview')
   AND (c_current_addr_sk = ca_address_sk)
   AND (ib_lower_bound >= 28128)
   AND (ib_upper_bound <= (28128 + 50000))
   AND (ib_income_band_sk = hd_income_band_sk)
   AND (cd_demo_sk = c_current_cdemo_sk)
   AND (hd_demo_sk = c_current_hdemo_sk)
   AND (sr_cdemo_sk = cd_demo_sk)
...
ORDER BY c_customer_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q84 query")
...
ORDER BY c_customer_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q84 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q84 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
