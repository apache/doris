...
suite("spm_tpcds_sf1_q29", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q29 query
    // (SQL unchanged from sql/q29.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q29 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY i_item_id ASC, i_item_desc ASC, s_store_id ASC, s_store_name ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q29 query")
...
WHERE (d1.d_moy = 8)
   ...
   AND (ss_ticket_number = sr_ticket_number)
   AND (sr_returned_date_sk = d2.d_date_sk)
   AND (d2.d_moy BETWEEN 8 AND (8 + 3))
   AND (d2.d_year = 2000)
   AND (sr_customer_sk = cs_bill_customer_sk)
   AND (sr_item_sk = cs_item_sk)
   AND (cs_sold_date_sk = d3.d_date_sk)
...
ORDER BY i_item_id ASC, i_item_desc ASC, s_store_id ASC, s_store_name ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q29 query")
...
ORDER BY i_item_id ASC, i_item_desc ASC, s_store_id ASC, s_store_name ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q29 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q29 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
