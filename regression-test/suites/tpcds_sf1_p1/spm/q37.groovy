...
suite("spm_tpcds_sf1_q37", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q37 query
    // (SQL unchanged from sql/q37.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q37 plan_sql should reference item: ${own[0][4]}")
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q37 query")
...
WHERE (i_current_price BETWEEN 69 AND (69 + 30))
   AND (inv_item_sk = i_item_sk)
   AND (d_date_sk = inv_date_sk)
   AND (CAST(d_date AS DATE) BETWEEN CAST('2000-03-01' AS DATE) AND (CAST('2000-03-01' AS DATE) + INTERVAL  '60' DAY))
   AND (i_manufact_id IN (158, 207, 313, 116))
   AND (inv_quantity_on_hand BETWEEN 200 AND 600)
   AND (cs_item_sk = i_item_sk)
GROUP BY i_item_id, i_item_desc, i_current_price
ORDER BY i_item_id ASC
    LIMIT 100"""
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q37 query")
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q37 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q37 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
