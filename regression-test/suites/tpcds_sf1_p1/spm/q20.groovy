...
suite("spm_tpcds_sf1_q20", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q20 query
    // (SQL unchanged from sql/q20.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q20 plan_sql should reference catalog_sales: ${own[0][4]}")
...
, i_current_price
, sum(cs_ext_sales_price) itemrevenue
, ((sum(cs_ext_sales_price) * 101) / sum(sum(cs_ext_sales_price)) OVER (PARTITION BY i_class)) revenueratio
FROM
  catalog_sales
, item
, date_dim
...
ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC
    LIMIT 101"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q20 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q20 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
