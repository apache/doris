...
suite("spm_tpcds_sf1_q38", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q38 query
    // (SQL unchanged from sql/q38.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q38 plan_sql should reference store_sales: ${own[0][4]}")
   ...
   WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
    ...
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q38 query")
   ...
   WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
    ...
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q38 query")
   ...
   WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
    ...
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q38 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q38 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
