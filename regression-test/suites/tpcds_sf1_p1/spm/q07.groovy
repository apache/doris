...
suite("spm_tpcds_sf1_q07", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q07 query
    // (SQL unchanged from sql/q07.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q07 plan_sql should reference store_sales: ${own[0][4]}")
