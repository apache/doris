...
suite("spm_tpcds_sf1_q55", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q55 query
    // (SQL unchanged from sql/q55.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("date_dim"),
                "q55 plan_sql should reference date_dim: ${own[0][4]}")
...
ORDER BY ext_price DESC, i_brand_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q55 query")
...
ORDER BY ext_price DESC, i_brand_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q55 query")
...
ORDER BY ext_price DESC, i_brand_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q55 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q55 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
