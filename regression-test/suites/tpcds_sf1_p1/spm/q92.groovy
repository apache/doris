...
suite("spm_tpcds_sf1_q92", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q92 query
    // (SQL unchanged from sql/q92.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    ...
    def bindSql = """SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q92 plan_sql should reference web_sales: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
ORDER BY sum(ws_ext_discount_amt) ASC
    ...
    def origWithoutSpm = sql """SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
ORDER BY sum(ws_ext_discount_amt) ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q92 query")
    
        // ===== match verification: a similar query (same structure, different literals) =====
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
WHERE (i_manufact_id = 351)
   AND (i_item_sk = ws_item_sk)
   AND (d_date BETWEEN CAST('2000-02-27' AS DATE) AND (CAST('2000-02-27' AS DATE) + INTERVAL  '90' DAY))
   AND (d_date_sk = ws_sold_date_sk)
   AND (ws_ext_discount_amt > (
      SELECT (CAST('1.1' AS DECIMAL(2,1)) * avg(ws_ext_discount_amt))
      FROM
        web_sales
      , date_dim
      WHERE (ws_item_sk = i_item_sk)
...
ORDER BY sum(ws_ext_discount_amt) ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
    def similarWithoutSpm = sql """SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
ORDER BY sum(ws_ext_discount_amt) ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q92 query")
    
        // ===== EXPLAIN check: original and similar queries must actually hit the baseline =====
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
ORDER BY sum(ws_ext_discount_amt) ASC
    LIMIT 100"""
    def explainSimilar = sql """EXPLAIN SELECT sum(ws_ext_discount_amt) 'Excess Discount Amount'
FROM
  web_sales
, item
, date_dim
...
ORDER BY sum(ws_ext_discount_amt) ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q92 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q92 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
