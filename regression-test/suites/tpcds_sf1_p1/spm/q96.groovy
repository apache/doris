...
suite("spm_tpcds_sf1_q96", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q96 query
    // (SQL unchanged from sql/q96.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q96 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY count(*) ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q96 query")
...
WHERE (ss_sold_time_sk = time_dim.t_time_sk)
   AND (ss_hdemo_sk = household_demographics.hd_demo_sk)
   AND (ss_store_sk = s_store_sk)
   AND (time_dim.t_hour = 21)
   AND (time_dim.t_minute >= 31)
...
ORDER BY count(*) ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q96 query")
...
ORDER BY count(*) ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q96 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q96 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
