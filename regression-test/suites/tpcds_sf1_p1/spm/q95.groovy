...
suite("spm_tpcds_sf1_q95", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q95 query
    // (SQL unchanged from sql/q95.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q95 plan_sql should reference web_sales: ${own[0][4]}")
...
ORDER BY count(DISTINCT ws_order_number) ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q95 query")
...
, customer_address
, web_site
WHERE (CAST(d_date AS DATE) BETWEEN CAST('2000-2-01' AS DATE) AND (CAST('2000-2-01' AS DATE) + INTERVAL  '60' DAY))
   AND (ws1.ws_ship_date_sk = d_date_sk)
   AND (ws1.ws_ship_addr_sk = ca_address_sk)
   AND (ca_state = 'CA')
   AND (ws1.ws_web_site_sk = web_site_sk)
...
ORDER BY count(DISTINCT ws_order_number) ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q95 query")
...
ORDER BY count(DISTINCT ws_order_number) ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q95 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q95 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
