...
suite("spm_tpcds_sf1_q11", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q11 query
    // (SQL unchanged from sql/q11.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q11 plan_sql should reference customer: ${own[0][4]}")
