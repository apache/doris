...
suite("spm_tpcds_sf1_q13", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q13 query
    // (SQL unchanged from sql/q13.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
   ...
   AND (((ss_addr_sk = ca_address_sk)
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q13 plan_sql should reference store_sales: ${own[0][4]}")
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q13 query")
...
WHERE (s_store_sk = ss_store_sk)
   ...
   AND (((ss_addr_sk = ca_address_sk)
         ...
         AND (ss_net_profit BETWEEN 200 AND 300))
      OR ((ss_addr_sk = ca_address_sk)
         AND (ca_country = 'United States')
         AND (ca_state IN ('NV'      , 'AZ'      , 'MN'))
         AND (ss_net_profit BETWEEN 250 AND 350))
      OR ((ss_addr_sk = ca_address_sk)
         AND (ca_country = 'United States')
         AND (ca_state IN ('FL'      , 'MI'      , 'NY'))
             AND (ss_net_profit BETWEEN 100 AND 300)))"""
        sql 'set enable_spm_rewrite=false'
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q13 query")
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q13 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q13 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
