...
suite("spm_tpcds_sf1_q27", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q27 query
    // (SQL unchanged from sql/q27.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q27 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY i_item_id ASC, s_state ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q27 query")
...
ORDER BY i_item_id ASC, s_state ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q27 query")
...
ORDER BY i_item_id ASC, s_state ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q27 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q27 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
