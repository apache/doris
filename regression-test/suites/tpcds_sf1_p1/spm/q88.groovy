...
suite("spm_tpcds_sf1_q88", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q88 query
    // (SQL unchanged from sql/q88.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
   ...
   WHERE (ss_sold_time_sk = time_dim.t_time_sk)
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q88 plan_sql should reference store_sales: ${own[0][4]}")
   ...
   WHERE (ss_sold_time_sk = time_dim.t_time_sk)
      ...
      AND (store.s_store_name = 'ese')
        ...
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q88 query")
  ...
  (
   ...
   WHERE (ss_sold_time_sk = time_dim.t_time_sk)
      ...
      AND (((household_demographics.hd_dep_count = 5)
            AND (household_demographics.hd_vehicle_count <= (5 + 2)))
         OR ((household_demographics.hd_dep_count = 3)
            AND (household_demographics.hd_vehicle_count <= (3 + 3)))
   ...
   WHERE (ss_sold_time_sk = time_dim.t_time_sk)
      ...
      AND (store.s_store_name = 'ese')
        ...
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q88 query")
   ...
   WHERE (ss_sold_time_sk = time_dim.t_time_sk)
      ...
      AND (store.s_store_name = 'ese')
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q88 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q88 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
