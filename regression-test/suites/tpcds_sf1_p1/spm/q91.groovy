...
suite("spm_tpcds_sf1_q91", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q91 query
    // (SQL unchanged from sql/q91.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY sum(cr_net_loss) DESC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("call_center"),
                "q91 plan_sql should reference call_center: ${own[0][4]}")
...
GROUP BY cc_call_center_id, cc_name, cc_manager, cd_marital_status, cd_education_status
    ORDER BY sum(cr_net_loss) DESC"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q91 query")
...
GROUP BY cc_call_center_id, cc_name, cc_manager, cd_marital_status, cd_education_status
    ORDER BY sum(cr_net_loss) DESC"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q91 query")
...
GROUP BY cc_call_center_id, cc_name, cc_manager, cd_marital_status, cd_education_status
    ORDER BY sum(cr_net_loss) DESC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q91 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q91 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
