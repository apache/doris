...
suite("spm_tpcds_sf1_q32", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q32 query
    // (SQL unchanged from sql/q32.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    ...
    def bindSql = """SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q32 plan_sql should reference catalog_sales: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
WHERE (i_manufact_id = 977)
   ...
   AND (cs_ext_discount_amt > (
    ...
    def origWithoutSpm = sql """SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
   ...
   AND (cs_ext_discount_amt > (
    ...
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q32 query")
    
        // ===== match verification: a similar query (same structure, different literals) =====
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
WHERE (i_manufact_id = 978)
   AND (i_item_sk = cs_item_sk)
   AND (d_date BETWEEN CAST('2000-02-27' AS DATE) AND (CAST('2000-02-27' AS DATE) + INTERVAL  '90' DAY))
   AND (d_date_sk = cs_sold_date_sk)
   AND (cs_ext_discount_amt > (
      SELECT (CAST('1.1' AS DECIMAL(2,1)) * avg(cs_ext_discount_amt))
      FROM
        catalog_sales
      , date_dim
      WHERE (cs_item_sk = i_item_sk)
    ...
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
    def similarWithoutSpm = sql """SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
   ...
   AND (cs_ext_discount_amt > (
    ...
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q32 query")
    
        // ===== EXPLAIN check: original and similar queries must actually hit the baseline =====
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
WHERE (i_manufact_id = 977)
   ...
   AND (cs_ext_discount_amt > (
    ...
    LIMIT 100"""
    def explainSimilar = sql """EXPLAIN SELECT sum(cs_ext_discount_amt) 'excess discount amount'
FROM
  catalog_sales
, item
, date_dim
   ...
   AND (cs_ext_discount_amt > (
    ...
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q32 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q32 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
