...
suite("spm_tpcds_sf1_q59", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q59 query
    // (SQL unchanged from sql/q59.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q59 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY s_store_name1 ASC, s_store_id1 ASC, d_week_seq1 ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q59 query")
...
, (
   ...
   WHERE (d.d_week_seq = wss.d_week_seq)
      AND (ss_store_sk = s_store_sk)
      AND (d_month_seq BETWEEN (1213 + 12) AND (1213 + 23))
)  x
WHERE (s_store_id1 = s_store_id2)
   AND (d_week_seq1 = (d_week_seq2 - 52))
ORDER BY s_store_name1 ASC, s_store_id1 ASC, d_week_seq1 ASC
...
ORDER BY s_store_name1 ASC, s_store_id1 ASC, d_week_seq1 ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q59 query")
...
ORDER BY s_store_name1 ASC, s_store_id1 ASC, d_week_seq1 ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q59 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q59 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
