...
suite("spm_tpcds_sf1_q08", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q08 query
    // (SQL unchanged from sql/q08.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q08 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY s_store_name ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q08 query")
...
ORDER BY s_store_name ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q08 query")
...
ORDER BY s_store_name ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q08 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q08 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
