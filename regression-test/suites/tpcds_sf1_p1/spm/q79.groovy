...
suite("spm_tpcds_sf1_q79", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q79 query
    // (SQL unchanged from sql/q79.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q79 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY c_last_name ASC, c_first_name ASC, substr(s_city, 1, 30) ASC, profit ASC, ss_ticket_number, amt
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q79 query")
...
FROM
  (
   ...
   WHERE (store_sales.ss_sold_date_sk = date_dim.d_date_sk)
      ...
      AND (date_dim.d_dow = 2)
      AND (date_dim.d_year IN (2000   , (2000 + 1)   , (2000 + 2)))
      AND (store.s_number_employees BETWEEN 210 AND 280)
   GROUP BY ss_ticket_number, ss_customer_sk, ss_addr_sk, store.s_city
)  ms
, customer
WHERE (ss_customer_sk = c_customer_sk)
ORDER BY c_last_name ASC, c_first_name ASC, substr(s_city, 1, 31) ASC, profit ASC, ss_ticket_number, amt
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY c_last_name ASC, c_first_name ASC, substr(s_city, 1, 31) ASC, profit ASC, ss_ticket_number, amt
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q79 query")
...
ORDER BY c_last_name ASC, c_first_name ASC, substr(s_city, 1, 31) ASC, profit ASC, ss_ticket_number, amt
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q79 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q79 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
