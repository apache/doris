...
suite("spm_tpcds_sf1_q05", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q05 query
    // (SQL unchanged from sql/q05.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q05 plan_sql should reference store_sales: ${own[0][4]}")
...
UNION ALL       SELECT
   ...
   , store
   WHERE (date_sk = d_date_sk)
      AND (d_date BETWEEN CAST('2001-08-23' AS DATE) AND (CAST('2001-08-23' AS DATE) + INTERVAL  '14' DAY))
      AND (store_sk = s_store_sk)
   GROUP BY s_store_id
)
, csr AS (
...
ORDER BY channel ASC, id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q05 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q05 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
