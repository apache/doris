...
suite("spm_tpcds_sf1_q14_2", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q14_2 query
    // (SQL unchanged from sql/q14_2.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q14_2 plan_sql should reference store_sales: ${own[0][4]}")
...
LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q14_2 query")
...
LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q14_2 query")
...
LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q14_2 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q14_2 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
