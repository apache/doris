...
suite("spm_tpcds_sf1_q82", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q82 query
    // (SQL unchanged from sql/q82.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q82 plan_sql should reference item: ${own[0][4]}")
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q82 query")
...
WHERE (i_current_price BETWEEN 63 AND (63 + 30))
   AND (inv_item_sk = i_item_sk)
   AND (d_date_sk = inv_date_sk)
   AND (CAST(d_date AS DATE) BETWEEN CAST('2000-06-25' AS DATE) AND (CAST('2000-06-25' AS DATE) + INTERVAL  '60' DAY))
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q82 query")
...
ORDER BY i_item_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q82 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q82 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
