...
suite("spm_tpcds_sf1_q67", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q67 query
    // (SQL unchanged from sql/q67_ignore_temporarily.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    // Source SQL: sql/q67_ignore_temporarily.sql (renamed in #19227 for an old
    // result-precision issue of the legacy sql-dir comparison; the SPM check here
    // only compares SPM on/off results, so no precision workaround is involved).
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q67 plan_sql should reference store_sales: ${own[0][4]}")
...
LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q67 query")
...
LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q67 query")
...
LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q67 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q67 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
