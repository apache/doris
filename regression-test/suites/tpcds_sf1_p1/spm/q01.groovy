...
suite("spm_tpcds_sf1_q01", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q01 query
    // (SQL unchanged from sql/q01.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_returns"),
                "q01 plan_sql should reference store_returns: ${own[0][4]}")
...
, customer
WHERE (ctr1.ctr_total_return > (
      SELECT (avg(ctr_total_return) * CAST('1.0' AS DECIMAL(2,1)))
      FROM
        customer_total_return ctr2
      WHERE (ctr1.ctr_store_sk = ctr2.ctr_store_sk)
   ))
...
ORDER BY c_customer_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q01 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q01 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
