...
suite("spm_tpcds_sf1_q15", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q15 query
    // (SQL unchanged from sql/q15.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q15 plan_sql should reference catalog_sales: ${own[0][4]}")
