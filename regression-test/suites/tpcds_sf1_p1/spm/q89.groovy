...
suite("spm_tpcds_sf1_q89", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q89 query
    // (SQL unchanged from sql/q89.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q89 plan_sql should reference item: ${own[0][4]}")
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, s_store_name ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q89 query")
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, s_store_name ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q89 query")
...
ORDER BY (sum_sales - avg_monthly_sales) ASC, s_store_name ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q89 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q89 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
