...
suite("spm_tpcds_sf1_q87", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q87 query
    // (SQL unchanged from sql/q87.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
(
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q87 plan_sql should reference store_sales: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
    ...
    def origWithoutSpm = sql """SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
      ...
      WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
        ...
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q87 query")
    
        // ===== match verification: a similar query (same structure, different literals) =====
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
      ...
      WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
        ...
        sql 'set enable_spm_rewrite=false'
    def similarWithoutSpm = sql """SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
      ...
      WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
        ...
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q87 query")
    
        // ===== EXPLAIN check: original and similar queries must actually hit the baseline =====
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
    ...
    def explainSimilar = sql """EXPLAIN SELECT count(*)
FROM
  (
(
      SELECT DISTINCT
      ...
      WHERE (web_sales.ws_sold_date_sk = date_dim.d_date_sk)
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q87 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q87 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
