...
suite("spm_tpcds_sf1_q53", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q53 query
    // (SQL unchanged from sql/q53.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("item"),
                "q53 plan_sql should reference item: ${own[0][4]}")
...
ORDER BY avg_quarterly_sales ASC, sum_sales ASC, i_manufact_id ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q53 query")
...
WHERE ((CASE WHEN (avg_quarterly_sales > 0) THEN (abs((CAST(sum_sales AS DECIMAL(22,4)) - avg_quarterly_sales)) / avg_quarterly_sales) ELSE null END) > CAST('0.2' AS DECIMAL(2,1)))
ORDER BY avg_quarterly_sales ASC, sum_sales ASC, i_manufact_id ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY avg_quarterly_sales ASC, sum_sales ASC, i_manufact_id ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q53 query")
...
ORDER BY avg_quarterly_sales ASC, sum_sales ASC, i_manufact_id ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q53 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q53 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
