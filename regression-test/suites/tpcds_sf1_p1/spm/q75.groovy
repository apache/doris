...
suite("spm_tpcds_sf1_q75", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q75 query
    // (SQL unchanged from sql/q75.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q75 plan_sql should reference catalog_sales: ${own[0][4]}")
...
ORDER BY sales_cnt_diff ASC, sales_amt_diff ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q75 query")
...
WHERE (curr_yr.i_brand_id = prev_yr.i_brand_id)
   ...
   AND (curr_yr.i_category_id = prev_yr.i_category_id)
   AND (curr_yr.i_manufact_id = prev_yr.i_manufact_id)
   AND (curr_yr.d_year = 2003)
   AND (prev_yr.d_year = (2003 - 1))
   AND ((CAST(curr_yr.sales_cnt AS DECIMAL(17,2)) / CAST(prev_yr.sales_cnt AS DECIMAL(17,2))) < CAST('0.95' AS DECIMAL(2,1)))
ORDER BY sales_cnt_diff ASC, sales_amt_diff ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY sales_cnt_diff ASC, sales_amt_diff ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q75 query")
...
ORDER BY sales_cnt_diff ASC, sales_amt_diff ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q75 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q75 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
