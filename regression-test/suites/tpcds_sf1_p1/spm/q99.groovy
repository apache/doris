...
suite("spm_tpcds_sf1_q99", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q99 query
    // (SQL unchanged from sql/q99.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q99 plan_sql should reference catalog_sales: ${own[0][4]}")
...
ORDER BY substr(w_warehouse_name, 1, 20) ASC, sm_type ASC, cc_name ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q99 query")
...
WHERE (d_month_seq BETWEEN 1201 AND (1201 + 11))
   ...
   AND (cs_ship_mode_sk = sm_ship_mode_sk)
   AND (cs_call_center_sk = cc_call_center_sk)
GROUP BY substr(w_warehouse_name, 1, 21), sm_type, cc_name
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, cc_name ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
...
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, cc_name ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q99 query")
...
ORDER BY substr(w_warehouse_name, 1, 21) ASC, sm_type ASC, cc_name ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q99 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q99 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
