...
suite("spm_tpcds_sf1_q03", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q03 query
    // (SQL unchanged from sql/q03.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("date_dim"),
                "q03 plan_sql should reference date_dim: ${own[0][4]}")
