...
suite("spm_tpcds_sf1_q16", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q16 query
    // (SQL unchanged from sql/q16.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q16 plan_sql should reference catalog_sales: ${own[0][4]}")
...
, customer_address
, call_center
WHERE (d_date BETWEEN CAST('2003-2-01' AS DATE) AND (CAST('2003-2-01' AS DATE) + INTERVAL  '60' DAY))
   AND (cs1.cs_ship_date_sk = d_date_sk)
   AND (cs1.cs_ship_addr_sk = ca_address_sk)
   AND (ca_state = 'CA')
   AND (cs1.cs_call_center_sk = cc_call_center_sk)
