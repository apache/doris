...
suite("spm_tpcds_sf1_q48", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q48 query
    // (SQL unchanged from sql/q48.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    ...
    def bindSql = """SELECT sum(ss_quantity)
FROM
  store_sales
, store
   ...
   AND (((ss_addr_sk = ca_address_sk)
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q48 plan_sql should reference store_sales: ${own[0][4]}")
    ...
    def origWithSpm = sql """SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
...
WHERE (s_store_sk = ss_store_sk)
   ...
   AND (((ss_addr_sk = ca_address_sk)
    ...
    def origWithoutSpm = sql """SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q48 query")
    
        // ===== match verification: a similar query (same structure, different literals) =====
        sql 'set enable_spm_rewrite=true'
    def similarWithSpm = sql """SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
...
WHERE (s_store_sk = ss_store_sk)
   ...
   AND (((cd_demo_sk = ss_cdemo_sk)
         ...
         AND (ss_sales_price BETWEEN CAST('110.00' AS DECIMAL(5,2)) AND CAST('160.00' AS DECIMAL(5,2))))
      OR ((cd_demo_sk = ss_cdemo_sk)
         AND (cd_marital_status = 'M')
         AND (cd_education_status = 'College')
         AND (ss_sales_price BETWEEN CAST('60.00' AS DECIMAL(5,2)) AND CAST('110.00' AS DECIMAL(5,2))))
      OR ((cd_demo_sk = ss_cdemo_sk)
         AND (cd_marital_status = 'D')
         AND (cd_education_status = 'Secondary')
         AND (ss_sales_price BETWEEN CAST('160.00' AS DECIMAL(5,2)) AND CAST('210.00' AS DECIMAL(5,2)))))
   AND (((ss_addr_sk = ca_address_sk)
         AND (ca_country = 'United States')
         AND (ca_state IN ('CA'      , 'WA'      , 'GA'))
         AND (ss_net_profit BETWEEN 0 AND 2001))
      OR ((ss_addr_sk = ca_address_sk)
         AND (ca_country = 'United States')
         AND (ca_state IN ('NV'      , 'AZ'      , 'MN'))
         AND (ss_net_profit BETWEEN 250 AND 3500))
      OR ((ss_addr_sk = ca_address_sk)
         AND (ca_country = 'United States')
         AND (ca_state IN ('FL'      , 'MI'      , 'NY'))
             AND (ss_net_profit BETWEEN 100 AND 26000)))"""
        sql 'set enable_spm_rewrite=false'
    def similarWithoutSpm = sql """SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q48 query")
    
        // ===== EXPLAIN check: original and similar queries must actually hit the baseline =====
        sql 'set enable_spm_rewrite=true'
    def explainOrig = sql """EXPLAIN SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
...
WHERE (s_store_sk = ss_store_sk)
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
         ...
         AND (ca_state IN ('VA'      , 'CA'      , 'MS'))
             AND (ss_net_profit BETWEEN 50 AND 25000)))"""
    def explainSimilar = sql """EXPLAIN SELECT sum(ss_quantity)
FROM
  store_sales
, store
, customer_demographics
   ...
   AND (((ss_addr_sk = ca_address_sk)
      ...
      OR ((ss_addr_sk = ca_address_sk)
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q48 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q48 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
