...
suite("spm_tpcds_sf1_q19", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q19 query
    // (SQL unchanged from sql/q19.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("date_dim"),
                "q19 plan_sql should reference date_dim: ${own[0][4]}")
