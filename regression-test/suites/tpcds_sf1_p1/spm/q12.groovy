...
suite("spm_tpcds_sf1_q12", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q12 query
    // (SQL unchanged from sql/q12.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q12 plan_sql should reference web_sales: ${own[0][4]}")
...
, i_current_price
, sum(ws_ext_sales_price) itemrevenue
, ((sum(ws_ext_sales_price) * 101) / sum(sum(ws_ext_sales_price)) OVER (PARTITION BY i_class)) revenueratio
FROM
  web_sales
, item
, date_dim
WHERE (ws_item_sk = i_item_sk)
   ...
   AND (ws_sold_date_sk = d_date_sk)
   AND (CAST(d_date AS DATE) BETWEEN CAST('2000-02-22' AS DATE) AND (CAST('2000-02-22' AS DATE) + INTERVAL  '30' DAY))
GROUP BY i_item_id, i_item_desc, i_category, i_class, i_current_price
ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC
    LIMIT 101"""
        sql 'set enable_spm_rewrite=false'
