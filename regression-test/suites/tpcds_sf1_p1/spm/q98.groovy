...
suite("spm_tpcds_sf1_q98", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q98 query
    // (SQL unchanged from sql/q98.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q98 plan_sql should reference store_sales: ${own[0][4]}")
...
GROUP BY i_item_id, i_item_desc, i_category, i_class, i_current_price
    ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q98 query")
...
, i_current_price
, sum(ss_ext_sales_price) itemrevenue
, ((sum(ss_ext_sales_price) * 101) / sum(sum(ss_ext_sales_price)) OVER (PARTITION BY i_class)) revenueratio
FROM
  store_sales
, item
, date_dim
...
GROUP BY i_item_id, i_item_desc, i_category, i_class, i_current_price
    ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q98 query")
...
GROUP BY i_item_id, i_item_desc, i_category, i_class, i_current_price
    ORDER BY i_category ASC, i_class ASC, i_item_id ASC, i_item_desc ASC, revenueratio ASC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q98 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q98 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
