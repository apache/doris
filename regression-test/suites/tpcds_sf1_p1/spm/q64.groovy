...
suite("spm_tpcds_sf1_q64", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q64 query
    // (SQL unchanged from sql/q64.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY cs1.product_name ASC, cs1.store_name ASC, cs2.cnt ASC, 14, 15, 16, 17, 18"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q64 plan_sql should reference catalog_sales: ${own[0][4]}")
   ...
   AND (cs1.store_zip = cs2.store_zip)
    ORDER BY cs1.product_name ASC, cs1.store_name ASC, cs2.cnt ASC, 14, 15, 16, 17, 18"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q64 query")
...
, cross_sales AS (
   ...
   WHERE (ss_store_sk = s_store_sk)
      ...
      AND (hd2.hd_income_band_sk = ib2.ib_income_band_sk)
      AND (cd1.cd_marital_status <> cd2.cd_marital_status)
      AND (i_color IN ('almond'   , 'antique'   , 'aquamarine'   , 'azure'   , 'beige'   , 'bisque'))
      AND (i_current_price BETWEEN 65 AND (65 + 10))
      AND (i_current_price BETWEEN (65 + 1) AND (65 + 15))
   GROUP BY i_product_name, i_item_sk, s_store_name, s_zip, ad1.ca_street_number, ad1.ca_street_name, ad1.ca_city, ad1.ca_zip, ad2.ca_street_number, ad2.ca_street_name, ad2.ca_city, ad2.ca_zip, d1.d_year, d2.d_year, d3.d_year
)
SELECT
  cs1.product_name
   ...
   AND (cs1.store_zip = cs2.store_zip)
    ORDER BY cs1.product_name ASC, cs1.store_name ASC, cs2.cnt ASC, 14, 15, 16, 17, 18"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q64 query")
   ...
   AND (cs1.store_zip = cs2.store_zip)
    ORDER BY cs1.product_name ASC, cs1.store_name ASC, cs2.cnt ASC, 14, 15, 16, 17, 18"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q64 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q64 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
