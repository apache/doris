...
suite("spm_tpcds_sf1_q04", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q04 query
    // (SQL unchanged from sql/q04.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q04 plan_sql should reference customer: ${own[0][4]}")
