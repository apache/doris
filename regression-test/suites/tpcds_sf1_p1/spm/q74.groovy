...
suite("spm_tpcds_sf1_q74", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q74 query
    // (SQL unchanged from sql/q74.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer"),
                "q74 plan_sql should reference customer: ${own[0][4]}")
...
ORDER BY 1 ASC, 1 ASC, 1 ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q74 query")
...
ORDER BY 1 ASC, 1 ASC, 1 ASC
    LIMIT 100"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q74 query")
...
ORDER BY 1 ASC, 1 ASC, 1 ASC
    LIMIT 100"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q74 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q74 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
