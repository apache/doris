...
suite("spm_tpcds_sf1_q76", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q76 query
    // (SQL unchanged from sql/q76.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q76 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY channel ASC, col_name ASC, d_year ASC, d_qoy ASC, i_category ASC
    LIMIT 100"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q76 query")
...
ORDER BY channel ASC, col_name ASC, d_year ASC, d_qoy ASC, i_category ASC
    LIMIT 101"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q76 query")
...
ORDER BY channel ASC, col_name ASC, d_year ASC, d_qoy ASC, i_category ASC
    LIMIT 101"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q76 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q76 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
