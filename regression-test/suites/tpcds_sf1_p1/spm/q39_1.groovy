...
suite("spm_tpcds_sf1_q39_1", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q39_1 query
    // (SQL unchanged from sql/q39_1.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    ...
    // q39 divides aggregates (stdev_samp / avg) into DOUBLE columns; the frozen replay
    // pins the captured physical plan, whose aggregation layout may differ from the
    // no-SPM plan and therefore produce a last-ulp double difference. Compare results
    // with the doubles rounded to 12 significant digits instead of exact equality.
    def normRows = { rows ->
        rows.collect { row ->
            row.collect { v ->
                v instanceof Double || v instanceof BigDecimal ?
                        new java.math.BigDecimal(v.toString()).round(new java.math.MathContext(12)) : v
            }
        }
    }
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("inventory"),
                "q39_1 plan_sql should reference inventory: ${own[0][4]}")
...
WHERE (inv1.i_item_sk = inv2.i_item_sk)
   ...
   AND (inv2.d_moy = (1 + 1))
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        assertEquals(normRows(origWithSpm), normRows(origWithoutSpm),
                "SPM rewrite must preserve the result of the q39_1 query")
...
WHERE (inv1.i_item_sk = inv2.i_item_sk)
   ...
   AND (inv2.d_moy = (2 + 1))
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        assertEquals(normRows(similarWithSpm), normRows(similarWithoutSpm),
                "SPM rewrite must preserve the result of a similar q39_1 query")
...
ORDER BY inv1.w_warehouse_sk ASC, inv1.i_item_sk ASC, inv1.d_moy ASC, inv1.mean ASC, inv1.cov ASC, inv2.d_moy ASC, inv2.mean ASC, inv2.cov ASC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q39_1 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q39_1 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
