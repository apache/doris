...
suite("spm_tpcds_sf1_q24", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q24 query
    // (SQL unchanged from sql/q24_1.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
    // q24 has two parts (sql/q24_1.sql / sql/q24_2.sql) that differ ONLY
    // in the i_color literal ('pale' vs 'chiffon'): their value-free digest is
    // identical, so ONE baseline serves both and the second part doubles as the
    // similar query (same structure, one different literal).
...
ORDER BY c_last_name, c_first_name, s_store_name"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q24 plan_sql should reference store_sales: ${own[0][4]}")
...
ORDER BY c_last_name, c_first_name, s_store_name"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q24 query")
...
ORDER BY c_last_name, c_first_name, s_store_name"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q24 query")
...
ORDER BY c_last_name, c_first_name, s_store_name"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q24 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q24 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
