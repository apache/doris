...
suite("spm_tpcds_sf1_q18", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q18 query
    // (SQL unchanged from sql/q18.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("catalog_sales"),
                "q18 plan_sql should reference catalog_sales: ${own[0][4]}")
...
WHERE (cs_sold_date_sk = d_date_sk)
   ...
   AND (c_current_cdemo_sk = cd2.cd_demo_sk)
   AND (c_current_addr_sk = ca_address_sk)
   AND (c_birth_month IN (2, 4, 6, 8, 10, 12))
   AND (d_year = 1999)
   AND (ca_state IN ('CA', 'WA', 'OR', 'NV', 'AZ', 'UT', 'CO'))
GROUP BY ROLLUP (i_item_id, ca_country, ca_state, ca_county)
ORDER BY ca_country ASC, ca_state ASC, ca_county ASC, i_item_id ASC
    LIMIT 100"""
        sql 'set enable_spm_rewrite=false'
