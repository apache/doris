...
suite("spm_tpcds_sf1_q06", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q06 query
    // (SQL unchanged from sql/q06.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
LIMIT 100"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("customer_address"),
                "q06 plan_sql should reference customer_address: ${own[0][4]}")
...
WHERE (a.ca_address_sk = c.c_current_addr_sk)
   ...
   AND (i.i_current_price > (CAST('1.0' AS DECIMAL(2,1)) * (
         SELECT avg(j.i_current_price)
         FROM
           item j
         WHERE (j.i_category = i.i_category)
