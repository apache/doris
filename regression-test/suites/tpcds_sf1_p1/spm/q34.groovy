...
suite("spm_tpcds_sf1_q34", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q34 query
    // (SQL unchanged from sql/q34.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY c_last_name ASC, c_first_name ASC, c_salutation ASC, c_preferred_cust_flag DESC, ss_ticket_number ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("store_sales"),
                "q34 plan_sql should reference store_sales: ${own[0][4]}")
   ...
   AND (cnt BETWEEN 15 AND 20)
    ORDER BY c_last_name ASC, c_first_name ASC, c_salutation ASC, c_preferred_cust_flag DESC, ss_ticket_number ASC"""
        assertEquals(origWithSpm, origWithoutSpm,
                "SPM rewrite must preserve the result of the q34 query")
...
FROM
  (
   ...
   WHERE (store_sales.ss_sold_date_sk = date_dim.d_date_sk)
      ...
      AND (store_sales.ss_hdemo_sk = household_demographics.hd_demo_sk)
      AND ((date_dim.d_dom BETWEEN 1 AND 3)
         OR (date_dim.d_dom BETWEEN 26 AND 28))
      AND ((household_demographics.hd_buy_potential = '>10000')
         OR (household_demographics.hd_buy_potential = 'Unknown'))
      AND (household_demographics.hd_vehicle_count > 0)
      AND ((CASE WHEN (household_demographics.hd_vehicle_count > 0) THEN (CAST(household_demographics.hd_dep_count AS DECIMAL(7,2)) / household_demographics.hd_vehicle_count) ELSE null END) > CAST('1.2' AS DECIMAL(2,1)))
   ...
   AND (cnt BETWEEN 15 AND 20)
    ORDER BY c_last_name ASC, c_first_name ASC, c_salutation ASC, c_preferred_cust_flag DESC, ss_ticket_number ASC"""
        assertEquals(similarWithSpm, similarWithoutSpm,
                "SPM rewrite must preserve the result of a similar q34 query")
   ...
   AND (cnt BETWEEN 15 AND 20)
    ORDER BY c_last_name ASC, c_first_name ASC, c_salutation ASC, c_preferred_cust_flag DESC, ss_ticket_number ASC"""
        ...
        assertTrue(explainOrig.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the original q34 query should report SPM baseline hit id " + id + ", got: " + explainOrig)
        assertTrue(explainSimilar.toString().contains("SPM baseline hit: id=" + id),
                "EXPLAIN of the similar q34 query should report SPM baseline hit id " + id + ", got: " + explainSimilar)
    } finally {
