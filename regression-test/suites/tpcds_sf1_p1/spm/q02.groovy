...
suite("spm_tpcds_sf1_q02", "spm") {

    // SPM baseline DDL + match verification on the ORIGINAL TPCDS q02 query
    // (SQL unchanged from sql/q02.sql) against the REAL TPCDS sf1 data loaded
    // by load.groovy.
...
ORDER BY d_week_seq1 ASC"""
    ...
    try {
        ...
        assertTrue(own[0][4].toString().contains("web_sales"),
                "q02 plan_sql should reference web_sales: ${own[0][4]}")
