...
suite("test_regression_test_tpcds_sf1_p1_q63", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("VTOP-N\n" + 
				"  |  order by: <slot 20> `i_manager_id` ASC, <slot 21> `avg_monthly_sales` ASC, <slot 22> `sum_sales` ASC") && 
		explainStr.contains("predicates: ((CASE WHEN (<slot 57> > 0) THEN (abs((<slot 60> <slot 15> sum(`ss_sales_price`) - <slot 57>)) / <slot 57>) ELSE NULL END) > 0.1)") && 
		explainStr.contains("order by: <slot 58> <slot 13> `i_manager_id` ASC") && 
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				"  |  output: sum(<slot 15> sum(`ss_sales_price`))\n" + 
				"  |  group by: <slot 13> `i_manager_id`, <slot 14> `d_moy`") && 
		...
		explainStr.contains("TABLE: item(item), PREAGGREGATION: ON\n" + 
				"     PREDICATES: `i_category` IN ('Books', 'Children', 'Electronics', 'Women', 'Music', 'Men'), `i_class` IN ('personal', 'portable', 'refernece', 'self-help', 'accessories', 'classical', 'fragrances', 'pants'), `i_brand` IN ('scholaramalgamalg #14', 'scholaramalgamalg #7', 'exportiunivamalg #9', 'scholaramalgamalg #9', 'amalgimporto #1', 'edu packscholar #1', 'exportiimporto #1', 'importoamalg #1'), (((`i_category` IN ('Books', 'Children', 'Electronics')) AND (`i_class` IN ('personal', 'portable', 'refernece', 'self-help')) AND (`i_brand` IN ('scholaramalgamalg #14', 'scholaramalgamalg #7', 'exportiunivamalg #9', 'scholaramalgamalg #9'))) OR ((`i_category` IN ('Women', 'Music', 'Men')) AND (`i_class` IN ('accessories', 'classical', 'fragrances', 'pants')) AND (`i_brand` IN ('amalgimporto #1', 'edu packscholar #1', 'exportiimporto #1', 'importoamalg #1'))))") 
