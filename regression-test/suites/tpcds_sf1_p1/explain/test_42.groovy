...
suite("test_regression_test_tpcds_sf1_p1_q42", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("VTOP-N\n" + 
				"  |  order by: <slot 14> <slot 13> sum(`ss_ext_sales_price`) DESC, <slot 15> <slot 10> `dt`.`d_year` ASC, <slot 16> <slot 11> `item`.`i_category_id` ASC, <slot 17> <slot 12> `item`.`i_category` ASC") && 
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				"  |  output: sum(<slot 13> sum(`ss_ext_sales_price`))\n" + 
				"  |  group by: <slot 10> `dt`.`d_year`, <slot 11> `item`.`i_category_id`, <slot 12> `item`.`i_category`") && 
		explainStr.contains("VAGGREGATE (update serialize)\n" + 
				"  |  STREAMING\n" + 
				"  |  output: sum(<slot 24>)\n" + 
				"  |  group by: <slot 27>, <slot 30>, <slot 31>") && 
