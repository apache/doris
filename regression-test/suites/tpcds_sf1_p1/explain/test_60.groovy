...
suite("test_regression_test_tpcds_sf1_p1_q60", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				...
				"  |  group by: <slot 32> `i_item_id`") && 
		explainStr.contains("VAGGREGATE (update serialize)\n" + 
				"  |  STREAMING\n" + 
				"  |  output: sum(<slot 125>)\n" + 
				"  |  group by: <slot 129>") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 118> = `i_item_id`") && 
		...
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				...
				"  |  group by: <slot 14> `i_item_id`") && 
		explainStr.contains("VAGGREGATE (update serialize)\n" + 
				"  |  STREAMING\n" + 
				"  |  output: sum(<slot 88>)\n" + 
				"  |  group by: <slot 92>") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: <slot 81> = `i_item_id`") && 
