...
suite("test_regression_test_tpcds_sf1_p1_q37", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("join op: INNER JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: (<slot 31> = `cs_item_sk`)") && 
		...
		explainStr.contains("TABLE: date_dim(date_dim), PREAGGREGATION: ON\n" + 
				"     PREDICATES: CAST(`d_date` AS DATE) >= '2000-02-01 00:00:00', CAST(`d_date` AS DATE) <= '2000-04-01 00:00:00'") && 
		explainStr.contains("TABLE: item(item), PREAGGREGATION: ON\n" + 
				"     PREDICATES: `i_current_price` >= 68, `i_current_price` <= 98, (`i_manufact_id` IN (677, 940, 694, 808))") 
