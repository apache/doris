...
suite("test_regression_test_tpcds_sf1_p1_q08", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("VTOP-N\n" + 
				"  |  order by: <slot 23> <slot 21> `s_store_name` ASC") && 
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				"  |  output: sum(<slot 22> sum(`ss_net_profit`))\n" + 
				"  |  group by: <slot 21> `s_store_name`") && 
		explainStr.contains("VAGGREGATE (update serialize)\n" + 
				"  |  STREAMING\n" + 
				"  |  output: sum(<slot 44>)\n" + 
				"  |  group by: <slot 47>") && 
		explainStr.contains("join op: INNER JOIN(BROADCAST)[Src column hash been transformed by expr]\n" + 
				"  |  equal join conjunct: (substr(<slot 36>, 1, 2) = substr(<slot 9> substr(`ca_zip`, 1, 5) `ca_zip`, 1, 2))") && 
		explainStr.contains("vec output tuple id: 16") && 
		explainStr.contains("output slot ids: 44 47 \n" + 
				"  |  hash output slot ids: 34 31 ") && 
		explainStr.contains("join op: INNER JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: (<slot 27> = `d_date_sk`)") && 
		explainStr.contains("vec output tuple id: 14") && 
		explainStr.contains("output slot ids: 31 34 36 \n" + 
				"  |  hash output slot ids: 25 28 30 ") && 
		...
		explainStr.contains("output slot ids: 25 27 28 30 \n" + 
				"  |  hash output slot ids: 16 20 12 13 ") && 
