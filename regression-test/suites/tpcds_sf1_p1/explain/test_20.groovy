...
suite("test_regression_test_tpcds_sf1_p1_q20", "regression_test_tpcds_sf1_p1") {
    ...
    explain {
		...
		explainStr.contains("VTOP-N\n" + 
				"  |  order by: <slot 18> <slot 13> `i_category` ASC, <slot 19> <slot 14> `i_class` ASC, <slot 20> <slot 11> `i_item_id` ASC, <slot 21> <slot 12> `i_item_desc` ASC, <slot 22> ((<slot 16> sum(`cs_ext_sales_price`) * 100) / <slot 17> sum(sum(`cs_ext_sales_price`)) OVER (PARTITION BY `i_class`)) ASC") && 
		explainStr.contains("order by: <slot 49> <slot 14> `i_class` ASC") && 
		explainStr.contains("VAGGREGATE (merge finalize)\n" + 
				"  |  output: sum(<slot 16> sum(`cs_ext_sales_price`))\n" + 
