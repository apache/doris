...
suite("compaction_variant_doc_snapshot", "p0") {
    ...
    try {
        ...
        for (int i = 0; i < key_types.size(); i++) {
            ...
            qt_sql_5 "select cast(v['b'] as string), cast(v['b']['c'] as string) from  ${tableName} where cast(v['b'] as string) != null or cast(v['b'] as string) != '{}' or cast(v['b'] as string) is not null or v['b'] is not null order by k desc, 1, 2 limit 10;"
