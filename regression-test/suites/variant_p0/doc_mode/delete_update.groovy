...
suite("variant_delete_and_update_doc_value", "p0"){
    ...
    sql """insert into ${table_name} values (1, ${variantV2Function}('{"a":1,"b":[1],"c":1.0, "d" : [{"x" : 1}]}'))"""
    sql """insert into ${table_name} values (2, ${variantV2Function}('{"a":2,"b":[1],"c":2.0, "d" : [{"y" : 1}]}'))"""
    sql """insert into ${table_name} values (3, ${variantV2Function}('{"a":3,"b":[3],"c":3.0, "d" : [{"o" : 1}]}'))"""
    sql """insert into ${table_name} values (4, ${variantV2Function}('{"a":4,"b":[4],"c":4.0, "d" : [{"p" : 1}]}'))"""
    sql """insert into ${table_name} values (5, ${variantV2Function}('{"a":5,"b":[5],"c":5.0, "d" : [{"q" : 1}]}'))"""

    sql "delete from ${table_name} where k = 1"
    sql """update ${table_name} set v = ${variantV2Function}('{"updated_value":123}') where k = 2"""
    qt_sql "select * from ${table_name} order by k"
    sql """update ${table_name} set v = ${variantV2Function}('{"updated_nested_value":[{"ommm" : "123"}]}') where k = 4"""
    qt_sql "select * from ${table_name} order by k"
    ...
    sql """insert into var_delete_update_mow values (6, ${variantV2Function}('{"a":4,"b":[4],"c":4.1}'), 'xxx')"""
    sql """insert into var_delete_update_mow values (7, ${variantV2Function}('{"a":4,"b":[4],"c":4.1}'), 'yyy')"""
    sql """update var_delete_update_mow set vs = '{"updated_value" : 123}' where k = 6"""
    sql """update var_delete_update_mow set v = ${variantV2Function}('{"updated_value":1111}') where k = 7"""
    sql """update var_delete_update_mow set v = ${variantV2Function}('{"updated_nested_value":[{"lalalal": 1.111}]}') where k = 2"""
    ...
    sql """insert into ${tableName} values
        (2, "doris2", 2000, 223, 2, ${variantV2Function}('{"id":2, "name":"doris2","score":2000,"test":223,"dft":2}')),
        (1, "doris", 1000, 123, 1, ${variantV2Function}('{"id":1, "name":"doris","score":1000,"test":123,"dft":1}')),
        (5, "doris5", 5000, 523, 5, ${variantV2Function}('{"id":5, "name":"doris5","score":5000,"test":523,"dft":5}')),
        (4, "doris4", 4000, 423, 4, ${variantV2Function}('{"id":4, "name":"doris4","score":4000,"test":423,"dft":4}')),
        (3, "doris3", 3000, 323, 3, ${variantV2Function}('{"id":3, "name":"doris3","score":3000,"test":323,"dft":3}'));"""
