...
suite("regression_test_variant_predefine_multi_var", "variant_type"){
    ...
    sql """INSERT INTO ${table_name} select k, v, ${variantV2Function}(cast(v as string)) from ${table_name}"""

    sql """alter table ${table_name} add column v3 variant<'k1' : int, 'k2' : string, 'k3' : array<int>, 'k4' : double> default null"""
    sql """INSERT INTO ${table_name} select k, v, ${variantV2Function}(cast(v as string)), ${variantV2Function}(cast(v as string)) from ${table_name}"""
    sql "alter table ${table_name} add column ss string default null"
    sql """INSERT INTO ${table_name} select k, v, ${variantV2Function}(cast(v as string)), ${variantV2Function}(cast(v as string)), cast(v as string) from ${table_name}"""
    ...
    for (int i = 0; i < 20; i++) {
        sql """insert into ${table_name}  values (1, ${variantV2Function}('{"a" : 1}'), ${variantV2Function}('{"a" : 1}'), ${variantV2Function}('{"a" : 1}'), '{"a" : 1}', ${variantV2Function}('{"a" : 1}'))"""
    }
    trigger_and_wait_compaction(table_name, "cumulative", 1800)
}
