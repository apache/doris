...
suite("regression_test_variant_predefine_hirachinal", "variant_type"){
    ...
    sql """
            CREATE TABLE IF NOT EXISTS ${table_name} (
                k bigint,
                v variant<'a' : largeint, 'c.d' : text, properties("variant_max_subcolumns_count" = "0")>
            )
            DUPLICATE KEY(`k`)
    ...
    order_qt_sql1 "select replace(cast(v['c'] as string), ', ', ',') from var_rs where k = -3 or k = -2 or k = -4 or (k = 1 and v['c'] = 1024) order by k"
    order_qt_sql2 "select replace(cast(v['c'] as string), ', ', ',') from var_rs where k = -3 or k = -2 or k = 1 order by k, cast(v['c'] as text) limit 3"
}
