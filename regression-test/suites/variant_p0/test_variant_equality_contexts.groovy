...
suite("test_variant_equality_contexts", "p0,nonConcurrent") {
    ...
    test {
        sql """
            SELECT DISTINCT CAST(CAST(number AS STRING) AS VARIANT)
            FROM numbers("number" = "2")
        """
        exception "Doris hll, bitmap"
    }
    ...
    test {
        sql """
            SELECT COUNT(DISTINCT CAST(CAST(number AS STRING) AS VARIANT))
            FROM numbers("number" = "2")
        """
    ...
    sql "SELECT json_extract(CAST('1' AS VARIANT), '\$')"
}
