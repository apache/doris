...
suite("test_table_properties") {
    ...
    qt_select_check_2 """select * from information_schema.table_properties where table_schema=\"${dbName}\" ORDER BY TABLE_CATALOG,TABLE_SCHEMA,TABLE_NAME,PROPERTY_NAME,PROPERTY_VALUE where PROPERTY_NAME != "default.replication_allocation" """
    sql """
        drop table listtable;
    """    
    qt_select_check_3 """select * from information_schema.table_properties where table_schema=\"${dbName}\" ORDER BY TABLE_CATALOG,TABLE_SCHEMA,TABLE_NAME,PROPERTY_NAME,PROPERTY_VALUE where PROPERTY_NAME != "default.replication_allocation" """
    ...
    connect(user=user, password='123abc!@#', url=url) {
       qt_select_check_4 """select * from information_schema.table_properties  ORDER BY TABLE_CATALOG,TABLE_SCHEMA,TABLE_NAME,PROPERTY_NAME,PROPERTY_VALUE where PROPERTY_NAME != "default.replication_allocation"; """
    }

    sql "GRANT SELECT_PRIV ON ${dbName}.duplicate_table  TO ${user}"
    connect(user=user, password='123abc!@#', url=url) {
       qt_select_check_5 """select * from information_schema.table_properties  ORDER BY TABLE_CATALOG,TABLE_SCHEMA,TABLE_NAME,PROPERTY_NAME,PROPERTY_VALUE where PROPERTY_NAME != "default.replication_allocation"; """
    }
 
    sql "REVOKE SELECT_PRIV ON ${dbName}.duplicate_table  FROM ${user}"
    connect(user=user, password='123abc!@#', url=url) {
       qt_select_check_6 """select * from information_schema.table_properties  ORDER BY TABLE_CATALOG,TABLE_SCHEMA,TABLE_NAME,PROPERTY_NAME,PROPERTY_VALUE where PROPERTY_NAME != "default.replication_allocation"; """       
    }
