...
suite("test_show_inverted_index_storage_format") {
    ...
    def assertPartitionFormats = { tableName, expectedPartitionCount, expectedFormat ->
        ...
        partitions.each { partition ->
            assertEquals(expectedFormat, partition.InvertedIndexStorageFormat)
            def partitionDetails = sql_return_maparray("SHOW PARTITION ${partition.PartitionId}")
            ...
            def tablets = sql_return_maparray(
                    "SHOW TABLETS FROM ${tableName} PARTITION(${partition.PartitionName})")
