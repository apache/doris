...
suite("explode") {
    ...
    qt_select25 """
        select * from (select 1) t1 lateral view explode(array(struct(1, "a"), struct(2, "b"))) t2 as c1 order by 1, struct_element(c1, 1);
    """
    qt_select26 """
    ...
    qt_select27 """
        select * from (select 1) t1 
            ...
            ) t2 as c1 order by 1, struct_element(struct_element(c1, 1), 1), struct_element(struct_element(c1, 2), 1);
    """
    qt_select28 """
        select * from (select 1) t1 
            lateral view explode(
                array(struct(1, "a"), struct(2, "b")),
                array(struct(3, "c"), struct(4, "d"))
            ) t2 as c1, c2 order by 1, struct_element(c1, 1), struct_element(c2, 1);
    """
}
