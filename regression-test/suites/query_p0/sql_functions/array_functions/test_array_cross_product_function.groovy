...
suite("test_array_cross_product_function") {
    ...
    sql """
        ...
        (4, [0, 1, 0], [1, 0, 0])
    """
    qt_sql "SELECT CROSS_PRODUCT(vec1, vec2) from ${tableName}"
    sql "DROP TABLE IF EXISTS ${tableName}"

    // abnormal test cases
    test {
        sql "SELECT cross_product(NULL, [1, 2, 3])"
        exception "First argument for function cross_product cannot be null"
    }
    test {
        sql "SELECT cross_product([1, 2, 3], NULL)"
        exception "Second argument for function cross_product cannot be null"
    }
    test {
    ...
    test {
        ...
        exception "function cross_product have different input element sizes of array: 3 and 2"
    }
    test {
