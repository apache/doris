...
suite("test_ipv6_cidr_to_range") {
    sql """ DROP TABLE IF EXISTS test_ipv6_cidr_to_range """

    sql """ SET enable_nereids_planner=true """
    sql """ SET enable_fallback_to_original_planner=false """

    sql """
        CREATE TABLE test_ipv6_cidr_to_range (
          `id` int,
          `addr` ipv6,
          `cidr` int
        ) ENGINE=OLAP
    ...
    sql """
        insert into test_ipv6_cidr_to_range values
    ...
    qt_sql "select id, struct_element(ipv6_cidr_to_range(addr, cidr), 'min') as min_range, struct_element(ipv6_cidr_to_range(addr, cidr), 'max') as max_range from test_ipv6_cidr_to_range order by id"

    sql """ DROP TABLE IF EXISTS test_ipv6_cidr_to_range """

    sql """
        CREATE TABLE test_str_cidr_to_range (
          `id` int,
          `addr` string,
          `cidr` int
        ) ENGINE=OLAP
    ...
    sql """
        insert into test_str_cidr_to_range values
        (1, '2001:0db8:0000:85a3:0000:0000:ac1f:8001', 0),
        (2, '2001:0db8:0000:85a3:ffff:ffff:ffff:ffff', 32),
        (3, 'ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff', 16),
        (4, '2001:df8:0:85a3::ac1f:8001', 32),
    ...
    qt_sql "select id, struct_element(ipv6_cidr_to_range(ipv6_string_to_num(addr), cidr), 'min') as min_range, struct_element(ipv6_cidr_to_range(ipv6_string_to_num(addr), cidr), 'max') as max_range from test_str_cidr_to_range order by id"

    sql """ DROP TABLE IF EXISTS test_str_cidr_to_range """
