...
suite("test_aggregate_all_functions", "arrow_flight_sql") {
    ...
    qt_select_all1 "select dt, page, bitmap_to_string(user_id) from pv_bitmap order by 1,2;"
    qt_select_all2 "select dt, page, bitmap_to_string(user_id) from pv_bitmap where dt = 20220202 order by 1,2;"
    ...
    qt_select_bitmap_page1 "select bitmap_union_count(user_id) from  ${tableName_03} group by page order by 1;"
    qt_select_bitmap_page2 "select bitmap_to_string(bitmap_union(user_id)) FROM ${tableName_03} group by page order by 1;"
