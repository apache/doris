...
suite("test_outfile_agg_state") {
    ...
    create table a_table(
        ...
        k4 agg_state<max(int not null)> generic not null
    )
    aggregate key (k1)
    distributed BY hash(k1) buckets 3
    properties("replication_num" = "1");
    """
    sql "insert into a_table values(1,max_by_state(3,1),group_concat_state('a'),max_state(92));"
    sql "insert into a_table values(1,max_by_state(2,2),group_concat_state('bb'),max_state(7));"
    sql "insert into a_table values(2,max_by_state(1,3),group_concat_state('ccc'),max_state(255));"

    qt_test "select k1,max_by_merge(k2),group_concat_merge(k3),max_merge(k4) from a_table group by k1 order by k1;"
    ...
    """
    Thread.sleep(10000)
    qt_test "select k1,max_by_merge(k2),group_concat_merge(k3),max_merge(k4) from a_table2 group by k1 order by k1;"
