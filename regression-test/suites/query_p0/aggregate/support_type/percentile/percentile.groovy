...
suite("percentile") {
    ...
    sql """set debug_skip_fold_constant=true;"""
    order_qt_percentile_window_const_expr """select id, percentile(v, coalesce(cast(null as double), cast(0.5 as double))) over(order by id rows between 1 preceding and current row) from percentile_nullable_t order by id;"""
    sql """set debug_skip_fold_constant=false;"""
    sql """set enable_aggregate_function_null_v2=true;"""
