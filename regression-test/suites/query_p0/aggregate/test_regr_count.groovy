...
suite("test_regr_count") {
    ...
    // exception
    test {
        sql """select regr_count('range', 1);"""
        exception "regr_count requires numeric for first parameter"
    }

    test {
        sql """select regr_count(1, 'hello');"""
        exception "regr_count requires numeric for second parameter"
    }
