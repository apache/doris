...
suite("test_extensions_schema", "p0") {
    ...
    // Filter by family works. This branch has no FILESYSTEM/CONNECTOR plugin family and ships
    // no built-in plugin on the FE classpath, so the inventory may legitimately be empty.
    def authRows = sql """
        SELECT EXTENSION_NAME FROM information_schema.extensions WHERE EXTENSION_TYPE = 'AUTHENTICATION'
    """
    Assert.assertTrue(authRows.size() <= rows.size())
