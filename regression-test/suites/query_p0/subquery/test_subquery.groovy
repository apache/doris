...
suite("test_subquery") {
        ...
        qt_uncorrelated_exists_with_limit_0 """
            select * from test_query_db.baseall where exists (select * from test_query_db.baseall limit 0)
        """

        // test uncorrelated scalar subquery with limit <= 1
        sql """
            select * from test_query_db.baseall where k1 = (select k1 from test_query_db.baseall limit 1)
        """

        // test uncorrelated subquery in having
        sql """
            select count(*) from test_query_db.baseall
            group by k0
            having min(k0) in (select k0 from test_query_db.baseall)
        """

        // test uncorrelated scalar subquery with more than one return rows
        test {
            sql """
                select * from test_query_db.baseall where k1 = (select k1 from test_query_db.baseall limit 2)
            """
        ...
        qt_uncorrelated_scalar_with_sort_and_limit """
                select * from test_query_db.baseall where k1 = (select k1 from test_query_db.baseall order by k1 desc limit 1)
            """
