...
suite("test_asof_join", "query_p0") {
    ...
    sql """ DROP TABLE IF EXISTS asof_join_left """
    sql """ DROP TABLE IF EXISTS asof_join_right """

    sql """
        CREATE TABLE asof_join_left (
            id INT,
            val1 INT,
            val2 INT,
            ts DATETIME
    ...
    sql """
        INSERT INTO asof_join_left VALUES
    ...
    sql """
        INSERT INTO asof_join_right VALUES
    ...
    qt_asof_expr_key """
        ...
        FROM asof_join_left l
        ASOF LEFT JOIN asof_join_right r
        MATCH_CONDITION(l.ts >= r.ts)
    ...
    // When multiple build rows have same ts, any one is valid - use validation instead of qt_sql
    def dup_ts_result = sql """
        ...
        ORDER BY l.id
    """
    // Validate: each row should match one of the valid build rows with max ts <= probe ts
    // For l.id=1 (ts=10:00): valid matches are id=4 or id=5 (both ts=10:00)
    // For l.id=2 (ts=10:00:30): valid matches are id=4 or id=5 (both ts=10:00, which is max <= 10:00:30)
    assertEquals(2, dup_ts_result.size())
    // Row 0: l.id=1, rid should be 4 or 5, rts should be 10:00, value should be 400 or 500
    assertEquals(1, dup_ts_result[0][0])
    assertTrue(dup_ts_result[0][2] == 4 || dup_ts_result[0][2] == 5, "rid should be 4 or 5, got: ${dup_ts_result[0][2]}")
    assertTrue(dup_ts_result[0][4] == 400 || dup_ts_result[0][4] == 500, "value should be 400 or 500")
    // Row 1: l.id=2, rid should be 4 or 5
    assertEquals(2, dup_ts_result[1][0])
    assertTrue(dup_ts_result[1][2] == 4 || dup_ts_result[1][2] == 5, "rid should be 4 or 5, got: ${dup_ts_result[1][2]}")
    assertTrue(dup_ts_result[1][4] == 400 || dup_ts_result[1][4] == 500, "value should be 400 or 500")
    ...
    // Cover: LEFT x INNER/OUTER x operators(>=,<=,>,<) x SlotRef/Expression
    // Use explain + contains to verify plan type before execution
    
    // ---------- Setup: Tables for ASOF JOIN tests ----------
    ...
    // ---------- Test 33: ASOF LEFT JOIN (verify plan) ----------
    ...
    // ---------- Test 38: Large group to trigger binary search (LEFT JOIN) ----------
    ...
    sql """ DROP TABLE IF EXISTS asof_multi_grp_left2 """
    sql """ DROP TABLE IF EXISTS asof_multi_grp_right2 """

    sql """
        CREATE TABLE asof_multi_grp_left2 (
            id INT,
            grp INT,
            ts DATETIME
        ) DISTRIBUTED BY HASH(id) BUCKETS 1
    ...
    sql """
        CREATE TABLE asof_multi_grp_right2 (
            id INT,
            grp INT,
            ts DATETIME,
            val INT
    ...
    sql """
        INSERT INTO asof_multi_grp_left2 VALUES
        (1, 1, '2024-01-01 10:00:00'),
        (2, 2, '2024-01-01 10:00:00'),
        (3, 3, '2024-01-01 10:00:00'),
        (4, 4, '2024-01-01 10:00:00'),
    ...
    sql """
        INSERT INTO asof_multi_grp_right2 VALUES
        (1, 1, '2024-01-01 09:00:00', 100),
    ...
    sql """ ANALYZE TABLE asof_multi_grp_left2 WITH SYNC """
    sql """ ANALYZE TABLE asof_multi_grp_right2 WITH SYNC """
    ...
    explain {
        sql """
            ...
            FROM asof_multi_grp_left2 l
            ASOF LEFT JOIN asof_multi_grp_right2 r
            MATCH_CONDITION(l.ts >= r.ts)
            ON l.grp = r.grp
            ORDER BY l.id
        """
