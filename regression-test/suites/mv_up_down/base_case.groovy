...
suite("mv_base_case_test") {
    ...
    explain {
        sql("${sql_stmt_dup}")
        contains "(${mv_name_dup})"
    }
    compare_res(sql_stmt_dup + " order by 1,2,3,4,5,6")

    def mv_name_uniq = "uniq_mv"
    def sql_stmt_uniq = """select o_orderkey, 
            case when o_shippriority > 1 then 1 else 2 end cnt_1,
            sum(o_orderkey)
            from orders_2_uniq  
            where  o_orderkey > (-3) + 5 group by o_orderkey, case when o_shippriority > 1 then 1 else 2 end 
            """
    explain {
        sql("${sql_stmt_uniq}")
        contains "(${mv_name_uniq})"
    }
