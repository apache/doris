...
suite("test_lance_vector_search_metrics", "p0,external") {
    ...
    Map<String, String> metricOf = [
            "vs_ivf_flat_f32_cosine": "cosine",
            "vs_ivf_pq_f32_cosine"  : "cosine",
            "vs_ivf_pq_f32_dot"     : "dot",
    ]
    // Only the quantized tables need refinement; IVF_FLAT already answers from the original
    // vectors, and adding refine_factor there would hide what the exactness check proves.
    Map<String, String> refineOf = [
            "vs_ivf_flat_f32_cosine": "",
            "vs_ivf_pq_f32_cosine"  : ', "refine_factor"="10"',
            "vs_ivf_pq_f32_dot"     : ', "refine_factor"="10"',
    ]
    ...
    for (Map.Entry<String, String> entry : metricOf.entrySet()) {
        ...
        explain {
            sql("""SELECT row_id, _distance
                   FROM ${search(table, headQuery, "5", "4", metric, refineOf[table])}
    ...
    for (Map.Entry<String, String> entry : metricOf.entrySet()) {
        ...
        def singleProbe = sql """
            ...
            FROM ${search(table, boundaryQuery, "9", "1", metric, refineOf[table])}
            ORDER BY _distance, row_id
        """
        def flat = sql """
            SELECT row_id, _distance
    ...
    qt_ivf_pq_cosine """
        SELECT row_id, label, _distance
        FROM ${search("vs_ivf_pq_f32_cosine", headQuery, "5", "4", "cosine", refineOf["vs_ivf_pq_f32_cosine"])}
        ORDER BY _distance, row_id
    """
    // dot is not a proper metric: a longer vector scores better than a closer one, so the
    // nearest row here is not the row the query vector was taken from. Lance reports it as a
    // negated inner product, so ORDER BY _distance ASC still puts the best match first.
    qt_ivf_pq_dot """
        SELECT row_id, label, _distance
        FROM ${search("vs_ivf_pq_f32_dot", headQuery, "5", "4", "dot", refineOf["vs_ivf_pq_f32_dot"])}
        ORDER BY _distance, row_id
    """
