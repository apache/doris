...
suite("test_lance_vector_search_two_phase", "p0,external") {
    ...
    // This frozen fixture has two fragments. With top_k=5 and offset=1, every fragment
    // contributes at most six phase-one candidates and the global TopN keeps rows 2..6.
    ...
    try {
        ...
        // Outer WHERE is a Doris scan postfilter, not a Lance prefilter. Each of the two fragments
        // first returns three ANN candidates: rows 1..3 and rows 513..515. Doris evaluates
        // category='odd' in the phase-one scan, leaving rows 2 and 514 before the global TopN.
        // Since Doris does not ask Lance for replacement candidates, fewer than top_k rows remain.
        String postfilterQuery = """
            SELECT row_id, category, label, _distance
            FROM vector_search(
                "table"="${tableName}",
                "column"="embedding",
        ...
        qt_postfilter_execution "${postfilterQuery}"
    } finally {
        // sql "DROP CATALOG IF EXISTS `${catalogName}`"
    }
}
