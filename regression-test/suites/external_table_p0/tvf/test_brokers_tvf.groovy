...
suite("test_brokers_tvf","p0") {
    ...
    try {
        ...
        // test for normal user without privilege
        ...
        def jdbc_url = tokens[0] + "//" + tokens[2] + "/" + "information_schema" + "?"
        connect(nonAdminUser, nonAdminPwd, jdbc_url) {
            try {
                sql """ desc function `brokers`(); """
                assertTrue(false)
            } catch (Exception e) {
                log.info("exception: ${e}")
                assertTrue(e.getMessage().contains("Access denied") && e.getMessage().contains("at least one of")
                    && e.getMessage().contains("ADMIN/OPERATOR") && e.getMessage().contains("for this operation"))
            }
            try {
