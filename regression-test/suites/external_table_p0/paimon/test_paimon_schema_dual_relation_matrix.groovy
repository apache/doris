...
suite("test_paimon_schema_dual_relation_matrix", "p0,external,paimon") {
    ...
    try {
        ...
        // Scenario TC07-join: each historical relation binds and scans its OWN snapshot schema, so a
        // self-join across the rename resolves old_name (old snapshot) and new_name (new snapshot).
        ...
        // Scenario TC07-reverse-join: per-relation snapshot binding is independent of relation order.
        assertEquals([[1, "old-1", "old-1"]], sql("""
        ...
        // Scenario TC07-union: top-level historical schemas stay relation-local across a UNION.
        assertEquals([[1, "old-1"], [1, "old-1"], [2, "new-2"]], sql("""
        ...
        // Scenario TC07-nested-union: nested field lookup is also relation-local (the renamed nested
        // field preserves its value, so info.added@old and info.renamed@new read the same data).
        assertEquals([[1, 10], [1, 10], [2, 20]], sql("""
        ...
        // Scenario TC07-CTE: CTE boundaries keep each snapshot's schema independent.
        assertEquals([[1, "old-1", "old-1"]], sql("""
            with old_ref as (
                select id, old_name
                from ${tableName} for version as of ${oldSnapshot}
        ...
        // Scenario TC07-correlated-subquery: the correlated subquery resolves its own snapshot schema.
