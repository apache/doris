    ...
    try {
        ...
        snapshotRes.eachWithIndex { snapshotRow, index ->
            ...
            try {
                ...
                "${baseQueryName}_version_group_by_is_paid" """select is_paid, count(*) from ${tableName} FOR VERSION AS OF ${snapshotId} GROUP BY is_paid;"""
                ...
                "${baseQueryName}_tag_group_by_is_paid" """select is_paid, count(*) from ${tableName} FOR VERSION AS OF '${tagName}' GROUP BY is_paid;"""
                ...
                "${baseQueryName}_time_string_group_by_is_paid" """select is_paid, count(*) from ${tableName} FOR TIME AS OF \"${snapshotTime}\" GROUP BY is_paid;"""
                ...
                "${baseQueryName}_time_millis_group_by_is_paid" """select is_paid, count(*) from ${tableName} FOR TIME AS OF ${timestamp} GROUP BY is_paid;"""
        ...
        branchesResult.eachWithIndex { branchRow, index ->
            ...
            try {
                ...
                "${baseQueryName}_group_by_is_paid_list" """select is_paid, count(*) from ${tableName}@branch(${branchName}) GROUP BY is_paid;"""
                "${baseQueryName}_group_by_is_paid_map" """select is_paid, count(*) from ${tableName}@branch(\"name\"="${branchName}") GROUP BY is_paid;"""
        ...
        tagsResult.eachWithIndex { tagRow, index ->
            ...
            try {
                ...
                "${baseQueryName}_group_by_is_paid_list" """select is_paid, count(*) from ${tableName}@tag(${tagName}) GROUP BY is_paid;"""
                "${baseQueryName}_group_by_is_paid_map" """select is_paid, count(*) from ${tableName}@tag(\"name\"="${tagName}") GROUP BY is_paid;"""
