...
suite("test_hive_write_static_partition", "p0,external,hive,external_docker,external_docker_hive") {
    ...
    def testStaticPartitionWrite = { String catalog_name ->
        ...
        sql """
            INSERT INTO ${tableName} PARTITION(ts_date='2026-07-26') (tag_value, user_id, ts)
            SELECT 'tagD', 'u4', 400;
        """
        sql """ refresh catalog ${catalog_name}; """
        ...
        // 7. Inline VALUES form (no explicit column list): partition column value comes from the
        //    PARTITION clause, so VALUES only supplies the non-partition columns.
        sql """
            INSERT INTO ${tableName} PARTITION(ts_date='2026-07-28')
            VALUES ('tagF', 'u6', 600);
        """
        sql """ refresh catalog ${catalog_name}; """
        def p28 = sql """ select tag_value, user_id, ts, ts_date from ${tableName} where ts_date='2026-07-28'; """
        ...
        // 8. INSERT OVERWRITE with inline VALUES should only replace the targeted partition.
        sql """
            INSERT OVERWRITE TABLE ${tableName} PARTITION(ts_date='2026-07-28')
            VALUES ('tagG', 'u7', 700);
        """
        sql """ refresh catalog ${catalog_name}; """
        def p28ow = sql """ select tag_value, ts, ts_date from ${tableName} where ts_date='2026-07-28'; """
    ...
    def testStaticPartitionErrors = { String catalog_name ->
        ...
        test {
            sql """
                INSERT INTO ${tableName} PARTITION(ts_date='2026-07-24') (tag_value, user_id, ts, ts_date)
        ...
        // 5.6 the same partition column specified twice with different casing is a duplicate.
        //     Hive column names are case-insensitive, so ts_date and TS_DATE are the same column.
        //     This must fail fast instead of silently letting one value overwrite the other.
        test {
            sql """
                INSERT INTO ${tableName} PARTITION(ts_date='2026-07-24', TS_DATE='2026-07-25')
                SELECT 'tagA', 'u1', 100;
            """
