...
suite("test_iceberg_v3_row_lineage_equality_delete_dv_interop", "p0,external,iceberg,external_docker,external_docker_iceberg") {
    ...
    def visibleIds = { tableName ->
        return sql("""
            select new_new_id
            from ${tableName}
            order by new_new_id
        """).collect { row -> row[0].toString().toInteger() }
    }

    def assertVisibleIds = { tableName, expectedIds ->
        def actualIds = visibleIds(tableName)
        ...
        def counts = sql("""
            ...
            from ${tableName}
        """)
        assertEquals(expectedIds.size(), counts[0][0].toString().toInteger())
        assertEquals(expectedIds.size(), counts[0][1].toString().toInteger())
    }

    def assertEqualityDeletedRowsStillHidden = { tableName ->
        def oldVersions = sql("""
            select count(*)
            from ${tableName}
            where (new_new_id = 1 and new_name in ('smith', 'smith2', 'smith3'))
               or (new_new_id = 2 and new_name = 'danny')
               or (new_new_id = 4 and new_name in ('bob', 'bob2'))
               or (new_new_id = 5 and new_name = 'dennis')
        """)
        assertEquals(0, oldVersions[0][0].toString().toInteger())
    }

    def assertLineageProjectionReadable = { tableName, expectedRowCount ->
        def rows = sql("""
    ...
    try {
        ...
        fixtureMetadataFiles.each { format, metadataFile ->
            ...
            try {
                ...
                assertVisibleIds(tableName, [1, 2, 3, 4, 5, 6, 7])
                assertEqualityDeletedRowsStillHidden(tableName)
                assertLineageProjectionReadable(tableName, 7)
                ...
                assertVisibleIds(tableName, [1, 3, 4, 5, 6, 7])
                assertEquals([], sql("""select new_new_id from ${tableName} where new_new_id = 2"""))
                assertEqualityDeletedRowsStillHidden(tableName)
                assertLineageProjectionReadable(tableName, 6)
