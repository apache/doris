...
suite("test_iceberg_runtime_filter_partition_pruning", "p0,external,doris,external_docker,external_docker_doris") {
    ...
    def test_runtime_filter_partition_pruning = {
        ...
        qt_runtime_filter_partition_pruning_binary1 """
            select count(*) from binary_partitioned where cast(partition_key as string) =
                (select cast(partition_key as string) from binary_partitioned
        ...
        qt_runtime_filter_partition_pruning_binary2 """
            select count(*) from binary_partitioned where cast(partition_key as string) in
                (select cast(partition_key as string) from binary_partitioned
                group by cast(partition_key as string) having count(*) > 0
