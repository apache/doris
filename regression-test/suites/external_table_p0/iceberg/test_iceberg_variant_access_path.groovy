...
suite("test_iceberg_variant_access_path",
    ...
    explain {
        sql """SELECT CAST(message['mainDomain'] AS STRING) FROM ${tableName}"""
        contains "all access paths: [message(2).mainDomain]"
    }

    order_qt_variant_leaf """
        SELECT id, CAST(message['mainDomain'] AS STRING), CAST(message['anomalyType'] AS STRING)
        FROM ${tableName}
    """
    order_qt_variant_group_by """
        SELECT CAST(message['mainDomain'] AS STRING), CAST(message['anomalyType'] AS STRING),
               COUNT(*)
        FROM ${tableName}
        GROUP BY CAST(message['mainDomain'] AS STRING), CAST(message['anomalyType'] AS STRING)
    """
    order_qt_variant_filter """
        SELECT id FROM ${tableName} WHERE CAST(message['anomalyType'] AS STRING) = 'dns'
    """
