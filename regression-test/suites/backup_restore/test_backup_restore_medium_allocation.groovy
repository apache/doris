...
suite("test_backup_restore_medium_allocation", "backup_restore") {
    ...
    test("restore_with_adaptive_mode") {
        ...
        def result = sql "SELECT * FROM ${dbName}.${tableName}_adaptive"
        assertEquals(result.size(), values.size())
        ...
        def snapshot = syncer.getSnapshotTimestamp(repoName, "${snapshotName}_adaptive")
        assertTrue(snapshot != null)
    ...
    test("restore_default_mode") {
        ...
        def snapshot = syncer.getSnapshotTimestamp(repoName, "${snapshotName}_default")
        assertTrue(snapshot != null)
        ...
        def result = sql "SELECT * FROM ${dbName}.${tableName}_default"
        assertEquals(result.size(), values.size())
    ...
    test("restore_invalid_mode") {
        ...
        def snapshot = syncer.getSnapshotTimestamp(repoName, "${snapshotName}_invalid")
        assertTrue(snapshot != null)
