...
suite("test_materialized_view_array", "rollup") {
    ...
    try {
        ...
        test {
            sql "CREATE materialized index idx AS select k2,k1, k3, k4, k5 from ${tableName}"
            exception "errCode = 2, detailMessage = The ARRAY column[`mv_k2` array<smallint(6)> NULL] not support to create materialized index"
        }
    } finally {
