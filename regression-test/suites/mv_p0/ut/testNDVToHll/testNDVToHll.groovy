...
suite ("testNDVToHll") {
    ...
    order_qt_select_mv "select user_id, ndv(tag_id) a from user_tags group by user_id order by user_id;"
    ...
    order_qt_select_mv "select user_id, approx_count_distinct(tag_id) a from user_tags group by user_id order by user_id;"
