...
suite ("multi_agg_with_same_slot") {
    ...
    qt_select_mv "select k1,k2,avg(k3),max(k3) from d_table group by grouping sets((k1),(k1,k2),()) order by 1,2;"

    explain {
        sql("select k1,k2,max(k5) from d_table group by grouping sets((k1),(k1,k2),()) order by 1,2;")
        contains "(kmv2)"
    }
    qt_select_mv "select k1,k2,avg(k5),max(k5) from d_table group by grouping sets((k1),(k1,k2),()) order by 1,2;"
}
