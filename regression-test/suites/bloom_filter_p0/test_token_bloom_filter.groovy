...
// under the License.
suite("test_token_bloom_filter") {
    sql "SHOW ALTER TABLE COLUMN"

    def test_tb = "test_token_bloom_filter_tb"
    ...
    sql """CREATE TABLE IF NOT EXISTS ${test_tb} (
            ...
            `a2` string NOT NULL, 
            `a3` text NOT NULL, 
            INDEX idx_tokenbf_a1 (`a1`) USING TOKEN_BF PROPERTIES("bf_size"="512") COMMENT 'a1 token_bf index',
            INDEX idx_tokenbf_a2 (`a2`) USING TOKEN_BF PROPERTIES("bf_size"="512") COMMENT 'a2 token_bf index',
            INDEX idx_tokenbf_a3 (`a3`) USING TOKEN_BF PROPERTIES("bf_size"="512") COMMENT 'a3 token_bf index'
            ) ENGINE=OLAP
            DUPLICATE KEY(`k1`)
