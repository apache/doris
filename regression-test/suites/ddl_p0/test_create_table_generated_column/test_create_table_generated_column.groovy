...
suite("test_generated_column") {
    ...
    sql "sync"
    qt_test_stream_load_common "select * from test_gen_col_common order by 1,2,3"

    streamLoad {
        table 'gencol_refer_gencol'
        set 'column_separator', ','
        file 'gen_col_data.csv'
        set 'columns', 'a,b'
        time 10000 // limit inflight 10s
    ...
    sql "sync"
    qt_test_stream_load_refer_gencol "select * from gencol_refer_gencol order by 1,2,3,4"
    ...
    sql "sync"
    qt_test_stream_load_common_json "select * from test_gen_col_common order by 1,2,3"
    ...
    sql "sync"
    qt_test_stream_load_refer_gencol_json "select * from gencol_refer_gencol order by 1,2,3,4"
    ...
    streamLoad {
        ...
        time 10000
        set 'sql',"""
            insert into ${db}.test_gen_col_common(a,b) select * from http_stream("format"="csv", "column_separator"=",")
        """
    }
    sql "sync"
    qt_http_stream_load_common_ "select * from test_gen_col_common order by 1,2,3"
    streamLoad {
        set 'version', '1'
        file 'gen_col_data.csv'
        time 10000
        set 'sql',"""
            insert into ${db}.gencol_refer_gencol(a,b) select * from http_stream("format"="csv", "column_separator"=",")
        """
    }
    sql "sync"
    qt_http_stream_load_refer_gencol "select * from gencol_refer_gencol order by 1,2,3,4"
    // mysql load
    def filepath = getLoalFilePath "gen_col_data.csv"
    sql """
        LOAD DATA LOCAL
        INFILE '${filepath}'
        INTO TABLE test_gen_col_common
        COLUMNS TERMINATED BY ','
        (a,b); """
    sql """
        LOAD DATA LOCAL
        INFILE '${filepath}'
        INTO TABLE gencol_refer_gencol
        COLUMNS TERMINATED BY ','
        (a,b);
    """
    sql "sync"
    qt_test_mysql_load_common "select * from test_gen_col_common order by 1,2,3"
    qt_test_mysql_load_refer_gencol "select * from gencol_refer_gencol order by 1,2,3,4"

    // specify value for generated column
    sql "truncate table gencol_refer_gencol"
    ...
    """
    sql "sync"
    qt_specify_value_for_gencol "select * from test_gen_col_common order by 1,2,3"
