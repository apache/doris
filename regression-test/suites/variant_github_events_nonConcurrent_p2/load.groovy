...
suite("regression_test_variant_github_events_p2", "nonConcurrent,p2"){
    setFeConfigTemporary([enable_variant_v2: true]) {
        ...
        // Pin the physical representation: this suite validates V2 shredded reads, schema changes,
        // indexes, and compaction rather than randomly alternating between root-only and S layouts.
        int max_subcolumns_count = 2048
        ...
        sql """
         CREATE TABLE IF NOT EXISTS github_events2 (
                k bigint,
                v variant<properties("variant_max_subcolumns_count" = "${max_subcolumns_count}")> not null
            )
            UNIQUE KEY(`k`)
            DISTRIBUTED BY HASH(k) BUCKETS 4
        ...
        sql """ALTER TABLE github_events2 SET("bloom_filter_columns" = "k,v")"""
