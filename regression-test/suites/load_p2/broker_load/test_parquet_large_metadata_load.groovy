...
suite("test_parquet_large_metadata_load_p2", "p2") {
    ...
    def paths = ["s3://${s3BucketName}/regression/load/metadata/parquet_large_metadata_100mb.parquet"
    ]
    String ak = getS3AK()
    String sk = getS3SK()
    ...
    String[][] tvf_result = sql """select `1`,`2` from s3(
                                     "uri" = "https://${s3BucketName}.${s3Endpoint}/regression/load/metadata/parquet_large_metadata_100mb.parquet",
                                     "s3.access_key" = "$ak",
                                     "s3.secret_key" = "$sk",
                                     "s3.region" = "${s3Region}",
                                     "format" = "parquet"
