...
suite("test_group_concat") {
    ...
    qt_select """
                SELECT group_concat(k6) FROM test where k6='false'
              """

    qt_select """
                SELECT group_concat(DISTINCT k6) FROM test where k6='false'
              """

    qt_select """
                SELECT abs(k3), group_concat(cast(abs(k2) as varchar) order by abs(k2), k1) FROM baseall group by abs(k3) order by abs(k3)
              """
    ...
    test {
        sql"""SELECT abs(k3), group_concat(distinct cast(abs(k2) as char) order by abs(k1), k2) FROM baseall group by abs(k3) order by abs(k3);"""
    ...
    qt_select """
                SELECT count(distinct k7), group_concat(k6 order by k6) FROM baseall;
              """
