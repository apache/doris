...
suite("test_asof_join_expr", "nereids_p0") {
    ...
    // ==================== Test 21: Expression with NULL values ====================
    sql "DROP TABLE IF EXISTS asof_expr_null_left"
    sql "DROP TABLE IF EXISTS asof_expr_null_right"

    sql """
        CREATE TABLE asof_expr_null_left (
            id INT,
            grp INT,
            ts DATETIME
        ) DISTRIBUTED BY HASH(id) BUCKETS 1
    ...
    sql """
        CREATE TABLE asof_expr_null_right (
            id INT,
            grp INT,
            ts DATETIME,
            data VARCHAR(20)
    ...
    sql """
        INSERT INTO asof_expr_null_left VALUES
        (1, 1, '2024-01-01 10:00:00'),
        (2, 1, NULL),
        (3, 1, '2024-01-01 12:00:00')
    """
    ...
    sql """
        INSERT INTO asof_expr_null_right VALUES
        (1, 1, '2024-01-01 08:00:00', 'r1'),
        (2, 1, NULL, 'r_null'),
        (3, 1, '2024-01-01 09:00:00', 'r3')
    """

    // NULL in expression should not match
    qt_expr_null_probe """
        SELECT l.id, l.ts, r.id as rid, r.ts as rts, r.data
        FROM asof_expr_null_left l
        ASOF LEFT JOIN asof_expr_null_right r
        MATCH_CONDITION(l.ts >= DATE_ADD(r.ts, INTERVAL 1 HOUR))
        ON l.grp = r.grp
        ORDER BY l.id
    """
