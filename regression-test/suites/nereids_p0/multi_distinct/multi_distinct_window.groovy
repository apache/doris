...
suite('multi_distinct_window') {
    ...
    qt_count "select id, v1, count(distinct v1) over() from multi order by id;"
    ...
    // select id, v1, multi_distinct_group_concat(v2) over() from multi;

    // select  group_concat(v2) from multi;
