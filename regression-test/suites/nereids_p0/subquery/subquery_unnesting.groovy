...
suite ("subquery_unnesting") {
    ...
    test {
        sql """select * from t1 where exists(select distinct k1 from t2 where t1.k1 > t2.k3)"""
        exception "Unsupported correlated NO-EQ subquery conjuncts with a group by clause";
    }
}
