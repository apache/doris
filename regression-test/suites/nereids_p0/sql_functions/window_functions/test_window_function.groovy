...
suite("test_window_function") {
    ...
    qt_sql """
           select /*+SET_VAR(parallel_pipeline_task_num=1) */ t1.new_time from (select closing_date, lead(closing_date, 1, '2014-10-02 00:00:00') over () as new_time from ${windowFunctionTable1}) as t1 left join ${windowFunctionTable1} t2 on t2.closing_date = t1.closing_date order by t1.new_time desc;
           """
    ...
    // LEAD not nullable coredump
    qt_sql """  
           select /*+SET_VAR(parallel_pipeline_task_num=1) */ t1.new_time from (select closing_date1, lead(closing_date1, 1, '2014-10-02 00:00:00') over () as new_time from ${windowFunctionTable1}) as t1 left join ${windowFunctionTable1} t2 on t2.closing_date1 = t1.closing_date1 order by t1.new_time desc;
           """
    ...
    // LEAD not nullable coredump
    qt_sql """
           select /*+SET_VAR(parallel_pipeline_task_num=1) */ t1.new_time from (select closing_date2, lead(closing_date2, 1, '2014-10-02 00:00:00') over () as new_time from ${windowFunctionTable1}) as t1 left join ${windowFunctionTable1} t2 on t2.closing_date2 = t1.closing_date2 order by t1.new_time desc;
           """
    ...
    // LEAD not nullable coredump
    qt_sql """
           select /*+SET_VAR(parallel_pipeline_task_num=1) */ t1.new_time from (select closing_date3, lead(closing_date3, 1, '2014-10-02 00:00:00') over () as new_time from ${windowFunctionTable1}) as t1 left join ${windowFunctionTable1} t2 on t2.closing_date3 = t1.closing_date3 order by t1.new_time desc;
           """
    ...
    // RANK
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ x, y, rank() over(partition by x order by y) as rank from ${windowFunctionTable3} order by x, y; """
    // DENSE_RANK
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ x, y, dense_rank() over(partition by x order by y) as rank from ${windowFunctionTable3} order by x, y; """
    // PERCENT_RANK
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ x, y, percent_rank() over(partition by x order by y) as rank from ${windowFunctionTable3} order by x, y; """
    // CUME_DIST
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ x, y, cume_dist() over(partition by x order by y) as rank from ${windowFunctionTable3} order by x, y; """
    // ROW_NUMBER
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ x, y, row_number() over(partition by x order by y) as rank from ${windowFunctionTable3} order by x, y; """
    ...
    // first_value
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ country, name,first_value(greeting) over (partition by country order by name, greeting) as greeting from ${windowFunctionTable4} order by country, name; """
    // last_value
    qt_sql """ select /*+SET_VAR(parallel_pipeline_task_num=1) */ country, name,last_value(greeting)  over (partition by country order by name, greeting) as greeting from ${windowFunctionTable4} order by country, name; """
    ...
    String k2 = fields[5]
    String k3 = fields[3]
    qt_first_value1"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, first_value(${k2}) over (partition by ${k1} order by ${k3}, ${k2})
             as wj from baseall  order by ${k1}, wj"""
    qt_first_value2"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, first_value(${k2}) over (partition by ${k1} order by ${k3}, ${k2}
             range between unbounded preceding and current row)
             as wj from baseall  order by ${k1}, wj"""
    qt_first_value3"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, first_value(${k2}) over (partition by ${k1} order by ${k3}, ${k2}
             rows between unbounded preceding and current row)
             as wj from baseall  order by ${k1}, wj"""
    qt_first_value4"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ a, min(d) as wjj from
    ...
    qt_last_value1"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, last_value(${k2}) over (partition by ${k1} order by ${k3},${k2})
                 as wj from baseall  order by ${k1}, wj"""
    qt_last_value2"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, last_value(${k2}) over (partition by ${k1} order by ${k3},${k2}
             range between unbounded preceding and current row)
             as wj from baseall  order by ${k1}, wj"""
    qt_last_value3"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, last_value(${k2}) over (partition by ${k1} order by ${k3},${k2}
             rows between unbounded preceding and current row)
             as wj from baseall  order by ${k1}, wj"""
    qt_last_value4"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ a, max(d) as wjj from
             (select t1.k1 as k1, t1.k2 as k2, t1.k3 as k3,
             t1.k4 as k4, t1.k5 as k5,t1.k6 as k6,
             t1.k7 as k7, t1.k8 as k8, t1.k9 as k9,
             t1.k10 as k10, t1.k11 as k11, 
    ...
    qt_row_number1"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, row_number() over (partition by ${k1} order by ${k3})
                    as wj from baseall order by ${k1}, wj"""
    qt_row_number2"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, count(k1) over (partition by ${k1} order by ${k3}
                    rows between unbounded preceding and current row)
                    as wj from baseall order by ${k1}, wj"""

    qt_lag_1 "select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, lag(${k2}) over (partition by ${k1} order by ${k3}) from baseall order by 1,2;"
    ...
    qt_lag_2 "select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, lag(${k2}, 1) over (partition by ${k1} order by ${k3}) from baseall order by 1,2;"
    
    qt_lead_1 "select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, lead(${k2}) over (partition by ${k1} order by ${k3}) from baseall order by 1,2;"
    ...
    qt_lead_2 "select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, lead(${k2}, 1) over (partition by ${k1} order by ${k3}) from baseall order by 1,2;"
    ...
    test {
        sql"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, sum(${k2}) over (partition by ${k1} order by ${k3} rows
    ...
    test {
        sql"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, rank(${k2}) over (partition by ${k1} order by ${k3}) as wj
            from baseall order by ${k1}, wj"""
        exception ""
    }
    test {
        sql"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, max() over (partition by ${k1} order by ${k3}) as wj
            from baseall order by ${k1}, wj"""
        exception ""
    }
    test {
        sql"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, count(${k2}) over (order by ${k1} rows partition by ${k3}) as wj
            from baseall order by ${k1}, wj"""
        exception ""
    }
    ...
    // test_query_rank
    k3 = fields[7]
    qt_rank1"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, rank() over (partition by ${k1} order by ${k3}) as wj
             from baseall order by ${k1}, wj"""
    qt_rank2"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ F2.${k1}, (F1.wj - F2.basewj + 1) as wj from
    ...
    //test_hang
    qt_window_hang1"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, row_number() over (partition by ${k1} order by ${k3}) as wj from
             baseall order by ${k1}, wj"""
    String line = "("
    String cur
    for (p in range(0, 829)) {
        if (p == 0) {
            cur = "(select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, 1 as wj from baseall order by ${k1}, ${k3} limit 1)".toString()
        }
        else {
            cur = """(select /*+SET_VAR(parallel_pipeline_task_num=1) */ ${k1}, ${p+1} as wj from baseall order by ${k1} , ${k3}
                    limit ${p}, 1 ) """.toString()
    ...
    //qt_window_hang2"""select /*+SET_VAR(parallel_pipeline_task_num=1) */ A.${k1}, A.wj - B.dyk + 1 as num from
    ...
    for (p in range(0, 829)) {
        if (p == 0 ) {
            cur = "(select /*+SET_VAR(parallel_pipeline_task_num=1) */ * from baseall order by k1, k6 limit 1)"
        } else {
            cur = "(select /*+SET_VAR(parallel_pipeline_task_num=1) */ * from baseall order by k1, k6 limit ${p}, 1)"
        }
        if (p < 828) {
            line = line + cur + " union all "
        } else {
    ...
    qt_hujie2"select /*+SET_VAR(parallel_pipeline_task_num=1) */ k1, k6 from baseall order by k1, k6"

    // test_bug
    order_qt_window_bug1"""SELECT /*+SET_VAR(parallel_pipeline_task_num=1) */ wj FROM (SELECT row_number() over (PARTITION BY k6 ORDER BY k1) AS wj
            FROM baseall ) AS A where wj = 2"""
    order_qt_window_bug2"""SELECT /*+SET_VAR(parallel_pipeline_task_num=1) */ A.k2 AS a, A.k1 as b, B.k1 as c, B.k2 as d FROM
