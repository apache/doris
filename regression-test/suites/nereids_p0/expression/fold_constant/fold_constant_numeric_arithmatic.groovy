...
//Exception: Ln of negative and zero values (undefined)
    ...
    explain {
        sql "SELECT LN(-1) AS ln_case_exception_1"
        contains("NaN")
    }
    explain {
        sql "SELECT LN(0) AS ln_case_exception_2"
        contains("-")
    }
...
//Exception: Log10 of negative and zero values (undefined)
    sql "set debug_skip_fold_constant=false;"
    explain {
        sql "SELECT LOG10(-100) AS log10_case_exception_1" //undefined (returns NULL or error)
        contains("NaN")
    }
    explain {
        sql "SELECT LOG10(0) AS log10_case_exception_2" //undefined (returns NULL or error)
        contains("-")
    }
...
//Exception: Acos out of range (-1 to 1)
    sql "set debug_skip_fold_constant=false;"
    explain {
        sql "SELECT ACOS(2) AS acos_case_exception"
        contains("NaN")
    }
...
//Exception: Asin out of range (-1 to 1)
    sql "set debug_skip_fold_constant=false;"
    explain {
        sql "SELECT ASIN(2) AS acos_case_exception"
        contains("NaN")
    }
