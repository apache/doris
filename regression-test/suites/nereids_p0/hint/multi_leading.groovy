...
suite("multi_leading") {
    ...
    qt_sql5_2 """with  cte as (select c11, c1 from t1)  SELECT count(c1) FROM cte group by c1 having sum(cte.c11) > (select /*+ leading(cte t1) */ 0.05 * avg(t1.c11) from t1 join cte on t1.c1 = cte.c11 ) order by c1 limit 5"""
}
