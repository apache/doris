...
//// test cte
    ...
    qt_select19 """explain shape plan with cte as (select * from t1 join t2 on c1 = c2) select /*+ leading(cte t3) */ * from cte, t3;"""
    qt_select20 """explain shape plan with cte as (select /*+ leading(t2 t1) */ * from t1 join t2 on c1 = c2) select * from cte, t3;"""
    qt_select21 """explain shape plan with cte as (select /*+ leading(t2 t1) */ * from t1 join t2 on c1 = c2) select /*+ leading(cte t3) */ * from cte, t3;"""
