...
suite('nereids_insert_with_hint') {
    ...
    def tableName5 = "nereids_insert_with_hint7"
    ...
    "enable_unique_key_merge_on_write"="true",
    "disable_auto_compaction"="true"); """
    sql "insert into ${tableName5} values(1,1,3,4),(2,2,4,5),(3,3,2,3),(4,4,1,2);"
    qt_5 "select * from ${tableName5} order by k;"
    ...
    def sql_stmt = "insert /*+ SET_VAR(enable_memtable_on_sink_node=true) */into ${tableName5}(k,v) select v2,v3 from ${tableName5};"
    sql sql_stmt
    def profileDetail = getProfile(sql_stmt)
    assertTrue(profileDetail.contains("DeltaWriterV2"))

    sql_stmt = "insert /*+ SET_VAR(enable_memtable_on_sink_node=false) */into ${tableName5}(k,v) select v2,v3 from ${tableName5};"
    sql sql_stmt
