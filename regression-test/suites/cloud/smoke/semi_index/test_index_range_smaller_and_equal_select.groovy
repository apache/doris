suite("smoke_test_index_smaller", "smoke"){
    if (context.config.cloudVersion != null && !context.config.cloudVersion.isEmpty()
        ...
        log.info("case: smoke_test_index_smaller, cloud version ${context.config.cloudVersion} bigger than 3.0.0, skip".toString());
        return
    }
    def indexTbName1 = "index_range_smaller_and_equal_select"
    ...
    qt_sql """
        select * from ${indexTbName1} where age<=10 and grade<='grade 5' and fatherName<= 'zhang yi' or studentInfo<="tall:120cm, weight: 35kg,"
        order by name
        """
    qt_sql """
        select * from ${indexTbName1} where selfComment<='i like' or grade<='grade 5' and fatherName<= 'zhang yi' or studentInfo<="tall:120cm, weight: 35kg,"
        order by name
        """
