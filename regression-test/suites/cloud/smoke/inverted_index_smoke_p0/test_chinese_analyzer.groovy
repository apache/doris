...
suite("smoke_test_chinese_analyzer", "smoke"){
    if (context.config.cloudVersion != null && !context.config.cloudVersion.isEmpty()
        ...
        log.info("case: smoke_test_chinese_analyzer, cloud version ${context.config.cloudVersion} less than 3.0.0, skip".toString());
        return
    }
    // prepare test table
