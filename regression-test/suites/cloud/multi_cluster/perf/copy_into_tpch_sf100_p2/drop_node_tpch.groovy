...
suite("drop_node_tpch") {
    ...
    for  (int i = 1; i < 23; ++i) {
        ...
        connect(user = 'admin', password = 'selectdb2022@', url = 'jdbc:mysql://192.144.213.234:18929/?useLocalSessionState=true') {
            ...
            sql """ insert into node_change_perf values (1, 6, 0, 'drop node tpch query', "q${i}", 1, ${latency}, "${formattedDate}", "${formattedTime}", 'luwei', 'cloud', '2.3.0') """;
        }
    }
    ...
    for  (int i = 1; i < 23; ++i) {
        ...
        connect(user = 'admin', password = 'selectdb2022@', url = 'jdbc:mysql://192.144.213.234:18929/?useLocalSessionState=true') {
            ...
            sql """ use performance_test_result """
            sql """ insert into node_change_perf values (1, 6, 1, 'drop node tpch query', "q${i}", 1, ${latency}, "${formattedDate}", "${formattedTime}", 'luwei', 'cloud', '2.3.0') """;
        }
    }
}
