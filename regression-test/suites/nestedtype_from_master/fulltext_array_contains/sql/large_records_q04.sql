...
SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ COUNT(*) FROM large_records_t2_dk_array;

SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ FTS_DOC_ID FROM large_records_t2_dk_array WHERE array_contains(b, 'row30col2word30');

SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ FTS_DOC_ID FROM large_records_t2_dk_array WHERE a MATCH_ANY 'row35col2word49' OR array_contains(b, 'row35col2word49');
