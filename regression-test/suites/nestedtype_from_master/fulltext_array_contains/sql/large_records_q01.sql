...
SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ COUNT(*) FROM large_records_t1_uk_array;

SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ FTS_DOC_ID FROM large_records_t1_uk_array WHERE a MATCH_ANY 'row35col2word49' OR array_contains(b, 'row35col2word49') order by FTS_DOC_ID;
...
SELECT/*+SET_VAR(enable_common_expr_pushdown=true)*/ a,b FROM large_records_t1_uk_array
        WHERE (a MATCH_ANY 'row5col2word49' OR array_contains(b, 'row5col2word49'))
