...
suite("test_nestedtypes_json_insert_into_with_s3", "p0") {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        qt_sql_arr_json_without_quote_doris """ select * from ${table_names[i]} order by k1 limit 1; """
    ...
    qt_sql_arr """ select array_size(c_tinyint) from tbl_json_array_nested_types_s3 order by k1; """
    qt_sql_arr """ select array_size(c_tinyint) from tbl_json_array_nested_types_s32 order by k1; """
    qt_sql_arr """ select array_size(c_tinyint) from tbl_json_array_map_types_s3 order by k1; """
    qt_sql_map_arr """ select array_size(map_values(c_tinyint)) from tbl_json_map_array_types_s3 order by k1; """
