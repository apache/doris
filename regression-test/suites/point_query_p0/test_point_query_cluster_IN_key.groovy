...
suite("test_point_query_cluster_IN_key") {
    ...
    try {
        ...
        def tableName = realDb + ".tbl_point_query_cluster_IN_key"
        ...
        for (int i = 0; i < 2; i++) {
            tableName = realDb + ".tbl_point_query_cluster_IN_key" + i
            sql """DROP TABLE IF EXISTS ${tableName}"""
            if (i == 0) {
            ...
            def result1 = connect(user=user, password=password, url=prepare_url) {
                ...
                stmt.setInt(3, 251)
                stmt.setInt(4, 252)
                stmt.setBigDecimal(5, new BigDecimal("119291.11"))
                stmt.setBigDecimal(6, new BigDecimal("120939.11130"))
                stmt.setBigDecimal(7, new BigDecimal("12222.99121135"))
                stmt.setString(8, "ddd")
                ...
                stmt_fn.setBigDecimal(3, new BigDecimal("119291.11"))
            ...
            def result2 = connect(user=user, password=password, url=context.config.jdbcUrl) {
                ...
                qt_sql """
                        SELECT 
                            /*+ SET_VAR(enable_nereids_planner=true) */ hex(k3), hex(k4), k7 + 10.1 
                        FROM 
                            ${tableName} 
                        WHERE 
                            k1 = 1237 AND k2 = 120939.11130 AND k3 = 'a    ddd'
                        """
                tableName = "test_query_cluster_IN_key"
                sql """DROP TABLE IF EXISTS ${tableName}"""
