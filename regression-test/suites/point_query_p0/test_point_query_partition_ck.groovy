...
suite("test_point_query_partition_ck") {
    ...
    def tableName = realDb + ".tbl_point_query_partition_ck"
    ...
    tableName = realDb + ".customer_ck"
    sql "DROP TABLE IF EXISTS ${tableName}";
    sql """
        CREATE TABLE ${tableName} (
          `customer_key` BIGINT NULL,
          `customer_value_0` TEXT NULL,
          `customer_value_1` varchar(100) NULL,
          `customer_value_2` varchar(100) NULL,
    ...
    """  
    sql """insert into ${tableName}(customer_key, customer_value_0, customer_value_1) values(686612, "686612", "686612")"""
    sql """insert into ${tableName}(customer_key, customer_value_0, customer_value_1) values(686613, "686613", "686613")"""
    def result3 = connectToDoris(user, password, prepare_url) {
        def stmt = prepareStatement "select /*+ SET_VAR(enable_nereids_planner=true) */ * from ${tableName} where customer_key = ?"
