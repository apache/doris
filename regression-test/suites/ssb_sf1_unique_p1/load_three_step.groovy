...
suite("load_three_step") {
    ...
    tables.each { tableName, rows ->
        ...
        for (j in 0..<2) {
            ...
            sql "sync"
            def tableVersion1 = sql "admin show replica status from ${tableName}"
            logger.info(" ${tableName} version: ${tableVersion1[0][3]}".toString())
            for (int i = 1; i <= 5; i++) {
        ...
        sql "sync"
        def tableVersion2 = sql "admin show replica status from ${tableName}"
        logger.info(" ${tableName} version: ${tableVersion2[0][3]}".toString())
        for (int i = 1; i <= 5; i++) {
            def loadRowCount = sql "select count(1) from ${tableName}"
            logger.info("select ${tableName} numbers: ${loadRowCount[0][0]}".toString())
