...
suite("nereids_gen_fn") {
	...
	qt_sql_explode_json_array_json_Varchar '''
		select id, e from fn_test lateral view explode_json_array_json('[{"id":1,"name":"John"},{"id":2,"name":"Mary"},{"id":3,"name":"Bob"}]') lv as e order by id, json_extract_int(e, '\$.id')'''
	...
	order_qt_sql_explode_TimestampNs "select id, e from fn_test lateral view explode(katsns) lv as e order by id, e"
	...
	order_qt_sql_explode_TimestampNs_notnull "select id, e from fn_test_not_nullable lateral view explode(katsns) lv as e order by id, e"
