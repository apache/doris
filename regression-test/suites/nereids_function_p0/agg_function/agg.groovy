...
suite("nereids_agg_fn") {
	...
	sql '''
		select histogram(ktsns) from fn_test group by kbool order by kbool'''
	sql '''
		select histogram(ktsns) from fn_test'''
	sql '''
		select count(id), histogram(ktsns) from fn_test group by id order by id'''
	sql '''
		select count(distinct id), histogram(ktsns) from fn_test'''
	sql '''
		select count(distinct id, kint), histogram(ktsns) from fn_test group by kbool order by kbool'''
	sql '''
		select count(distinct id), histogram(ktsns) from fn_test'''
	sql '''
		select histogram(ktsns) from fn_test_not_nullable group by kbool order by kbool'''
	sql '''
		select histogram(ktsns) from fn_test_not_nullable'''
	sql '''
		select count(id), histogram(ktsns) from fn_test_not_nullable group by id order by id'''
	sql '''
		select count(distinct id), histogram(ktsns) from fn_test_not_nullable'''
	sql '''
		select count(distinct id, kint), histogram(ktsns) from fn_test group by kbool order by kbool'''
	sql '''
		select count(distinct id), histogram(ktsns) from fn_test'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_gb '''
		select sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test group by kbool order by kbool'''
	qt_sql_sequence_count_String_TimestampNs_Boolean '''
		select sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_1 '''
		select count(id), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test group by id order by id'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_2 '''
		select count(distinct id), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_3 '''
		select count(distinct id, kint), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test group by kbool order by kbool'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_4 '''
		select count(distinct id), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_gb_notnull '''
		select sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test_not_nullable group by kbool order by kbool'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_notnull '''
		select sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test_not_nullable'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_1_notnull '''
		select count(id), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test_not_nullable group by id order by id'''
	qt_sql_sequence_count_String_TimestampNs_Boolean_agg_phase_2_notnull '''
		select count(distinct id), sequence_count('(?1)(?2)', ktsns, kint = 1, kint = 5) from fn_test_not_nullable'''
	...
	qt_sql_sequence_match_String_TimestampNs_Boolean_gb '''
		select sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test group by kbool order by kbool'''
	qt_sql_sequence_match_String_TimestampNs_Boolean '''
		select sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_1 '''
		select count(id), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test group by id order by id'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_2 '''
		select count(distinct id), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_3 '''
		select count(distinct id, kint), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test group by kbool order by kbool'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_4 '''
		select count(distinct id), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_gb_notnull '''
		select sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test_not_nullable group by kbool order by kbool'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_notnull '''
		select sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test_not_nullable'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_1_notnull '''
		select count(id), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test_not_nullable group by id order by id'''
	qt_sql_sequence_match_String_TimestampNs_Boolean_agg_phase_2_notnull '''
		select count(distinct id), sequence_match('(?1)(?2)', ktsns, kint = 1, kint = 2) from fn_test_not_nullable'''
	...
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_gb '''
		select window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test group by kbool order by kbool'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean '''
		select window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_agg_phase_1 '''
		select count(id), window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test group by id order by id'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_agg_phase_2 '''
		select count(distinct id), window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_agg_phase_3 '''
		select count(distinct id, kint), window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test group by kbool order by kbool'''
	...
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_gb_notnull '''
		select window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test_not_nullable group by kbool order by kbool'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_notnull '''
		select window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test_not_nullable'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_agg_phase_1_notnull '''
		select count(id), window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test_not_nullable group by id order by id'''
	qt_sql_window_funnel_BigInt_String_TimestampNs_Boolean_agg_phase_2_notnull '''
		select count(distinct id), window_funnel(3600 * 3, 'default', ktsns, kint = 1, kint = 2) from fn_test_not_nullable'''
