...
suite("nereids_agg_fn") {
	...
	qt_sql_hll_union_Hll_gb "select hll_union(hll_raw_agg(kint)) from fn_test"
	...
	qt_sql_hll_union_Hll_gb_notnull_gb_notnull "select hll_union(hll_raw_agg(kint)) from fn_test_not_nullable"

	qt_sql_hll_union_agg_Hll_gb "select hll_union_agg(hll_raw_agg(kint)) from fn_test group by kint order by kint"
	qt_sql_hll_union_agg_Hll_gb "select hll_union_agg(hll_raw_agg(kint)) from fn_test"
	...
	qt_sql_hll_union_agg_Hll_gb_notnull_gb_notnull "select hll_union_agg(hll_raw_agg(kint)) from fn_test_not_nullable"
	...
	qt_sql_quantile_union_QuantileState_gb "select quantile_union(to_quantile_state(kvchrs1, 2048)) from fn_test"
	...
	qt_sql_quantile_union_QuantileState_gb_notnull_gb_notnull "select quantile_union(to_quantile_state(kvchrs1, 2048)) from fn_test_not_nullable"
