...
suite("nereids_agg1_fn") {
	...
	qt_sql_bitmap_union_int_BigInt_agg_phase_1 '''
		select count(id), bitmap_union_int(kbint) from fn_test group by id order by id'''
	...
	qt_sql_bitmap_union_int_BigInt_agg_phase_3 '''
		select /*+SET_VAR(disable_nereids_rules='THREE_PHASE_AGGREGATE_WITH_DISTINCT, TWO_PHASE_AGGREGATE_WITH_DISTINCT')*/ count(distinct id, kint), bitmap_union_int(kbint) from fn_test group by kbool order by kbool'''
	...
	qt_sql_bitmap_union_int_BigInt_agg_phase_1_notnull '''
		select count(id), bitmap_union_int(kbint) from fn_test_not_nullable group by id order by id'''
