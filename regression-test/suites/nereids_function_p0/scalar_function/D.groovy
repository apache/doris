...
suite("nereids_scalar_fn_D") {
	...
	qt_sql_date_TimestampNs "select date(ktsns) from fn_test order by ktsns"
	qt_sql_date_DateTimeV2_notnull "select date(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_date_TimestampNs_notnull "select date(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_date_format_TimestampNs_Varchar "select date_format(ktsns, '2006-01-02 12:00:00') from fn_test order by ktsns"
	...
	qt_sql_date_format_TimestampNs_Varchar_notnull "select date_format(ktsns, '2006-01-02 12:00:00') from fn_test_not_nullable order by ktsns"
	...
	qt_sql_date_trunc_TimestampNs_Varchar "select date_trunc(ktsns, 'day') from fn_test where id not in (0, 5) order by ktsns, kvchrs1"
	...
	qt_sql_date_trunc_TimestampNs_Varchar_notnull "select date_trunc(ktsns, 'hour') from fn_test_not_nullable where id not in (0, 5) order by ktsns, kvchrs1"
	...
	qt_sql_datediff_TimestampNs_TimestampNs "select datediff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_datediff_TimestampNs_TimestampNs_notnull "select datediff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_datediff_DateTimeV2_DateV2 "select datediff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_datediff_TimestampNs_DateV2 "select datediff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_datediff_TimestampNs_DateV2_notnull "select datediff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	qt_sql_datediff_DateV2_DateTimeV2 "select datediff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_datediff_DateV2_TimestampNs "select datediff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_datediff_DateV2_TimestampNs_notnull "select datediff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	...
	qt_sql_datediff_TimestampNs_DateTime "select datediff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_datediff_TimestampNs_DateTime_notnull "select datediff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	...
	qt_sql_datev2_TimestampNs "select datev2(ktsns) from fn_test order by ktsns"
	qt_sql_datev2_DateTimeV2_notnull "select datev2(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_datev2_TimestampNs_notnull "select datev2(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_day_ceil_TimestampNs "select day_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_day_ceil_DateTimeV2_notnull "select day_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_day_ceil_TimestampNs_notnull "select day_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_day_ceil_TimestampNs_TimestampNs "select day_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_day_ceil_TimestampNs_TimestampNs_notnull "select day_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_day_ceil_DateTimeV2_Integer "select day_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_day_ceil_TimestampNs_Integer "select day_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_day_ceil_TimestampNs_Integer_notnull "select day_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_day_ceil_TimestampNs_Integer_TimestampNs "select day_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_day_ceil_TimestampNs_Integer_TimestampNs_notnull "select day_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_day_floor_TimestampNs "select day_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_day_floor_DateTimeV2_notnull "select day_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_day_floor_TimestampNs_notnull "select day_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_day_floor_TimestampNs_TimestampNs "select day_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_day_floor_TimestampNs_TimestampNs_notnull "select day_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_day_floor_DateTimeV2_Integer "select day_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_day_floor_TimestampNs_Integer "select day_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_day_floor_TimestampNs_Integer_notnull "select day_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_day_floor_TimestampNs_Integer_TimestampNs "select day_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_day_floor_TimestampNs_Integer_TimestampNs_notnull "select day_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_dayname_TimestampNs "select dayname(ktsns) from fn_test order by ktsns"
	qt_sql_dayname_DateTimeV2_notnull "select dayname(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_dayname_TimestampNs_notnull "select dayname(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_dayofmonth_TimestampNs "select dayofmonth(ktsns) from fn_test order by ktsns"
	qt_sql_dayofmonth_DateTimeV2_notnull "select dayofmonth(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_dayofmonth_TimestampNs_notnull "select dayofmonth(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_dayofweek_TimestampNs "select dayofweek(ktsns) from fn_test order by ktsns"
	qt_sql_dayofweek_DateTimeV2_notnull "select dayofweek(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_dayofweek_TimestampNs_notnull "select dayofweek(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_dayofyear_TimestampNs "select dayofyear(ktsns) from fn_test order by ktsns"
	qt_sql_dayofyear_DateTimeV2_notnull "select dayofyear(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_dayofyear_TimestampNs_notnull "select dayofyear(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_days_add_TimestampNs_Integer "select days_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_days_add_TimestampNs_Integer_notnull "select days_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_days_diff_TimestampNs_TimestampNs "select days_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_days_diff_TimestampNs_TimestampNs_notnull "select days_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_days_diff_DateV2_DateTimeV2 "select days_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_days_diff_DateV2_TimestampNs "select days_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_days_diff_DateV2_TimestampNs_notnull "select days_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_days_diff_DateTimeV2_DateV2 "select days_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_days_diff_TimestampNs_DateV2 "select days_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_days_diff_TimestampNs_DateV2_notnull "select days_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_days_diff_TimestampNs_DateTime "select days_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_days_diff_TimestampNs_DateTime_notnull "select days_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_days_diff_DateTime_DateTimeV2 "select days_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_days_diff_DateTime_TimestampNs "select days_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_days_diff_DateTime_TimestampNs_notnull "select days_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
	qt_sql_days_sub_DateTimeV2_Integer "select days_sub(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_days_sub_TimestampNs_Integer "select days_sub(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_days_sub_TimestampNs_Integer_notnull "select days_sub(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
