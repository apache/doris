...
suite("nereids_scalar_fn_S") {
	...
	qt_sql_second_TimestampNs "select second(ktsns) from fn_test order by ktsns"
	qt_sql_second_DateTimeV2_notnull "select second(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_second_TimestampNs_notnull "select second(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_second_ceil_TimestampNs "select second_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_second_ceil_DateTimeV2_notnull "select second_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_second_ceil_TimestampNs_notnull "select second_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_second_ceil_TimestampNs_TimestampNs "select second_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_second_ceil_TimestampNs_TimestampNs_notnull "select second_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_second_ceil_DateTimeV2_Integer "select second_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_second_ceil_TimestampNs_Integer "select second_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_second_ceil_TimestampNs_Integer_notnull "select second_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_second_ceil_TimestampNs_Integer_TimestampNs "select second_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_second_ceil_TimestampNs_Integer_TimestampNs_notnull "select second_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_second_floor_TimestampNs "select second_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_second_floor_DateTimeV2_notnull "select second_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_second_floor_TimestampNs_notnull "select second_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_second_floor_TimestampNs_TimestampNs "select second_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_second_floor_TimestampNs_TimestampNs_notnull "select second_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_second_floor_DateTimeV2_Integer "select second_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_second_floor_TimestampNs_Integer "select second_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_second_floor_TimestampNs_Integer_notnull "select second_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_second_floor_TimestampNs_Integer_TimestampNs "select second_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_second_floor_TimestampNs_Integer_TimestampNs_notnull "select second_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_seconds_add_TimestampNs_Integer "select seconds_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_seconds_add_TimestampNs_Integer_notnull "select seconds_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_seconds_diff_TimestampNs_TimestampNs "select seconds_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_seconds_diff_TimestampNs_TimestampNs_notnull "select seconds_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_seconds_diff_DateV2_DateTimeV2 "select seconds_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_seconds_diff_DateV2_TimestampNs "select seconds_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_seconds_diff_DateV2_TimestampNs_notnull "select seconds_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_seconds_diff_DateTimeV2_DateV2 "select seconds_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_seconds_diff_TimestampNs_DateV2 "select seconds_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_seconds_diff_TimestampNs_DateV2_notnull "select seconds_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_seconds_diff_TimestampNs_DateTime "select seconds_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_seconds_diff_TimestampNs_DateTime_notnull "select seconds_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_seconds_diff_DateTime_DateTimeV2 "select seconds_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_seconds_diff_DateTime_TimestampNs "select seconds_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_seconds_diff_DateTime_TimestampNs_notnull "select seconds_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
