...
//	qt_sql_mask_last_n_String_Integer_notnull "select mask_last_n(kstr, kint) from fn_test_not_nullable order by kstr, kint"
	...
	qt_sql_minute_TimestampNs "select minute(ktsns) from fn_test order by ktsns"
	qt_sql_minute_DateTimeV2_notnull "select minute(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_minute_TimestampNs_notnull "select minute(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_minute_ceil_TimestampNs "select minute_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_minute_ceil_DateTimeV2_notnull "select minute_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_minute_ceil_TimestampNs_notnull "select minute_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_minute_ceil_TimestampNs_TimestampNs "select minute_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_minute_ceil_TimestampNs_TimestampNs_notnull "select minute_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_minute_ceil_DateTimeV2_Integer "select minute_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_minute_ceil_TimestampNs_Integer "select minute_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minute_ceil_TimestampNs_Integer_notnull "select minute_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minute_ceil_TimestampNs_Integer_TimestampNs "select minute_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_minute_ceil_TimestampNs_Integer_TimestampNs_notnull "select minute_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_minute_floor_TimestampNs "select minute_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_minute_floor_DateTimeV2_notnull "select minute_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_minute_floor_TimestampNs_notnull "select minute_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_minute_floor_TimestampNs_TimestampNs "select minute_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_minute_floor_TimestampNs_TimestampNs_notnull "select minute_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_minute_floor_DateTimeV2_Integer "select minute_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_minute_floor_TimestampNs_Integer "select minute_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minute_floor_TimestampNs_Integer_notnull "select minute_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minute_floor_TimestampNs_Integer_TimestampNs "select minute_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_minute_floor_TimestampNs_Integer_TimestampNs_notnull "select minute_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_minutes_add_TimestampNs_Integer "select minutes_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minutes_add_TimestampNs_Integer_notnull "select minutes_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minutes_diff_TimestampNs_TimestampNs "select minutes_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_minutes_diff_TimestampNs_TimestampNs_notnull "select minutes_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_minutes_diff_DateV2_DateTimeV2 "select minutes_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_minutes_diff_DateV2_TimestampNs "select minutes_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_minutes_diff_DateV2_TimestampNs_notnull "select minutes_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_minutes_diff_DateTimeV2_DateV2 "select minutes_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_minutes_diff_TimestampNs_DateV2 "select minutes_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_minutes_diff_TimestampNs_DateV2_notnull "select minutes_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_minutes_diff_TimestampNs_DateTime "select minutes_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_minutes_diff_TimestampNs_DateTime_notnull "select minutes_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_minutes_diff_DateTime_DateTimeV2 "select minutes_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_minutes_diff_DateTime_TimestampNs "select minutes_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_minutes_diff_DateTime_TimestampNs_notnull "select minutes_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
	...
	qt_sql_minutes_sub_TimestampNs_Integer "select minutes_sub(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_minutes_sub_TimestampNs_Integer_notnull "select minutes_sub(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_month_TimestampNs "select month(ktsns) from fn_test order by ktsns"
	qt_sql_month_DateTimeV2_notnull "select month(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_month_TimestampNs_notnull "select month(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_month_ceil_TimestampNs "select month_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_month_ceil_DateTimeV2_notnull "select month_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_month_ceil_TimestampNs_notnull "select month_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_month_ceil_TimestampNs_TimestampNs "select month_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_month_ceil_TimestampNs_TimestampNs_notnull "select month_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_month_ceil_DateTimeV2_Integer "select month_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_month_ceil_TimestampNs_Integer "select month_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_month_ceil_TimestampNs_Integer_notnull "select month_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_month_ceil_TimestampNs_Integer_TimestampNs "select month_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_month_ceil_TimestampNs_Integer_TimestampNs_notnull "select month_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_month_floor_TimestampNs "select month_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_month_floor_DateTimeV2_notnull "select month_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_month_floor_TimestampNs_notnull "select month_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_month_floor_TimestampNs_TimestampNs "select month_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_month_floor_TimestampNs_TimestampNs_notnull "select month_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_month_floor_DateTimeV2_Integer "select month_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_month_floor_TimestampNs_Integer "select month_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_month_floor_TimestampNs_Integer_notnull "select month_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_month_floor_TimestampNs_Integer_TimestampNs "select month_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_month_floor_TimestampNs_Integer_TimestampNs_notnull "select month_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_monthname_TimestampNs "select monthname(ktsns) from fn_test order by ktsns"
	qt_sql_monthname_DateTimeV2_notnull "select monthname(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_monthname_TimestampNs_notnull "select monthname(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_months_add_TimestampNs_Integer "select months_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_months_add_TimestampNs_Integer_notnull "select months_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_months_diff_TimestampNs_TimestampNs "select months_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_months_diff_TimestampNs_TimestampNs_notnull "select months_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_months_diff_DateV2_DateTimeV2 "select months_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_months_diff_DateV2_TimestampNs "select months_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_months_diff_DateV2_TimestampNs_notnull "select months_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_months_diff_DateTimeV2_DateV2 "select months_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_months_diff_TimestampNs_DateV2 "select months_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_months_diff_TimestampNs_DateV2_notnull "select months_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_months_diff_TimestampNs_DateTime "select months_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_months_diff_TimestampNs_DateTime_notnull "select months_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_months_diff_DateTime_DateTimeV2 "select months_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_months_diff_DateTime_TimestampNs "select months_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_months_diff_DateTime_TimestampNs_notnull "select months_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
	...
	qt_sql_months_sub_TimestampNs_Integer "select months_sub(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_months_sub_TimestampNs_Integer_notnull "select months_sub(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
