...
suite("nereids_scalar_fn_H") {
	...
	qt_sql_hour_TimestampNs "select hour(ktsns) from fn_test order by ktsns"
	qt_sql_hour_DateTimeV2_notnull "select hour(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_hour_TimestampNs_notnull "select hour(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_hour_ceil_TimestampNs "select hour_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_hour_ceil_DateTimeV2_notnull "select hour_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_hour_ceil_TimestampNs_notnull "select hour_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_hour_ceil_TimestampNs_TimestampNs "select hour_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_hour_ceil_TimestampNs_TimestampNs_notnull "select hour_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_hour_ceil_DateTimeV2_Integer "select hour_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_hour_ceil_TimestampNs_Integer "select hour_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hour_ceil_TimestampNs_Integer_notnull "select hour_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hour_ceil_TimestampNs_Integer_TimestampNs "select hour_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_hour_ceil_TimestampNs_Integer_TimestampNs_notnull "select hour_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_hour_floor_TimestampNs "select hour_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_hour_floor_DateTimeV2_notnull "select hour_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_hour_floor_TimestampNs_notnull "select hour_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_hour_floor_TimestampNs_TimestampNs "select hour_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_hour_floor_TimestampNs_TimestampNs_notnull "select hour_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_hour_floor_DateTimeV2_Integer "select hour_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_hour_floor_TimestampNs_Integer "select hour_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hour_floor_TimestampNs_Integer_notnull "select hour_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hour_floor_TimestampNs_Integer_TimestampNs "select hour_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_hour_floor_TimestampNs_Integer_TimestampNs_notnull "select hour_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_hours_add_TimestampNs_Integer "select hours_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hours_add_TimestampNs_Integer_notnull "select hours_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hours_diff_TimestampNs_TimestampNs "select hours_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_hours_diff_TimestampNs_TimestampNs_notnull "select hours_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_hours_diff_DateV2_DateTimeV2 "select hours_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_hours_diff_DateV2_TimestampNs "select hours_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_hours_diff_DateV2_TimestampNs_notnull "select hours_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_hours_diff_DateTimeV2_DateV2 "select hours_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_hours_diff_TimestampNs_DateV2 "select hours_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_hours_diff_TimestampNs_DateV2_notnull "select hours_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_hours_diff_TimestampNs_DateTime "select hours_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_hours_diff_TimestampNs_DateTime_notnull "select hours_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_hours_diff_DateTime_DateTimeV2 "select hours_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_hours_diff_DateTime_TimestampNs "select hours_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_hours_diff_DateTime_TimestampNs_notnull "select hours_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
	...
	qt_sql_hours_sub_TimestampNs_Integer "select hours_sub(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_hours_sub_TimestampNs_Integer_notnull "select hours_sub(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
