...
suite("nereids_scalar_fn_Y") {
	...
	qt_sql_year_TimestampNs "select year(ktsns) from fn_test order by ktsns"
	qt_sql_year_DateTimeV2_notnull "select year(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_year_TimestampNs_notnull "select year(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_year_ceil_TimestampNs "select year_ceil(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_year_ceil_DateTimeV2_notnull "select year_ceil(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_year_ceil_TimestampNs_notnull "select year_ceil(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_year_ceil_TimestampNs_TimestampNs "select year_ceil(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_year_ceil_TimestampNs_TimestampNs_notnull "select year_ceil(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_year_ceil_DateTimeV2_Integer "select year_ceil(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_year_ceil_TimestampNs_Integer "select year_ceil(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_year_ceil_TimestampNs_Integer_notnull "select year_ceil(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_year_ceil_TimestampNs_Integer_TimestampNs "select year_ceil(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_year_ceil_TimestampNs_Integer_TimestampNs_notnull "select year_ceil(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_year_floor_TimestampNs "select year_floor(ktsns) from fn_test where id not in (0, 5) order by ktsns"
	qt_sql_year_floor_DateTimeV2_notnull "select year_floor(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_year_floor_TimestampNs_notnull "select year_floor(ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_sql_year_floor_TimestampNs_TimestampNs "select year_floor(ktsns, ktsns) from fn_test where id not in (0, 5) order by ktsns, ktsns"
	...
	qt_sql_year_floor_TimestampNs_TimestampNs_notnull "select year_floor(ktsns, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, ktsns"
	qt_sql_year_floor_DateTimeV2_Integer "select year_floor(kdtmv2s1, kint) from fn_test order by kdtmv2s1, kint"
	qt_sql_year_floor_TimestampNs_Integer "select year_floor(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_year_floor_TimestampNs_Integer_notnull "select year_floor(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_year_floor_TimestampNs_Integer_TimestampNs "select year_floor(ktsns, kint, ktsns) from fn_test where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_year_floor_TimestampNs_Integer_TimestampNs_notnull "select year_floor(ktsns, kint, ktsns) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint, ktsns"
	...
	qt_sql_years_add_TimestampNs_Integer "select years_add(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_years_add_TimestampNs_Integer_notnull "select years_add(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_years_diff_TimestampNs_TimestampNs "select years_diff(ktsns, ktsns) from fn_test order by ktsns, ktsns"
	...
	qt_sql_years_diff_TimestampNs_TimestampNs_notnull "select years_diff(ktsns, ktsns) from fn_test_not_nullable order by ktsns, ktsns"
	qt_sql_years_diff_DateV2_DateTimeV2 "select years_diff(kdtv2, kdtmv2s1) from fn_test order by kdtv2, kdtmv2s1"
	qt_sql_years_diff_DateV2_TimestampNs "select years_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test order by kdtv2, ktsns"
	...
	qt_sql_years_diff_DateV2_TimestampNs_notnull "select years_diff(cast(kdtv2 as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtv2, ktsns"
	qt_sql_years_diff_DateTimeV2_DateV2 "select years_diff(kdtmv2s1, kdtv2) from fn_test order by kdtmv2s1, kdtv2"
	qt_sql_years_diff_TimestampNs_DateV2 "select years_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test order by ktsns, kdtv2"
	...
	qt_sql_years_diff_TimestampNs_DateV2_notnull "select years_diff(ktsns, cast(kdtv2 as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtv2"
	...
	qt_sql_years_diff_TimestampNs_DateTime "select years_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test order by ktsns, kdtm"
	...
	qt_sql_years_diff_TimestampNs_DateTime_notnull "select years_diff(ktsns, cast(kdtm as timestamp_ns)) from fn_test_not_nullable order by ktsns, kdtm"
	qt_sql_years_diff_DateTime_DateTimeV2 "select years_diff(kdtm, kdtmv2s1) from fn_test order by kdtm, kdtmv2s1"
	qt_sql_years_diff_DateTime_TimestampNs "select years_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test order by kdtm, ktsns"
	...
	qt_sql_years_diff_DateTime_TimestampNs_notnull "select years_diff(cast(kdtm as timestamp_ns), ktsns) from fn_test_not_nullable order by kdtm, ktsns"
	...
	qt_sql_years_sub_TimestampNs_Integer "select years_sub(ktsns, kint) from fn_test where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_years_sub_TimestampNs_Integer_notnull "select years_sub(ktsns, kint) from fn_test_not_nullable where id not in (0, 5) order by ktsns, kint"
	...
	qt_sql_yearweek_TimestampNs "select yearweek(ktsns) from fn_test order by ktsns"
	qt_sql_yearweek_DateTimeV2_notnull "select yearweek(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_yearweek_TimestampNs_notnull "select yearweek(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_yearweek_TimestampNs_Integer "select yearweek(ktsns, kint) from fn_test order by ktsns, kint"
	...
	qt_sql_yearweek_TimestampNs_Integer_notnull "select yearweek(ktsns, kint) from fn_test_not_nullable order by ktsns, kint"
