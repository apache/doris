...
suite("nereids_scalar_fn_F") {
	...
	qt_sql_field_TimestampNs "select field(ktsns, 1, 2) from fn_test order by ktsns"
	qt_sql_field_DateTimeV2_notnull "select field(kdtmv2s1, 1, 2) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_field_TimestampNs_notnull "select field(ktsns, 1, 2) from fn_test_not_nullable order by ktsns"
