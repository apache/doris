...
suite("nereids_scalar_fn_G") {
    ...
    qt_sql_greatest_TimestampNs "select greatest(ktsns) from fn_test order by ktsns"
    qt_sql_greatest_DateTimeV2_notnull "select greatest(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
    qt_sql_greatest_TimestampNs_notnull "select greatest(ktsns) from fn_test_not_nullable order by ktsns"
