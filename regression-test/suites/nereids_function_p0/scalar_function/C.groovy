...
suite("nereids_scalar_fn_C") {
	...
	qt_sql_coalesce_TimestampNs "select coalesce(ktsns) from fn_test order by ktsns"
	qt_sql_coalesce_DateTimeV2_notnull "select coalesce(kdtmv2s1) from fn_test_not_nullable order by kdtmv2s1"
	qt_sql_coalesce_TimestampNs_notnull "select coalesce(ktsns) from fn_test_not_nullable order by ktsns"
	...
	qt_sql_convert_tz_TimestampNs_Varchar_Varchar "select convert_tz(ktsns, 'Asia/Shanghai', 'Europe/Sofia') from fn_test where id not in (0, 5) order by ktsns"
	...
	qt_sql_convert_tz_TimestampNs_Varchar_Varchar_notnull "select convert_tz(ktsns, 'Asia/Shanghai', 'Europe/Sofia') from fn_test_not_nullable where id not in (0, 5) order by ktsns"
	...
	qt_km_tsns_tint "select count(km_tsns_tint) from fn_test "
	qt_km_tsns_tint_not_nullable "select count(km_tsns_tint) from fn_test_not_nullable "
	...
	qt_km_tint_tsns "select count(km_tint_tsns) from fn_test "
	qt_km_tint_tsns_not_nullable "select count(km_tint_tsns) from fn_test_not_nullable "
	...
	qt_kstruct_tsns "select count(kstruct_tsns) from fn_test "
	qt_kstruct_tsns_not_nullable "select count(kstruct_tsns) from fn_test_not_nullable "
	order_qt_kstruct_tsns_value "select struct_element(kstruct_tsns, 'ts') from fn_test order by id"
	order_qt_kstruct_tsns_value_not_nullable "select struct_element(kstruct_tsns, 'ts') from fn_test_not_nullable order by id"
	...
	qt_kadtmv2_timestamp_ns "select count(katsns) from fn_test "
	qt_kadtmv2_not_nullable "select count(kadtmv2) from fn_test_not_nullable "
	qt_kadtmv2_not_nullable_timestamp_ns "select count(katsns) from fn_test_not_nullable "
