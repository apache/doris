...
//		select kint, count(kbint) over(order by kint) as wf from fn_test'''
    qt_sql_count_pb '''
		select kstr, count(kbint) over(partition by kstr) as wf from fn_test'''
    qt_sql_count_pb_ob '''
		select kstr, kint, count(kbint) over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, count(kbint) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_count_pb_notnull '''
		select kstr, count(kbint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_count_pb_ob_notnull '''
		select kstr, kint, count(kbint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
    qt_sql_count_f_1 '''
...
//		select kint, avg(kbint) over(order by kint) as wf from fn_test'''
    qt_sql_avg_pb '''
		select kstr, avg(kbint) over(partition by kstr) as wf from fn_test'''
    qt_sql_avg_pb_ob '''
		select kstr, kint, avg(kbint) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_avg_notnull '''
...
//		select kint, avg(kbint) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_avg_pb_notnull '''
		select kstr, avg(kbint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_avg_pb_ob_notnull '''
		select kstr, kint, avg(kbint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
    qt_sql_avg_f_1 '''
...
//		select kint, min(kbint) over(order by kint) as wf from fn_test'''
    qt_sql_min_pb '''
		select kstr, min(kbint) over(partition by kstr) as wf from fn_test'''
    qt_sql_min_pb_ob '''
		select kstr, kint, min(kbint) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_min_notnull '''
...
//		select kint, min(kbint) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_min_pb_notnull '''
		select kstr, min(kbint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_min_pb_ob_notnull '''
		select kstr, kint, min(kbint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
    qt_sql_min_f_1 '''
...
//		select kint, max(kbint) over(order by kint) as wf from fn_test'''
    qt_sql_max_pb '''
		select kstr, max(kbint) over(partition by kstr) as wf from fn_test'''
    qt_sql_max_pb_ob '''
		select kstr, kint, max(kbint) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_max_notnull '''
...
//		select kint, max(kbint) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_max_pb_notnull '''
		select kstr, max(kbint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_max_pb_ob_notnull '''
		select kstr, kint, max(kbint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
    qt_sql_max_f_1 '''
...
//		select kint, sum(kbint) over(order by kint) as wf from fn_test'''
    qt_sql_sum_pb '''
		select kstr, sum(kbint) over(partition by kstr) as wf from fn_test'''
    qt_sql_sum_pb_ob '''
		select kstr, kint, sum(kbint) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_sum_notnull '''
...
//		select kint, sum(kbint) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_sum_pb_notnull '''
		select kstr, sum(kbint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_sum_pb_ob_notnull '''
		select kstr, kint, sum(kbint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
    qt_sql_sum_f_1 '''
...
//		select kint, dense_rank() over(order by kint) as wf from fn_test'''
    qt_sql_dense_rank_pb '''
		select kstr, dense_rank() over(partition by kstr) as wf from fn_test'''
    qt_sql_dense_rank_pb_ob '''
		select kstr, kint, dense_rank() over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, dense_rank() over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_dense_rank_pb_notnull '''
		select kstr, dense_rank() over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_dense_rank_pb_ob_notnull '''
		select kstr, kint, dense_rank() over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, first_value(kint) over(order by kint) as wf from fn_test'''
    sql '''
		select kstr, first_value(kint) over(partition by kstr) as wf from fn_test'''
    qt_sql_first_value_pb_ob '''
		select kstr, kint, first_value(kint) over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, first_value(kint) over(order by kint) as wf from fn_test_not_nullable'''
    sql '''
		select kstr, first_value(kint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_first_value_pb_ob_notnull '''
		select kstr, kint, first_value(kint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, lag(kint, 2, 1) over(order by kint) as wf from fn_test'''
    sql '''
		select kstr, lag(kint, 2, 1) over(partition by kstr) as wf from fn_test'''
    qt_sql_lag_pb_ob '''
		select kstr, kint, lag(kint, 2, 1) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_lag_notnull '''
...
//		select kint, lag(kint, 2, 1) over(order by kint) as wf from fn_test_not_nullable'''
    sql '''
		select kstr, lag(kint, 2, 1) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_lag_pb_ob_notnull '''
		select kstr, kint, lag(kint, 2, 1) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, last_value(kint) over(order by kint) as wf from fn_test'''
    sql '''
		select kstr, last_value(kint) over(partition by kstr) as wf from fn_test'''
    qt_sql_last_value_pb_ob '''
		select kstr, kint, last_value(kint) over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, last_value(kint) over(order by kint) as wf from fn_test_not_nullable'''
    sql '''
		select kstr, last_value(kint) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_last_value_pb_ob_notnull '''
		select kstr, kint, last_value(kint) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, lead(kint, 2, 1) over(order by kint) as wf from fn_test'''
    sql '''
		select kstr, lead(kint, 2, 1) over(partition by kstr) as wf from fn_test'''
    qt_sql_lead_pb_ob '''
		select kstr, kint, lead(kint, 2, 1) over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_lead_notnull '''
...
//		select kint, lead(kint, 2, 1) over(order by kint) as wf from fn_test_not_nullable'''
    sql '''
		select kstr, lead(kint, 2, 1) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_lead_pb_ob_notnull '''
		select kstr, kint, lead(kint, 2, 1) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, ntile(3) over(order by kint) as wf from fn_test'''
    qt_sql_ntile_pb '''
		select kstr, ntile(3) over(partition by kstr) as wf from fn_test'''
    qt_sql_ntile_pb_ob '''
		select kstr, kint, ntile(3) over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, ntile(3) over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_ntile_pb_notnull '''
		select kstr, ntile(3) over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_ntile_pb_ob_notnull '''
		select kstr, kint, ntile(3) over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, rank() over(order by kint) as wf from fn_test'''
    qt_sql_rank_pb '''
		select kstr, rank() over(partition by kstr) as wf from fn_test'''
    qt_sql_rank_pb_ob '''
		select kstr, kint, rank() over(partition by kstr order by kint) as wf from fn_test'''
    // qt_sql_rank_notnull '''
...
//		select kint, rank() over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_rank_pb_notnull '''
		select kstr, rank() over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_rank_pb_ob_notnull '''
		select kstr, kint, rank() over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
...
//		select kint, row_number() over(order by kint) as wf from fn_test'''
    qt_sql_row_number_pb '''
		select kstr, row_number() over(partition by kstr) as wf from fn_test'''
    qt_sql_row_number_pb_ob '''
		select kstr, kint, row_number() over(partition by kstr order by kint) as wf from fn_test'''
...
//		select kint, row_number() over(order by kint) as wf from fn_test_not_nullable'''
    qt_sql_row_number_pb_notnull '''
		select kstr, row_number() over(partition by kstr) as wf from fn_test_not_nullable'''
    qt_sql_row_number_pb_ob_notnull '''
		select kstr, kint, row_number() over(partition by kstr order by kint) as wf from fn_test_not_nullable'''
