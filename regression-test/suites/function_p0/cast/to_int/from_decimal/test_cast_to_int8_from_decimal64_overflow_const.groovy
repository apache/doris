...
suite("test_cast_to_int8_from_decimal64_overflow_const") {

    // This test case is generated from the correspoinding be UT test case,
    // update this case if the correspoinding be UT test case is updated,
    // e.g.: ../run-be-ut.sh --run --filter=FunctionCastToDecimalTest.* --gen_regression_case
    sql "set debug_skip_fold_constant = true;"
    def test_cast_to_int8_from_decimal64_overflow_0_test_data = ["""128""","""-129""","""9999999999""","""-9999999999""","""9999999998""","""-9999999998"""]
    sql "set enable_strict_cast=true;"

    for (b in ["false", "true"]) {
        sql """set debug_skip_fold_constant = "${b}";"""
        for (test_str in test_cast_to_int8_from_decimal64_overflow_0_test_data) {
            test {
    ...
    for (test_str in test_cast_to_int8_from_decimal64_overflow_0_test_data) {
        qt_sql_test_cast_to_int8_from_decimal64_overflow_0 """select cast(cast("${test_str}" as decimalv3(10, 0)) as tinyint);"""
        testFoldConst("""select cast(cast("${test_str}" as decimalv3(10, 0)) as tinyint);""")
    }
    sql "set debug_skip_fold_constant = true;"
    def test_cast_to_int8_from_decimal64_overflow_1_test_data = ["""128.00000""","""128.00001""","""128.00009""","""128.99999""","""128.99998""","""128.09999""","""128.90000""","""128.90001""","""-129.00000""","""-129.00001""","""-129.00009""","""-129.99999""","""-129.99998""","""-129.09999""","""-129.90000""","""-129.90001""","""99999.00000""","""99999.00001""","""99999.00009""","""99999.99999""",
    ...
    for (b in ["false", "true"]) {
        sql """set debug_skip_fold_constant = "${b}";"""
        for (test_str in test_cast_to_int8_from_decimal64_overflow_1_test_data) {
            test {
    ...
    for (test_str in test_cast_to_int8_from_decimal64_overflow_1_test_data) {
        qt_sql_test_cast_to_int8_from_decimal64_overflow_1 """select cast(cast("${test_str}" as decimalv3(10, 5)) as tinyint);"""
        testFoldConst("""select cast(cast("${test_str}" as decimalv3(10, 5)) as tinyint);""")
    }
    sql "set debug_skip_fold_constant = true;"
    def test_cast_to_int8_from_decimal64_overflow_4_test_data = ["""128""","""-129""","""999999999999999999""","""-999999999999999999""","""999999999999999998""","""-999999999999999998"""]
    sql "set enable_strict_cast=true;"

    for (b in ["false", "true"]) {
        sql """set debug_skip_fold_constant = "${b}";"""
        for (test_str in test_cast_to_int8_from_decimal64_overflow_4_test_data) {
            test {
    ...
    for (test_str in test_cast_to_int8_from_decimal64_overflow_4_test_data) {
        qt_sql_test_cast_to_int8_from_decimal64_overflow_4 """select cast(cast("${test_str}" as decimalv3(18, 0)) as tinyint);"""
        testFoldConst("""select cast(cast("${test_str}" as decimalv3(18, 0)) as tinyint);""")
    }
    sql "set debug_skip_fold_constant = true;"
    def test_cast_to_int8_from_decimal64_overflow_5_test_data = ["""128.0""","""128.1""","""128.9""","""128.9""","""128.8""","""128.0""","""128.9""","""128.8""","""-129.0""","""-129.1""","""-129.9""","""-129.9""","""-129.8""","""-129.0""","""-129.9""","""-129.8""","""99999999999999999.0""","""99999999999999999.1""","""99999999999999999.9""","""99999999999999999.9""",
    ...
    for (b in ["false", "true"]) {
        sql """set debug_skip_fold_constant = "${b}";"""
        for (test_str in test_cast_to_int8_from_decimal64_overflow_5_test_data) {
            test {
    ...
    for (b in ["false", "true"]) {
        sql """set debug_skip_fold_constant = "${b}";"""
        for (test_str in test_cast_to_int8_from_decimal64_overflow_6_test_data) {
            test {
    ...
    for (test_str in test_cast_to_int8_from_decimal64_overflow_6_test_data) {
        qt_sql_test_cast_to_int8_from_decimal64_overflow_6 """select cast(cast("${test_str}" as decimalv3(18, 9)) as tinyint);"""
