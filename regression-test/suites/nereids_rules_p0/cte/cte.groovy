...
suite("nereids_cte") {
    ...
    // test cte inline
    qt_sql """explain with  cte as (select col2, col1 from t1) select * from cte t1;"""

    // test cte materialize : use cte in inline view
    qt_sql """explain with  cte1 as (select col2, col1 from t1) select * from (SELECT * FROM cte1) v1, cte1"""

    // use cte in scalar query
    qt_sql """explain with  cte as (select col2, col1 from t1)  SELECT col1 FROM cte group by col1 having sum(col2) > (select 0.05 * avg(col2) from cte )"""

    // test filter push down
    qt_sql """explain with  cte as (select col2, col1 from t1) select * from cte t1,  cte t2 where t1.col2 = 3 and t2.col1 = 2 """

    // test filter of inline view push down
    qt_sql """explain with  cte1 as (select col2, col1 from t1) select * from (SELECT * FROM cte1 WHERE col2 = 3) v1, cte1 WHERE cte1.col2 = 4"""

    // test filter of scalar query push down
    qt_sql """explain with  cte as (select col2, col1 from t1)  SELECT col1 FROM cte group by col1 having sum(col2) > (select 0.05 * avg(col2) from cte where col2 > 1 )"""

    // test filter of scalar query  push down and column prune
    qt_sql """explain with  cte as (select col2, col3, col4, col1 from t1)  SELECT col1 FROM cte group by col1 having sum(col2) > (select 0.05 * avg(col4) from cte where col2 > 1 )"""

    // test project prune of inline vew
    qt_sql """explain with  cte1 as (select col2, col1, col3, col4 from t1) select v1.col1, v2.col2 from (select col1 FROM cte1) v1, cte1 v2"""
