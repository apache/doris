...
suite("normalize_window_nullable_agg") {
    ...
    qt_topn "select topn(b,3) over(order by c rows between 2 preceding and 1 preceding) from normalize_window_nullable_agg"
    qt_topn_array "select topn_array(b,3) over(order by c rows between 2 preceding and 1 preceding) from normalize_window_nullable_agg"
    qt_topn_weighted "select topn_weighted(b,c,3) over(order by c rows between 2 preceding and 1 preceding) from normalize_window_nullable_agg"
    qt_max_by "select max_by(b,a) over(order by c rows between 2 preceding and 1 preceding) from normalize_window_nullable_agg"
    qt_min_by "select min_by(b,a) over(order by c rows between 2 preceding and 1 preceding) from normalize_window_nullable_agg"
