...
suite("pre_write_strategy") {
    ...
    // set pre strategy is FORCE_IN_RBO, explain should contain RBO.
    def mv_0 =
            """
    ...
    // if final rewrite plan output is different from tmp plan, though use FORCE_IN_RBO, should use rewritten in final
    // CBO current
    def mv0_1 = """
    ...
    // set pre strategy is TRY_IN_RBO, use ELIMINATE_CONST_JOIN_CONDITION and rewrite success
    def mv_1 =
            """
    ...
    def query_1 =
            """
            select t1.l_partkey, t1.l_suppkey,
            sum(o_totalprice),
            max(o_totalprice),
            min(o_totalprice),
            count(*),
            count(distinct case when o_shippriority > 1 and o_orderkey IN (1, 3) then o_custkey else null end)
            from lineitem t1
            inner join orders on t1.l_orderkey = orders.o_orderkey and t1.l_shipdate = o_orderdate
            where l_shipdate = '2023-12-11'
            group by
            l_partkey,
            l_suppkey;
            """
    order_qt_query1_before "${query_1}"
    async_mv_rewrite_success(db, mv_1, query_1, "mv_1")
    explain {
        sql("${query_1}")
        contains("RBO.")
    }
    order_qt_query1_after "${query_1}"
    ...
    def query_1_0 =
            ...
            l_suppkey
            """
    order_qt_query1_0_before "${query_1_0}"
    ...
    sql "set pre_materialized_view_rewrite_strategy = 'TRY_IN_RBO';"
    // set pre strategy is TRY_IN_RBO, rule not in pre rewrite rule list, should use CBO
    def mv_1_1 =
            """
            select l_shipdate, o_orderdate, l_partkey, l_suppkey,
            sum(o_totalprice) as sum_total,
    ...
    def query_1_1 =
            ...
            l_suppkey
            """
    order_qt_query1_1_before "${query_1_1}"
    async_mv_rewrite_success(db, mv_1_1, query_1_1, "mv_1_1")
    explain {
        sql("${query_1_1}")
        contains("CBO.")
    }
    order_qt_query1_1_after "${query_1_1}"
    sql """ DROP MATERIALIZED VIEW IF EXISTS mv_1_1"""


    // set pre strategy is TRY_IN_RBO, use ELIMINATE_CONST_JOIN_CONDITION and ELIMINATE_GROUP_BY_KEY and rewrite success
    // tmp plan output is different from final rbo plan, should not use rewrite in RBO, so fail in final CBO
    def mv_1_2 =
            """
            select l_shipdate, o_orderdate, l_partkey, l_suppkey,
            sum(o_totalprice) as sum_total,
    ...
    // final CBO can rewrite successfully
    ...
    def query_2 =
            ...
            l_suppkey
            """
    order_qt_query2_before "${query_2}"
    async_mv_rewrite_success(db, mv_2, query_2, "mv_2")
    explain {
        sql("${query_2}")
        contains("CBO.")
    }
    order_qt_query2_after "${query_2}"
    sql """ DROP MATERIALIZED VIEW IF EXISTS mv_2"""


    // should fail when use ELIMINATE_CONST_JOIN_CONDITION and rewrite success
    def mv_2_0 =
            """
            select l_shipdate, o_orderdate, l_partkey, l_suppkey,
            sum(o_totalprice) as sum_total,
    ...
    def query_2_0 =
            ...
            l_suppkey
            """
    order_qt_query2_0_before "${query_2_0}"
    async_mv_rewrite_fail(db, mv_2_0, query_2_0, "mv_2_0")
    explain {
        sql("${query_2}")
        contains("CBO.")
    }
