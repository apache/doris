...
suite("relation_implied_predicate_nested_mtmv") {
    ...
    sql """DROP VIEW IF EXISTS rip_fact_view"""
    ...
    sql """CREATE VIEW rip_fact_view AS SELECT * FROM rip_fact WHERE k1 > 0"""

    def query = """
        SELECT f.k1, f.dim_id, f.amount, d.tag
        FROM rip_fact f
        INNER JOIN rip_dim d ON f.dim_id = d.dim_id
        WHERE f.sku_type = '1' AND f.k1 > 0
        ORDER BY 1, 2, 3, 4
    """
    ...
    create_async_mv(db, "rip_mv_fact", """
        SELECT k1, dim_id, amount, sku_type
        FROM rip_fact_view
        WHERE sku_type = '1'
    """)
    ...
    // Only the lower MV adopts the new view definition. The upper still stores k1 = 1 and 3.
    sql """ALTER MATERIALIZED VIEW rip_mv_join SET ('grace_period' = '31536000')"""
    sql """ALTER VIEW rip_fact_view AS SELECT * FROM rip_fact WHERE k1 > 2"""
    sql """REFRESH MATERIALIZED VIEW rip_mv_fact COMPLETE"""
    waitingMTMVTaskFinished(getJobName(db, "rip_mv_fact"))
    ...
    def lowerMvQuery = """
        ...
        ORDER BY 1, 2, 3, 4
    """
    def staleUpperQuery = lowerMvQuery.replaceFirst("SELECT", "SELECT /*+ use_mv(rip_mv_join) */")
    def currentLowerQuery = lowerMvQuery.replaceFirst("SELECT",
            "SELECT /*+ SET_VAR(enable_materialized_view_rewrite=false) */")
    // The explicit query filter must still reject k1 = 1 when grace_period permits the old upper MV.
    mv_rewrite_success(staleUpperQuery, "rip_mv_join")
    check_sqls_result_equal(staleUpperQuery, currentLowerQuery)
}
