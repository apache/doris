...
suite("nested_mtmv_in_level_20") {
    ...
    def in_mv = """
        select l_orderkey from lineitem_1 inner join orders_1 on lineitem_1.l_orderkey = orders_1.o_orderkey where lineitem_1.l_shipdate > "2023-10-17" group by l_orderkey
        """
    def join_mv_1 = """
    ...
    def join_mv_3 = """
        ...
        where t1.l_orderkey in (select l_orderkey from ${mv_in_name})
...
//        """
    ...
    def sql_3 = """
        ...
        left join lineitem_1 as t2
        on t1.l_orderkey = t2.l_orderkey
        where t1.l_orderkey in (select l_orderkey from (
            select l_orderkey from lineitem_1 inner join orders_1 on lineitem_1.l_orderkey = orders_1.o_orderkey where lineitem_1.l_shipdate > "2023-10-17" group by l_orderkey
            ) t3)
        group by t2.l_orderkey, t2.l_partkey, t2.l_suppkey, t1.o_orderkey, t1.o_custkey, t1.ps_partkey, t1.ps_suppkey, t1.agg1, t1.agg2, t1.agg3, t1.agg4, t1.agg5, t1.agg6
        """
    ...
    for (int i = 4; i <= 10; i++) {
        ...
        query_list[i-2] = """
            ...
            left join lineitem_1 as t2
            on t1.l_orderkey = t2.l_orderkey
            where t1.l_orderkey in (select l_orderkey from (select l_orderkey from lineitem_1 inner join orders_1 on lineitem_1.l_orderkey = orders_1.o_orderkey where lineitem_1.l_shipdate > "2023-10-17" group by l_orderkey) as t3)
            group by t2.l_orderkey, t2.l_partkey, t2.l_suppkey, t1.o_orderkey, t1.o_custkey, t1.ps_partkey, t1.ps_suppkey, t1.agg1, t1.agg2, t1.agg3, t1.agg4, t1.agg5, t1.agg6
            """
        explain {
            sql("${query_list[i-2]}")
