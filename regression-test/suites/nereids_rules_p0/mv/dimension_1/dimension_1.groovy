...
suite("partition_mv_rewrite_dimension_1") {
    ...
    (1, 1, 'ok', 99.5, 'a', 'b', 1, 'yy', '2023-10-17'),
    (2, 2, 'ok', 109.2, 'c','d',2, 'mm', '2023-10-18'),
    (3, 3, 'ok', 99.5, 'a', 'b', 1, 'yy', '2023-10-19'); 
    """
...
//        contains(mv_name_2)
    ...
    def join_filter_stmt_3 = """
        select l_shipdate, o_orderdate, l_partkey, l_suppkey 
        from lineitem 
        left join (select * from orders where o_orderdate = '2023-12-11' ) t2 
    ...
    def join_filter_stmt_4 = """
        ...
        left join orders 
        on lineitem.l_orderkey = orders.o_orderkey 
        where l_shipdate = '2023-12-11' and o_orderdate = '2023-12-11'"""
        ...
        for (int j = 0; j < join_type_stmt_list.size(); j++) {
            if (i == j) {
                explain {
                    ...
                    contains(join_type_mv)
                }
            } else {
                explain {
                    sql("${join_type_stmt_list[j]}")
                    notContains(join_type_mv)
                }
            }
        }
...
//        contains(query_partition_mv_name_1)
//    }
//    explain {
...
//    sql """DROP MATERIALIZED VIEW IF EXISTS ${query_partition_mv_name_1};"""
    ...
    def view_partition_mv_stmt_1 = """
        select l_shipdate, l_partkey, l_orderkey, count(*) from lineitem group by l_shipdate, l_partkey, l_orderkey"""
    ...
    def view_partition_sql_1 = """select l_shipdate, o_orderdate, l_partkey, count(*) 
        from lineitem 
        left join orders   
        on lineitem.l_orderkey = orders.o_orderkey group by l_shipdate, o_orderdate, l_partkey
        """
    explain {
...
//    sql """DROP MATERIALIZED VIEW IF EXISTS ${union_mv_name_1};"""
    ...
    def predicate_mv_stmt_1 = """
        ...
        where l_shipdate >= "2023-12-04"
        """
    create_mv_orders(predicate_mv_name_1, predicate_mv_stmt_1)
    ...
    def predicate_sql_1 = """
        ...
        left join orders   
        on lineitem.l_orderkey = orders.o_orderkey
        where l_shipdate >= "2023-12-04" and l_partkey = 1
        """
    explain {
        sql("${predicate_sql_1}")
