...
suite("full_join_range_date_increment_create", "increment_create") {
    ...
    sql """CREATE TABLE `orders_full_2` (
    ...
    sql """
    drop table if exists lineitem_full_2
    """

    sql """CREATE TABLE `lineitem_full_2` (
    ...
    sql """
    insert into orders_full_2 values 
    ...
    sql """
    insert into lineitem_full_2 values 
    ...
    sql """analyze table orders_full_2 with sync;"""
    sql """analyze table lineitem_full_2 with sync;"""


    def mv_name = "mv_full_range_date"
    ...
    def mv_sql_1 = """select l_shipdate, l_orderkey, o_orderdate, o_orderkey, 1 
        ...
        on lineitem_full_2.l_shipdate = orders_full_2.o_orderdate
    ...
    def mv_sql_7 = """select l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey, 
        count(orders_full_2.O_ORDERDATE) over (partition by lineitem_full_2.L_SHIPDATE order by lineitem_full_2.L_ORDERKEY) as window_count 
        from lineitem_full_2 
        full join orders_full_2 
        on lineitem_full_2.l_orderkey = orders_full_2.o_orderkey 
        group by l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey"""
    ...
    def mv_sql_8 = """select l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey, 
        count(orders_full_2.O_ORDERDATE) over (partition by lineitem_full_2.l_orderkey order by lineitem_full_2.l_orderkey) as window_count
        from lineitem_full_2
        full join orders_full_2
        on lineitem_full_2.l_orderkey = orders_full_2.o_orderkey
        group by l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey"""
    ...
    def mv_sql_9 = """select l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey, 
        count(orders_full_2.O_ORDERDATE) over (order by lineitem_full_2.l_orderkey) as window_count 
        from lineitem_full_2 
        full join orders_full_2 
        on lineitem_full_2.l_orderkey = orders_full_2.o_orderkey 
        group by l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey"""
    ...
    def mv_sql_11 = """select l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey, 
        count(orders_full_2.O_ORDERDATE) over (partition by orders_full_2.o_orderkey order by lineitem_full_2.l_orderkey) as window_count
        from lineitem_full_2
        full join orders_full_2
        on lineitem_full_2.l_orderkey = orders_full_2.o_orderkey
        group by l_shipdate, l_orderkey, O_ORDERDATE, o_orderkey"""
