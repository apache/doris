...
suite("test_iceberg_azure_native_credentials", "p2,external,iceberg") {
    ...
    def readNativeRange = { String table, String stage, boolean hasDeletes ->
        String tag = "azure_native_${table}_${stage}_" + UUID.randomUUID().toString()
        // Disable the caches for this probe so a cached result cannot replace Azure range I/O.
        ...
        if (hasDeletes) {
            assertTrue(positiveCounter(profile, "NumDeleteRows"), "${tag}: expected applied delete rows")
        }
    ...
    for (int version : [2, 3]) {
        for (String format : ["parquet", "orc"]) {
            ...
            "order_qt_${table}_azure_files" """
                ...
                from ${table}\$files
            """
            readNativeRange(table, "insert_read", false)
            ...
            "order_qt_${table}_delete_files" """
                select count(*) > 0, sum(record_count) > 0,
                       min(lower(file_format)), max(lower(file_format))
                from ${table}\$delete_files
            """
            "order_qt_${table}_position_deletes" """
                select count(*) > 0, min(pos) >= 0, sum(length(file_path)) > 0
                from ${table}\$position_deletes
            """
            readNativeRange(table, "delete_read", true)

            sql """update ${table} set score = score + 100 where id = 2"""
            readNativeRange(table, "update_read", true)
            "order_qt_${table}_old_deletes_before_merge" """
                select count(*) > 0, sum(record_count) > 0 from ${table}\$delete_files
            """
            sql """
                merge into ${table} t
            ...
            """
            readNativeRange(table, "merge_read", true)
            "order_qt_${table}_position_deletes_after_merge" """
                select count(*) > 0, min(pos) >= 0, sum(length(file_path)) > 0
                from ${table}\$position_deletes
            """

            sql """insert overwrite table ${table} values (7, 'seven', 70), (8, 'eight', 80)"""
            readNativeRange(table, "overwrite_read", false)
        }
    }
}
