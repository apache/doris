...
suite("iceberg_rest_s3_storage_vended_test", "p2,external,iceberg,external_docker,external_docker_iceberg_rest,new_catalog_property") {
    ...
    // Temporarily close the vendored credential tests of different cloud vendors and wait for the subsequent testing environment to be ready
    ...
    if (enabled != null && enabled.equalsIgnoreCase("true")) {
        ...
        String iceberg_rest_type_prop_s3_vended = """
            'type'='iceberg',
        ...
        String iceberg_rest_type_prop_oss_vended = """
            'type'='iceberg',
            'iceberg.catalog.type'='rest',
        ...
        String iceberg_rest_type_prop_cos_vended = """
            'type'='iceberg',
            'iceberg.catalog.type'='rest',
        ...
        """
        testQueryAndInsert(iceberg_rest_type_prop_oss_vended + warehouse,"iceberg_rest_on_oss_vended")

        // -------- REST on COS --------
        warehouse = """
         'warehouse' = 's3://${cos_parent_path}/iceberg_rest_warehouse'
        """
        testQueryAndInsert(iceberg_rest_type_prop_cos_vended + warehouse, "iceberg_rest_on_cos_vended")

        // -------- REST on S3 --------
        warehouse = """
         'warehouse' = 's3://${s3_parent_path}/iceberg_rest_warehouse'
        """
        testQueryAndInsert(iceberg_rest_type_prop_s3_vended + warehouse, "iceberg_rest_on_s3_vended")
        warehouse = """
         'warehouse' = 's3a://${s3_parent_path}/iceberg_rest_warehouse'
        """
        testQueryAndInsert(iceberg_rest_type_prop_s3_vended + warehouse, "iceberg_rest_on_s3a_vended")
    }
}