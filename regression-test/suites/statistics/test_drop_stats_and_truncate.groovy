...
suite("test_drop_stats_and_truncate") {
    ...
    result = sql """show table stats non_part"""
    all_columns = result[0][4]
    // Unlike `drop stats`, truncate drops the collected statistics but keeps the stats record itself,
    // so that the rows loaded after the truncation are still accounted for.
    assertEquals("[]", all_columns)
