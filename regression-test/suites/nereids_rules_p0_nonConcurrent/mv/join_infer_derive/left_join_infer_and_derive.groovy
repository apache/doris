 ...
 */
suite("left_join_infer_and_derive") {
    ...
    sql """CREATE TABLE `lineitem_left` (
    ...
    sql """
    insert into orders_left values 
    ...
    sql """
    insert into lineitem_left values 
    ...
    def mtmv_full_join_stmt = """
        ...
        from lineitem_left
        full join orders_left
        on lineitem_left.l_orderkey = orders_left.o_orderkey"""
    def mv_name_1 = "mv_left"
    def mtmv_stmt_list = [mtmv_full_join_stmt]

    def query_stmt_0 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, orders_left.o_orderkey 
        from (select l_shipdate, l_partkey, l_suppkey, l_orderkey from lineitem_left where l_shipdate = '2023-10-17') t
        left join orders_left 
        on t.l_orderkey = orders_left.o_orderkey"""
    def query_stmt_1 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey
        from lineitem_left  
        left join (select o_orderdate,o_orderkey from orders_left where o_orderdate = '2023-10-17' ) t 
        on lineitem_left.l_orderkey = t.o_orderkey"""
    def query_stmt_2 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from lineitem_left  
        left join orders_left 
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where l_shipdate = '2023-10-17'"""
    def query_stmt_3 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from lineitem_left  
        left join orders_left 
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where o_orderdate = '2023-10-17'"""
    def query_stmt_4 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from lineitem_left  
        left join orders_left 
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where o_orderkey = 1"""
    def query_stmt_5 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from lineitem_left  
        left join orders_left 
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where l_suppkey = 2"""
    def query_stmt_6 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, orders_left.o_orderkey 
        from orders_left 
        left join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from lineitem_left  where l_shipdate = '2023-10-17') t 
        on t.l_orderkey = orders_left.o_orderkey"""
    def query_stmt_7 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey 
        from (select o_orderdate, o_orderkey from orders_left where o_orderdate = '2023-10-17' ) t 
        left join lineitem_left   
        on lineitem_left.l_orderkey = t.o_orderkey"""
    def query_stmt_8 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from orders_left  
        left join lineitem_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where l_shipdate = '2023-10-17' """
    def query_stmt_9 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from orders_left 
        left join lineitem_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey 
        where o_orderdate = '2023-10-17'  """
    def query_stmt_10 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from orders_left  
        left join lineitem_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey
        where o_orderkey = 1"""
    def query_stmt_11 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from orders_left  
        left join lineitem_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey
        where l_suppkey = 2"""
    def query_stmt_12 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from orders_left  
        left join lineitem_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey
        where l_suppkey = 2 and o_orderdate = '2023-10-17'"""
    def query_stmt_13 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, orders_left.o_orderkey 
        from lineitem_left
        left join orders_left  
        on lineitem_left.l_orderkey = orders_left.o_orderkey
        where l_suppkey = 2 and o_orderdate = '2023-10-17'"""
