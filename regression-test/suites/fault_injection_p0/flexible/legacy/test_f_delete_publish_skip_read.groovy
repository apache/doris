...
suite("test_delete_publish_skip_read", "nonConcurrent") {
    ...
    try {
        ...
        t1.join()
        t2.join()
        // Wait until the delete of k1=2 is actually published and visible instead of guessing a
        // fixed duration: when publish is delayed (e.g. the block-disable HTTP is queued behind the
        // in-flight stream load and only clears at the FE publish timeout ~152s), a fixed sleep can
        // read before the delete's version becomes visible. The normal-read count is monotonic 3 -> 2.
        Awaitility.await().atMost(180, TimeUnit.SECONDS).pollInterval(1, TimeUnit.SECONDS).until {
