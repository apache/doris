...
suite("test_explain_tpch_sf_1_q20", "tpch_sf1") {
    explain {
		...
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[Tables are not in the same group]\n" + 
				"  |  equal join conjunct: `s_suppkey` = `ps_suppkey`\n" + 
				"  |  runtime filters: RF000[in_or_bloom] <- `ps_suppkey`") && 
		explainStr.contains("output slot ids: 18 19 \n" + 
		...
		explainStr.contains("output slot ids: 17 18 19 \n" + 
				"  |  hash output slot ids: 17 18 19 ") && 
		explainStr.contains("TABLE: supplier(supplier), PREAGGREGATION: ON\n" + 
				"     runtime filters: RF000[in_or_bloom] -> `s_suppkey`, RF001[in_or_bloom] -> `s_nationkey`") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[The src data has been redistributed]\n" + 
				"  |  equal join conjunct: `ps_suppkey` = <slot 9> `l_suppkey`\n" + 
				"  |  equal join conjunct: `ps_partkey` = <slot 8> `l_partkey`\n" + 
				"  |  other join predicates: `ps_availqty` > 0.5 * <slot 10> sum(`l_quantity`)") && 
		explainStr.contains("other join predicates: `ps_availqty` > 0.5 * <slot 10> sum(`l_quantity`)\n" + 
		...
		explainStr.contains("output slot ids: 15 \n" + 
				"  |  hash output slot ids: 15 14 10 ") && 
		explainStr.contains("join op: LEFT SEMI JOIN(BROADCAST)[Tables are not in the same group]\n" + 
				"  |  equal join conjunct: `ps_partkey` = `p_partkey`\n" + 
				"  |  runtime filters: RF004[in_or_bloom] <- `p_partkey`") && 
		explainStr.contains("output slot ids: 3 14 15 \n" + 
				"  |  hash output slot ids: 3 14 15 ") && 
		explainStr.contains("TABLE: partsupp(partsupp), PREAGGREGATION: ON\n" + 
				"     runtime filters: RF002[in_or_bloom] -> `ps_suppkey`, RF003[in_or_bloom] -> `ps_partkey`, RF004[in_or_bloom] -> `ps_partkey`") && 
