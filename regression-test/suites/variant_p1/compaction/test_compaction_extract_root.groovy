...
suite("test_compaction_extract_root", "p1") {
    ...
    sql """insert into ${tableName}  select 0, parse_to_variant('{"a": 11245, "b" : {"state" : "open", "code" : 2}}')  as json_str
        union  all select 8, parse_to_variant('{"a": 1123}') as json_str union all (select 0, parse_to_variant('{"a" : 1234, "xxxx" : "aaaaa"}') as json_str from numbers("number" = "4096") limit 4096) ;"""


    sql """insert into ${tableName} select 1, parse_to_variant('{"a": 11245, "b" : {"state" : "colse", "code" : 2}}')  as json_str
    ...
    sql """insert into ${tableName} select 2, parse_to_variant('{"a": 11245, "b" : {"state" : "flat", "code" : 3}}')  as json_str
    ...
    sql """insert into ${tableName}  select 3, parse_to_variant('{"a" : 1234, "xxxx" : 4, "point" : 5}')  as json_str
