...
suite("test_ranger_access_resource_column", "p2,ranger,external") {
	...
	def checkColumnAccess = { catalogType, access, user, password, catalog, dbName, tableName, columns ->
		connect("$user", "$password", "$defaultJdbcUrl") {
			...
			executeSqlWithLogging("""SELECT ${columns} FROM ${catalog}.${dbName}.${tableName}""", "Error executing SELECT")
		}
	}
	...
	if (enabled != null && enabled.equalsIgnoreCase("true")) {
		...
		// prepare user
		List<String> userList = ['ranger_test_column_user_1', 'ranger_test_column_user_2', 'ranger_test_column_user_3',
		                         'ranger_test_column_user_4', 'ranger_test_column_user_5']
		...
		resource.put("table", new RangerPolicy.RangerPolicyResource(internalTable))
		resource.put("column", new RangerPolicy.RangerPolicyResource(["c3"], false, false))
		...
		checkColumnAccess("internal", "deny", userList[0], pwd, "internal", internalDb, internalTable, "*")
		rangerClient.deletePolicy(rangerServiceName, policy1)
		...
		resource.put("table", new RangerPolicy.RangerPolicyResource(catalogTable))
		resource.put("column", new RangerPolicy.RangerPolicyResource(["c1", "c2"], false, false))
		...
		checkColumnAccess("hive", "deny", userList[1], pwd, catalog1, catalogDb, catalogTable, "c3")
		checkColumnAccess("hive", "deny", userList[1], pwd, catalog1, catalogDb, catalogTable, "c1,c3")
		checkColumnAccess("hive", "deny", userList[1], pwd, catalog1, catalogDb, catalogTable, "*")
		rangerClient.deletePolicy(rangerServiceName, policy2)
		...
		checkColumnAccess("internal", "deny", userList[2], pwd, "internal", internalDb, internalTable, "c1")
		checkColumnAccess("internal", "deny", userList[2], pwd, "internal", internalDb, internalTable, "*")
		rangerClient.deletePolicy(rangerServiceName, policy3)
		...
		checkColumnAccess("hive", "deny", userList[3], pwd, catalog1, catalogDb, catalogTable, "*")
		checkColumnAccess("internal", "allow", userList[3], pwd, "internal", internalDb, internalTable, "c1,c2")
		checkColumnAccess("internal", "allow", userList[3], pwd, "internal", internalDb, internalTable, "c2")
		checkColumnAccess("internal", "deny", userList[3], pwd, "internal", internalDb, internalTable, "c3")
		checkColumnAccess("internal", "deny", userList[3], pwd, "internal", internalDb, internalTable, "*")
		rangerClient.deletePolicy(rangerServiceName, policy4)
		...
		resource.put("database", new RangerPolicy.RangerPolicyResource("*"))
		resource.put("table", new RangerPolicy.RangerPolicyResource("*"))
		resource.put("column", new RangerPolicy.RangerPolicyResource(["c1", "c2"], true, false))
		policy.setResources(resource)
		...
		checkColumnAccess("internal", "allow", userList[4], pwd, "internal", internalDb, internalTable, "c3")
		checkColumnAccess("internal", "deny", userList[4], pwd, "internal", internalDb, internalTable, "c1")
		checkColumnAccess("internal", "deny", userList[4], pwd, "internal", internalDb, internalTable, "*")
		rangerClient.deletePolicy(rangerServiceName, policy5)
