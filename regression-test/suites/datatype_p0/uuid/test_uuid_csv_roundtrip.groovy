...
suite("test_uuid_csv_roundtrip", "p0,external") {
    ...
    if (localMode) {
        ...
        outputPath = "s3://${context.config.otherConfigs.get('s3BucketName')}/uuid_file_csv/${token}_"
        sinkProperties = """
    ...
    String projection = "id, u"
    String properties = sinkProperties.isEmpty() ? "" : "PROPERTIES(${sinkProperties})"
    sql """SELECT ${projection} FROM uuid_file_csv ORDER BY id
           INTO OUTFILE "${outputPath}csv_" FORMAT AS csv ${properties}"""
    String tvf = localMode ? "local" : "s3"
    String pathKey = localMode ? "file_path" : "uri"
    String path = localMode
            ? "${context.config.otherConfigs.get('uuidLocalTvfPath')}_${token}_csv_*"
            : "${outputPath}csv_*"
    sql "DROP TABLE IF EXISTS uuid_file_reload_csv"
    sql "CREATE TABLE uuid_file_reload_csv LIKE uuid_file_csv"
    String columns = "(id,u)"
    sql """INSERT INTO uuid_file_reload_csv ${columns}
           SELECT * FROM ${tvf}("${pathKey}"="${path}", "format"="csv", ${sourceProperties})"""
    qt_roundtrip "SELECT ${projection} FROM uuid_file_reload_csv ORDER BY id"
