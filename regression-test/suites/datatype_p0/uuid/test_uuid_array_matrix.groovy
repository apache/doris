...
suite("test_uuid_array_matrix", "p0") {
    ...
    sql """CREATE TABLE uuid_matrix_array (${uuidMatrixSchema()})
           DUPLICATE KEY(id) DISTRIBUTED BY HASH(id) BUCKETS 1 PROPERTIES('replication_num'='1')"""
    sql "INSERT INTO uuid_matrix_array VALUES ${uuidMatrixValues()}"

    uuidRunMatrix('unary', 'uuid_matrix_array', ['a'], { a ->
    ...
    uuidRunMatrix('sets', 'uuid_matrix_array', ['a','a2'], { a,b ->
    ...
    uuidRunMatrix('element', 'uuid_matrix_array', ['a','u'], { a,u ->
    ...
    uuidRunMatrix('capture', 'uuid_matrix_array', ['a','u'], { a,u ->
    ...
    uuidRunMatrix('index', 'uuid_matrix_array', ['a','idx'], { a,n ->
        [element_value: "ELEMENT_AT(${a},${n})", subscript_value: "(${a})[${n}]", sliced: "ARRAY_SLICE(${a},${n})"]
    })
    uuidRunMatrix('slice', 'uuid_matrix_array', ['a','idx','num'], { a,n,k ->
        [sliced: "ARRAY_SLICE(${a},${n},${k})"]
    })
    uuidRunMatrix('repeat', 'uuid_matrix_array', ['u','num'], { u,n ->
        [repeated: "ARRAY_REPEAT(${u},${n})", with_constant: "ARRAY_WITH_CONSTANT(${n},${u})"]
    })
    uuidRunMatrix('seed', 'uuid_matrix_array', ['a','num'], { a,n ->
        [shuffled_members: "ARRAY_SORT(ARRAY_SHUFFLE(${a},${n}))"]
    })
    uuidRunMatrix('constructor', 'uuid_matrix_array', ['u','v','w'], { u,v,w ->
        [array_value: "ARRAY(${u},${v},${w})"]
    })
    uuidRunMatrix('aligned', 'uuid_matrix_array', ['a','a2'], { a,b ->
    ...
    uuidRunMatrix('split', 'uuid_matrix_array', ['a','flags'], { a,b ->
    ...
    for (String function : ['EXPLODE','EXPLODE_OUTER']) {
        uuidRunMatrix(function.toLowerCase(), 'uuid_matrix_array', ['a'], { a -> [element: 'e'] },
            [lateral: { a -> "LATERAL VIEW ${function}(${a}) generated AS e" }, orderBy: 'id,e'])
    }
    for (String function : ['POSEXPLODE','POSEXPLODE_OUTER']) {
        uuidRunMatrix(function.toLowerCase(), 'uuid_matrix_array', ['a'], { a -> [position: 'pos', element: 'e'] },
