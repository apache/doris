...
suite("test_nested_types_insert_into_with_localtvf", "p0") {
    ...
    def dataFilePath = context.config.dataPath + "/datatype_p0/nested_types/query"
    ...
    if (backends.size() > 1) {
        ...
        for (List<Object> backend : backends) {
            ...
            for (int i = 0; i < orcFiles.size(); ++i) {
                scpFiles("root", be_host, orcFiles[i], outFilePath, false)
                orcFiles.set(i, StringUtils.replace(orcFiles[i], dataFilePath, outFilePath))
            }
            for (int i = 0; i < parquetFiles.size(); ++i) {
                scpFiles("root", be_host, parquetFiles[i], outFilePath, false)
                parquetFiles.set(i, StringUtils.replace(parquetFiles[i], dataFilePath, outFilePath))
            }
        }
    }
    ...
    def nested_table_dup = "tbl_array_nested_types_tvf"
    ...
    def nested_table_dup2 = "tbl_array_nested_types_tvf2"
    ...
    def nested_table_array_map_dup = "tbl_array_map_types_tvf"
    ...
    def nested_table_map_dup = "tbl_map_types_tvf"
    ...
    def nested_table_map_array_dup = "tbl_map_array_types_tvf"
    ...
    // step1. select * from local_tvf with 0-100 items
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        select count() from local(
            "file_path" = "${orcFiles[i]}",
            "backend_id" = "${be_id}",
            "format" = "orc");"""

        sql """
        insert into ${table_names[i]} select * from local (
            "file_path" = "${orcFiles[i]}",
            "backend_id" = "${be_id}",
             "format" = "orc");"""

        qt_sql_arr_orc_doris """ select count() from ${table_names[i]}; """
    }

    for (int i = 0; i < 2; ++i) {
        qt_sql_arr_parquet_tvf """
        select count() from local(
            "file_path" = "${parquetFiles[i]}",
            "backend_id" = "${be_id}",
            "format" = "parquet");"""
        ...
        qt_sql_arr_parquet_doris """ select count() from ${table_names[i]}; """
    }
    ...
    for (int i = 2; i < orcFiles.size(); ++i) {
        ...
        insert into ${table_names[i]} (k1, c_bool, c_tinyint,  c_smallint, c_int, c_bigint, c_float, c_double, c_decimal, c_decimalv3, c_date, c_datetime, c_datev2,c_datetimev2, c_varchar, c_string)  
            select k1, c_bool, c_tinyint,  c_smallint, c_int, c_bigint, c_float, c_double, c_decimal, c_decimalv3, c_date, c_datetime, c_datev2,c_datetimev2, c_varchar, c_string from local (
                "file_path" = "${orcFiles[i]}",
                "backend_id" = "${be_id}",
                "format" = "orc");"""
    ...
    qt_sql_arr """ select array_size(c_tinyint) from tbl_array_nested_types_tvf order by k1; """
    qt_sql_arr """ select array_size(c_tinyint) from tbl_array_nested_types_tvf2 order by k1; """
    qt_sql_arr """ select array_size(c_tinyint) from tbl_array_map_types_tvf order by k1; """
    qt_sql_map_arr """ select array_size(map_values(c_tinyint)) from tbl_map_array_types_tvf order by k1; """
