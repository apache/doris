...
suite("modify_partition_storage_policy") {
    ...
    // Unique Key table enables merge-on-write can not modify storage policy
    sql """
    ...
    def metadata_url = tablet_result[0][19]
    def (code, out, err) = curl("GET", metadata_url)
    def json = parseJson(out)
    def be__policy_id1 = Long.parseLong(json.storage_policy_id.toString())

    // assert is equal: storage id in fe & be meta
    def policy_id = Long.parseLong(get_policy_id_from_name.call(policy_name1).toString())
    ...
    def metadata_url2 = tablet_result2[0][19]
    def (code2, out2, err2) = curl("GET", metadata_url2)
    def json2 = parseJson(out)
    def be_policy_id2 = Long.parseLong(json2.storage_policy_id.toString())
