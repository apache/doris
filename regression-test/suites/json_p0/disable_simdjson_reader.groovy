...
suite("disable_simdjson_reader", "p0") {

    def backendId_to_backendIP = [:];
    def backendId_to_backendHttpPort = [:]
    def backendId_to_enable_simdjson_reader = [:]
    getBackendIpHttpPort(backendId_to_backendIP, backendId_to_backendHttpPort);

    // set enable_simdjson_reader=false in all be 
    for (String id in backendId_to_backendIP.keySet())
    {
        ...
        for (Object ele in (List) configList) {
            ...
            if (((List<String>) ele)[0] == "enable_simdjson_reader") {
                logger.info("In Backend:"+id+" ,old value enable_simdjson_reader="+((List<String>) ele)[2])
                // save old value, restore when done
                backendId_to_enable_simdjson_reader.put(id, ((List<String>) ele)[2])
            }
        }
        // set enable_simdjson_reader=false
        (code, out, err) = curl("POST", String.format("http://%s:%s/api/update_config?%s=%s", beIp, bePort, "enable_simdjson_reader", "false"))
        ...
        for (Object ele in (List) configList) {
            if (((List<String>) ele)[0] == "enable_simdjson_reader") {
                logger.info("In Backend:"+id+" ,new value enable_simdjson_reader="+((List<String>) ele)[2])
                assertFalse(Boolean.parseBoolean(((List<String>)ele)[2]))
            }
        }
    }
    ...
    // restore enable_simdjson_reader old value to all be 
    for (String id in backendId_to_backendIP.keySet())
    {
        def beIp = backendId_to_backendIP.get(id)
        def bePort = backendId_to_backendHttpPort.get(id)

        String boolStr = Boolean.parseBoolean(backendId_to_enable_simdjson_reader.get(id)) ? "true" : "false" 

        def (code, out, err) = curl("POST", String.format("http://%s:%s/api/update_config?%s=%s", beIp, bePort, "enable_simdjson_reader", boolStr))
        assertTrue(out.contains("OK"))
    }
