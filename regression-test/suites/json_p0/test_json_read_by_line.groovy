...
suite("test_json_read_by_line", "p0") {
    ...
    // define a sql table
    def testTable = "tbl_read_by_line"
    def dataFile = "read_by_line.json"
    ...
    streamLoad {
        ...
        set 'format', 'json'
        set 'read_json_by_line', 'true'
        set 'jsonpaths', '[\"$.id.abc\", \"$.city.abc\", \"$.code.c1\"]'
        // if declared a check callback, the default check condition will ignore.
        // So you must check all condition
        check { result, exception, startTime, endTime ->
            if (exception != null) {
