...
suite("test_search_cache", "p0") {
    ...
    def result1 = sql """
        SELECT /*+SET_VAR(enable_segment_limit_pushdown=true,enable_inverted_index_query_cache=true) */
        id FROM ${tableName}
    ...
    def result_no_cache = sql """
        SELECT /*+SET_VAR(enable_segment_limit_pushdown=true,enable_inverted_index_query_cache=false) */
        id FROM ${tableName}
        WHERE search('title:apple')
        ORDER BY id
    """
