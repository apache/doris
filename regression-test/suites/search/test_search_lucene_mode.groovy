 ...
 *
 * Enable Lucene mode with options parameter:
 *   search(dsl, default_field, default_operator, '{"mode":"lucene"}')
 */
suite("test_search_lucene_mode") {
    ...
    qt_lucene_and """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND banana', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_or """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple OR date', 'title', 'or', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_complex_and_or """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND banana OR cherry', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_min_should_match_1 """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND banana OR cherry', 'title', 'and', '{"mode":"lucene","minimum_should_match":1}')
        ORDER BY id
    """
    ...
    qt_lucene_not """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('NOT apple', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_and_not """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND NOT banana', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_or_not """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple OR NOT banana', 'title', 'or', '{"mode":"lucene","minimum_should_match":1}')
        ORDER BY id
    """
    ...
    qt_lucene_or_only """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple OR date OR fig', 'title', 'or', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_cross_field """
        ...
        FROM ${tableName}
        WHERE search('title:apple AND category:fruit', null, 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_phrase """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('"apple banana"', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_lucene_wildcard """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('app* AND ban*', 'title', 'and', '{"mode":"lucene"}')
        ORDER BY id
    """
    ...
    qt_empty_options """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND banana', 'title', 'and', '{}')
        ORDER BY id
    """
    ...
    qt_lucene_min_should_match_0 """
        SELECT /*+SET_VAR(enable_common_expr_pushdown=true) */ id, title
        FROM ${tableName}
        WHERE search('apple AND banana OR date', 'title', 'and', '{"mode":"lucene","minimum_should_match":0}')
        ORDER BY id
    """
