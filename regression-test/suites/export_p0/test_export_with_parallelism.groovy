...
suite("test_export_with_parallelism", "p0") {
        ...
        while (true) {
            ...
            if (res[0][2] == "FINISHED") {
                ...
                for (int idx = 0; idx < parallelism && idx < 3; ++idx) {
                    ...
                    log.info("outfile_path: ${json.url[idx]}")
                    resultUrl.add(json.url[idx])
                }
    ...
    def test_export = {format, file_suffix, isDelete, parallelism, whereExpr ->
        ...
        logger.info("outfile_url_list = " + outfile_url_list);
        ...
        for (int j = 0; j < parallelism && j < 3; ++j) {
            ...
            sql """ insert into ${table_load_name}
                        select * from s3(
                        "uri" = "http://${bucket}.${s3_endpoint}${outfile_url_list.get(j).substring(5 + bucket.length())}0.${file_suffix}",
                        "s3.access_key"= "${ak}",
                        "s3.secret_key" = "${sk}",
                        "format" = "${format}",
                        "region" = "${region}"
        ...
        order_qt_select """ select * from ${table_load_name} order by id; """
    }
    ...
    test_export('parquet', 'parquet', true, 6,  """ where name != 'daidai-1'  """);
