...
suite("test_outfile_date_year_zero", "p0") {
    ...
    sql """ DROP TABLE IF EXISTS test_outfile_date_year_zero_table """
    sql """
        CREATE TABLE test_outfile_date_year_zero_table (
            `id` INT NOT NULL,
            `d` DATE NULL
        ) ENGINE=OLAP
        DUPLICATE KEY(`id`)
    ...
    sql """
        INSERT INTO test_outfile_date_year_zero_table VALUES
    ...
    // Cast to string: the MySQL JDBC driver cannot materialise a year-zero DATE as java.sql.Date.
    order_qt_source """
        SELECT CAST(id AS STRING), CAST(d AS STRING) FROM test_outfile_date_year_zero_table
    """

    def outfile_to_S3 = { format ->
        def res = sql """
            SELECT * FROM test_outfile_date_year_zero_table t
    ...
    // Both read-backs must return exactly the source rows: same values, same NULL.
    order_qt_readback_parquet(outfile_to_S3("parquet"))
    order_qt_readback_orc(outfile_to_S3("orc"))
}
