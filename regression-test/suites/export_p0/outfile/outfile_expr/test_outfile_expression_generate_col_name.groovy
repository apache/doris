...
suite("test_outfile_expression_generate_col_name", "p0") {
    ...
    def create_table = {
        ...
        sql """
            CREATE TABLE IF NOT EXISTS outfile_expr_export_test (
            `id` int(11) NULL,
            `name` string NULL,
            `age` int(11) NULL
            )
    ...
    def check_outfile_data = { outfile_url, outfile_format ->
        order_qt_select_tvf """ SELECT * FROM S3 (
                            "uri" = "http://${s3_endpoint}${outfile_url.substring(4)}0.${outfile_format}",
                            "ACCESS_KEY"= "${ak}",
                            "SECRET_KEY" = "${sk}",
    ...
    def test_q3 = { outfile_format ->
        ...
        """
        def outfile_url = res[0][3]
        println ("outfile_url outfile_url " + outfile_url)
