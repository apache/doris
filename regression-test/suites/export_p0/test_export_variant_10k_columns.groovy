...
suite("test_export_variant_10k_columns", "p0") {
    ...
    try {
        ...
        int max_retry = 60
        while (max_retry-- > 0) {
            def rows = sql "SHOW LOAD WHERE Label = '${load_label}' ORDER BY CreateTime DESC LIMIT 1"
            if (rows.size() > 0) {
                def state = rows[0][2]
                if (state == "FINISHED") {
                    break
                } else if (state == "CANCELLED") {
                    logger.error("Load failed: " + rows[0][7])
                    throw new RuntimeException("Load failed: " + rows[0][7])
                }
            }
            sleep(1000)
        }
