...
if [[ "${BUILD_FE}" -eq 1 ]]; then
    ...
    for _fs_mod in s3 oss cos obs azure hdfs-base hdfs oss-hdfs jfs local broker; do
...
if [[ "${BUILD_FE}" -eq 1 ]]; then
    ...
    for fs_module in s3 azure oss cos obs hdfs oss-hdfs jfs local broker; do
