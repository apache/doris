...
if [[ "${BUILD_FE}" -eq 1 ]]; then
    ...
    for conn_module in es jdbc maxcompute trino hms hive paimon hudi iceberg adbc; do
        ...
        for conn_conf_tpl in "${conn_plugin_target}"/*.conf.template; do
            ...
            # BSD cp -n returns 1 when the destination exists, which aborts this script under set -e.
            # Preserve existing files and symlinks without invoking cp in that case.
            if [[ ! -e "${conn_conf_tpl%.template}" && ! -L "${conn_conf_tpl%.template}" ]]; then
