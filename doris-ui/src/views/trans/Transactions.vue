...
<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>Transations</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    goto_monitor(row, event, column){
      console.log("DbId = "+ row.DbId)
       self.$router.push({ 
         path: '/trans/Transactions_list/'+row.DbId.replace(/%/g, '%25') 
        })
    },
    // 获取Frontend列表
    getdata(parameter) {
      this.loading = true
      transationsList(parameter)
        .then(res => {
          this.loading = false
