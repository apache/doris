...
## 2026-07-20 — session 8：缓存隔离落地（越权修复，RD-4 完成，读写重构主线收官）

- **动码前设计先行 + grounding**（workflow `wf_ffad1480-f41`，6 读者 recon + 3 安全断言对抗验证，并自行 clean-room 读码交叉核对）。**取证对原计划三处纠正**：①真正**活跃**的越权只 2 个——iceberg `latestSnapshotCache`（`beginQuerySnapshot` 命中不走 per-user loadTable）+ fe-core schema 缓存（`SchemaCacheKey` 只按表名、无 `shouldBypassSchemaCache`）；`partitionCache`/`formatCache` 其实**非活跃漏**（读取点都先调 per-user `resolveTableForRead`/`resolveTable`，因 session=user 下 tableCache=null 现载重鉴权），今天安全但**脆弱**（靠上游恰好先载表）。②`commentCache` 早已对 session=user 排除，是同类威胁的**先例**。③**异构 HMS 网关是"潜在"非"活跃"洞**——兄弟被 `IcebergSiblingProperties.synthesize` 无条件强制 `iceberg.catalog.type=hms`，永不满足 session=user（REST-flavor 门），泄漏今天不可达；原计划"传属主标签到 fe-core 分片"被证伪（标签只是类型判别串、非身份/能力，且不桥接 fe-core 跨语句缓存）。
- **中文方案 + 三决策交用户签字**（不引任务代号）：讲清 list≠load 为何越权、禁用 vs 分片对比（禁用更简单/无新 SPI/无撤权陈旧窗口/贴合 Doris 现状——session=user 本就是"每次现载原始表"档位，且 Doris 无 Trino 的缓存之上访问控制层）、影响面（仅 REST+session=user + fe-core schema）。**用户签字**：①**禁用路线**（非身份分片）；②**三投影缓存统一处理**（成"session=user ⇒ iceberg 无活跃跨查询元数据缓存"不变量）；③**网关加 fail-loud 守卫**。威胁模型签字点=禁用路线下无撤权陈旧窗口、泄漏为元数据非凭证/数据。
- **落地（4 独立 commit，各自绿）**：
  - **4a iceberg 三缓存禁用**（`9f88aa4`）：`latestSnapshotCache`/`partitionCache`/`formatCache` 在 `isUserSessionEnabled()` 时置空；`beginQuerySnapshot` 加 null 回退（抽 `loadLatestSnapshotPin`，镜像 `resolveTableForRead` 的 tableCache 三元）；`invalidate*` 三处补 null 守卫（原无条件、null 化后会 NPE）；partition/format 读取点已 null-safe 无需改。测试 22（含三缓存 session=user 置空 + 非 session 仍建 + null 缓存失效不抛）。
  - **4b fe-core schema bypass**（`92289e3`）：新增 `ExternalCatalog.shouldBypassSchemaCache(SessionContext)`(默认 false) + `PluginDrivenExternalCatalog` override（判据 `supportsUserSession() && ctx.hasDelegatedCredential()`，与库/表名缓存 bypass 完全一致）；`ExternalTable.getSchemaCacheValue()` bypass 时 `initSchemaAndUpdateTime` 现读、不碰共享缓存（覆盖 MVCC latest 路径经 `super.getSchemaCacheValue()`）。测试 6（谓词 + 读路径 bypass）。
  - **4c hive 网关 fail-loud 守卫**（`c40af11`）：`getOrCreateIcebergSibling` 建成兄弟后若 `getCapabilities()` 含 `SUPPORTS_USER_SESSION` 即抛（fe-core bypass 只看前门 hive 能力→session=user 兄弟会静默泄漏）。今天恒不触发（兄弟强制 hms）。测试 2 + `FakeSibling` 加可选能力集。
  - **4d 防漂移门禁**（`5cb5fb9`）：`tools/check-authz-cache-sharding.sh`（marker 门禁，仿 funnel 门禁）强制 IcebergConnector 每个 `final *Cache`/`Cache<`/`LoadingCache<` 持有字段带 `authz-cache-session-user-disabled` 或 `authz-cache-exempt` 标记（decl 行或上一行）；+ RED/GREEN 自测 + 挂 fe-connector pom validate（inherited=false，与 import 门禁并列）。五缓存带 disabled、manifest 带 exempt。
- **clean-room 对抗复审**（workflow `wf_e89da31d-9fe`，4 维度 + 对抗验证）：**0 blocker/major**——安全修复本体（null 安全完整性、泄漏是否真关闭、schema bypass 是否覆盖 MVCC 全路径、非 session 字节中性）全 clean。2 findings 均针对**门禁本身**（非修复）：minor=门禁正则原只匹配 `Iceberg*Cache` 会漏 raw `Cache<`/`LoadingCache<`/静态/非 private 形态→**已修**（正则放宽到任意 `final *Cache`/`Cache<` 持有字段 + 自测补齐这些形态，实测真实文件仍精确匹配 6 字段零误报）；nit=marker 只认 decl 行或上一行（fail-closed 安全、与 funnel 门禁一致）→保持。
- **验证**：116 目标单测全绿（iceberg 22+29+51 / fe-core 6 / hive 16 等）；checkstyle 0；三门禁（fecore-funnel + connector-imports + authz-cache-sharding）exit 0；门禁自测 PASS。异构网关 + 越权 e2e 沿用"择机统一补"。
- **读写重构主线（RD-1→RD-4 / STEP 1–4）至此全部完成**：读取键石 + HMS 兄弟扇出 + 写入共用 + 缓存隔离。定稿 → `designs/P4-cache-isolation-implementation-design.md`。
- **下一步**：见 HANDOFF —— 主线四步已收官；剩欠账 = 统一补 e2e（异构网关 INSERT/DELETE/MERGE/ALTER/EXECUTE + can-list-cannot-load 越权命中不泄漏），落 `regression-test/suites/external_table_p2/refactor_catalog_param`。
