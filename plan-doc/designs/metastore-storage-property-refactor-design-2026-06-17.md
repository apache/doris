...
**测试（决策驱动，强制）**
- **T1 新旧等价性**：对 S3/OSS/COS/OBS/Azure/HDFS 代表输入，断言新 `toHadoopConfigurationMap()` / `toBackendProperties().toMap()` 与 fe-core 旧产物 key/value 全等（含默认调优值）。这是切换的回归闸（背景报告指出当前**缺**此测试）。
