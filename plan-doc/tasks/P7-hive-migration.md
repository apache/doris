...
| **P7.3-T07** | fe-core + 插件（读侧 ACID） | 移 `AcidUtil`/`HiveTransaction`(读侧)/`AcidInfo`/`DeleteDeltaInfo` 入插件；valid-write-ids 线程；**修 delete-delta `setFileNames` 回归**（`HiveScanRange.populateTransactionalHiveParams` 填回文件名）。锁保持无 heartbeat（OQ-LOCK）。 |
