...
- **状态**：🚧 进行中（**设计已批准 2026-06-06 [D-023]**；Batch A 待启）
...
| P4-T02 | 连接器 **分区**：impl `listPartitions/listPartitionNames/listPartitionValues`（港 ODPS `getPartitions` + 连接器自有 client/cache）| **A** gate 关 | ⏳ | 翻闸后 SHOW PARTITIONS / 分区 TVF 依赖；确认是否需自有 MetaCache（OQ-4）|
| P4-T03 | 连接器 **写/事务 SPI**：`ConnectorWriteOps.beginTransaction` + `ConnectorTransaction`（港 `MCTransaction`：`addCommitData` 反序列化 `TMCCommitData`、block 分配、begin/finish/commit/rollback、getUpdateCnt）| **B** gate 关 | ⏳ | over W4 委派；协议 `TBinaryProtocol` 对齐（golden 红线）|
| P4-T04 | 连接器 **写计划**：`Connector.getWritePlanProvider` → `planWrite` 产 `TMaxComputeTableSink`（填 W5 write-context seam：txn_id/write_session_id/static_partition_spec/block_id；港 legacy `MaxComputeTableSink` config-read）| **B** gate 关 | ⏳ | over W5 opaque-sink；runtime txn_id/write_session_id 注入路径需在 plugin-driven 侧重建（DV-009 / OQ-2）|
...
- **OQ-2（write-context 填充）**：W5 留**空** context（base `InsertCommandContext` 无通用 overwrite/静态分区）；runtime `txn_id`/`write_session_id` 原经 `MCInsertExecutor.beforeExec` 注入，需在 plugin-driven 写侧**重建**该注入路径 → **P4-T04 核心难点**（DV-009 标注）。
- **OQ-3（反向引用穷举）**：本 session re-grep 得 ~19（含全部 live），但 category-C 注册站点（gson/enum/metacache 等）未穷举 → **P4-T07 入口先完整 re-grep**。
- **OQ-4（连接器缓存层）**：`MaxComputeExternalMetaCache`(115)/`MaxComputeSchemaCacheValue`(67) 是 fe-core legacy；连接器分区/schema 是否需自有缓存，还是 SPI 层已覆盖 → **P4-T02 入口确认**。
