...
- **状态**：🚧 进行中（**设计已批准 2026-06-06 [D-023]**；**Batch A ✅ + Batch B ✅**（T01–T04 全完成，gate 关 dormant）；**Batch C 设计 ✅ 签字 [D-026]**；**T05 ✅ 实现 gate-green（待 commit，用户定时机）**，下一 = **T06（live，两 commit）**，前置 R-004 防御测）
...
| P4-T06 | **翻闸**：`CatalogFactory.SPI_READY_TYPES += "max_compute"` + 删 `CatalogFactory` case（:146）+ **插件 harness ODPS 连通性防御测（R-004）** | **C** **live cutover** | ⏳ | **设计✅[D-026]**：两 commit（`[P4-T06a]` 写接线 W-a..d+静态分区/overwrite 绑定+R-004 隔离测 dormant；`[P4-T06b]` flip 末提）；2 SPI 新增（`ConnectorSession.setCurrentTransaction`/`ConnectorWriteOps.usesConnectorTransaction`）。翻闸即 live；R-004 part-2 用户跑、过方算完成 |
