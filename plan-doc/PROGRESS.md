...
> 最后更新：**2026-06-06** | 当前阶段：**P4 maxcompute 启动·scope=C（写-SPI RFC 先行）**——三写者（mc/hive/iceberg）写能力完整调研 + 写/事务 SPI RFC 已出并经用户批准；下一步 W-phase 共享解耦（behind gate、零行为变更）。P3 hybrid 已 **#64143 合入** `branch-catalog-spi`（`5c240dc7a34`）| 项目总进度：**34%**
...
| P4 | maxcompute 迁移 | 2 周 | ▰▰▱▱▱▱▱▱▱▱ 15% | 🚧 **scope=C**；W-phase **W1+W2 ✅**（写/事务 SPI 面 + fe-core `Transaction` 泛化 + 3 txn override，gate 内零行为变更）；W3–W7 待续 | [RFC](./tasks/designs/connector-write-spi-rfc.md) |
...
- **本 session 已完成**：W-phase **W1+W2**（写/事务 SPI 面 + fe-core `Transaction` 泛化 + 3 txn override，behind gate、零行为变更、golden 等价 by construction）；fe-connector-api + fe-core 的 compile/checkstyle/import-gate 全绿（真实 exit code 核验）。**未提交**（待用户决定提交时机/粒度）
- **下一个 session 应做**：W-phase **W3**（解耦 `Coordinator`/`LoadProcessor` 3 cast → `txn.addCommitData(TSerialize)`、`FrontendServiceImpl` instanceof → `supportsWriteBlockAllocation`/`allocateWriteBlockRange`，删 3 concrete-type import）+ **W6 golden 测**，再 W4/W5/W7。精确锚点见 [HANDOFF](./HANDOFF.md)。**不翻闸/不搬类/不删 legacy**（P4 adopter 阶段）
- **是否需要 handoff**：**是**——本场已 rewrite [HANDOFF.md](./HANDOFF.md)（W1/W2 完成 + W3 精确行号锚点 + TBinaryProtocol 序列化契约 + maven 绝对-f / 真实-exit-code 坑）
