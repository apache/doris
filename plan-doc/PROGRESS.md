...
> 最后更新：**2026-06-06** | 当前阶段：**P4 maxcompute·scope=C（写-SPI RFC 先行）**——写/事务 SPI RFC 已批准；W-phase **W1+W2**(committed `be945476ba7`) + **W3+W6**(本场：解耦 3 热路径 cast/instanceof 走多态 `Transaction` + 共享 `CommitDataSerializer` + golden 测，behind gate、零行为变更、未提交) ✅；下一步 W4/W5（plugin-driven 写接线）+ W7（文档）。P3 hybrid 已 **#64143 合入** `branch-catalog-spi`（`5c240dc7a34`）| 项目总进度：**34%**
...
| P4 | maxcompute 迁移 | 2 周 | ▰▰▰▱▱▱▱▱▱▱ 20% | 🚧 **scope=C**；W-phase **W1+W2 ✅**(committed `be945476ba7`) + **W3+W6 ✅**(解耦 3 热路径 cast/instanceof + golden 测，gate 内零行为变更，未提交)；W4/W5/W7 待续 | [RFC](./tasks/designs/connector-write-spi-rfc.md) |
...
- **本 session 已完成**：W-phase **W3+W6**（解耦 3 热路径 cast/instanceof 走多态 `Transaction` + 共享 `CommitDataSerializer` + golden 等价测，behind gate、零行为变更、golden by TDD：RED 故意错协议→GREEN）；compile BUILD SUCCESS + 9 测全过 + checkstyle 0 + import-gate（真实 exit code 核验）。**关键修正**：`getTxnById` 抛异常非返 null，故 guard 在 "任一 commit 字段 set" 内。**未提交**（W1+W2 已是 `be945476ba7`；W3+W6 应独立 commit）
- **下一个 session 应做**：W-phase **W4**（`PluginDrivenTransaction` override 4 default 委派 SPI `ConnectorTransaction`）+ **W5**（新 `PluginDrivenTableSink` + `PhysicalPlanTranslator.visitPhysicalXxxTableSink` → `ConnectorWritePlanProvider.planWrite`，定 `ConnectorWriteHandle`/`ConnectorSinkPlan` 最终字段）+ **W7**（D-021/D-022 入 decisions-log + 01-spi-rfc E11）。精确锚点见 [HANDOFF](./HANDOFF.md)。**不翻闸/不搬类/不删 legacy**（P4 adopter 阶段）
- **是否需要 handoff**：**是**——本场已 rewrite [HANDOFF.md](./HANDOFF.md)（W3+W6 完成 + getTxnById guard 红线 + TDD RED/GREEN 机制 + W4/W5/W7 锚点）
