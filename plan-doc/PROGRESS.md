...
> 最后更新：**2026-05-25** | 当前阶段：**P1 scan-node 收口 + 重复清理**（P0 已合入 `branch-catalog-spi`，PR #63582 全绿，新分支 `catalog-spi-02`） | 项目总进度：**16%**
...
| **P1** | scan-node 收口 + 重复清理 | 1 周 | ▰▰▰▰▰▰▱▱▱▱ 60% | 🚧 进行中（批 A=T3+T4+T5 已完成；批 B=T1 待启动；T2 推迟 P4/P5）| [tasks/P1](./tasks/P1-scan-node-cleanup.md) |
...
| P1-T01 | 删除 13 个 `Jdbc*Client.java` + `JdbcFieldSchema.java` | 批 B | @me | ⏳ | — | 先解耦 3 个外部 caller |
| P1-T02 | 重复 PaimonPredicateConverter + McStructureHelper 处理 | 🚫 | — | 🚫 | — | 用户决议 Q2 推迟到 P4/P5 |
...
- **本 session 已完成**：P0 收尾（PR #63582 全绿合入）→ 新建 `catalog-spi-02` 分支 → 创建 P1 task 文件 + 跟踪文档更新 → 对 5 个 P1 子任务做了代码侧 recon（详见 HANDOFF）→ 等待用户确认批次划分后开始动代码
- **下一个 session 应做**：用户确认 P1 批次方案后，按批次实施
