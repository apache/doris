...
- **当前**：**P0-3 FIX-BIND-STATIC-PARTITION ✅ DONE**（3 轮收敛 0 mustFix，commit `7cc86c66440`；**3 P0 写 blocker 全清**）。**下一步 = P1-4 FIX-PRUNE-PUSHDOWN**（DG-1：分区裁剪集在 translator 被丢、ODPS read session 仍跨全分区；修复或重分类+DV，见 HANDOFF §P1 / 报告 §B DG-1）。建议在此 `/clear` 开新 session。
  - 已完成：P0-1 FIX-OVERWRITE-GATE（2 轮，`59699a62f33`）、P0-2 FIX-WRITE-DISTRIBUTION（1 轮，`f0adedba20c`）、P0-3 FIX-BIND-STATIC-PARTITION（3 轮，`7cc86c66440`）。
  - ⚠️ **doc-sync WIP（未 commit，沿 P0-1/P0-2 惯例留批量处理）**：`decisions-log`（D-030）、`deviations-log`（DV-014）、`P4-T05-T06-cutover-design`（§4.2 更正）、`P4-T06e-FIX-WRITE-DISTRIBUTION-design`（§index-by-cols superseded 注）。横切 todo。
