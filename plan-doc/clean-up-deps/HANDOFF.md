...
- 当前状态：**Batch 1 + Batch 2 完成**；**Batch 3 大部完成**——5 个 iceberg 测试类**已迁**入 `fe-connector-iceberg`（用户拍板 migrate 而非 delete），iceberg 依赖簇（iceberg-core/aws/glue/s3tables/s3-tables-catalog）**已从 fe-core 删除**，fe-core 与连接器双双 test-compile 绿。**Batch 3 尾巴（待做）**：`aws-json-protocol` 摘除 + `avro` 显式声明删除 + `parquet-avro→parquet-hadoop` 替换——这三项耦合、且 nearest-wins 剪枝掩盖了传递供给，须用 `dependency:tree -Dverbose`（或删声明后重跑 tree）确认运行期不缺类后再动。之后 Batch 4（依赖定性）/ Batch 5（LIVE 迁移）。
...
| 2026-07-21 | Batch 3c aws-json/avro/parquet | ⏳ **待做**（deferred） | — | `aws-json-protocol` 摘除、`avro` 显式声明删除、`parquet-avro→parquet-hadoop` 替换 + 改 avro "For Iceberg" 注释。三者耦合：`dependency:tree -Dincludes` 因 nearest-wins 把 direct 声明提到顶层、**掩盖**了 kept 模块是否传递供给→不能据此判安全。做法：删声明后重跑 `-am dependency:tree`/`-Dverbose` 看是否仍传递可得 + 运行期协议推理（json 仅 glue/s3tables/iceberg-aws 用；avro 由 hive-exec runtime 兜底）+ 全量 test-compile（ParquetReader 用 parquet-hadoop）。 |
