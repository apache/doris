...
| 5 | LIVE 源特有逻辑迁移/废弃清理 | 高 | B1–3 | ✅ **全部完成**：T5.1 engine=hive✅（废弃→持久化空壳）；T5.2 ranger-hive✅（**误判→原地保留**）；T5.3 hudi TVF✅（整删）；T5.4 散点分支✅（建表校验→下沉连接器；其余 4 项复核保留）；T5.5 es 桩✅（**已死但必须保留→0 源码改动 + 补 Gson 守卫测试**）；T5.6 iceberg 行级DML✅（工作流2 rowid 走连接器 `3cb3d0113e4` + 工作流1 机械改名 `f63aecf1001`） |
