...
# 🎯 下一个 session 的任务 — **P6.2-T04（merge-on-read delete files → `TIcebergDeleteFileDesc`：position bounds / equality field-ids / deletion-vector offset+length；v2+ `delete_files` list；`getDeleteFiles` EXPLAIN read-back）**（**P6.2-T03 typed range-params 已绿，见下「✅ P6.2-T03 = DONE」；T01 骨架 / T02 谓词+split 见各自 DONE 块；P6.1〔T01–T10〕全绿**）

> **T04 scope**：把 T03 留空的 `TIcebergFileDesc.delete_files`（现 v2+ 整段未发，T03 review 实证「unset==empty 对 BE 无 delete 表安全」）实装。镜像 legacy `IcebergScanNode.setIcebergParams` 删除循环(:318-368)：`getDeleteFileFilters`(:1072)+`IcebergDeleteFileFilter`——POSITION→`PositionDelete`(position_lower/upper_bound) / PUFFIN→`DeletionVector`(content_offset+content_size_in_bytes，content=3) / EQUALITY→`EqualityDelete`(field_ids，content=2)；per-delete `setDeleteFileFormat`(parquet/orc)。FileScanTask `task.deletes()` 提供 delete 文件；编码进既有 `ConnectorDeleteFile.properties`（recon D-delete，不破 SPI）或 range typed 字段后于 `populateRangeParams` 发 `TIcebergDeleteFileDesc`。**`getDeleteFiles(TTableFormatFileDesc)` override** 解码回 delete 路径供 VERBOSE EXPLAIN（`deleteFileNum`/`deleteSplitNum`，镜像 paimon `PaimonScanPlanProvider.getDeleteFiles`）。**起步先读** T03 设计 `designs/P6-T03-iceberg-scan-rangeparams-design.md` + recon §2 delete 行 + legacy `getDeleteFileFilters`/`IcebergDeleteFileFilter` + thrift `TIcebergDeleteFileDesc`(`PlanNodes.thrift:314`)。**field-id（T06）仍最高危**（equality-delete 投错列依赖 field-id 正确）。
...
- **工作分支 = `catalog-spi-10-iceberg`**（P6.1 起步前 HEAD = `e5959e1b53d` #64655 P3b）。branch off `branch-catalog-spi`，PR 时 squash 合并。**所有 commit 均未 push。**
  ...
  - **已 commit（P6.2-T03 = 本 HANDOFF commit）**：typed range-params + `populateRangeParams`→`TIcebergFileDesc` + native fileFormat + `path_partition_keys`：新 `IcebergPartitionUtils`（port 分区助手，ZoneId/Jackson 两 delta）、`IcebergScanRange` typed carriers + populateRangeParams、`IcebergScanPlanProvider` 填 carriers + `getScanNodeProperties`、pom `fe-thrift`(provided)。5-dim 对抗 workflow 修 2 真 bug（Bug1 非 orc/parquet fail-loud；**Bug2 用户裁定加非破坏 SPI `isPartitionBearing()`——P6.2「0 新 SPI」唯一例外**）+ delete_files 误报证伪。iceberg UT 164/0（1 skip）+ fe-core PluginDriven* 53/0 无回归。详见上「✅ P6.2-T03 = DONE」。
