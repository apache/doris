...
# 🎯 下一个 session 的任务 = **P6.6 C3b-core 耦合核心 step 2：② cast 放宽 + `getIcebergPartitioning` dual-mode helper（含 3 parity）** —— ① handles ✅ 本 session

**① handles 泛化 ✅ 本 session（1 commit，additive/dormant）**。**全锚点 recon 复核 ✅（6-slice wf + 3 adversarial verify，详设计 §11.4.1，下个 session 直接信该节行号）**。**P6.1–P6.5 = ✅ 全 DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + step1+2 ✅ + impl-recon ✅ + ③-infra part1+part2 ✅ + ctx 重命名 ✅ + 耦合核心 ① handles ✅ → 余下（② cast+partitioning / ③ getFullSchema+新SPI / bridge）待办**。翻闸 = 5 commit-stream（C1/C2/C3/C4/C5），**C5 最后翻闸（唯一不可逆点）**。iceberg **仍不在** `SPI_READY_TYPES`。

## ✅ 本 session 完成（1 commit + 全锚点 recon 复核）
### 全锚点 recon 复核（doc-only，详设计 §11.4.1）
6-slice recon wf + 3 adversarial verify 对 HEAD（ctx-rename commit 后）复核全部耦合核心锚点：**零结构漂移、仅 doc 行号漂移**。关键修正（下个 session 用这些）：`supportsDelete/Merge` 在 **`ConnectorWriteOps` SPI**（iceberg override 现 :496/:501）；`buildInsertPartitionFields` decl :269（getIcebergTable() body :273、call+cast :194-195、getPartitionColumns cast :188 原位）；**`PluginDrivenWriteHandle` 类不存在→实为 `ConnectorWriteHandle`**（getWriteOperation default INSERT，enum 已含 DELETE/MERGE/UPDATE/REWRITE）；`IcebergRowIdInjector` 是 `IcebergNereidsUtils` 内部类、guard :159。**② 三 parity 全 upheld**（不可证伪）；**③ KEY 缺口**：请求级 STRUCT row-id 须**新增 tiny default-empty SPI accessor**（类型 carrier 齐、投递通道缺），③ 须双治 getFullSchema+injector，gap (b) format-version 信号已 DROP。命令 run()/getExplainPlan() cast = **DEAD**（建议删，3 reachable DB cast = :219/:240/:464）；executor beginXxx cast 冗余（transaction 层已 ExternalTable）；12 sink withX cast 放宽 ctor 即全消。详 §11.4.1。

### commit — ① handles 泛化（additive/dormant，TDD+4-mutation）
`IcebergRowLevelDmlTransform.handles:69` 加中立 capability arm：`table instanceof IcebergExternalTable || (table instanceof PluginDrivenExternalTable && pluginConnectorSupportsRowLevelDml(t))`；新 private static helper 仿 `InsertOverwriteTableCommand.pluginConnectorSupportsInsertOverwrite:338`（`catalog.getConnector().getMetadata(buildConnectorSession()).supportsDelete()||supportsMerge()`）。op-agnostic（per-op 校验留 `checkMode`）。
- **安全闸实证（CRITICAL）**：grep 全连接器 = **仅 iceberg override supportsDelete/Merge=true**，全 SPI_READY（jdbc/es/trino/max_compute/paimon）+ hudi/hive 继承 `ConnectorWriteOps` default false → 真 dormant（iceberg 不在 SPI_READY_TYPES）。
- **⚠️ 登记潜在 capability-proxy 耦合**（非阻塞）：未来某 SPI_READY 连接器若声明 supportsDelete/Merge=true 会被 `handles` 误纳入→路由进 iceberg synthesis。capability-proxy 设计已知性质，届时须 op+连接器双判别。
- **iron-law**：本文件 = legacy 豁免（`instanceof IcebergExternalTable` OR 合法）；新 arm 全中立。
- **验证**：TDD RED（plugin test 先红 line110）→GREEN；**4-mutation 全杀**〔arm 禁用 / 丢 legacy arm / `||`→`&&` / helper 恒真，逐条 KILLED〕；`IcebergRowLevelDmlTransformTest` **8/0/0**（拆 `handlesLegacyIcebergExternalTable`+`handlesPluginDrivenTableByRowLevelDmlCapability`，+`pluginTable(d,m)` mock helper）+ `IcebergDDLAndDMLPlanTest` **14/0/0**（pre-flip parity）；import-gate PASS；`SPI_READY_TYPES` 未改；**0 新 DV**。
...
4. **耦合核心 step 2 = ② cast 放宽 + `getIcebergPartitioning` dual-mode helper ⟵ 下个 session 起点**：
   - **cast 放宽**：transform 4 cast（`:74/90/110/166`）+ 3 sink ctor 参数→`ExternalDatabase`/`ExternalTable`（消 12 withX downcast + `MergeSink:188` 冗余 cast）+ 3 命令 reachable DB cast（`Delete:219`/`Update:240`/`Merge:464`，**死方法 run/getExplainPlan 建议删或不碰**）+ executor ctor 参数放宽（beginXxx cast 冗余可消，**`collectFor{Delete,Merge}` cast 留 O2/step-6**）+ `InsertIntoTableCommand:568`/`BindSink:1058`。**与 ① handles lockstep**（下游签名放宽后 ① 才完全自洽）。
   - **`getIcebergPartitioning` dual-mode helper**（替 `PhysicalIcebergMergeSink.buildInsertPartitionFields:269`，落该 legacy-exempt sink）：pre-flip `(IcebergExternalTable)`→native walk（byte-identical）；post-flip→连接器 `getWritePartitioning` carrier→`DistributionSpecMerge.IcebergPartitionField`。**含 §11.4.1 三 parity**：PARITY-1 null sourceColumnName/exprId-miss→整 spec 硬失败(clear→RANDOM)且构造前短路；PARITY-2 hasNonIdentity 从 transform 字符串 `!"identity".equals` over 全 fields 重算；PARITY-3 双闸门不变（post-flip MERGE 仍走 `PhysicalIcebergMergeSink`）。
5. **耦合核心 step 3 = ③ row-id 注入**：**新增 tiny additive default-empty SPI accessor**（`ConnectorWritePlanProvider`/`ConnectorWriteOps` 返 `List<ConnectorColumn> getSyntheticWriteColumns`，iceberg override 声明 STRUCT row-id；连接器禁 import fe-core）+ fe-core `PluginDrivenExternalTable.getFullSchema` override（`ctx.needsSyntheticWriteColForTable(id)||showHidden` 门控、经 `ConnectorColumnConverter` 注入）+ `IcebergRowIdInjector:159` guard 放宽（中立能力，非 `instanceof Iceberg*`）+ **pre-flip parity 回归**。**两处耦合双治**（getFullSchema + injector）。
...
- **[C3b-core 设计 ✅ / impl-recon ✅ / 实现进行中]**（[D-072]+[D-073]）：① handles ✅ + ②[Option A getWritePartitioning] 待 + ③ row-id 双注入[新SPI accessor] 待 + commit-bridge[Option(a) 路由连接器] 待，coupled。③-infra part1+part2 ✅ + 耦合核心 ① handles ✅。详 §10+§11（§11.4 = 本 session）。
...
**[pre-flip 行为偏差中央登记]**：P6.4=DV-045/046/047；P6.5=DV-048/049。C1/C2/C3a/C3b-pre/C3b-core(③-infra part1+part2 + 耦合核心 ① handles) 无新 DV。**C3b-core 余下预计 0 新 DV**（dormant + pre-flip byte-identical）。
...
- **本 session commit（① handles 泛化）= 1 改 + 1 测 + 2 doc**：`fe/fe-core/.../nereids/trees/plans/commands/IcebergRowLevelDmlTransform.java`〔+3 import（ConnectorMetadata/PluginDrivenExternalCatalog/PluginDrivenExternalTable）+handles arm +helper〕；测试 `.../IcebergRowLevelDmlTransformTest.java`〔+mock helper +2 handles UT〕；`plan-doc/HANDOFF.md` + `plan-doc/tasks/designs/P6.6-C3-ws-write-design.md`〔§11.4〕。**勿** `git add -A`（path-whitelist；scratch/游离文件如仓根 `fe/IcebergScanPlanProvider.java` 勿混入）。
- commit message：`[refactor](catalog) P6.6 iceberg: C3b-core 耦合核心 step1 — ① handles 泛化（中立 capability arm；additive dormant；TDD+4-mutation；8/0/0+14/0/0；全锚点 recon 复核）` + 末尾 `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>` + `Claude-Session: …`（squash 入上游剥离）。PR base = `branch-catalog-spi`，squash。
...
- **P6.1–P6.5 = ✅ DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + impl-recon ✅ + ③-infra part1+part2 ✅ + ctx 重命名 ✅ + 耦合核心 ① handles ✅ → 余下（② cast+partitioning / ③ getFullSchema+新SPI / bridge）待办**。
...
- **上下文超 30% 即交接**。本 session = 全锚点 recon 复核（6-slice wf + 3 adversarial verify）+ 耦合核心 step 1（① handles 泛化，TDD+4-mutation），在干净节点交接。**下个 session 起点 = step 2（② cast 放宽 + `getIcebergPartitioning` dual-mode helper）**：先信设计 §11.4.1 复核过的行号（仍可快速 re-grep），② 与 ① lockstep（下游签名放宽后 ① 才自洽）；逐 sub-step green+mutation；getIcebergPartitioning 三 parity 务必逐条 UT；O2（V3 DeleteFile 迁连接器）做到 step-6 bridge 时专门 recon；**C5 前切忌动 `SPI_READY_TYPES`**。
