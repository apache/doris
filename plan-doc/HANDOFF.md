...
> **⚠️ 本 session（2026-06-29 续）= H-10 路线 A 完整修复实现并提交 `83db1ebf486`**。前序同日 session：H-2 + H-3 + H-4 + H-7 + H-8 + H-9 ✅。**下一步 = ENG-1 能力孪生审计**（见 ⏭）。
> **H-10 修完成 `83db1ebf486`**——翻闸后 iceberg `SELECT s.a`（struct/list/map 子字段）不再做嵌套裁剪 = 读整列（读放大）。根因 = 嵌套裁剪三层耦合、翻闸后均按 `instanceof IcebergExternalTable`（死码）门控/缺失：L1 `LogicalFileScan.supportPruneNestedColumn` plugin 臂占位 `return false`；L2 `SlotTypeReplacer:380` 名→iceberg字段编号翻译门控死码；L3 中立 `ConnectorType` 无嵌套 field-id、`parseSchema`/`ConnectorColumnConverter` 不给数据列设 uniqueId（legacy `IcebergUtils.updateIcebergColumnUniqueId` 曾递归给整棵列树 setUniqueId=fieldId）。**⚠️ 实现期认识更正（Rule 7/12，诚实留痕）**：实现前一轮 BE 核实**误报「连普通标量列读取都坏了」**（误把 `column_ids` 当读取投影）；**更深一轮 BE 核实推翻、印证设计原判**——BE 实际读哪些列由**列名**驱动（`vparquet_reader.cpp:504 _init_read_columns(column_names)` → 按名字匹配 `_read_file_columns`；标量 reader 不查 `column_ids`，`vparquet_column_reader.cpp:197`），按字段编号建的 `column_ids`（`_create_column_ids`）**只**门控嵌套子列裁剪 + 行组/页过滤。∴ 翻闸当前态 = **所有列读得对、只是复杂列不裁剪（慢）**；危险 = 裸开 L1 而不补编号链路 → 复杂列子列因 `column_ids` 里编号错/缺 → `SkipReadingReader` → 子字段 NULL（真读错）。故三层一起做、一次性提交。**修（路线 A，用户裁定；fe-core 0 新增 iceberg 判别——L2 净减一个 instanceof）**：L1 新中立能力 `SUPPORTS_NESTED_COLUMN_PRUNE`（`ConnectorCapability`+`PluginDrivenExternalTable.supportsNestedColumnPrune()`+`LogicalFileScan` plugin 臂返回它+`IcebergConnector` 声明）；L2 `SlotTypeReplacer` 门控 instanceof→`PluginDrivenExternalTable`+能力 + 翻译方法去 Iceberg 命名（逻辑不变，本就通用读 `column.getUniqueId()/getChildren()`）；L3 `ConnectorType` 加并行 `childrenFieldIds`（+wither/getter，**排除 equals/hashCode**）、`IcebergTypeMapping` 逐层设（struct `fieldId()`/list `elementId()`/map `keyId()+valueId()`）、`parseSchema` 顶层 `withUniqueId(field.fieldId())`（既有通道）、`ConnectorColumnConverter` 递归 `applyNestedFieldIds` 落子列 uniqueId（对不带编号的连接器惰性）。验证：iceberg 连接器全量 **842/0（1skip）** + 新增 L1/L3 单测全绿 / checkstyle 0 / import-gate 干净 / **mutation 5/5 KILLED** / clean-room 8 探针全 **SAFE_TO_COMMIT**（legacy parity 每层含 map key+任意深度 / 铁律 / 跨连接器惰性 / equals 排除 / EXPLAIN cosmetic 已逐行证实）。**已知 LOW（cosmetic/非正确性，Rule 3+铁律不动）**：① `PlanNode` 的 `IcebergScanNode` EXPLAIN 访问路径合并臂死码（仅 EXPLAIN 显示 `name` vs `name(id)`，BE 仍收编号形路径）；② `LogicalFileScan` 保留 legacy `IcebergExternalTable` L1 臂（翻闸后死码、被新增 PluginDriven 臂先截获，仅「反翻闸」才成隐患）——留 ENG-1/dead-code 清理。**e2e（`iceberg_complex_type`/`test_iceberg_struct_schema_evolution`，含 renamed nested 子字段+子字段谓词，确认不返 NULL + 只读子字段）flip-gated 未跑**。设计/小结 `designs/P6.6-FIX-H10-nested-column-prune-capability-{design,summary}.md`（权威，含三层根因+BE 证据+三路线+认识更正）。
...
> **✅ DONE = `H-2`（REST 3 级 namespace）`b0c34ec8fe7` + `H-3`（filesystem Kerberos 认证器）`7850c6ad03f` + `H-4`（表级行数统计）`7b26fdbac88` + `H-7`（`FOR VERSION AS OF` branch∪tag）`a8f1ae9ca2e` + `H-8`（视图 schema）`5bf7c90a710` + `H-9`（rewrite WHERE 精确下推否则报错）`89658999f49` + `H-10`（嵌套列裁剪 L1+L2+L3）`83db1ebf486`**。
>
> **⏭ 下一步（新 session 从这里起）= `ENG-1` 全量审计 legacy iceberg `instanceof` 臂的能力孪生覆盖**：
> - **背景**：翻闸后运行时类型是 `PluginDriven*`，所有 `instanceof IcebergExternalTable/Catalog/Sys` 求值 false → 正确性逐点依赖人工写的「能力孪生臂」。**H-10 是已实证的一次漏写=静默回归（且有多道跨 FE+BE 耦合关卡：开关 + 路径翻译 + 字段编号可得性，逐个补会漏）。** ENG-1 = 逐个 legacy iceberg `instanceof` 臂核对其翻闸后是否有等价 `PluginDriven` 臂/能力门控，是防「逐点静默回归」的唯一保证。H-10 是模板样本。
> - **先对齐** `designs/connector-capability-unification-design.md`（用户已确认的「统一能力声明」设计，与翻闸解耦、翻闸稳定后独立落地；与新增能力枚举有交集，避免重复/冲突）。
> - **H-10 已识别的两条 LOW 死码线索可一并纳入 ENG-1 清理**：`PlanNode.mergeIcebergAccessPathsWithId`（EXPLAIN 死码）、`LogicalFileScan.supportPruneNestedColumn` 的 legacy `IcebergExternalTable||IcebergSysExternalTable` 臂（与 L2 现已不一致、反翻闸隐患）。
> - 处理顺序（任务清单 §8）：B-1+H-1 ✅ → B-2 ✅ → H-5+H-6 ✅ → H-2 ✅ → H-3 ✅ → H-4 ✅ → H-7 ✅ → H-8 ✅ → H-9 ✅ → **H-10 ✅** → **ENG-1 能力孪生审计（下一）** → P2(M-*) → P3(L-BATCH) → ENG-3 flip-gated e2e 全跑 → 用户二签翻闸。
...
2. **关键 P1 关**：至少 H-1/H-2/H-3/H-5+H-6/H-8（破坏主力部署的回归）；H-4/H-7/H-9/H-10 ✅。
