...
# 🎯 下一个 session 的任务 = **P6.6 C3b-core 继续：耦合核心（① handles + cast 放宽 + getIcebergPartitioning dual-mode + ③ getFullSchema/injector 注入 STRUCT row-id + pre-flip parity）→ commit-bridge**

**③-infra part2（连接器 v3-lineage emit）✅ + ctx 中立重命名 ✅ 本 session（2 commit）**。**impl-recon ✅（[D-073]，O1-O4 全解、用户裁 ③ carrier=A）+ ③-infra part1 ✅**。**P6.1–P6.5 = ✅ 全 DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + step1+2 ✅ + impl-recon ✅ + ③-infra part1 ✅ + ③-infra part2 ✅ + ctx 重命名 ✅ → 余下（耦合核心 / bridge）待办**。翻闸 = 5 commit-stream（C1/C2/C3/C4/C5），**C5 最后翻闸（唯一不可逆点）**。iceberg **仍不在** `SPI_READY_TYPES`。

## ✅ 本 session 完成（2 commit）
### commit 1 — ③-infra part2（additive/dormant，TDD+mutation+对抗 review GO）
连接器 `IcebergConnectorMetadata.buildTableSchema` 按 `getFormatVersion(table)>=3` append v3 lineage 两列（`_row_id` uid 2147483540 / `_last_updated_sequence_number` uid 2147483539，BIGINT/nullable/非 key/invisible，`new ConnectorColumn(name,ConnectorType.of("BIGINT"),"",true,null,false).invisible().withUniqueId(id)`，data 列之**后**）。**5 私有字面量常量本地复制**（连接器禁 import fe-core）。**字节级 parity 已核**（= legacy `IcebergUtils.appendRowLineageColumnsForV3`：`Column(name,BIGINT,isKey=false,agg=null,isAllowNull=true,default=null,comment="")`+`setIsVisible(false)`+`setUniqueId`）。
- **⚠️ 前置已验**：schema-cache round-trip 保 invisible+uniqueId（`toSchemaCacheValue`→`convertColumns`→`PluginDrivenSchemaCacheValue` 内存持引用→`getFullSchema`；iceberg lineage 列 identity-mapped 故**不**走会丢属性的 remap 分支 `:188-199`）。
- **sys 表自然排除**（非 DV）：iceberg metadata 表 `BaseMetadataTable.properties()` 返空 + 非 `BaseTable`→`getFormatVersion`=默认 2→`>=3` 闸门排除，与 legacy `IcebergSysExternalTable`（从不 append lineage）**结果**等价（机制不同：连接器靠闸门，legacy 靠不调用）。bytecode 已核。
- **测试**：连接器 `IcebergConnectorMetadataTest` **29/0/0**（+3：`...AppendsV3RowLineageColumnsWhenFormatVersionAtLeast3` / `...ForFormatVersionAbove3`(v4 钉死 `>=3` 下界) / `...OmitsV3RowLineageColumnsBelowFormatVersion3`）；fe-core 契约钉 `IcebergUtilsTest.testAppendRowLineageColumnsForV3AddsInvisibleColumns` **+2 uniqueId 断言**（16/0/0，钉 canonical 2147483540/2147483539 防连接器字面量漂移；列名由 `PluginDrivenScanNodeClassifyColumnTest#connectorRowIdConstantContractIsPinned` 钉）。
- **mutation 全杀**：①闸门弱化(`>=3`→`>=1`)→Omits+ParsesColumns 红；②去 `.invisible()`→红；③去 `.withUniqueId()`→uid=-1 红；④两 uid 互换→红；⑤`>=3`→`==3`→v4 test 红。
- **全模块 558/0/0+1 live-skip**；import-gate PASS；SPI 无新 `org.apache.iceberg` 泄漏（连接器 impl 自身用 native iceberg 合法）；`SPI_READY_TYPES` 未改；**0 新 DV**。

### commit 2 — ctx 中立重命名（纯机械 rename，0 行为）
`ConnectContext` 的 iceberg-命名成员重命名为中立名（D-071 Option ii「合成写列」词汇）：`icebergRowIdTargetTableId`→`syntheticWriteColTargetTableId`、`get/setIcebergRowIdTargetTableId`→`get/setSyntheticWriteColTargetTableId`、`needIcebergRowId()`→`needsSyntheticWriteCol()`、`needIcebergRowIdForTable(long)`→`needsSyntheticWriteColForTable(long)`；+ 中立化 ConnectContext 上的 4 处 javadoc/字段注释（保留「今唯一 consumer = iceberg `__DORIS_ICEBERG_ROWID_COL__`」说明）。**8 文件**：`ConnectContext.java`（定义）+ 6 调用方〔`ExplainCommand`/`RowLevelDmlCommand`/`Iceberg{Update,Delete,Merge}Command`/`IcebergExternalTable:289`〕+ 测试 `IcebergDDLAndDMLPlanTest`（用无参 `needsSyntheticWriteCol()` + get/set save-restore）。
- **scoped sed（最长 symbol 先，防前缀碰撞）+ 注释 Edit**；repo-wide grep **0 残留旧 symbol**（仅 doc/scratch 历史提及）；`ConnectContext` **非 GSON 持久**（0 `@SerializedName`、纯 class）→ 重命名零 compat 影响。
- **验证**：fe-core 全编译通过（含 ConnectContext 全引用）；`IcebergDDLAndDMLPlanTest` **14/0/0**（专测这些成员的 row-id 注入门控，行为不变 = 0-behavior 守门）。纯 rename 无新逻辑 → 无 mutation 适用。
...
1. **③-infra part2 ✅ DONE 本 session**（连接器 `buildTableSchema:240` format≥3 append v3 lineage 两列；详上「✅ 本 session 完成」）。
2. **ctx 中立重命名 ✅ DONE 本 session**（`ConnectContext` iceberg-命名成员 → 中立 `syntheticWriteColTargetTableId`/`needsSyntheticWriteCol[ForTable]`/`get·setSyntheticWriteColTargetTableId`；8 文件；0 行为；详上「✅ 本 session 完成 commit 2」）。
...
**[pre-flip 行为偏差中央登记]**：P6.4=DV-045/046/047；P6.5=DV-048/049。C1/C2/C3a/C3b-pre/C3b-core(step1+2+③-infra part1+part2) 无新 DV。**C3b-core 预计 0 新 DV**（dormant + pre-flip byte-identical）。
...
- **本 session commit 1（③-infra part2，已提交 `ee9bf61d768`）= 1 改 + 2 测 + 2 doc**：连接器 `IcebergConnectorMetadata.java`〔+ConnectorType import +5 常量 +buildTableSchema format≥3 append〕；`IcebergConnectorMetadataTest.java`〔+3 UT〕·fe-core 契约钉 `IcebergUtilsTest.java`〔+2 uniqueId 断言〕；`plan-doc/HANDOFF.md` + design §11。
- **本 session commit 2（ctx 中立重命名）= 6 生产 + 1 测 + 2 doc**：`qe/ConnectContext.java`〔定义 rename + 4 注释中立化〕·`nereids/trees/plans/commands/{ExplainCommand,RowLevelDmlCommand,IcebergUpdateCommand,IcebergDeleteCommand,IcebergMergeCommand}.java`·`datasource/iceberg/IcebergExternalTable.java`；测试 `datasource/iceberg/IcebergDDLAndDMLPlanTest.java`；`plan-doc/HANDOFF.md` + design §11.3/§6-D3。**勿** `git add -A`（path-whitelist；scratch/游离文件勿混入）。
- commit message：`[refactor](catalog) P6.6 iceberg: C3b-core ctx 中立重命名 — ConnectContext iceberg row-id 成员→中立 syntheticWriteCol（纯机械 rename；0 行为；8 文件；IcebergDDLAndDMLPlanTest 14/0/0）` + 末尾 `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>` + `Claude-Session: …`（squash 入上游剥离）。PR base = `branch-catalog-spi`，squash。
...
- **P6.1–P6.5 = ✅ DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + step1+2 ✅ + impl-recon ✅ + ③-infra part1 ✅ + ③-infra part2 ✅ + ctx 重命名 ✅ → 余下（耦合核心 / bridge）待办**。
...
- **上下文超 30% 即交接**。本 session = ③-infra part2（recon+TDD+5-mutation+对抗 review GO）+ ctx 中立重命名（机械 rename，0 行为）2 增量，在干净节点交接；下个增量 = **耦合核心**（1 个 coherent commit，大活、跨 session，fresh session 全 budget；逐 sub-step green+mutation；O2 做到 step-4 时专门 recon；**C5 前切忌动 `SPI_READY_TYPES`**）。
