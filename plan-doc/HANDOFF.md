...
# 🎯 下一个 session 的任务 — **P6.2-T06（field-id 数据字典 `history_schema_info`，P6.2 最高危）**（**P6.2-T05 COUNT 下推已绿 `2cc510608f6`，见下「✅ P6.2-T05 = DONE」；T01–T05 / P6.1〔T01–T10〕全绿**）

> **T06 scope**：移植 schema-evolution field-id 数据字典（**这是 P6.2 最高危**——丢失会让 BE 在演化表上 SIGSEGV/DCHECK，CI #969249 类，paimon 踩过）。`getScanNodeProperties` 用 iceberg `Schema` 构 `history_schema_info`（`-1`/current 按请求列名 + 历史枚举全 schema-id + name-mapping），`populateScanLevelParams` 落 `TFileScanRangeParams`（镜像 paimon `FIX-SCHEMA-EVOLUTION`，**不改 `ConnectorColumn`**——field-id 走字符串属性，决策签字见 recon §field-id）。**起步先读** paimon `PaimonScanPlanProvider.getScanNodeProperties`/`populateScanLevelParams`（schema dict 构建）+ paimon `catalog-spi-paimon-ro-schemadict-crash`/`ci-969249-rca` 教训 + legacy `IcebergUtils` schema-history + BE `table_schema_change_helper`（字典须 ⊇ slot 列，-1 entry 不可缺）。UT 喂多 schema-id / 重命名表断字典完整。**注意**：T04 equality-delete `field_ids` 来自 delete 元数据（独立于此数据字典，已正确）；本任务只管**数据**列 schema 演化字典。
