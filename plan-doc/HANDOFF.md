...
2. **元存储子线收尾**（[`metastore-storage-refactor/`](./metastore-storage-refactor/)）：P2-T04（剩 paimon pom + gate；
   ✅ `MetaStoreProviders` ServiceLoader 已改 2-arg 显式 loader `MetaStoreProvider.class.getClassLoader()`（`2612af5e88f`）——
   CI external **973270**（commit `13d3876`）实证:1-arg TCCL 在插件 child loader 下 `registered providers: []`，全 37 个
   paimon 家族测试 CREATE CATALOG 挂；单测单 classpath 不可区分 1-arg/2-arg（44/0 绿），真闸仍在 P2-T05 docker）→ P2-T05（docker
