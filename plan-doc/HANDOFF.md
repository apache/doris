...
- **P6.4 = 🟢 进行中**：T01 ✅（recon + 设计 + 三签字 [D-062]，0 产品码）；T02 ✅（`ConnectorProcedureOps` SPI 骨架 + dormant 占位，connector-api 37/0）；T03 ✅（base+factory+arg 框架 port → `connector.iceberg.action` + `IcebergProcedureOps` dispatch 骨架，iceberg 412/0/1，faithfulness wf 4→0 confirmed）；T04–T09 未做。**三 commit 待 push。**
...
## 本 session 完成 = P6.4-T03（base+factory+arg 框架 port + dispatch 骨架），1 commit，未 push
