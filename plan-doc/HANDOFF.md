...
# 🎯 下一个 session 的任务 — **P6 修复进行中：R3-residual DONE → 下一个 = R1-table (MINOR)**

> **进度（2026-06-19）**：P6 发现项按 `task-list-P6-fixes.md` 的 prioritized list 逐个修（单任务循环：
> design → 红队 → 实现 → impl 验证 → build+UT → commit）。
> **✅ C1 (MinIO, MAJOR) `9967846ef64`** / **✅ C2 (HDFS XML, MAJOR) `e95128aed5b`**（详见 git log + 各自 design/summary）。
...
> **下一个 = R1-table (MINOR)**：bridge `createTable` 补 `remoteExists && !ifNotExists` 臂 → 报 `ERR_TABLE_EXISTS_ERROR`(1050)。
> 然后 3 个 MINOR（C4 / R2-catalog / R3-catalog 可合一），最后 accept-as-deviation 批次。
...
0. **修复 P6 发现项**（报告 §Coverage gaps & follow-ups → prioritized fix-task list；每个独立 fix task；
   ...
   - **R1 table**（MINOR，下一个）：bridge `createTable` 补 `remoteExists && !ifNotExists` 臂报 `ERR_TABLE_EXISTS_ERROR`(1050)。
...
# 📦 仓库 / 进度状态
- **HEAD = `44499f073e8`**（P6 修复 R3-residual；前序 `e95128aed5b` C2 HDFS XML、`9967846ef64` C1 MinIO）。当前分支 **`catalog-spi-07-paimon`**（非 master）；
