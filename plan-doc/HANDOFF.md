...
> **✅ 本 session 已完成（2026-06-29）= H-8**（前序同日 session：H-2 + H-3 + H-4 + H-7）。
...
> **⚠️ 本 session 旁注（与翻闸无关，已按用户指示搁置）**：仓内有今日新建、用户已确认的设计 `designs/connector-capability-unification-design.md`（统一 Connector 能力声明，Trino「seam 即声明」）。**该设计自身声明与翻闸解耦、应在翻闸稳定后作为独立任务落地**；用户本 session 明确指示「只收尾视图修复、按原计划走、其他不管」。其与"嵌套列裁剪"修复（计划新增能力枚举）有交集，认领后者前宜先对齐该设计。
...
> **✅ DONE = `H-2`（REST 3 级 namespace）`b0c34ec8fe7` + `H-3`（filesystem Kerberos 认证器）`7850c6ad03f` + `H-4`（表级行数统计）`7b26fdbac88` + `H-7`（`FOR VERSION AS OF` branch∪tag）`a8f1ae9ca2e` + `H-8`（视图 schema）`5bf7c90a710`**。
>
> **⏭ 下一步（新 session 从这里起；用户指示 H-9 从下个 session 起做）= 处理顺序第 4 项剩余：其余 high（`H-9`/`H-10`，各自独立可并行认领）**：
...
> - 处理顺序（任务清单 §8）：B-1+H-1 ✅ → B-2 ✅ → H-5+H-6 ✅ → H-2 ✅ → H-3 ✅ → H-4 ✅ → H-7 ✅ → H-8 ✅ → **H-9/H-10 ⏭（H-9 下个 session 起）** → ENG-1 能力孪生审计 → P2(M-*) → P3(L-BATCH) → ENG-3 flip-gated e2e 全跑 → 用户二签翻闸。
