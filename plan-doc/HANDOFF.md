...
# 🎯 下一个 session 的任务 — **P6.2-T08（cache，连接器内部 D6）**（**P6.2-T07 MVCC 已绿，见下「✅ P6.2-T07 = DONE」；T01–T07 / P6.1〔T01–T10〕全绿**）

> **T08 scope**：cache（决策 D6，**全连接器内部，0 SPI**，镜像 paimon）——`IcebergLatestSnapshotCache`（per-catalog TTL，镜像 `PaimonLatestSnapshotCache`；`beginQuerySnapshot` 现 T07 是 **live `currentSnapshot()` 读**，T08 改走该 cache 以跨查询钉稳定 snapshot-id）+ `IcebergSchemaAtMemo`（`(handle,schemaId)→schema` memo，`getTableSchema(@snapshot)` 现 live 读 `table.schemas().get`，T08 改走 memo）+ `IcebergManifestCache`（path-keyed，移植 fe-core loader/value，仅 REFRESH CATALOG 清）+ `IcebergConnector` override `invalidateTable`/`invalidateAll`（**不清 manifest**，镜像 paimon `testInvalidateTableKeepsManifestCache`）+ wire `ExternalMetaCacheMgr`。**起步先读** paimon `PaimonLatestSnapshotCache`/`PaimonSchemaAtMemo`/`PaimonConnector`(invalidate 接线) + fe-core `datasource/iceberg/IcebergExternalMetaCache`(289)/`cache/`(2)/`*CacheValue` + recon §cache 行 + T07 注入点（`beginQuerySnapshot`/`getTableSchema(@snapshot)` 注释已标 "T08"）。
>
> **✅ T07 已闭合 T06 留下的 fail-loud 竞态（Option A，用户签字 2026-06-23）**：钉快照时 field-id 字典从**全 pinned schema**（= BE scan slot 超集）构建，非 pruned 请求列——连接器内闭合，**0 fe-core / 0 paimon / 0 SPI**。**纠正 T06 设计 §6/§7**：真实失败是 **BE `iceberg_reader.cpp:181` StructNode DCHECK 崩**（重命名列在上游 `buildColumnHandles` 被丢→字典缺该 slot），**非** FE `buildCurrentSchema` fail-loud 抛；设计原列 ①（从 handle field-id 建）/②（钉后解析 getColumnHandles）**都不可行**（①handle 已被丢、②`getColumnHandles` 无 snapshot 参数=须改共享 fe-core/SPI 且治不了 paimon）。深层缺陷（`getColumnHandles` 无 snapshot-aware 重载，**paimon snapshot-id time-travel + rename 同潜伏**）登记为 P6.6 holistic 关注项。
>
