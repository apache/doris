...
- [~] **丢 batch-mode 异步 split**（minor, F6/F13）🔵 **进行中（design-first）**：**用户改定「实现 batch SPI 路径」**（非 DV）。通用插件层缺口（每 full-adopter 继承非-batch 默认）；与 FIX-PRUNE-PUSHDOWN 耦合（裁剪已落，batch-by-spec 才有意义）。**设计文档 `tasks/designs/P4-T06e-FIX-BATCH-MODE-SPLIT-design.md` 供用户评审中、过目后再进实现**。骨架：扩 `ConnectorScanPlanProvider` 按 spec 分批/流式入口（默认委托现有同步 `planScan`、零破坏 non-MC）+ `PluginDrivenScanNode` override `isBatchMode/numApproximateSplits/startSplit`、泛型化引入 legacy `MaxComputeScanNode:214-298` 的 `splitAssignment` 异步机器。**Batch-D 红线**：本 fix 落地才能删 legacy `MaxComputeScanNode` batch 逻辑副本。**P3 最后一项。**
