...
> **✅ 本 session 已完成（2026-06-28）**：**B-2 第 2 层（连接器）** commit `b09d364888b`——iceberg 连接器移植 master 分区枚举（资格门 / PARTITIONS 枚举 / transform→RANGE 数学 / 演进重叠合并 / snapshot-id 解析）+ `getMvccPartitionView` + `listPartitionNames`（修 M-10）。clean-room 对抗 review（44 agents，逐发现 refute）查出 **1 must-fix**（合并须喂去重集，否则重名分区 byte-identical range 自包含被丢→0 行）+ **4 should-fix**（DTO 空上界=successor 契约落档 / 按 handle pin 枚举保快照一致 / not-found 失败到底而非降级 / tie-break 等下界测试补缺）**全部已修**。验证：fe-connector-iceberg **808 tests 绿 / checkstyle 0 / mutation 17-17 KILLED / 连接器无 fe-core import**。**e2e flip-gated 未跑**（`test_iceberg_mtmv.groovy` 翻闸后才能真验）。**未 push**（沿用铁律）。前序：B-1+H-1 `203cda3e31a`、B-2 SPI 地基 `d7482a39ab9`。
> **◐ 进行中 = 处理顺序第 2 项 `B-2`（+H-11 +M-10，合并）**：iceberg 作 MTMV 基表的分区增量刷新整体坏。设计 `designs/P6.6-FIX-B2-mtmv-listpartitions-range-design.md`（用户裁「完整 RANGE 平价 + 一并合并」；M-10 重裁=真回归）。**3 层分层实现：(1/3) SPI 地基 ✅ `d7482a39ab9`、(2/3) 连接器 ✅ `b09d364888b`（含 clean-room review 1 must-fix + 4 should-fix 全修）、(3/3) fe-core 通用模型 ⏭ 下一步（下个 session 从这里起）**。
>
> **分层实现进度（3 层）**：
> - **(1/3) ✅ DONE `d7482a39ab9`** = connector-api 中立 SPI 地基：新 `ConnectorMvccPartitionView`（style UNPARTITIONED/RANGE + freshness SNAPSHOT_ID/LAST_MODIFIED + List）+ `ConnectorMvccPartition`（name + 预渲染 `[lower,upper)` 字符串边界 + freshnessValue）+ `ConnectorMetadata.getMvccPartitionView` 默认 `Optional.empty()`。additive 零行为变化，paimon 走旧路径字节不变。编译+checkstyle 绿。
> - **(2/3) ✅ DONE `b09d364888b`** = iceberg 连接器移植 master 分区枚举：`IcebergPartitionUtils`（`isValidRelatedTable` 门 / `loadRawPartitions`+`generateRawPartition` PARTITIONS 枚举 / `buildRange(name,value,transform,sourceType,...)` transform→[lower,upper) 渲染字符串边界 / `mergeOverlapPartitions` LocalDateTime enclose 合并 / `latestSnapshotId` 全集查 max）+ `IcebergConnectorMetadata.getMvccPartitionView`（按 handle pin 或 current 枚举，not-found **失败到底**）+ `listPartitionNames`（修 M-10，not-found 降级空）。clean-room review（44 agents）查 1 must-fix（合并须喂去重集 `allByName.values()`，否则重名分区自包含被丢）+ 4 should-fix（DTO 契约落档 / 按 pin 枚举 / not-found 失败到底 / tie-break 测试）全修。808 tests 绿 / mutation 17-17 KILLED / checkstyle 0 / 无 fe-core import。
> - **(3/3) ⏭ TODO = fe-core 通用模型**：`PluginDrivenMvccExternalTable.materializeLatest` 加 present 臂（试 `getMvccPartitionView`：present→按 `view.getStyle()` 建 RANGE item〔`PartitionKey.createPartitionKey`+`Range.closedOpen`，已 import〕+ 按 `view.getFreshness()` 选 `MTMVSnapshotIdSnapshot`/`MTMVTimestampSnapshot`；absent→旧 LIST/timestamp 臂**字节不变**）；`PluginDrivenMvccSnapshot` 加字段（partitionType + freshnessKind + nameToFreshnessValue）；accessor（`getPartitionType`/`getPartitionSnapshot`/`getPartitionColumns`）按存储 style/freshness 分支。**连接器侧已就绪的两个契约（必须遵守）**：① **空 `upperBound` = NULL-min 分区**——fe-core 须 `lowerKey.successor()` 求上界（连接器无列类型/精度，故不预渲染；DTO `ConnectorMvccPartition` javadoc 已落档）；② **快照一致性**——materializeLatest 须先 `applySnapshot(handle, beginQuerySnapshot 的 snapshot)` 得 pinnedHandle，再传给 `getMvccPartitionView`（连接器已实现按 `iceHandle.getSnapshotId()>=0` 在该 pin 枚举，否则 current），否则分区集/新鲜度与数据扫描 pin 不一致。Mockito 单测（present RANGE/snapshot-id + NULL-min successor + pin 一致；absent 旧路径**paimon parity 守护**）。
> - **然后**：mutation（present 臂 style/freshness/successor 选择）→ clean-room 对抗 review → e2e `test_iceberg_mtmv.groovy`（flip-gated 未跑）→ 回填任务清单 B-2/H-11/M-10 ☑。
...
- **进度**：P6.1–P6.5 ✅ / P6.6 C1–C3 ✅ / C4 R1–R7 ✅ / C5 DDL/ALTER B1–B5 ✅ / flip-readiness 只读退化 ✅ / 视图 B0–B3 ✅ / 路由翻闸 `18e1b297d7e` ✅ / GSON 迁移 `e68eb5c00c9` ✅ → **⛔ 现卡在 clean-room review 发现修复（见 `P6.6-iceberg-flip-blockers-tasklist.md`）**：**B-1+H-1 ✅ `203cda3e31a`** → **B-2 进行中**（SPI 地基 ✅ `d7482a39ab9` + 连接器 ✅ `b09d364888b`；下一 = 3/3 fe-core 通用模型）→ 其余 H/M/L → ENG-1 审计 → ENG-3 e2e → 翻闸二签。
