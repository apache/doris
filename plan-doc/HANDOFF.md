...
>
> **✅ 权威设计 = `plan-doc/tasks/designs/iceberg-metastore-auth-connector-rewire-design.md`**（含纠正后前提 + 7 刀序 + 已裁定子决策 D1-D4 + 风险 + 验收口径）。**下个 session/续跑先读它。** 7 刀：**✅CUT 1（`cf8dda9f058`）** → **✅CUT 2（`eb9201dc0a6`，SHOW CREATE 脱敏改绑）** → CUT 3 vended gate 走 raw prop（⚠️见下）→ CUT 4 warehouse→fs.defaultFS 重新安置 → CUT 5 REWIRE 停造 fe-core iceberg 簇（flip-gated 全 flavor e2e）→ CUT 6 删死连通性探测 → CUT 7 删属性簇。
>
