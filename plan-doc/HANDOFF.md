...
# 🎯 下一个 session 的任务 — **P6.2-T09（vended credentials，O2 复用接缝，仅 REST）**（**P6.2-T08 cache 已绿，见下「✅ P6.2-T08 = DONE」；T01–T08 / P6.1〔T01–T10〕全绿**）

> **T09 scope**：vended（O2 复用既有 `ConnectorContext` 接缝，**仅 REST，0 新 SPI**）——连接器 `extractVendedToken(table)`（从 `table.io()` 的 `RESTTokenFileIO`/`SupportsStorageCredentials` 拿原始 cloud props map，自包含移植 `IcebergVendedCredentialsProvider:756`；gate = FileIO 类型 `instanceof RESTTokenFileIO`，**非** catalog-wide flag；在 `planScan` 每次 scan 取一次，`validToken()` 按需刷新解 ~1h TTL）→ 复用 `context.vendStorageCredentials(rawToken)` + `normalizeStorageUri(uri, token)`（`DefaultConnectorContext` 已实现，引擎中立）→ 发 `location.*`；DLF 凭据走 HiveConf（catalog-bind，T07/T10 已接，不在 T09）。**起步先读** recon §5（vended old→new 映射）+ paimon vended 链（`context.vendStorageCredentials`/`normalizeStorageUri` 消费点）+ 现 `IcebergScanPlanProvider.buildRange`/`buildDeleteFiles` 的 `context.normalizeStorageUri` 调用点（T04 现是单-arg static map，T09 改 2-arg vended token-aware）。
>
