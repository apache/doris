...
> **P3 进度**：limit-split 默认反转(F11) ✅ DONE @`<hash>`（P3-9，连接器局部恢复三重闸+闭 F2/F12）。**剩余**（见下 🟡 P3 段）：isKey=false 元数据(F3/F10)、丢 batch-mode split(F6/F13)、post-commit refresh 吞异常(F15)。多为「修或 DV」。另有 P2-7 暴出的 **KNOWN PRE-EXISTING GAP**（非-IFNE+本地-only CREATE TABLE 不 fail-loud，见 FIX-CTAS review-rounds）待用户定。
