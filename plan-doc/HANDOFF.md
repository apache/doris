...
# 🎯 下一个 session 的任务 = **P6.6 C3b-core 继续：ctx 中立重命名（机械 ~28 站点）→ 耦合核心 → commit-bridge**

**③-infra part2（连接器 v3-lineage emit）✅ 本 session（GO，对抗 review `wf_1a2f2188-b02` 三镜审 + 主 session 亲核）**。**impl-recon ✅（[D-073]，O1-O4 全解、O1/O2 verdict upheld、用户裁 ③ carrier=A）+ ③-infra part1（`ConnectorColumn.uniqueId` carrier）✅**。**P6.1–P6.5 = ✅ 全 DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + step1+2 ✅ + impl-recon ✅ + ③-infra part1 ✅ + ③-infra part2 ✅ → 余下（ctx 重命名 / 耦合核心 / bridge）待办**。翻闸 = 5 commit-stream（C1/C2/C3/C4/C5），**C5 最后翻闸（唯一不可逆点）**。iceberg **仍不在** `SPI_READY_TYPES`。

## ✅ 本 session 完成（1 commit：③-infra part2）
**③-infra part2（additive/dormant，TDD+mutation+对抗 review GO）**：连接器 `IcebergConnectorMetadata.buildTableSchema` 按 `getFormatVersion(table)>=3` append v3 lineage 两列（`_row_id` uid 2147483540 / `_last_updated_sequence_number` uid 2147483539，BIGINT/nullable/非 key/invisible，`new ConnectorColumn(name,ConnectorType.of("BIGINT"),"",true,null,false).invisible().withUniqueId(id)`，data 列之**后**）。**5 私有字面量常量本地复制**（连接器禁 import fe-core）。**字节级 parity 已核**（= legacy `IcebergUtils.appendRowLineageColumnsForV3`：`Column(name,BIGINT,isKey=false,agg=null,isAllowNull=true,default=null,comment="")`+`setIsVisible(false)`+`setUniqueId`）。
...
2. **ctx 中立重命名（机械，additive，下个增量）**：`ConnectContext.icebergRowIdTargetTableId:290`/`needIcebergRowIdForTable:1128`/`needIcebergRowId:1124`/get/set → 中立名（如 `syntheticWriteColTargetTableId`/`needsSyntheticWriteColForTable`）；~28 调用行 across `ExplainCommand`/`RowLevelDmlCommand`/`Iceberg{Delete,Update,Merge}Command`/`IcebergExternalTable:289` + `IcebergDDLAndDMLPlanTest`（含死命令方法须一并改）。**0 行为**。动前 re-grep 行号（可能漂移）。
...
- **本 session commit（③-infra part2）= 1 改 + 2 测 + 2 doc**：连接器 `fe-connector-iceberg/.../IcebergConnectorMetadata.java`〔+ConnectorType import +5 常量 +buildTableSchema format≥3 append〕；连接器测试 `fe-connector-iceberg/src/test/.../IcebergConnectorMetadataTest.java`〔+3 UT〕·fe-core 契约钉 `fe-core/src/test/.../iceberg/IcebergUtilsTest.java`〔+2 uniqueId 断言〕；`plan-doc/HANDOFF.md`（+ design §11 part2 标 done，若改）。**勿** `git add -A`（path-whitelist）。
- commit message：`[refactor](catalog) P6.6 iceberg: C3b-core ③-infra part2 — 连接器 buildTableSchema format≥3 emit v3 row-lineage 两列（additive dormant；连接器 29/0/0 + fe-core 契约钉 16/0/0；5-mutation 全杀；对抗 review GO）` + 末尾 `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>` + `Claude-Session: …`（squash 入上游剥离）。PR base = `branch-catalog-spi`，squash。
...
- **P6.1–P6.5 = ✅ DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ + step1+2 ✅ + impl-recon ✅ + ③-infra part1 ✅ + ③-infra part2 ✅ → 余下（ctx 重命名 / 耦合核心 / bridge）待办**。
...
- **上下文超 30% 即交接**。本 session = ③-infra part2 单增量（recon+TDD+5-mutation+对抗 review GO），在干净节点交接；下个增量 = ctx 中立重命名（机械 additive，动前 re-grep 行号）。
