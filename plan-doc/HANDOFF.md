...
> **◐ 进行中 = 处理顺序第 2 项 `B-2`（+H-11 +M-10，合并）**：iceberg 作 MTMV 基表的分区增量刷新整体坏——连接器未实现 `listPartitions`。**recon + 用户裁决 + 设计文档完成**（`designs/P6.6-FIX-B2-mtmv-listpartitions-range-design.md`）：用户裁「完整 RANGE 平价 + 一并实现分区演进重叠合并」；M-10 已回代码重裁 = **真回归**（master 抛 `not allowed`，翻闸后静默 0 行）。
>
> **分层实现进度（3 层）**：
...
> - **(2/3) ⏭ TODO = iceberg 连接器**：`IcebergConnectorMetadata.getMvccPartitionView` + `listPartitionNames`（修 M-10）。**移植 master**（连接器空间，无 fe-core 类型）：`isValidRelatedTable` 门（单字段 + YEAR/MONTH/DAY/HOUR + 源列稳定）→ 门失败发 `ConnectorMvccPartitionView.unpartitioned()`；PARTITIONS 元数据表枚举（`MetadataTableType.PARTITIONS` + `useSnapshot`，row 0-10，nullable 9/10 try-catch NPE）；`getPartitionRange`（`java.time` epoch.plus*，按 iceberg 源列类型 date→`yyyy-MM-dd`/timestamp→`yyyy-MM-dd HH:mm:ss` 渲染字符串边界，NULL→`0000-01-01`+successor）；`mergeOverlapPartitions`（在 LocalDateTime 边界 enclose 合并）；snapshot-id 三段回退（partition→table→throw）。源在 `git show master:fe/fe-core/.../iceberg/IcebergUtils.java:1372-1610` + `IcebergPartitionInfo.java:43-63` + `IcebergExternalTable.java:179-260`。全程 `context.executeAuthenticated`。连接器单测（真 InMemoryCatalog + PARTITIONS fake）。
> - **(3/3) ⏭ TODO = fe-core 通用模型**：`PluginDrivenMvccExternalTable.materializeLatest` 加 present 臂（试 `getMvccPartitionView`：present→按 style 建 RANGE item〔`PartitionKey.createPartitionKey`+`Range.closedOpen`，已 import〕+ 按 freshness 选 `MTMVSnapshotIdSnapshot`/`MTMVTimestampSnapshot`；absent→旧 LIST/timestamp 臂**字节不变**）；`PluginDrivenMvccSnapshot` 加字段（partitionType + freshnessKind + nameToFreshnessValue）；accessor（`getPartitionType`/`getPartitionSnapshot`/`getPartitionColumns`）按存储 style/freshness 分支。Mockito 单测（present RANGE/snapshot-id；absent 旧路径**paimon parity 守护**）。
> - **然后**：mutation（门/range 边界/合并/snapshot-id 回退/freshness 选择/style 选择）→ clean-room 对抗 review → e2e `test_iceberg_mtmv.groovy`（flip-gated 未跑）→ 回填任务清单 B-2/H-11/M-10 ☑。
