...
# 🎯 下一个 session 的任务 — **P3b：kerberos authenticator 机制收口到 `fe-kerberos`（元存储子线，先于 P6 iceberg）**
...
3. **🟢 NEXT — P3b（kerberos authenticator 机制收口到 `fe-kerberos`）= 见上 §头条，先于 P6**。元存储子线 [`metastore-storage-refactor/`](./metastore-storage-refactor/) **P2 全 5/5 ✅**（P2-T04 `2612af5e88f` + P2-T05 用户手动 docker 验证）。P3b 详 scope 见子线 tasks P3b-T01 + D-017。
