...
**设计 ✅ DONE 本 session（[D-072]，详 `tasks/designs/P6.6-C3-ws-write-design.md` §10）**。**P6.1–P6.5 = ✅ 全 DONE**。**P6.6 翻闸进行中：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core 设计 ✅ → C3b-core 实现待办**。翻闸 = 5 commit-stream（C1/C2/C3/C4/C5），**C5 最后翻闸（唯一不可逆点）**。iceberg **仍不在** `SPI_READY_TYPES`。

## ⚠️ 本 session 是 **design-only checkpoint**（0 行产品码；scope 远超原计划，budget 用尽前在 clean 节点交接）
- 起步对抗 recon 把 **C3b-core scope 大幅改写**：用户三裁 ②=**完整中立 SPI**（非降级）/ commit-bridge=**现在做** / **不拆**（plan-time+bridge 一个 coherent commit）。原以为「plan-time + 登记 commit gap」→ 现为「plan-time + 真接通 commit」。
- recon **推翻用户的桥前提**：连接器是真插件**子优先隔离 classloader** → native `org.apache.iceberg.Table` **跨不过** connector→fe-core（必 CCE）。但 recon 同时发现**连接器侧 commit 机器（`IcebergConnectorTransaction`）已全建好** + fe-core 通用 row-level DML SPI commit 链路**已 dormant** → 真桥 = **路由 DML 提交到连接器事务**（非暴露 native 表）。
- ⇒ 设计已据此重做（§10），**实现是大活（~15-20 文件、coupled atomic commit、含 translator dual-mode + executor 改道 + 新 SPI + ③ STRUCT 注入）**，留 fresh session 全 budget 做。
...
3. **建议实现顺序（§10.5；coupled 必须一个 commit、但 sub-step 内逐 green）**：① ③-infra（`ConnectorColumn` visibility 标志 + converter + 中立 format-version + ctx 中立重命名，机械/additive/dormant）→ ② SPI（`ConnectorWritePartitionField` carrier + `getWritePartitioning` default-null + iceberg override）→ ③ 耦合核心（① handles + 全 cast 放宽 + `getIcebergPartitioning` dual-mode helper + ③ getFullSchema/injector 注入 + **pre-flip parity 回归 byte-identical**）→ ④ commit-bridge（translator dual-mode 路由 + executor 经 connector tx + V3 DeleteFile iceberg-only seam）。
...
## 本 session 完成 = P6.6 C3b-core 全量设计 + commit-bridge recon（design-only，0 产品码）
...
- **产出（doc-only）**：设计 §10（全量设计 + §10.5 工作分解 + §10.6 开放项 O1-O4）+ §8 TODO 更新 + decisions-log [D-072] + 本 HANDOFF 覆盖。**0 行产品码、0 新 DV、`SPI_READY_TYPES` 未改**。
...
- **path-whitelist `git add`，严禁 `git add -A`**（scrub `regression-test/conf/regression-conf.groovy` 明文 key + `*.bak` + scratch `.audit-scratch/`·`conf.cmy/`·`META-INF/`·`docker/...`·**仓根游离 `fe/IcebergScanPlanProvider.java`**〔勿混淆，真文件在 `fe/fe-connector/...`〕·`plan-doc/reviews/P5-paimon-rereview3-*`〔非本线〕）。本 session = **doc-only（3 文件，0 产品码）**：
  - `plan-doc/tasks/designs/P6.6-C3-ws-write-design.md`〔+§10 全量设计 + §8 TODO 更新〕· `plan-doc/decisions-log.md`〔+D-072 索引行〕· `plan-doc/HANDOFF.md`（覆盖）
- message `[refactor](catalog) P6.6 iceberg: C3b-core 设计 + commit-bridge recon（doc-only；recon 推翻"暴露native表"→bridge=路由连接器；用户三裁②=A/bridge=now/不拆）` + 末尾 `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>` + `Claude-Session: …`（squash 入上游剥离）。PR base = `branch-catalog-spi`，squash。
