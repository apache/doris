...
# 🎯 下一个 session 的任务 = **C4 步骤 R2（连接器 scan 路径集作用域，dormant）**

**本 session（session 14）= C4 起步对抗 recon + 全量设计（用户裁 Option B）+ R1（executionMode SPI）✅**。**P6.1–P6.5 = ✅ 全 DONE**。**P6.6：C1 ✅ / C2 ✅ / C3a ✅ / C3b-pre ✅ / C3b-core+commit-bridge 全闭 ✅ → C4 进行中（R1 ✅，R2–R8 待）**。翻闸 = 5 commit-stream（C1/C2/C3 ✅ / C4 进行中 / C5 FLIP 不可逆待）。iceberg **仍不在** `SPI_READY_TYPES`。
...
- **R1 impl（commit `1bddf3426d6`，additive/dormant）**：新中立枚举 `ProcedureExecutionMode{SINGLE_CALL,DISTRIBUTED}`；`ConnectorProcedureOps` 加 `default getExecutionMode(name)=SINGLE_CALL`；`IcebergProcedureOps` override：rewrite_data_files→DISTRIBUTED（大小写不敏感）余 SINGLE_CALL。无 live caller（driver 未建）。
- **验证**：`ConnectorProcedureOpsDefaultsTest` 4/0 + `IcebergProcedureOpsTest` 11/0（全量含 checkstyle，未 skip）；2-mutation 全 KILLED。
...
## 🚦 R2 起步（连接器 scan 路径集作用域，dormant）
- 新不可变 handle 变体 `IcebergTableHandle.withRewriteFileScope(Set<String> rawPaths)`（仿 `withSnapshot:141`）；`IcebergScanPlanProvider.planScanInternal:~332` 在 `for(FileScanTask task: tasks)` 处过滤到 path-set。
- **⚠️ [INV-M1] path-normalization 不变式**（正确性地雷）：过滤 key 须两侧统一基准（raw vs normalized 不能混），否则匹配 0/错文件→over-read→重复行/OCC 丢并发写。mutation 锁「仅规范化差异的路径仍 match」。
- **[INV-deletes]**：`getDataFiles()` 丢 deletes，但 `planScanInternal` re-enumerate 经 `buildDeleteFiles` 重挂 deletes→可恢复；UT 验 scoped task deletes 仍齐。
- offline UT（provider pre-flip 不跑）。**动码前 re-grep** `IcebergTableHandle` 的 `with*`/`NO_PIN` 范式 + `RewriteDataGroup.getDataFiles()` 返回类型 + `dataFile.path()` 是 raw 还是 normalized（定 [INV-M1] 基准）。
...
- **R1 commit = `1bddf3426d6`**（5 文件：新 `ProcedureExecutionMode` + `ConnectorProcedureOps`/`IcebergProcedureOps` + 2 test + design doc）；HANDOFF 单独 commit。
...
- **⚠️ 推送状态**：P6.4 T01–T06+arg-move 已推 `origin`；**其后全部（含 commit-bridge + C4 R1）未 push**。**用户未要求 push**——留用户裁量。
- **P6.1–P6.5 ✅**。**P6.6：C1/C2/C3a/C3b-pre/C3b-core+commit-bridge 全闭 ✅ → C4 R1 ✅ / R2–R8 待 → C5 翻闸**。
...
- **上下文超 30% 即交接**。本 session 在 R1 收官（C4 起步全闭）干净节点交接 R2。
