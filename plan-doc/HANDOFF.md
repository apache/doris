...
# 🎯 下一个 session 的任务 — **P6.2-T05（COUNT 下推 `getCountFromSnapshot` + batch mode 检测）**（**P6.2-T04 delete files 已绿 + 数据路径归一化跟修，见下「✅ P6.2-T04 = DONE」；T01–T03 / P6.1〔T01–T10〕全绿**）

> **T05 scope**：移植 legacy `IcebergScanNode.getCountFromSnapshot`(:1142) 的 COUNT(*) 下推：读 snapshot summary——有 equality-delete（`TOTAL_EQUALITY_DELETES != "0"`）→ 不可下推（-1）；`TOTAL_POSITION_DELETES == 0` → 直接返 `TOTAL_RECORDS`；position-delete > 0 且 session `ignoreIcebergDanglingDelete` → 返 `TOTAL_RECORDS - deleteCount`；否则 -1。下推时 `tableLevelPushDownCount` → `populateRangeParams` 把 `setTableLevelRowCount(realCount)`（现 T03 恒发 -1，T05 改）+ 保持 split 整不切（legacy `splittable=!applyCountPushdown`）。**+ batch mode 检测**（manifest 计数阈值，paimon 同族延后项，T05 评估是否纳入或继续延后）。**起步先读** T03/T04 设计 + legacy `getCountFromSnapshot`(:1142-1167) + `IcebergUtils.TOTAL_*` 常量 + `setIcebergParams` 的 `tableLevelPushDownCount` 分支(:297-302) + `ConnectorSession` 读 `ignoreIcebergDanglingDelete` session 变量。**field-id（T06）仍最高危**（equality-delete 投错列依赖 field-id 正确）。
