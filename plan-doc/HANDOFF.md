...
# 🎯 下一个 session 的任务 — **P6.3 写路径实现：下一 = T07**（**✅ P6.3-T01 + T02 + T03 + T04 + T05 + T06 = DONE（框架统一·SPI 收口 + jdbc planWrite·config-bag 删除·EXPLAIN-保留 hook + `IcebergConnectorTransaction` 骨架·`addCommitData`·`getUpdateCnt`·`writeOperation` 枚举 + op 选择·`IcebergWriterHelper` 等价·begin* guards·baseSnapshotId 捕获 + commit 校验套件·O5-2 `applyWriteConstraint`〔新 `ConnectorPredicate` SPI default-no-op〕·V3 DV `removeDeletes` + **sink 统一〔INSERT/OVERWRITE〕`IcebergWritePlanProvider`·写排序 SPI·`getBackendFileType` 接缝·首动 fe-core/planner**）；P6.2〔T01–T11〕/ P6.1〔T01–T10〕全绿；RFC ✅ 评审通过**）
...
# ✅ P6.3-T06 = DONE（2026-06-23，本 session，未 push）— sink 统一（INSERT/OVERWRITE）增量·dormant + 写排序 SPI + `getBackendFileType` 接缝

> 设计文档 = `tasks/designs/P6.3-T06-iceberg-sink-unification-design.md`。**起步 recon + 3 项用户签字裁定 scope**（见下）→ research-design-workflow（design-doc-first）→ TDD（RED=cannot-find-symbol→GREEN）→ 对抗 parity workflow `wf_aaa45689-db4`（4 维[sink-byte-parity / sort-info-correctness / seam-and-gate / scope-and-regression] + 每发现独立 refute-by-default skeptic verify；25 agent / 358 tool-use / 1.6M token）= **21 raw / 2 confirmed〔均已修，均 dormant〕/ 14 refuted / 5 known-deviation**。**全绿**：fe-connector-api **34/0**、fe-connector-spi `ConnectorContextTest` **2/0**、fe-connector-iceberg **354/0/1**（341→354；`IcebergWritePlanProviderTest` 12 + `IcebergConnectorTest` +1 capability）、fe-connector-jdbc **190/0**、fe-connector-maxcompute **102/0/1**、fe-core `PluginDrivenTableSinkTest` **3/0** + `DefaultConnectorContextNormalizeUriTest` **9/0**、checkstyle 0（api/spi/iceberg/fe-core）、`check-connector-imports.sh` exit 0、iceberg 仍**不在** `SPI_READY_TYPES`、**0 BE 改**。

- **⚠️ 3 项用户签字裁定 scope（本 task 起步，code-grounded 暴露 HANDOFF/RFC §5.5/任务表 stale）**：`IcebergExternalTable extends ExternalTable`（**非** `PluginDrivenExternalTable`）⟹ 线上 iceberg 写**全 legacy、连接器写 dormant 直到 P6.6**；通用 `visitPhysicalConnectorTableSink` 强转 `PluginDrivenExternalTable`，iceberg 表进不去。① **不删 legacy sink 链**（现删打断 live iceberg INSERT，违 RFC §9「零行为变更直到 P6.6」）→ T06 纯增量；`IcebergTableSink`/`visitPhysicalIcebergTableSink` 等删除留 **P6.7**。② **T06 仅 INSERT/OVERWRITE**（`TIcebergTableSink`）；DELETE/MERGE 的 `TIceberg{Delete,Merge}Sink` 方言挪 **T07**（与三 DML 命令壳化一起，避免 T06 写无消费者死码）。③ **sort_info 做对**（完整字节 parity）。
- **新 `IcebergWritePlanProvider`（镜像 `MaxComputeWritePlanProvider`）**：`planWrite` 建字节-parity `TIcebergTableSink`（**非 rewrite**：不发 writeType=REWRITE / 不 appendRowLineage=P6.4），经 `currentTransaction(session)` 取 `IcebergConnectorTransaction`→`beginWrite`（loadTable+guards+newTransaction）→`getTable()`→`buildSink`。**op-promotion**：`buildWriteContext` INSERT+isOverwrite→`WriteOperation.OVERWRITE`（通用 handle 只带布尔），对齐 T04 commit switch。接线 `IcebergConnector.getWritePlanProvider()`（镜像 `getScanPlanProvider`）。`appendExplainInfo` 回吐 `ICEBERG TABLE: db.t`（OQ-3）。byte-parity 字段全核 MATCH（schema-json/spec/format/compress/location/overwrite/staticPartition；`getFileCompress`/`dataLocation`/`getTFileCompressType` 逐字 port）。
- **sort_info 写排序 SPI（决策 3）**：新 `ConnectorWriteSortColumn`（columnIndex+asc+nullsFirst）+ `ConnectorWritePlanProvider.getWriteSortColumns(session,tableHandle)` + `ConnectorWriteHandle.getSortInfo()`（`TSortInfo` default null）。连接器声明排序列（identity sortField→schema 列序），通用 translator `buildConnectorWriteSortInfo` 从 `connectorTableSink.getOutput().get(idx)` 建 `TSortInfo` 经 `PluginDrivenTableSink`（新 6-arg ctor）喂回 handle，planWrite stamp。
- **新 `ConnectorContext.getBackendFileType(uri,vended)` 接缝**（**thrift-free 返回 `TFileType` enum 名 String**，镜像 `normalizeStorageUri`；默认 scheme 映射，`DefaultConnectorContext` 经 `LocationPath.of(uri,vended).getTFileTypeForBE().name()` broker-aware override）；连接器 `TFileType.valueOf(...)`。`restVendedCredentialsEnabled` 提 **package-static**（scan/write 共享，零行为变更）。
- **🔴 对抗复核抓 2 confirmed（均已修，均 dormant 仅 P6.6 激活）**：① **HIGH**=iceberg 未声明 `SINK_REQUIRE_FULL_SCHEMA_ORDER`→`bindConnectorTableSink` name-mapped（用户列序）非 full-schema→`INSERT INTO t(name,id)` 写排序 columnIndex 解析错列 + 写错列。**修=`getCapabilities` 加 `SINK_REQUIRE_FULL_SCHEMA_ORDER`**（镜像 mc；legacy `bindIcebergTableSink:797-803` 恒投 full-schema）。② **LOW**=legacy `if(isSorted())` 内**无条件** setSortInfo（空也发）→纯 transform sort（`bucket`、无 identity 列）发空 sort_info→BE sort writer；连接器漏发→file 布局差。**修=`getWriteSortColumns` `null`=无 sort order / 非 null list（可空）=有**（镜像 isSorted gate）；translator null→无、非 null→建（空→空 `TSortInfo`）；SPI default→null。
- **deviation（设计 §6，T08 中央登记）**：DV-T06 vended（hadoopConfig 无 vended overlay，REST 对象存储 P6.6）/broker（FILE_BROKER 地址漏）/branch（branch-INSERT 未线程化）/hadoopconfig（连接器用 fe-filesystem `toHadoopConfigurationMap` vs legacy fe-core `getBackendConfigProperties`——**禁 import fe-core**）/distribution（`getRequirePhysicalProperties` iceberg 分区 hash 延后 P6.6）；OQ-3 EXPLAIN 标签 diff。**全程 0 BE 改，legacy 写路径仍 live（P6.7 删）。**
- **下一步 = P6.3-T07**（通用 `RowLevelDmlCommand` 壳 + capability 派发；含 T05 挪入的 O5-2 fe-core 生产半 [D-061]；DELETE/MERGE sink 方言 + 三命令壳化 + 删 `Iceberg{Delete,Merge}Sink` 链）。

---

# ✅ P6.3-T05 = DONE（2026-06-23，本 session，未 push）— commit 校验套件 + O5-2 `applyWriteConstraint` + V3 DV `removeDeletes`

> 设计文档 = `tasks/designs/P6.3-T05-iceberg-commit-validation-suite-o5-design.md`。TDD（headline conflict 测 watch-RED→GREEN）→ 对抗 parity workflow `wf_0960ef5f-52c`（4 维[validation-suite-order / conflict-filter-build / v3-dv-removeDeletes / o5-2-seam] + 每发现独立 refute-by-default skeptic verify，high effort；4 agent / 46 tool-use / 215k token）= **0 raw finding / 0 confirmed**。**全绿**：connector-api UT **30/0/0**、fe-connector-iceberg UT **341/0/1**（333→341=+8；1 skip env-gated live）、checkstyle 0（api+iceberg）、`check-connector-imports.sh` exit 0、iceberg 仍**不在** `SPI_READY_TYPES`、**0 BE / 0 fe-core / 0 pom 改**（jdbc/maxcompute/paimon 未触；es/trino 继承 default-no-op 不需回归）。

- **⚠️ 边界裁定（用户签字 [D-061]）= O5-2 拆「连接器消费半（A）= T05」+「fe-core 生产半（B）= T07」**：(A) 新 SPI `ConnectorPredicate` + `applyWriteConstraint` default-no-op + 连接器 override（转 iceberg expr 暂存）+ commit 校验套件 + V3 DV，现在 InMemoryCatalog 直测、独立绿；(B) 从 analyzed plan 抽 target-only 合取 + 命令调 `applyWriteConstraint` **挪 T07**——其唯一产品消费者是 T07 通用 `RowLevelDmlCommand` 壳（RFC §5.4/数据流 line163），在 T05 做=fe-core 悬空件 + 需 nereids plan 测试夹具（连接器禁 import nereids，测不到）；同 T01→T03 把 0-消费者 SPI 载体挪到首消费者 task 的先例。**T05 仍定义 SPI 接缝供 T07 直接调**。
- **新 SPI（`fe-connector-api`）**：`org.apache.doris.connector.api.pushdown.ConnectorPredicate`（不可变,包一个 `ConnectorExpression`,复用 scan 下推中立表示）+ `ConnectorTransaction.applyWriteConstraint(ConnectorPredicate)` **default-no-op**（jdbc/maxcompute/es/trino/paimon 零影响；`NoOpConnectorTransactionTest` 加 no-op 守，`ConnectorPredicateTest` 新）。
- **连接器 `IcebergConnectorTransaction`（演进 T04）**：① `applyWriteConstraint` override = 暂存中立谓词；**惰性转**——`applyWriteConstraint` 在 plan 时调、`beginWrite` 之前→表未 load→commit 时 `buildWriteConstraintExpression(table)` 经 `new IcebergPredicateConverter(table.schema(),zone).convert(...)` AND 折叠（legacy 存已转 iceberg expr，连接器存中立形 commit 时转，正确处理 call-order）。② commit 校验套件逐字移植 legacy `IcebergTransaction`:655-784（`applyRowDeltaValidations`/`applyBaseSnapshotValidation`〔消费 `baseSnapshotId`〕/`applyConflictDetectionFilter`〔write-constraint AND identity-分区 filter〕/`buildConflictDetectionFilter`/`combineConflictDetectionFilters`/`areAllIdentityPartitions`/`buildIdentityPartitionExpression`〔source 列名+"null"→isNull+3-arg zone-aware 解析〕/`extractPartitionValues`/`isSerializableIsolationLevel`〔`delete_isolation_level` 默认 serializable〕）。③ V3 DV 移植 :786-851（`shouldRewritePreviousDeleteFiles`〔fmt≥3，私有 `formatVersion` 镜像 `IcebergConnectorMetadata.getFormatVersion`〕/`collectRewrittenDeleteFiles`〔`ContentFileUtil.isFileScoped`+LinkedHashMap dedup〕/`buildDeleteFileDedupKey`〔PUFFIN path#offset#size〕/`collectReferencedDataFiles`+package-visible `setRewrittenDeleteFilesByReferencedDataFile`〔T07 产品喂，UT 直填〕）。④ 接 `updateManifestAfter{Delete,Merge}` 顺序保真：rewritten 在 empty-check 前算；merge 用 `deleteCommitData` 算 rewritten/referenced 但 conflict filter 用全 `commitDataList`；`scanManifestsWith` 仍丢（DV-T04-a 延续）。
...
- **deviation（设计 §6，T08 登记 deviations-log DV-T05-a..f）**：a O5-2 拆分（连接器收中立谓词惰性转，fe-core 生产半 T07）；b 异常型 `DorisConnectorException`+SDK `ValidationException`；c `IcebergPredicateConverter` 丢不可转合取项→filter 变宽=更保守安全；d 3-arg zone-aware `parsePartitionValueFromString`；e 私有 `formatVersion` 与 metadata 重复；f `scanManifestsWith` 丢。
- **下一步 = P6.3-T06**（sink 统一；见顶部任务头「T06 起步必读」）。

---

# ✅ P6.3-T04 = DONE（2026-06-23，本 session，未 push）— op 选择 + `IcebergWriterHelper` 等价 + begin* guards + baseSnapshotId 捕获

> 设计文档 = `tasks/designs/P6.3-T04-iceberg-op-selection-writerhelper-design.md`。TDD（RED=cannot-find-symbol→GREEN）→ 对抗 parity workflow `wf_a9356dd4-b17`（4 维[op-selection/begin-guards/writer-helper/parse-helpers] + 每发现独立 refute-by-default skeptic verify，high effort）= **0 findings / 0 confirmed**（4 reviewer 各 3–12 Read 逐方法核 legacy 字节等价）。**全绿**：fe-connector-iceberg UT **333/0/1**（295→333=+38）、checkstyle 0、`check-connector-imports.sh` exit 0、iceberg 仍**不在** `SPI_READY_TYPES`、**0 SPI / 0 BE / 0 fe-core / 0 pom 改**（jdbc/maxcompute/paimon 未触不需回归）。

- **🔑 关键架构发现（决定 T04 形态）**：新统一 `ConnectorTransaction` SPI **无 `finishWrite()` 钩子**（仅 `commit`/`rollback`/`close`/`addCommitData`）。maxcompute 把全部写工作放进 `commit()`。⟹ iceberg op 选择 + manifest 装配也**收进 `IcebergConnectorTransaction.commit()`**：先据 `WriteOperation` 建 op stage 进 SDK transaction，再 `transaction.commitTransaction()`——与 legacy `finishX`+`commit` 两步 SDK 语义字节等价。op-context（writeOp/overwrite/static/branch）由调用方在 `beginWrite` 传入暂存（**T06 `planWrite` 接线**，镜像 mc `setWriteSession`）。
- **`IcebergConnectorTransaction` 演进**：① op-aware `beginWrite(session,db,table,IcebergWriteContext)`——loadTable+`applyBeginGuards`+`newTransaction()` 同一 auth 块；guards = DELETE/UPDATE/MERGE 须 fmt≥2(`HasTableOperations`)+捕获 `baseSnapshotId`+merge 强制 `branchName=null`，INSERT/OVERWRITE 校验 branch(存在+`isBranch()` 非 tag)+`baseSnapshotId=null`；捕获 `zone`。② `commit()`→`buildPendingOperation()` switch `WriteOperation`（INSERT→`commitAppendTxn`、OVERWRITE→static?`commitStaticPartitionOverwrite`:`commitReplaceTxn`、DELETE→`updateManifestAfterDelete`、UPDATE/MERGE→`updateManifestAfterMerge`），末 `commitTransaction()`，全包 auth。③ op helpers 逐字移植 legacy（append/replace〔空+未分区→`OverwriteFiles` 清空表〕/static `overwriteByRowFilter`+`buildPartitionFilter`〔identity 用 source 列名〕/RowDelta delete〔**T05 校验/removeDeletes 留空**〕/RowDelta merge data+delete 拆分/`convertCommitDataToDeleteFiles` per-spec-id 分组/`getSnapshotIdIfPresent`）。
- **新 `IcebergWriterHelper`（连接器内）**：移植 legacy `helper/IcebergWriterHelper`（`convertToWriterResult`/`genDataFile`〔内联 `CommonStatistics`〕/`convertToPartitionData`〔`"null"`→null,zone〕/`buildDataFileMetrics`〔`TIcebergColumnStats` 5 map→`Metrics`〕/`convertToDeleteFiles`〔position/DV→PUFFIN/equality 拒绝 `VerifyException`/分区数据〕/`getFileFormat` 3-tier）。**新 `IcebergPartitionUtils` parse 助手**：`parsePartitionValueFromString`(9 type;TIMESTAMP canonical parser+`shouldAdjustToUTC?sessionZone:UTC`+micros)/`parsePartitionValuesFromJson`(iceberg Jackson)。**新 `IcebergWriteContext`**（包内 holder）= 连接器内 `IcebergInsertCommandContext` 等价物，T06 从 `ConnectorWriteHandle` 填。
- **🟡 T04/T05 边界（设计 §3）**：DELETE/MERGE 的 `RowDelta` **故意不含**冲突检测校验套件 + V3 DV `removeDeletes`（整套 T05）；`baseSnapshotId` T04 捕获、`getBaseSnapshotId()` 暴露供 T05 消费。REWRITE（procedure 写半）= P6.4 不 port。
- **自查修 1（test-only，非产品 bug）**：「仅删文件无新增」的 `OverwriteFiles` iceberg 标 `delete`（非 overwrite，正确语义，legacy 同款）；修测试断言。
- **deviation（设计 §6，T08 登记 deviations-log DV-T04-a..f）**：a `scanManifestsWith` 线程池丢(SDK 默认)；b 异常型 `DorisConnectorException`/`IllegalArgumentException`/`VerifyException`；c TIMESTAMP canonical parser+显式 zone(非 nereids `DateLiteral`+thread-local)；d partition_data_json iceberg Jackson(非 Gson)；e `CommonStatistics` 内联+单 `beginWrite`/`commit` switch(非 3 begin/finish)；f `ZoneId` 形参。
- **下一步 = P6.3-T05**（见顶部任务头）。
...
# ✅ P6.2-T07 = DONE（2026-06-23，本 session `31b51b6d4d1` 已 commit，未 push）— MVCC / time-travel + T06 竞态 Option A

> 方法：4-reader recon workflow（`wf_924c3e43-07c`：legacy time-travel / paimon MVCC 模板 / 通用 seam pin-ordering / 现状 scan）→ **用户裁定竞态修法（中文 AskUserQuestion，选 Option A）** → 设计文档 `designs/P6-T07-iceberg-scan-mvcc-design.md` → TDD（每件 RED→GREEN）→ 5-dim 对抗 parity workflow（`wf_cf21d2e4-d7d`，每发现独立 skeptic verify）。**全绿**：fe-connector-iceberg UT **235/0/0**（1 skip env-gated；205→235 = +30）、checkstyle 0、import-gate 净、iceberg 仍**不在** `SPI_READY_TYPES`、**无 SPI/fe-core/pom 改**。**对抗复核 5 维度 = 0 findings**（predicate-schema 分歧已在复核前自查并修，见下）。

- **`IcebergConnector.getCapabilities()`**：声明 `SUPPORTS_MVCC_SNAPSHOT`（= `PluginDrivenMvccExternalTable` 实例化闸门，否则 iceberg 表建成 plain 非-MVCC 表、time-travel 静默读 latest）+ `SUPPORTS_TIME_TRAVEL`（镜像 paimon）。iceberg 原先**一个 capability 都没声明**。翻闸前 inert（capability 只在 plugin-driven 路径消费，iceberg 未进 SPI_READY 故不触）。
- **`IcebergTableHandle`** 加类型化 pin 载体（`snapshotId`/`ref`/`schemaId`，默认无 pin）+ `withSnapshot` wither + `hasSnapshotPin()`；pin 入 handle identity（equals/hashCode/toString）。**iceberg 用 typed 载体非 paimon 的 scan-option map**（iceberg SDK 经 `scan.useSnapshot/useRef` 钉，非 `Table.copy(props)`）。
- **`IcebergConnectorMetadata`** 4 MVCC 方法（镜像 paimon + 移植 legacy `getQuerySpecSnapshot`/`getLatestIcebergSnapshot`/`getSchema`）：
  - `beginQuerySnapshot`=current snapshot id（空表 -1）+ **latest** schema id（`table.schema().schemaId()`,非 currentSnapshot 的;legacy `getLatestIcebergSnapshot:1412` schema-only 变更不进新 snapshot）。**T08 接 cache,现 live 读**。
  - `resolveTimeTravel` 5 kinds：SNAPSHOT_ID(`table.snapshot(id)`,缺→empty)、TIMESTAMP(digital→parseLong;否则 `IcebergTimeUtils.datetimeToMillis` 会话时区→`SnapshotUtil.snapshotIdAsOfTime`,无≤time 抛 IAE→catch empty)、TAG/BRANCH(`table.refs().get(name)`+isTag/isBranch 校验,**钉 REF 名**经 `iceberg.scan.ref` 属性,schema 经 `SnapshotUtil.schemaFor`)、**INCREMENTAL fail-loud**(`DorisConnectorException`;legacy 从不 dispatch @incr,静默读 latest)。empty-if-not-found(fe-core 渲染用户错),malformed 才抛。
  - `applySnapshot`=线程 pin 入 handle;`snapshotId<0 && ref==null`(空表/query-begin latest pin)→handle 不变(`useSnapshot(-1)`是不存在 snapshot,镜像 paimon -1 guard)。
  - `getTableSchema(@snapshot)` override=`table.schemas().get((int)schemaId)`(镜像 legacy `getSchema:1075`:schemaId>=0 且 currentSnapshot!=null,否则 latest);抽共享 `buildTableSchema(Table,Schema)` 与 latest 路防漂移。getTableSchema/getColumnHandles 顺手抽 `loadTable(handle)` auth-wrap 助手(字节不变)。
- **`IcebergScanPlanProvider.buildScan`**：加 `useRef(ref)`(tag/branch)否则 `useSnapshot(id)`(镜像 legacy `createTableScan:577-602`)。**谓词转换用 `table.schema()`〔current〕非 pinned schema**——**对抗复核前自查纠正**:legacy `createTableScan:589` `convertToIcebergExpr(conjunct, icebergTable.schema())` 用 current;重命名列谓词→无 field→落 BE residual=legacy parity(常见无-rename 路恒同;unbound expr 仍按 pinned snapshot schema 绑定)。COUNT 经 `scan.snapshot()` 自动跟 pin。
- **🟢 T06 竞态 = Option A（用户签字 2026-06-23,中文 AskUserQuestion）**:`getScanNodeProperties` 钉快照时字典从**全 pinned schema**(`encodeSchemaEvolutionProp(table, pinnedSchema, emptyList())`=超集覆盖所有 BE slot)构建,非 pruned `columns`;无 pin 保 T06 pruned(CI #969249)。`IcebergSchemaUtils` 加 3-arg `encodeSchemaEvolutionProp(table,dictSchema,names)`(2-arg 委派 `table.schema()`)。**连接器内闭合**(iceberg 投影 BE-tuple 驱动,`columns` 只喂字典——已核实 `planScan` 无 `.select(columns)`)。详见设计 §6 + 顶部任务头「✅ T07 已闭合」。
- **`IcebergTimeUtils`(新)**:抽 TZ alias map(原内联 `IcebergScanPlanProvider`)+ `resolveSessionZone`(scan provider 现委派)+ `datetimeToMillis`(byte-parity legacy `TimeUtils.timeStringToLong:273`=`LocalDateTime.parse("yyyy-MM-dd HH:mm:ss").atZone(sessionZone).toEpochMilli`,parse 失败→`DateTimeException("can't parse time")`)。
- **deviation（UT 不可见,P6.6 docker 验,登记设计 §9）**:INCREMENTAL fail-loud vs legacy 静默读 latest;TIMESTAMP digital 当 epoch-millis(安全超集,legacy 恒按字符串 parse);beginQuerySnapshot live 读(T08 加 cache);tag/branch 钉 REF 名非 id(legacy parity);**partition-spec 演化 + time-travel** 下 `path_partition_keys` 仍读 current spec(niche,schema-race 的分区版,T07 范围外)。
- **测试**(真 InMemoryCatalog 2-snapshot+rename+tag+branch,无 Mockito):`IcebergConnectorMetadataMvccTest` 15 + `IcebergTableHandleTest` 5 + `IcebergTimeUtilsTest` 5 + scan provider +4(pin 文件集/useRef 优先/COUNT 跟 pin/Option-A 全列字典)+ connector capability 1。
- **下一步 = P6.2-T08**（见顶部任务头）：cache（连接器内部，D6）。

---

# ✅ P6.2-T08 = DONE（2026-06-23，本 session，未 push）— 连接器内部 cache（D6）+ manifest 级 planning（用户扩 scope）

> 方法：4-reader recon workflow（`wf_57863154-e49`：paimon 模板 / fe-core legacy cache / 现 iceberg 注入点 / manifest 可行性+失效派发）→ **2 项 code-grounded 复核暴露 + 用户 2 轮 AskUserQuestion 裁定**（① schema memo 跳过 ② manifest cache「扩 scope 走 manifest 级」③ DeleteFileIndex 1.10.1 包级私有→「vendoring」）→ 设计文档 `designs/P6-T08-iceberg-cache-design.md` → TDD（每件 RED→GREEN）→ 3-dim 对抗 parity workflow（`wf_428a2e5a-ab3`，每发现独立 skeptic verify）。**全绿**：fe-connector-iceberg UT **258/0/0**（1 skip env-gated；235→258 = +23）、checkstyle 0、import-gate 净、iceberg 仍**不在** `SPI_READY_TYPES`、**0 SPI/fe-core/paimon 改**（仅 iceberg 连接器文件 + iceberg pom +caffeine）。**对抗复核 3 维度 = 0 confirmed**（2 LOW 均 REFUTED-as-bug，登记设计 §4.8/§4.9）。

- **🔑 recon 暴露 + 用户裁定（3 项，签字 2026-06-23）**：
  1. **`IcebergSchemaAtMemo` 跳过**（用户选「跳过」）：iceberg 所有历史 schema 内嵌 table metadata，`table.schemas().get(id)` 是 O(1) 内存 map 查找**无 I/O**（≠ paimon `schemaAt` 读 schema 文件）；且 `getTableSchema(@snapshot)` 仍须 `loadTable`（I/O 省不掉）→ memo 零收益。`getTableSchema(@snapshot)` 保持 T07 现状（live map 查找）。
  2. **manifest cache「扩 scope 走 manifest 级」**（用户选）：原 HANDOFF 只列「移植 cache」，但 connector scan（T02 起）走 SDK `planFiles()`，**不消费** Doris manifest cache → 单移植=死代码。用户裁定**一并移植 legacy `planFileScanTaskWithManifestCache`**，gate `meta.cache.iceberg.manifest.enable`（默认 false → 默认仍 SDK splitFiles，零回归）。
  3. **vendoring `org.apache.iceberg.DeleteFileIndex`**（用户选）：iceberg **1.10.1 把 `DeleteFileIndex` 改成包级私有**（legacy 靠 fe-core vendoring 906 行 public 副本，连接器不能复用 fe-core）→ 用户选「vendoring 进连接器」（全 parity）。
- **Cache ① `IcebergLatestSnapshotCache`**（镜像 `PaimonLatestSnapshotCache`）：**值 = `(snapshotId, schemaId)` 二元组**（唯一对 paimon 偏差；`beginQuerySnapshot` 须原子钉 snapshot-id + 最新 schema-id=`table.schema().schemaId()`，避 schema-only-ALTER 漂移；移植 legacy `IcebergSnapshot` 形态）。key=`TableIdentifier.of(db,table)`（put@beginQuerySnapshot / remove@invalidateTable 同 remote 名）；TTL `meta.cache.iceberg.table.ttl-second`（默认 86400，`<=0` 禁=always live）；注入时钟测试。`beginQuerySnapshot` 命中跳过 `loadTable`（省 I/O+稳定钉）。`IcebergConnectorMetadata` 3-arg ctor 传**禁用** cache（保现有 MVCC 测试 always-live 零回归），4-arg ctor 生产注入。
- **Cache ③ manifest（移植 fe-core，零 fe-core import）**：新 `ManifestCacheValue`/`IcebergManifestEntryKey`（path+content，跨表共享）/`IcebergManifestCache`（ConcurrentHashMap，**no-TTL，capacity 100000**=legacy 实测值，`getManifestCacheValue` load DATA→`ManifestFiles.read`/DELETES→`readDeleteManifest`，每文件 `.copy()`）。`IcebergScanPlanProvider`：`planScanInternal:201` 的 `splitFiles` 换 gated `planFileScanTask`；off→SDK splitFiles，on→`planFileScanTaskWithManifestCache`（逐字移植 legacy：Phase1 delete manifest `ManifestEvaluator` 裁→cache→`DeleteFileIndex`；Phase2 `getMatchingManifest`→cache→`InclusiveMetricsEvaluator`+`ResidualEvaluator`→`BaseFileScanTask`）+ `getMatchingManifest`（HashMap 替 Caffeine）+ `isManifestCacheEnabled`（gate `enable && ttl!=0 && capacity!=0`=`CacheSpec.isCacheEnabled` parity）+ 异常 fallback splitFiles。输出 `CloseableIterable<FileScanTask>` 同 splitFiles → 下游 `buildRange`(T03-T07) 不变。
- **vendored `org.apache.iceberg.DeleteFileIndex`**（906 行，逐字 copy fe-core，仅加类注释）：package `org.apache.iceberg` → 包访问 SDK 内部（`ManifestEntry`/`PartitionMap`...）；连接器仅用 `builderFor(deleteFiles)` 路（Caffeine-free），但 split-package **shadow** iceberg-core jar 的包私有副本→SDK 自身 `planFiles()` 也用它→runtime 须 caffeine（pom +`com.github.ben-manes.caffeine:caffeine:2.9.3` compile；iceberg-core 已 runtime 带，plugin-zip 实证含 caffeine-2.9.3.jar + vendored class）。
- **失效矩阵**（镜像 paimon）：REFRESH TABLE→`invalidateTable`→清 latest-snapshot 条目、**留 manifest**（legacy `testInvalidateTableKeepsManifestCache` parity）；REFRESH CATALOG→连接器重建（fe-core 无 `invalidateAll()` 调用点，recon 实证）→ final cache 字段随 GC 清；`invalidateAll()` 清 latest（对称 paimon，dead）。
- **deviation（UT 不可见，P6.6 docker 验，登记设计 §4）**：①值二元组 vs paimon long；②schema memo 跳过；③manifest hit/miss EXPLAIN 统计丢弃（同 profile drop 族）；④`getMatchingManifest` HashMap vs Caffeine；⑤manifest 级 filter/metrics/schema 用 current schema（time-travel 下 legacy parity）；⑥`.ttl-second`/`.capacity` 仅喂 gate 公式不 size cache（legacy quirk）；⑦**split-package shadowing**（vendored DeleteFileIndex 与 iceberg-core jar 共存，jar 序定胜者，fe-core 同款 proven，UT 不可见）；⑧manifest 三键不进 `validateProperties`（恶值静默回落，非正确性）；⑨不调 `ManifestFiles.dropCache`（内存释放 gap，非 stale-read）。
- **对抗复核（3-dim/6-agent workflow `wf_428a2e5a-ab3`）= 0 confirmed**：latest-snapshot（6 检查全 TRUE）/ manifest-planning（count-pushdown 走 `scan.planFiles` REFUTED=T05 设计 / `spec==null` NPE REFUTED=legacy 同款被 try/catch）/ cache-gate-vendored（vendored DeleteFileIndex byte-identical fe-core，2 LOW REFUTED-as-bug）。
- **测试**（无 Mockito，真 InMemoryCatalog+注入时钟）：`IcebergLatestSnapshotCacheTest` 5 + `IcebergConnectorCacheTest` 5 + `IcebergConnectorMetadataMvccTest` +2（enabled cache 钉稳定+loadTable 一次 / disabled 每调 load）+ `IcebergManifestEntryKeyTest` 3 + `IcebergManifestCacheTest` 3 + `IcebergScanPlanProviderTest` +5（gate-on==SDK 并消费 cache / 分区裁剪 parity / 空表空 / ttl=0 gate-off / **delete 关联 parity=真 exercise vendored DeleteFileIndex**）。
- **下一步 = P6.2-T09**（见顶部任务头）：vended credentials（O2 复用接缝，仅 REST）。

---

# ✅ P6.2-T09 = DONE（2026-06-23，本 session，未 push）— vended credentials + 静态 location.* 凭据（用户扩 scope）

> 方法：主线 code-grounded recon（直读 legacy `IcebergVendedCredentialsProvider`/`IcebergScanNode.doInitialize`+`getLocationProperties`/`VendedCredentialsFactory`/`AbstractVendedCredentialsProvider` + paimon vended 链 + `DefaultConnectorContext` 三接缝 + `PluginDrivenScanNode.getLocationProperties`）→ **2 项用户裁定（中文 AskUserQuestion）** → 设计文档 `designs/P6-T09-iceberg-scan-vended-design.md` → TDD → 4-dim 对抗 parity workflow（`wf_a6996983-799`，每发现独立 skeptic verify）。**全绿**：fe-connector-iceberg UT **270/0/0**（1 skip env-gated；258→270 = +12）、checkstyle 0、import-gate 净、iceberg 仍**不在** `SPI_READY_TYPES`、**0 SPI/fe-core/paimon/pom 改**（仅 4 个 iceberg 连接器文件，iceberg SDK + fe-filesystem-api 已是依赖）。

- **🔑 recon 暴露 + 用户裁定（2 项，签字 2026-06-23）**：
  1. **静态 `location.*` 凭据缺口（纳入 T09，单 commit）**：现 iceberg `getScanNodeProperties` 一个 `location.*` 都不发（grep 实证）→ `PluginDrivenScanNode.getLocationProperties()` 只从 `location.*` 取 BE 存储凭据（无第二接缝）→ 翻闸后**任何** native 读（含非 REST 静态 AK/SK catalog）BE 拿不到凭据→403。T03 只补 `path_partition_keys` 没补凭据。镜像 paimon `getScanNodeProperties:661-681`（静态 + vended overlay 两块）。
  2. **vended gate = catalog flag（legacy 忠实）**：非 paimon 的 FileIO-类型 gate。
- **`IcebergScanPlanProvider`**：①新静态 `extractVendedToken(Table, boolean)`（忠实移植 legacy `extractRawVendedCredentials`=`table.io().properties()` ∪ `SupportsStorageCredentials.credentials().config()`；iceberg SDK，**非** paimon 的 `RESTTokenFileIO.validToken()`）；②`restVendedCredentialsEnabled()` **两段式 gate**（见下对抗修）；③`planScanInternal` 每 scan 取一次 token，线程进 `buildRange`/`planCountPushdown`/`buildDeleteFiles`/`convertDelete` 的 `normalizeUri(path, vendedToken)`（`normalizePath`→`normalizeUri` 升 2-arg 接缝；`originalPath` 仍 raw）；④`getScanNodeProperties` 发静态 `location.*`（`context.getStorageProperties()`→`toBackendProperties().toMap()`，规范 AWS_* 非 raw 别名）+ vended overlay `location.*`（vended 赢冲突，legacy precedence）。
- **🔴 对抗复核（4-dim workflow `wf_a6996983-799`，每发现 skeptic verify）= 2 confirmed（both LOW）+ 3 refuted + 0 in URI-threading**：
  1. **gate 单段→两段（low，已修+测）**：legacy `isVendedCredentialsEnabled` 是 `instanceof IcebergRestProperties` **且** flag 两段；初版只移植 flag。非 REST catalog 误带 `iceberg.rest.vended-credentials-enabled=true`（连接器不 reject 未知属性，可建）会触发 vended，legacy 抑制。修=AND 上 `TYPE_REST.equals(resolveFlavor(props))`（flag 仅声明在 REST，flavor==rest ⟺ instanceof）。正确配置字节不变。新增 `getScanNodePropertiesOmitsVendedWhenFlagSetButNonRestFlavor` 守（mutation 验 RED）。
  2. **`extractVendedToken` 非 fail-soft（low，**与 paimon 同款，不修，登记**）**：legacy 在提取外层 fail-soft 回落静态；SPI 把 fail-soft 移到 `vendStorageCredentials`（对已提取 map），提取本身无 guard。**不修**（paimon `:677` 同样无 guard，单修 iceberg 引入不一致；真实 REST FileIO 内存访问器几乎不抛；仅 P6.6 后可触）。登记设计 §4.6。
  - 3 refuted：overlay-vs-replace precedence（REST 静态 map 空→等价）、B-9 规范化（无 raw 别名漏出）、no-context 安全 + vended-wins。
- **deviation（UT 不可见，P6.6 docker 验，设计 §4）**：①overlay vs 替换（REST 静态空→实践等价）；②gate=flag+flavor vs paimon FileIO-类型；③无 `validToken()` 刷新（每查询 `loadTable` 新鲜）；④无条件读 `io().properties()`（legacy parity）；⑤fail-soft 缺口（§4.6，与 paimon 同款）。
- **测试**（无 Mockito；扩 `RecordingConnectorContext`〔token-aware `vendStorageCredentials` + 2-arg `normalizeStorageUri` 记 `lastVendedToken`〕+ `FakeIcebergTable.setIo` + test-local fake FileIO/SupportsStorageCredentials + `fakeBackendStorage`）：`extractVendedToken` 4（merge/io-only/disabled-gate/null）+ `getScanNodeProperties` 静态/overlay/flag-off-gate/flavor-gate/no-context/skip-no-BE 6 + planScan 2-arg threading 1 + convertDelete 显式 token 1 = +12。改 T04 的 5 处 `convertDelete(delete)`→2-arg。
- **下一步 = P6.2-T10**（见顶部任务头）：parity UT 套件审计（多数 T02–T09 已写）。

---

# ✅ P6.2-T10 = DONE（2026-06-23，本 session，未 push）— parity-UT 覆盖审计 + 缺口补全

> 方法：10 维度审计 workflow（`wf_9d88fe61-5c7`，每维度 auditor 读 legacy `IcebergScanNode`/`IcebergUtils` 期望**值** vs 连接器 impl vs 连接器测试，断 value-parity / weak / 缺；每发现独立 skeptic adversarial verify，refute-by-default + 对照 T02–T09 已签字延后 deviation）→ 主线**亲自复核**每个确认缺口（读真实代码核证，非盲信）→ TDD 补测 → mutation-verify 最高危测。设计文档 = `designs/P6-T10-iceberg-scan-parity-audit-design.md`。**全绿**：fe-connector-iceberg UT **270→278/0/0**（1 skip env-gated；+8 测）、checkstyle 0、import-gate 净、iceberg 仍**不在** `SPI_READY_TYPES`、**0 SPI/fe-core/paimon/pom/产品码改**（仅 3 个测试文件）。

- **审计结论**：12 缺口报告 → **10 确认 + 2 驳回**；assertion-quality **8 维 value-parity** + 2 维 mixed（predicate / e2e）。两确认重复（G2==E2E-1）+ E2E-2 折入 → **8 测**补 10 缺口。
- **补的 8 测**（断 legacy 期望值，非类名）：
  1. **PRED-1**（`comparisonOperatorsMatchLegacyOperations`@`IcebergPredicateConverterTest`）：旧 EQ 网格只断**可下推性**，5 个比较算子→iceberg `Operation` 从未钉。补 GT→GT/LT→LT/GE→GT_EQ/LE→LT_EQ + **NE→not(equal)〔NOT-over-EQ 非 NOT_EQ〕**（connector `:201-214`）。
  2. **PP-1**（`planScanDefaultSplitHeuristicTilesAndMaxFileSplitNumCapCollapses`）：默认 split 启发式 + `max_file_split_num` cap 从未按 range 计数断。**纠审计建议**（offset 文件下 cap 对计数无效）→ 用**无 offset** 96MB 文件（fixed-size split，target 直控计数）+ `max_file_split_num=1`→整文件→1 range。
  3. **NF-1**（`planScanPartitionBearingFileWithNoIdentityValuesEmitsNoColumnsFromPath`）：bucket-only spec（T03 Bug2 形）→ `isPartitionBearing()==true` + 空 `getPartitionValues()` + 无 columns-from-path，端到端走 `buildRange:356-372`。
  4. **G1**（`convertDeletePositionDeleteTreatsStoredMinusOneBoundAsUnset`）：**存储的 -1L** position bound（哨兵）vs 缺 map 区分（`readPositionBound:483` `value==-1L→null`，旧测走 null-map 早返不触此 arm）。
  5. **MVCC-1**（`beginQuerySnapshotPinsLatestSchemaAfterSchemaOnlyAlter`@`IcebergConnectorMetadataMvccTest`）：schema-only ALTER 后 `beginQuerySnapshot` 钉 `table.schema().schemaId()`〔latest〕非 `currentSnapshot().schemaId()`〔滞后〕（旧测两 id 重合不可见）。
  6. **MVCC-2**（`resolveTimestampDatetimeStringResolvesSnapshot`）：`FOR TIME AS OF '<datetime 串>'`（`isDigital==false`→`datetimeToMillis(sessionZone)`→`snapshotIdAsOfTime`）从未经 `resolveTimeTravel` 跑（旧测仅 digital epoch-millis `parseLong` 路）。**纠审计**：null session→UTC 即可，无需 ConnectorSession stub（zone 正确性已由 `IcebergTimeUtilsTest` 覆盖）。
  7. **VC-2**（`extractVendedTokenCredentialWinsOnKeyCollision`）：vended 合并键碰撞 credential-wins 序（`io.properties()` then `putAll(cred.config())`；旧 merge 测用不相交键）。
  8. **G2+E2E-1+E2E-2 合并**（`planScanCombinesPartitionPruneDeleteAndPathNormalizeOnOneRange`）：partitioned v2 oss:// 表 `WHERE p=1` 裁 p=2，幸存单 range **同时**带 归一化数据路径+raw original_file_path + columns-from-path + 归一化 position delete（旧 delete e2e 全用非分区表）。
- **驳回 2（对抗复核正确驳回）**：FIDX-1（field-id 已 value-asserted via `renamePreservesFieldIdAcrossEvolution`）；VC-1（vended whole-map replace vs merge 在生产**不可达**——REST 静态 map 设计为空）。
- **mutation-verify（Rule 9 守门）**：在 `IcebergPredicateConverter` 交换 GE↔GT → PRED-1 变红（`GT expected:<GT> but was:<GT_EQ>`），证旧 EQ-only 网格抓不到的 transposition 现被钉；已 revert 干净（产品码 0 diff）。
- **下一步 = P6.2-T11**（见顶部任务头）：deviation 汇总 + validation gate 核对 + P6.2 设计文档/HANDOFF 收尾。
...
- **工作分支 = `catalog-spi-10-iceberg`**（P6.1 起步前 HEAD = `e5959e1b53d` #64655 P3b）。branch off `branch-catalog-spi`，PR 时 squash 合并。**所有 commit 均未 push。**
  ...
  - **已 commit（P6.2-T07 = 本 HANDOFF 前一 commit）**：`31b51b6d4d1`——MVCC/time-travel：`IcebergConnector.getCapabilities`（MVCC_SNAPSHOT+TIME_TRAVEL）+ `IcebergTableHandle` typed pin 载体 + `IcebergConnectorMetadata.{beginQuerySnapshot,resolveTimeTravel〔5 kinds〕,applySnapshot,getTableSchema(@snapshot)}` + `IcebergScanPlanProvider.buildScan` useRef/useSnapshot（谓词用 current schema=legacy parity）+ Option-A 全-pinned-schema 字典 + 新 `IcebergTimeUtils`（TZ alias 抽取 + datetimeToMillis）+ `IcebergSchemaUtils` 3-arg encode + 5 测试件。T06 竞态 Option A（用户签字）。对抗复核 `wf_cf21d2e4-d7d` 5-dim = 0 findings（predicate-schema 自查先修）。UT 205→235（1 skip）、checkstyle 0、import-gate 净、iceberg 仍不在 SPI_READY_TYPES、**无 SPI/fe-core/pom 改**。详见上「✅ P6.2-T07 = DONE」。
