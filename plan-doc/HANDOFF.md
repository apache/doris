...
# 🎯 下一个 session 的任务 — **P6.3 写路径实现：下一 = T05**（**✅ P6.3-T01 + T02 + T03 + T04 = DONE（框架统一·SPI 收口 + jdbc planWrite·config-bag 删除·EXPLAIN-保留 hook + `IcebergConnectorTransaction` 骨架·`addCommitData`·`getUpdateCnt`·`writeOperation` 枚举 + op 选择·`IcebergWriterHelper` 等价·begin* guards·baseSnapshotId 捕获）；P6.2〔T01–T11〕/ P6.1〔T01–T10〕全绿；RFC ✅ 评审通过**）
...
>
> **🎯 下一步 = P6.3-T05**（T01 ✅ + T02 ✅ + T03 ✅ + T04 ✅ DONE，见下「✅ P6.3-T04 = DONE」）。剩余 §11 TODO：~~**T04** op 选择+`IcebergWriterHelper` 等价~~ ✅ DONE → **T05** commit 校验套件+O5-2 `applyWriteConstraint`（新 `ConnectorTransaction.applyWriteConstraint(ConnectorPredicate)` default-no-op + fe-core 通用抽 target-only 合取→中性 `ConnectorPredicate` + 连接器复用 P6.2-T02 `IcebergPredicateConverter` 转 iceberg expr 暂存；commit 套件顺序 `validateFromSnapshot`(消费 T04 捕获的 `baseSnapshotId`)/合并 `conflictDetectionFilter`/serializable `validateNoConflictingDataFiles`/`validateDeletedFiles`/`validateNoConflictingDeleteFiles`/`validateDataFilesExist`，`delete_isolation_level` 默认 serializable + V3 DV `removeDeletes`〔`rewrittenDeleteFilesByReferencedDataFile`〕）→ **T06** sink 统一（删 3 planner sink，走 `visitPhysicalConnectorTableSink`；**iceberg 可复用 T02 `appendExplainInfo` hook 保留 sink-detail EXPLAIN，缩小 OQ-3 diff**）→ **T07** 通用 `RowLevelDmlCommand`+capability 派发（iceberg plan 合成留 fe-core，DV-04x）→ **T08** parity 审计+deviation 注册 → **T09** 收口 = **P6.3 DONE**。
>
> **⚠️ T05 起步必读 = T04 实现记录 + 设计 `designs/P6.3-T04-iceberg-op-selection-writerhelper-design.md`**：`IcebergConnectorTransaction.commit()` 已据 `WriteOperation` 派发建 op（无 finishWrite 钩子，op 装配收进 `commit()`）；DELETE/MERGE 的 `updateManifestAfterDelete`/`updateManifestAfterMerge` 已建 `RowDelta`（addDeletes/addRows + commit）但**故意留空校验**——T05 在两处 `transaction.newRowDelta()` 后插 `applyRowDeltaValidations(rowDelta,...)`（移植 legacy:661-673 全套 callee）+ V3 DV `removeDeletes`（`shouldRewritePreviousDeleteFiles`/`collectRewrittenDeleteFiles`/`buildDeleteFileDedupKey`/`collectReferencedDataFiles`）+ 消费已捕获的 `getBaseSnapshotId()`。O5-2 = 新 SPI `applyWriteConstraint`（连接器 override 用既有 `IcebergPredicateConverter`，暂存进 `conflictDetectionFilter` 字段，commit 时与 identity-分区 filter AND 合并）。legacy 校验套件源 = `IcebergTransaction` :655-784。
>
