...
# 🎯 下一个 session 的任务 — **P6.3 写路径实现：下一 = T06**（**✅ P6.3-T01 + T02 + T03 + T04 + T05 = DONE（框架统一·SPI 收口 + jdbc planWrite·config-bag 删除·EXPLAIN-保留 hook + `IcebergConnectorTransaction` 骨架·`addCommitData`·`getUpdateCnt`·`writeOperation` 枚举 + op 选择·`IcebergWriterHelper` 等价·begin* guards·baseSnapshotId 捕获 + commit 校验套件·O5-2 `applyWriteConstraint`〔新 `ConnectorPredicate` SPI default-no-op〕·V3 DV `removeDeletes`）；P6.2〔T01–T11〕/ P6.1〔T01–T10〕全绿；RFC ✅ 评审通过**）
...
>
> **🎯 下一步 = P6.3-T06**（T01 ✅ + T02 ✅ + T03 ✅ + T04 ✅ + T05 ✅ DONE，见下「✅ P6.3-T05 = DONE」）。剩余 §11 TODO：~~**T05** commit 校验套件+O5-2+V3 DV~~ ✅ DONE → **T06** sink 统一（连接器 `planWrite` 据 `writeOperation` 自建 `TIceberg{Table,Delete,Merge}Sink`〔同款 thrift 零 BE 改 C2；schema-json/sort/spec/row-lineage/`rewritableDeleteFileSets`/`setMaterializedColumnName` 连接器内构建；vended-creds 经 P6.2-T09 接缝〕；删 planner `Iceberg{Table,Delete,Merge}Sink` + translator `visitPhysicalIceberg*Sink`；走既有 `visitPhysicalConnectorTableSink`〔DV-009 路径〕；**复用 T02 `appendExplainInfo` hook 保留 sink-detail EXPLAIN，缩小 OQ-3 diff**；写分布/sort 经 `ConnectorCapability` C8）→ **T07** 通用 `RowLevelDmlCommand`+capability 派发（iceberg plan 合成留 fe-core DV-04x；**含从 T05 挪入的 O5-2 fe-core 生产半**：analyzed-plan 抽 target-only → `ConnectorPredicate` + 命令调 `applyWriteConstraint`，[D-061]）→ **T08** parity 审计+deviation 注册（含 DV-04x + DV-T0x 中央登记）→ **T09** 收口 = **P6.3 DONE**。
>
> **⚠️ T06 起步必读 = T02 实现记录（`appendExplainInfo` hook + `JdbcWritePlanProvider`/`MaxComputeWritePlanProvider` 模板）+ RFC §5.5（sink 统一）+ T04 实现记录（`IcebergWriteContext`：T06 的 `planWrite` 从 `ConnectorWriteHandle` 填 op-context 喂 `beginWrite`）**：T06 = 镜像 maxcompute/jdbc 的 `planWrite` 路，新 `IcebergWritePlanProvider implements ConnectorWritePlanProvider`，据 `getWriteOperation()` 自建 3 个 iceberg sink thrift（INSERT/OVERWRITE→`TIcebergTableSink`、DELETE→`TIcebergDeleteSink`、UPDATE/MERGE→`TIcebergMergeSink`），并接线 `IcebergConnector.getWritePlanProvider()` + 产品调 `beginWrite(session,db,table,IcebergWriteContext)`（从 handle 派生 op-context）；删 fe-core planner `Iceberg{Table,Delete,Merge}Sink` + `visitPhysicalIceberg*Sink`（**注意：这是本期首次动 fe-core / planner，前 T01–T05 全 0 fe-core 改**）。legacy thrift 装配源 = `IcebergTableSink`/`IcebergDeleteSink`/`IcebergMergeSink` + translator。
>
