...
**⭐ 下一步（下个 session）：**
1. **复核修复续批**（跟踪表）：**批次 1 进行中**——全 5 条已一轮 recon+对抗红队（`wf_40498e52-19f`，机制全 HEAD 确认、无 UNSOUND，设计存 `plan-doc/tasks/designs/FIX-M*-design.md` + scratch `batch1/`）。**M5 DONE**（`84f580c9075`：iceberg 表级行数恢复 equality-delete 护栏，对齐当前旧版；**推翻先前签字的「不 gate」决定，用户 2026-07-11 已签字**；3 处 P6.6-FIX-H4 文档已批注 SUPERSEDED）。**续做 M7→M6→M4→M2**（红队要点见跟踪表滚动区：M7 仅注释措辞、M6 companion 升必做、M4 最干净、**M2 非照抄 MaxCompute 须额外 override `planScanForPartitionBatch`**）→ 批次 2（M3/M1 fe-core 通用节点）→ ...。⏸ 决策类（L2/L10/L12/L20）先问用户。
