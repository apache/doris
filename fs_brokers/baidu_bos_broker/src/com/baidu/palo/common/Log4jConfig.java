...
import com.baidu.palo.broker.bos.BrokerConfig;
...
public class Log4jConfig extends XmlConfiguration {
    ...
    private static String xmlConfTemplate = ""
            ...
            + "<Configuration status='debug' packages='com.baidu.bdg'>"
            + "<Appenders>"
            + "<RollingFile name='Sys' fileName='${sys_log_dir}/baidu_bos_broker.log' "
            + "filePattern='${sys_log_dir}/baidu_bos_broker.log.${sys_file_pattern}'>"
            + "<PatternLayout>"
            ...
            + "</RollingFile>"
            + "<RollingFile name='SysWF' fileName='${sys_log_dir}/baidu_bos_broker.warn.log' "
            + "filePattern='${sys_log_dir}/baidu_bos_broker.warn.log.${sys_file_pattern}'>"
            + "<PatternLayout>"
            + "<Pattern>%d{yyyy-MM-dd HH:mm:ss,SSS} %p %tid [%C{1}.%M():%L] %m%n</Pattern>"
            + "</PatternLayout>"
            + "<Policies>"
            ...
            + "<DefaultRolloverStrategy max='${sys_roll_num}' fileIndex='min'/>"
            + "</RollingFile>"
            + "<RollingFile name='Auditfile' fileName='${audit_log_dir}/baidu_bos_broker.audit.log' "
            + "filePattern='${audit_log_dir}/baidu_bos_broker.audit.log.${audit_file_pattern}'>"
            + "<PatternLayout>"
            ...
            + "<Root level='${sys_log_level}'>" + "<AppenderRef ref='Sys'/>"
            + "<AppenderRef ref='SysWF' level='WARN'/>" + "</Root>"
            + "<Logger name='audit' level='ERROR' additivity='false'>"
            + "<AppenderRef ref='Auditfile'/>" + "</Logger>"
    ...
    private static void reconfig() throws IOException {
        ...
        String sysLogDir = BrokerConfig.sys_log_dir;
        String sysRollNum = String.valueOf(BrokerConfig.sys_log_roll_num);
        ...
        String sysrollMode = BrokerConfig.sys_log_roll_mode;
        if (sysrollMode.equals("TIME-HOUR")) {
            ...
            sysRollMaxSize = String.valueOf(Integer.parseInt(sysrollMode
                    .replaceAll("SIZE-MB-", "")));
            sysFilePattern = "%i";
        ...
        String auditLogDir = BrokerConfig.audit_log_dir;
        String auditRollNum = String.valueOf(BrokerConfig.audit_log_roll_num);

        String auditFilePattern = "";
        String auditRollMaxSize = "1000000000"; // default ~= 1PB
        String auditRollMode = BrokerConfig.audit_log_roll_mode;
        if (auditRollMode.equals("TIME-HOUR")) {
            ...
            auditRollMaxSize = String.valueOf(Integer.parseInt(auditRollMode
                    .replaceAll("SIZE-MB-", "")));
            auditFilePattern = "%i";
            newXmlConfTemplate = newXmlConfTemplate.replaceAll(
                    "<TimeBasedTriggeringPolicy /><!--AUDITLOG-->", "");
        ...
        ByteArrayInputStream bis = new ByteArrayInputStream(
                newXmlConfTemplate.getBytes("UTF-8"));
    ...
    public synchronized static void initLogging() throws IOException {
        sysLogLevel = BrokerConfig.sys_log_level;
        verboseModules = BrokerConfig.sys_log_verbose_modules;
        auditModules = BrokerConfig.audit_log_modules;
        reconfig();
    }

    public synchronized static Tuple<String, String[], String[]> updateLogging(
            String level, String[] verboseNames, String[] auditNames)
            throws IOException {
