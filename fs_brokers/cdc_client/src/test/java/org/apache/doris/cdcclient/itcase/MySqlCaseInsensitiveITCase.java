 ...
 *
 * <p>MySQL folds every table name to lowercase in this mode, so the user's DDL casing is
 * irrelevant — the stored name and Debezium both return {@code t_users} even when the DDL
 * says {@code T_Users}. The include_tables and all other config must use lowercase to match.
 */
@Testcontainers
class MySqlCaseInsensitiveITCase {
    ...
    void setUp() throws Exception {
        ...
        try (Connection conn = rootConnection("");
            ...
            // DDL uses uppercase, but MySQL folds to lowercase under lower_case_table_names=1.
            st.execute(
    ...
    void readsSnapshotThenBinlogWithLowercaseConfig() throws Exception {
        // include_tables must use the MySQL-stored (lowercase) name.
