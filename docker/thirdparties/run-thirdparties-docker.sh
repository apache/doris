    ...
    create_kafka_topics() {
       ...
       local backup_dir=/home/work/pipline/backup_center
        ...
        for topic in "${topics[@]}"; do
            echo "docker exec "${container_id}" bash -c echo '/opt/kafka/bin/kafka-topics.sh --create --broker-list '${ip_host}:19193' --topic '${topic}'" 
            docker exec "${container_id}" bash -c "/opt/kafka/bin/kafka-topics.sh --create --broker-list '${ip_host}:19193' --topic '${topic}'"
        done
