...
| 变量 | 默认值 | 作用 |
|---|---|---|
| `HIVE_BASELINE_TARBALL_CACHE` | `custom_settings.env` 中的 `docker/thirdparties/docker-compose/hive/scripts/cache/baseline` | 下载 tarball 的本地缓存目录；缓存文件名会带上 `HIVE_BASELINE_VERSION` |
| `HIVE_BASELINE_VERSION` | `custom_settings.env` 中的 `20260415` | baseline 发布的唯一版本变量：写入 `/mnt/state/baseline.version`，并同时用于本地缓存文件名和自动拼接的 OSS tarball URL |
