...
1. 先在 `${HIVE_BASELINE_TARBALL_CACHE:-/tmp/hive-baseline-cache}/<hive_version>-baseline-<version>.tar.gz` 查找本地缓存。
...
| `HIVE_BASELINE_TARBALL_CACHE` | `/tmp/hive-baseline-cache` | 下载 tarball 的本地缓存目录；缓存文件名会带上 `HIVE_BASELINE_VERSION` |
